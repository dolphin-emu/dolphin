...
#define v_034e                                                                 \
  mem[0x034e] /* init to 0, later set to most recent value written in a_04fc   \
               */
...
#define a_0b00                                                                 \
  (mem + 0x0b00) /* array from 0x0b00-0x0bff, involves accelerator? */
...
void main() {
  ...
  for (ac1.m = 0x1000, ac0.m = 0; ac1.m > 0; ac1.m--)
    mem[ac0.m++] = 0; /* clear all vars */
...
mainloop:
  ...
  if (!(size & 0x8000)) { /* size > 0x7fff invalid */
    ...
    case 8: /* mix */
      ...
      fcn_0525(mem[0x344], mem[0x346], mem[0x347],
               0x400); /* size, addrH, addrL, dsp addr */
...
void fcn_0573(short a, short l, short v) { fcn_0561(a, l, 0x0001); }

/* a=an address in ac1.m, l=a length in ar0, v=a value? in ac0.m, f is a flag?
 * in ac0.h */
/* return is in ax0.h */
...
/* dma in, I assume */
/* size=words to transfer in ar0, addrL=low word of RAM address in ac0.l,
 * addrH=high word of RAM address in ac0.m, dspaddr=dsp address in ac1.m */
...
/* dma out, I assume */
/* size=words to transfer in ar0, addrL=low word of RAM address is ac0.l,
 * addrH=high word of RAM address in ac0.m, dspaddr=dsp address in ac1.m */
...
/* addr1=address in ar0, addr2=address in ar3, size=size of table at addr1 in
 * ac1.m, val=in ax0.l */
...
/* addr1=address in ar0 (source 4 element table?), addr2=address in ar1
 * (accelerator?), addr3=address in ar2 (destination 4 element table?) */
