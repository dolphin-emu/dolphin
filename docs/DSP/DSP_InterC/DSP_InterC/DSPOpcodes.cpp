...
bool CheckCondition(uint8 _Condition)
{
        ...
        switch (_Condition & 0xf)
        {
            ...
            case 0x3:
                    ...
                    if ((g_dsp.r[R_SR] & 0x02) || (g_dsp.r[R_SR] & 0x04) ||
(g_dsp.r[R_SR] & 0x08))
                    {
                            taken = true;
                    }
...
void (*dsp_op[])(uint16 opc) = {
    dsp_op0, dsp_op1, dsp_op2,  dsp_op3,  dsp_op4,  dsp_op5,  dsp_op6, dsp_op7,
    dsp_op8, dsp_op9, dsp_opab, dsp_opab, dsp_opcd, dsp_opcd, dsp_ope, dsp_opf,
};

void DecodeOpcode(uint16 op) { dsp_op[op >> 12](op); }
...
void dsp_opc_mulc(uint16 opc) {
  ...
  // sint64 prod = dsp_get_acc_m(sreg) * dsp_get_ax_h(treg) *
  // GetMultiplyModifier(); dsp_set_long_prod(prod);
...
void dsp_opc_mulcac(uint16 opc) {
  ...
  // sint64 Prod = (sint64)dsp_get_acc_m(sreg) * (sint64)dsp_get_acc_h(sreg) *
  // GetMultiplyModifier(); dsp_set_long_prod(Prod);
...
void dsp_opc_movr(uint16 opc) {
  ...
  OutBuffer::AddCode("ACC%i = (%s << 16) & ~0xFFFF", areg,
                     OutBuffer::GetRegName(sreg));
...
void dsp_opc_movax(uint16 opc) {
  ...
  OutBuffer::AddCode("%s = %s", OutBuffer::GetRegName(0x1c + dreg),
                     OutBuffer::GetRegName(0x18 + sreg));
  OutBuffer::AddCode("%s = %s", OutBuffer::GetRegName(0x1e + dreg),
                     OutBuffer::GetRegName(0x1a + sreg));

  // g_dsp.r[0x1c + dreg] = g_dsp.r[0x18 + sreg];
  // g_dsp.r[0x1e + dreg] = g_dsp.r[0x1a + sreg];

  OutBuffer::AddCode("%s = (%s < 0) ? 0xFFFF : 0x0000",
                     OutBuffer::GetRegName(0x10 + dreg),
                     OutBuffer::GetRegName(0x1a + sreg));
...
void dsp_opc_subr(uint16 opc) {
  ...
  OutBuffer::AddCode("ACC%i -= (sint64)((sint16)%s << 16)", areg,
                     OutBuffer::GetRegName(sreg));
  OutBuffer::AddCode("Update_SR_Register(ACC%i)", areg);
...
void dsp_opc_madd(uint16 opc) {
  ...
  // sint64 prod = dsp_get_long_prod();
  // prod += (sint64)dsp_get_ax_l(sreg) * (sint64)dsp_get_ax_h(sreg) *
  // GetMultiplyModifier(); dsp_set_long_prod(prod);
}
...
union UOpcodeShifti {
  ...
  UOpcodeShifti(uint16 _Hex) : Hex(_Hex) {}
};
...
void dsp_opc_mul(uint16 opc) {
  ...
  // uint8 sreg  = (opc >> 11) & 0x1;
  // sint64 prod = (sint64)dsp_get_ax_h(sreg) * (sint64)dsp_get_ax_l(sreg) *
  // GetMultiplyModifier();
...
void dsp_opc_mulac(uint16 opc) {
  ...
  //// math new prod
  // uint8 sreg = (opc >> 11) & 0x1;
  // sint64 prod = dsp_get_ax_l(sreg) * dsp_get_ax_h(sreg) *
  // GetMultiplyModifier(); dsp_set_long_prod(prod);
...
void dsp_opc_mulmvz(uint16 opc) {
  ...
  //// math prod
  // prod = (sint64)g_dsp.r[0x18 + sreg] * (sint64)g_dsp.r[0x1a + sreg] *
  // GetMultiplyModifier(); dsp_set_long_prod(prod);
...
void dsp_opc_mulx(uint16 opc) {
  ...
  OutBuffer::AddCode("PROD = %s * %s * MultiplyModifier",
                     (sreg == 0) ? "AX0_l" : "AX0_h",
                     (treg == 0) ? "AX1_l" : "AX1_h");
...
void dsp_op2(uint16 opc) {
  ...
  if (opc & 0x0800) {
    OutBuffer::AddCode("WriteDMEM(%s, %s)", OutBuffer::GetMemName(addr),
                       OutBuffer::GetRegName(reg));

    // srs
    // dsp_dmem_write(addr, g_dsp.r[reg]);
  } else {
    OutBuffer::AddCode("%s = ReadDMEM(%s)", OutBuffer::GetRegName(reg),
                       OutBuffer::GetMemName(addr));
