...
//
// LINK TABLE
//  The link table contains references to blocks of previously decompressed
//  data. Each link is 2 bytes long in the format:
//   CCOO OOOO
...
template <int height_scale> class image_generic {
public:
  ...
  unsigned height() const { return height_scale * data.size() / width; }
};
...
public:
  ...
  ~ft_library_wrapper() { FT_Done_FreeType(_library); }

  operator FT_Library() { return _library; }
...
static std::vector<uint8_t> yay0_compress(const std::vector<uint8_t> &input) {
  ...
  while (input_pos < input.size()) {
    ...
    for (uint16_t offset = 1; offset <= 0x1000 && offset <= input_pos;
         offset++) {
      uint16_t count = 0;
...
static std::vector<uint8_t> yay0_decompress(const std::vector<uint8_t> &input) {
  ...
  try {
    // Validate header
    if (input.at(0) != 'Y' || input.at(1) != 'a' || input.at(2) != 'y' ||
        input.at(3) != '0') {
  ...
  for (unsigned block_row = 0; block_row < height_blocks; block_row++) {
    for (unsigned block = 0; block < width_blocks; block++) {
      for (unsigned row = 0; row < 8; row++) {
        ...
        unsigned dst_row_offset =
            (block_row * width_blocks * 16) + (block * 16) + (row * 2);

        out.data[dst_row_offset] = (data[src_row_offset] & 0xC0) |
                                   (data[src_row_offset + 1] >> 2 & 0x30) |
                                   (data[src_row_offset + 2] >> 4 & 0x0C) |
                                   (data[src_row_offset + 3] >> 6);
        out.data[dst_row_offset + 1] = (data[src_row_offset + 4] & 0xC0) |
...
// Dithers the given image (in-place) using the Floyd–Steinberg dithering
// algorithm
  ...
  for (unsigned y = 0; y < height; y++) {
    for (unsigned x = 0; x < image.width; x++) {
      ...
      if (y + 1 < height) {
        ...
        if (x > 0)
          dither_add_clamp(image.data[(y + 1) * image.width + x - 1],
                           error * 3 / 16);
        if (x + 1 < image.width)
          dither_add_clamp(image.data[(y + 1) * image.width + x + 1],
                           error / 16);
...
static std::vector<uint8_t> generate_fnt(font_type type,
...
static void freetype_to_fnt_data(const std::vector<uint8_t> &font_buf,
  ...
  // Initialize fontface
  FT_Face face;
  if (FT_New_Memory_Face(library, font_buf.data(), font_buf.size(), 0, &face) !=
      0)
  ...
  for (unsigned i = 0; i < font_table_size; i++) {
    ...
    // Record width
    out_widths[i] =
        clamp(face->glyph->metrics.horiAdvance >> 6, 0, FNT_CELL_SIZE);
    ...
    const int cell_offset = cell_page * FNT_PIXMAP_WIDTH * FNT_PIXMAP_WIDTH +
...
freetype_to_fnt(const std::vector<uint8_t> &font_buf, font_type type,
                bool dither) {
...
static void usage() {
  ...
  std::cerr
      << " a = generate a windows-1252 gamecube font file from a true type font"
      << std::endl;
  std::cerr
      << " s = generate a shift jis gamecube font file from a true type font"
      << std::endl;
  std::cerr << " b = like a, but do not dither the final image" << std::endl;
  std::cerr << " t = like s, but do not dither the final image" << std::endl;
  std::cerr
      << " v = generate a bitmap showing the contents of a gamecube font file"
      << std::endl;
  std::cerr << " input and output may be files or - for stdin/stdout"
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    switch (mode_char) {
    case 'c':
      result = yay0_compress(input);
      break;
    case 'd':
      result = yay0_decompress(input);
      break;
    case 'a':
      result = freetype_to_fnt(input, font_type::windows_1252, true);
      break;
    case 's':
      result = freetype_to_fnt(input, font_type::shift_jis, true);
      break;
    case 'b':
      result = freetype_to_fnt(input, font_type::windows_1252, false);
      break;
    case 't':
      result = freetype_to_fnt(input, font_type::shift_jis, false);
      break;
    case 'v':
      result = fnt_to_bmp(input);
      break;
    default:
      usage();
      return 1;
    ...
    std::cerr << "gc-font-tool: io error: " << std::strerror(errno)
              << std::endl;
    return 1;
