...
void process_fragment(in DolphinFragmentInput frag_input, out DolphinFragmentOutput frag_output)
{
	...
	else
	{
		...
		vec2 texcoord = frag_input.tex0.z == 0.0 ? frag_input.tex0.xy : frag_input.tex0.xy / frag_input.tex0.z;
		...
		if (custom_uniforms.parallax_scale != 0.0)
		{
			...
			if(new_uv.x > 1.0 || new_uv.y > 1.0 || new_uv.x < 0.0 || new_uv.y < 0.0)
				discard;
			/*if (new_uv.x > 1.0)
				new_uv.x = new_uv.x - 1.0;
			if (new_uv.y > 1.0)
				new_uv.y = new_uv.y - 1.0;
			if (new_uv.x < 0.0)
				new_uv.x = new_uv.x + 1.0;
			if (new_uv.y < 0.0)
				new_uv.y = new_uv.y + 1.0;*/
		...
		//vec4 base_color = tex * custom_uniforms.albedo_tint;
#else
...
#ifdef HAS_ARM_TEX
		vec3 arm = texture(samp_ARM_TEX, texcoord).rgb;
		
		float ao = arm.r * custom_uniforms.ao_modifier;
		float roughness = arm.g * custom_uniforms.roughness_modifier;
		float metallic = arm.b * custom_uniforms.metallic_modifier;
#else
...
#ifdef ANSIOTROPY_ENABLED
			sum_color += direct_lighting(normal, eye, base_color.rgb, light_dir, light_color, F0, roughness, metallic, attenuation, visibility, ansiotropy, ansiotropy_direction);
#else
			sum_color += direct_lighting(normal, eye, base_color.rgb, light_dir, light_color, F0, roughness, metallic, attenuation, visibility);
#endif
		}
		if (frag_input.light_chan0_color_count == 0 && frag_input.light_chan1_color_count == 0)
		{
			float visibility = 1.0;
			float attenuation = 1.0;
			//vec3 light_dir = frag_input.position.xyz - vec3(5.0, 10.0, 0);
			vec3 light_dir = normalize(vec3(5.0, 10.0, 0));
