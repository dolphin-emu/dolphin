...
static wxString replayvertex_desc = wxTRANSLATE("Additionally records and replays the entire vertex buffer along with the opcodes. This is only needed for certain games that overwrite the vertex buffer midframe, causing corruption with Opcode Replay enabled, e.g. Metroid Prime.\nIf unsure, leave this unchecked.");
