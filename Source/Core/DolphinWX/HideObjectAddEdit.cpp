...
CHideObjectAddEdit::CHideObjectAddEdit(int _selection, wxWindow* parent, wxWindowID id,
...
void CHideObjectAddEdit::CreateGUIControls(int _selection)
{
  ...
  EditHideObjectType = new wxRadioBox(this, wxID_ANY, _("Size"), wxDefaultPosition, wxDefaultSize,
                                      wxArrayStringFor_EditHideObjectType, 8, wxRA_SPECIFY_COLS);
  ...
  EditHideObjectValue =
      new wxTextCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(225, 20));
  if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 8)
    EditHideObjectValue->SetValue(
        wxString::Format("%0*X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type),
                         (u32)tempEntries[0].value_lower));
  else if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 16)
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 8,
        (u32)((tempEntries[0].value_lower & 0xffffffff00000000) >> 32),
        (u32)((tempEntries[0].value_lower & 0xffffffff))));
  else if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 24)
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 16,
        (u32)(tempEntries[0].value_upper & 0xffffffff),
        (u32)((tempEntries[0].value_lower & 0xffffffff00000000) >> 32),
        (u32)((tempEntries[0].value_lower & 0xffffffff))));
  else if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 32)
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X%08X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 24,
        (u32)((tempEntries[0].value_upper & 0xffffffff00000000) >> 32),
        (u32)(tempEntries[0].value_upper & 0xffffffff),
        (u32)((tempEntries[0].value_lower & 0xffffffff00000000) >> 32),
        (u32)((tempEntries[0].value_lower & 0xffffffff))));
  ...
  wxString* BruteForceToolTip =
      new wxString(_("The up or down button can be used to find new codes.  While the game is "
                     "playing, find an object you want to hide and select '8bits'.  Keep clicking "
                     "the up button until the object disappears.  Now choose '16bits' and "
                     "continue clicking up until the object is hidden again.  Repeat this process "
                     "until the code is long enough to be unique.  Warning: Too short of a code "
                     "may result in other objects being hidden."));
  ...
  sgEntry->Add(EditHideObjectValueText, wxGBPosition(2, 0), wxGBSpan(1, 1),
  ...
  sEditHideObject->Add(CreateButtonSizer(wxOK | wxCANCEL), 0,
...
void CHideObjectAddEdit::ButtonSize(wxCommandEvent& event)
{
  ...
  HideObjectEngine::HideObjectType temp_type =
      (HideObjectEngine::HideObjectType)EditHideObjectType->GetSelection();
...
void CHideObjectAddEdit::ButtonUporDown(wxCommandEvent& event)
{
  ...
  if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 8)
    ...
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 8,
        (u32)((value_lower & 0xffffffff00000000) >> 32), (u32)(value_lower & 0xffffffff)));
  else if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 24)
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 16,
        (u32)(value_upper & 0xffffffff), (u32)((value_lower & 0xffffffff00000000) >> 32),
        (u32)(value_lower & 0xffffffff)));
  else if (HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) <= 32)
    EditHideObjectValue->SetValue(wxString::Format(
        "%0*X%08X%08X%08X", HideObjectEngine::GetHideObjectTypeCharLength(tempEntries[0].type) - 24,
        (u32)((value_upper & 0xffffffff00000000) >> 32), (u32)(value_upper & 0xffffffff),
        (u32)((value_lower & 0xffffffff00000000) >> 32), (u32)(value_lower & 0xffffffff)));
...
bool CHideObjectAddEdit::UpdateTempEntryData(
    std::vector<HideObjectEngine::HideObjectEntry>::iterator iterEntry)
{
  unsigned long long value_lower;
  unsigned long long value_upper;

  HideObjectEngine::HideObjectType tempType = (*iterEntry).type =
      (HideObjectEngine::HideObjectType)EditHideObjectType->GetSelection();
...
bool CHideObjectAddEdit::ParseValue(unsigned long long value,
                                    HideObjectEngine::HideObjectType tempType)
{
