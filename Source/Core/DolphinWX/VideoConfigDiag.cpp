...
static wxString pptrigger_desc = wxTRANSLATE("Determines when to apply post-processing.\nOn Swap will apply post-processing at the end of a frame. On Projection applies post-processing before the game draws 2D elements on the screen. However, this may not work with all games. On EFB Copy applies post-processing upon an EFB copy of a perspective scene. This may work for other games.\n\nIf unsure, and for best compatibility, select On Swap.");
static wxString ppshader_list_desc = wxTRANSLATE("Applies post-processing effects when the chosen trigger occurs, by default this is at the end of a frame. Post-processing is performed at the selected internal resolution, and effects are applied in the order indicated by this list.\n\nIf unsure, leave the list empty.");
static wxString ppshader_options_desc = wxTRANSLATE("Some effects offer tweakable options. This will open a dialog where you can change the values of these options.");
...
static wxString scalingshader_options_desc = wxTRANSLATE("Some effects offer tweakable options. This will open a dialog where you can change the values of these options.");
static wxString stereoshader_desc = wxTRANSLATE("Selects which shader will be used to transform the two eye images into a single image when stereoscopy is enabled.");
	...
	, vconfig(g_Config)
{
	...
	if (vconfig.backend_info.bSupportsGeometryShaders)
	{
		...
		if (vconfig.backend_info.bSupportsPostProcessing)
		{
			...
			szr_stereo->Add(choice_stereoshader);
