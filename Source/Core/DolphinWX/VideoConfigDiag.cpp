...
template <>
SettingCheckBox::BoolSetting(wxWindow *parent, const wxString &label,
                             const wxString &tooltip, bool &setting,
...
template <>
SettingRadioButton::BoolSetting(wxWindow *parent, const wxString &label,
                                const wxString &tooltip, bool &setting,
                                bool reverse, long style)
    : wxRadioButton(parent, wxID_ANY, label, wxDefaultPosition, wxDefaultSize,
                    style),
      m_setting(setting), m_reverse(reverse) {
  SetToolTip(tooltip);
  SetValue(m_setting ^ m_reverse);
...
SettingChoice::SettingChoice(wxWindow *parent, int &setting,
                             const wxString &tooltip, int num,
                             const wxString choices[], long style)
    : wxChoice(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, num,
               choices),
      m_setting(setting) {
  SetToolTip(tooltip);
  Select(m_setting);
...
void VideoConfigDiag::Event_ClickClose(wxCommandEvent &) { Close(); }
...
#if defined(_WIN32)
static wxString backend_desc = wxTRANSLATE(
    "Selects what graphics API to use internally.\nThe software renderer is "
    "extremely "
    "slow and only useful for debugging, so you'll want to use either Direct3D "
    "or "
    "OpenGL. Different games and different GPUs will behave differently on "
    "each "
...
static wxString backend_desc =
    wxTRANSLATE("Selects what graphics API to use internally.\nThe software "
                "renderer is extremely "
                "slow and only useful for debugging, so unless you have a "
                "reason to use it you'll "
                "want to select OpenGL here.\n\nIf unsure, select OpenGL.");
#endif
static wxString adapter_desc = wxTRANSLATE(
    "Selects a hardware adapter to use.\n\nIf unsure, use the first one.");
static wxString display_res_desc = wxTRANSLATE(
    "Selects the display resolution used in fullscreen mode.\nThis should "
    "always be "
    "bigger than or equal to the internal resolution. Performance impact is "
    "negligible.\n\nIf unsure, select auto.");
static wxString use_fullscreen_desc =
    wxTRANSLATE("Enable this if you want the whole screen to be used for "
                "rendering.\nIf this is disabled, a "
                "render window will be created instead.\n\nIf unsure, leave "
                "this unchecked.");
static wxString auto_window_size_desc =
    wxTRANSLATE("Automatically adjusts the window size to your internal "
                "resolution.\n\nIf unsure, "
                "leave this unchecked.");
static wxString keep_window_on_top_desc =
    wxTRANSLATE("Keep the game window on top of all other windows.\n\nIf "
                "unsure, leave this unchecked.");
static wxString hide_mouse_cursor_desc =
    wxTRANSLATE("Hides the mouse cursor if it's on top of the emulation "
                "window.\n\nIf unsure, "
                "leave this unchecked.");
static wxString render_to_main_win_desc = wxTRANSLATE(
    "Enable this if you want to use the main Dolphin window for rendering "
    "rather than "
    "a separate render window.\n\nIf unsure, leave this unchecked.");
static wxString prog_scan_desc =
    wxTRANSLATE("Enables progressive scan if supported by the emulated "
                "software.\nMost games don't "
                "care about this.\n\nIf unsure, leave this unchecked.");
static wxString ar_desc =
    wxTRANSLATE("Select what aspect ratio to use when rendering:\nAuto: Use "
                "the native aspect "
                "ratio\nForce 16:9: Mimic an analog TV with a widescreen "
                "aspect ratio.\nForce 4:3: "
                "Mimic a standard 4:3 analog TV.\nStretch to Window: Stretch "
                "the picture to the "
                "window size.\n\nIf unsure, select Auto.");
static wxString ws_hack_desc = wxTRANSLATE(
    ...
    "detrimental) if using any "
    "AR/Gecko-code widescreen patches.\n\nIf unsure, leave this unchecked.");
static wxString vsync_desc = wxTRANSLATE(
    "Wait for vertical blanks in order to reduce tearing.\nDecreases "
    "performance if "
    "emulation speed is below 100%.\n\nIf unsure, leave this unchecked.");
static wxString af_desc =
    wxTRANSLATE("Enable anisotropic filtering.\nEnhances visual quality of "
                "textures that are at oblique "
                "viewing angles.\nMight cause issues in a small number of "
                "games.\n\nIf unsure, select 1x.");
static wxString aa_desc = wxTRANSLATE(
    "Reduces the amount of aliasing caused by rasterizing 3D graphics. This "
    "smooths "
    "out jagged edges on objects.\nIncreases GPU load and sometimes causes "
    "graphical "
    "issues. SSAA is significantly more demanding than MSAA, but provides top "
    "quality "
    "geometry anti-aliasing and also applies anti-aliasing to lighting, shader "
    "effects, and textures.\n\nIf unsure, select None.");
static wxString scaled_efb_copy_desc =
    wxTRANSLATE("Greatly increases quality of textures generated using "
                "render-to-texture effects.\nRaising the "
                "internal resolution will improve the effect of this "
...
static wxString pixel_lighting_desc = wxTRANSLATE(
    ...
    "smoothing out the "
    "appearance of lit polygons and making individual triangles less "
    "noticeable.\nRarely causes "
    "slowdowns or graphical issues.\n\nIf unsure, leave this unchecked.");
static wxString fast_depth_calc_desc =
    wxTRANSLATE("Use a less accurate algorithm to calculate depth "
                "values.\nCauses issues in a few "
                "games, but can give a decent speedup depending on the game "
                "and/or your GPU.\n\nIf "
                "unsure, leave this checked.");
static wxString disable_bbox_desc =
    wxTRANSLATE("Disable the bounding box emulation.\nThis may improve the GPU "
                "performance a lot, "
                "but some games will break.\n\nIf unsure, leave this checked.");
static wxString force_filtering_desc = wxTRANSLATE(
...
static wxString borderless_fullscreen_desc =
    wxTRANSLATE("Implement fullscreen mode with a borderless window spanning "
                "the whole screen instead of using "
                "exclusive mode.\nAllows for faster transitions between "
                "fullscreen and windowed mode, but "
                "slightly increases input latency, makes movement less smooth "
                "and slightly decreases "
                "performance.\nExclusive mode is required for Nvidia 3D Vision "
                "to work in the Direct3D "
                "backend.\n\nIf unsure, leave this unchecked.");
static wxString internal_res_desc =
    wxTRANSLATE("Specifies the resolution used to render at. A high resolution "
                "greatly improves "
                "visual quality, but also greatly increases GPU load and can "
                "cause issues in "
                "certain games.\n\"Multiple of 640x528\" will result in a size "
                "slightly larger "
                "than \"Window Size\" but yield fewer issues. Generally "
                "speaking, the lower the "
                "internal resolution is, the better your performance will be. "
                "Auto (Window Size), "
                "1.5x, and 2.5x may cause issues in some games.\n\nIf unsure, "
                "select Native.");
static wxString efb_access_desc =
    wxTRANSLATE("Ignore any requests from the CPU to read from or write to the "
                "EFB.\nImproves "
                "performance in some games, but might disable some "
                "gameplay-related features or "
                "graphical effects.\n\nIf unsure, leave this unchecked.");
static wxString efb_emulate_format_changes_desc = wxTRANSLATE(
...
static wxString skip_efb_copy_to_ram_desc =
    wxTRANSLATE("Stores EFB Copies exclusively on the GPU, bypassing system "
                "memory. Causes graphical defects "
                "in a small number of games.\n\nEnabled = EFB Copies to "
                "Texture\nDisabled = EFB Copies to RAM "
                "(and Texture)\n\nIf unsure, leave this checked.");
static wxString stc_desc =
    wxTRANSLATE("The \"Safe\" setting eliminates the likelihood of the GPU "
                "missing texture updates "
                "from RAM.\nLower accuracies cause in-game text to appear "
                "garbled in certain "
                "games.\n\nIf unsure, use the rightmost value.");
static wxString wireframe_desc = wxTRANSLATE(
    "Render the scene as a wireframe.\n\nIf unsure, leave this unchecked.");
static wxString disable_fog_desc = wxTRANSLATE(
    "Makes distant objects more visible by removing fog, thus increasing the "
    "overall detail.\nDisabling fog will break some games which rely on proper "
    "fog emulation.\n\nIf unsure, leave this unchecked.");
...
static wxString log_render_time_to_file_desc = wxTRANSLATE(
    ...
    "unsure, leave this unchecked.");
static wxString show_stats_desc = wxTRANSLATE(
    "Show various rendering statistics.\n\nIf unsure, leave this unchecked.");
static wxString texfmt_desc =
    wxTRANSLATE("Modify textures to show the format they're encoded in. Needs "
                "an emulation reset "
                "in most cases.\n\nIf unsure, leave this unchecked.");
static wxString xfb_desc =
    wxTRANSLATE("Disable any XFB emulation.\nSpeeds up emulation a lot but "
                "causes heavy glitches in many games "
                "which rely on them (especially homebrew applications).\n\nIf "
                "unsure, leave this checked.");
static wxString xfb_virtual_desc =
    wxTRANSLATE("Emulate XFBs using GPU texture objects.\nFixes many games "
                "which don't work without XFB "
                "emulation while not being as slow as real XFB emulation. "
                "However, it may still fail for a lot "
                "of other games (especially homebrew applications).\n\nIf "
                "unsure, leave this checked.");
static wxString xfb_real_desc = wxTRANSLATE(
...
static wxString dump_textures_desc =
    wxTRANSLATE("Dump decoded game textures to "
...
static wxString dump_efb_desc =
    wxTRANSLATE("Dump the contents of EFB copies to User/Dump/Textures/.\n\nIf "
                "unsure, leave this unchecked.");
...
static wxString free_look_desc =
    wxTRANSLATE("This feature allows you to change the game's camera.\nMove "
                "the mouse while holding the right "
                "mouse button to pan and while holding the middle button to "
                "move.\nHold SHIFT and press one of "
                "the WASD keys to move the camera by a certain step distance "
                "(SHIFT+2 to move faster and "
                "SHIFT+1 to move slower). Press SHIFT+R to reset the camera "
                "and SHIFT+F to reset the "
                "speed.\n\nIf unsure, leave this unchecked.");
static wxString crop_desc =
    wxTRANSLATE("Crop the picture from its native aspect ratio to 4:3 or "
                "16:9.\n\nIf unsure, leave this unchecked.");
static wxString ppshader_desc =
    wxTRANSLATE("Apply a post-processing effect after finishing a frame.\n\nIf "
                "unsure, select (off).");
static wxString cache_efb_copies_desc =
    wxTRANSLATE("Slightly speeds up EFB to RAM copies by sacrificing emulation "
                "accuracy.\nIf "
                "you're experiencing any issues, try raising texture cache "
                "accuracy or disable "
                "this option.\n\nIf unsure, leave this unchecked.");
static wxString stereo_3d_desc =
    wxTRANSLATE("Selects the stereoscopic 3D mode. Stereoscopy allows you to "
                "get a better feeling "
                "of depth if you have the necessary hardware.\nSide-by-Side "
                "and Top-and-Bottom are "
                "used by most 3D TVs.\nAnaglyph is used for Red-Cyan colored "
                "glasses.\nHeavily "
                "decreases emulation speed and sometimes causes issues.\n\nIf "
                "unsure, select Off.");
static wxString stereo_depth_desc =
    wxTRANSLATE("Controls the separation distance between the virtual "
                "cameras.\nA higher value "
                "creates a stronger feeling of depth while a lower value is "
                "more comfortable.");
static wxString stereo_convergence_desc = wxTRANSLATE(
...
static wxString stereo_swap_desc =
    wxTRANSLATE("Swaps the left and right eye. Mostly useful if you want to "
                "view side-by-side "
                "cross-eyed.\n\nIf unsure, leave this unchecked.");
...
VideoConfigDiag::VideoConfigDiag(wxWindow *parent, const std::string &title)
    ...
    // - basic
    {
      ...
      // backend
      {
        ...
        for (const auto &backend : g_available_video_backends) {
          choice_backend->AppendString(
              wxGetTranslation(StrToWxStr(backend->GetDisplayName())));
        }

        choice_backend->SetStringSelection(
            wxGetTranslation(StrToWxStr(g_video_backend->GetDisplayName())));
        choice_backend->Bind(wxEVT_CHOICE, &VideoConfigDiag::Event_Backend,
                             this);
      ...
      if (vconfig.backend_info.Adapters.size()) {
        choice_adapter = CreateChoice(page_general, vconfig.iAdapter,
                                      wxGetTranslation(adapter_desc));
...

      {
        ...
        // display resolution
        {
          ...
          label_display_resolution = new wxStaticText(
              page_general, wxID_ANY, _("Fullscreen Resolution:"));
          choice_display_resolution =
              new wxChoice(page_general, wxID_ANY, wxDefaultPosition,
                           wxDefaultSize, res_list);
          RegisterControl(choice_display_resolution,
                          wxGetTranslation(display_res_desc));
          choice_display_resolution->Bind(
              wxEVT_CHOICE, &VideoConfigDiag::Event_DisplayResolution, this);

          choice_display_resolution->SetStringSelection(
              StrToWxStr(SConfig::GetInstance().strFullscreenResolution));

          szr_display->Add(label_display_resolution, 1, wxALIGN_CENTER_VERTICAL,
                           0);
        ...
        // aspect-ratio
        {
          const wxString ar_choices[] = {_("Auto"), _("Force 16:9"),
                                         _("Force 4:3"),
                                         _("Stretch to Window")};

          szr_display->Add(
              new wxStaticText(page_general, wxID_ANY, _("Aspect Ratio:")), 1,
              wxALIGN_CENTER_VERTICAL, 0);
          wxChoice *const choice_aspect = CreateChoice(
              page_general, vconfig.iAspectRatio, wxGetTranslation(ar_desc),
        ...
        {
          szr_display->Add(CreateCheckBox(page_general, _("V-Sync"),
                                          wxGetTranslation(vsync_desc),
                                          vconfig.bVSync));
      ...
      {
        szr_other->Add(CreateCheckBox(page_general, _("Show FPS"),
                                      wxGetTranslation(show_fps_desc),
                                      vconfig.bShowFPS));
        ...
        szr_other->Add(
            CreateCheckBox(page_general, _("Log Render Time to File"),
                           wxGetTranslation(log_render_time_to_file_desc),
                           vconfig.bLogRenderTimeToFile));
        szr_other->Add(
            CreateCheckBox(page_general, _("Auto adjust Window Size"),
        ...
        szr_other->Add(render_to_main_checkbox = CreateCheckBox(
                           page_general, _("Render to Main Window"),
      ...
      group_display->Add(szr_display, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM,
                         5);
  ...
  // -- ENHANCEMENTS --
  {
    ...
    // Internal resolution
    {
      const wxString efbscale_choices[] = {
          ...
          _("Native (640x528)"), _("1.5x Native (960x792)"),
          _("2x Native (1280x1056) for 720p"), _("2.5x Native (1600x1320)"),
      ...
      wxChoice *const choice_efbscale = CreateChoice(
          ...
          (vconfig.iEFBScale > 11) ? ArraySize(efbscale_choices)
                                   : ArraySize(efbscale_choices) - 1,
          efbscale_choices);
      ...
      szr_enh->Add(
          new wxStaticText(page_enh, wxID_ANY, _("Internal Resolution:")), 1,
          wxALIGN_CENTER_VERTICAL, 0);
    ...
    // AF
    {
      ...
      szr_enh->Add(
          new wxStaticText(page_enh, wxID_ANY, _("Anisotropic Filtering:")), 1,
          wxALIGN_CENTER_VERTICAL, 0);
      szr_enh->Add(CreateChoice(page_enh, vconfig.iMaxAnisotropy,
                                wxGetTranslation(af_desc), 5, af_choices));
    }
    ...
    if (vconfig.backend_info.bSupportsPostProcessing) {
      ...
      choice_ppshader->Bind(wxEVT_CHOICE, &VideoConfigDiag::Event_PPShader,
                            this);
      button_config_pp->Bind(wxEVT_BUTTON,
                             &VideoConfigDiag::Event_ConfigurePPShader, this);

      szr_enh->Add(
          new wxStaticText(page_enh, wxID_ANY, _("Post-Processing Effect:")), 1,
          wxALIGN_CENTER_VERTICAL, 0);
    ...
    szr_enh->Add(CreateCheckBox(page_enh, _("Force Texture Filtering"),
                                ...
                                vconfig.bForceFiltering));
    szr_enh->Add(CreateCheckBox(page_enh, _("Widescreen Hack"),
                                wxGetTranslation(ws_hack_desc),
                                vconfig.bWidescreenHack));
    szr_enh->Add(CreateCheckBox(page_enh, _("Disable Fog"),
                                wxGetTranslation(disable_fog_desc),
                                vconfig.bDisableFog));
    ...
    if (vconfig.backend_info.bSupportsGeometryShaders) {
      ...
      szr_stereo->Add(
          new wxStaticText(page_enh, wxID_ANY, _("Stereoscopic 3D Mode:")), 1,
          wxALIGN_CENTER_VERTICAL, 0);

      const wxString stereo_choices[] = {_("Off"), _("Side-by-Side"),
                                         _("Top-and-Bottom"), _("Anaglyph"),
                                         _("Nvidia 3D Vision")};
      wxChoice *stereo_choice = CreateChoice(
          page_enh, vconfig.iStereoMode, wxGetTranslation(stereo_3d_desc),
          vconfig.backend_info.bSupports3DVision
              ? ArraySize(stereo_choices)
              : ArraySize(stereo_choices) - 1,
          stereo_choices);
      stereo_choice->Bind(wxEVT_CHOICE, &VideoConfigDiag::Event_StereoMode,
                          this);
      szr_stereo->Add(stereo_choice);

      wxSlider *const sep_slider =
          new wxSlider(page_enh, wxID_ANY, vconfig.iStereoDepth, 0, 100,
      ...
      szr_stereo->Add(new wxStaticText(page_enh, wxID_ANY, _("Depth:")), 1,
                      wxALIGN_CENTER_VERTICAL, 0);
      szr_stereo->Add(sep_slider, 0, wxEXPAND | wxRIGHT);

      conv_slider = new wxSlider(
          page_enh, wxID_ANY, vconfig.iStereoConvergencePercentage, 0, 200,
          wxDefaultPosition, wxDefaultSize, wxSL_AUTOTICKS);
      conv_slider->ClearTicks();
      conv_slider->SetTick(100);
      conv_slider->Bind(wxEVT_SLIDER, &VideoConfigDiag::Event_StereoConvergence,
                        this);
      ...
      szr_stereo->Add(CreateCheckBox(page_enh, _("Swap Eyes"),
                                     wxGetTranslation(stereo_swap_desc),
                                     vconfig.bStereoSwapEyes));

      wxStaticBoxSizer *const group_stereo =
          new wxStaticBoxSizer(wxVERTICAL, page_enh, _("Stereoscopy"));
      group_stereo->Add(szr_stereo, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM,
                        5);
  ...
  // -- SPEED HACKS --
  {
    ...
    // - EFB hacks
    wxStaticBoxSizer *const szr_efb = new wxStaticBoxSizer(
        wxVERTICAL, page_hacks, _("Embedded Frame Buffer (EFB)"));
    ...
    szr_efb->Add(CreateCheckBox(page_hacks, _("Skip EFB Access from CPU"),
                 ...
                 0, wxBOTTOM | wxLEFT, 5);
    szr_efb->Add(
        CreateCheckBox(page_hacks, _("Ignore Format Changes"),
                       wxGetTranslation(efb_emulate_format_changes_desc),
                       vconfig.bEFBEmulateFormatChanges, true),
        0, wxBOTTOM | wxLEFT, 5);
    szr_efb->Add(CreateCheckBox(page_hacks,
                                _("Store EFB Copies to Texture Only"),
    ...
    // Texture cache
    {
      ...
      wxSlider *const stc_slider =
          new wxSlider(page_hacks, wxID_ANY, 0, 0, 2, wxDefaultPosition,
      ...
      if (vconfig.iSafeTextureCache_ColorSamples == 0)
        ...
        stc_slider->Disable(); // Using custom number of samples; TODO: Inform
                               // the user why this is
                               // disabled..

      szr_safetex->Add(new wxStaticText(page_hacks, wxID_ANY, _("Accuracy:")),
                       0, wxALL, 5);
    ...
    // - XFB
    {
      wxStaticBoxSizer *const group_xfb = new wxStaticBoxSizer(
          wxHORIZONTAL, page_hacks, _("External Frame Buffer (XFB)"));

      SettingCheckBox *disable_xfb =
          CreateCheckBox(page_hacks, _("Disable"), wxGetTranslation(xfb_desc),
                         vconfig.bUseXFB, true);
      virtual_xfb = CreateRadioButton(page_hacks, _("Virtual"),
                                      wxGetTranslation(xfb_virtual_desc),
                                      vconfig.bUseRealXFB, true, wxRB_GROUP);
      real_xfb = CreateRadioButton(page_hacks, _("Real"),
                                   wxGetTranslation(xfb_real_desc),
                                   vconfig.bUseRealXFB);
  ...
  // -- ADVANCED --
  {
    ...
    // - utility
    {
      ...
      szr_utility->Add(CreateCheckBox(page_advanced, _("Dump Textures"),
                                      ...
                                      vconfig.bDumpTextures));
      szr_utility->Add(CreateCheckBox(
          page_advanced, _("Load Custom Textures"),
          wxGetTranslation(load_hires_textures_desc), vconfig.bHiresTextures));
      cache_hires_textures =
          CreateCheckBox(page_advanced, _("Prefetch Custom Textures"),
                         wxGetTranslation(cache_hires_textures_desc),
                         vconfig.bCacheHiresTextures);
      ...
      group_utility->Add(szr_utility, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM,
                         5);
    }
    ...
    // - misc
    {
      ...
      szr_misc->Add(CreateCheckBox(page_advanced, _("Crop"),
                                   wxGetTranslation(crop_desc), vconfig.bCrop));

      // Progressive Scan
      {
        progressive_scan_checkbox = new wxCheckBox(
            page_advanced, wxID_ANY, _("Enable Progressive Scan"));
        RegisterControl(progressive_scan_checkbox,
                        wxGetTranslation(prog_scan_desc));
        progressive_scan_checkbox->Bind(
            wxEVT_CHECKBOX, &VideoConfigDiag::Event_ProgressiveScan, this);

        progressive_scan_checkbox->SetValue(
            SConfig::GetInstance().bProgressive);
        // A bit strange behavior, but this needs to stay in sync with the main
        // progressive boolean;
        // TODO: Is this still necessary?
        SConfig::GetInstance().m_SYSCONF->SetData(
            "IPL.PGS", SConfig::GetInstance().bProgressive);
      ...
      borderless_fullscreen =
          CreateCheckBox(page_advanced, _("Borderless Fullscreen"),
...
SettingCheckBox *VideoConfigDiag::CreateCheckBox(wxWindow *parent,
                                                 const wxString &label,
                                                 const wxString &description,
                                                 bool &setting, bool reverse,
                                                 long style) {
...
SettingChoice *VideoConfigDiag::CreateChoice(wxWindow *parent, int &setting,
                                             const wxString &description,
                                             int num, const wxString choices[],
                                             long style) {
  SettingChoice *const ch =
      new SettingChoice(parent, setting, wxString(), num, choices, style);
  RegisterControl(ch, description);
  return ch;
}

SettingRadioButton *
VideoConfigDiag::CreateRadioButton(wxWindow *parent, const wxString &label,
                                   const wxString &description, bool &setting,
                                   bool reverse, long style) {
  SettingRadioButton *const rb = new SettingRadioButton(
      parent, label, wxString(), setting, reverse, style);
  RegisterControl(rb, description);
  return rb;
}

/* Use this to register descriptions for controls which have NOT been created
 * using the Create*
 * functions from above */
wxControl *VideoConfigDiag::RegisterControl(wxControl *const control,
                                            const wxString &description) {
...
void VideoConfigDiag::Evt_EnterControl(wxMouseEvent &ev) {
  ...
  // look up description text object from the control's parent (which is the
  // wxPanel of the current
  // tab)
  wxStaticText *descr_text = desc_texts[ctrl->GetParent()];
  if (!descr_text)
    return;

  // look up the description of the selected control and assign it to the
...
#define DEFAULT_DESC_TEXT                                                      \
  _("Move the mouse pointer over an option to display a detailed "             \
    "description.\n\n\n\n\n\n\n")
...
void VideoConfigDiag::CreateDescriptionArea(wxPanel *const page,
                                            wxBoxSizer *const sizer) {
  // Create description frame
  wxStaticBoxSizer *const desc_sizer =
      new wxStaticBoxSizer(wxVERTICAL, page, _("Description"));
  sizer->Add(desc_sizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);

  // Need to call SetSizerAndFit here, since we don't want the description texts
  // to change the
  // dialog width
  ...
  wxStaticText *const desc_text =
      new wxStaticText(page, wxID_ANY, DEFAULT_DESC_TEXT);
...
void VideoConfigDiag::PopulateAAList() {
  ...
  for (int mode : aa_modes) {
    if (mode == 1) {
      ...
      _assert_msg_(VIDEO, !supports_ssaa || m_msaa_modes == 0,
                   "SSAA setting won't work correctly");
    } else {
  ...
  auto index =
      std::find(aa_modes.begin(), aa_modes.end(), vconfig.iMultisamples);
