...
CARCodeAddEdit::CARCodeAddEdit(int _selection,
                               std::vector<ActionReplay::ARCode> *_arCodes,
                               wxWindow *parent, wxWindowID id,
                               const wxString &title, const wxPoint &position,
                               const wxSize &size, long style)
    : wxDialog(parent, id, title, position, size, style), arCodes(_arCodes),
  ...
  wxStaticBoxSizer *sbEntry =
      new wxStaticBoxSizer(wxVERTICAL, this, _("Cheat Code"));
  wxGridBagSizer *sgEntry = new wxGridBagSizer(0, 0);

  wxStaticText *EditCheatNameText =
      new wxStaticText(this, wxID_ANY, _("Name:"));
  wxStaticText *EditCheatCodeText =
      new wxStaticText(this, wxID_ANY, _("Code:"));
  ...
  sgEntry->Add(EditCheatNameText, wxGBPosition(0, 0), wxGBSpan(1, 1),
               wxALIGN_CENTER | wxALL, 5);
  sgEntry->Add(EditCheatCodeText, wxGBPosition(1, 0), wxGBSpan(1, 1),
               wxALIGN_CENTER | wxALL, 5);
  sgEntry->Add(EditCheatName, wxGBPosition(0, 1), wxGBSpan(1, 1),
               wxEXPAND | wxALL, 5);
  sgEntry->Add(EntrySelection, wxGBPosition(0, 2), wxGBSpan(2, 1),
               wxEXPAND | wxALL, 5);
  sgEntry->Add(EditCheatCode, wxGBPosition(1, 1), wxGBSpan(1, 1),
               wxEXPAND | wxALL, 5);
...
void CARCodeAddEdit::ChangeEntry(wxSpinEvent &event) {
  ActionReplay::ARCode currentCode =
      arCodes->at((int)arCodes->size() - event.GetPosition());
...
void CARCodeAddEdit::SaveCheatData(wxCommandEvent &WXUNUSED(event)) {
  ...
  for (size_t i = 0; i < userInputLines.size(); i++) {
    ...
    if (pieces.size() == 2 && pieces[0].size() == 8 && pieces[1].size() == 8) {
      ...
      if (pieces.size() == 3 && pieces[0].size() == 4 &&
          pieces[1].size() == 4 && pieces[2].size() == 5) {
  ...
  if (encryptedLines.size()) {
    // TODO: what if both decrypted AND encrypted lines are entered into a
    // single AR code?
    ActionReplay::DecryptARCode(encryptedLines, decryptedLines);
  }

  // Codes with no lines appear to be deleted/hidden from the list.  Let's
  // prevent that.
  if (!decryptedLines.size()) {
    WxUtils::ShowErrorDialog(
        _("The resulting decrypted AR code doesn't contain any lines."));
    return;
  }
...
void CARCodeAddEdit::UpdateTextCtrl(ActionReplay::ARCode arCode) {
  ...
  if (arCode.name != "") {
    for (auto &op : arCode.ops)
      EditCheatCode->AppendText(
          wxString::Format("%08X %08X\n", op.cmd_addr, op.value));
  }
}
