...
static wxButton* CreateButtonWithMinSize(wxWindow* parent, wxWindowID id, const wxString& label, wxSize size)
{
	wxButton* configure_btn = new wxButton(parent, id, label);
	...
	// NOTE: This works because fonts are inherited from the parent window object immediately.
	wxSize best_size = configure_btn->GetBestSize();
...
wxStaticBoxSizer* ControllerConfigDiag::CreateGamecubeSizer()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		gamecube_configure_bt[i] = CreateButtonWithMinSize(this, button_id, _("Configure"), wxSize(100, 25));
	...
	gamecube_static_sizer->Add(gamecube_flex_sizer, 1, wxEXPAND | wxBOTTOM, 5);
...
wxStaticBoxSizer* ControllerConfigDiag::CreateWiimoteConfigSizer()
{
	...
	for (unsigned int i = 0; i < MAX_WIIMOTES; ++i)
	{
		...
		wiimote_configure_bt[i] = CreateButtonWithMinSize(this, config_bt_id, _("Configure"), wxSize(100, 25));
...
wxStaticBoxSizer* ControllerConfigDiag::CreateBalanceBoardSizer()
{
	...
	bb_group->Add(bb_source, 1, wxALIGN_CENTER_VERTICAL);
...
wxStaticBoxSizer* ControllerConfigDiag::CreateGeneralWiimoteSettingsSizer()
{
	...
	wxBoxSizer* const sens_bar_pos_sizer = new wxBoxSizer(wxHORIZONTAL);
	sens_bar_pos_sizer->Add(WiiSensBarPos, 1, wxALIGN_CENTER_VERTICAL);
	...
	choice_sizer->Add(sens_bar_pos_sizer, 0, wxEXPAND);
	...
	general_sizer->Add(wiimote_speaker, 0, wxTOP, 5);
