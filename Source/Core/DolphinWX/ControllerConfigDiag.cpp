...
ControllerConfigDiag::ControllerConfigDiag(wxWindow *const parent)
  ...
  m_gc_pad_type_strs = {{_("None"), _("Standard Controller"),
                         _("GameCube Adapter for Wii U"), _("Steering Wheel"),
                         _("Dance Mat"), _("DK Bongos"), _("GBA"),
  ...
  main_sizer->Add(CreateButtonSizer(wxOK | wxCANCEL), 0,
...
wxStaticBoxSizer *ControllerConfigDiag::CreateGamecubeSizer() {
  ...
  for (int i = 0; i < 4; i++) {
    pad_labels[i] =
        new wxStaticText(this, wxID_ANY, wxString::Format(_("Port %i"), i + 1));
    ...
    gamecube_configure_bt[i]->Bind(
        wxEVT_BUTTON, &ControllerConfigDiag::OnGameCubeConfigButton, this);
    ...
    if (i == 0)
      pad_type_choices[i] =
          new wxChoice(this, choice_id, wxDefaultPosition, wxDefaultSize,
                       m_gc_pad_type_strs.size(), m_gc_pad_type_strs.data());
    else
      pad_type_choices[i] = new wxChoice(
          this, choice_id, wxDefaultPosition, wxDefaultSize,
          m_gc_pad_type_strs.size() - 1, m_gc_pad_type_strs.data());

    pad_type_choices[i]->Bind(
        wxEVT_CHOICE, &ControllerConfigDiag::OnGameCubePortChanged, this);
...
wxStaticBoxSizer *ControllerConfigDiag::CreateWiimoteConfigSizer() {
  ...
  for (unsigned int i = 0; i < MAX_WIIMOTES; ++i) {
    ...
    static const std::array<wxString, 4> src_choices = {
        {_("None"), _("Emulated Wiimote"), _("Real Wiimote"),
         _("Hybrid Wiimote")}};
    ...
    wiimote_source_ch[i] =
        new wxChoice(this, source_ctrl_id, wxDefaultPosition, wxDefaultSize,
                     src_choices.size(), src_choices.data());
    wiimote_source_ch[i]->Bind(wxEVT_CHOICE,
                               &ControllerConfigDiag::SelectSource, this);
    wiimote_configure_bt[i] = new wxButton(this, config_bt_id, _("Configure"),
                                           wxDefaultPosition, wxSize(80, 25));
    wiimote_configure_bt[i]->Bind(
        wxEVT_BUTTON, &ControllerConfigDiag::ConfigEmulatedWiimote, this);

    // Disable controller type selection for certain circumstances.
    bool wii_game_started = SConfig::GetInstance().bWii ||
  ...
  // "Wiimotes" layout
  wxStaticBoxSizer *const wiimote_group =
      new wxStaticBoxSizer(wxVERTICAL, this, _("Wiimotes"));
  ...
  wiimote_group->AddSpacer(5);
  wiimote_group->Add(CreateRealWiimoteSizer(), 0,
...
wxStaticBoxSizer *ControllerConfigDiag::CreateBalanceBoardSizer() {
  wxStaticBoxSizer *const bb_group =
      new wxStaticBoxSizer(wxHORIZONTAL, this, _("Balance Board"));
  ...
  wxChoice *const bb_source =
      new wxChoice(this, source_ctrl_id, wxDefaultPosition, wxDefaultSize,
                   src_choices.size(), src_choices.data());
  bb_source->Bind(wxEVT_CHOICE, &ControllerConfigDiag::SelectSource, this);

  m_orig_wiimote_sources[WIIMOTE_BALANCE_BOARD] =
      g_wiimote_sources[WIIMOTE_BALANCE_BOARD];
...
wxStaticBoxSizer *ControllerConfigDiag::CreateRealWiimoteSizer() {
  ...
  refresh_btn->Bind(wxEVT_BUTTON, &ControllerConfigDiag::RefreshRealWiimotes,
                    this);
  ...
  wxCheckBox *const continuous_scanning =
      new wxCheckBox(this, wxID_ANY, _("Continuous Scanning"));
  continuous_scanning->Bind(wxEVT_CHECKBOX,
                            &ControllerConfigDiag::OnContinuousScanning, this);
  continuous_scanning->SetValue(
...
wxStaticBoxSizer *ControllerConfigDiag::CreateGeneralWiimoteSettingsSizer() {
  ...
  wxCheckBox *const WiimoteMotor =
      new wxCheckBox(this, wxID_ANY, _("Wiimote Motor"));

  auto wiimote_speaker =
      new wxCheckBox(this, wxID_ANY, _("Enable Speaker Data"));
  wiimote_speaker->Bind(wxEVT_CHECKBOX, &ControllerConfigDiag::OnEnableSpeaker,
                        this);
  ...
  wxStaticText *const WiiSensBarSensText =
      new wxStaticText(this, wxID_ANY, _("IR Sensitivity:"));
  wxStaticText *const WiiSensBarSensMinText =
      new wxStaticText(this, wxID_ANY, _("Min"));
  wxStaticText *const WiiSensBarSensMaxText =
      new wxStaticText(this, wxID_ANY, _("Max"));
  wxStaticText *const WiimoteSpkVolumeText =
      new wxStaticText(this, wxID_ANY, _("Speaker Volume:"));
  wxStaticText *const WiimoteSpkVolumeMinText =
      new wxStaticText(this, wxID_ANY, _("Min"));
  wxStaticText *const WiimoteSpkVolumeMaxText =
      new wxStaticText(this, wxID_ANY, _("Max"));
  ...
  WiiSensBarPos->SetSelection(
      SConfig::GetInstance().m_SYSCONF->GetData<u8>("BT.BAR"));
  WiiSensBarSens->SetValue(
      SConfig::GetInstance().m_SYSCONF->GetData<u32>("BT.SENS"));
  WiimoteSpkVolume->SetValue(
      SConfig::GetInstance().m_SYSCONF->GetData<u8>("BT.SPKV"));
  WiimoteMotor->SetValue(
      SConfig::GetInstance().m_SYSCONF->GetData<bool>("BT.MOT"));

  WiiSensBarPos->Bind(wxEVT_CHOICE, &ControllerConfigDiag::OnSensorBarPos,
                      this);
  WiiSensBarSens->Bind(wxEVT_SLIDER,
                       &ControllerConfigDiag::OnSensorBarSensitivity, this);
  WiimoteSpkVolume->Bind(wxEVT_SLIDER, &ControllerConfigDiag::OnSpeakerVolume,
                         this);
...
void ControllerConfigDiag::SelectSource(wxCommandEvent &event) {
  ...
  if (index != WIIMOTE_BALANCE_BOARD) {
    ...
    WiimoteReal::ChangeWiimoteSource(index, event.GetInt() ? WIIMOTE_SRC_REAL
                                                           : WIIMOTE_SRC_NONE);
  }
}
...
void ControllerConfigDiag::OnGameCubeConfigButton(wxCommandEvent &event) {
  ...
  if (SConfig::GetInstance().m_SIDevice[port_num] == SIDEVICE_GC_KEYBOARD) {
    InputConfigDialog m_ConfigFrame(
        this, *key_plugin, _("GameCube Controller Configuration"), port_num);
    m_ConfigFrame.ShowModal();
  } else if (SConfig::GetInstance().m_SIDevice[port_num] ==
             SIDEVICE_WIIU_ADAPTER) {
    GCAdapterConfigDiag m_ConfigFramg(
        this, _("Wii U Gamecube Controller Adapter Configuration"), port_num);
    m_ConfigFramg.ShowModal();
  } else {
    InputConfigDialog m_ConfigFrame(
        this, *pad_plugin, _("GameCube Controller Configuration"), port_num);
    m_ConfigFrame.ShowModal();
  }
