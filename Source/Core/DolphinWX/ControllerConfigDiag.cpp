...
void ControllerConfigDiag::UpdateUI()
{
  ...
  for (unsigned int i = 0; i < MAX_WIIMOTES; ++i)
  {
    ...
    m_wiimote_conf_bt[i]->Enable(!enable_bt_passthrough_mode);

    m_orig_wiimote_sources[i] = g_wiimote_sources[i];
    m_wiimote_sources[i]->Select(m_orig_wiimote_sources[i]);
...
wxStaticBoxSizer* ControllerConfigDiag::CreateWiimoteConfigSizer()
{
  ...
  m_passthrough_bt_radio->Bind(wxEVT_RADIOBUTTON, &ControllerConfigDiag::OnBluetoothModeChanged,
                               this);
  box->Add(m_passthrough_bt_radio, 0, wxLEFT | wxRIGHT | wxTOP, 5);
  box->Add(CreatePassthroughBTConfigSizer(), 0, wxALL | wxEXPAND, 5);

  box->AddSpacer(10);

  m_emulated_bt_radio = new wxRadioButton(this, wxID_ANY, _("Emulate a Bluetooth adapter"));
  m_emulated_bt_radio->Bind(wxEVT_RADIOBUTTON, &ControllerConfigDiag::OnBluetoothModeChanged, this);

  box->Add(m_emulated_bt_radio, 0, wxALL, 5);
...
wxBoxSizer* ControllerConfigDiag::CreatePassthroughBTConfigSizer()
{
  ...
  m_passthrough_sync_btn =
      new wxButton(this, wxID_ANY, _("Sync"), wxDefaultPosition, wxSize(100, 30));
  ...
  m_passthrough_reset_btn =
      new wxButton(this, wxID_ANY, _("Reset"), wxDefaultPosition, wxSize(100, 30));
...
wxBoxSizer* ControllerConfigDiag::CreateEmulatedBTConfigSizer()
{
  ...
  for (unsigned int i = 0; i < MAX_WIIMOTES; ++i)
  {
    ...
    m_wiimote_sources[i]->Bind(wxEVT_CHOICE, &ControllerConfigDiag::SelectSource, this);

    m_wiimote_conf_bt[i] =
        new wxButton(this, config_bt_id, _("Configure"), wxDefaultPosition, wxSize(100, 25));
    m_wiimote_conf_bt[i]->Bind(wxEVT_BUTTON, &ControllerConfigDiag::ConfigEmulatedWiimote, this);

    grid->Add(m_wiimote_labels[i], 0, wxALIGN_CENTER_VERTICAL);
    grid->Add(m_wiimote_sources[i], 0, wxALIGN_CENTER_VERTICAL | wxEXPAND);
    grid->Add(m_wiimote_conf_bt[i], 1, wxEXPAND);
  }
  ...
  m_refresh_wm_button =
      new wxButton(this, wxID_ANY, _("Refresh"), wxDefaultPosition, wxSize(100, 30));
  m_refresh_wm_button->Bind(wxEVT_BUTTON, &ControllerConfigDiag::RefreshRealWiimotes, this);
  ...
  sizer->Add(scanning_sizer, 0, wxEXPAND, 5);
  ...
  sizer->Add(m_balance_board_checkbox, 0, wxEXPAND);
  sizer->AddSpacer(5);
