    ...
    : wxFrame(parent, wxID_ANY, _("Dolphin NetPlay Setup")), m_game_list(game_list)
{
...

  {
    ...
    temp.clear();
    netplay_section.Get("ConnectPort", &temp, "2626");
    m_connect_port_text->SetValue(StrToWxStr(temp));

    temp.clear();
    netplay_section.Get("HostPort", &temp, "2626");
    ...
    m_traversal_string = _("Traversal Server:") + " " + centralServer + ":" + centralPort;
...
wxNotebook* NetPlaySetupFrame::CreateNotebookGUI(wxWindow* parent)
{
  ...
  // connect tab
  {
    ...
    m_connect_port_text = new wxTextCtrl(connect_tab, wxID_ANY, "2626");
  ...
  // host tab
  {
    ...
    m_host_port_text = new wxTextCtrl(host_tab, wxID_ANY, "2626");
...
void NetPlaySetupFrame::OnResetTraversal(wxCommandEvent& event)
{
  ...
  m_traversal_string = _("Traversal Server: ") + "stun.dolphin-emu.org:6262";
  m_traversal_lbl->SetLabelText(m_traversal_string);
}
