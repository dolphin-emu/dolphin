...
wxSizer* HotkeyInputConfigDialog::CreateMainSizer()
{
  ...
  main_sizer->Add(b_szr, 0, wxEXPAND | wxLEFT | wxRIGHT, space5);
  main_sizer->AddSpacer(space5);
...
void HotkeyInputConfigDialog::InitializeNotebook()
{
  ...
  m_notebook->AddPage(CreateVRPanel(), _("VR"));
...
wxPanel* HotkeyInputConfigDialog::CreateVRPanel()
{
  const int space5 = FromDIP(5);
  // VR
  auto* const tab_vr = new wxPanel(m_notebook);
