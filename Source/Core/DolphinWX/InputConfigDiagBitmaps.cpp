...
void InputConfigDialog::UpdateBitmaps(wxTimerEvent& WXUNUSED(event))
{
	...
	for (ControlGroupBox* ctrlBox : current_page->control_groups)
	{
		// if this control group has a bitmap
		if (ctrlBox->static_bitmap)
		{
			wxMemoryDC dc;
			wxBitmap bitmap(ctrlBox->static_bitmap->GetBitmap());
			...
			if (bitmap.GetHeight() == 64)
				dc.DrawText(StrToWxStr(ctrlBox->control_group->name).Upper(), 4, 2);

			switch (ctrlBox->control_group->type)
			{
			...
			case GROUP_TYPE_CURSOR:
				{
					...
					{
					case GROUP_TYPE_STICK:
						((ControllerEmu::AnalogStick*)ctrlBox->control_group)->GetState(&x, &y, 32.0, 32-1.5);
						break;
					case GROUP_TYPE_TILT:
						((ControllerEmu::Tilt*)ctrlBox->control_group)->GetState(&x, &y, 32.0, 32-1.5);
						break;
					case GROUP_TYPE_CURSOR:
						((ControllerEmu::Cursor*)ctrlBox->control_group)->GetState(&x, &y, &z);
						x *= (32-1.5); x+= 32;
						y *= (32-1.5); y+= 32;
						break;
					}

					xx = ctrlBox->control_group->controls[3]->control_ref->State();
					xx -= ctrlBox->control_group->controls[2]->control_ref->State();
					yy = ctrlBox->control_group->controls[1]->control_ref->State();
					yy -= ctrlBox->control_group->controls[0]->control_ref->State();
					xx *= 32 - 1; xx += 32;
					yy *= 32 - 1; yy += 32;
					...
					if (GROUP_TYPE_CURSOR == ctrlBox->control_group->type)
					{
						if (z)
						{
							dc.SetPen(*wxRED_PEN);
					...
					if (GROUP_TYPE_STICK == ctrlBox->control_group->type)
					{
						...
						float diagonal;
						float box = 64;
						float d_of = box / 256.0;
						float x_of = box / 2.0;

						if (strcmp(ctrlBox->control_group->name, "Main Stick") == 0)
						{
							max = (87.0f / 127.0f) * 100;
							diagonal = (55.0f / 127.0f) * 100.0;
						}
						else if (strcmp(ctrlBox->control_group->name, "C-Stick") == 0)
						{
					...
					if (GROUP_TYPE_CURSOR != ctrlBox->control_group->type)
					{
						...
						dc.DrawCircle(32, 32, (ctrlBox->control_group)->settings[SETTING_DEADZONE]->value * 32);
					}
			...
			case GROUP_TYPE_FORCE:
				{
					float raw_dot[3];
					float adj_dot[3];
					const float deadzone = 32 * (ctrlBox->control_group)->settings[0]->value;

					// adjusted
					((ControllerEmu::Force*)ctrlBox->control_group)->GetState(adj_dot, 32.0, 32 - 1.5);

					// raw
					for (unsigned int i=0; i<3; ++i)
					{
						raw_dot[i] = ctrlBox->control_group->controls[i * 2 + 1]->control_ref->State()
							- ctrlBox->control_group->controls[i * 2]->control_ref->State();
			...
			case GROUP_TYPE_BUTTONS :
				{
					const unsigned int button_count = ((unsigned int)ctrlBox->control_group->controls.size());
					...
					((ControllerEmu::Buttons*)ctrlBox->control_group)->GetState(&buttons, bitmasks);
					...
					for (unsigned int n = 0; n<button_count; ++n)
					{
						...
						{
							unsigned char amt = 255 - ctrlBox->control_group->controls[n]->control_ref->State() * 128;
							dc.SetBrush(wxBrush(wxColor(amt, amt, amt)));
						}
						dc.DrawRectangle(n * 12, 0, 14, 12);

						// text
						const char* const name = ctrlBox->control_group->controls[n]->name;
			...
			case GROUP_TYPE_TRIGGERS :
				{
					const unsigned int trigger_count = ((unsigned int)(ctrlBox->control_group->controls.size()));

					// draw the shit
					dc.SetPen(*wxGREY_PEN);
					ControlState deadzone = ctrlBox->control_group->settings[0]->value;

					unsigned int* const trigs = new unsigned int[trigger_count];
					((ControllerEmu::Triggers*)ctrlBox->control_group)->GetState(trigs, 64);

					for (unsigned int n = 0; n < trigger_count; ++n)
					{
						ControlState trig_r = ctrlBox->control_group->controls[n]->control_ref->State();
						...
						// text
						dc.DrawText(StrToWxStr(ctrlBox->control_group->controls[n]->name), 3, n * 12 + 1);
					}
			...
			case GROUP_TYPE_MIXED_TRIGGERS:
				{
					const unsigned int trigger_count = ((unsigned int)(ctrlBox->control_group->controls.size() / 2));

					// draw the shit
					dc.SetPen(*wxGREY_PEN);
					ControlState thresh = ctrlBox->control_group->settings[0]->value;

					for (unsigned int n = 0; n < trigger_count; ++n)
					{
						dc.SetBrush(*wxRED_BRUSH);
						ControlState trig_d = ctrlBox->control_group->controls[n]->control_ref->State();

						ControlState trig_a = (trig_d > thresh)
						    ? 1
						    : ctrlBox->control_group->controls[n + trigger_count]->control_ref->State();
						...
						// text
						dc.DrawText(StrToWxStr(ctrlBox->control_group->controls[n + trigger_count]->name), 3, n * 12 + 1);
						dc.DrawText(StrToWxStr(std::string(1, ctrlBox->control_group->controls[n]->name[0])), 64 + 3, n * 12 + 1);
					}
			...
			case GROUP_TYPE_SLIDER:
				{
					const ControlState deadzone = ctrlBox->control_group->settings[0]->value;

					ControlState state = ctrlBox->control_group->controls[1]->control_ref->State() - ctrlBox->control_group->controls[0]->control_ref->State();
					dc.SetPen(*wxGREY_PEN);
					dc.SetBrush(*wxGREY_BRUSH);
					dc.DrawRectangle(31 + state * 30, 0, 2, 14);

					((ControllerEmu::Slider*)ctrlBox->control_group)->GetState(&state, 1);
					if (state)
					{
						dc.SetPen(*wxRED_PEN);
						dc.SetBrush(*wxRED_BRUSH);
			...
			ctrlBox->static_bitmap->SetBitmap(bitmap);
		}
	}
}
