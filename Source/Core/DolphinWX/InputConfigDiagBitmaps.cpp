...
static void DrawButton(unsigned int* const bitmasks, unsigned int buttons, unsigned int n,
                       wxGraphicsContext* gc, ControlGroupBox* g, unsigned int row,
...
void InputConfigDialog::UpdateBitmaps(wxTimerEvent& WXUNUSED(event))
{
  ...
  for (ControlGroupBox* g : current_page->control_groups)
  {
    ...
    std::unique_ptr<wxGraphicsContext> gc{
        wxGraphicsRenderer::GetDefaultRenderer()->CreateContext(dc)};
    gc->DisableOffset();
