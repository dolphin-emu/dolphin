...
CConfigMain::CConfigMain(wxWindow *parent, wxWindowID id, const wxString &title,
  ...
  Bind(wxDOLPHIN_CFG_REFRESH_LIST, &CConfigMain::OnSetRefreshGameListOnClose,
       this);
...
void CConfigMain::CreateGUIControls() {
  ...
  wxPanel *const interface_pane =
      new InterfaceConfigPane(Notebook, ID_DISPLAYPAGE);
  wxPanel *const audio_pane = new AudioConfigPane(Notebook, ID_AUDIOPAGE);
  wxPanel *const gamecube_pane =
      new GameCubeConfigPane(Notebook, ID_GAMECUBEPAGE);
  ...
  wxPanel *const advanced_pane =
      new AdvancedConfigPane(Notebook, ID_ADVANCEDPAGE);
  ...
  main_sizer->Add(CreateButtonSizer(wxOK), 0,
...
void CConfigMain::OnOk(wxCommandEvent &WXUNUSED(event)) {
  ...
  // Save the config. Dolphin crashes too often to only save the settings on
  // closing
