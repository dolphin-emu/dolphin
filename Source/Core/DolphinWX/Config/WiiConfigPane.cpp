...
void WiiConfigPane::InitializeGUI()
{
  ...
  m_usb_passthrough_reset_btn = new wxButton(this, wxID_ANY, _("Reset to default"));
  m_bt_sensor_bar_pos =
  ...
  usb_passthrough_btn_sizer->Add(m_usb_passthrough_reset_btn, 0, wxALIGN_CENTER_VERTICAL);
...
void WiiConfigPane::BindEvents()
{
  ...
  m_usb_passthrough_reset_btn->Bind(wxEVT_BUTTON, &WiiConfigPane::OnUSBWhitelistResetButton, this);
...
void WiiConfigPane::OnUSBWhitelistResetButton(wxCommandEvent&)
{
  SConfig::GetInstance().ResetUSBWhitelistToDefault();
  LoadGUIValues();
}
