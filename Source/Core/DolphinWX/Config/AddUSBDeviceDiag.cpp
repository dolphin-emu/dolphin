...
AddUSBDeviceDiag::AddUSBDeviceDiag(wxWindow* const parent)
    : wxDialog(parent, wxID_ANY, _("Add a new USB device"))
{
  InitControls();

  RefreshDeviceList();
  Bind(wxEVT_TIMER, &AddUSBDeviceDiag::RefreshDeviceList, this, m_refresh_devices_timer.GetId());
  ...
  sizer->SetItemMinSize(device_list_sizer, -1, FromDIP(150));
...
AddUSBDeviceDiag::~AddUSBDeviceDiag()
{
  m_refresh_devices_timer.Stop();
}
...
void AddUSBDeviceDiag::RefreshDeviceList()
{
  ...
  const auto selection_string =
      m_inserted_devices_listbox->GetString(m_inserted_devices_listbox->GetSelection());
  m_inserted_devices_listbox->Clear();
  for (const auto& device : current_devices)
  {
    if (SConfig::GetInstance().m_usb_passthrough_devices.count(device.first))
      continue;
...
void AddUSBDeviceDiag::OnSave(wxCommandEvent&)
{
  ...
  {
    // i18n: Here, VID means Vendor ID, and PID Product ID.
    WxUtils::ShowErrorDialog(_("The entered VID or VID is invalid."));
    return;
  }
