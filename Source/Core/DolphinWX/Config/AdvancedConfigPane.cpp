...
{
  wxDateTime custom_rtc =
      wxDateTime(static_cast<time_t>(SConfig::GetInstance().m_customRTCValue)).FromUTC();
...
{
  wxDateTime custom_rtc = wxDateTime(static_cast<time_t>(temp_date + temp_time)).ToUTC();
  SConfig::GetInstance().m_customRTCValue = custom_rtc.GetTicks();
  m_custom_rtc_date_picker->SetValue(custom_rtc.FromUTC());
  m_custom_rtc_time_picker->SetValue(custom_rtc.FromUTC());
}
