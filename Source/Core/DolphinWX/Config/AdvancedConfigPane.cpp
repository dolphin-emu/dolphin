...
void AdvancedConfigPane::InitializeGUI()
{
  ...
  m_sync_gpu_checkbox = new wxCheckBox(this, wxID_ANY, _("Enable Syncronous GPU"));
...
void AdvancedConfigPane::LoadGUIValues()
{
  ...
  int gpuOCFactor =
      (int)(std::log2f(SConfig::GetInstance().fSyncGpuOverclock) * 25.f + 100.f + 0.5f);
  ...
  m_gpu_clock_override_slider->SetValue(gpuOCFactor);
...
{
  int percent = (int)(std::roundf(SConfig::GetInstance().fSyncGpuOverclock * 100.f));
