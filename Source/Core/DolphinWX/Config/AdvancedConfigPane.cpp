...
{
  SConfig::GetInstance().bEnableCustomRTC = m_custom_rtc_checkbox->IsChecked();
  m_custom_rtc_date_picker->Enable(SConfig::GetInstance().bEnableCustomRTC);
  m_custom_rtc_time_picker->Enable(SConfig::GetInstance().bEnableCustomRTC);
}

void AdvancedConfigPane::OnCustomRTCDateChanged(wxCommandEvent& event)
{
  temp_date = m_custom_rtc_date_picker->GetValue().GetTicks();
  UpdateCustomRTC(temp_date, temp_time);
}
...
{
  wxDateTime custom_rtc = wxDateTime(static_cast<time_t>(
      ConvertToFromLocalTimezone(SConfig::GetInstance().m_customRTCValue, false)));
  ...
  if (SConfig::GetInstance().bWii)
    m_custom_rtc_date_picker->SetRange(wxDateTime(1, wxDateTime::Month(0), 2000),
                                       wxDateTime(31, wxDateTime::Month(11), 2035));
  else
    m_custom_rtc_date_picker->SetRange(wxDateTime(1, wxDateTime::Month(0), 2000),
                                       wxDateTime(31, wxDateTime::Month(11), 2099));
  m_custom_rtc_date_picker->Enable(custom_rtc_enabled);
  m_custom_rtc_time_picker->Enable(custom_rtc_enabled);
}

void AdvancedConfigPane::UpdateCustomRTC(u32 temp_date, u32 temp_time)
{
  SConfig::GetInstance().m_customRTCValue =
      ConvertToFromLocalTimezone((temp_date + temp_time), true);
  m_custom_rtc_date_picker->SetValue(static_cast<time_t>(temp_date + temp_time));
  m_custom_rtc_time_picker->SetValue(static_cast<time_t>(temp_date + temp_time));
}

u32 AdvancedConfigPane::ConvertToFromLocalTimezone(u32 input_date, bool to_gmt)
{
  // wxWidgets's To/FromUTC() functions, ...well, suck. Instead, we roll our own.
  time_t tzDiff, tzDST;
  time_t inputDate = static_cast<time_t>(input_date);
  tm* gmTime = localtime(&inputDate);
  ...
  gmTime = gmtime(&inputDate);
  if (to_gmt)
  {
    tzDiff = inputDate - mktime(gmTime);
    return static_cast<u32>(inputDate + tzDiff + tzDST);
  }
  else
  {
    tzDiff = mktime(gmTime) - inputDate;
    return static_cast<u32>(inputDate + tzDiff - tzDST);
  }
}
