...
void InterfaceConfigPane::InitializeGUI() {
  ...
  // keep these in sync with the language_ids array at the beginning of this
  // file
  ...
  m_interface_lang_strings.Add(
      L"Portugu\u00EAs (Brasil)");                   // Portuguese (Brazil)
  ...
  m_confirm_stop_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Confirm on Stop"));
  m_panic_handlers_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Use Panic Handlers"));
  m_osd_messages_checkbox =
      new wxCheckBox(this, wxID_ANY, _("On-Screen Display Messages"));
  m_pause_focus_lost_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Pause on Focus Lost"));
  ...
  m_confirm_stop_checkbox->Bind(
      wxEVT_CHECKBOX, &InterfaceConfigPane::OnConfirmStopCheckBoxChanged, this);
  m_panic_handlers_checkbox->Bind(
      wxEVT_CHECKBOX, &InterfaceConfigPane::OnPanicHandlersCheckBoxChanged,
      this);
  m_osd_messages_checkbox->Bind(
      wxEVT_CHECKBOX, &InterfaceConfigPane::OnOSDMessagesCheckBoxChanged, this);
  m_pause_focus_lost_checkbox->Bind(
      wxEVT_CHECKBOX, &InterfaceConfigPane::OnPauseOnFocusLostCheckBoxChanged,
      this);
  m_interface_lang_choice->Bind(
      wxEVT_CHOICE, &InterfaceConfigPane::OnInterfaceLanguageChoiceChanged,
      this);
  m_theme_choice->Bind(wxEVT_CHOICE, &InterfaceConfigPane::OnThemeSelected,
                       this);

  m_confirm_stop_checkbox->SetToolTip(
      _("Show a confirmation box before stopping a game."));
  m_panic_handlers_checkbox->SetToolTip(
      _("Show a message box when a potentially serious error has "
        "occurred.\nDisabling this may "
        "avoid annoying and non-fatal messages, but it may result in major "
        "crashes having no "
        "explanation at all."));
  ...
  m_pause_focus_lost_checkbox->SetToolTip(
      _("Pauses the emulator when focus is taken away from the emulation "
        "window."));
  ...
  language_and_theme_grid_sizer->Add(
      new wxStaticText(this, wxID_ANY, _("Language:")), wxGBPosition(0, 0),
      wxDefaultSpan, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  language_and_theme_grid_sizer->Add(
      m_interface_lang_choice, wxGBPosition(0, 1), wxDefaultSpan, wxALL, 5);
  language_and_theme_grid_sizer->Add(
      new wxStaticText(this, wxID_ANY, _("Theme:")), wxGBPosition(1, 0),
      wxDefaultSpan, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  language_and_theme_grid_sizer->Add(m_theme_choice, wxGBPosition(1, 1),
  ...
  main_static_box_sizer->Add(language_and_theme_grid_sizer, 0, wxEXPAND | wxALL,
                             0);
...
void InterfaceConfigPane::LoadGUIValues() {
  ...
  m_pause_focus_lost_checkbox->SetValue(
      SConfig::GetInstance().m_PauseOnFocusLost);
...
void InterfaceConfigPane::LoadThemes() {
  auto sv = DoFileSearch({""}, {File::GetUserPath(D_THEMES_IDX),
  ...
  m_theme_choice->SetStringSelection(
      StrToWxStr(SConfig::GetInstance().theme_name));
}
...
void InterfaceConfigPane::OnPanicHandlersCheckBoxChanged(
    wxCommandEvent &event) {
  SConfig::GetInstance().bUsePanicHandlers =
...
void InterfaceConfigPane::OnOSDMessagesCheckBoxChanged(wxCommandEvent &event) {
  SConfig::GetInstance().bOnScreenDisplayMessages =
      m_osd_messages_checkbox->IsChecked();
}
...
void InterfaceConfigPane::OnInterfaceLanguageChoiceChanged(
  ...
  if (SConfig::GetInstance().m_InterfaceLanguage !=
    ...
    SuccessAlertT(
        "You must restart Dolphin in order for the change to take effect.");

  SConfig::GetInstance().m_InterfaceLanguage =
      language_ids[m_interface_lang_choice->GetSelection()];
}

void InterfaceConfigPane::OnPauseOnFocusLostCheckBoxChanged(
    wxCommandEvent &event) {
  SConfig::GetInstance().m_PauseOnFocusLost =
      m_pause_focus_lost_checkbox->IsChecked();
}

void InterfaceConfigPane::OnThemeSelected(wxCommandEvent &event) {
  SConfig::GetInstance().theme_name =
      WxStrToStr(m_theme_choice->GetStringSelection());
