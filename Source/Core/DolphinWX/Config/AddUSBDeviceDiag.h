...
{
public:
  USBPassthroughDeviceEntry(const std::pair<u16, u16>& pair) : m_vid(pair.first), m_pid(pair.second)
  {
  }
  const u16 m_vid;
  const u16 m_pid;
...
{
public:
  AddUSBDeviceDiag(wxWindow* const parent);
  ~AddUSBDeviceDiag();
...
private:
  ...
  void RefreshDeviceList(wxTimerEvent&) { RefreshDeviceList(); }
