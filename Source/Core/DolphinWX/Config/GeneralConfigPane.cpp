...
void GeneralConfigPane::InitializeGUI()
{
  ...
  m_gpu_determinism =
      new wxChoice(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, m_gpu_determinism_string);
  m_idle_skip_checkbox = new wxCheckBox(
...
void GeneralConfigPane::OnGPUDeterminsmChanged(wxCommandEvent& event)
{
  SConfig::GetInstance().m_GPUDeterminismMode =
      (GPUDeterminismMode)m_gpu_determinism->GetSelection();
  SConfig::GetInstance().m_strGPUDeterminismMode =
      m_gpu_determinism_string[SConfig::GetInstance().m_GPUDeterminismMode];
}
