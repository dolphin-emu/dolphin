...
void GeneralConfigPane::InitializeGUI() {
  ...
  {
    if (i == 100)
      m_throttler_array_string.Add(
          wxString::Format(_("%i%% (Normal Speed)"), i));
    else
  ...
  m_dual_core_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Enable Dual Core (speedup)"));
  m_idle_skip_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Enable Idle Skipping (speedup)"));
  m_cheats_checkbox = new wxCheckBox(this, wxID_ANY, _("Enable Cheats"));
  m_force_ntscj_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Force Console as NTSC-J"));
  m_analytics_checkbox =
      new wxCheckBox(this, wxID_ANY, _("Enable Usage Statistics Reporting"));
#ifdef __APPLE__
  m_analytics_new_id =
      new wxButton(this, wxID_ANY, _("Generate a New Statistics Identity"),
                   wxDefaultPosition, wxSize(350, 25));
#else
  m_analytics_new_id =
      new wxButton(this, wxID_ANY, _("Generate a New Statistics Identity"));
#endif
  m_throttler_choice = new wxChoice(this, wxID_ANY, wxDefaultPosition,
                                    wxDefaultSize, m_throttler_array_string);
  m_cpu_engine_radiobox = new wxRadioBox(
      this, wxID_ANY, _("CPU Emulator Engine"), wxDefaultPosition,
      wxDefaultSize, m_cpu_engine_array_string, 0, wxRA_SPECIFY_ROWS);

  m_dual_core_checkbox->SetToolTip(
      _("Splits the CPU and GPU threads so they can be run on separate "
        "cores.\nProvides major "
        "speed improvements on most modern PCs, but can cause occasional "
        "crashes/glitches."));
  m_idle_skip_checkbox->SetToolTip(
      _("Attempt to detect and skip wait-loops.\nIf unsure, leave this "
        "checked."));
  m_cheats_checkbox->SetToolTip(
      _("Enables the use of Action Replay and Gecko cheats."));
  m_force_ntscj_checkbox->SetToolTip(
      _("Forces NTSC-J mode for using the Japanese ROM font.\nIf left "
        "unchecked, Dolphin defaults "
        "to NTSC-U and automatically enables this setting when playing "
        "Japanese games."));
  m_analytics_checkbox->SetToolTip(
      _("Enables the collection and sharing of usage statistics data with the "
        "Dolphin development "
        "team. This data is used to improve the emulator and help us "
        "understand how our users "
        "interact with the system. No private data is ever collected."));
  m_analytics_new_id->SetToolTip(
      _("Usage statistics reporting uses a unique random per-machine "
        "identifier to distinguish "
        "users from one another. This button generates a new random identifier "
        "for this machine "
        "which is dissociated from the previous one."));
  m_throttler_choice->SetToolTip(
      _("Limits the emulation speed to the specified percentage.\nNote "
        "that raising or lowering the emulation speed will also raise "
        "or lower the audio pitch to prevent audio from stuttering."));

  m_dual_core_checkbox->Bind(
      wxEVT_CHECKBOX, &GeneralConfigPane::OnDualCoreCheckBoxChanged, this);
  m_idle_skip_checkbox->Bind(
      wxEVT_CHECKBOX, &GeneralConfigPane::OnIdleSkipCheckBoxChanged, this);
  m_cheats_checkbox->Bind(wxEVT_CHECKBOX,
                          &GeneralConfigPane::OnCheatCheckBoxChanged, this);
  m_force_ntscj_checkbox->Bind(
      wxEVT_CHECKBOX, &GeneralConfigPane::OnForceNTSCJCheckBoxChanged, this);
  m_analytics_checkbox->Bind(
      wxEVT_CHECKBOX, &GeneralConfigPane::OnAnalyticsCheckBoxChanged, this);
  m_analytics_new_id->Bind(
      wxEVT_BUTTON, &GeneralConfigPane::OnAnalyticsNewIdButtonClick, this);
  m_throttler_choice->Bind(wxEVT_CHOICE,
                           &GeneralConfigPane::OnThrottlerChoiceChanged, this);
  m_cpu_engine_radiobox->Bind(
      wxEVT_RADIOBOX, &GeneralConfigPane::OnCPUEngineRadioBoxChanged, this);
  ...
  throttler_sizer->Add(new wxStaticText(this, wxID_ANY, _("Speed Limit:")), 0,
                       ...
                       5);
  throttler_sizer->Add(m_throttler_choice, 0,
  ...
  wxStaticBoxSizer *const analytics_sizer = new wxStaticBoxSizer(
      wxVERTICAL, this, _("Usage Statistics Reporting Settings"));
...
void GeneralConfigPane::OnThrottlerChoiceChanged(wxCommandEvent &event) {
  if (m_throttler_choice->GetSelection() != wxNOT_FOUND)
    SConfig::GetInstance().m_EmulationSpeed =
        m_throttler_choice->GetSelection() * 0.1f;
}
...
void GeneralConfigPane::OnCPUEngineRadioBoxChanged(wxCommandEvent &event) {
  ...
  if (main_frame->g_pCodeWindow) {
    bool using_interp =
        (SConfig::GetInstance().iCPUCore == PowerPC::CORE_INTERPRETER);
...
void GeneralConfigPane::OnAnalyticsCheckBoxChanged(wxCommandEvent &event) {
  SConfig::GetInstance().m_analytics_enabled =
      m_analytics_checkbox->IsChecked();
  DolphinAnalytics::Instance()->ReloadConfig();
}

void GeneralConfigPane::OnAnalyticsNewIdButtonClick(wxCommandEvent &event) {
  DolphinAnalytics::Instance()->GenerateNewIdentity();
  wxMessageBox(_("New identity generated."), _("Identity generation"),
               wxICON_INFORMATION);
}
