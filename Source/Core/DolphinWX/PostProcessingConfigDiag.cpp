...
PostProcessingConfigDiag::PostProcessingConfigDiag(wxWindow *parent,
                                                   const std::string &shader)
    : wxDialog(parent, wxID_ANY, _("Post Processing Shader Configuration")),
      m_shader(shader) {
  // Depending on if we are running already, either use the one from the
  // videobackend
  // or generate our own.
  ...
  const PostProcessingShaderConfiguration::ConfigMap &config_map =
  ...
  for (const auto &it : config_map) {
    if (it.second.m_type == PostProcessingShaderConfiguration::
      ...
      ConfigGrouping *group = new ConfigGrouping(
          ConfigGrouping::WidgetType::TYPE_TOGGLE, it.second.m_gui_name,
          it.first, it.second.m_dependent_option, &it.second);
      m_config_map[it.first] = group;
    } else {
      ConfigGrouping *group = new ConfigGrouping(
          ConfigGrouping::WidgetType::TYPE_SLIDER, it.second.m_gui_name,
          it.first, it.second.m_dependent_option, &it.second);
      m_config_map[it.first] = group;
    }
  }
  ...
  for (const auto &it : m_config_groups) {
    if (it->HasChildren()) {
      ...
      if (!add_general_page) {
        // Make it so it doesn't show up if there aren't any options without
        // children.
  ...
  btn_close->Bind(wxEVT_BUTTON, &PostProcessingConfigDiag::Event_ClickClose,
                  this);
...
void PostProcessingConfigDiag::ConfigGrouping::GenerateUI(
    PostProcessingConfigDiag *dialog, wxWindow *parent,
  ...
  if (m_type == WidgetType::TYPE_TOGGLE) {
    ...
    m_option_checkbox->Bind(wxEVT_CHECKBOX,
                            &PostProcessingConfigDiag::Event_CheckBox, dialog,
    ...
    if (m_config_option->m_type ==
        PostProcessingShaderConfiguration::ConfigurationOption::OptionType::
            OPTION_INTEGER)
      vector_size = m_config_option->m_integer_values.size();
    else
      vector_size = m_config_option->m_float_values.size();

    wxFlexGridSizer *const szr_values =
        new wxFlexGridSizer(vector_size + 1, 0, 0);
    wxStaticText *const option_static_text =
        new wxStaticText(parent, wxID_ANY, _(m_gui_name));
    ...
    for (size_t i = 0; i < vector_size; ++i) {
      ...
      if (m_config_option->m_type ==
        ...
        double range = m_config_option->m_integer_max_values[i] -
                       m_config_option->m_integer_min_values[i];

        // How many steps we have is the range divided by the step interval
        // configured.
        // This may not be 100% spot on accurate since developers can have odd
        // stepping intervals
        // set.
        // Round up so if it is outside our range, then set it to the minimum or
        // maximum
        ...
        float range = m_config_option->m_float_max_values[i] -
                      m_config_option->m_float_min_values[i];
        steps = ceil(range / m_config_option->m_float_step_values[i]);

        // We need to convert our default float value from a float to the
        // nearest step value range
        current_value = (m_config_option->m_float_values[i] /
      ...
      // Disable the textctrl, it's only there to show the absolute value from
      // the slider
      text_ctrl->Disable();
    ...
    if (vector_size == 1) {
      szr_values->Add(m_option_sliders[0],
      ...
      for (size_t i = 0; i < vector_size; ++i) {
        szr_inside->Add(m_option_sliders[i],
                        wxSizerFlags().Align(wxALIGN_CENTER_VERTICAL));
...
void PostProcessingConfigDiag::ConfigGrouping::EnableDependentChildren(
    bool enable) {
...
void PostProcessingConfigDiag::Event_Slider(wxCommandEvent &ev) {
  ...
  size_t vector_size = 0;
  if (option_data.m_type == PostProcessingShaderConfiguration::
  ...
  for (size_t i = 0; i < vector_size; ++i) {
    ...
    if (option_data.m_type ==
        PostProcessingShaderConfiguration::ConfigurationOption::OptionType::
            OPTION_INTEGER) {
      s32 value = option_data.m_integer_step_values[i] * current_step +
                  option_data.m_integer_min_values[i];
      m_post_processor->SetOptioni(config->GetOption(), i, value);
      string_value = std::to_string(value);
    } else {
      float value = option_data.m_float_step_values[i] * current_step +
                    option_data.m_float_min_values[i];
