...
public:
  PadSettingExtension(wxWindow *const parent,
...
public:
  PadSettingSpin(wxWindow *const parent,
      ...
      : PadSetting(new wxSpinCtrl(
            parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(54, -1),
            0, _setting->low, _setting->high, (int)(_setting->value * 100))),
        setting(_setting) {}
...
public:
  ControlButton(wxWindow *const parent,
...
public:
  ControlGroupBox(ControllerEmu::ControlGroup *const group,
                  wxWindow *const parent, GamepadPage *const eventsink);
  ~ControlGroupBox();
...
public:
  InputConfigDialog(wxWindow *const parent, InputConfig &config,
                    const wxString &name, const int tab_num = 0);
