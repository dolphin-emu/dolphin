...
class CheatWarningMessage : public wxPanel
{
public:
      ...
      : wxPanel(parent), m_game_id(std::move(game_id))
  {
    ...
    m_message = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                                 wxST_NO_AUTORESIZE);
    m_enabler = new wxButton(this, wxID_ANY, _("Enable Cheats"));

    m_enabler->Bind(wxEVT_BUTTON, &CheatWarningMessage::OnCheatsEnable, this);
    ...
    sizer->Add(m_enabler, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, 10);
    sizer->AddSpacer(10);
  ...
  void UpdateState()
  {
    ...
    States new_state = States::Hidden;
    if (!SConfig::GetInstance().bEnableCheats)
      new_state = core_running ? States::RunningWithDisabledCheats : States::DisabledCheats;
    else if (core_running && SConfig::GetInstance().GetUniqueID() == m_game_id)
      new_state = States::GameRunning;
...
private:
  ...
  void OnCheatsEnable(wxCommandEvent&)
  {
    ...
    if (!sconfig.bEnableCheats)
    {
      if (Core::IsRunning())
      {
        ApplyState(States::RunningWithDisabledCheats);
        return;
      }

      sconfig.bEnableCheats = true;
      sconfig.SaveSettings();
      main_frame->UpdateGUI();  // Enable "Cheat Manager" menu item.
    }
    ApplyState(States::Hidden);
  }
  ...
  void ApplyState(States new_state)
  {
    ...
    case States::DisabledCheats:
    case States::RunningWithDisabledCheats:
      m_enabler->Show();
      m_enabler->Enable(new_state == States::DisabledCheats);
      m_message->SetLabelText(_("Dolphin's Cheat Engine is currently disabled."));
      break;
