    ...
    : wxDialog(parent, id, title, position, size, style), OpenGameListItem(game_list_item)
{
  ...
  bool wii = m_open_iso->GetVolumeType() != DiscIO::EPlatform::GAMECUBE_DISC;
  ...
  if (m_open_iso->GetVolumeType() != DiscIO::EPlatform::WII_WAD)
  {
    if (m_open_iso->GetVolumeType() == DiscIO::EPlatform::WII_DISC)
    {
...
void CISOProperties::CreateGUIControls()
{
  ...
  if (m_open_iso->GetVolumeType() == DiscIO::EPlatform::GAMECUBE_DISC)
  {
  ...
  bool wii = m_open_iso->GetVolumeType() != DiscIO::EPlatform::GAMECUBE_DISC;
  DiscIO::ELanguage preferred_language = SConfig::GetInstance().GetCurrentLanguage(wii);

  std::vector<DiscIO::ELanguage> languages = OpenGameListItem.GetLanguages();
...
{
  DiscIO::IFileSystem* const fs = m_open_iso->GetVolumeType() == DiscIO::EPlatform::WII_DISC ?
...
void CISOProperties::ChangeBannerDetails(DiscIO::ELanguage language)
{
