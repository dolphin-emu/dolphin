...
CISOProperties::CISOProperties(const GameListItem &game_list_item,
  ...
  GameIniDefault =
      SConfig::LoadDefaultGameIni(game_id, m_open_iso->GetRevision());
  ...
  if (m_open_iso->GetVolumeType() != DiscIO::IVolume::WII_WAD) {
    if (m_open_iso->GetVolumeType() == DiscIO::IVolume::WII_DISC) {
      ...
      for (int group = 0; group < 4; group++) {
        for (u32 i = 0; i < 0xFFFFFFFF; i++) // yes, technically there can be
                                             // OVER NINE THOUSAND partitions...
        {
          std::unique_ptr<DiscIO::IVolume> volume(
              DiscIO::CreateVolumeFromFilename(OpenGameListItem.GetFileName(),
                                               group, i));
          if (volume != nullptr) {
            ...
            if (file_system != nullptr) {
              ...
              wxTreeItemId PartitionRoot = m_Treectrl->AppendItem(
                  RootId, wxString::Format(_("Partition %i"), partition_count),
                  0, 0);

              m_Treectrl->SetItemData(PartitionRoot, partition);
              CreateDirectoryTree(PartitionRoot,
                                  partition->FileSystem->GetFileList());
  ...
  wxTheApp->Bind(DOLPHIN_EVT_LOCAL_INI_CHANGED,
                 &CISOProperties::OnLocalIniModified, this);
}

CISOProperties::~CISOProperties() {}

size_t CISOProperties::CreateDirectoryTree(
    wxTreeItemId &parent, const std::vector<DiscIO::SFileInfo> &fileInfos) {
  if (fileInfos.empty())
    return 0;
  else
    return CreateDirectoryTree(parent, fileInfos, 1,
                               fileInfos.at(0).m_FileSize);
}
...
size_t CISOProperties::CreateDirectoryTree(
  ...
  while (CurrentIndex < _LastIndex) {
    ...
    // Say we have "/music/stream/stream1.strm", the result will be
    // "stream1.strm".
    ...
    if (rFileInfo.IsDirectory()) {
      wxTreeItemId item =
          m_Treectrl->AppendItem(parent, StrToWxStr(filePath), 1, 1);
      CurrentIndex = CreateDirectoryTree(item, fileInfos, CurrentIndex + 1,
                                         (size_t)rFileInfo.m_FileSize);
    } else {
...
void CISOProperties::CreateGUIControls() {
  wxButton *const EditConfig =
      new wxButton(this, ID_EDITCONFIG, _("Edit Config"));
  EditConfig->SetToolTip(
      _("This will let you manually edit the INI config file."));

  wxButton *const EditConfigDefault =
      new wxButton(this, ID_SHOWDEFAULTCONFIG, _("Show Defaults"));
  EditConfigDefault->SetToolTip(_("Opens the default (read-only) configuration "
                                  "for this game in an external text editor."));
  ...
  wxStaticText *const OverrideText = new wxStaticText(
      m_GameConfig, wxID_ANY,
      _("These settings override core Dolphin settings.\nUndetermined "
        "means the game uses Dolphin's setting."));

  // Core
  CPUThread = new wxCheckBox(
      m_GameConfig, ID_USEDUALCORE, _("Enable Dual Core"), wxDefaultPosition,
      wxDefaultSize, GetElementStyle("Core", "CPUThread"));
  SkipIdle = new wxCheckBox(m_GameConfig, ID_IDLESKIP,
                            _("Enable Idle Skipping"), wxDefaultPosition,
                            wxDefaultSize, GetElementStyle("Core", "SkipIdle"));
  MMU = new wxCheckBox(m_GameConfig, ID_MMU, _("Enable MMU"), wxDefaultPosition,
                       wxDefaultSize, GetElementStyle("Core", "MMU"));
  MMU->SetToolTip(_("Enables the Memory Management Unit, needed for some "
                    "games. (ON = Compatible, OFF = Fast)"));
  DCBZOFF = new wxCheckBox(m_GameConfig, ID_DCBZOFF, _("Skip DCBZ clearing"),
                           wxDefaultPosition, wxDefaultSize,
                           GetElementStyle("Core", "DCBZ"));
  DCBZOFF->SetToolTip(_(
      "Bypass the clearing of the data cache by the DCBZ instruction. Usually "
      "leave this option disabled."));
  FPRF =
      new wxCheckBox(m_GameConfig, ID_FPRF, _("Enable FPRF"), wxDefaultPosition,
                     wxDefaultSize, GetElementStyle("Core", "FPRF"));
  FPRF->SetToolTip(_("Enables Floating Point Result Flag calculation, needed "
                     "for a few games. (ON "
                     "= Compatible, OFF = Fast)"));
  SyncGPU = new wxCheckBox(m_GameConfig, ID_SYNCGPU,
                           _("Synchronize GPU thread"), wxDefaultPosition,
                           wxDefaultSize, GetElementStyle("Core", "SyncGPU"));
  SyncGPU->SetToolTip(_(
      "Synchronizes the GPU and CPU threads to help prevent random freezes in "
      "Dual Core mode. (ON = Compatible, OFF = Fast)"));
  FastDiscSpeed =
      new wxCheckBox(m_GameConfig, ID_DISCSPEED,
                     _("Speed up Disc Transfer Rate"), wxDefaultPosition,
                     wxDefaultSize, GetElementStyle("Core", "FastDiscSpeed"));
  ...
  DSPHLE = new wxCheckBox(m_GameConfig, ID_AUDIO_DSP_HLE,
                          _("DSP HLE emulation (fast)"), wxDefaultPosition,
                          wxDefaultSize, GetElementStyle("Core", "DSPHLE"));
  ...
  GPUDeterminism =
      new wxChoice(m_GameConfig, ID_GPUDETERMINISM, wxDefaultPosition,
                   wxDefaultSize, arrayStringFor_GPUDeterminism);
  sGPUDeterminism->Add(GPUDeterminismText);
  sGPUDeterminism->Add(GPUDeterminism);

  // Wii Console
  EnableWideScreen = new wxCheckBox(
      m_GameConfig, ID_ENABLEWIDESCREEN, _("Enable WideScreen"),
  ...
  DepthPercentage->SetToolTip(_("This value is multiplied with the depth set "
                                "in the graphics configuration."));
  ...
  Convergence->SetToolTip(_("This value is added to the convergence value set "
                            "in the graphics configuration."));
  sConvergence->Add(ConvergenceText);
  sConvergence->Add(Convergence);

  MonoDepth = new wxCheckBox(
      m_GameConfig, ID_MONODEPTH, _("Monoscopic Shadows"), wxDefaultPosition,
      wxDefaultSize,
      GetElementStyle("Video_Stereoscopy", "StereoEFBMonoDepth"));
  MonoDepth->SetToolTip(
      _("Use a single depth buffer for both eyes. Needed for a few games."));
  ...
  EmuState = new wxChoice(m_GameConfig, ID_EMUSTATE, wxDefaultPosition,
                          wxDefaultSize, arrayStringFor_EmuState);
  ...
  wxStaticBoxSizer *const sbGameConfig = new wxStaticBoxSizer(
      wxVERTICAL, m_GameConfig, _("Game-Specific Settings"));
  ...
  wxButton *const AddPatch =
      new wxButton(m_PatchPage, ID_ADDPATCH, _("Add..."));
  ...
  wxButton *const AddCheat =
      new wxButton(m_CheatPage, ID_ADDCHEAT, _("Add..."));
  ...
  m_InternalName =
      new wxTextCtrl(m_Information, ID_NAME, wxEmptyString, wxDefaultPosition,
  ...
  wxStaticText *const m_DateText =
      new wxStaticText(m_Information, wxID_ANY, _("Apploader Date:"));
  ...
  m_FST = new wxTextCtrl(m_Information, ID_FST, wxEmptyString,
                         wxDefaultPosition, wxDefaultSize, wxTE_READONLY);
  wxStaticText *const m_MD5SumText =
      new wxStaticText(m_Information, wxID_ANY, _("MD5 Checksum:"));
  m_MD5Sum = new wxTextCtrl(m_Information, ID_MD5SUM, wxEmptyString,
                            wxDefaultPosition, wxDefaultSize, wxTE_READONLY);
  m_MD5SumCompute = new wxButton(m_Information, ID_MD5SUMCOMPUTE, _("Compute"));

  wxStaticText *const m_LangText =
      new wxStaticText(m_Information, wxID_ANY, _("Show Language:"));

  bool wii = m_open_iso->GetVolumeType() != DiscIO::IVolume::GAMECUBE_DISC;
  DiscIO::IVolume::ELanguage preferred_language =
      SConfig::GetInstance().GetCurrentLanguage(wii);

  std::vector<DiscIO::IVolume::ELanguage> languages =
      OpenGameListItem.GetLanguages();
  ...
  m_Lang = new wxChoice(m_Information, ID_LANG, wxDefaultPosition,
                        wxDefaultSize, arrayStringFor_Lang);
  ...
  m_Name = new wxTextCtrl(m_Information, ID_SHORTNAME, wxEmptyString,
                          wxDefaultPosition, wxDefaultSize, wxTE_READONLY);
  wxStaticText *const m_MakerText =
      new wxStaticText(m_Information, wxID_ANY, _("Maker:"));
  m_Maker = new wxTextCtrl(m_Information, ID_MAKER, wxEmptyString,
                           wxDefaultPosition, wxDefaultSize, wxTE_READONLY);
  wxStaticText *const m_CommentText =
      new wxStaticText(m_Information, wxID_ANY, _("Description:"));
  m_Comment = new wxTextCtrl(m_Information, ID_COMMENT, wxEmptyString,
                             wxDefaultPosition, wxDefaultSize,
                             wxTE_MULTILINE | wxTE_READONLY);
  wxStaticText *const m_BannerText =
      new wxStaticText(m_Information, wxID_ANY, _("Banner:"));
  m_Banner = new wxStaticBitmap(m_Information, ID_BANNER, wxNullBitmap,
  ...
  sISODetails->Add(m_InternalName, wxGBPosition(0, 1), wxGBSpan(1, 2),
                   wxEXPAND | wxALL, 5);
  sISODetails->Add(m_GameIDText, wxGBPosition(1, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_GameID, wxGBPosition(1, 1), wxGBSpan(1, 2),
                   wxEXPAND | wxALL, 5);
  sISODetails->Add(m_CountryText, wxGBPosition(2, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_Country, wxGBPosition(2, 1), wxGBSpan(1, 2),
                   wxEXPAND | wxALL, 5);
  sISODetails->Add(m_MakerIDText, wxGBPosition(3, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_MakerID, wxGBPosition(3, 1), wxGBSpan(1, 2),
                   wxEXPAND | wxALL, 5);
  sISODetails->Add(m_RevisionText, wxGBPosition(4, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_Revision, wxGBPosition(4, 1), wxGBSpan(1, 2),
                   wxEXPAND | wxALL, 5);
  sISODetails->Add(m_DateText, wxGBPosition(5, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_Date, wxGBPosition(5, 1), wxGBSpan(1, 2), wxEXPAND | wxALL,
                   5);
  sISODetails->Add(m_FSTText, wxGBPosition(6, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_FST, wxGBPosition(6, 1), wxGBSpan(1, 2), wxEXPAND | wxALL,
                   5);
  sISODetails->Add(m_MD5SumText, wxGBPosition(7, 0), wxGBSpan(1, 1),
                   wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sISODetails->Add(m_MD5Sum, wxGBPosition(7, 1), wxGBSpan(1, 1),
                   wxEXPAND | wxALL, 5);
  wxSizer *sMD5SumButtonSizer = CreateButtonSizer(wxNO_DEFAULT);
  sMD5SumButtonSizer->Add(m_MD5SumCompute);
  sISODetails->Add(sMD5SumButtonSizer, wxGBPosition(7, 2), wxGBSpan(1, 1),
                   wxEXPAND | wxALL, 5);
  ...
  sBannerDetails->Add(m_LangText, wxGBPosition(0, 0), wxGBSpan(1, 1),
                      wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sBannerDetails->Add(m_Lang, wxGBPosition(0, 1), wxGBSpan(1, 1),
                      wxEXPAND | wxALL, 5);
  sBannerDetails->Add(m_NameText, wxGBPosition(1, 0), wxGBSpan(1, 1),
                      wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sBannerDetails->Add(m_Name, wxGBPosition(1, 1), wxGBSpan(1, 1),
                      wxEXPAND | wxALL, 5);
  sBannerDetails->Add(m_MakerText, wxGBPosition(2, 0), wxGBSpan(1, 1),
                      wxALIGN_CENTER_VERTICAL | wxALL, 5);
  sBannerDetails->Add(m_Maker, wxGBPosition(2, 1), wxGBSpan(1, 1),
                      wxEXPAND | wxALL, 5);
  sBannerDetails->Add(m_CommentText, wxGBPosition(3, 0), wxGBSpan(1, 1), wxALL,
                      5);
  sBannerDetails->Add(m_Comment, wxGBPosition(3, 1), wxGBSpan(1, 1),
                      wxEXPAND | wxALL, 5);
  sBannerDetails->Add(m_BannerText, wxGBPosition(4, 0), wxGBSpan(1, 1), wxALL,
                      5);
  sBannerDetails->Add(m_Banner, wxGBPosition(4, 1), wxGBSpan(1, 1),
                      wxEXPAND | wxALL, 5);
  ...
  for (const std::string &ini_filename :
    ...
    if (File::Exists(File::GetSysDirectory() + GAMESETTINGS_DIR DIR_SEP +
                     ini_filename)) {
...
void CISOProperties::OnClose(wxCloseEvent &WXUNUSED(event)) {
  if (!SaveGameConfig())
    WxUtils::ShowErrorDialog(
        wxString::Format(_("Could not save %s."), GameIniFileLocal.c_str()));
  Destroy();
}

void CISOProperties::OnCloseClick(wxCommandEvent &WXUNUSED(event)) { Close(); }
...
void CISOProperties::OnBannerImageSave(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxFileDialog dialog(this, _("Save as..."), wxGetHomeDir(&dirHome),
                      wxString::Format("%s.png", m_GameID->GetValue().c_str()),
...
void CISOProperties::OnExtractFile(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxString Path =
      wxFileSelector(_("Export File"), wxEmptyString, File, wxEmptyString,
  ...
  while (m_Treectrl->GetItemParent(m_Treectrl->GetSelection()) !=
    ...
    wxString temp = m_Treectrl->GetItemText(
        m_Treectrl->GetItemParent(m_Treectrl->GetSelection()));
    File = temp + DIR_SEP_CHR + File;

    m_Treectrl->SelectItem(
        m_Treectrl->GetItemParent(m_Treectrl->GetSelection()));
  }

  if (m_open_iso->GetVolumeType() == DiscIO::IVolume::WII_DISC) {
    const wxTreeItemId tree_selection = m_Treectrl->GetSelection();
    WiiPartition *partition = reinterpret_cast<WiiPartition *>(
        m_Treectrl->GetItemData(tree_selection));
    File.erase(0, m_Treectrl->GetItemText(tree_selection).length() +
                      1); // Remove "Partition x/"
...
void CISOProperties::ExportDir(const std::string &_rFullPath,
                               const std::string &_rExportFolder,
                               const WiiPartition *partition) {
  DiscIO::IFileSystem *const fs =
      m_open_iso->GetVolumeType() == DiscIO::IVolume::WII_DISC
  ...
  if (_rFullPath.empty()) {
    ...
    DEBUG_LOG(DISCIO, "Directory found from %u to %u\nextracting to:\n%s",
              index, size, _rExportFolder.c_str());
  }

  wxString dialogTitle =
      (index != 0) ? _("Extracting Directory") : _("Extracting All Files");
  ...
  for (u32 i = index; i < size; i++) {
    dialog.SetTitle(wxString::Format(
        "%s : %d%%", dialogTitle.c_str(),
        (u32)(((float)(i - index) / (float)(size - index)) * 100)));

    dialog.Update(
        i, wxString::Format(_("Extracting %s"), StrToWxStr(fst[i].m_FullPath)));

    if (dialog.WasCancelled())
      break;

    if (fst[i].IsDirectory()) {
      const std::string exportName = StringFromFormat(
          "%s/%s/", _rExportFolder.c_str(), fst[i].m_FullPath.c_str());
      ...
      if (!File::Exists(exportName) && !File::CreateFullPath(exportName)) {
        ...
        if (!File::IsDirectory(exportName))
          ERROR_LOG(DISCIO, "%s already exists and is not a directory",
                    exportName.c_str());
      ...
      if (!File::Exists(exportName) &&
          !fs->ExportFile(fst[i].m_FullPath, exportName)) {
...
void CISOProperties::OnExtractDir(wxCommandEvent &event) {
  ...
  if (event.GetId() == IDM_EXTRACTALL) {
    if (m_open_iso->GetVolumeType() == DiscIO::IVolume::WII_DISC) {
      ...
      while (item.IsOk()) {
        ExportDir("", WxStrToStr(Path), reinterpret_cast<WiiPartition *>(
  ...
  if (m_open_iso->GetVolumeType() == DiscIO::IVolume::WII_DISC) {
    ...
    WiiPartition *partition = reinterpret_cast<WiiPartition *>(
        m_Treectrl->GetItemData(tree_selection));
    Directory.erase(0,
                    m_Treectrl->GetItemText(tree_selection).length() +
                        1); // Remove "Partition x/"
...
void CISOProperties::OnExtractDataFromHeader(wxCommandEvent &event) {
  ...
  if (!ret)
    WxUtils::ShowErrorDialog(wxString::Format(_("Failed to extract to %s!"),
...
void CISOProperties::CheckPartitionIntegrity(wxCommandEvent &event) {
  ...
  wxProgressDialog dialog(_("Checking integrity..."), _("Working..."), 1000,
                          this,
  ...
  if (!thread.Wait()) {
    wxMessageBox(wxString::Format(
                     _("Integrity check for %s failed. The disc image is most "
    ...
    wxMessageBox(_("Integrity check completed. No errors have been found."),
                 _("Integrity check completed"), wxOK | wxICON_INFORMATION,
                 this);
  }
}
...
void CISOProperties::SetCheckboxValueFromGameini(const char *section,
                                                 const char *key,
                                                 wxCheckBox *checkbox) {
...
void CISOProperties::LoadGameConfig() {
  ...
  SetCheckboxValueFromGameini("Video_Stereoscopy", "StereoEFBMonoDepth",
                              MonoDepth);
  ...
  IniFile::Section *default_emustate =
      GameIniDefault.GetOrCreateSection("EmuState");
  ...
  IniFile::Section *default_stereoscopy =
      GameIniDefault.GetOrCreateSection("Video_Stereoscopy");
  default_stereoscopy->Get("StereoDepthPercentage", &iTemp, 100);
  GameIniLocal.GetIfExists("Video_Stereoscopy", "StereoDepthPercentage",
                           &iTemp);
...
void CISOProperties::SaveGameIniValueFrom3StateCheckbox(const char *section,
                                                        const char *key,
                                                        wxCheckBox *checkbox) {
  // Delete any existing entries from the local gameini if checkbox is
  // undetermined.
...
bool CISOProperties::SaveGameConfig() {
  ...
  SaveGameIniValueFrom3StateCheckbox("Video_Stereoscopy", "StereoEFBMonoDepth",
                                     MonoDepth);
...
#define SAVE_IF_NOT_DEFAULT(section, key, val, def)                            \
  ...
  SAVE_IF_NOT_DEFAULT("Video", "PH_SZNear", (m_PHack_Data.PHackSZNear ? 1 : 0),
                      0);
  SAVE_IF_NOT_DEFAULT("Video", "PH_SZFar", (m_PHack_Data.PHackSZFar ? 1 : 0),
                      0);
  SAVE_IF_NOT_DEFAULT("Video", "PH_ZNear", m_PHack_Data.PHZNear, "");
  SAVE_IF_NOT_DEFAULT("Video", "PH_ZFar", m_PHack_Data.PHZFar, "");
  SAVE_IF_NOT_DEFAULT("EmuState", "EmulationStateId", EmuState->GetSelection(),
                      0);
  ...
  int depth =
      DepthPercentage->GetValue() > 0 ? DepthPercentage->GetValue() : 100;
...
void CISOProperties::LaunchExternalEditor(const std::string &filename,
  ...
  [[NSWorkspace sharedWorkspace]
             openFile:[NSString stringWithUTF8String:filename.c_str()]
  ...
  if (OpenCommand.IsEmpty()) {
    WxUtils::ShowErrorDialog(
        _("Couldn't find open command for extension 'ini'!"));
    return;
  }
...
void CISOProperties::OnComputeMD5Sum(wxCommandEvent &WXUNUSED(event)) {
  wxProgressDialog progressDialog(
      _("Computing MD5 checksum"), _("Working..."), 100, this,
      wxPD_APP_MODAL | wxPD_CAN_ABORT | wxPD_ELAPSED_TIME |
          wxPD_ESTIMATED_TIME | wxPD_REMAINING_TIME | wxPD_SMOOTH);

  m_MD5Sum->SetValue(MD5::MD5Sum(OpenGameListItem.GetFileName(),
...
void CISOProperties::OnShowDefaultConfig(wxCommandEvent &WXUNUSED(event)) {
  for (const std::string &filename :
       SConfig::GetGameIniFilenames(game_id, m_open_iso->GetRevision())) {
    std::string path =
        File::GetSysDirectory() + GAMESETTINGS_DIR DIR_SEP + filename;
    if (File::Exists(path))
...
void CISOProperties::ListSelectionChanged(wxCommandEvent &event) {
  ...
  case ID_PATCHES_LIST:
    if (Patches->GetSelection() == wxNOT_FOUND ||
        DefaultPatches.find(Patches->GetString(Patches->GetSelection())
                                .ToStdString()) != DefaultPatches.end()) {
      EditPatch->Disable();
      RemovePatch->Disable();
    } else {
      EditPatch->Enable();
  ...
  case ID_CHEATS_LIST:
    if (Cheats->GetSelection() == wxNOT_FOUND ||
        DefaultCheats.find(
            Cheats->RemoveMnemonics(Cheats->GetString(Cheats->GetSelection()))
                .ToStdString()) != DefaultCheats.end()) {
      EditCheat->Disable();
      RemoveCheat->Disable();
    } else {
      EditCheat->Enable();
...
void CISOProperties::OnActionReplayCodeChecked(wxCommandEvent &event) {
  arCodes[event.GetSelection()].active =
      Cheats->IsChecked(event.GetSelection());
}
...
void CISOProperties::PatchList_Load() {
  ...
  PatchEngine::LoadPatchSection("OnFrame", onFrame, GameIniDefault,
                                GameIniLocal);
...
void CISOProperties::ChangeBannerDetails(DiscIO::IVolume::ELanguage language) {
  ...
  wxString const comment =
      StrToWxStr(OpenGameListItem.GetDescription(language));
  ...
  SetTitle(StrToWxStr(StringFromFormat(
               "%s%s: %s - ", filename.c_str(), extension.c_str(),
