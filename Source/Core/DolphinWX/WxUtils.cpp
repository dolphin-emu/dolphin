...
wxBitmap LoadResourceBitmap(const std::string &name,
                            const wxSize &padded_size) {
  const std::string path_base =
      File::GetSysDirectory() + RESOURCES_DIR + DIR_SEP + name;
  ...
  if (padded_size != wxSize()) {
    // Add padding if necessary (or crop, but images aren't supposed to be large
    // enough to require
    // that).
...
void AddToolbarButton(wxToolBar *toolbar, int toolID, const wxString &label,
  ...
  toolbar->AddTool(toolID, label, bitmap,
                   WxUtils::CreateDisabledButtonBitmap(bitmap), wxITEM_NORMAL,
                   shortHelp);
}

} // namespace

std::string WxStrToStr(const wxString &str) { return str.ToUTF8().data(); }
