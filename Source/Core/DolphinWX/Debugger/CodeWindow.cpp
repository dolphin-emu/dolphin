...
CCodeWindow::CCodeWindow(const SConfig &_LocalCoreStartupParameter,
                         CFrame *parent, wxWindowID id, const wxPoint &position,
                         const wxSize &size, long style, const wxString &name)
    : wxPanel(parent, id, position, size, style, name), Parent(parent),
      m_RegisterWindow(nullptr), m_WatchWindow(nullptr),
      m_BreakpointWindow(nullptr), m_MemoryWindow(nullptr),
      m_JitWindow(nullptr), m_SoundWindow(nullptr), m_VideoWindow(nullptr),
      codeview(nullptr) {
  InitBitmaps();
  ...
  symbols = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0,
                          nullptr, wxLB_SORT);
  symbols->Bind(wxEVT_LISTBOX, &CCodeWindow::OnSymbolListChange, this);

  calls = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0,
                        nullptr, wxLB_SORT);
  calls->Bind(wxEVT_LISTBOX, &CCodeWindow::OnCallsListChange, this);

  callers = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0,
                          nullptr, wxLB_SORT);
  ...
  wxSearchCtrl *const address_searchctrl =
      new wxSearchCtrl(m_aui_toolbar, IDM_ADDRBOX);
  ...
  m_aui_manager.AddPane(
      codeview,
      wxAuiPaneInfo().CenterPane().CloseButton(false).Floatable(false));
  m_aui_manager.Update();

  // Menu
  Bind(wxEVT_MENU, &CCodeWindow::OnCPUMode, this, IDM_INTERPRETER,
       IDM_JIT_SR_OFF);
  Bind(wxEVT_MENU, &CCodeWindow::OnChangeFont, this, IDM_FONT_PICKER);
  Bind(wxEVT_MENU, &CCodeWindow::OnJitMenu, this, IDM_CLEAR_CODE_CACHE,
       IDM_SEARCH_INSTRUCTION);
  Bind(wxEVT_MENU, &CCodeWindow::OnSymbolsMenu, this, IDM_CLEAR_SYMBOLS,
       IDM_PATCH_HLE_FUNCTIONS);
  Bind(wxEVT_MENU, &CCodeWindow::OnProfilerMenu, this, IDM_PROFILE_BLOCKS,
       IDM_WRITE_PROFILE);
...
CCodeWindow::~CCodeWindow() { m_aui_manager.UnInit(); }

wxMenuBar *CCodeWindow::GetMenuBar() { return Parent->GetMenuBar(); }

wxToolBar *CCodeWindow::GetToolBar() { return Parent->m_ToolBar; }
...
void CCodeWindow::OnHostMessage(wxCommandEvent &event) {
  ...
  case IDM_UPDATE_JIT_PANE:
    // Check if the JIT pane is in the AUI notebook. If not, add it and switch
    // to it.
    if (!m_JitWindow)
...
void CCodeWindow::OnCodeViewChange(wxCommandEvent &event) { UpdateLists(); }

void CCodeWindow::OnAddrBoxChange(wxCommandEvent &event) {
  wxSearchCtrl *pAddrCtrl =
      (wxSearchCtrl *)m_aui_toolbar->FindControl(IDM_ADDRBOX);
...
void CCodeWindow::UpdateLists() {
  ...
  for (auto &call : symbol->callers) {
    ...
    if (caller_symbol) {
      int idx = callers->Append(
          StrToWxStr(StringFromFormat("< %s (%08x)",
                                      caller_symbol->name.c_str(), caller_addr)
                         .c_str()));
  ...
  for (auto &call : symbol->calls) {
    ...
    if (call_symbol) {
      int idx = calls->Append(StrToWxStr(
          StringFromFormat("> %s (%08x)", call_symbol->name.c_str(), call_addr)
              .c_str()));
...
void CCodeWindow::CreateMenu(const SConfig &core_startup_parameter,
                             wxMenuBar *pMenuBar) {
  // CPU Mode
  wxMenu *pCoreMenu = new wxMenu;

  wxMenuItem *interpreter =
      pCoreMenu->Append(IDM_INTERPRETER, _("&Interpreter core"),
                        _("This is necessary to get break points"
                          " and stepping to work as explained in the Developer "
                          "Documentation. But it can be very"
                          " slow, perhaps slower than 1 fps."),
                        wxITEM_CHECK);
  interpreter->Check(core_startup_parameter.iCPUCore ==
                     PowerPC::CORE_INTERPRETER);
  ...
  pCoreMenu->Append(
      ...
      _("Avoid any involuntary JIT cache clearing, this may prevent Zelda TP "
        "from "
  ...
  pCoreMenu->Append(
      ...
      wxITEM_CHECK);
  pCoreMenu->Append(IDM_JIT_LS_OFF, _("&JIT LoadStore off"), wxEmptyString,
                    wxITEM_CHECK);
  ...
  pCoreMenu->Append(IDM_JIT_LSP_OFF, _("&JIT LoadStore Paired off"),
                    wxEmptyString, wxITEM_CHECK);
  pCoreMenu->Append(IDM_JIT_FP_OFF, _("&JIT FloatingPoint off"), wxEmptyString,
                    wxITEM_CHECK);
  pCoreMenu->Append(IDM_JIT_I_OFF, _("&JIT Integer off"), wxEmptyString,
                    wxITEM_CHECK);
  pCoreMenu->Append(IDM_JIT_P_OFF, _("&JIT Paired off"), wxEmptyString,
                    wxITEM_CHECK);
  ...
  pDebugMenu->AppendSubMenu(pPerspectives, _("Perspectives"),
                            _("Edit Perspectives"));
  ...
  pPerspectives->AppendSubMenu(Parent->m_SavedPerspectives,
                               _("Saved perspectives"));
  ...
  pPerspectives->Append(IDM_NO_DOCKING, _("Disable docking"),
                        "Disable docking of perspective panes to main window",
                        wxITEM_CHECK);
...
void CCodeWindow::CreateMenuOptions(wxMenu *pMenu) {
  wxMenuItem *boottopause = pMenu->Append(
      IDM_BOOT_TO_PAUSE, _("Boot to pause"),
      _("Start the game directly instead of booting to pause"), wxITEM_CHECK);
  boottopause->Check(bBootToPause);

  wxMenuItem *automaticstart =
      pMenu->Append(IDM_AUTOMATIC_START, _("&Automatic start"),
                    _("Automatically load the Default ISO when Dolphin starts, "
                      "or the last game you loaded,"
                      " if you have not given it an elf file with the --elf "
                      "command line. [This can be"
                      " convenient if you are bug-testing with a certain game "
                      "and want to rebuild"
                      " and retry it several times, either with changes to "
                      "Dolphin or if you are"
...
void CCodeWindow::OnCPUMode(wxCommandEvent &event) {
  ...
  case IDM_INTERPRETER:
    PowerPC::SetMode(UseInterpreter() ? PowerPC::MODE_INTERPRETER
                                      : PowerPC::MODE_JIT);
    break;
...
void CCodeWindow::OnJitMenu(wxCommandEvent &event) {
  ...
  case IDM_SEARCH_INSTRUCTION: {
    ...
    for (u32 addr = 0x80000000; addr < 0x80180000; addr += 4) {
      const char *name =
          PPCTables::GetInstructionName(PowerPC::HostRead_U32(addr));
...
void CCodeWindow::InitBitmaps() {
  m_Bitmaps[Toolbar_Step] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_step");
  m_Bitmaps[Toolbar_StepOver] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_step_over");
  m_Bitmaps[Toolbar_StepOut] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_step_out");
  m_Bitmaps[Toolbar_Skip] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_skip");
  m_Bitmaps[Toolbar_GotoPC] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_goto_pc");
  m_Bitmaps[Toolbar_SetPC] =
      WxUtils::LoadResourceBitmap("toolbar_debugger_set_pc");
}
...
void CCodeWindow::PopulateToolbar(wxToolBar *toolBar) {
  ...
  WxUtils::AddToolbarButton(toolBar, IDM_STEP, _("Step"),
                            m_Bitmaps[Toolbar_Step],
                            _("Step into the next instruction"));
  WxUtils::AddToolbarButton(toolBar, IDM_STEPOVER, _("Step Over"),
                            m_Bitmaps[Toolbar_StepOver],
                            _("Step over the next instruction"));
  WxUtils::AddToolbarButton(toolBar, IDM_STEPOUT, _("Step Out"),
                            m_Bitmaps[Toolbar_StepOut],
                            _("Step out of the current function"));
  WxUtils::AddToolbarButton(toolBar, IDM_SKIP, _("Skip"),
                            m_Bitmaps[Toolbar_Skip],
                            _("Skips the next instruction completely"));
  toolBar->AddSeparator();
  WxUtils::AddToolbarButton(toolBar, IDM_GOTOPC, _("Show PC"),
                            m_Bitmaps[Toolbar_GotoPC],
                            _("Go to the current instruction"));
  WxUtils::AddToolbarButton(toolBar, IDM_SETPC, _("Set PC"),
                            m_Bitmaps[Toolbar_SetPC],
...
void CCodeWindow::Update() {
  ...
  // Do not automatically show the current PC position when a breakpoint is hit
  // or
