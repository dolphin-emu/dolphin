...
void CCodeWindow::StepOut()
{
  if (CPU::IsStepping())
  {
    ...
    if (PowerPC::breakpoints.IsAddressBreakPoint(PC))
      Core::DisplayMessage(_("Breakpoint encountered! Step out aborted.").ToStdString(), 3000);
    else if (clock::now() >= timeout)
      Core::DisplayMessage(_("Step out timed out!").ToStdString(), 3000);
    else
      Core::DisplayMessage(_("Step out successful!").ToStdString(), 3000);
