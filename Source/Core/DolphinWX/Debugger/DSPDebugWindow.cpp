...
DSPDebuggerLLE::DSPDebuggerLLE(wxWindow *parent, wxWindowID id)
  ...
  Bind(wxEVT_MENU, &DSPDebuggerLLE::OnChangeState, this, ID_RUNTOOL,
       ID_SHOWPCTOOL);
  ...
  m_Toolbar = new wxAuiToolBar(this, ID_TOOLBAR, wxDefaultPosition,
  ...
  m_Toolbar->AddTool(ID_SHOWPCTOOL, _("Show PC"),
                     wxArtProvider::GetBitmap(wxART_GO_TO_PARENT, wxART_OTHER,
                                              wxSize(10, 10)));
  m_Toolbar->AddSeparator();

  m_addr_txtctrl =
      new wxTextCtrl(m_Toolbar, wxID_ANY, wxEmptyString, wxDefaultPosition,
                     wxDefaultSize, wxTE_PROCESS_ENTER);
  m_addr_txtctrl->Bind(wxEVT_TEXT_ENTER, &DSPDebuggerLLE::OnAddrBoxChange,
                       this);
  ...
  m_CodeView =
      new CCodeView(&debug_interface, &DSPSymbols::g_dsp_symbol_db, code_panel);
  m_CodeView->SetPlain();
  ...
  m_mgr.AddPane(
      m_Toolbar,
      wxAuiPaneInfo().ToolbarPane().Top().LeftDockable(false).RightDockable(
          false));
  ...
  m_mgr.AddPane(m_Regs, wxAuiPaneInfo()
                            .Right()
                            .CloseButton(false)
                            .Caption(_("Registers"))
                            .Dockable(true));
...
void DSPDebuggerLLE::OnClose(wxCloseEvent &event) { event.Skip(); }
...
void DSPDebuggerLLE::UpdateState() {
  if (DSPCore_GetState() == DSPCORE_RUNNING) {
    ...
    m_Toolbar->SetToolBitmap(
        ID_RUNTOOL, wxArtProvider::GetBitmap(wxART_GO_FORWARD, wxART_OTHER,
                                             wxSize(10, 10)));
