...
CMemoryWindow::CMemoryWindow(wxWindow *parent, wxWindowID id,
  ...
  valbox = new wxTextCtrl(this, IDM_VALBOX, "", wxDefaultPosition,
                          wxDefaultSize, wxTE_PROCESS_ENTER);
  valbox->Bind(wxEVT_TEXT_ENTER, &CMemoryWindow::SetMemoryValueFromValBox,
               this);
  ...
  dump_sizer->Add(new wxButton(this, IDM_DUMP_MEMORY, _("Dump MRAM")), 0,
                  wxEXPAND);
  dump_sizer->Add(new wxButton(this, IDM_DUMP_MEM2, _("Dump EXRAM")), 0,
                  wxEXPAND);
  if (!SConfig::GetInstance().bMMU)
    dump_sizer->Add(new wxButton(this, IDM_DUMP_FAKEVMEM, _("Dump FakeVMEM")),
                    0, wxEXPAND);

  wxStaticBoxSizer *const sizerSearchType =
      new wxStaticBoxSizer(wxVERTICAL, this, _("Search"));
  ...
  wxStaticBoxSizer *const sizerDataTypes =
      new wxStaticBoxSizer(wxVERTICAL, this, _("Data Type"));
...
void CMemoryWindow::JumpToAddress(u32 _Address) { memview->Center(_Address); }
...
void CMemoryWindow::SetMemoryValue(wxCommandEvent &event) {
  ...
  if (!TryParse(std::string("0x") + str_addr, &addr)) {
    WxUtils::ShowErrorDialog(
        wxString::Format(_("Invalid address: %s"), str_addr.c_str()));
    return;
  }

  if (!TryParse(std::string("0x") + str_val, &val)) {
    WxUtils::ShowErrorDialog(
        wxString::Format(_("Invalid value: %s"), str_val.c_str()));
    return;
  }
...
static void DumpArray(const std::string &filename, const u8 *data,
                      size_t length) {
  if (data) {
...
void CMemoryWindow::OnDumpMemory(wxCommandEvent &event) {
  DumpArray(File::GetUserPath(F_RAMDUMP_IDX), Memory::m_pRAM,
            Memory::REALRAM_SIZE);
}
...
void CMemoryWindow::OnDumpMem2(wxCommandEvent &event) {
  if (SConfig::GetInstance().bWii) {
    DumpArray(File::GetUserPath(F_ARAMDUMP_IDX), Memory::m_pEXRAM,
              Memory::EXRAM_SIZE);
  } else {
    DumpArray(File::GetUserPath(F_ARAMDUMP_IDX), DSP::GetARAMPtr(),
              DSP::ARAM_SIZE);
  }
}
...
void CMemoryWindow::OnDumpFakeVMEM(wxCommandEvent &event) {
  DumpArray(File::GetUserPath(F_FAKEVMEMDUMP_IDX), Memory::m_pFakeVMEM,
...
void CMemoryWindow::onAscii(wxCommandEvent &event) { chkHex->SetValue(0); }

void CMemoryWindow::onHex(wxCommandEvent &event) { chkAscii->SetValue(0); }
