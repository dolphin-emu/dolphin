...
void CBreakPointView::Update() {
  ...
  const BreakPoints::TBreakPoints &rBreakPoints =
      PowerPC::breakpoints.GetBreakPoints();
  for (const auto &rBP : rBreakPoints) {
    if (!rBP.bTemporary) {
      ...
      if (symbol) {
        wxString symbol_description =
            StrToWxStr(g_symbolDB.GetDescription(rBP.iAddress));
  ...
  for (const auto &rMemCheck : rMemChecks) {
    wxString memcheck_on_str =
        StrToWxStr((rMemCheck.Break || rMemCheck.Log) ? "on" : " ");
    ...
    if (symbol) {
      wxString memcheck_start_addr =
          StrToWxStr(g_symbolDB.GetDescription(rMemCheck.StartAddress));
      SetItem(item, 2, memcheck_start_addr);
    }

    std::string address_range_str = StringFromFormat(
        "%08x to %08x", rMemCheck.StartAddress, rMemCheck.EndAddress);
