...
void CCodeWindow::Load() {
  ...
  const char *SettingName[] = {
      "Log",    "LogConfig", "Console", "Registers", "Breakpoints",
  ...
  for (int i = 0; i <= IDM_VIDEO_WINDOW - IDM_LOG_WINDOW; i++)
    ini.GetOrCreateSection("ShowOnStart")
        ->Get(SettingName[i], &bShowOnStart[i], false);

  // Get notebook affiliation
  std::string section =
      "P - " + ((Parent->ActivePerspective < Parent->Perspectives.size())
  ...
  for (int i = 0; i <= IDM_CODE_WINDOW - IDM_LOG_WINDOW; i++)
    ini.GetOrCreateSection("Float")->Get(SettingName[i],
                                         &Parent->bFloatWindow[i], false);
}
...
void CCodeWindow::Save() {
  ...
  std::string section =
      "P - " + Parent->Perspectives[Parent->ActivePerspective].Name;
  for (int i = 0; i <= IDM_CODE_WINDOW - IDM_LOG_WINDOW; i++)
...
void CCodeWindow::CreateMenuSymbols(wxMenuBar *pMenuBar) {
  ...
  pSymbolsMenu->Append(
      IDM_SAVEMAPFILE, _("&Save symbol map"),
  ...
  pSymbolsMenu->Append(
      IDM_SAVE_MAP_FILE_AS, _("Save symbol map &as..."),
  ...
  pProfilerMenu->Append(IDM_PROFILE_BLOCKS, _("&Profile blocks"), wxEmptyString,
                        wxITEM_CHECK);
...
void CCodeWindow::OnProfilerMenu(wxCommandEvent &event) {
  ...
  case IDM_WRITE_PROFILE:
    ...
    if (Core::GetState() == Core::CORE_PAUSE &&
        PowerPC::GetMode() == PowerPC::MODE_JIT) {
      if (jit != nullptr) {
        std::string filename =
            File::GetUserPath(D_DUMP_IDX) + "Debug/profiler.txt";
        File::CreateFullPath(filename);
        Profiler::WriteProfileResults(filename);

        wxFileType *filetype = nullptr;
        if (!(filetype =
                  wxTheMimeTypesManager->GetFileTypeFromExtension("txt"))) {
          // From extension failed, trying with MIME type now
          if (!(filetype = wxTheMimeTypesManager->GetFileTypeFromMimeType(
                    "text/plain")))
...
void CCodeWindow::OnSymbolsMenu(wxCommandEvent &event) {
  ...
  bool map_exists =
      CBoot::FindMapFile(&existing_map_file, &writable_map_file, &title_id_str);
  switch (event.GetId()) {
  ...
  case IDM_SCAN_FUNCTIONS: {
    ...
    if (db.Load(File::GetSysDirectory() + TOTALDB)) {
      ...
      Parent->StatusBarMessage("'%s' not found, no symbol names generated",
                               TOTALDB);
    }
    // HLE::PatchFunctions();
    // Update GUI
  ...
  case IDM_LOAD_MAP_FILE:
    if (!map_exists) {
      ...
      if (db.Load(File::GetSysDirectory() + TOTALDB))
        db.Apply(&g_symbolDB);
      Parent->StatusBarMessage(
          "'%s' not found, scanning for common functions instead",
          writable_map_file.c_str());
    } else {
      g_symbolDB.LoadMap(existing_map_file);
      Parent->StatusBarMessage("Loaded symbols from '%s'",
                               existing_map_file.c_str());
    }
    HLE::PatchFunctions();
    NotifyMapLoaded();
    break;
  case IDM_LOAD_MAP_FILE_AS: {
    const wxString path =
        wxFileSelector(_("Load map file"), File::GetUserPath(D_MAPS_IDX),
                       title_id_str + ".map", ".map",
                       _("Dolphin Map File (*.map)") + "|*.map|" +
  ...
  case IDM_LOAD_BAD_MAP_FILE: {
    const wxString path =
        wxFileSelector(_("Load bad map file"), File::GetUserPath(D_MAPS_IDX),
                       title_id_str + ".map", ".map",
                       _("Dolphin Map File (*.map)") + "|*.map|" +
                           wxGetTranslation(wxALL_FILES),
                       wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
  ...
  case IDM_SAVE_MAP_FILE_AS: {
    const wxString path =
        wxFileSelector(_("Save map file as"), File::GetUserPath(D_MAPS_IDX),
                       title_id_str + ".map", ".map",
                       _("Dolphin Map File (*.map)") + "|*.map|" +
                           wxGetTranslation(wxALL_FILES),
  ...
  case IDM_RENAME_SYMBOLS: {
    const wxString path = wxFileSelector(
        ...
        _("Dolphin Symbol Rename File (*.sym)") + "|*.sym|" +
            wxGetTranslation(wxALL_FILES),
        wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
  ...
  case IDM_CREATE_SIGNATURE_FILE: {
    wxTextEntryDialog input_prefix(
        this, _("Only export symbols with prefix:\n(Blank for all symbols)"),
    ...
    if (input_prefix.ShowModal() == wxID_OK) {
      ...
      wxString path = wxFileSelector(
          _("Save signature as"), File::GetSysDirectory(), wxEmptyString,
          wxEmptyString, _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                             wxGetTranslation(wxALL_FILES),
          wxFD_SAVE | wxFD_OVERWRITE_PROMPT, this);
      if (!path.IsEmpty()) {
        SignatureDB db;
  ...
  case IDM_APPEND_SIGNATURE_FILE: {
    ...
    if (input_prefix.ShowModal() == wxID_OK) {
      ...
      wxString path = wxFileSelector(
          _("Append signature to"), File::GetSysDirectory(), wxEmptyString,
          wxEmptyString, _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                             wxGetTranslation(wxALL_FILES),
          wxFD_SAVE, this);
      if (!path.IsEmpty()) {
  ...
  case IDM_USE_SIGNATURE_FILE: {
    wxString path = wxFileSelector(
        _("Apply signature file"), File::GetSysDirectory(), wxEmptyString,
        wxEmptyString, _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                           wxGetTranslation(wxALL_FILES),
        wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
    if (!path.IsEmpty()) {
  ...
  case IDM_COMBINE_SIGNATURE_FILES: {
    wxString path1 = wxFileSelector(
        _("Choose priority input file"), File::GetSysDirectory(), wxEmptyString,
        wxEmptyString, _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                           wxGetTranslation(wxALL_FILES),
        wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
    if (!path1.IsEmpty()) {
      SignatureDB db;
      wxString path2 =
          wxFileSelector(_("Choose secondary input file"),
                         File::GetSysDirectory(), wxEmptyString, wxEmptyString,
                         _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                             wxGetTranslation(wxALL_FILES),
                         wxFD_OPEN | wxFD_FILE_MUST_EXIST, this);
      if (!path2.IsEmpty()) {
        ...
        path2 = wxFileSelector(_("Save combined output file as"),
                               File::GetSysDirectory(), wxEmptyString, ".dsy",
                               _("Dolphin Signature File (*.dsy)") + "|*.dsy|" +
                                   wxGetTranslation(wxALL_FILES),
                               wxFD_SAVE | wxFD_OVERWRITE_PROMPT, this);
...
void CCodeWindow::ToggleRegisterWindow(bool bShow) {
  ...
  if (bShow) {
    ...
    Parent->DoAddPage(
        m_RegisterWindow, iNbAffiliation[IDM_REGISTER_WINDOW - IDM_LOG_WINDOW],
...
void CCodeWindow::ToggleBreakPointWindow(bool bShow) {
  ...
  if (bShow) {
    if (!m_BreakpointWindow)
      m_BreakpointWindow =
          new CBreakPointWindow(this, Parent, IDM_BREAKPOINT_WINDOW);
    Parent->DoAddPage(
        m_BreakpointWindow,
