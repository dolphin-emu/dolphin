...
wxMenuBar *CFrame::CreateMenu() {
  ...
  emulationMenu->Append(IDM_FRAMESTEP, GetMenuLabel(HK_FRAME_ADVANCE),
                        wxEmptyString);

  wxMenu *skippingMenu = new wxMenu;
  emulationMenu->AppendSubMenu(skippingMenu, _("Frame S&kipping"));
  for (int i = 0; i < 10; i++)
    skippingMenu->AppendRadioItem(IDM_FRAME_SKIP_0 + i,
                                  wxString::Format("%i", i));
  skippingMenu->Check(IDM_FRAME_SKIP_0 + SConfig::GetInstance().m_FrameSkip,
                      true);
  ...
  emulationMenu->Append(IDM_SELECT_SLOT, _("Select State Slot"),
                        slotSelectMenu);

  saveMenu->Append(IDM_SAVE_STATE_FILE, GetMenuLabel(HK_SAVE_STATE_FILE));
  saveMenu->Append(IDM_SAVE_SELECTED_SLOT,
                   GetMenuLabel(HK_SAVE_STATE_SLOT_SELECTED));
  ...
  loadMenu->Append(IDM_LOAD_SELECTED_SLOT,
                   GetMenuLabel(HK_LOAD_STATE_SLOT_SELECTED));
  loadMenu->Append(IDM_UNDO_LOAD_STATE, GetMenuLabel(HK_UNDO_LOAD_STATE));
  loadMenu->AppendSeparator();

  for (unsigned int i = 0; i < State::NUM_STATES; i++) {
    loadMenu->Append(IDM_LOAD_SLOT_1 + i,
                     GetMenuLabel(HK_LOAD_STATE_SLOT_1 + i));
    saveMenu->Append(IDM_SAVE_SLOT_1 + i,
                     GetMenuLabel(HK_SAVE_STATE_SLOT_1 + i));
    slotSelectMenu->Append(IDM_SELECT_SLOT_1 + i,
                           GetMenuLabel(HK_SELECT_STATE_SLOT_1 + i));
  }

  loadMenu->AppendSeparator();
  for (unsigned int i = 0; i < State::NUM_STATES; i++)
    loadMenu->Append(IDM_LOAD_LAST_1 + i,
                     GetMenuLabel(HK_LOAD_LAST_STATE_1 + i));
  ...
  movieMenu->Append(IDM_RECORD_READ_ONLY, GetMenuLabel(HK_READ_ONLY_MODE),
                    wxEmptyString, wxITEM_CHECK);
  movieMenu->Append(IDM_TAS_INPUT, _("TAS Input"));
  movieMenu->AppendSeparator();
  movieMenu->AppendCheckItem(IDM_TOGGLE_PAUSE_MOVIE,
                             _("Pause at End of Movie"));
  ...
  movieMenu->Check(IDM_SHOW_FRAME_COUNT,
                   SConfig::GetInstance().m_ShowFrameCount);
  movieMenu->Check(IDM_RECORD_READ_ONLY, true);
  movieMenu->AppendCheckItem(IDM_SHOW_INPUT_DISPLAY, _("Show Input Display"));
  movieMenu->Check(IDM_SHOW_INPUT_DISPLAY,
                   SConfig::GetInstance().m_ShowInputDisplay);
  movieMenu->AppendSeparator();
  ...
  UpdateWiiMenuChoice(
      toolsMenu->Append(IDM_LOAD_WII_MENU, "Dummy string to keep wxw happy"));
  ...
  wiimoteMenu->AppendCheckItem(IDM_CONNECT_WIIMOTE1,
                               GetMenuLabel(HK_WIIMOTE1_CONNECT));
  wiimoteMenu->AppendCheckItem(IDM_CONNECT_WIIMOTE2,
                               GetMenuLabel(HK_WIIMOTE2_CONNECT));
  wiimoteMenu->AppendCheckItem(IDM_CONNECT_WIIMOTE3,
                               GetMenuLabel(HK_WIIMOTE3_CONNECT));
  wiimoteMenu->AppendCheckItem(IDM_CONNECT_WIIMOTE4,
                               GetMenuLabel(HK_WIIMOTE4_CONNECT));
  wiimoteMenu->AppendSeparator();
  wiimoteMenu->AppendCheckItem(IDM_CONNECT_BALANCEBOARD,
                               GetMenuLabel(HK_BALANCEBOARD_CONNECT));

  menubar->Append(toolsMenu, _("&Tools"));

  wxMenu *viewMenu = new wxMenu;
  viewMenu->AppendCheckItem(IDM_TOGGLE_TOOLBAR, _("Show &Toolbar"));
  viewMenu->Check(IDM_TOGGLE_TOOLBAR,
                  SConfig::GetInstance().m_InterfaceToolbar);
  viewMenu->AppendCheckItem(IDM_TOGGLE_STATUSBAR, _("Show &Status Bar"));
  viewMenu->Check(IDM_TOGGLE_STATUSBAR,
                  SConfig::GetInstance().m_InterfaceStatusbar);
  viewMenu->AppendSeparator();
  viewMenu->AppendCheckItem(IDM_LOG_WINDOW, _("Show &Log"));
  viewMenu->AppendCheckItem(IDM_LOG_CONFIG_WINDOW,
                            _("Show Log &Configuration"));
  viewMenu->AppendSeparator();
  ...
  if (g_pCodeWindow) {
    ...
    static const wxString menu_text[] = {
        _("&Registers"), _("&Watch"), _("&Breakpoints"), _("&Memory"),
        _("&JIT"),       _("&Sound"), _("&Video")};
    ...
    viewMenu->Check(IDM_LOG_WINDOW,
                    SConfig::GetInstance().m_InterfaceLogWindow);
    viewMenu->Check(IDM_LOG_CONFIG_WINDOW,
  ...
  regionMenu->Check(IDM_LIST_NETHERLANDS,
  ...
  columnsMenu->Check(IDM_SHOW_FILENAME,
  ...
  columnsMenu->Check(IDM_SHOW_REGION,
...
wxString CFrame::GetMenuLabel(int Id) {
  ...
  case HK_WIIMOTE4_CONNECT:
    Label =
        wxString::Format(_("Connect Wiimote %i"), Id - HK_WIIMOTE1_CONNECT + 1);
    break;
  ...
  case HK_LOAD_STATE_SLOT_10:
    Label = wxString::Format(
        _("Slot %i - %s"), Id - HK_LOAD_STATE_SLOT_1 + 1,
  ...
  case HK_SAVE_STATE_SLOT_10:
    Label = wxString::Format(
        _("Slot %i - %s"), Id - HK_SAVE_STATE_SLOT_1 + 1,
  ...
  case HK_SELECT_STATE_SLOT_10:
    Label = wxString::Format(_("Select Slot %i - %s"),
                             ...
                             StrToWxStr(State::GetInfoStringOfSlot(
                                 Id - HK_SELECT_STATE_SLOT_1 + 1)));
    break;
...
void CFrame::PopulateToolbar(wxToolBar *ToolBar) {
  ...
  WxUtils::AddToolbarButton(ToolBar, wxID_OPEN, _("Open"),
                            m_Bitmaps[Toolbar_FileOpen], _("Open file..."));
  WxUtils::AddToolbarButton(ToolBar, wxID_REFRESH, _("Refresh"),
                            m_Bitmaps[Toolbar_Refresh], _("Refresh game list"));
  ToolBar->AddSeparator();
  WxUtils::AddToolbarButton(ToolBar, IDM_PLAY, _("Play"),
                            m_Bitmaps[Toolbar_Play], _("Play"));
  WxUtils::AddToolbarButton(ToolBar, IDM_STOP, _("Stop"),
                            m_Bitmaps[Toolbar_Stop], _("Stop"));
  WxUtils::AddToolbarButton(ToolBar, IDM_TOGGLE_FULLSCREEN, _("FullScr"),
                            m_Bitmaps[Toolbar_FullScreen],
                            _("Toggle fullscreen"));
  WxUtils::AddToolbarButton(ToolBar, IDM_SCREENSHOT, _("ScrShot"),
                            m_Bitmaps[Toolbar_Screenshot],
                            _("Take screenshot"));
  ToolBar->AddSeparator();
  WxUtils::AddToolbarButton(ToolBar, wxID_PREFERENCES, _("Config"),
                            m_Bitmaps[Toolbar_ConfigMain], _("Configure..."));
  WxUtils::AddToolbarButton(ToolBar, IDM_CONFIG_GFX_BACKEND, _("Graphics"),
                            m_Bitmaps[Toolbar_ConfigGFX],
                            _("Graphics settings"));
  WxUtils::AddToolbarButton(ToolBar, IDM_CONFIG_CONTROLLERS, _("Controllers"),
                            m_Bitmaps[Toolbar_Controller],
...
void CFrame::InitBitmaps() {
  auto const dir =
      StrToWxStr(File::GetThemeDir(SConfig::GetInstance().theme_name));
  ...
  m_Bitmaps[Toolbar_ConfigGFX].LoadFile(dir + "graphics.png",
                                        wxBITMAP_TYPE_PNG);
  m_Bitmaps[Toolbar_Controller].LoadFile(dir + "classic.png",
                                         wxBITMAP_TYPE_PNG);
  m_Bitmaps[Toolbar_Screenshot].LoadFile(dir + "screenshot.png",
                                         wxBITMAP_TYPE_PNG);
  m_Bitmaps[Toolbar_FullScreen].LoadFile(dir + "fullscreen.png",
                                         wxBITMAP_TYPE_PNG);
...
void CFrame::OnTASInput(wxCommandEvent &event) {
  for (int i = 0; i < 4; ++i) {
    if (SConfig::GetInstance().m_SIDevice[i] != SIDEVICE_NONE &&
      ...
      g_TASInputDlg[i]->SetTitle(
          wxString::Format(_("TAS Input - Controller %d"), i + 1));
    }
    ...
    if (g_wiimote_sources[i] == WIIMOTE_SRC_EMU &&
      ...
      g_TASInputDlg[i + 4]->SetTitle(
          wxString::Format(_("TAS Input - Wiimote %d"), i + 1));
    }
  }
}
...
void CFrame::OnChangeDisc(wxCommandEvent &WXUNUSED(event)) { DoOpen(false); }

void CFrame::OnRecord(wxCommandEvent &WXUNUSED(event)) {
  if ((!Core::IsRunningAndStarted() && Core::IsRunning()) ||
...
void CFrame::OnPlayRecording(wxCommandEvent &WXUNUSED(event)) {
  wxString path = wxFileSelector(
      _("Select The Recording File"), wxEmptyString, wxEmptyString,
      wxEmptyString,
...
void CFrame::StartGame(const std::string &filename) {
  ...
  if (SConfig::GetInstance().bRenderToMain) {
    ...
    if (SConfig::GetInstance().bKeepWindowOnTop)
      m_RenderFrame->SetWindowStyle(m_RenderFrame->GetWindowStyle() |
                                    wxSTAY_ON_TOP);
    else
      m_RenderFrame->SetWindowStyle(m_RenderFrame->GetWindowStyle() &
                                    ~wxSTAY_ON_TOP);
    ...
    // flag.)
    m_RenderParent->SetWindowStyle(m_RenderParent->GetWindowStyle() &
                                   ~wxTAB_TRAVERSAL);
  } else {
    ...
    if ((leftPos + width) < (position.x + size.GetWidth()) ||
        leftPos > position.x ||
        (topPos + height) < (position.y + size.GetHeight()) ||
        topPos > position.y)
      position.x = position.y = wxDefaultCoord;
#endif
    m_RenderFrame =
        new CRenderFrame((wxFrame *)this, wxID_ANY, _("Dolphin"), position);
    if (SConfig::GetInstance().bKeepWindowOnTop)
      m_RenderFrame->SetWindowStyle(m_RenderFrame->GetWindowStyle() |
                                    wxSTAY_ON_TOP);
    else
    ...
    // child.
    m_RenderParent = new wxPanel(m_RenderFrame, IDM_MPANEL, wxDefaultPosition,
                                 wxDefaultSize, 0);
#endif
  ...
  if (Core::GetState() != Core::CORE_UNINITIALIZED ||
    ...
    if (SConfig::GetInstance().bConfirmStop) {
      ...
      wxMessageDialog m_StopDlg(
          this, _("Do you want to stop the current emulation?"),
...
void CFrame::OnStop(wxCommandEvent &WXUNUSED(event)) { DoStop(); }
...
void CFrame::OnImportSave(wxCommandEvent &WXUNUSED(event)) {
  wxString path = wxFileSelector(
      _("Select the save file"), wxEmptyString, wxEmptyString, wxEmptyString,
...
void CFrame::OnInstallWAD(wxCommandEvent &event) {
  ...
  case IDM_MENU_INSTALL_WAD: {
    wxString path = wxFileSelector(
        _("Select a Wii WAD file to install"), wxEmptyString, wxEmptyString,
        wxEmptyString,
...
void CFrame::UpdateWiiMenuChoice(wxMenuItem *WiiMenuItem) {
  ...
  const DiscIO::CNANDContentLoader &SysMenu_Loader =
      DiscIO::CNANDContentManager::Access().GetNANDLoader(
  ...
  if (SysMenu_Loader.IsValid()) {
    ...
    WiiMenuItem->Enable();
    WiiMenuItem->SetItemLabel(wxString::Format(_("Load Wii System Menu %d%c"),
                                               sysmenuVersion, sysmenuRegion));
  } else {
...
void CFrame::ConnectWiimote(int wm_idx, bool connect) {
  if (Core::IsRunning() && SConfig::GetInstance().bWii) {
    ...
    wxString msg(
        wxString::Format(_("Wiimote %i %s"), wm_idx + 1,
...
void CFrame::OnConnectWiimote(wxCommandEvent &event) {
  bool was_unpaused = Core::PauseAndLock(true);
  ConnectWiimote(
      event.GetId() - IDM_CONNECT_WIIMOTE1,
      !GetUsbPointer()
...
void CFrame::OnLoadStateFromFile(wxCommandEvent &WXUNUSED(event)) {
  wxString path = wxFileSelector(
      _("Select the state to load"), wxEmptyString, wxEmptyString,
      wxEmptyString,
...
void CFrame::OnSaveStateToFile(wxCommandEvent &WXUNUSED(event)) {
  wxString path = wxFileSelector(
      _("Select the state to save"), wxEmptyString, wxEmptyString,
      wxEmptyString,
      _("All Save States (sav, s##)") +
          wxString::Format("|*.sav;*.s??|%s", wxGetTranslation(wxALL_FILES)),
      wxFD_SAVE, this);
...
void CFrame::OnSelectSlot(wxCommandEvent &event) {
  ...
  Core::DisplayMessage(
      StringFromFormat("Selected slot %d - %s", g_saveSlot,
...
void CFrame::UpdateGUI() {
  ...
  GetMenuBar()
      ->FindItem(IDM_CHEATS)
      ->Enable(SConfig::GetInstance().bEnableCheats);
  ...
  if (m_ToolBar) {
    ...
    if (PlayTool) {
      ...
      if (Running) {
        ...
        m_ToolBar->InsertTool(
            position, IDM_PLAY, _("Pause"), m_Bitmaps[Toolbar_Pause],
            WxUtils::CreateDisabledButtonBitmap(m_Bitmaps[Toolbar_Pause]),
            wxITEM_NORMAL, _("Pause"));
      } else {
        m_ToolBar->DeleteTool(IDM_PLAY);
        m_ToolBar->InsertTool(
            position, IDM_PLAY, _("Play"), m_Bitmaps[Toolbar_Play],
...
void CFrame::UpdateGameList() { m_GameListCtrl->Update(); }
