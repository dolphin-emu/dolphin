...
ActionReplayCodesPanel::ActionReplayCodesPanel(wxWindow* parent, int styles) : wxPanel(parent)
{
...
void ActionReplayCodesPanel::SetCodePanelStyle(int styles)
{
  m_styles = styles;
...
{
  // If the code editor is open then we're going to have a conflict.
  if (m_editor)
...
void ActionReplayCodesPanel::UpdateSidePanel()
{
  ...
  m_label_code_name->SetLabelText(_("Code Name: ") + name);
  m_label_code_name->Wrap(m_label_code_name->GetSize().GetWidth());
  m_label_code_name->InvalidateBestSize();
  m_label_num_codes->SetLabel(wxString::Format("%s%zu", _("Number of Codes: "), code_count));
  Layout();
}
...
void ActionReplayCodesPanel::UpdateModifyButtons()
{
  ...
  m_btn_edit_code->SetLabel(is_user_defined ? _("&Edit Code") : _("Clone and &Edit"));
...
{
  ARCodeAddEdit editor{{}, this, wxID_ANY, _("Add Action Replay Code")};
  m_editor = &editor;
...
void ActionReplayCodesPanel::OnEditCodeClick(wxCommandEvent&)
{
  ...
  {
    ARCodeAddEdit editor{code, this, wxID_ANY, _("Duplicate Bundled Action Replay Code")};
    m_editor = &editor;
    if (editor.ShowModal() == wxID_SAVE)
      AppendNewCode(editor.GetCode());
    m_editor = nullptr;
