...
CodeConfigPanel::CodeConfigPanel(wxWindow *const parent) : wxPanel(parent) {
  ...
  m_listbox_gcodes->Bind(wxEVT_CHECKLISTBOX, &CodeConfigPanel::ToggleCode,
                         this);

  m_infobox.label_name =
      new wxStaticText(this, wxID_ANY, wxGetTranslation(wxstr_name));
  m_infobox.label_creator =
      new wxStaticText(this, wxID_ANY, wxGetTranslation(wxstr_creator));
  m_infobox.label_notes =
      new wxStaticText(this, wxID_ANY, wxGetTranslation(wxstr_notes));
  m_infobox.textctrl_notes =
  ...
  btn_download =
      new wxButton(this, wxID_ANY, _("Download Codes (WiiRD Database)"),
...
void CodeConfigPanel::UpdateCodeList(bool checkRunning) {
  ...
  btn_download->Enable((!checkRunning || Core::IsRunning()) &&
                       !m_gameid.empty());
...
void CodeConfigPanel::LoadCodes(const IniFile &globalIni,
...
void CodeConfigPanel::UpdateInfoBox(wxCommandEvent &) {
  ...
  if (sel > -1) {
    ...
    for (const GeckoCode::Code &code : m_gcodes[sel].codes) {
      m_infobox.listbox_codes->Append(
          wxString::Format("%08X %08X", code.address, code.data));
    }
  } else {
    m_infobox.label_name->SetLabel(wxGetTranslation(wxstr_name));
    m_infobox.textctrl_notes->Clear();
  ...
  if (sf::Http::Response::Ok == resp.getStatus()) {
    ...
    if (gcodes.size()) {
      ...
      for (const GeckoCode &code : gcodes) {
        ...
        std::vector<GeckoCode>::const_iterator existing_gcodes_iter =
                                                   m_gcodes.begin(),
                                               existing_gcodes_end =
                                                   m_gcodes.end();
