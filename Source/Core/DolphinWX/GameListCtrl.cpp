...
void CGameListCtrl::ScanForISOs()
{
  ...
  {
    // wxProgressDialog uses native Win32 Task Dialogs but the dialog is used incorrectly.
    // wxWidgets creates the dialog on a thread with no parent which causes the dialog
    // to automatically bond with the current front window. During start-up this generally
    // means Windows Explorer / Desktop. When the dialog is closed, it raises its bonded
    // window instead of Dolphin. It's essentially a race condition.
    // Just use the generic progress dialog instead which does not use threads so it works
    // properly even if it looks ugly.
