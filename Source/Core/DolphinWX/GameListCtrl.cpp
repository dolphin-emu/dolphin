...
void CGameListCtrl::InitBitmaps()
{
  ...
  m_FlagImageIndex.resize(static_cast<size_t>(DiscIO::ECountry::NUMBER_OF_COUNTRIES));
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_JAPAN, "Flag_Japan");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_EUROPE, "Flag_Europe");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_USA, "Flag_USA");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_AUSTRALIA, "Flag_Australia");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_FRANCE, "Flag_France");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_GERMANY, "Flag_Germany");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_ITALY, "Flag_Italy");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_KOREA, "Flag_Korea");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_NETHERLANDS,
             "Flag_Netherlands");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_RUSSIA, "Flag_Russia");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_SPAIN, "Flag_Spain");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_TAIWAN, "Flag_Taiwan");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_WORLD, "Flag_International");
  InitBitmap(img_list, &m_FlagImageIndex, DiscIO::ECountry::COUNTRY_UNKNOWN, "Flag_Unknown");

  m_PlatformImageIndex.resize(static_cast<size_t>(DiscIO::EPlatform::NUMBER_OF_PLATFORMS));
  InitBitmap(img_list, &m_PlatformImageIndex, DiscIO::EPlatform::GAMECUBE_DISC,
             "Platform_Gamecube");
  InitBitmap(img_list, &m_PlatformImageIndex, DiscIO::EPlatform::WII_DISC, "Platform_Wii");
  InitBitmap(img_list, &m_PlatformImageIndex, DiscIO::EPlatform::WII_WAD, "Platform_Wad");
  InitBitmap(img_list, &m_PlatformImageIndex, DiscIO::EPlatform::ELF_DOL, "Platform_File");
      ...
      if (iso_file->IsValid())
      {
        ...
        case DiscIO::ECountry::COUNTRY_WORLD:
          if (!SConfig::GetInstance().m_ListWorld)
            list = false;
          break;
        case DiscIO::ECountry::COUNTRY_UNKNOWN:
        default:
...
void CGameListCtrl::OnRightClick(wxMouseEvent& event)
{
  ...
  if (GetSelectedItemCount() == 1)
  {
    ...
    {
      wxMenu popupMenu;
      DiscIO::EPlatform platform = selected_iso->GetPlatform();

      if (platform != DiscIO::EPlatform::ELF_DOL)
      {
        popupMenu.Append(IDM_PROPERTIES, _("&Properties"));
        popupMenu.Append(IDM_GAME_WIKI, _("&Wiki"));
        popupMenu.AppendSeparator();
      }
      if (platform == DiscIO::EPlatform::WII_DISC || platform == DiscIO::EPlatform::WII_WAD)
      {
      ...
      if (platform == DiscIO::EPlatform::GAMECUBE_DISC || platform == DiscIO::EPlatform::WII_DISC)
      {
      ...
      if (platform == DiscIO::EPlatform::WII_WAD)
...
void CGameListCtrl::CompressSelection(bool _compress)
{
  ...
  for (const GameListItem* iso : GetAllSelectedISOs())
  {
    ...
    if (iso->GetPlatform() != DiscIO::EPlatform::GAMECUBE_DISC &&
        iso->GetPlatform() != DiscIO::EPlatform::WII_DISC)
      continue;
...

  {
    ...
    for (const GameListItem* iso : items_to_compress)
    {
      if (!iso->IsCompressed() && _compress)
      {
        ...
        all_good &=
            DiscIO::CompressFileToBlob(iso->GetFileName(), OutputFileName,
                                       (iso->GetPlatform() == DiscIO::EPlatform::WII_DISC) ? 1 : 0,
      ...
      else if (iso->IsCompressed() && !_compress)
      {
        ...
        SplitPath(iso->GetFileName(), nullptr, &FileName, nullptr);
        progress.current_filename = FileName;
        if (iso->GetPlatform() == DiscIO::EPlatform::WII_DISC)
          FileName.append(".iso");
        else
          FileName.append(".gcm");
...
void CGameListCtrl::OnCompressISO(wxCommandEvent& WXUNUSED(event))
{
  ...
  do
  {
    ...
    {
      if (iso->GetPlatform() == DiscIO::EPlatform::WII_DISC && !WiiCompressWarning())
        return;
...

  {
    ...
    if (is_compressed)
      ...
      all_good = DiscIO::CompressFileToBlob(
          ...
          (iso->GetPlatform() == DiscIO::EPlatform::WII_DISC) ? 1 : 0, 16384, &CompressCB, &dialog);
  }
