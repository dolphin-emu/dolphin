...
bool CGameListCtrl::MSWOnNotify(int id, WXLPARAM lparam, WXLPARAM* result)
{
  ...
  case CDDS_SUBITEM | CDDS_ITEMPREPAINT:  // About to paint a column (nmlv->iSubItem) on the current
    ...
    {
      nmlv->clrText = GetItemTextColour(nmlv->nmcd.dwItemSpec).GetRGB();
      nmlv->clrTextBk = GetItemBackgroundColour(nmlv->nmcd.dwItemSpec).GetRGB();
      wxFont font{GetItemFont(nmlv->nmcd.dwItemSpec)};
      if (!font.IsOk())
        font = GetFont();
      ::SelectObject(nmlv->nmcd.hdc, font.GetHFONT());
      *result = CDRF_NEWFONT;
      return true;
    }
  }
