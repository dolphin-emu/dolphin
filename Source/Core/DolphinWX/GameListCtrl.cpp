...
public:
  CompressionProgress(int items_done_, int items_total_,
                      const std::string &current_filename_,
                      wxProgressDialog *dialog_)
      : items_done(items_done_), items_total(items_total_),
        current_filename(current_filename_), dialog(dialog_) {}
...
static int CompareGameListItems(const GameListItem *iso1,
  ...
  case CGameListCtrl::COLUMN_MAKER:
    return strcasecmp(iso1->GetCompany().c_str(), iso2->GetCompany().c_str()) *
           t;
  ...
  case CGameListCtrl::COLUMN_ID:
    return strcasecmp(iso1->GetUniqueID().c_str(),
                      iso2->GetUniqueID().c_str()) *
           t;
...
CGameListCtrl::CGameListCtrl(wxWindow *parent, const wxWindowID id,
  ...
  Bind(wxEVT_MENU, &CGameListCtrl::OnOpenContainingFolder, this,
       IDM_OPEN_CONTAINING_FOLDER);
  Bind(wxEVT_MENU, &CGameListCtrl::OnOpenSaveFolder, this,
  ...
  Bind(wxEVT_MENU, &CGameListCtrl::OnMultiCompressISO, this,
       IDM_MULTI_COMPRESS_ISO);
  Bind(wxEVT_MENU, &CGameListCtrl::OnMultiDecompressISO, this,
  ...
  wxTheApp->Bind(DOLPHIN_EVT_LOCAL_INI_CHANGED,
                 &CGameListCtrl::OnLocalIniModified, this);
}

CGameListCtrl::~CGameListCtrl() { ClearIsoFiles(); }
...
void CGameListCtrl::InitBitmaps() {
  ...
  m_EmuStateImageIndex[0] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating0", size));
  m_EmuStateImageIndex[1] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating1", size));
  m_EmuStateImageIndex[2] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating2", size));
  m_EmuStateImageIndex[3] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating3", size));
  m_EmuStateImageIndex[4] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating4", size));
  m_EmuStateImageIndex[5] =
      img_list->Add(WxUtils::LoadResourceBitmap("rating5", size));
}
...
void CGameListCtrl::Update() {
  ...
  if (m_ISOFiles.size() != 0) {
    ...
    SetColumnWidth(COLUMN_PLATFORM,
                   SConfig::GetInstance().m_showSystemColumn
                       ? 32 + platform_icon_padding + platform_padding
                       : 0);
    SetColumnWidth(COLUMN_BANNER, SConfig::GetInstance().m_showBannerColumn
                                      ? 96 + platform_padding
                                      : 0);
    SetColumnWidth(COLUMN_TITLE, 175 + platform_padding);
    SetColumnWidth(COLUMN_MAKER, SConfig::GetInstance().m_showMakerColumn
                                     ? 150 + platform_padding
                                     : 0);
    SetColumnWidth(COLUMN_FILENAME, SConfig::GetInstance().m_showFileNameColumn
                                        ? 100 + platform_padding
                                        : 0);
    SetColumnWidth(COLUMN_ID, SConfig::GetInstance().m_showIDColumn
                                  ? 75 + platform_padding
                                  : 0);
    SetColumnWidth(COLUMN_COUNTRY, SConfig::GetInstance().m_showRegionColumn
                                       ? 32 + platform_padding
                                       : 0);
    SetColumnWidth(
    ...
    for (int i = 0; i < (int)m_ISOFiles.size(); i++) {
      ...
      if (SConfig::GetInstance().m_ColorCompressed &&
          m_ISOFiles[i]->IsCompressed())
    ...
    SetColumnWidth(COLUMN_SIZE, SConfig::GetInstance().m_showSizeColumn
                                    ? wxLIST_AUTOSIZE
                                    : 0);
  } else {
    ...
    if (IsHidingItems()) {
      errorString = _("Dolphin is currently set to hide all games. "
                      "Double-click here to show all games...");
    } else {
      errorString = _("Dolphin could not find any GameCube/Wii ISOs or WADs. "
...
static wxString NiceSizeFormat(u64 size) {
  ...
  const char *const unit_symbols[] = {"B",   "KiB", "MiB", "GiB",
                                      "TiB", "PiB", "EiB"};
...
void CGameListCtrl::UpdateItemAtColumn(long _Index, int column) {
  ...
  case COLUMN_TITLE: {
    ...
    if (disc_number > 1 &&
        name.Lower().find(wxString::Format("disc %i", disc_number)) ==
            std::string::npos &&
        name.Lower().find(wxString::Format("disc%i", disc_number)) ==
            std::string::npos) {
...
void CGameListCtrl::SetBackgroundColor() {
  for (long i = 0; i < GetItemCount(); i++) {
    wxColour color =
        (i & 1) ? blend50(wxSystemSettings::GetColour(wxSYS_COLOUR_3DLIGHT),
...
void CGameListCtrl::ScanForISOs() {
  ...
  OpenFStream(titlestxt, File::GetUserPath(D_LOAD_IDX) + "titles.txt",
              std::ios::in);

  if (!titlestxt.is_open())
    OpenFStream(titlestxt, File::GetUserPath(D_LOAD_IDX) + "wiitdb.txt",
                std::ios::in);
  ...
  if (rFilenames.size() > 0) {
    wxProgressDialog dialog(
        _("Scanning for ISOs"), _("Scanning..."), (int)rFilenames.size() - 1,
        this,
    ...
    for (u32 i = 0; i < rFilenames.size(); i++) {
      ...
      dialog.Update(i,
                    wxString::Format(_("Scanning %s"), StrToWxStr(FileName)));
      if (dialog.WasCancelled())
        break;

      auto iso_file =
          std::make_unique<GameListItem>(rFilenames[i], custom_title_map);
...
void CGameListCtrl::OnColBeginDrag(wxListEvent &event) {
  ...
  if (column_id != COLUMN_TITLE && column_id != COLUMN_MAKER &&
...
static int wxCALLBACK wxListCompare(wxIntPtr item1, wxIntPtr item2,
                                    wxIntPtr sortData) {
...
void CGameListCtrl::OnMouseMotion(wxMouseEvent &event) {
  ...
  if (item != wxNOT_FOUND) {
    ...
    if (Rect.GetX() + Rect.GetWidth() - GetColumnWidth(COLUMN_EMULATION_STATE) <
        event.GetX())
#endif
    {
      ...
      // Emulation status
      static const char *const emuState[] = {"Broken", "Intro", "In-Game",
                                             "Playable", "Perfect"};
      ...
      toolTip->SetBoundingRect(
          wxRect(mx - GetColumnWidth(COLUMN_EMULATION_STATE), my,
...
void CGameListCtrl::OnRightClick(wxMouseEvent &event) {
  ...
  if (GetSelectedItemCount() == 1) {
    ...
    if (selected_iso) {
      ...
      if (platform == DiscIO::IVolume::WII_DISC ||
          platform == DiscIO::IVolume::WII_WAD) {
      ...
      popupMenu.Append(IDM_OPEN_CONTAINING_FOLDER,
                       _("Open &containing folder"));

      if (platform != DiscIO::IVolume::ELF_DOL)
        popupMenu.AppendCheckItem(IDM_SET_DEFAULT_ISO,
                                  _("Set as &default ISO"));
      ...
      if (platform == DiscIO::IVolume::GAMECUBE_DISC ||
          platform == DiscIO::IVolume::WII_DISC) {
        ...
        wxMenuItem *changeDiscItem =
            popupMenu.Append(IDM_LIST_CHANGE_DISC, _("Change &Disc"));
    ...
    popupMenu.Append(IDM_MULTI_DECOMPRESS_ISO,
...
void CGameListCtrl::OnExportSave(wxCommandEvent &WXUNUSED(event)) {
  ...
  std::unique_ptr<DiscIO::IVolume> volume(
      DiscIO::CreateVolumeFromFilename(iso->GetFileName()));
...
void CGameListCtrl::OnDeleteISO(wxCommandEvent &WXUNUSED(event)) {
  const wxString message = GetSelectedItemCount() == 1
                               ...
                               : _("Are you sure you want to delete these "
                                   "files? They will be gone forever!");

  if (wxMessageBox(message, _("Warning"), wxYES_NO | wxICON_EXCLAMATION) ==
      wxYES) {
...
void CGameListCtrl::OnWiki(wxCommandEvent &WXUNUSED(event)) {
  ...
  std::string wikiUrl =
      "https://wiki.dolphin-emu.org/dolphin-redirect.php?gameid=" +
      iso->GetUniqueID();
  WxUtils::Launch(wikiUrl);
}

bool CGameListCtrl::MultiCompressCB(const std::string &text, float percent,
                                    void *arg) {
  CompressionProgress *progress = static_cast<CompressionProgress *>(arg);

  float total_percent =
      ((float)progress->items_done + percent) / (float)progress->items_total;
  wxString text_string(StrToWxStr(StringFromFormat(
      "%s (%i/%i) - %s", progress->current_filename.c_str(),
      progress->items_done + 1, progress->items_total, text.c_str())));

  return progress->dialog->Update(total_percent * progress->dialog->GetRange(),
                                  text_string);
}
...
void CGameListCtrl::CompressSelection(bool _compress) {
  ...
  for (const GameListItem *iso : GetAllSelectedISOs()) {
    ...
    // Show the Wii compression warning if it's relevant and it hasn't been
    // shown already
...

  {
    wxProgressDialog progressDialog(
        ...
        _("Working..."),
        1000, // Arbitrary number that's larger than the dialog's width in
              // pixels
        this, wxPD_APP_MODAL | wxPD_CAN_ABORT | wxPD_ELAPSED_TIME |
                  wxPD_ESTIMATED_TIME | wxPD_REMAINING_TIME | wxPD_SMOOTH);

    CompressionProgress progress(0, items_to_compress.size(), "",
                                 &progressDialog);
    ...
    for (const GameListItem *iso : items_to_compress) {
      if (!iso->IsCompressed() && _compress) {
        ...
        BuildCompleteFilename(OutputFileName,
                              WxStrToStr(browseDialog.GetPath()), FileName);
        ...
        all_good &= DiscIO::CompressFileToBlob(
            iso->GetFileName(), OutputFileName,
            (iso->GetPlatform() == DiscIO::IVolume::WII_DISC) ? 1 : 0, 16384,
  ...
  if (!all_good)
    WxUtils::ShowErrorDialog(
        _("Dolphin was unable to complete the requested action."));

  Update();
}

bool CGameListCtrl::CompressCB(const std::string &text, float percent,
                               void *arg) {
...
void CGameListCtrl::OnCompressISO(wxCommandEvent &WXUNUSED(event)) {
  ...
  do {
    if (is_compressed) {
      ...
      path = wxFileSelector(
          _("Save decompressed GCM/ISO"), StrToWxStr(FilePath),
      ...
      path = wxFileSelector(
          _("Save compressed GCM/ISO"), StrToWxStr(FilePath),
...

  {
    ...
    if (is_compressed)
      all_good = DiscIO::DecompressBlobToFile(
          iso->GetFileName(), WxStrToStr(path), &CompressCB, &dialog);
    else
      all_good = DiscIO::CompressFileToBlob(
...
bool CGameListCtrl::WiiCompressWarning() {
  return wxMessageBox(_("Compressing a Wii disc image will irreversibly change "
                        "the compressed copy "
                        "by removing padding data. Your disc image will still "
                        "work. Continue?"),
