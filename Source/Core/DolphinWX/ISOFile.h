...
public:
  ...
  std::string GetName(DiscIO::ELanguage language) const;
  std::string GetName() const;
  std::string GetDescription(DiscIO::ELanguage language) const;
  std::string GetDescription() const;
  std::vector<DiscIO::ELanguage> GetLanguages() const;
  std::string GetCompany() const { return m_company; }
  u16 GetRevision() const { return m_Revision; }
  const std::string& GetUniqueID() const { return m_UniqueID; }
  const std::string GetWiiFSPath() const;
  DiscIO::ECountry GetCountry() const { return m_Country; }
  DiscIO::EPlatform GetPlatform() const { return m_Platform; }
...
private:
  ...
  std::map<DiscIO::ELanguage, std::string> m_names;
  std::map<DiscIO::ELanguage, std::string> m_descriptions;
