...
void GamepadPage::ConfigExtension(wxCommandEvent &event) {
  ControllerEmu::Extension *const ex =
      ((ExtensionButton *)event.GetEventObject())->extension;

  // show config diag, if "none" isn't selected
  if (ex->switch_extension) {
    wxDialog dlg(this, wxID_ANY,
                 wxGetTranslation(StrToWxStr(
                     ex->attachments[ex->switch_extension]->GetName())));

    wxBoxSizer *const main_szr = new wxBoxSizer(wxVERTICAL);
    const std::size_t orig_size = control_groups.size();

    ControlGroupsSizer *const szr =
        new ControlGroupsSizer(ex->attachments[ex->switch_extension].get(),
                               &dlg, this, &control_groups);
    main_szr->Add(szr, 0, wxLEFT, 5);
    main_szr->Add(dlg.CreateButtonSizer(wxOK), 0,
...
PadSettingExtension::PadSettingExtension(wxWindow *const parent,
  ...
  for (auto &attachment : extension->attachments) {
    ((wxChoice *)wxcontrol)
        ->Append(wxGetTranslation(StrToWxStr(attachment->GetName())));
  }
...
PadSettingCheckBox::PadSettingCheckBox(
    wxWindow *const parent,
    ControllerEmu::ControlGroup::Setting *const _setting)
    : PadSetting(new wxCheckBox(parent, wxID_ANY,
                                wxGetTranslation(StrToWxStr(_setting->name)))),
      setting(_setting) {
  UpdateGUI();
}
...
ControlDialog::ControlDialog(GamepadPage *const parent, InputConfig &config,
    ...
    : wxDialog(parent, wxID_ANY, _("Configure Control"), wxDefaultPosition,
  ...
  // wxTE_PROCESS_ENTER);
  device_cbox = new wxComboBox(
      this, wxID_ANY, StrToWxStr(m_devq.ToString()), wxDefaultPosition,
  ...
  wxStaticBoxSizer *const d_szr =
      new wxStaticBoxSizer(wxVERTICAL, this, _("Device"));
...
ControlButton::ControlButton(wxWindow *const parent,
    ...
    : wxButton(parent, wxID_ANY, "", wxDefaultPosition, wxSize(width, 20)),
...
void ControlDialog::UpdateGUI() {
  ...
  m_bound_label->SetLabel(
      wxString::Format(_("Bound Controls: %lu"),
...
inline void GetExpressionForControl(
    wxString &expr, wxString &control_name,
...
void GamepadPage::AdjustSetting(wxCommandEvent &event) {
  ((PadSetting *)((wxControl *)event.GetEventObject())->GetClientData())
      ->UpdateValue();
}
...
void GamepadPage::AdjustControlOption(wxCommandEvent &) {
  m_control_dialog->control_reference->range =
      (ControlState)(m_control_dialog->GetRangeSliderValue()) /
      SLIDER_TICK_COUNT;
}

void GamepadPage::ConfigControl(wxEvent &event) {
  m_control_dialog = new ControlDialog(
      this, m_config,
...
void ControlDialog::DetectControl(wxCommandEvent &event) {
  ...
  if (dev) {
    ...
    ciface::Core::Device::Control *const ctrl =
        control_reference->Detect(DETECT_WAIT_TIME, dev);
...
bool GamepadPage::DetectButton(ControlButton *button) {
  bool success = false;
  // find device :/
  ciface::Core::Device *const dev =
      g_controller_interface.FindDevice(controller->default_device);
  if (dev) {
    m_event_filter.BlockEvents(true);
    ...
    if (ctrl) {
      ...
      g_controller_interface.UpdateReference(button->control_reference,
...
wxStaticBoxSizer *
ControlDialog::CreateControlChooser(GamepadPage *const parent) {
  ...
  textctrl = new wxTextCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition,
                            wxSize(-1, 48), wxTE_MULTILINE | wxTE_RICH2);
  wxFont font = textctrl->GetFont();
  font.SetFamily(wxFONTFAMILY_MODERN);
  textctrl->SetFont(font);

  wxButton *const detect_button = new wxButton(
      this, wxID_ANY, control_reference->is_input ? _("Detect") : _("Test"));
  ...
  control_lbox =
      new wxListBox(this, wxID_ANY, wxDefaultPosition, wxSize(-1, 64));
  ...
  range_slider =
      new wxSlider(this, wxID_ANY, SLIDER_TICK_COUNT, -SLIDER_TICK_COUNT * 5,
                   SLIDER_TICK_COUNT * 5, wxDefaultPosition, wxDefaultSize,
                   wxSL_TOP | wxSL_LABELS /*| wxSL_AUTOTICKS*/);
  ...
  range_slider->Bind(wxEVT_SCROLL_CHANGED, &GamepadPage::AdjustControlOption,
                     parent);
  wxStaticText *const range_label =
      new wxStaticText(this, wxID_ANY, _("Range"));
...
void InputConfigDialog::UpdateDeviceComboBox() {
  ...
  for (GamepadPage *page : m_padpages) {
    ...
    page->device_cbox->SetValue(
        StrToWxStr(page->controller->default_device.ToString()));
  }
}
...
ControlGroupBox::ControlGroupBox(ControllerEmu::ControlGroup *const group,
                                 wxWindow *const parent,
  ...
  const std::vector<std::string> exclude_groups = {
      "IR", "Swing", "Tilt", "Shake", "UDP Wiimote", "Extension", "Rumble"};

  wxFont m_SmallFont(7, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
                     wxFONTWEIGHT_NORMAL);
  for (auto &control : group->controls) {
    wxStaticText *const label = new wxStaticText(
        parent, wxID_ANY, wxGetTranslation(StrToWxStr(control->name)));

    ControlButton *const control_button =
        new ControlButton(parent, control->control_ref.get(), 80);
    control_button->SetFont(m_SmallFont);

    control_buttons.push_back(control_button);
    if (std::find(exclude_groups.begin(), exclude_groups.end(),
                  control_group->name) == exclude_groups.end() &&
        std::find(exclude_buttons.begin(), exclude_buttons.end(),
                  control->name) == exclude_buttons.end())
    ...
    if (control->control_ref->is_input) {
      ...
      control_button->Bind(wxEVT_BUTTON, &GamepadPage::DetectControl,
                           eventsink);
    } else {
      control_button->SetToolTip(
          _("Left/Right-click for more options.\nMiddle-click to clear."));
      control_button->Bind(wxEVT_BUTTON, &GamepadPage::ConfigControl,
                           eventsink);
    }

    control_button->Bind(wxEVT_MIDDLE_DOWN, &GamepadPage::ClearControl,
                         eventsink);
    control_button->Bind(wxEVT_RIGHT_UP, &GamepadPage::ConfigControl,
                         eventsink);
  ...
  case GROUP_TYPE_FORCE: {
    ...
    dc.Clear();
    static_bitmap =
        new wxStaticBitmap(parent, wxID_ANY, bitmap, wxDefaultPosition,
                           wxDefaultSize, wxBITMAP_TYPE_BMP);

    wxBoxSizer *const szr = new wxBoxSizer(wxVERTICAL);
    for (auto &groupSetting : group->settings) {
      PadSettingSpin *setting = new PadSettingSpin(parent, groupSetting.get());
      setting->wxcontrol->Bind(wxEVT_SPINCTRL, &GamepadPage::AdjustSetting,
                               eventsink);
      options.push_back(setting);
      szr->Add(new wxStaticText(
          parent, wxID_ANY, wxGetTranslation(StrToWxStr(groupSetting->name))));
  ...
  case GROUP_TYPE_BUTTONS: {
    ...
    unsigned int button_cols =
        group->controls.size() > 8 ? 8 : group->controls.size();
    ...
    PadSettingSpin *const threshold_cbox =
        new PadSettingSpin(parent, group->settings[0].get());
    threshold_cbox->wxcontrol->Bind(wxEVT_SPINCTRL, &GamepadPage::AdjustSetting,
                                    eventsink);
    ...
    szr->Add(new wxStaticText(parent, wxID_ANY, wxGetTranslation(StrToWxStr(
  ...
  case GROUP_TYPE_EXTENSION: {
    ...
    wxButton *const configure_btn =
        new ExtensionButton(parent, (ControllerEmu::Extension *)group);
  ...
  default: {
    ...
    for (auto &groupSetting : group->settings) {
      if (groupSetting->high == DEFAULT_HIGH_VALUE) {
        PadSettingCheckBox *setting_cbox =
            new PadSettingCheckBox(parent, groupSetting.get());
        if (groupSetting->is_iterate == true) {
          setting_cbox->wxcontrol->Bind(
              wxEVT_CHECKBOX, &GamepadPage::AdjustSettingUI, eventsink);
          groupSetting->value = 0;
        } else {
          setting_cbox->wxcontrol->Bind(wxEVT_CHECKBOX,
                                        &GamepadPage::AdjustSetting, eventsink);
        }
        options.push_back(setting_cbox);
        Add(setting_cbox->wxcontrol, 0, wxALL | wxLEFT, 5);
      } else {
        PadSettingSpin *setting =
            new PadSettingSpin(parent, groupSetting.get());
        setting->wxcontrol->Bind(wxEVT_SPINCTRL, &GamepadPage::AdjustSetting,
                                 eventsink);
        options.push_back(setting);
        wxBoxSizer *const szr = new wxBoxSizer(wxHORIZONTAL);
...
ControlGroupsSizer::ControlGroupsSizer(ControllerEmu *const controller,
  ...
  for (auto &group : controller->groups) {
    ControlGroupBox *control_group_box =
        new ControlGroupBox(group.get(), parent, eventsink);
    wxStaticBoxSizer *control_group = new wxStaticBoxSizer(
        wxVERTICAL, parent, wxGetTranslation(StrToWxStr(group->ui_name)));
...
GamepadPage::GamepadPage(wxWindow *parent, InputConfig &config,
                         const int pad_num,
  ...
  wxBoxSizer *control_group_sizer =
      new ControlGroupsSizer(controller, this, this, &control_groups);

  wxStaticBoxSizer *profile_sbox =
      new wxStaticBoxSizer(wxHORIZONTAL, this, _("Profile"));

  // device chooser

  wxStaticBoxSizer *const device_sbox =
      new wxStaticBoxSizer(wxHORIZONTAL, this, _("Device"));

  device_cbox =
      new wxComboBox(this, wxID_ANY, "", wxDefaultPosition, wxSize(64, -1));
  device_cbox->ToggleWindowStyle(wxTE_PROCESS_ENTER);

  wxButton *refresh_button =
      new wxButton(this, wxID_ANY, _("Refresh"), wxDefaultPosition,
  ...
  wxButton *const default_button =
      new wxButton(this, wxID_ANY, _("Default"), wxDefaultPosition,
                   wxDefaultSize, wxBU_EXACTFIT);
  wxButton *const clearall_button =
      new wxButton(this, wxID_ANY, _("Clear"), wxDefaultPosition, wxDefaultSize,
                   wxBU_EXACTFIT);

  wxStaticBoxSizer *const clear_sbox =
      new wxStaticBoxSizer(wxHORIZONTAL, this, _("Reset"));
  ...
  wxButton *const pload_btn =
      new wxButton(this, wxID_ANY, _("Load"), wxDefaultPosition, wxDefaultSize,
                   wxBU_EXACTFIT);
  wxButton *const psave_btn =
      new wxButton(this, wxID_ANY, _("Save"), wxDefaultPosition, wxDefaultSize,
                   wxBU_EXACTFIT);
...
InputConfigDialog::InputConfigDialog(wxWindow *const parent,
  ...
  szr->Add(CreateButtonSizer(wxOK | wxCANCEL | wxNO_DEFAULT), 0,
           wxEXPAND | wxALL, 5);
