...
void DolphinApp::ParseCommandLine()
{
  ...
  m_watch_server = options.is_set("watch");
  m_watch_server_port = options.get("watch");
}
...
{
  NOTICE_LOG(DOLPHINWATCH, "watch server: %s on port %d", m_watch_server ? "true" : "false", m_watch_server_port);
  if (m_watch_server)
