...
// The 'main program' equivalent that creates the main window and return the
// main frame
...
// do not allow windows to be created off the desktop.
  ...
  if ((leftPos + width) < (x + w) || leftPos > x ||
      (topPos + height) < (y + h) || topPos > y)
    x = y = wxDefaultCoord;
  ...
  main_frame =
      new CFrame(nullptr, wxID_ANY, StrToWxStr(scm_rev_str), wxPoint(x, y),
                 wxSize(w, h), m_use_debugger, m_batch_mode, m_use_logger);
...
void DolphinApp::OnInitCmdLine(wxCmdLineParser &parser) {
  static const wxCmdLineEntryDesc desc[] = {
      ...
      {wxCMD_LINE_SWITCH, "b", "batch", "Exit Dolphin with emulator",
       wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION, "c", "confirm", "Set Confirm on Stop",
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION, "v", "video_backend", "Specify a video backend",
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION, "a", "audio_emulation",
       "Low level (LLE) or high level (HLE) audio", wxCMD_LINE_VAL_STRING,
       wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION, "m", "movie", "Play a movie file",
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION, "u", "user", "User folder path",
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
...
bool DolphinApp::OnCmdLineParsed(wxCmdLineParser &parser) {
  ...
  m_select_audio_emulation =
      parser.Found("audio_emulation", &m_audio_emulation_name);
...
void DolphinApp::InitLanguageSupport() {
  ...
  ini.GetOrCreateSection("Interface")
      ->Get("Language", &language, wxLANGUAGE_DEFAULT);
...
// Specify where dolphins *.gmo files are located on each operating system
    ...
    m_locale->AddCatalogLookupPathPrefix(
        StrToWxStr(File::GetExeDirectory() + DIR_SEP "Languages"));
    ...
    if (!m_locale->IsOk()) {
      wxMessageBox(
          _("Error loading selected language. Falling back to system default."),
          _("Error"));
...
void DolphinApp::OnFatalException() { WiimoteReal::Shutdown(); }
...
bool wxMsgAlert(const char *caption, const char *text, bool yes_no,
                int /*Style*/) {
#ifdef __WXGTK__
  if (wxIsMainThread()) {
#endif
...
CFrame *DolphinApp::GetCFrame() { return main_frame; }
...
void *Host_GetRenderHandle() { return main_frame->GetRenderHandle(); }
...
void Host_SetStartupDebuggingParameters() {
  ...
  StartUp.bEnableDebugging =
      main_frame->g_pCodeWindow ? true : false; // RUNNING_DEBUG
}
...
void Host_SetWiiMoteConnectionState(int _State) {
  ...
  NOTICE_LOG(WIIMOTE, "%s",
             static_cast<const char *>(event.GetString().c_str()));

  main_frame->GetEventHandler()->AddPendingEvent(event);
}

bool Host_UIHasFocus() { return main_frame->UIHasFocus(); }

bool Host_RendererHasFocus() { return main_frame->RendererHasFocus(); }

bool Host_RendererIsFullscreen() { return main_frame->RendererIsFullscreen(); }

void Host_ConnectWiimote(int wm_idx, bool connect) {
  if (connect) {
    wxCommandEvent event(wxEVT_HOST_COMMAND,
                         IDM_FORCE_CONNECT_WIIMOTE1 + wm_idx);
    main_frame->GetEventHandler()->AddPendingEvent(event);
  } else {
    wxCommandEvent event(wxEVT_HOST_COMMAND,
                         IDM_FORCE_DISCONNECT_WIIMOTE1 + wm_idx);
    main_frame->GetEventHandler()->AddPendingEvent(event);
  }
}
...
void Host_ShowVideoConfig(void *parent, const std::string &backend_name,
  ...
  if (backend_name == "Direct3D 11" ||
      backend_name == "Direct3D 12 (experimental)" ||
      backend_name == "OpenGL") {
