...
namespace Vulkan
{
PerfQuery::PerfQuery(StateTracker* state_tracker) : m_state_tracker(state_tracker)
{
  if (!CreateQueryPool())
...
PerfQuery::~PerfQuery()
{
  ...
  if (m_query_pool != VK_NULL_HANDLE)
    vkDestroyQueryPool(g_object_cache->GetDevice(), m_query_pool, nullptr);
}
...
void PerfQuery::EnableQuery(PerfQueryGroup type)
{
  ...
  if (type == PQG_ZCOMP_ZCOMPLOC || type == PQG_ZCOMP)
  {
    ...
    if (g_object_cache->SupportsPreciseOcclusionQueries())
...
bool PerfQuery::CreateQueryPool()
{
  ...
  VkResult res = vkCreateQueryPool(g_object_cache->GetDevice(), &info, nullptr, &m_query_pool);
  if (res != VK_SUCCESS)
  {
