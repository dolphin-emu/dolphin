...
bool CommandBufferManager::CreateCommandBuffers()
{
  ...
  for (auto& [command_pool, command_buffers, fence, semaphore, _fence_counter,
         init_command_buffer_used, semaphore_used, _waiting_for_submit, _frame_index,
         _cleanup_resources] : m_command_buffers)
  {
    ...
    VkCommandBufferAllocateInfo buffer_info = {
        ...
        VK_COMMAND_BUFFER_LEVEL_PRIMARY, static_cast<uint32_t>(command_buffers.size())};

    res = vkAllocateCommandBuffers(device, &buffer_info, command_buffers.data());
    if (res != VK_SUCCESS)
    {
    ...
    res = vkCreateSemaphore(device, &semaphore_create_info, nullptr, &semaphore);
    if (res != VK_SUCCESS)
    {
...
void CommandBufferManager::DestroyCommandBuffers()
{
  ...
  for (auto& [command_pool, _command_buffers, _fence, semaphore, _fence_counter,
    _init_command_buffer_used, _semaphore_used, _waiting_for_submit, _frame_index,
    cleanup_resources] : m_command_buffers)
  {
  ...
  for (auto& [descriptor_pools, _current_descriptor_pool_index] : m_frame_resources)
  {
...
VkDescriptorSet CommandBufferManager::AllocateDescriptorSet(VkDescriptorSetLayout set_layout)
{
  ...
  auto& [descriptor_pools, current_descriptor_pool_index] = GetCurrentFrameResources();

  if (!descriptor_pools.empty()) [[likely]]
  {
    const VkDescriptorSetAllocateInfo allocate_info = {
        VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr,
        descriptor_pools[current_descriptor_pool_index], 1, &set_layout};

    const VkResult res =
        vkAllocateDescriptorSets(g_vulkan_context->GetDevice(), &allocate_info, &descriptor_set);
    if (res != VK_SUCCESS &&
        descriptor_pools.size() > current_descriptor_pool_index + 1)
    {
      // Mark the next descriptor set as active and try again.
      current_descriptor_pool_index++;
  ...
  if (descriptor_set == VK_NULL_HANDLE) [[unlikely]]
  {
    ...
    current_descriptor_pool_index =
        static_cast<u32>(descriptor_pools.size()) - 1;
    descriptor_set = AllocateDescriptorSet(set_layout);
  }
...
bool CommandBufferManager::CreateSubmitThread()
{
  m_submit_thread.Reset("VK submission thread", [this](const PendingCommandBufferSubmit submit) {
    ...
    auto& [_command_pool, _command_buffers, _fence, _semaphore, _fence_counter,
      _init_command_buffer_used, _semaphore_used, waiting_for_submit, _frame_index,
      _cleanup_resources] = m_command_buffers[submit.command_buffer_index];
    waiting_for_submit.store(false, std::memory_order_release);
  });
...
{
  const auto& [_resources_command_pool, _resources_command_buffers, resources_fence,
    _resources_semaphore, resources_fence_counter, _resources_init_command_buffer_used,
    _resources_semaphore_used, resources_waiting_for_submit, _resources_frame_index,
    _resources_cleanup_resources] = m_command_buffers[index];

  // Ensure this command buffer has been submitted.
  if (resources_waiting_for_submit.load(std::memory_order_acquire))
  {
    WaitForWorkerThreadIdle();
    ASSERT_MSG(VIDEO, !resources_waiting_for_submit.load(std::memory_order_relaxed),
               "Submit thread is idle but command buffer is still waiting for submission!");
  }

  // Wait for this command buffer to be completed.
  const VkResult res =
      vkWaitForFences(g_vulkan_context->GetDevice(), 1, &resources_fence, VK_TRUE, UINT64_MAX);
  if (res != VK_SUCCESS)
    LOG_VULKAN_ERROR(res, "vkWaitForFences failed: ");

  // Clean up any resources for command buffers between the last known completed buffer and this
  // now-completed command buffer. If we use >2 buffers, this may be more than one buffer.
  const u64 now_completed_counter = resources_fence_counter;
  u32 cleanup_index = (m_current_cmd_buffer + 1) % NUM_COMMAND_BUFFERS;
  while (cleanup_index != m_current_cmd_buffer)
  {
    auto& [_command_pool, _command_buffers, _fence, _semaphore, cleanup_resources_fence_counter,
      _init_command_buffer_used, _semaphore_used, _waiting_for_submit, _frame_index,
      cleanup_resources] = m_command_buffers[cleanup_index];
    if (cleanup_resources_fence_counter > now_completed_counter)
      break;

    if (cleanup_resources_fence_counter > m_completed_fence_counter)
    {
      for (auto& it : cleanup_resources)
        it();
      cleanup_resources.clear();
    }
                                               ...
                                               const uint32_t present_image_index)
{
  ...
  auto& [_command_pool, command_buffers, _fence, _semaphore, _fence_counter,
    _init_command_buffer_used, _semaphore_used, waiting_for_submit, _frame_index,
    _cleanup_resources] = GetCurrentCmdBufferResources();
  ...
  {
    waiting_for_submit.store(true, std::memory_order_relaxed);
  ...
  if (present_swap_chain != VK_NULL_HANDLE)
  {
    ...
    {
      const auto& [_command_pool, _command_buffers, _fence, _semaphore, fence_counter,
        _init_command_buffer_used, _semaphore_used, _waiting_for_submit, frame_index,
        _cleanup_resources] = m_command_buffers[cmd_buffer_index];
      if (frame_index == m_current_frame && fence_counter != 0 &&
          fence_counter > m_completed_fence_counter)
      {
...
{
  auto& [command_pool, command_buffers, fence, semaphore, _fence_counter, init_command_buffer_used,
        semaphore_used, _waiting_for_submit, _frame_index, _cleanup_resources] =
      m_command_buffers[command_buffer_index];
  ...
  const VkResult res =
      vkQueueSubmit(g_vulkan_context->GetGraphicsQueue(), 1, &submit_info, fence);
  if (res != VK_SUCCESS)
  {
...
void CommandBufferManager::BeginCommandBuffer()
{
  ...
  auto& [command_pool, command_buffers, fence, _semaphore, fence_counter, init_command_buffer_used,
    semaphore_used, _waiting_for_submit, frame_index, _cleanup_resources] =
      m_command_buffers[next_buffer_index];

  // Wait for the GPU to finish with all resources for this command buffer.
  if (fence_counter > m_completed_fence_counter)
    WaitForCommandBufferCompletion(next_buffer_index);

  // Reset fence to unsignaled before starting.
  VkResult res = vkResetFences(g_vulkan_context->GetDevice(), 1, &fence);
  if (res != VK_SUCCESS)
    LOG_VULKAN_ERROR(res, "vkResetFences failed: ");

  // Reset command pools to beginning since we can re-use the memory now
  res = vkResetCommandPool(g_vulkan_context->GetDevice(), command_pool, 0);
  if (res != VK_SUCCESS)
  ...
  init_command_buffer_used = false;
  semaphore_used = false;
  fence_counter = m_next_fence_counter++;
...
{
  auto& [command_pool, _command_buffers, _fence, _semaphore, _fence_counter,
    _init_command_buffer_used, _semaphore_used, _waiting_for_submit, _frame_index,
    cleanup_resources] = GetCurrentCmdBufferResources();
  cleanup_resources.push_back(
...
void CommandBufferManager::DeferBufferDestruction(VkBuffer buffer, VmaAllocation alloc)
{
  auto& [_command_pool, _command_buffers, _fence, _semaphore, _fence_counter,
    _init_command_buffer_used, _semaphore_used, _waiting_for_submit, _frame_index,
    cleanup_resources] = GetCurrentCmdBufferResources();
  cleanup_resources.push_back([buffer, alloc]() {
