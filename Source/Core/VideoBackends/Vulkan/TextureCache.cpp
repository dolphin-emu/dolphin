                                            ...
                                            const MathUtil::Rectangle<int>& src_rect)
{
  ...
  if (src_rect.GetWidth() == dst_rect.GetWidth() && src_rect.GetHeight() == dst_rect.GetHeight())
  {
    ...
    vkCmdCopyImage(command_buffer, src_texture->GetImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                   dst_texture->GetTexture()->GetImage(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                   &image_copy);
  ...
  UtilityShaderDraw draw(
      ...
      TextureCache::GetInstance()->GetRenderPassForTextureUpdate(dst_texture->GetTexture()),
...
TextureCache::TCacheEntry::~TCacheEntry()
{
  ...
  g_state_tracker->UnbindTexture(m_texture->GetView());
...
{
  g_state_tracker->SetTexture(stage, m_texture->GetView());
}
