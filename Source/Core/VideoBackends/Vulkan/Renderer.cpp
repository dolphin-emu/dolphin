...
bool Renderer::Initialize()
{
  ...
  if (g_vulkan_context->SupportsBoundingBox())
  {
    ...
    g_state_tracker->SetBBoxBuffer(m_bounding_box->GetGPUBuffer(),
                           ...
                           bool z_enable, u32 color, u32 z)
{
  ...
  if (g_state_tracker->InRenderPass())
  {
  ...
  if (use_clear_render_pass)
  {
    ...
    g_state_tracker->BeginClearRenderPass(target_vk_rc, clear_values);
    return;
  }
  ...
  // We can't use this when preserving alpha but clearing color.
  {
    ...
    if (num_clear_attachments > 0)
    {
      ...
      if (!g_state_tracker->IsWithinRenderArea(target_vk_rc.offset.x, target_vk_rc.offset.y,
...
void Renderer::BindEFBToStateTracker()
{
  ...
  g_state_tracker->SetRenderPass(FramebufferManager::GetInstance()->GetEFBLoadRenderPass(),
                                 FramebufferManager::GetInstance()->GetEFBClearRenderPass());
  g_state_tracker->SetFramebuffer(FramebufferManager::GetInstance()->GetEFBFramebuffer(),
                                  framebuffer_size);

  // Update rasterization state with MSAA info
  RasterizationState rs_state = {};
  rs_state.bits = g_state_tracker->GetRasterizationState().bits;
  rs_state.samples = FramebufferManager::GetInstance()->GetEFBSamples();
  rs_state.per_sample_shading = g_ActiveConfig.bSSAA ? VK_TRUE : VK_FALSE;
  g_state_tracker->SetRasterizationState(rs_state);
}
...
void Renderer::SetGenerationMode()
{
  ...
  new_rs_state.bits = g_state_tracker->GetRasterizationState().bits;
  ...
  g_state_tracker->SetRasterizationState(new_rs_state);
}
...
void Renderer::SetDepthMode()
{
  ...
  g_state_tracker->SetDepthStencilState(new_ds_state);
}
...
void Renderer::SetColorMask()
{
  ...
  new_blend_state.bits = g_state_tracker->GetBlendState().bits;
  new_blend_state.write_mask = color_mask;

  g_state_tracker->SetBlendState(new_blend_state);
}
...
void Renderer::SetSamplerState(int stage, int texindex, bool custom_tex)
{
  ...
  g_state_tracker->SetSampler(bind_index, sampler);
...
void Renderer::ResetSamplerStates()
{
  ...
  {
    m_sampler_states[i].bits = std::numeric_limits<decltype(m_sampler_states[i].bits)>::max();
    g_state_tracker->SetSampler(i, g_object_cache->GetPointSampler());
  }
...
void Renderer::SetScissorRect(const EFBRectangle& rc)
{
  ...
  g_state_tracker->SetScissor(scissor);
}
...
void Renderer::SetViewport()
{
  ...
  g_state_tracker->SetViewport(viewport);
}
