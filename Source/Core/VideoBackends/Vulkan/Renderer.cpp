                             ...
                             u32 fb_stride, u32 fb_height, u64 ticks)
{
  ...
  StagingTexture2D* readback_texture =
      PrepareFrameDumpImage(m_current_frame_dump_image, width, height, ticks);
  if (!readback_texture)
    return false;
...
void Renderer::WriteFrameDumpImage(size_t index)
{
  ...
  AVIDump::Frame state = AVIDump::FetchState(frame.ticks);
  FinishFrameData();
  DumpFrameData(reinterpret_cast<const u8*>(frame.readback_texture->GetMapPointer()),
                ...
                static_cast<int>(frame.readback_texture->GetRowStride()), state);

  frame.pending = false;
}

StagingTexture2D* Renderer::PrepareFrameDumpImage(size_t index, u32 width, u32 height, u64 ticks)
{
  FrameDumpImage& image = m_frame_dump_images[index];

  // If there is an image that hasn't been written to the frame dump yet, write it.
  if (image.pending)
    WriteFrameDumpImage(index);
  ...
  // This frame is queued, so use the next buffer for the next frame.
