...
bool RasterFont::CreateTexture()
{
  ...
  m_texture = Texture2D::Create(m_object_cache, m_command_buffer_mgr,
                                CHAR_WIDTH * CHAR_COUNT, CHAR_HEIGHT,
  ...
  uint32_t memory_type_index = m_object_cache->GetMemoryType(memory_requirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
  ...
  res = vkAllocateMemory(m_object_cache->GetDevice(), &memory_allocate_info, nullptr, &temp_buffer_memory);
  if (res != VK_SUCCESS)
  {
  ...
  // Copy into buffer
  void* mapped_ptr;
  res = vkMapMemory(m_object_cache->GetDevice(), temp_buffer_memory, 0, buffer_info.size, 0, &mapped_ptr);
  if (res != VK_SUCCESS)
  {
  ...
  {
    0, CHAR_WIDTH * CHAR_COUNT, CHAR_HEIGHT,
    { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 },
    { 0, 0, 0 }, { CHAR_WIDTH * CHAR_COUNT, CHAR_HEIGHT, 1 }
  };
  m_texture->TransitionToLayout(m_command_buffer_mgr->GetCurrentCommandBuffer(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  vkCmdCopyBufferToImage(m_command_buffer_mgr->GetCurrentCommandBuffer(),
                         temp_buffer,
                         m_texture->GetImage(),
  ...
  m_texture->TransitionToLayout(m_command_buffer_mgr->GetCurrentCommandBuffer(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
{
  m_vertex_shader = m_object_cache->GetVertexShaderCache().CompileAndCreateShader(VERTEX_SHADER_SOURCE);
  m_fragment_shader = m_object_cache->GetPixelShaderCache().CompileAndCreateShader(FRAGMENT_SHADER_SOURCE);
...
void RasterFont::PrintMultiLineText(VkRenderPass render_pass,
                                    ...
                                    u32 color)
{
  ...
  UtilityShaderDraw draw(m_object_cache,
  ...
  UtilityShaderVertex* vertices = draw.ReserveVertices(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
                                                       text.length() * 6);
