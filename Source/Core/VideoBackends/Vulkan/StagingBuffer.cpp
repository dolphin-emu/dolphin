...
StagingBuffer::Create(STAGING_BUFFER_TYPE type, VkDeviceSize size, VkBufferUsageFlags usage)
{
  ...
  vkGetBufferMemoryRequirements(g_object_cache->GetDevice(), buffer, &requirements);

  bool is_coherent;
  u32 type_index;
  if (type == STAGING_BUFFER_TYPE_UPLOAD)
    type_index = g_object_cache->GetUploadMemoryType(requirements.memoryTypeBits, &is_coherent);
  else
    type_index = g_object_cache->GetReadbackMemoryType(requirements.memoryTypeBits, &is_coherent);
