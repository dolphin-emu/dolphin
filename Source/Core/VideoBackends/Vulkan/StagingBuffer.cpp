...
void StagingBuffer::InvalidateGPUCache(VkCommandBuffer command_buffer,
    VkAccessFlagBits dest_access_flags, VkPipelineStageFlagBits dest_pipeline_stage,
...
void StagingBuffer::PrepareForGPUWrite(VkCommandBuffer command_buffer,
    VkAccessFlagBits dst_access_flags, VkPipelineStageFlagBits dst_pipeline_stage,
    VkDeviceSize offset, VkDeviceSize size)
{
  ASSERT((offset + size) <= m_size || (offset < m_size && size == VK_WHOLE_SIZE));
...
void StagingBuffer::Write(
    VkDeviceSize offset, const void* data, size_t size, bool invalidate_caches)
{
  ASSERT((offset + size) <= m_size);
...
bool StagingBuffer::AllocateBuffer(STAGING_BUFFER_TYPE type, VkDeviceSize size,
    VkBufferUsageFlags usage, VkBuffer* out_buffer, VmaAllocation* out_alloc, char** out_map_ptr)
{
  ...
  if (type == STAGING_BUFFER_TYPE_UPLOAD)
  {
    ...
    vmaGetMemoryTypeProperties(
        g_vulkan_context->GetMemoryAllocator(), alloc_info.memoryType, &flags);
...
std::unique_ptr<StagingBuffer> StagingBuffer::Create(
