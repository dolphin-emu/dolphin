...
PaletteTextureConverter::PaletteTextureConverter(ObjectCache* object_cache, CommandBufferManager* command_buffer_mgr, StateTracker* state_tracker)
...
void PaletteTextureConverter::ConvertTexture(Texture2D* dst_texture, VkFramebuffer dst_framebuffer, Texture2D* src_texture, u32 width, u32 height, void* palette, TlutFormat format)
{
  ...
  if (!uniform_buffer->ReserveMemory(sizeof(PSUniformBlock), m_object_cache->GetUniformBufferAlignment()) ||
      !m_palette_stream_buffer->ReserveMemory(palette_size, m_object_cache->GetTexelBufferAlignment()) ||
      (descriptor_set = m_command_buffer_mgr->AllocateDescriptorSet(m_set_layout)) == VK_NULL_HANDLE)
  {
  ...
  VkDescriptorBufferInfo uniform_buffer_info = { m_object_cache->GetUtilityShaderUniformBuffer()->GetBuffer(), 0, sizeof(PSUniformBlock) };
  VkDescriptorImageInfo sampler_info = { m_object_cache->GetPointSampler(), src_texture->GetView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
  ...
  {
    {
      ...
      nullptr, &uniform_buffer_info, nullptr
    },
    {
      VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
      descriptor_set, 0, 1,
      1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
      &sampler_info, nullptr, nullptr
    },
    {
      VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
      descriptor_set, 0, 1,
      2, VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
      nullptr, nullptr, &m_palette_buffer_view
    }
  };
  vkUpdateDescriptorSets(m_object_cache->GetDevice(), ARRAYSIZE(set_writes), set_writes, 0, nullptr);
  ...
  {
    VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr, 0,
    dst_framebuffer, { { 0, 0 }, { width, height } },
    0, nullptr
  };
  vkCmdBeginRenderPass(command_buffer, &render_pass_begin, VK_SUBPASS_CONTENTS_INLINE);
  vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelines[format]);
  vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1, &descriptor_set, 1, &uniforms_offset);
...
bool PaletteTextureConverter::CreateBuffers()
{
  ...
  m_palette_stream_buffer = StreamBuffer::Create(m_object_cache, m_command_buffer_mgr,
  ...
  VkResult res = vkCreateBufferView(m_object_cache->GetDevice(),
                                    &view_info,
                                    nullptr,
...
bool PaletteTextureConverter::CompileShaders()
{
  ...
  std::string palette_ia8_program = StringFromFormat("%s\n%s", "#define DECODE DecodePixel_IA8", PALETTE_CONVERSION_FRAGMENT_SHADER_SOURCE);
  std::string palette_rgb565_program = StringFromFormat("%s\n%s", "#define DECODE DecodePixel_RGB565", PALETTE_CONVERSION_FRAGMENT_SHADER_SOURCE);
  std::string palette_rgb5a3_program = StringFromFormat("%s\n%s", "#define DECODE DecodePixel_RGB5A3", PALETTE_CONVERSION_FRAGMENT_SHADER_SOURCE);
  if ((m_shaders[GX_TL_IA8] = m_object_cache->GetPixelShaderCache().CompileAndCreateShader(palette_ia8_program)) == nullptr ||
      (m_shaders[GX_TL_RGB565] = m_object_cache->GetPixelShaderCache().CompileAndCreateShader(palette_rgb565_program)) == nullptr ||
      (m_shaders[GX_TL_RGB5A3] = m_object_cache->GetPixelShaderCache().CompileAndCreateShader(palette_rgb5a3_program)) == nullptr)
  {
    return false;
  }
...
bool PaletteTextureConverter::CreateRenderPass()
{
	...
	VkResult res = vkCreateRenderPass(m_object_cache->GetDevice(), &pass_info, nullptr, &m_render_pass);
	if (res != VK_SUCCESS)
	{
...
bool PaletteTextureConverter::CreateDescriptorLayout()
{
  ...
  static const VkDescriptorSetLayoutCreateInfo set_info =
  {
    ...
    0, ARRAYSIZE(set_bindings), set_bindings
  };

  VkResult res = vkCreateDescriptorSetLayout(m_object_cache->GetDevice(), &set_info, nullptr, &m_set_layout);
  if (res != VK_SUCCESS)
  {
  ...
  res = vkCreatePipelineLayout(m_object_cache->GetDevice(), &pipeline_layout_info, nullptr, &m_pipeline_layout);
  if (res != VK_SUCCESS)
  {
...
bool PaletteTextureConverter::CreatePipelines()
{
  ...
  for (u32 i = 0; i < NUM_PALETTE_CONVERSION_SHADERS; i++)
  {
    ...
    {
      {
        ...
        m_object_cache->GetStaticShaderCache().GetScreenQuadVertexShader(), // VkShaderModule                  module
    ...
    VkResult res = vkCreateGraphicsPipelines(m_object_cache->GetDevice(),
                                             VK_NULL_HANDLE,
                                             1,
                                             &pipeline_info,
                                             nullptr,
