...
static const int VCHAR_WIDTH = 8;
static const int VCHAR_HEIGHT = 13;
static const int VCHAR_OFFSET = 32;
static const int VCHAR_COUNT = 95;

static const u8 rasters[VCHAR_COUNT][VCHAR_HEIGHT] = {
...
static const VCHAR *s_vertexShaderSrc =
	"uniform vec2 VCHARSize;\n"
	"uniform vec2 offset;"
	"in vec2 rawpos;\n"
	"in vec2 tex0;\n"
	"out vec2 uv0;\n"
...
static const VCHAR *s_fragmentShaderSrc =
...
RasterFont::RasterFont()
{
	...
	std::vector<u32> texture_data(VCHAR_WIDTH * VCHAR_COUNT * VCHAR_HEIGHT);
	for (int y = 0; y < VCHAR_HEIGHT; y++)
	{
		for (int c = 0; c < VCHAR_COUNT; c++)
		{
			for (int x = 0; x < VCHAR_WIDTH; x++)
			{
				bool pixel = (0 != (rasters[c][y] & (1 << (VCHAR_WIDTH - x - 1))));
				texture_data[VCHAR_WIDTH * VCHAR_COUNT * y + VCHAR_WIDTH * c + x] = pixel ? -1 : 0;
			}
		}
	}
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, VCHAR_WIDTH * VCHAR_COUNT, VCHAR_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, texture_data.data());
	...
	// bound uniforms
	glUniform2f(glGetUniformLocation(s_shader.glprogid,"VCHARSize"), 1.0f / GLfloat(VCHAR_COUNT), 1.0f);
...
void RasterFont::printMultilineText(const std::string& text, double start_x, double start_y, double z, int bbWidth, int bbHeight, u32 color)
{
	...
	int usage = 0;
	GLfloat delta_x = GLfloat(2 * VCHAR_WIDTH) / GLfloat(bbWidth);
	GLfloat delta_y = GLfloat(2 * VCHAR_HEIGHT) / GLfloat(bbHeight);
	...
	for (const TCHAR& c : text)
	{
		...
		if (c < VCHAR_OFFSET || c >= VCHAR_COUNT + VCHAR_OFFSET)
			continue;

		vertices[usage++] = x;
		vertices[usage++] = y;
		vertices[usage++] = GLfloat(c - VCHAR_OFFSET);
		vertices[usage++] = 0.0f;

		vertices[usage++] = x + delta_x;
		vertices[usage++] = y;
		vertices[usage++] = GLfloat(c - VCHAR_OFFSET + 1);
		vertices[usage++] = 0.0f;
