...
void SHADER::SetProgramBindings() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    glBindAttribLocation(glprogid, SHADER_TEXTURE0_ATTRIB + i,
                         attrib_name.c_str());
  }
}
...
void ProgramShaderCache::UploadConstants() {
  if (PixelShaderManager::dirty || VertexShaderManager::dirty ||
    ...
    memcpy(buffer.first, &PixelShaderManager::constants,
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 2, s_buffer->m_buffer,
                      buffer.second +
                          ROUND_UP(sizeof(PixelShaderConstants), s_ubo_align),
    ...
    glBindBufferRange(
        GL_UNIFORM_BUFFER, 4, s_buffer->m_buffer,
...
SHADER *ProgramShaderCache::SetShader(DSTALPHA_MODE dstAlphaMode,
                                      u32 primitive_type) {
  SHADERUID uid;
  ...
  ShaderCode vcode =
      GenerateVertexShaderCode(API_OPENGL, uid.vuid.GetUidData());
  // ShaderCode pcode = GeneratePixelShaderCode(dstAlphaMode, API_OPENGL,
  // uid.puid.GetUidData());
  ShaderCode pcode = UberShader::GenPixelShader(
      dstAlphaMode, API_OPENGL, false, g_ActiveConfig.iMultisamples > 1,
  ...
  if (g_ActiveConfig.iLog & CONF_SAVESHADERS) {
    ...
    std::string filename = StringFromFormat(
        "%svs_%04i.txt", File::GetUserPath(D_DUMP_IDX).c_str(), counter++);
    SaveData(filename, vcode.GetBuffer());

    filename = StringFromFormat(
        "%sps_%04i.txt", File::GetUserPath(D_DUMP_IDX).c_str(), counter++);
    SaveData(filename, pcode.GetBuffer());

    if (!gcode.GetBuffer().empty()) {
      filename = StringFromFormat(
          "%sgs_%04i.txt", File::GetUserPath(D_DUMP_IDX).c_str(), counter++);
...
bool ProgramShaderCache::CompileShader(SHADER &shader, const std::string &vcode,
  ...
  if (linkStatus != GL_TRUE || (length > 1 && DEBUG_GLSL)) {
    ...
    std::string filename =
        StringFromFormat("%sbad_p_%d.txt",
    ...
    if (linkStatus != GL_TRUE) {
      PanicAlert("Failed to link shaders: %s\n"
                 "Debug info (%s, %s, %s):\n%s",
                 filename.c_str(), g_ogl_config.gl_vendor,
                 g_ogl_config.gl_renderer, g_ogl_config.gl_version, infoLog);
    }
...
GLuint ProgramShaderCache::CompileSingleShader(GLuint type,
  ...
  if (compileStatus != GL_TRUE || (length > 1 && DEBUG_GLSL)) {
    ...
    ERROR_LOG(VIDEO, "%s Shader info log:\n%s",
              type == GL_VERTEX_SHADER ? "VS" : type == GL_FRAGMENT_SHADER
                                                    ? "PS"
                                                    : "GS",
              infoLog);
    ...
    std::string filename = StringFromFormat(
        ...
        type == GL_VERTEX_SHADER ? "vs" : type == GL_FRAGMENT_SHADER ? "ps"
                                                                     : "gs",
        num_failures++);
    std::ofstream file;
    OpenFStream(file, filename, std::ios_base::out);
    ...
    if (compileStatus != GL_TRUE) {
      PanicAlert("Failed to compile %s shader: %s\n"
                 ...
                 type == GL_VERTEX_SHADER
                     ? "vertex"
                     : type == GL_FRAGMENT_SHADER ? "pixel" : "geometry",
                 filename.c_str(), g_ogl_config.gl_vendor,
                 g_ogl_config.gl_renderer, g_ogl_config.gl_version, infoLog);
    }
...
void ProgramShaderCache::GetShaderId(SHADERUID *uid, DSTALPHA_MODE dstAlphaMode,
                                     u32 primitive_type) {
  ...
  if (g_ogl_config.bSupportsGLSLCache) {
    ...
    if (!Supported) {
      ...
      std::string cache_filename =
          StringFromFormat("%sogl-%s-shaders.cache",
...
void ProgramShaderCache::Shutdown() {
  ...
  if (g_ogl_config.bSupportsGLSLCache) {
    for (auto &entry : pshaders) {
      ...
      glGetProgramiv(entry.second.shader.glprogid, GL_LINK_STATUS,
                     &link_status);
      glGetProgramiv(entry.second.shader.glprogid, GL_DELETE_STATUS,
                     &delete_status);
      glGetProgramiv(entry.second.shader.glprogid, GL_PROGRAM_BINARY_LENGTH,
                     &binary_size);
      if (glGetError() != GL_NO_ERROR || link_status == GL_FALSE ||
          delete_status == GL_TRUE || !binary_size) {
        continue;
      }
      ...
      glGetProgramBinary(entry.second.shader.glprogid, binary_size, nullptr,
                         prog_format, binary);
      if (glGetError() != GL_NO_ERROR) {
        continue;
      }

      g_program_disk_cache.Append(entry.first, &data[0],
                                  binary_size + sizeof(GLenum));
    }
...
void ProgramShaderCache::ProgramShaderCacheInserter::Read(const SHADERUID &key,
                                                          const u8 *value,
                                                          u32 value_size) {
