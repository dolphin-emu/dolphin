...
static const u32 EFB_CACHE_HEIGHT =
    (EFB_HEIGHT + EFB_CACHE_RECT_SIZE - 1) / EFB_CACHE_RECT_SIZE;
static bool s_efbCacheValid[2][EFB_CACHE_WIDTH * EFB_CACHE_HEIGHT];
static bool s_efbCacheIsCleared = false;
static std::vector<u32>
    s_efbCache[2][EFB_CACHE_WIDTH *
                  EFB_CACHE_HEIGHT]; // 2 for PEEK_Z and PEEK_COLOR
...
static void APIENTRY ErrorCallback(GLenum source, GLenum type, GLuint id,
  ...
  case GL_DEBUG_SEVERITY_HIGH_ARB:
    ERROR_LOG(HOST_GPU, "id: %x, source: %s, type: %s - %s", id, s_source,
              s_type, message);
    break;
  case GL_DEBUG_SEVERITY_MEDIUM_ARB:
    WARN_LOG(HOST_GPU, "id: %x, source: %s, type: %s - %s", id, s_source,
             s_type, message);
    break;
  ...
  case GL_DEBUG_SEVERITY_NOTIFICATION:
    DEBUG_LOG(HOST_GPU, "id: %x, source: %s, type: %s - %s", id, s_source,
              s_type, message);
    break;
  default:
    ERROR_LOG(HOST_GPU, "id: %x, source: %s, type: %s - %s", id, s_source,
...
static void APIENTRY ClearDepthf(GLfloat depthval) { glClearDepth(depthval); }
...
static void InitDriverInfo() {
  ...
  if (svendor == "NVIDIA Corporation" && srenderer != "NVIDIA Tegra") {
    vendor = DriverDetails::VENDOR_NVIDIA;
             ...
             svendor == "Advanced Micro Devices, Inc.") {
  ...
  case DriverDetails::VENDOR_QUALCOMM: {
    ...
    double glVersion;
    sscanf(g_ogl_config.gl_version, "OpenGL ES %lg V@%lg", &glVersion,
           &version);
  } break;
  ...
  case DriverDetails::VENDOR_MESA: {
    ...
    int minor = 0;
    int release = 0;
    sscanf(g_ogl_config.gl_version, "%*s (Core Profile) Mesa %d.%d.%d", &major,
           &minor, &release);
  ...
  case DriverDetails::VENDOR_INTEL: // Happens in OS X/Windows
  {
    ...
    sscanf(g_ogl_config.gl_version, "%d.%d.0 - Build %d.%d.%d.%d", &glmajor,
           &glminor, &major, &minor, &release, &revision);
  ...
  case DriverDetails::VENDOR_NVIDIA: {
    ...
    sscanf(g_ogl_config.gl_version, "%d.%d.%d NVIDIA %d.%d", &glmajor, &glminor,
           &glrelease, &major, &minor);
...
Renderer::Renderer() {
  ...
  g_Config.backend_info.bSupportsPrimitiveRestart =
      ...
      ((GLExtensions::Version() >= 310) ||
       GLExtensions::Supports("GL_NV_primitive_restart"));
  ...
  g_Config.backend_info.bSupportsSSAA =
      GLExtensions::Supports("GL_ARB_gpu_shader5") &&
  ...
  g_Config.backend_info.bSupportsClipControl =
      GLExtensions::Supports("GL_ARB_clip_control");
  g_ogl_config.bSupportsCopySubImage =
      (GLExtensions::Supports("GL_ARB_copy_image") ||
  ...
  g_ogl_config.bSupportsGLBaseVertex =
      GLExtensions::Supports("GL_ARB_draw_elements_base_vertex") ||
      GLExtensions::Supports("GL_EXT_draw_elements_base_vertex") ||
      GLExtensions::Supports("GL_OES_draw_elements_base_vertex");
  g_ogl_config.bSupportsGLBufferStorage =
      GLExtensions::Supports("GL_ARB_buffer_storage") ||
      GLExtensions::Supports("GL_EXT_buffer_storage");
  g_ogl_config.bSupportsMSAA =
      GLExtensions::Supports("GL_ARB_texture_multisample");
  g_ogl_config.bSupportViewportFloat =
      GLExtensions::Supports("GL_ARB_viewport_array");
  ...
  g_ogl_config.bSupportsConservativeDepth =
      GLExtensions::Supports("GL_ARB_conservative_depth");
  g_ogl_config.bSupportsAniso =
      GLExtensions::Supports("GL_EXT_texture_filter_anisotropic");
  ...
  if (GLInterface->GetMode() == GLInterfaceMode::MODE_OPENGLES3) {
    ...
    g_ogl_config.SupportedESTextureBuffer =
        GLExtensions::Supports("VERSION_GLES_3_2")
    ...
    if (strstr(g_ogl_config.glsl_version, "3.0")) {
      ...
      g_ogl_config.bSupportsAEP =
          GLExtensions::Supports("GL_ANDROID_extension_pack_es31a");
      ...
      g_Config.backend_info.bSupportsGSInstancing =
          g_Config.backend_info.bSupportsGeometryShaders &&
          g_ogl_config.SupportedESPointSize > 0;
      ...
      if (g_ActiveConfig.iStereoMode > 0 && g_ActiveConfig.iMultisamples > 1 &&
        ...
        OSD::AddMessage("MSAA Stereo rendering isn't supported by your GPU.",
                        10000);
    ...
    if (strstr(g_ogl_config.glsl_version, "1.00") ||
        strstr(g_ogl_config.glsl_version, "1.10") ||
  ...
  if (g_ogl_config.bSupportsDebug) {
    ...
    if (LogManager::GetInstance()->IsEnabled(LogTypes::HOST_GPU,
                                             LogTypes::LERROR))
  ...
  OSD::AddMessage(
      StringFromFormat("Video Info: %s, %s, %s", g_ogl_config.gl_vendor,
                       g_ogl_config.gl_renderer, g_ogl_config.gl_version),
      5000);

  WARN_LOG(
      VIDEO, "Missing OGL Extensions: %s%s%s%s%s%s%s%s%s%s%s%s%s",
  ...
  glViewport(0, 0, GetTargetWidth(),
             GetTargetHeight()); // Reset The Current Viewport
...
void Renderer::Init() {
  ...
  g_framebuffer_manager = std::make_unique<FramebufferManager>(
      s_target_width, s_target_height, s_MSAASamples);
...
void Renderer::RenderText(const std::string &text, int left, int top,
                          u32 color) {
  ...
  s_raster_font->printMultilineText(
      text, left * 2.0f / (float)nBackbufferWidth - 1,
...
void Renderer::UpdateEFBCache(EFBAccessType type, u32 cacheRectIdx,
                              const EFBRectangle &efbPixelRc,
                              const TargetRectangle &targetPixelRc,
                              const void *data) {
  u32 cacheType = (type == PEEK_Z ? 0 : 1);

  if (!s_efbCache[cacheType][cacheRectIdx].size())
    s_efbCache[cacheType][cacheRectIdx].resize(EFB_CACHE_RECT_SIZE *
                                               EFB_CACHE_RECT_SIZE);
  ...
  for (u32 yCache = 0; yCache < efbPixelRcHeight; ++yCache) {
    ...
    u32 yPixel = (EFBToScaledY(EFB_HEIGHT - yEFB) +
                  EFBToScaledY(EFB_HEIGHT - yEFB - 1)) /
                 2;
    ...
    for (u32 xCache = 0; xCache < efbPixelRcWidth; ++xCache) {
      ...
      if (type == PEEK_Z) {
        ...
        value = MathUtil::Clamp<u32>(
            (u32)(ptr[yData * targetPixelRcWidth + xData] * 16777216.0f), 0,
            0xFFFFFF);
      } else {
        u32 *ptr = (u32 *)data;
        value = ptr[yData * targetPixelRcWidth + xData];
      }
      s_efbCache[cacheType][cacheRectIdx]
                [yCache * EFB_CACHE_RECT_SIZE + xCache] = value;
    }
  }
...
u32 Renderer::AccessEFB(EFBAccessType type, u32 x, u32 y, u32 poke_data) {
  u32 cacheRectIdx =
      (y / EFB_CACHE_RECT_SIZE) * EFB_CACHE_WIDTH + (x / EFB_CACHE_RECT_SIZE);
  ...
  if (type == PEEK_COLOR || type == PEEK_Z) {
    ...
    efbPixelRc.right =
        std::min(efbPixelRc.left + EFB_CACHE_RECT_SIZE, (u32)EFB_WIDTH);
    efbPixelRc.bottom =
        std::min(efbPixelRc.top + EFB_CACHE_RECT_SIZE, (u32)EFB_HEIGHT);
  } else {
    efbPixelRc.left = x;
    efbPixelRc.top = y;
    ...
    if (!s_efbCacheValid[0][cacheRectIdx]) {
      if (s_MSAASamples > 1) {
        ...
        glBindFramebuffer(GL_READ_FRAMEBUFFER,
                          FramebufferManager::GetResolvedFramebuffer());

        g_renderer->RestoreAPIState();
      }

      std::unique_ptr<float[]> depthMap(
          new float[targetPixelRcWidth * targetPixelRcHeight]);

      glReadPixels(targetPixelRc.left, targetPixelRc.bottom, targetPixelRcWidth,
                   targetPixelRcHeight, GL_DEPTH_COMPONENT, GL_FLOAT,
                   depthMap.get());

      UpdateEFBCache(type, cacheRectIdx, efbPixelRc, targetPixelRc,
                     depthMap.get());
    }
  ...
  case PEEK_COLOR: // GXPeekARGB
  {
    ...
    if (!s_efbCacheValid[1][cacheRectIdx]) {
      ...
      std::unique_ptr<u32[]> colorMap(
          new u32[targetPixelRcWidth * targetPixelRcHeight]);

      if (GLInterface->GetMode() == GLInterfaceMode::MODE_OPENGLES3)
        // XXX: Swap colours
        glReadPixels(targetPixelRc.left, targetPixelRc.bottom,
                     targetPixelRcWidth, targetPixelRcHeight, GL_RGBA,
                     GL_UNSIGNED_BYTE, colorMap.get());
      else
        glReadPixels(targetPixelRc.left, targetPixelRc.bottom,
                     targetPixelRcWidth, targetPixelRcHeight, GL_BGRA,
                     GL_UNSIGNED_INT_8_8_8_8_REV, colorMap.get());
    ...
    u32 yRect = y % EFB_CACHE_RECT_SIZE;
    u32 color =
        s_efbCache[1][cacheRectIdx][yRect * EFB_CACHE_RECT_SIZE + xRect];

    // check what to do with the alpha channel (GX_PokeAlphaRead)
    PixelEngine::UPEAlphaReadReg alpha_read_mode =
        PixelEngine::GetAlphaReadMode();
...
void Renderer::PokeEFB(EFBAccessType type, const EfbPokeData *points,
                       size_t num_points) {
...
void Renderer::SetViewport() {
  ...
  float X = EFBToScaledXf(xfmem.viewport.xOrig - xfmem.viewport.wd -
                          (float)scissorXOff);
  float Y = EFBToScaledYf((float)EFB_HEIGHT - xfmem.viewport.yOrig +
                          xfmem.viewport.ht + (float)scissorYOff);
  ...
  float GLNear = MathUtil::Clamp<float>(
                     ...
                     0.0f, 16777215.0f) /
                 16777216.0f;
  float GLFar = MathUtil::Clamp<float>(xfmem.viewport.farZ, 0.0f, 16777215.0f) /
                16777216.0f;
  if (Width < 0) {
    X += Width;
    Width *= -1;
...
void Renderer::ClearScreen(const EFBRectangle &rc, bool colorEnable,
  ...
  glClearColor(float((color >> 16) & 0xFF) / 255.0f,
  ...
  glScissor(targetRc.left, targetRc.bottom, targetRc.GetWidth(),
            targetRc.GetHeight());
...
void Renderer::BlitScreen(TargetRectangle src, TargetRectangle dst,
                          GLuint src_texture, int src_width, int src_height) {
  if (g_ActiveConfig.iStereoMode == STEREO_SBS ||
      g_ActiveConfig.iStereoMode == STEREO_TAB) {
    ...
    m_post_processor->BlitFromTexture(src, leftRc, src_texture, src_width,
                                      src_height, 0);
    m_post_processor->BlitFromTexture(src, rightRc, src_texture, src_width,
                                      src_height, 1);
  } else {
    m_post_processor->BlitFromTexture(src, dst, src_texture, src_width,
                                      src_height);
  }
}
...
void Renderer::ReinterpretPixelData(unsigned int convtype) {
  if (convtype == 0 || convtype == 2) {
    ...
    ERROR_LOG(
        VIDEO,
        "Trying to reinterpret pixel data with unsupported conversion type %d",
        convtype);
  }
}
...
void Renderer::SetBlendMode(bool forceUpdate) {
  ...
  bool useDualSource =
      useDstAlpha && g_ActiveConfig.backend_info.bSupportsDualSourceBlend;
...
void Renderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbStride, u32 fbHeight,
  ...
  if (Fifo::WillSkipCurrentFrame() ||
      (!XFBWrited && !g_ActiveConfig.RealXFBEnabled()) || !fbWidth ||
      !fbHeight) {
  ...
  if (g_ActiveConfig.bUseXFB) {
    ...
    for (u32 i = 0; i < xfbCount; ++i) {
      ...
      if (g_ActiveConfig.bUseRealXFB) {
        ...
        int hOffset =
            ((s32)xfbSource->srcAddr - (s32)xfbAddr) / ((s32)fbStride * 2);

        drawRc.top =
            flipped_trc.top - hOffset * flipped_trc.GetHeight() / (s32)fbHeight;
        drawRc.bottom =
            flipped_trc.top -
            (hOffset + xfbHeight) * flipped_trc.GetHeight() / (s32)fbHeight;
        drawRc.left = flipped_trc.left +
                      (flipped_trc.GetWidth() -
                       xfbWidth * flipped_trc.GetWidth() / (s32)fbStride) /
                          2;
      ...
      BlitScreen(sourceRc, drawRc, xfbSource->texture, xfbSource->texWidth,
                 xfbSource->texHeight);
    }
  } else {
...
// Frame dumps are handled a little differently in Windows
  ...
  if (SConfig::GetInstance().m_DumpFrames) {
    ...
    glReadPixels(flipped_trc.left, flipped_trc.bottom, w, h, GL_RGBA,
                 GL_UNSIGNED_BYTE, &frame_data[0]);
    if (w > 0 && h > 0) {
      if (!bLastFrameDumped) {
  ...
  glViewport(0, 0, GLInterface->GetBackBufferWidth(),
             GLInterface->GetBackBufferHeight());
...
bool Renderer::SaveScreenshot(const std::string &filename,
                              const TargetRectangle &back_rc) {
  u32 W = back_rc.GetWidth();
  u32 H = back_rc.GetHeight();
  std::unique_ptr<u8[]> data(new u8[W * 4 * H]);
  glPixelStorei(GL_PACK_ALIGNMENT, 1);

  glReadPixels(back_rc.left, back_rc.bottom, W, H, GL_RGBA, GL_UNSIGNED_BYTE,
               data.get());
