...
void PerfQuery::EnableQuery(PerfQueryGroup type) { m_query->EnableQuery(type); }

void PerfQuery::DisableQuery(PerfQueryGroup type) {
  m_query->DisableQuery(type);
}

bool PerfQuery::IsFlushed() const { return 0 == m_query_count; }

// TODO: could selectively flush things, but I don't think that will do much
void PerfQuery::FlushResults() { m_query->FlushResults(); }
...
void PerfQueryGL::EnableQuery(PerfQueryGroup type) {
  ...
  if (type == PQG_ZCOMP_ZCOMPLOC || type == PQG_ZCOMP) {
    auto &entry = m_query_buffer[(m_query_read_pos + m_query_count) %
                                 m_query_buffer.size()];
...
void PerfQueryGL::FlushOne() {
  ...
  m_results[entry.query_type] += (u64)result * EFB_WIDTH /
...
void PerfQueryGLESNV::WeakFlush() {
  while (!IsFlushed()) {
    ...
    GLuint result = GL_FALSE;
    glGetOcclusionQueryuivNV(entry.query_id, GL_PIXEL_COUNT_AVAILABLE_NV,
                             &result);
...
void PerfQueryGLESNV::FlushOne() {
  ...
  GLuint result = 0;
  glGetOcclusionQueryuivNV(entry.query_id, GL_OCCLUSION_TEST_RESULT_HP,
                           &result);
