...
void BoundingBox::Init() {
  if (g_ActiveConfig.backend_info.bSupportsBBox) {
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, 4 * sizeof(s32), initial_values,
                 GL_DYNAMIC_DRAW);
...
void BoundingBox::Set(int index, int value) {
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, s_bbox_buffer_id);
  glBufferSubData(GL_SHADER_STORAGE_BUFFER, index * sizeof(int), sizeof(int),
                  &value);
...
int BoundingBox::Get(int index) {
  ...
  if (!DriverDetails::HasBug(DriverDetails::BUG_SLOWGETBUFFERSUBDATA)) {
    ...
    // slow
    // on nVidia drivers. This is more noticeable at higher internal
    // resolutions.
    // Using glGetBufferSubData instead does not seem to exhibit this slowdown.
    glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, index * sizeof(int),
                       sizeof(int), &data);
  } else {
    // Using glMapBufferRange is faster on AMD cards by a measurable margin.
    void *ptr = glMapBufferRange(GL_SHADER_STORAGE_BUFFER, index * sizeof(int),
                                 sizeof(int), GL_MAP_READ_BIT);
    if (ptr) {
