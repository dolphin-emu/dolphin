...
void SamplerCache::SetSamplerState(int stage, const TexMode0 &tm0,
                                   const TexMode1 &tm1, bool custom_tex) {
  ...
  // TODO: Should keep a circular buffer for each stage of recently used
  // samplers.

  auto &active_sampler = m_active_samplers[stage];
  if (active_sampler.first != params || !active_sampler.second.sampler_id) {
    // Active sampler does not match parameters (or is invalid), bind the proper
    // one.
...
SamplerCache::Value &SamplerCache::GetEntry(const Params &params) {
  ...
  if (!val.sampler_id) {
    ...
    // TODO: Maybe kill old samplers if the cache gets huge. It doesn't seem to
    // get huge though.
...
void SamplerCache::SetParameters(GLuint sampler_id, const Params &params) {
  ...
  if (GLInterface->GetMode() == GLInterfaceMode::MODE_OPENGL)
    glSamplerParameterf(sampler_id, GL_TEXTURE_LOD_BIAS,
                        (s32)tm0.lod_bias / 32.f);
  ...
  if (g_ActiveConfig.iMaxAnisotropy > 0 && g_ogl_config.bSupportsAniso &&
    ...
    min_filter = SamplerCommon::AreBpTexMode0MipmapsEnabled(tm0)
                     ? GL_LINEAR_MIPMAP_LINEAR
                     : GL_LINEAR;
