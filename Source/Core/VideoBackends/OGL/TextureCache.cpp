...
bool TextureCache::TCacheEntry::Save(const std::string &filename,
                                     unsigned int level) {
  return SaveTexture(filename, GL_TEXTURE_2D_ARRAY, texture, config.width,
                     config.height, level);
}

TextureCache::TCacheEntryBase *
TextureCache::CreateTexture(const TCacheEntryConfig &config) {
  ...
  if (config.rendertarget) {
    for (u32 level = 0; level <= config.levels; level++) {
      glTexImage3D(GL_TEXTURE_2D_ARRAY, level, GL_RGBA, config.width,
                   config.height, config.layers, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                   nullptr);
    }
    glGenFramebuffers(1, &entry->framebuffer);
    FramebufferManager::SetFramebuffer(entry->framebuffer);
    FramebufferManager::FramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                           GL_TEXTURE_2D_ARRAY, entry->texture,
                                           0);
  }
...
void TextureCache::TCacheEntry::CopyRectangleFromTexture(
    const TCacheEntryBase *source, const MathUtil::Rectangle<int> &srcrect,
  ...
  if (srcrect.GetWidth() == dstrect.GetWidth() &&
    ...
    glCopyImageSubData(srcentry->texture, GL_TEXTURE_2D_ARRAY, 0, srcrect.left,
                       srcrect.top, 0, texture, GL_TEXTURE_2D_ARRAY, 0,
                       dstrect.left, dstrect.top, 0, dstrect.GetWidth(),
                       dstrect.GetHeight(), srcentry->config.layers);
    return;
  } else if (!framebuffer) {
  ...
  glViewport(dstrect.left, dstrect.top, dstrect.GetWidth(),
             dstrect.GetHeight());
  s_ColorCopyProgram.Bind();
  glUniform4f(s_ColorCopyPositionUniform, float(srcrect.left),
              float(srcrect.top), float(srcrect.GetWidth()),
...
void TextureCache::TCacheEntry::Load(unsigned int width, unsigned int height,
  ...
  if (level >= config.levels)
    PanicAlert("Texture only has %d levels, can't update level %d",
               config.levels, level);
  if (width != std::max(1u, config.width >> level) ||
    ...
    PanicAlert("size of level %d must be %dx%d, but %dx%d requested", level,
               std::max(1u, config.width >> level),
               std::max(1u, config.height >> level), width, height);
  ...
  glTexImage3D(GL_TEXTURE_2D_ARRAY, level, GL_RGBA, width, height, 1, 0,
...
void TextureCache::TCacheEntry::FromRenderTarget(
    u8 *dstPointer, PEControl::PixelFormat srcFormat,
  ...
  glUniform4f(uniform_location, static_cast<float>(R.left),
              static_cast<float>(R.top), static_cast<float>(R.right),
...
void TextureCache::CopyEFB(u8 *dst, u32 format, u32 native_width,
  ...
  TextureConverter::EncodeToRamFromTexture(
      dst, format, native_width, bytes_per_row, num_blocks_y, memory_stride,
      srcFormat, isIntensity, scaleByHalf, srcRect);
}
...
TextureCache::TextureCache() {
  ...
  if (g_ActiveConfig.backend_info.bSupportsPaletteConversion) {
    ...
    s_palette_stream_buffer =
        StreamBuffer::Create(GL_TEXTURE_BUFFER, buffer_size);
...
void TextureCache::CompileShaders() {
  ...
  constexpr const char *color_matrix_program =
      ...
      "	ocol0 = texcol * mat4(colmat[0], colmat[1], colmat[2], "
      "colmat[3]) + colmat[4];\n"
      "}\n";
  ...
  constexpr const char *vertex_program =
      ...
      "	%s_uv0 = vec3(mix(copy_position.xy, copy_position.zw, rawpos) / "
      "vec2(textureSize(samp9, "
      "0).xy), 0.0);\n"
      "	gl_Position = vec4(rawpos*2.0-1.0, 0.0, 1.0);\n"
      "}\n";

  const std::string geo_program =
  ...
  s_ColorMatrixUniform =
      glGetUniformLocation(s_ColorMatrixProgram.glprogid, "colmat");
  s_DepthMatrixUniform =
      glGetUniformLocation(s_DepthMatrixProgram.glprogid, "colmat");
  s_ColorCbufid = -1;
  s_DepthCbufid = -1;

  s_ColorCopyPositionUniform =
      glGetUniformLocation(s_ColorCopyProgram.glprogid, "copy_position");
  s_ColorMatrixPositionUniform =
      glGetUniformLocation(s_ColorMatrixProgram.glprogid, "copy_position");
  s_DepthCopyPositionUniform =
      glGetUniformLocation(s_DepthMatrixProgram.glprogid, "copy_position");
  ...
  if (g_ActiveConfig.backend_info.bSupportsPaletteConversion) {
    ...
    s_palette_multiplier_uniform[GX_TL_IA8] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_IA8].glprogid, "multiplier");
    s_palette_copy_position_uniform[GX_TL_IA8] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_IA8].glprogid, "copy_position");
    ...
    s_palette_multiplier_uniform[GX_TL_RGB565] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_RGB565].glprogid, "multiplier");
    s_palette_copy_position_uniform[GX_TL_RGB565] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_RGB565].glprogid, "copy_position");
    ...
    s_palette_multiplier_uniform[GX_TL_RGB5A3] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_RGB5A3].glprogid, "multiplier");
    s_palette_copy_position_uniform[GX_TL_RGB5A3] = glGetUniformLocation(
        s_palette_pixel_shader[GX_TL_RGB5A3].glprogid, "copy_position");
  }
}
...
void TextureCache::ConvertTexture(TCacheEntryBase *_entry,
                                  TCacheEntryBase *_unconverted, void *palette,
                                  TlutFormat format) {
  if (!g_ActiveConfig.backend_info.bSupportsPaletteConversion)
    return;
  ...
  glUniform4f(s_palette_copy_position_uniform[format], 0.0f, 0.0f,
              (float)unconverted->config.width,
