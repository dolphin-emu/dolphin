...
OpenGLPostProcessing::~OpenGLPostProcessing() { m_shader.Destroy(); }

void OpenGLPostProcessing::BlitFromTexture(TargetRectangle src,
                                           TargetRectangle dst, int src_texture,
  ...
  glUniform4f(m_uniform_resolution, (float)src_width, (float)src_height,
              1.0f / (float)src_width, 1.0f / (float)src_height);
  glUniform4f(m_uniform_src_rect, src.left / (float)src_width,
              src.bottom / (float)src_height, src.GetWidth() / (float)src_width,
              src.GetHeight() / (float)src_height);
  ...
  if (m_config.IsDirty()) {
    for (auto &it : m_config.GetOptions()) {
      if (it.second.m_dirty) {
        ...
        case PostProcessingShaderConfiguration::ConfigurationOption::
            OptionType::OPTION_BOOL:
          glUniform1i(m_uniform_bindings[it.first], it.second.m_bool_value);
          break;
        case PostProcessingShaderConfiguration::ConfigurationOption::
            OptionType::OPTION_INTEGER:
          switch (it.second.m_integer_values.size()) {
          case 1:
            glUniform1i(m_uniform_bindings[it.first],
          ...
          case 3:
            glUniform3i(
                m_uniform_bindings[it.first], it.second.m_integer_values[0],
          ...
          break;
        case PostProcessingShaderConfiguration::ConfigurationOption::
            OptionType::OPTION_FLOAT:
          switch (it.second.m_float_values.size()) {
          case 1:
            glUniform1f(m_uniform_bindings[it.first],
          ...
          case 3:
            glUniform3f(
                m_uniform_bindings[it.first], it.second.m_float_values[0],
...
void OpenGLPostProcessing::ApplyShader() {
  ...
  if (!ProgramShaderCache::CompileShader(m_shader, s_vertex_shader, code)) {
    ERROR_LOG(VIDEO, "Failed to compile post-processing shader %s",
              m_config.GetShader().c_str());
  ...
  for (const auto &it : m_config.GetOptions()) {
    ...
    m_uniform_bindings[it.first] =
        glGetUniformLocation(m_shader.glprogid, glsl_name.c_str());
  }
  m_initialized = true;
}
...
void OpenGLPostProcessing::CreateHeader() {
  m_glsl_header =
      ...
      "#define SampleOffset(offset) textureOffset(samp9, float3(uv0, layer), "
      "offset)\n"
...
std::string OpenGLPostProcessing::LoadShaderOptions(const std::string &code) {
  ...
  for (const auto &it : m_config.GetOptions()) {
    if (it.second.m_type == PostProcessingShaderConfiguration::
                                ConfigurationOption::OptionType::OPTION_BOOL) {
      glsl_options +=
          StringFromFormat("uniform int     option_%s;\n", it.first.c_str());
    } else if (it.second.m_type ==
               PostProcessingShaderConfiguration::ConfigurationOption::
                   OptionType::OPTION_INTEGER) {
      ...
      if (count == 1)
        glsl_options +=
            StringFromFormat("uniform float   option_%s;\n", it.first.c_str());
      else
