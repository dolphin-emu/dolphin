...
public:
  ...
  static GLuint GetEFBFramebuffer(unsigned int layer = 0) {
    return (layer < m_EFBLayers) ? m_efbFramebuffer[layer]
                                 : m_efbFramebuffer.back();
  }
  ...
  static void FramebufferTexture(GLenum target, GLenum attachment,
                                 GLenum textarget, GLuint texture, GLint level);

  // If in MSAA mode, this will perform a resolve of the specified rectangle,
  ...
  // After calling this, before you render anything else, you MUST bind the
  ...
  static void PokeEFB(EFBAccessType type, const EfbPokeData *points,
                      size_t num_points);
...
private:
  ...
  static GLuint m_efbColorSwap; // will be hot swapped with m_efbColor when
                                // reinterpreting EFB pixel formats
