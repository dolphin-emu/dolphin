...
namespace DX12 {
// YUYV data is packed into half-width RGBA, with Y values in (R,B) and UV in
// (G,A)
...
constexpr size_t XFB_UPLOAD_BUFFER_SIZE =
    D3D::AlignValue(XFB_TEXTURE_WIDTH * sizeof(u32),
...
XFBEncoder::XFBEncoder() {
  ...
  CheckHR(D3D::device12->CreateCommittedResource(
      ...
      &CD3DX12_RESOURCE_DESC::Tex2D(
          DXGI_FORMAT_R8G8B8A8_UNORM, XFB_TEXTURE_WIDTH, XFB_TEXTURE_HEIGHT, 1,
          1, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET),
      D3D12_RESOURCE_STATE_RENDER_TARGET, nullptr, IID_PPV_ARGS(&texture)));

  m_yuyv_texture =
      new D3DTexture2D(texture, TEXTURE_BIND_FLAG_SHADER_RESOURCE |
  ...
  m_upload_buffer = std::make_unique<D3DStreamBuffer>(
      XFB_UPLOAD_BUFFER_SIZE, XFB_UPLOAD_BUFFER_SIZE, nullptr);
...
void XFBEncoder::EncodeTextureToRam(u8 *dst, u32 dst_pitch, u32 dst_height,
  ...
  _assert_msg_(VIDEO,
               dst_width <= MAX_XFB_WIDTH && dst_height <= MAX_XFB_HEIGHT,
  ...
  EncodeParameters parameters = {
      ...
      {0.0f, 0.0f}};
  m_encode_params_buffer->AllocateSpaceInBuffer(
      sizeof(parameters), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
  memcpy(m_encode_params_buffer->GetCPUAddressOfCurrentAllocation(),
         &parameters, sizeof(parameters));

  // Convert RGBA texture to YUYV intermediate texture.
  // Performs downscaling through a linear filter. Probably not ideal, but it's
  // not going to look
  // perfect anyway.
  CD3DX12_RECT src_texture_rect(src_rect.left, src_rect.top, src_rect.right,
                                src_rect.bottom);
  D3D12_RESOURCE_STATES src_texture_state =
      src_texture->GetResourceUsageState();
  m_yuyv_texture->TransitionToResourceState(D3D::current_command_list,
                                            D3D12_RESOURCE_STATE_RENDER_TARGET);
  D3D::current_command_list->OMSetRenderTargets(1, &m_yuyv_texture->GetRTV12(),
                                                FALSE, nullptr);
  D3D::current_command_list->SetGraphicsRootConstantBufferView(
      DESCRIPTOR_TABLE_PS_CBVONE,
      m_encode_params_buffer->GetGPUAddressOfCurrentAllocation());
  ...
  src_texture->TransitionToResourceState(D3D::current_command_list,
                                         src_texture_state);

  // Copy from YUYV intermediate texture to readback buffer. It's likely the
  // pitch here is going to
  // be different to dst_pitch.
  u32 readback_pitch = D3D::AlignValue(dst_width * sizeof(u16),
                                       D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
  D3D12_PLACED_SUBRESOURCE_FOOTPRINT dst_footprint = {
      0,
      {DXGI_FORMAT_R8G8B8A8_UNORM, dst_texture_width, dst_height, 1,
       readback_pitch}};
  ...
  // Wait until the GPU completes the copy. Resets back to known state
  // automatically.
  ...
  for (u32 row = 0; row < dst_height; row++) {
    const u8 *row_src =
        reinterpret_cast<u8 *>(readback_texture_map) + readback_pitch * row;
...
void XFBEncoder::DecodeToTexture(D3DTexture2D *dst_texture, const u8 *src,
                                 u32 src_width, u32 src_height) {
  _assert_msg_(VIDEO,
               src_width <= MAX_XFB_WIDTH && src_height <= MAX_XFB_HEIGHT,
               "XFB source does not exceed maximum size");

  // Copy to XFB upload buffer. Each row has to be done separately due to pitch
  // differences.
  u32 buffer_pitch = D3D::AlignValue(src_width / 2 * sizeof(u32),
                                     D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
  m_upload_buffer->AllocateSpaceInBuffer(
      buffer_pitch * src_height, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
  for (u32 row = 0; row < src_height; row++) {
    const u8 *row_src = src + (src_width * 2) * row;
    u8 *row_dst = reinterpret_cast<u8 *>(
                      m_upload_buffer->GetCPUAddressOfCurrentAllocation()) +
                  buffer_pitch * row;
  ...
  CD3DX12_TEXTURE_COPY_LOCATION src_location(m_upload_buffer->GetBuffer(),
                                             src_footprint);
  ...
  dst_texture->TransitionToResourceState(D3D::current_command_list,
                                         D3D12_RESOURCE_STATE_RENDER_TARGET);
  D3D::current_command_list->OMSetRenderTargets(1, &dst_texture->GetRTV12(),
                                                FALSE, nullptr);
  D3D::SetViewportAndScissor(0, 0, src_width, src_height);
  D3D::DrawShadedTexQuad(m_yuyv_texture, &src_texture_rect, XFB_TEXTURE_WIDTH,
                         XFB_TEXTURE_HEIGHT,
  ...
  dst_texture->TransitionToResourceState(
