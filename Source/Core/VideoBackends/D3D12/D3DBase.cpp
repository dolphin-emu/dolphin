...
HRESULT LoadDXGI() {
  ...
  create_dxgi_factory =
      (CREATEDXGIFACTORY)GetProcAddress(s_dxgi_dll, "CreateDXGIFactory");
...
HRESULT LoadD3D() {
  ...
  if (!s_d3d12_dll) {
    MessageBoxA(nullptr, "Failed to load d3d12.dll", "Critical error",
                MB_OK | MB_ICONERROR);
    --s_d3d12_dll_ref;
    return E_FAIL;
  }
  ...
  d3d12_create_device =
      (D3D12CREATEDEVICE)GetProcAddress(s_d3d12_dll, "D3D12CreateDevice");
  if (d3d12_create_device == nullptr) {
    MessageBoxA(nullptr, "GetProcAddress failed for D3D12CreateDevice!",
                "Critical error", MB_OK | MB_ICONERROR);
    return E_FAIL;
  }

  d3d12_serialize_root_signature = (D3D12SERIALIZEROOTSIGNATURE)GetProcAddress(
      s_d3d12_dll, "D3D12SerializeRootSignature");
  if (d3d12_serialize_root_signature == nullptr) {
    MessageBoxA(nullptr,
                "GetProcAddress failed for D3D12SerializeRootSignature!",
                "Critical error", MB_OK | MB_ICONERROR);
    return E_FAIL;
  }

  d3d12_get_debug_interface = (D3D12GETDEBUGINTERFACE)GetProcAddress(
      s_d3d12_dll, "D3D12GetDebugInterface");
  if (d3d12_get_debug_interface == nullptr) {
    MessageBoxA(nullptr, "GetProcAddress failed for D3D12GetDebugInterface!",
                "Critical error", MB_OK | MB_ICONERROR);
    return E_FAIL;
  }
...
HRESULT LoadD3DCompiler() {
  ...
  if (!s_d3d_compiler_dll) {
    ...
    if (!s_d3d_compiler_dll) {
      MessageBoxA(
          nullptr,
          "Failed to load D3DCompiler_42.dll, update your DX12 runtime, please",
          "Critical error", MB_OK | MB_ICONERROR);
      return E_FAIL;
    } else {
  ...
  d3d_create_blob =
      (D3DCREATEBLOB)GetProcAddress(s_d3d_compiler_dll, "D3DCreateBlob");
  if (d3d_create_blob == nullptr)
    MessageBoxA(nullptr, "GetProcAddress failed for D3DCreateBlob!",
                "Critical error", MB_OK | MB_ICONERROR);
...
std::vector<DXGI_SAMPLE_DESC> EnumAAModes(ID3D12Device *device) {
  ...
  for (int samples = 0; samples < D3D12_MAX_MULTISAMPLE_SAMPLE_COUNT;
       ++samples) {
...
HRESULT Create(HWND wnd) {
  ...
  // Enabling the debug layer will fail if the Graphics Tools feature is not
  // installed.
  ...
  if (SUCCEEDED(hr)) {
    ...
    MessageBox(wnd, _T("WARNING: Failed to enable D3D12 debug layer, please ")
                    _T("ensure the Graphics ")
  ...
  hr = d3d12_create_device(adapter, D3D_FEATURE_LEVEL_11_0,
  ...
  CheckHR(device12->CreateCommandQueue(&command_queue_desc,
                                       IID_PPV_ARGS(&command_queue)));

  CheckHR(
      factory->CreateSwapChain(command_queue, &swap_chain_desc, &s_swap_chain));

  s_current_back_buf = 0;

  // Query the monitor refresh rate, to ensure proper Present throttling
  // behavior.
  DEVMODE dev_mode;
  ...
  command_list_mgr = std::make_unique<D3DCommandListManager>(
      D3D12_COMMAND_LIST_TYPE_DIRECT, device12, command_queue);
  ...
  for (UINT i = 0; i < SWAP_CHAIN_BUFFER_COUNT; i++) {
    ...
    s_backbuf[i] = new D3DTexture2D(
        buf12, TEXTURE_BIND_FLAG_RENDER_TARGET, DXGI_FORMAT_UNKNOWN,
  ...
  s_backbuf[s_current_back_buf]->TransitionToResourceState(
      current_command_list, D3D12_RESOURCE_STATE_RENDER_TARGET);
  current_command_list->OMSetRenderTargets(
      1, &s_backbuf[s_current_back_buf]->GetRTV12(), FALSE, nullptr);
  ...
  OSD::AddMessage(StringFromFormat(
      "Using D3D Adapter: %s.", UTF16ToUTF8(adapter_desc.Description).c_str()));
...
void CreateDescriptorHeaps() {
...

  {
    ...
    gpu_descriptor_heap_mgr = std::make_unique<D3DDescriptorHeapManager>(
        &gpu_descriptor_heap_desc, device12, 50000);

    gpu_descriptor_heaps[0] = gpu_descriptor_heap_mgr->GetDescriptorHeap();

    D3D12_CPU_DESCRIPTOR_HANDLE descriptor_heap_cpu_base =
        gpu_descriptor_heap_mgr->GetDescriptorHeap()
            ->GetCPUDescriptorHandleForHeapStart();

    resource_descriptor_size = device12->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    sampler_descriptor_size = device12->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);

    D3D12_GPU_DESCRIPTOR_HANDLE null_srv_gpu = {};
    gpu_descriptor_heap_mgr->Allocate(&null_srv_cpu, &null_srv_gpu,
                                      &null_srv_cpu_shadow);
    ...
    for (UINT i = 0; i < 500000; i++) {
      ...
      destination_descriptor.ptr =
          descriptor_heap_cpu_base.ptr + i * resource_descriptor_size;

      device12->CreateShaderResourceView(NULL, &null_srv_desc,
...

  {
    ...
    sampler_descriptor_heap_desc.Flags =
    ...
    sampler_descriptor_heap_mgr = std::make_unique<D3DDescriptorHeapManager>(
        &sampler_descriptor_heap_desc, device12);
...

  {
    ...
    dsv_descriptor_heap_mgr = std::make_unique<D3DDescriptorHeapManager>(
        &dsv_descriptor_heap_desc, device12);
  }

  {
    // D3D12TODO: Temporary workaround.. really need to properly suballocate out
    // of render target
    // heap.
    ...
    rtv_descriptor_heap_mgr = std::make_unique<D3DDescriptorHeapManager>(
        &rtv_descriptor_heap_desc, device12);
  }
}
...
void CreateRootSignatures() {
  D3D12_DESCRIPTOR_RANGE desc_range_srv = {
      ...
      D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND // UINT
                                           // OffsetInDescriptorsFromTableStart;
  };

  D3D12_DESCRIPTOR_RANGE desc_range_sampler = {
      D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, // D3D12_DESCRIPTOR_RANGE_TYPE
                                           // RangeType;
      8,                                   // UINT NumDescriptors;
      0,                                   // UINT BaseShaderRegister;
      0,                                   // UINT RegisterSpace;
  ...
  root_parameters[DESCRIPTOR_TABLE_PS_SRV].DescriptorTable.NumDescriptorRanges =
      1;
  root_parameters[DESCRIPTOR_TABLE_PS_SRV].DescriptorTable.pDescriptorRanges =
      &desc_range_srv;
  root_parameters[DESCRIPTOR_TABLE_PS_SRV].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;

  root_parameters[DESCRIPTOR_TABLE_PS_SAMPLER].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  root_parameters[DESCRIPTOR_TABLE_PS_SAMPLER]
      .DescriptorTable.NumDescriptorRanges = 1;
  root_parameters[DESCRIPTOR_TABLE_PS_SAMPLER]
      .DescriptorTable.pDescriptorRanges = &desc_range_sampler;
  root_parameters[DESCRIPTOR_TABLE_PS_SAMPLER].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;

  root_parameters[DESCRIPTOR_TABLE_GS_CBV].ParameterType =
  ...
  root_parameters[DESCRIPTOR_TABLE_GS_CBV].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_GEOMETRY;

  root_parameters[DESCRIPTOR_TABLE_VS_CBV].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_CBV;
  ...
  root_parameters[DESCRIPTOR_TABLE_VS_CBV].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_VERTEX;

  root_parameters[DESCRIPTOR_TABLE_PS_CBVONE].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_CBV;
  ...
  root_parameters[DESCRIPTOR_TABLE_PS_CBVONE].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;

  root_parameters[DESCRIPTOR_TABLE_PS_CBVTWO].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_CBV;
  ...
  root_parameters[DESCRIPTOR_TABLE_PS_CBVTWO].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;

  root_parameters[DESCRIPTOR_TABLE_PS_UAV].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  root_parameters[DESCRIPTOR_TABLE_PS_UAV].DescriptorTable.NumDescriptorRanges =
      1;
  root_parameters[DESCRIPTOR_TABLE_PS_UAV].DescriptorTable.pDescriptorRanges =
      &desc_range_uav;
  root_parameters[DESCRIPTOR_TABLE_PS_UAV].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;
  ...
  CheckHR(d3d12_serialize_root_signature(
      &root_signature_desc, D3D_ROOT_SIGNATURE_VERSION_1,
      &text_root_signature_blob, &text_root_signature_error_blob));

  CheckHR(D3D::device12->CreateRootSignature(
      0, text_root_signature_blob->GetBufferPointer(),
...
void Close() {
  ...
  if ((!s_debug_device12 && remaining_references) ||
      (s_debug_device12 && remaining_references > 1)) {
  ...
  if (s_debug_device12) {
    --remaining_references; // the debug interface increases the refcount of the
                            // device, subtract
                            // that.
...
const std::string VertexShaderVersionString() { return "vs_5_0"; }

const std::string GeometryShaderVersionString() { return "gs_5_0"; }

const std::string PixelShaderVersionString() { return "ps_5_0"; }

D3DTexture2D *&GetBackBuffer() { return s_backbuf[s_current_back_buf]; }

unsigned int GetBackBufferWidth() { return s_xres; }

unsigned int GetBackBufferHeight() { return s_yres; }
...
void Reset() {
  ...
  // This will also final-release all pending resources (including the
  // backbuffer above)
  command_list_mgr->ExecuteQueuedWork(true);
  ...
  // The 'about-to-be-presented' back buffer index is always set back to '0'
  // upon ResizeBuffers,
  // just like
...
void Present() {
  // The Present function contains logic to ensure we never Present faster than
  // Windows can
  // send to the monitor. If we Present too fast, the Present call will start to
  // block, and we'll be
  // throttled - obviously not desired if vsync is disabled and the emulated CPU
  // speed is > 100%.

  // The throttling logic ensures that we don't Present more than twice in a
  // given monitor vsync.
  // This is accomplished through timing data - there is a programmatic way to
  // determine if a
  // Present call will block, however after investigation that is not feasible
  // here (without
  // invasive
  // workarounds), due to the fact this method does not actually call Present -
  // we just queue a
  // Present
  ...
  if (g_ActiveConfig.IsVSync() == false &&
      time_elapsed_since_last_present <
          (1.0 / static_cast<double>(s_monitor_refresh_rate)) / 2.0) {
    present_flags = DXGI_PRESENT_TEST; // Causes Present to be a no-op.
  } else {
    s_last_present_qpc = current_qpc;

    s_backbuf[s_current_back_buf]->TransitionToResourceState(
        current_command_list, D3D12_RESOURCE_STATE_PRESENT);
    s_current_back_buf = (s_current_back_buf + 1) % SWAP_CHAIN_BUFFER_COUNT;
  }

  command_list_mgr->ExecuteQueuedWorkAndPresent(
      s_swap_chain, g_ActiveConfig.IsVSync() ? 1 : 0, present_flags);

  command_list_mgr->m_cpu_access_last_frame =
      command_list_mgr->m_cpu_access_this_frame;
  command_list_mgr->m_cpu_access_this_frame = false;
  command_list_mgr->m_draws_since_last_execution = 0;
}

HRESULT SetFullscreenState(bool enable_fullscreen) { return S_OK; }

HRESULT GetFullscreenState(bool *fullscreen_state) {
  // Fullscreen exclusive intentionally not supported in DX12 backend. No
  // performance
  // difference between it and windowed full-screen due to usage of a FLIP swap
  // chain.
