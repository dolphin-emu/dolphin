...
namespace DX12 {
PerfQuery::PerfQuery() {
  D3D12_QUERY_HEAP_DESC desc = {D3D12_QUERY_HEAP_TYPE_OCCLUSION,
                                PERF_QUERY_BUFFER_SIZE, 0};
  ...
  m_tracking_fence = D3D::command_list_mgr->RegisterQueueFenceCallback(
      this, &PerfQuery::QueueFenceCallback);
}
...
void PerfQuery::EnableQuery(PerfQueryGroup type) {
  ...
  if (type == PQG_ZCOMP_ZCOMPLOC || type == PQG_ZCOMP) {
    ...
    D3D::current_command_list->BeginQuery(
        m_query_heap, D3D12_QUERY_TYPE_OCCLUSION, static_cast<UINT>(index));
...
void PerfQuery::DisableQuery(PerfQueryGroup type) {
  if (type == PQG_ZCOMP_ZCOMPLOC || type == PQG_ZCOMP) {
    size_t index =
        (m_query_read_pos + m_query_count + m_query_buffer.size() - 1) %
        m_query_buffer.size();
    auto &entry = m_query_buffer[index];

    D3D::current_command_list->EndQuery(
        m_query_heap, D3D12_QUERY_TYPE_OCCLUSION, static_cast<UINT>(index));
    D3D::current_command_list->ResolveQueryData(
        m_query_heap, D3D12_QUERY_TYPE_OCCLUSION, static_cast<UINT>(index), 1,
...
void PerfQuery::FlushOne() {
  ...
  D3D12_RANGE read_range = {sizeof(UINT64) * index,
                            sizeof(UINT64) * (index + 1)};
  CheckHR(m_query_readback_buffer->Map(0, &read_range, &readback_buffer_map));

  UINT64 result;
  memcpy(&result,
         reinterpret_cast<u8 *>(readback_buffer_map) + sizeof(UINT64) * index,
         sizeof(UINT64));
  ...
  m_results[entry.query_type] +=
      (u32)(result * EFB_WIDTH / g_renderer->GetTargetWidth() * EFB_HEIGHT /
...
void PerfQuery::FlushResults() {
  ...
  // Map the whole readback buffer. Shouldn't have much overhead, and saves
  // taking the
  ...
  while (m_query_count > 0) {
    ...
    UINT64 result;
    memcpy(&result, reinterpret_cast<u8 *>(readback_buffer_map) +
                        sizeof(UINT64) * m_query_read_pos,
           sizeof(UINT64));
...
bool PerfQuery::IsFlushed() const { return m_query_count == 0; }
