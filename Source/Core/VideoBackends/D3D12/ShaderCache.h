...
public:
  ...
  static void LoadAndSetActiveShaders(DSTALPHA_MODE ps_dst_alpha_mode,
                                      u32 gs_primitive_type);

  template <class UidType, class ShaderCacheType>
  static D3D12_SHADER_BYTECODE InsertByteCode(const UidType &uid,
  ...
  GetGeometryShaderFromUid(const GeometryShaderUid *uid);
  static D3D12_SHADER_BYTECODE GetPixelShaderFromUid(const PixelShaderUid *uid);
  static D3D12_SHADER_BYTECODE
  GetVertexShaderFromUid(const VertexShaderUid *uid);
...
private:
  ...
  static void HandleGSUIDChange(GeometryShaderUid gs_uid,
                                u32 gs_primitive_type);
