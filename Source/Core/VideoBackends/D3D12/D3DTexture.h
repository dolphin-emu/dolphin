...
void ReplaceRGBATexture2D(
    ID3D12Resource *pTexture, const u8 *buffer, unsigned int width,
...
public:
  ...
  //     either create an ID3D12Resource object, pass it to the constructor and
  //     specify what views
  //     to create
  //     or let the texture automatically be created by D3DTexture2D::Create

  D3DTexture2D(
      ID3D12Resource *texptr, u32 bind,
      DXGI_FORMAT srv_format = DXGI_FORMAT_UNKNOWN,
      DXGI_FORMAT dsv_format = DXGI_FORMAT_UNKNOWN,
      DXGI_FORMAT rtv_format = DXGI_FORMAT_UNKNOWN, bool multisampled = false,
      D3D12_RESOURCE_STATES resource_state = D3D12_RESOURCE_STATE_COMMON);
  static D3DTexture2D *Create(unsigned int width, unsigned int height, u32 bind,
                              DXGI_FORMAT fmt, unsigned int levels = 1,
