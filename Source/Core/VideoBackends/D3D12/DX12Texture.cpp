                     ...
                     size_t buffer_size, u32 layer)
{
  ...
  const D3D12_TEXTURE_COPY_LOCATION dst_loc = {
      ...
      {.SubresourceIndex = static_cast<UINT>(CalcSubresource(level, layer))}};
                                         ...
                                         u32 dst_layer, u32 dst_level)
{
  ...
  const D3D12_TEXTURE_COPY_LOCATION dst_loc = {
      m_resource.Get(),
      D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
      {.SubresourceIndex = static_cast<UINT>(CalcSubresource(dst_level, dst_layer))}};
  const D3D12_TEXTURE_COPY_LOCATION src_loc = {
      src_dxtex->m_resource.Get(),
      D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
      {.SubresourceIndex = static_cast<UINT>(src_dxtex->CalcSubresource(src_level, src_layer))}};
                                       ...
                                       u32 src_level, const MathUtil::Rectangle<int>& dst_rect)
{
  ...
  const D3D12_TEXTURE_COPY_LOCATION src_loc = {
      src_tex->GetResource(),
      D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
      {.SubresourceIndex = static_cast<UINT>(src_tex->CalcSubresource(src_level, src_layer))}};
  const D3D12_BOX src_box = RectangleToBox(src_rect);
  g_dx_context->GetCommandList()->CopyTextureRegion(&dst_loc, dst_rect.left, dst_rect.top, 0,
                                                    &src_loc, &src_box);
                                     ...
                                     u32 dst_level)
{
  ...
  const D3D12_TEXTURE_COPY_LOCATION dst_loc = {
      dst_tex->GetResource(),
      D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
      {.SubresourceIndex = static_cast<UINT>(dst_tex->CalcSubresource(dst_level, dst_layer))}};
  const D3D12_TEXTURE_COPY_LOCATION src_loc = {
      m_resource.Get(),
      D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT,
      {{0,
