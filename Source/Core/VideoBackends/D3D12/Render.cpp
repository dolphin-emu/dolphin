...
static void SetupDeviceObjects() {
  ...
  depth_desc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
  s_clear_depth_descs[CLEAR_DEPTH_DESC_DEPTH_ENABLED_WRITES_DISABLED] =
      depth_desc;
  ...
  s_clear_blend_descs[CLEAR_BLEND_DESC_ALL_CHANNELS_ENABLED] =
      g_reset_blend_desc;
  ...
  D3D12_RASTERIZER_DESC rast_desc = CD3DX12_RASTERIZER_DESC(
      D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, false, 0, 0.f, 0.f, false,
...
void CreateScreenshotTexture() {
  ...
  const unsigned int screenshot_buffer_size =
      D3D::AlignValue(D3D::GetBackBufferWidth() * 4,
...
static D3D12_BOX GetScreenshotSourceBox(const TargetRectangle &target_rc) {
  // Since the screenshot buffer is copied back to the CPU, we can't access
  // pixels that
  ...
  return CD3DX12_BOX(std::max(target_rc.left, 0), std::max(target_rc.top, 0), 0,
                     std::min(D3D::GetBackBufferWidth(),
                              static_cast<unsigned int>(target_rc.right)),
                     std::min(D3D::GetBackBufferHeight(),
                              static_cast<unsigned int>(target_rc.bottom)),
                     1);
}
...
Renderer::Renderer(void *&window_handle) {
  ...
  D3D::current_command_list->ClearRenderTargetView(
      FramebufferManager::GetEFBColorTexture()->GetRTV12(), clear_color, 0,
      nullptr);
...
void Renderer::RenderText(const std::string &text, int left, int top,
                          u32 color) {
  D3D::font.DrawTextScaled(static_cast<float>(left + 1),
                           static_cast<float>(top + 1), 20.f, 0.0f,
                           color & 0xFF000000, text);
  D3D::font.DrawTextScaled(static_cast<float>(left), static_cast<float>(top),
                           20.f, 0.0f, color, text);
}
...
u32 Renderer::AccessEFB(EFBAccessType type, u32 x, u32 y, u32 poke_data) {
  if (type == PEEK_COLOR) {
    ...
    color = ((color & 0xFF00FF00) | ((color >> 16) & 0xFF) |
             ((color << 16) & 0xFF0000));
  ...
  } else // if (type == PEEK_Z)
  {
    ...
    if (bpmem.zcontrol.pixel_format == PEControl::RGB565_Z16) {
      ...
      ret = MathUtil::Clamp<u32>(static_cast<u32>(depth * 16777216.0f), 0,
                                 0xFFFFFF);
    }
...
void Renderer::PokeEFB(EFBAccessType type, const EfbPokeData *points,
  ...
  if (type == POKE_COLOR) {
    // In the D3D12 backend, the rt/db/viewport is passed into DrawEFBPokeQuads,
    // and set there.
    D3D::DrawEFBPokeQuads(
        type, points, num_points, &g_reset_blend_desc, &g_reset_depth_desc,
        &FramebufferManager::GetEFBColorTexture()->GetRTV12(), nullptr,
        FramebufferManager::GetEFBColorTexture()->GetMultisampled());
  } else // if (type == POKE_Z)
  {
    D3D::DrawEFBPokeQuads(
        type, points, num_points,
...
void Renderer::ClearScreen(const EFBRectangle &rc, bool color_enable,
  ...
  u32 rgba_color = (color & 0xFF00FF00) | ((color >> 16) & 0xFF) |
                   ((color << 16) & 0xFF0000);
  D3D::SetViewportAndScissor(target_rc.left, target_rc.top,
                             target_rc.GetWidth(), target_rc.GetHeight());
  D3D::DrawClearQuad(
      rgba_color, 1.0f - (z & 0xFFFFFF) / 16777216.0f, blend_desc,
      depth_stencil_desc,
      FramebufferManager::GetEFBColorTexture()->GetMultisampled());
...
void Renderer::ReinterpretPixelData(unsigned int convtype) {
  ...
  D3D12_RECT source = CD3DX12_RECT(0, 0, g_renderer->GetTargetWidth(),
  ...
  D3D::SetViewportAndScissor(0, 0, g_renderer->GetTargetWidth(),
                             g_renderer->GetTargetHeight());

  FramebufferManager::GetEFBColorTempTexture()->TransitionToResourceState(
      D3D::current_command_list, D3D12_RESOURCE_STATE_RENDER_TARGET);
  D3D::current_command_list->OMSetRenderTargets(
      1, &FramebufferManager::GetEFBColorTempTexture()->GetRTV12(), FALSE,
      nullptr);

  D3D::SetPointCopySampler();
  D3D::DrawShadedTexQuad(
      FramebufferManager::GetEFBColorTexture(), &source,
      g_renderer->GetTargetWidth(), g_renderer->GetTargetHeight(), pixel_shader,
...
void Renderer::SetBlendMode(bool force_update) {
  ...
  const D3D12_BLEND d3d_src_factors[8] = {
      D3D12_BLEND_ZERO, D3D12_BLEND_ONE, D3D12_BLEND_DEST_COLOR,
      D3D12_BLEND_INV_DEST_COLOR, D3D12_BLEND_SRC_ALPHA,
      D3D12_BLEND_INV_SRC_ALPHA, // NOTE: Use SRC1_ALPHA if dst alpha is
                                 // enabled!
...
bool Renderer::SaveScreenshot(const std::string &filename,
  ...
  saved_png = TextureToPng(static_cast<u8 *>(screenshot_texture_map),
                           dst_location.PlacedFootprint.Footprint.RowPitch,
                           filename, source_box.right - source_box.left,
...
void Renderer::SwapImpl(u32 xfb_addr, u32 fb_width, u32 fb_stride,
                        u32 fb_height, const EFBRectangle &rc, float gamma) {
  if (Fifo::WillSkipCurrentFrame() ||
      (!XFBWrited && !g_ActiveConfig.RealXFBEnabled()) || !fb_width ||
      !fb_height) {
  ...
  const XFBSourceBase *const *xfb_source_list =
      FramebufferManager::GetXFBSource(xfb_addr, fb_stride, fb_height,
                                       &xfb_count);
  if ((!xfb_source_list || xfb_count == 0) && g_ActiveConfig.bUseXFB &&
  ...
  // Invalidate EFB access copies. Not strictly necessary, but this avoids
  // having the buffers mapped
  ...
  D3D::GetBackBuffer()->TransitionToResourceState(
      D3D::current_command_list, D3D12_RESOURCE_STATE_RENDER_TARGET);
  D3D::current_command_list->OMSetRenderTargets(
      1, &D3D::GetBackBuffer()->GetRTV12(), FALSE, nullptr);

  float clear_color[4] = {0.f, 0.f, 0.f, 1.f};
  D3D::current_command_list->ClearRenderTargetView(
      D3D::GetBackBuffer()->GetRTV12(), clear_color, 0, nullptr);
  ...
  if (g_ActiveConfig.bUseXFB) {
    ...
    for (u32 i = 0; i < xfb_count; ++i) {
      ...
      int hOffset =
          (static_cast<s32>(xfb_source->srcAddr) - static_cast<s32>(xfb_addr)) /
      ...
      if (g_ActiveConfig.bUseRealXFB) {
        ...
        drawRc.top =
            target_rc.top +
            hOffset * target_rc.GetHeight() / static_cast<s32>(fb_height);
        drawRc.bottom = target_rc.top +
                        (hOffset + xfb_height) * target_rc.GetHeight() /
        ...
        // float vScale = static_cast<float>(fbHeight) /
        // static_cast<float>(s_backbuffer_height);
        // float hScale = static_cast<float>(fbWidth) /
        // static_cast<float>(s_backbuffer_width);
        // drawRc.top *= vScale;
      ...
      BlitScreen(source_rc, drawRc, xfb_source->m_tex, xfb_source->texWidth,
                 xfb_source->texHeight, gamma);
    }
  } else {
    TargetRectangle source_rc = Renderer::ConvertEFBRectangle(rc);

    // EXISTINGD3D11TODO: Improve sampling algorithm for the pixel shader so
    // that we can use the
    // multisampled EFB texture as source
    D3DTexture2D *read_texture =
        FramebufferManager::GetResolvedEFBColorTexture();

    BlitScreen(source_rc, target_rc, read_texture, GetTargetWidth(),
  ...
  if (SConfig::GetInstance().m_DumpFrames) {
    ...
    dst_location.PlacedFootprint.Footprint.Width =
        GetTargetRectangle().GetWidth();
    dst_location.PlacedFootprint.Footprint.Height =
        GetTargetRectangle().GetHeight();
    dst_location.PlacedFootprint.Footprint.Depth = 1;
    dst_location.PlacedFootprint.Footprint.RowPitch =
        D3D::AlignValue(dst_location.PlacedFootprint.Footprint.Width * 4,
                        D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
    ...
    if (!bLastFrameDumped) {
      ...
      if (!bAVIDumping) {
        ...
        std::string msg = StringFromFormat(
            ...
            File::GetUserPath(D_DUMPFRAMES_IDX).c_str(), s_record_width,
            s_record_height);
    ...
    if (bAVIDumping) {
      ...
      D3D12_RANGE read_range = {
          0, dst_location.PlacedFootprint.Footprint.RowPitch * source_height};
      CheckHR(
          s_screenshot_texture->Map(0, &read_range, &screenshot_texture_map));
      formatBufferDump(static_cast<u8 *>(screenshot_texture_map),
                       &frame_data[0], source_width, source_height,
                       dst_location.PlacedFootprint.Footprint.RowPitch);
  ...
  if (FramebufferManagerBase::LastXfbWidth() != fb_stride ||
    ...
    xfb_changed = true;
    unsigned int xfb_w = (fb_stride < 1 || fb_stride > MAX_XFB_WIDTH)
                             ? MAX_XFB_WIDTH
                             : fb_stride;
    unsigned int xfb_h = (fb_height < 1 || fb_height > MAX_XFB_HEIGHT)
                             ? MAX_XFB_HEIGHT
                             : fb_height;
  ...
  if (CalculateTargetSize(s_backbuffer_width, s_backbuffer_height) ||
      xfb_changed || window_resized ||
      s_last_efb_scale != g_ActiveConfig.iEFBScale ||
      s_last_multisamples != g_ActiveConfig.iMultisamples ||
      s_last_stereo_mode != (g_ActiveConfig.iStereoMode > 0)) {
    s_last_xfb_mode = g_ActiveConfig.bUseRealXFB;

    // Block on any changes until the GPU catches up, so we can free resources
    // safely.
    D3D::command_list_mgr->ExecuteQueuedWork(true);
...
void Renderer::ApplyState(bool use_dst_alpha) {
  ...
  if (D3D::command_list_mgr->GetCommandListDirtyState(
          COMMAND_LIST_STATE_SAMPLERS)) {
    D3D12_GPU_DESCRIPTOR_HANDLE sample_group_gpu_handle;
    sample_group_gpu_handle =
        D3D::sampler_descriptor_heap_mgr->GetHandleForSamplerGroup(
            gx_state.sampler, 8);

    D3D::current_command_list->SetGraphicsRootDescriptorTable(
        DESCRIPTOR_TABLE_PS_SAMPLER, sample_group_gpu_handle);
  ...
  if (D3D::command_list_mgr->GetCommandListDirtyState(COMMAND_LIST_STATE_PSO) ||
      s_previous_vertex_format !=
          reinterpret_cast<D3DVertexFormat *>(
              VertexLoaderManager::GetCurrentVertexFormat())) {
    s_previous_vertex_format = reinterpret_cast<D3DVertexFormat *>(
        VertexLoaderManager::GetCurrentVertexFormat());
    ...
    SmallPsoDesc pso_desc = {
        ShaderCache::GetActiveGeometryShaderBytecode(), // D3D12_SHADER_BYTECODE
                                                        // GS;
        ShaderCache::GetActivePixelShaderBytecode(),    // D3D12_SHADER_BYTECODE
                                                        // PS;
        ShaderCache::GetActiveVertexShaderBytecode(),   // D3D12_SHADER_BYTECODE
                                                        // VS;
  ...
  // Always called prior to drawing, so we can invalidate the CPU EFB copies
  // here.
  FramebufferManager::InvalidateEFBAccessCopies();
}
...
void Renderer::SetLogicOpMode() {
  ...
  if (bpmem.blendmode.logicopenable && !bpmem.blendmode.blendenable) {
    ...
    gx_state.blend.src_blend =
        d3d_logic_op_src_factors[bpmem.blendmode.logicmode];
    gx_state.blend.dst_blend =
        d3d_logic_op_dest_factors[bpmem.blendmode.logicmode];
  } else {
    SetBlendMode(true);
  }
...
int Renderer::GetMaxTextureSize() { return DX12::D3D::GetMaxTextureSize(); }
...
void Renderer::BlitScreen(TargetRectangle src, TargetRectangle dst,
  ...
  if (g_ActiveConfig.iStereoMode == STEREO_SBS ||
    ...
    D3D::SetViewportAndScissor(left_rc.left, left_rc.top, left_rc.GetWidth(),
                               left_rc.GetHeight());
    D3D::DrawShadedTexQuad(
    ...
    // D3D12TODO
    // Not currently supported on D3D12 backend. Implemented (but untested) code
    // kept for reference.

    // if (!s_3d_vision_texture)
    //	Create3DVisionTexture(s_backbuffer_width, s_backbuffer_height);

    // D3D12_VIEWPORT leftVp12 = { static_cast<float>(dst.left),
    // static_cast<float>(dst.top),
    // static_cast<float>(dst.GetWidth()), static_cast<float>(dst.GetHeight()),
    // D3D12_MIN_DEPTH,
    // D3D12_MAX_DEPTH };
    // D3D12_VIEWPORT rightVp12 = { static_cast<float>(dst.left +
    // s_backbuffer_width),
    ...
    // &s_3d_vision_texture->GetRTV12(), FALSE,
    // nullptr);
    ...
    // StaticShaderCache::GetSimpleVertexShader(),
    // StaticShaderCache::GetSimpleVertexShaderInputLayout(),
    // D3D12_SHADER_BYTECODE(), gamma, 0,
    // DXGI_FORMAT_R8G8B8A8_UNORM, false,
    // s_3d_vision_texture->GetMultisampled());
    ...
    // D3D12_BOX box = CD3DX12_BOX(0, 0, 0, s_backbuffer_width,
    // s_backbuffer_height, 1);
    ...
    // D3D12_RESOURCE_STATE_RENDER_TARGET);
    // D3D::current_command_list->OMSetRenderTargets(1,
    // &D3D::GetBackBuffer()->GetRTV12(), FALSE,
    // nullptr);
  } else {
    D3D::SetViewportAndScissor(dst.left, dst.top, dst.GetWidth(),
                               dst.GetHeight());
...
D3D12_BLEND_DESC Renderer::GetResetBlendDesc() { return g_reset_blend_desc; }
