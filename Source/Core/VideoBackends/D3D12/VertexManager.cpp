...
namespace DX12 {
static constexpr unsigned int MAX_IBUFFER_SIZE =
    VertexManager::MAXIBUFFERSIZE * sizeof(u16) * 16;
static constexpr unsigned int MAX_VBUFFER_SIZE =
    VertexManager::MAXVBUFFERSIZE * 4;

void VertexManager::SetIndexBuffer() {
  D3D12_INDEX_BUFFER_VIEW ib_view = {
      m_index_stream_buffer
          ->GetBaseGPUAddress(), // D3D12_GPU_VIRTUAL_ADDRESS BufferLocation;
...
void VertexManager::CreateDeviceObjects() {
  ...
  m_vertex_stream_buffer =
      std::make_unique<D3DStreamBuffer>(MAXVBUFFERSIZE * 2, MAX_VBUFFER_SIZE,
                                        &m_vertex_stream_buffer_reallocated);
  m_index_stream_buffer = std::make_unique<D3DStreamBuffer>(
      MAXIBUFFERSIZE * sizeof(u16) * 2, MAXIBUFFERSIZE * sizeof(u16) * 16,
...
VertexManager::VertexManager() { CreateDeviceObjects(); }
...
void VertexManager::Draw(u32 stride) {
  ...
  if (D3D::command_list_mgr->GetCommandListDirtyState(
          COMMAND_LIST_STATE_VERTEX_BUFFER) ||
    ...
    D3D::command_list_mgr->SetCommandListDirtyState(
        COMMAND_LIST_STATE_VERTEX_BUFFER, false);
    s_previous_stride = stride;
  }
  ...
  if (D3D::command_list_mgr->GetCommandListPrimitiveTopology() !=
  ...
  D3D::current_command_list->DrawIndexedInstanced(indices, 1, start_index,
                                                  base_vertex, 0);
...
void VertexManager::vFlush(bool use_dst_alpha) {
  ShaderCache::LoadAndSetActiveShaders(
      use_dst_alpha ? DSTALPHA_DUAL_SOURCE_BLEND : DSTALPHA_NONE,
  ...
  // Many Gamecube/Wii titles read from the EFB each frame to determine what new
  // rendering work to
  // submit, e.g. where sun rays are
  // occluded and where they aren't. When the CPU wants to read this data (done
  // in
  // Renderer::AccessEFB), it requires that the GPU
  // finish all oustanding work. As an optimization, when we detect that the CPU
  // is likely to read
  // back data this frame, we break
  // up the rendering work and submit it more frequently to the GPU (via
  // ExecuteCommandList). Thus,
  // when the CPU finally needs the
  // the GPU to finish all of its work, there is (hopefully) less work
  // outstanding to wait on at
  // that moment.

  // D3D12TODO: Decide right threshold for drawCountSinceAsyncFlush at runtime
  // depending on
...
void VertexManager::ResetBuffer(u32 stride) {
  ...
  s_pBaseBufferPointer =
      static_cast<u8 *>(m_vertex_stream_buffer->GetBaseCPUAddress());
  s_pEndBufferPointer =
      s_pBaseBufferPointer + m_vertex_stream_buffer->GetSize();
  s_pCurBufferPointer = static_cast<u8 *>(
      m_vertex_stream_buffer->GetCPUAddressOfCurrentAllocation());
  m_vertex_draw_offset =
      static_cast<u32>(m_vertex_stream_buffer->GetOffsetOfCurrentAllocation());

  m_index_stream_buffer->AllocateSpaceInBuffer(MAXIBUFFERSIZE * sizeof(u16),
                                               sizeof(u16));
  ...
  m_index_draw_offset =
      static_cast<u32>(m_index_stream_buffer->GetOffsetOfCurrentAllocation());
  IndexGenerator::Start(static_cast<u16 *>(
      m_index_stream_buffer->GetCPUAddressOfCurrentAllocation()));
}
