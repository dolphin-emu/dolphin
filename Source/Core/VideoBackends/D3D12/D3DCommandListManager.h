...
public:
  D3DCommandListManager(D3D12_COMMAND_LIST_TYPE command_list_type,
                        ID3D12Device *device,
  ...
  void ExecuteQueuedWorkAndPresent(IDXGISwapChain *swap_chain,
  ...
  void
  SetCommandListPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY primitive_topology);
  ...
  // Allow other components to register for a callback each time a fence is
  // queued.
  using PFN_QUEUE_FENCE_CALLBACK = void(void *owning_object,
                                        UINT64 fence_value);
  ID3D12Fence *
  RegisterQueueFenceCallback(void *owning_object,
...
private:
  ...
  D3D_PRIMITIVE_TOPOLOGY m_command_list_current_topology =
  ...
  std::array<std::vector<ID3D12CommandAllocator *>, 2>
      m_command_allocator_lists;
