...
D3DCommandListManager::D3DCommandListManager(
    D3D12_COMMAND_LIST_TYPE command_list_type, ID3D12Device *device,
    ID3D12CommandQueue *command_queue)
    : m_device(device), m_command_queue(command_queue) {
  // Create two lists, with two command allocators each. This corresponds to up
  // to two frames in
  // flight at once.
  ...
  for (UINT i = 0; i < COMMAND_ALLOCATORS_PER_LIST; i++) {
    for (UINT j = 0; j < m_command_allocator_lists.size(); j++) {
      ...
      CheckHR(m_device->CreateCommandAllocator(
          command_list_type, IID_PPV_ARGS(&command_allocator)));
  ...
  CheckHR(m_device->CreateCommandList(
      0, command_list_type,
      m_command_allocator_lists[m_current_command_allocator_list][0], nullptr,
      IID_PPV_ARGS(&m_backing_command_list)));

#ifdef USE_D3D12_QUEUED_COMMAND_LISTS
  m_queued_command_list =
      new ID3D12QueuedCommandList(m_backing_command_list, m_command_queue);
#endif

  // Create fence that will be used to measure GPU progress of app rendering
  // requests (e.g. CPU
  ...
  // Create fence that will be used internally by D3DCommandListManager for
  // frame-level resource
  // tracking.
  m_queue_frame_fence_value = 0;
  CheckHR(m_device->CreateFence(m_queue_frame_fence_value,
                                D3D12_FENCE_FLAG_NONE,
                                IID_PPV_ARGS(&m_queue_frame_fence)));

  // Create event that will be used for waiting on CPU until a fence is signaled
  // by GPU.
  ...
  std::fill(m_command_allocator_list_fences.begin(),
            m_command_allocator_list_fences.end(), 0);
  std::fill(m_deferred_destruction_list_fences.begin(),
            m_deferred_destruction_list_fences.end(), 0);
}
...
void D3DCommandListManager::SetInitialCommandListState() {
  ...
  command_list->SetDescriptorHeaps(
      static_cast<unsigned int>(D3D::gpu_descriptor_heaps.size()),
      D3D::gpu_descriptor_heaps.data());
  command_list->SetGraphicsRootSignature(D3D::default_root_signature);

  if (g_renderer) {
    // It is possible that we change command lists in the middle of the frame.
...
void D3DCommandListManager::GetCommandList(
...
void D3DCommandListManager::ExecuteQueuedWork(bool wait_for_gpu_completion) {
  ...
  m_queued_command_list->QueueFenceGpuSignal(m_queue_fence,
                                             m_queue_fence_value);
...
void D3DCommandListManager::ExecuteQueuedWorkAndPresent(
  ...
  // Move to the next command allocator, this may mean switching allocator
  // lists.
...
#ifdef USE_D3D12_QUEUED_COMMAND_LISTS
  m_queued_command_list->QueueFenceGpuSignal(m_queue_frame_fence,
                                             m_queue_frame_fence_value);
  m_queued_command_list->ProcessQueuedItems(true);
#else
  CheckHR(
      m_command_queue->Signal(m_queue_frame_fence, m_queue_frame_fence_value));
#endif

  WaitOnCPUForFence(m_queue_frame_fence, m_queue_frame_fence_value);

  // GPU is up to date with us. Therefore, it has finished with any pending
  // resources.
...
void D3DCommandListManager::PerformGPURolloverChecks() {
  ...
  // We now know that the previous 'set' of command lists has completed on GPU,
  // and it is safe to
  ...
  UINT safe_to_delete_deferred_destruction_list =
      (m_current_deferred_destruction_list - 1) %
      m_deferred_destruction_lists.size();
  WaitOnCPUForFence(m_queue_frame_fence,
                    m_deferred_destruction_list_fences
                        [safe_to_delete_deferred_destruction_list]);

  for (UINT i = 0;
       i <
       m_deferred_destruction_lists[safe_to_delete_deferred_destruction_list]
           .size();
       i++) {
    CHECK(m_deferred_destruction_lists[safe_to_delete_deferred_destruction_list]
                                      [i]->Release() == 0,
          "Resource leak.");
  }
  ...
  for (UINT i = 0;
       i <
       m_command_allocator_lists[safe_to_reset_command_allocator_list].size();
       i++) {
    CheckHR(m_command_allocator_lists[safe_to_reset_command_allocator_list]
                                     [i]->Reset());
  }

  m_command_allocator_list_fences[m_current_command_allocator_list] =
      m_queue_frame_fence_value;
...
void D3DCommandListManager::ResetCommandList() {
  ...
  CheckHR(command_list->Reset(
      m_command_allocator_lists[m_current_command_allocator_list]
                               [m_current_command_allocator],
      nullptr));
}

void D3DCommandListManager::DestroyResourceAfterCurrentCommandListExecuted(
    ID3D12Resource *resource) {
  CHECK(resource, "Null resource being inserted!");

  m_deferred_destruction_lists[m_current_deferred_destruction_list].push_back(
      resource);
}
...
void D3DCommandListManager::WaitOnCPUForFence(ID3D12Fence *fence,
                                              UINT64 fence_value) {
...
void D3DCommandListManager::SetCommandListDirtyState(
    unsigned int command_list_state, bool dirty) {
  if (dirty)
...
bool D3DCommandListManager::GetCommandListDirtyState(
    COMMAND_LIST_STATE command_list_state) const {
...
D3DCommandListManager::GetCommandListPrimitiveTopology() const {
...
ID3D12Fence *D3DCommandListManager::RegisterQueueFenceCallback(
    void *owning_object, PFN_QUEUE_FENCE_CALLBACK *callback_function) {
