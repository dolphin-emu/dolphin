...
D3DStreamBuffer::D3DStreamBuffer(size_t initial_size, size_t max_size,
  ...
  // Register for callback from D3DCommandListManager each time a fence is
  // queued to be signaled.
  m_buffer_tracking_fence = D3D::command_list_mgr->RegisterQueueFenceCallback(
      this, &D3DStreamBuffer::QueueFenceCallback);
}
...
// Function returns true if (worst case), needed to flush existing command list
// in order to
// ensure the GPU finished with current use of buffer. The calling function will
// need to take
// care to reset GPU state to what it was previously.

// Obviously this is non-performant, so the buffer max_size should be large
// enough to
// ensure this never happens.
bool D3DStreamBuffer::AllocateSpaceInBuffer(size_t allocation_size,
                                            size_t alignment,
                                            bool allow_execute) {
  CHECK(allocation_size <= m_buffer_max_size,
        "Error: Requested allocation size in D3DStreamBuffer "
  ...
  if (alignment && m_buffer_offset > 0) {
    ...
    // Check for case when adding alignment causes CPU offset to equal GPU
    // offset,
  ...
  // First, check if there is available (not-in-use-by-GPU) space in existing
  // buffer.
  if (AttemptToAllocateOutOfExistingUnusedSpaceInBuffer(allocation_size)) {
    return false;
  }

  // Slow path. No room at front, or back, due to the GPU still (possibly)
  // accessing parts of the
  // buffer.
  // Resize if possible, else stall.
  bool command_list_executed =
      AttemptBufferResizeOrElseStall(allocation_size, allow_execute);
...
// After the vertex data is written, we can choose to specify the 'real'
// allocation
// size to avoid wasting space.
void D3DStreamBuffer::OverrideSizeOfPreviousAllocation(
    size_t override_allocation_size) {
  m_buffer_offset =
      m_buffer_current_allocation_offset + override_allocation_size;
}
...
// command list
// referencing all of buffer's contents, AND we are already at max_size. No
// alternative but to
...
bool D3DStreamBuffer::AttemptBufferResizeOrElseStall(size_t allocation_size,
  ...
  // to running out of room. If the buffer is already at its maximum size
  // specified
  // at creation time, then stall waiting for the GPU to finish with the
  // currently
  // requested memory.
  ...
  // 1) First, let's check if GPU has already continued farther along buffer. If
  // it has freed up
  ...
  size_t new_size =
      std::min(static_cast<size_t>(m_buffer_size * 1.5f), m_buffer_max_size);
  ...
  const bool found_fence_to_wait_on =
      AttemptToFindExistingFenceToStallOn(allocation_size);

  if (found_fence_to_wait_on) {
    return false;
  }

  // If allow_execute is false, the caller cannot handle command list execution
  // (and the associated
  ...
  // 4) If we get to this point, that means there is no outstanding queued GPU
  // work, and we're still
  // out of room.
  // This is bad - and performance will suffer due to the CPU/GPU serialization,
  // but the show must
  // go on.

  // This is guaranteed to succeed, since we've already CHECK'd that the
  // allocation_size <=
...
bool D3DStreamBuffer::AttemptToAllocateOutOfExistingUnusedSpaceInBuffer(
...
bool D3DStreamBuffer::AttemptToFindExistingFenceToStallOn(
    size_t allocation_size) {
  ...
  while (m_queued_fences.size() > 0) {
    ...
    if (m_buffer_offset >= m_buffer_gpu_completion_offset) {
      // At this point, we need to wrap around, so req'd gpu offset is
      // allocation_size.
      ...
      if (m_buffer_offset + allocation_size <= m_buffer_size) {
        if (tracking_information.buffer_offset >=
            m_buffer_offset + allocation_size) {
          fence_value_required = tracking_information.fence_value;
          m_buffer_current_allocation_offset = m_buffer_offset;
  ...
  // Check if we found a fence we can wait on, for GPU to make sufficient
  // progress.
  // If so, wait on it.
  if (fence_value_required > 0) {
    D3D::command_list_mgr->WaitOnCPUForFence(m_buffer_tracking_fence,
                                             fence_value_required);
    return true;
  }
...
void D3DStreamBuffer::UpdateGPUProgress() {
  ...
  while (m_queued_fences.size() > 0) {
    ...
    if (fence_value >= tracking_information.fence_value) {
      ...
      // Fences are stored in ascending order, so once we hit a fence we haven't
      // yet crossed on GPU,
      // abort search.
      break;
    }
...
void D3DStreamBuffer::QueueFenceCallback(void *owning_object,
                                         UINT64 fence_value) {
  D3DStreamBuffer *owning_stream_buffer =
      reinterpret_cast<D3DStreamBuffer *>(owning_object);
...
ID3D12Resource *D3DStreamBuffer::GetBuffer() const { return m_buffer; }

D3D12_GPU_VIRTUAL_ADDRESS
D3DStreamBuffer::GetGPUAddressOfCurrentAllocation() const {
...
void *D3DStreamBuffer::GetCPUAddressOfCurrentAllocation() const {
  return static_cast<u8 *>(m_buffer_cpu_address) +
         m_buffer_current_allocation_offset;
}
...
size_t D3DStreamBuffer::GetSize() const { return m_buffer_size; }
