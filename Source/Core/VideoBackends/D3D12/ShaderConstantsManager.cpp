...
static const unsigned int
    s_shader_constant_buffer_padded_sizes[SHADER_STAGE_COUNT] = {
...
void ShaderConstantsManager::Init() {
  ...
  std::generate(std::begin(s_shader_constant_stream_buffers),
                std::end(s_shader_constant_stream_buffers), []() {
                  return std::make_unique<D3DStreamBuffer>(
                      2 * 1024 * 1024, 64 * 1024 * 1024, nullptr);
                });
}
...
bool ShaderConstantsManager::LoadAndSetGeometryShaderConstants() {
  ...
  if (GeometryShaderManager::dirty) {
    command_list_executed =
        s_shader_constant_stream_buffers[SHADER_STAGE_GEOMETRY_SHADER]
            ->AllocateSpaceInBuffer(s_shader_constant_buffer_padded_sizes
                                        [SHADER_STAGE_GEOMETRY_SHADER],
                                    0 // The padded sizes are already aligned to
                                    // 256 bytes, so don't need to worry about
    ...
    D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_GS_CBV,
                                                    true);
  }

  if (D3D::command_list_mgr->GetCommandListDirtyState(
          COMMAND_LIST_STATE_GS_CBV)) {
...
bool ShaderConstantsManager::LoadAndSetPixelShaderConstants() {
  ...
  if (PixelShaderManager::dirty) {
    command_list_executed =
        s_shader_constant_stream_buffers[SHADER_STAGE_PIXEL_SHADER]
            ->AllocateSpaceInBuffer(s_shader_constant_buffer_padded_sizes
...
bool ShaderConstantsManager::LoadAndSetVertexShaderConstants() {
  ...
  if (VertexShaderManager::dirty) {
    ...
    D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_VS_CBV,
                                                    true);
  }

  if (D3D::command_list_mgr->GetCommandListDirtyState(
          COMMAND_LIST_STATE_VS_CBV)) {
    const D3D12_GPU_VIRTUAL_ADDRESS calculated_gpu_va =
        s_shader_constant_stream_buffers[SHADER_STAGE_VERTEX_SHADER]
            ->GetGPUAddressOfCurrentAllocation();

    D3D::current_command_list->SetGraphicsRootConstantBufferView(
        DESCRIPTOR_TABLE_VS_CBV, calculated_gpu_va);

    if (g_ActiveConfig.bEnablePixelLighting)
      D3D::current_command_list->SetGraphicsRootConstantBufferView(
          DESCRIPTOR_TABLE_PS_CBVTWO, calculated_gpu_va);
