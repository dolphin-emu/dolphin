...
void BBox::Init() {
  ...
  CD3DX12_RESOURCE_DESC staging_buffer_desc(CD3DX12_RESOURCE_DESC::Buffer(
      BBOX_BUFFER_SIZE, D3D12_RESOURCE_FLAG_NONE, 0));
  ...
  CheckHR(D3D::device12->CreateCommittedResource(
      ...
      &staging_buffer_desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
      IID_PPV_ARGS(&s_bbox_staging_buffer)));

  s_bbox_stream_buffer = std::make_unique<D3DStreamBuffer>(
      BBOX_STREAM_BUFFER_SIZE, BBOX_STREAM_BUFFER_SIZE, nullptr);

  // D3D12 root signature UAV must be raw or structured buffers, not typed.
  // Since we used a typed
  // buffer,
  // we have to use a descriptor table. Luckily, we only have to allocate this
  // once, and it never
  // changes.
  D3D12_CPU_DESCRIPTOR_HANDLE cpu_descriptor_handle;
  if (!D3D::gpu_descriptor_heap_mgr->Allocate(
          &cpu_descriptor_handle, &s_bbox_descriptor_handle, nullptr, false))
    PanicAlert("Failed to create bounding box UAV descriptor");

  D3D12_UNORDERED_ACCESS_VIEW_DESC view_desc = {DXGI_FORMAT_R32_SINT,
...
void BBox::Bind() {
  D3D::current_command_list->SetGraphicsRootDescriptorTable(
      DESCRIPTOR_TABLE_PS_UAV, s_bbox_descriptor_handle);
}
...
void BBox::Set(int index, int value) {
  // If the buffer is currently mapped, compare the value, and update the
  // staging buffer.
  if (s_bbox_staging_buffer_map) {
    int current_value;
    memcpy(&current_value, reinterpret_cast<u8 *>(s_bbox_staging_buffer_map) +
                               (index * sizeof(int)),
           sizeof(int));
    ...
    memcpy(reinterpret_cast<u8 *>(s_bbox_staging_buffer_map) +
               (index * sizeof(int)),
           &value, sizeof(int));
  }
  ...
  memcpy(s_bbox_stream_buffer->GetCPUAddressOfCurrentAllocation(), &value,
         sizeof(int));
...
int BBox::Get(int index) {
  if (!s_bbox_staging_buffer_map) {
    ...
    // Copy from real buffer to staging buffer, then block until we have the
    // results.
    D3D::ResourceBarrier(D3D::current_command_list, s_bbox_buffer,
                         D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                         D3D12_RESOURCE_STATE_COPY_SOURCE, 0);
    D3D::current_command_list->CopyBufferRegion(
        s_bbox_staging_buffer, 0, s_bbox_buffer, 0, BBOX_BUFFER_SIZE);
    D3D::ResourceBarrier(D3D::current_command_list, s_bbox_buffer,
    ...
    CheckHR(
        s_bbox_staging_buffer->Map(0, &read_range, &s_bbox_staging_buffer_map));
  }

  int value;
  memcpy(&value, &reinterpret_cast<int *>(s_bbox_staging_buffer_map)[index],
         sizeof(int));
  return value;
}
};
