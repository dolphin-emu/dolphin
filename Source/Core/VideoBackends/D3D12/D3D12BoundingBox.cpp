...
std::vector<BBoxType> D3D12BoundingBox::Read(u32 index, u32 length)
{
  ...
  g_dx_context->GetCommandList()->CopyBufferRegion(
      m_readback_buffer.Get(), 0, m_gpu_buffer.Get(), 0, BUFFER_SIZE);
  ResourceBarrier(g_dx_context->GetCommandList(), m_gpu_buffer.Get(),
...
void D3D12BoundingBox::Write(u32 index, std::span<const BBoxType> values)
{
  ...
  g_dx_context->GetCommandList()->CopyBufferRegion(m_gpu_buffer.Get(), index * sizeof(BBoxType),
      m_upload_buffer.GetBuffer(), upload_buffer_offset, copy_size);
...
bool D3D12BoundingBox::CreateBuffers()
{
  ...
  D3D12_RESOURCE_DESC buffer_desc = {D3D12_RESOURCE_DIMENSION_BUFFER, 0, BUFFER_SIZE, 1, 1, 1,
      DXGI_FORMAT_UNKNOWN, {1, 0}, D3D12_TEXTURE_LAYOUT_ROW_MAJOR,
      D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS};

  HRESULT hr =
      g_dx_context->GetDevice()->CreateCommittedResource(&gpu_heap_properties, D3D12_HEAP_FLAG_NONE,
          &buffer_desc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_gpu_buffer));
  ...
  g_dx_context->GetDevice()->CreateUnorderedAccessView(
      m_gpu_buffer.Get(), nullptr, &uav_desc, m_gpu_descriptor.cpu_handle);

  buffer_desc.Flags = D3D12_RESOURCE_FLAG_NONE;
  hr =
      g_dx_context->GetDevice()->CreateCommittedResource(&cpu_heap_properties, D3D12_HEAP_FLAG_NONE,
          &buffer_desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_readback_buffer));
