...
#include "VideoBackends/OGL/AbstractTexture.h"  // HAXX
...
user and we block until completion.
  ...
  D3D12_TEXTURE_COPY_LOCATION src_location = CD3DX12_TEXTURE_COPY_LOCATION(m_texture->GetTex12(),
level);
...
srcentry->config.layers);
    D3D12_TEXTURE_COPY_LOCATION dst_location = CD3DX12_TEXTURE_COPY_LOCATION(m_texture->GetTex12(),
0);
...
D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
  ...
  FramebufferManager::GetEFBColorTexture()->TransitionToResourceState(D3D::current_command_list,
D3D12_RESOURCE_STATE_RENDER_TARGET);
  FramebufferManager::GetEFBDepthTexture()->TransitionToResourceState(D3D::current_command_list,
...
void TextureCache::TCacheEntry::FromRenderTarget(u8* dst, PEControl::PixelFormat src_format, const
  ...
  bool scale_by_half, unsigned int cbuf_id, const float* colmat)
{
  ...
  if (s_efb_copy_last_cbuf_id != cbuf_id)
  {
    ...
    memcpy(s_efb_copy_stream_buffer->GetCPUAddressOfCurrentAllocation(), colmat, 28 *
sizeof(float));
  ...
  D3D::current_command_list->SetGraphicsRootConstantBufferView(DESCRIPTOR_TABLE_PS_CBVONE,
s_efb_copy_stream_buffer->GetGPUAddressOfCurrentAllocation());
  ...
  const D3D12_RECT sourcerect = CD3DX12_RECT(targetSource.left, targetSource.top,
targetSource.right, targetSource.bottom);
...
void TextureCache::ConvertTexture(TCacheEntry* entry, TCacheEntry* unconverted, void* palette,
                                  TlutFormat format)
{
  ...
  static_cast<TCacheEntry*>(unconverted)->m_texture->TransitionToResourceState(D3D::current_command_list,
  D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
  ...
  const D3D12_RECT source_rect = CD3DX12_RECT(0, 0, unconverted->config.width,
  ...
  static_cast<TCacheEntry*>(entry)->m_texture->TransitionToResourceState(D3D::current_command_list,
  D3D12_RESOURCE_STATE_RENDER_TARGET);
  D3D::current_command_list->OMSetRenderTargets(1,
  &static_cast<TCacheEntry*>(entry)->m_texture->GetRTV12(), FALSE, nullptr);
...
void TextureCache::BindTextures()
{
  /*	unsigned int last_texture = 0;
    ...
    DX12::D3D::gpu_descriptor_heap_mgr->AllocateGroup(&s_group_base_texture_cpu_handle, 8,
    &s_group_base_texture_gpu_handle, nullptr, true);
    ...
    for (unsigned int stage = 0; stage < 8; stage++)
    {
      if (bound_textures[stage] != nullptr)
      {
        ...
        textureDestDescriptor.ptr = s_group_base_texture_cpu_handle.ptr + stage *
    D3D::resource_descriptor_size;
      ...
      else
      {
        ...
        nullDestDescriptor.ptr = s_group_base_texture_cpu_handle.ptr + stage *
    D3D::resource_descriptor_size;
