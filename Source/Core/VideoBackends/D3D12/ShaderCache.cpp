...
void ShaderCache::Init() {
  ...
  std::string gs_cache_filename = StringFromFormat(
      "%sdx11-%s-gs.cache", shader_cache_path.c_str(), title_unique_id.c_str());
  std::string ps_cache_filename = StringFromFormat(
      "%sdx11-%s-ps.cache", shader_cache_path.c_str(), title_unique_id.c_str());
  std::string vs_cache_filename = StringFromFormat(
      "%sdx11-%s-vs.cache", shader_cache_path.c_str(), title_unique_id.c_str());

  ShaderCacheInserter<GeometryShaderUid, GsBytecodeCache, &s_gs_bytecode_cache>
      gs_inserter;
  s_gs_disk_cache.OpenAndRead(gs_cache_filename, gs_inserter);

  ShaderCacheInserter<PixelShaderUid, PsBytecodeCache, &s_ps_bytecode_cache>
      ps_inserter;
  s_ps_disk_cache.OpenAndRead(ps_cache_filename, ps_inserter);

  ShaderCacheInserter<VertexShaderUid, VsBytecodeCache, &s_vs_bytecode_cache>
      vs_inserter;
  ...
  SETSTAT(stats.numVertexShadersCreated,
          static_cast<int>(s_vs_bytecode_cache.size()));
}
...
void ShaderCache::HandleGSUIDChange(GeometryShaderUid gs_uid,
                                    u32 gs_primitive_type) {
  ...
  if (gs_iterator != s_gs_bytecode_cache.end()) {
    ...
    ShaderCode gs_code =
        GenerateGeometryShaderCode(API_D3D, gs_uid.GetUidData());
    ...
    s_last_geometry_shader_bytecode =
        InsertByteCode(gs_uid, &s_gs_bytecode_cache, gs_bytecode);
...
void ShaderCache::HandlePSUIDChange(PixelShaderUid ps_uid,
  ...
  if (ps_iterator != s_ps_bytecode_cache.end()) {
    ...
    s_last_pixel_shader_bytecode =
        InsertByteCode(ps_uid, &s_ps_bytecode_cache, ps_bytecode);
    s_ps_disk_cache.Append(
        ps_uid, reinterpret_cast<u8 *>(ps_bytecode->GetBufferPointer()),
...
void ShaderCache::HandleVSUIDChange(VertexShaderUid vs_uid) {
  ...
  if (vs_iterator != s_vs_bytecode_cache.end()) {
    ...
    s_last_vertex_shader_bytecode =
        InsertByteCode(vs_uid, &s_vs_bytecode_cache, vs_bytecode);
    s_vs_disk_cache.Append(
        vs_uid, reinterpret_cast<u8 *>(vs_bytecode->GetBufferPointer()),
...
D3D12_SHADER_BYTECODE ShaderCache::InsertByteCode(const UidType &uid,
                                                  ShaderCacheType *shader_cache,
