...
class PipelineStateCacheInserter
    : public LinearDiskCacheReader<SmallPsoDiskDesc, u8> {
public:
  void Read(const SmallPsoDiskDesc &key, const u8 *value, u32 value_size) {
    ...
    desc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM; // This state changes in
    // PSTextureEncoder::Encode.
    desc.DSVFormat = DXGI_FORMAT_D32_FLOAT; // This state changes in
                                            // PSTextureEncoder::Encode.
    ...
    desc.InputLayout = reinterpret_cast<D3DVertexFormat *>(native.get())
                           ->GetActiveInputLayout12();
    ...
    HRESULT hr =
        D3D::device12->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&pso));

    if (FAILED(hr)) {
      // Failure can occur if disk cache is corrupted, or a driver upgrade
...
void StateCache::Init() {
  // Root signature isn't available at time of StateCache construction, so fill
  // it in now.
  gx_state_cache.m_current_pso_desc.pRootSignature =
      D3D::default_root_signature;

  // Multi-sample configuration isn't available at time of StateCache
  // construction, so fille it in
  // now.
  gx_state_cache.m_current_pso_desc.SampleDesc.Count =
  ...
  std::string cache_filename = StringFromFormat(
      "%sdx12-%s-pso.cache", File::GetUserPath(D_SHADERCACHE_IDX).c_str(),
  ...
  if (s_cache_is_corrupted) {
    ...
    // - A driver/HW change has occurred, causing the existing cache blobs to be
    // invalid.
    //
    // In either case, we want to re-create the disk cache. This should not be a
    // frequent occurence.
...
D3D12_SAMPLER_DESC StateCache::GetDesc12(SamplerState state) {
  ...
  if (g_ActiveConfig.iMaxAnisotropy > 0 &&
      !SamplerCommon::IsBpTexMode0PointFiltering(state)) {
  ...
  sampdc.BorderColor[0] = sampdc.BorderColor[1] = sampdc.BorderColor[2] =
      sampdc.BorderColor[3] = 1.0f;

  sampdc.MaxLOD = SamplerCommon::AreBpTexMode0MipmapsEnabled(state)
                      ? state.max_lod / 16.f
                      : 0.f;
...
D3D12_BLEND_DESC StateCache::GetDesc12(BlendState state) {
  ...
  blenddc.RenderTarget[0].SrcBlendAlpha =
      GetBlendingAlpha(blenddc.RenderTarget[0].SrcBlend);
  blenddc.RenderTarget[0].DestBlendAlpha =
      GetBlendingAlpha(blenddc.RenderTarget[0].DestBlend);
...
HRESULT StateCache::GetPipelineStateObjectFromCache(
    D3D12_GRAPHICS_PIPELINE_STATE_DESC *pso_desc, ID3D12PipelineState **pso) {
  ...
  if (it == m_pso_map.end()) {
    ...
    HRESULT hr = D3D::device12->CreateGraphicsPipelineState(
        pso_desc, IID_PPV_ARGS(&new_pso));
...
HRESULT StateCache::GetPipelineStateObjectFromCache(
    SmallPsoDesc *pso_desc, ID3D12PipelineState **pso,
    D3D12_PRIMITIVE_TOPOLOGY_TYPE topology, const GeometryShaderUid *gs_uid,
    const PixelShaderUid *ps_uid, const VertexShaderUid *vs_uid) {
  auto it = m_small_pso_map.find(*pso_desc);

  if (it == m_small_pso_map.end()) {
    // Not found, create new PSO.

    // RootSignature, SampleMask, SampleDesc, NumRenderTargets, RTVFormats,
    // DSVFormat
    ...
    m_current_pso_desc.DepthStencilState =
        GetDesc12(pso_desc->depth_stencil_state);
    m_current_pso_desc.RasterizerState = GetDesc12(pso_desc->rasterizer_state);
    m_current_pso_desc.PrimitiveTopologyType = topology;
    m_current_pso_desc.InputLayout =
        pso_desc->input_layout->GetActiveInputLayout12();

    ID3D12PipelineState *new_pso = nullptr;
    HRESULT hr = D3D::device12->CreateGraphicsPipelineState(
        &m_current_pso_desc, IID_PPV_ARGS(&new_pso));
    ...
    *pso = new_pso;
    ...
    // This contains all of the information needed to reconstruct a PSO at
    // startup.
    ...
    disk_desc.vertex_declaration =
        pso_desc->input_layout->GetVertexDeclaration();
    ...
    if (SUCCEEDED(hr)) {
      s_pso_disk_cache.Append(
          disk_desc, reinterpret_cast<const u8 *>(psoBlob->GetBufferPointer()),
