...
// static_assert(std::is_trivially_copyable<K>::value, "K must be a trivially
// copyable type");
...
struct SmallPsoDiskDesc {
  ...
  PortableVertexDeclaration
      vertex_declaration; // Used to construct the input layout.
};
...
public:
  ...
  HRESULT
  GetPipelineStateObjectFromCache(D3D12_GRAPHICS_PIPELINE_STATE_DESC *pso_desc,
                                  ID3D12PipelineState **pso);
  HRESULT GetPipelineStateObjectFromCache(
...
private:
  ...
  struct hash_pso_desc {
    size_t
    operator()(const D3D12_GRAPHICS_PIPELINE_STATE_DESC &pso_desc) const {
  ...
  struct equality_pipeline_state_desc {
    bool operator()(const D3D12_GRAPHICS_PIPELINE_STATE_DESC &lhs,
      ...
      return std::tie(lhs.PS.pShaderBytecode, lhs.VS.pShaderBytecode,
                      lhs.GS.pShaderBytecode, lhs.RasterizerState.CullMode,
                      ...
                      lhs.RTVFormats[0], lhs.SampleDesc.Count) ==
             std::tie(rhs.PS.pShaderBytecode, rhs.VS.pShaderBytecode,
                      rhs.GS.pShaderBytecode, rhs.RasterizerState.CullMode,
                      ...
                      rhs.RTVFormats[0], rhs.SampleDesc.Count);
    }
  };

  std::unordered_map<D3D12_GRAPHICS_PIPELINE_STATE_DESC, ID3D12PipelineState *,
                     hash_pso_desc, equality_pipeline_state_desc>
      m_pso_map;
    ...
    size_t operator()(const SmallPsoDesc &pso_desc) const {
      return ((uintptr_t)pso_desc.vs_bytecode.pShaderBytecode << 10) ^
             ((uintptr_t)pso_desc.ps_bytecode.pShaderBytecode) +
                 pso_desc.blend_state.hex + pso_desc.depth_stencil_state.hex;
    }
  };

  struct equality_small_pipeline_state_desc {
    bool operator()(const SmallPsoDesc &lhs, const SmallPsoDesc &rhs) const {
      return std::tie(lhs.ps_bytecode.pShaderBytecode,
                      lhs.vs_bytecode.pShaderBytecode,
                      lhs.gs_bytecode.pShaderBytecode, lhs.input_layout,
                      lhs.blend_state.hex, lhs.depth_stencil_state.hex,
                      lhs.rasterizer_state.hex) ==
             std::tie(rhs.ps_bytecode.pShaderBytecode,
                      rhs.vs_bytecode.pShaderBytecode,
                      rhs.gs_bytecode.pShaderBytecode, rhs.input_layout,
                      rhs.blend_state.hex, rhs.depth_stencil_state.hex,
                      rhs.rasterizer_state.hex);
    }
  };
  ...
  struct equality_shader_bytecode {
    bool operator()(const D3D12_SHADER_BYTECODE &lhs,
                    const D3D12_SHADER_BYTECODE &rhs) const {
