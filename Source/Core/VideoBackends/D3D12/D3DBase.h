...
// D3D12TODO: Support this from Graphics Settings, not require a recompile to
// enable.
...
#define CHECK(cond, Message, ...)                                              \
  if (!(cond)) {                                                               \
    __debugbreak();                                                            \
    PanicAlert(__FUNCTION__ " failed in %s at line %d: " Message, __FILE__,    \
               __LINE__, __VA_ARGS__);                                         \
  }

// DEBUGCHECK is for high-frequency functions that we only want to check on
// debug builds.
...
inline void CheckHR(HRESULT hr) { CHECK(SUCCEEDED(hr), "Failed HRESULT."); }
...
static void SetDebugObjectName12(ID3D12Resource *resource, LPCSTR name) {
  HRESULT hr = resource->SetPrivateData(WKPDID_D3DDebugObjectName,
                                        (UINT)(name ? strlen(name) : 0), name);
  if (FAILED(hr)) {
...
static std::string GetDebugObjectName12(ID3D12Resource *resource) {
  ...
  if (resource) {
    UINT size = 0;
    resource->GetPrivateData(WKPDID_D3DDebugObjectName, &size,
                             nullptr); // get required size
    name.resize(size);
    resource->GetPrivateData(WKPDID_D3DDebugObjectName, &size,
...
using D3D12CREATEDEVICE = HRESULT(WINAPI *)(IUnknown *, D3D_FEATURE_LEVEL,
                                            REFIID, void **);
