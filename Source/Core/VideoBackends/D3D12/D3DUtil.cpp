...
namespace DX12 {
namespace D3D {
void ResourceBarrier(ID3D12GraphicsCommandList *command_list,
  ...
  D3D12_RESOURCE_BARRIER resourceBarrierDesc = {
      D3D12_RESOURCE_BARRIER_TYPE_TRANSITION, // D3D12_RESOURCE_TRANSITION_BARRIER_DESC
                                              // Transition
...
public:
  explicit UtilVertexBuffer(size_t size) {
    m_stream_buffer =
        std::make_unique<D3DStreamBuffer>(size, size * 4, nullptr);
  }
  ...
  size_t AppendData(const void *data, size_t size, size_t vertex_size) {
    ...
    memcpy(
        static_cast<u8 *>(m_stream_buffer->GetCPUAddressOfCurrentAllocation()),
        data, size);
...
int CD3DFont::Init() {
  ...
  for (int c = 0; c < 127 - 32; c++) {
    ...
    ExtTextOutA(hDC, x + 1, y + 0, ETO_OPAQUE | ETO_CLIPPED, nullptr, str, 1,
                nullptr);
    ...
    x += size.cx + 3; // 3 to work around annoying ij conflict (part of the j
                      // ends up with the i)
  }

  // Create a new texture for the font
  // possible optimization: store the converted data in a buffer and fill the
  // texture on creation.
  // That way, we can use a static texture
  std::unique_ptr<byte[]> tex_initial_data(
      new byte[m_tex_width * m_tex_height * 4]);

  for (y = 0; y < m_tex_height; y++) {
    u32 *pDst32 = reinterpret_cast<u32 *>(
        static_cast<u8 *>(tex_initial_data.get()) + y * m_tex_width * 4);
  ...
  CheckHR(D3D::device12->CreateCommittedResource(
      ...
      &CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_R8G8B8A8_UNORM, m_tex_width,
                                    m_tex_height, 1, 1),
  ...
  CheckHR(D3D::device12->CreateCommittedResource(
      ...
      &CD3DX12_RESOURCE_DESC::Buffer(
          AlignValue(m_tex_width * 4, D3D12_TEXTURE_DATA_PITCH_ALIGNMENT) *
          m_tex_height),
  ...
  D3D::ResourceBarrier(
      D3D::current_command_list, m_texture12, D3D12_RESOURCE_STATE_COMMON,
  ...
  CHECK(0 != UpdateSubresources(D3D::current_command_list, m_texture12,
                                temporaryFontTextureUploadBuffer, 0, 0, 1,
                                &subresource_data_dest),
  ...
  D3D::device12->CreateShaderResourceView(m_texture12, &srv_desc,
                                          m_texture12_cpu);

  D3D::ResourceBarrier(D3D::current_command_list, m_texture12,
  ...
  D3D12_RASTERIZER_DESC rastdesc = {D3D12_FILL_MODE_SOLID,
  ...
  m_vertex_buffer = std::make_unique<D3DStreamBuffer>(
      text_vb_size * 2, text_vb_size * 16, nullptr);

  D3D12_GRAPHICS_PIPELINE_STATE_DESC text_pso_desc = {
      default_root_signature, // ID3D12RootSignature *pRootSignature;
      {vsbytecode->GetBufferPointer(),
       vsbytecode->GetBufferSize()}, // D3D12_SHADER_BYTECODE VS;
      {psbytecode->GetBufferPointer(),
       psbytecode->GetBufferSize()}, // D3D12_SHADER_BYTECODE PS;
      ...
      CD3DX12_DEPTH_STENCIL_DESC(
          D3D12_DEFAULT), // D3D12_DEPTH_STENCIL_DESC DepthStencilState
  ...
  CheckHR(DX12::gx_state_cache.GetPipelineStateObjectFromCache(&text_pso_desc,
                                                               &m_pso));
...
int CD3DFont::DrawTextScaled(float x, float y, float size, float spacing,
                             u32 dwColor, const std::string &text) {
  if (!m_vertex_buffer)
    return 0;
  ...
  D3D::current_command_list->IASetPrimitiveTopology(
      D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  D3D::command_list_mgr->SetCommandListPrimitiveTopology(
      D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

  D3D::current_command_list->SetGraphicsRootDescriptorTable(
      DESCRIPTOR_TABLE_PS_SRV, m_texture12_gpu);

  // upper bound is nchars * 6, assuming no spaces
  m_vertex_buffer->AllocateSpaceInBuffer(static_cast<u32>(text.length()) * 6 *
                                             sizeof(FONT2DVERTEX),
                                         sizeof(FONT2DVERTEX), false);

  FONT2DVERTEX *vertices12 = reinterpret_cast<FONT2DVERTEX *>(
      m_vertex_buffer->GetCPUAddressOfCurrentAllocation());
  int num_triangles = 0;
  float start_x = sx;
  for (char c : text) {
    if (c == '\n') {
    ...
    float h =
        static_cast<float>(ty1 - ty2) * m_tex_height * scale_y * sizeratio;
  ...
  if (num_triangles > 0) {
    ...
    D3D12_VERTEX_BUFFER_VIEW vb_view = {
        m_vertex_buffer->GetGPUAddressOfCurrentAllocation(), written_size,
        sizeof(FONT2DVERTEX)};
...
void InitUtils() {
  ...
  D3D::sampler_descriptor_heap_mgr->Allocate(&point_copy_sampler12CPU,
  ...
  D3D::sampler_descriptor_heap_mgr->Allocate(&linear_copy_sampler12CPU,
...
void SetPointCopySampler() {
  D3D::current_command_list->SetGraphicsRootDescriptorTable(
      DESCRIPTOR_TABLE_PS_SAMPLER, point_copy_sampler12GPU);
  D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_SAMPLERS,
                                                  true);
}

void SetLinearCopySampler() {
  D3D::current_command_list->SetGraphicsRootDescriptorTable(
      DESCRIPTOR_TABLE_PS_SAMPLER, linear_copy_sampler12GPU);
  D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_SAMPLERS,
                                                  true);
}

void SetViewportAndScissor(int top_left_x, int top_left_y, int width,
                           int height, float min_depth, float max_depth) {
  ...
  D3D12_RECT scissor = {static_cast<LONG>(top_left_x),
...
void DrawShadedTexQuad(D3DTexture2D *texture, const D3D12_RECT *rSource,
                       ...
                       DXGI_FORMAT rt_format, bool inherit_srv_binding,
  ...
  if (tex_quad_data.u1 != u1 || tex_quad_data.v1 != v1 ||
    ...
    stq_offset =
        util_vbuf_stq->AppendData(coords, sizeof(coords), sizeof(STQVertex));
  ...
  D3D12_VERTEX_BUFFER_VIEW vb_view = {
      ...
      static_cast<UINT>(
          util_vbuf_stq
              ->GetSize()), // UINT SizeInBytes; This is the size of the
  ...
  D3D::current_command_list->IASetVertexBuffers(0, 1, &vb_view);
  D3D::command_list_mgr->SetCommandListDirtyState(
      COMMAND_LIST_STATE_VERTEX_BUFFER, true);

  if (!inherit_srv_binding) {
    texture->TransitionToResourceState(
        D3D::current_command_list, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    D3D::current_command_list->SetGraphicsRootDescriptorTable(
        DESCRIPTOR_TABLE_PS_SRV, texture->GetSRV12GPU());
  }
  ...
  D3D12_GRAPHICS_PIPELINE_STATE_DESC pso_desc = {
      ...
      UINT_MAX,                      // UINT SampleMask;
      Renderer::GetResetRasterizerDesc(),   // D3D12_RASTERIZER_DESC
                                            // RasterizerState
      Renderer::GetResetDepthStencilDesc(), // D3D12_DEPTH_STENCIL_DESC
                                            // DepthStencilState
  ...
  CheckHR(
      DX12::gx_state_cache.GetPipelineStateObjectFromCache(&pso_desc, &pso));

  D3D::current_command_list->SetPipelineState(pso);
  D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_PSO, true);

  D3D::current_command_list->DrawInstanced(4, 1, static_cast<UINT>(stq_offset),
                                           0);
}
...
void DrawClearQuad(u32 Color, float z, D3D12_BLEND_DESC *blend_desc,
  ...
  if (clear_quad_data.col != Color || clear_quad_data.z != z) {
    clearq_offset = util_vbuf_clearq->AppendData(coords, sizeof(coords),
                                                 sizeof(ClearVertex));
  ...
  D3D12_GRAPHICS_PIPELINE_STATE_DESC pso_desc = {
      ...
      *depth_stencil_desc, // D3D12_DEPTH_STENCIL_DESC DepthStencilState
      StaticShaderCache::
          GetClearVertexShaderInputLayout(),     // D3D12_INPUT_LAYOUT_DESC
                                                 // InputLayout
      D3D12_INDEX_BUFFER_STRIP_CUT_VALUE_0xFFFF, // D3D12_INDEX_BUFFER_PROPERTIES
                                                 // IndexBufferProperties
      D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, // D3D12_PRIMITIVE_TOPOLOGY_TYPE
  ...
  D3D::current_command_list->DrawInstanced(4, 1,
                                           static_cast<UINT>(clearq_offset), 0);
}
...
void DrawEFBPokeQuads(EFBAccessType type, const EfbPokeData *points,
                      size_t num_points, D3D12_BLEND_DESC *blend_desc,
                      D3D12_DEPTH_STENCIL_DESC *depth_stencil_desc,
                      D3D12_CPU_DESCRIPTOR_HANDLE *render_target,
                      D3D12_CPU_DESCRIPTOR_HANDLE *depth_buffer,
                      bool rt_multisampled) {
  // The viewport and RT/DB are passed in so we can reconstruct the state if we
  // need to execute in
  ...
  // If drawing a large number of points at once, this will have to be split
  ...
  while (current_point_index < num_points) {
    ...
    size_t points_to_draw =
        std::min(num_points - current_point_index, points_per_draw);
    size_t required_bytes = COL_QUAD_SIZE * points_to_draw;

    void *buffer_ptr = nullptr;
    size_t base_vertex_index = util_vbuf_efbpokequads->BeginAppendData(
        &buffer_ptr, required_bytes, sizeof(ColVertex));

    CHECK(base_vertex_index * 16 + required_bytes <=
              util_vbuf_efbpokequads->GetSize(),
          "Uh oh");

    // Corresponding dirty flags set outside loop.
    D3D::current_command_list->OMSetRenderTargets(1, render_target, FALSE,
                                                  depth_buffer);
    D3D::current_command_list->IASetPrimitiveTopology(
        D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    D3D::command_list_mgr->SetCommandListPrimitiveTopology(
    ...
    D3D12_VERTEX_BUFFER_VIEW vb_view = {
        util_vbuf_efbpokequads->GetBuffer12()
            ->GetGPUVirtualAddress(), // D3D12_GPU_VIRTUAL_ADDRESS
                                      // BufferLocation;
        static_cast<UINT>(
            util_vbuf_efbpokequads
                ->GetSize()), // UINT SizeInBytes; This is the size
    ...
    D3D::current_command_list->SetPipelineState(pso);
    D3D::command_list_mgr->SetCommandListDirtyState(COMMAND_LIST_STATE_PSO,
                                                    true);
    ...
    for (size_t i = 0; i < points_to_draw; i++) {
      ...
      float z = (type == POKE_Z)
                    ? (1.0f - float(point->data & 0xFFFFFF) / 16777216.0f)
                    : 0.0f;
      u32 col = (type == POKE_Z) ? 0 : ((point->data & 0xFF00FF00) |
                                        ((point->data >> 16) & 0xFF) |
    ...
    // Issue the draw
    D3D::current_command_list->DrawInstanced(
        6 * static_cast<UINT>(points_to_draw), 1,
