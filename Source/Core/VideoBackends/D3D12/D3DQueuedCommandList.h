...
struct SetVertexBuffersArguments {
  ...
  // UINT numBuffers; - Only supporting single vertex buffer set since that's
  // all Dolphin uses.
};
...
public:
  ...
  void ProcessQueuedItems(bool eligible_to_move_to_front_of_queue = false,
                          ...
                          bool terminate_worker_thread = false);
  ...
  HRESULT STDMETHODCALLTYPE QueryInterface(
      _In_ REFIID riid, _COM_Outptr_ void __RPC_FAR *__RPC_FAR *ppvObject);

  // ID3D12Object methods

  HRESULT STDMETHODCALLTYPE
  GetPrivateData(_In_ REFGUID guid, _Inout_ UINT *pDataSize,
                 _Out_writes_bytes_opt_(*pDataSize) void *pData);

  HRESULT STDMETHODCALLTYPE SetPrivateData(_In_ REFGUID guid,
                                           _In_ UINT DataSize,
                                           _In_reads_bytes_opt_(DataSize)
                                               const void *pData);

  HRESULT STDMETHODCALLTYPE
  SetPrivateDataInterface(_In_ REFGUID guid, _In_opt_ const IUnknown *pData);
  ...
  void STDMETHODCALLTYPE DrawInstanced(_In_ UINT VertexCountPerInstance,
  ...
  void STDMETHODCALLTYPE Dispatch(_In_ UINT ThreadGroupCountX,
  ...
  void STDMETHODCALLTYPE CopyBufferRegion(_In_ ID3D12Resource *pDstBuffer,
                                          UINT64 DstOffset,
  ...
  IASetPrimitiveTopology(_In_ D3D12_PRIMITIVE_TOPOLOGY PrimitiveTopology);
  ...
  void STDMETHODCALLTYPE
  SetPipelineState(_In_ ID3D12PipelineState *pPipelineState);
  ...
  void STDMETHODCALLTYPE
  ExecuteBundle(_In_ ID3D12GraphicsCommandList *command_list);

  void STDMETHODCALLTYPE BeginQuery(_In_ ID3D12QueryHeap *pQueryHeap,
                                    _In_ D3D12_QUERY_TYPE Type,
                                    _In_ UINT Index);

  void STDMETHODCALLTYPE EndQuery(_In_ ID3D12QueryHeap *pQueryHeap,
                                  _In_ D3D12_QUERY_TYPE Type, _In_ UINT Index);

  void STDMETHODCALLTYPE
  ResolveQueryData(_In_ ID3D12QueryHeap *pQueryHeap, _In_ D3D12_QUERY_TYPE Type,
  ...
  void STDMETHODCALLTYPE
  SetDescriptorHeaps(_In_ UINT NumDescriptorHeaps,
                     _In_reads_(NumDescriptorHeaps)
                         ID3D12DescriptorHeap *const *pDescriptorHeaps);

  void STDMETHODCALLTYPE
  SetComputeRootSignature(_In_ ID3D12RootSignature *pRootSignature);

  void STDMETHODCALLTYPE
  SetGraphicsRootSignature(_In_ ID3D12RootSignature *pRootSignature);
  ...
  void STDMETHODCALLTYPE
  SetComputeRoot32BitConstant(_In_ UINT RootParameterIndex, _In_ UINT SrcData,
                              _In_ UINT DestOffsetIn32BitValues);

  void STDMETHODCALLTYPE
  SetGraphicsRoot32BitConstant(_In_ UINT RootParameterIndex, _In_ UINT SrcData,
                               _In_ UINT DestOffsetIn32BitValues);
  ...
  void STDMETHODCALLTYPE
  IASetIndexBuffer(_In_opt_ const D3D12_INDEX_BUFFER_VIEW *pDesc);

  void STDMETHODCALLTYPE
  IASetVertexBuffers(_In_ UINT StartSlot, _In_ UINT NumBuffers,
                     _In_ const D3D12_VERTEX_BUFFER_VIEW *pDesc);

  void STDMETHODCALLTYPE
  SOSetTargets(_In_ UINT StartSlot, _In_ UINT NumViews,
  ...
  void STDMETHODCALLTYPE
  ClearRenderTargetView(_In_ D3D12_CPU_DESCRIPTOR_HANDLE RenderTargetView,
  ...
  void STDMETHODCALLTYPE
  DiscardResource(_In_ ID3D12Resource *pResource,
  ...
  void STDMETHODCALLTYPE BeginEvent(UINT Metadata, _In_reads_bytes_opt_(Size)
                                                       const void *pData,
                                    UINT Size);
...
private:
  ...
  static void
  BackgroundThreadFunction(ID3D12QueuedCommandList *parent_queued_command_list);
