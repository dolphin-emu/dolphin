...
void ReplaceRGBATexture2D(ID3D12Resource *texture12, const u8 *buffer,
  ...
  if (upload_size > MAXIMUM_TEXTURE_UPLOAD_BUFFER_SIZE) {
    // If the texture is too large to fit in the upload buffer, create a
    // temporary buffer instead.
    // This will only be the case for large (e.g. 8192x8192) textures from
    ...
    CheckHR(upload_buffer->Map(0, &read_range,
    ...
    if (!s_texture_upload_stream_buffer)
      s_texture_upload_stream_buffer = std::make_unique<D3DStreamBuffer>(
          ...
          MAXIMUM_TEXTURE_UPLOAD_BUFFER_SIZE, nullptr);

    s_texture_upload_stream_buffer->AllocateSpaceInBuffer(
    ...
    upload_buffer_offset =
        s_texture_upload_stream_buffer->GetOffsetOfCurrentAllocation();
    dest_data = reinterpret_cast<u8 *>(
        s_texture_upload_stream_buffer->GetCPUAddressOfCurrentAllocation());
  }
  ...
  D3D::device12->GetCopyableFootprints(
      &texture12->GetDesc(), level, 1, upload_buffer_offset, &upload_footprint,
      &upload_rows, &upload_row_size_in_bytes, &upload_total_bytes);

  const u8 *src_data = reinterpret_cast<const u8 *>(buffer);
  for (u32 y = 0; y < upload_rows; ++y) {
    memcpy(dest_data + upload_footprint.Footprint.RowPitch * y,
           src_data + src_pitch * y, upload_row_size_in_bytes);
  }
  ...
  ResourceBarrier(D3D::current_command_list, texture12,
                  D3D12_RESOURCE_STATE_COPY_DEST, current_resource_state,
                  level);

  // Release temporary buffer after commands complete.
  // We block here because otherwise if there was a large number of texture
  // uploads, we may run out
  // of memory.
  if (!s_texture_upload_stream_buffer ||
      upload_buffer != s_texture_upload_stream_buffer->GetBuffer()) {
...
D3DTexture2D *D3DTexture2D::Create(unsigned int width, unsigned int height,
                                   u32 bind, DXGI_FORMAT fmt,
                                   unsigned int levels, unsigned int slices,
                                   D3D12_SUBRESOURCE_DATA *data) {
  ID3D12Resource *texture12 = nullptr;

  D3D12_RESOURCE_DESC texdesc12 =
      CD3DX12_RESOURCE_DESC::Tex2D(fmt, width, height, slices, levels);
  ...
  D3D::SetDebugObjectName12(texture12,
                            "Texture created via D3DTexture2D::Create");
  D3DTexture2D *ret = new D3DTexture2D(
      texture12, bind, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN,
      DXGI_FORMAT_UNKNOWN, false, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);

  if (data) {
    DX12::D3D::ReplaceRGBATexture2D(
        texture12, reinterpret_cast<const u8 *>(data->pData), width, height,
        static_cast<unsigned int>(data->RowPitch), 0,
...
void D3DTexture2D::AddRef() { m_ref.fetch_add(1); }
...
bool D3DTexture2D::GetMultisampled() const { return m_multisampled; }

ID3D12Resource *D3DTexture2D::GetTex12() const { return m_tex12; }
...
D3D12_CPU_DESCRIPTOR_HANDLE D3DTexture2D::GetDSV12() const { return m_dsv12; }

D3D12_CPU_DESCRIPTOR_HANDLE D3DTexture2D::GetRTV12() const { return m_rtv12; }
...
D3DTexture2D::D3DTexture2D(ID3D12Resource *texptr, u32 bind,
    ...
    : m_tex12(texptr), m_resource_state(resource_state),
      m_multisampled(multisampled) {
  D3D12_SRV_DIMENSION srv_dim12 = multisampled
                                      ? D3D12_SRV_DIMENSION_TEXTURE2DMSARRAY
                                      : D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
  D3D12_DSV_DIMENSION dsv_dim12 = multisampled
                                      ? D3D12_DSV_DIMENSION_TEXTURE2DMSARRAY
                                      : D3D12_DSV_DIMENSION_TEXTURE2DARRAY;
  D3D12_RTV_DIMENSION rtv_dim12 = multisampled
  ...
  if (bind & TEXTURE_BIND_FLAG_SHADER_RESOURCE) {
    ...
    CHECK(D3D::gpu_descriptor_heap_mgr->Allocate(&m_srv12_cpu, &m_srv12_gpu,
                                                 &m_srv12_gpu_cpu_shadow),
          "Error: Ran out of permenant slots in GPU descriptor heap, but don't "
          "support rolling over "
          "heap.");

    D3D::device12->CreateShaderResourceView(m_tex12, &srv_desc, m_srv12_cpu);
    D3D::device12->CreateShaderResourceView(m_tex12, &srv_desc,
                                            m_srv12_gpu_cpu_shadow);
  }
...
void D3DTexture2D::TransitionToResourceState(
    ID3D12GraphicsCommandList *command_list,
    D3D12_RESOURCE_STATES state_after) {
  DX12::D3D::ResourceBarrier(command_list, m_tex12, m_resource_state,
                             state_after,
...
D3DTexture2D::~D3DTexture2D() {
  DX12::D3D::command_list_mgr->DestroyResourceAfterCurrentCommandListExecuted(
      m_tex12);
}
