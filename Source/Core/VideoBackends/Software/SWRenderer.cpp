...
void SWRenderer::RenderText(const std::string &pstr, int left, int top,
                            u32 color) {
  ...
  for (u16 y = 0; y < fbHeight; y++) {
    for (u16 x = 0; x < fbWidth; x += 2) {
      ...
      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y1 + 1.596f * V), 0, 255);
      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y1 - 0.392f * U - 0.813f * V), 0, 255);
      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y1 + 2.017f * U), 0, 255);
      TexturePointer[offset++] = 255;

      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y2 + 1.596f * V), 0, 255);
      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y2 - 0.392f * U - 0.813f * V), 0, 255);
      TexturePointer[offset++] =
          MathUtil::Clamp(int(1.164f * Y2 + 2.017f * U), 0, 255);
      TexturePointer[offset++] = 255;
    }
    xfb += fbWidth;
  }
...
void SWRenderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbStride, u32 fbHeight,
  ...
  if (!Fifo::WillSkipCurrentFrame()) {
    if (g_ActiveConfig.bUseXFB) {
      EfbInterface::yuv422_packed *xfb =
          (EfbInterface::yuv422_packed *)Memory::GetPointer(xfbAddr);
      UpdateColorTexture(xfb, fbWidth, fbHeight);
    } else {
      EfbInterface::BypassXFB(GetCurrentColorTexture(), fbWidth, fbHeight, rc,
                              Gamma);
    }
    ...
    if (s_bScreenshot) {
      ...
      if (TextureToPng(GetCurrentColorTexture(), fbWidth * 4, s_sScreenshotName,
                       fbWidth, fbHeight, false))
  ...
  SWOGLWindow::s_instance->ShowImage(GetCurrentColorTexture(), fbWidth * 4,
                                     fbWidth, fbHeight, 1.0);
...
u16 SWRenderer::BBoxRead(int index) { return BoundingBox::coords[index]; }
...
void SWRenderer::ClearScreen(const EFBRectangle &rc, bool colorEnable,
