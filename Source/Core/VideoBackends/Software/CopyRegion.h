                ...
                const int dst_width, const int dst_height)
{
  ...
  const int copy_width = dstrect.GetWidth();
  const int copy_height = dstrect.GetHeight();

  const double x_ratio = srcrect.GetWidth() / static_cast<double>(dstrect.GetWidth());
  const double y_ratio = srcrect.GetHeight() / static_cast<double>(dstrect.GetHeight());

  for (int y_off = 0; y_off < copy_height; y_off++)
  {
    for (int x_off = 0; x_off < copy_width; x_off++)
    {
      const int dst_x = dstrect.left + x_off;
      const int dst_y = dstrect.top + y_off;
      int dst_offset = (dst_y * dst_width) + dst_x;

      const int src_x = srcrect.left + static_cast<int>(std::round(x_off * x_ratio));
      const int src_y = srcrect.top + static_cast<int>(std::round(y_off * y_ratio));
