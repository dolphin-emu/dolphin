...
static void MultipleVec3Perspective(const Vec3 &vec, const float *proj,
                                    Vec4 &result) {
...
static void MultipleVec3Ortho(const Vec3 &vec, const float *proj,
                              Vec4 &result) {
...
void TransformPosition(const InputVertexData *src, OutputVertexData *dst) {
  ...
  if (xfmem.projection.type == GX_PERSPECTIVE) {
    ...
    MultipleVec3Ortho(dst->mvPosition, xfmem.projection.rawProjection,
                      dst->projectedPosition);
  }
}
...
static void TransformTexCoordRegular(const TexMtxInfo &texinfo, int coordNum,
                                     bool specialCase,
                                     const InputVertexData *srcVertex,
                                     OutputVertexData *dstVertex) {
  const Vec3 *src;
  ...
  case XF_SRCBINORMAL_B_INROW:
    ...
    break;
  default:
    _assert_(texinfo.sourcerow >= XF_SRCTEX0_INROW &&
             texinfo.sourcerow <= XF_SRCTEX7_INROW);
  ...
  if (xfmem.dualTexTrans.enabled) {
    ...
    if (specialCase) {
      ...
      dst->x = postMat[0] * tempCoord.x + postMat[1] * tempCoord.y +
               postMat[2] + postMat[3];
      dst->y = postMat[4] * tempCoord.x + postMat[5] * tempCoord.y +
               postMat[6] + postMat[7];
      dst->z = 1.0f;
    } else {
...
static inline void AddScaledIntegerColor(const u8 *src, float scale,
                                         Vec3 &dst) {
  dst.x += src[1] * scale;
  dst.y += src[2] * scale;
  dst.z += src[3] * scale;
...
static float CalculateLightAttn(const LightPointer *light, Vec3 *_ldir,
                                const Vec3 &normal, const LitChannel &chan) {
  float attn = 1.0f;
  Vec3 &ldir = *_ldir;
  ...
  case LIGHTATTN_SPOT: {
    ...
    float cosAtt = light->cosatt.x + (light->cosatt.y * attn) +
                   (light->cosatt.z * attn * attn);
    float distAtt = light->distatt.x + (light->distatt.y * dist) +
                    (light->distatt.z * dist2);
...
static void LightColor(const Vec3 &pos, const Vec3 &normal, u8 lightNum,
                       LitChannel &chan, Vec3 &lightCol) {
...
static void LightAlpha(const Vec3 &pos, const Vec3 &normal, u8 lightNum,
                       const LitChannel &chan, float &lightCol) {
  const LightPointer *light = (const LightPointer *)&xfmem.lights[lightNum];
...
void TransformTexCoord(const InputVertexData *src, OutputVertexData *dst,
  ...
  for (u32 coordNum = 0; coordNum < xfmem.numTexGen.numTexGens; coordNum++) {
    ...
    case XF_TEXGEN_EMBOSS_MAP: {
      const LightPointer *light =
          (const LightPointer *)&xfmem.lights[texinfo.embosslightshift];

      Vec3 ldir = (light->pos - dst->mvPosition).Normalized();
      float d1 = ldir * dst->normal[1];
      float d2 = ldir * dst->normal[2];

      dst->texCoords[coordNum].x =
          dst->texCoords[texinfo.embosssourceshift].x + d1;
      dst->texCoords[coordNum].y =
          dst->texCoords[texinfo.embosssourceshift].y + d2;
  ...
  for (u32 coordNum = 0; coordNum < xfmem.numTexGen.numTexGens; coordNum++) {
    dst->texCoords[coordNum][0] *=
        (bpmem.texcoords[coordNum].s.scale_minus_1 + 1);
    dst->texCoords[coordNum][1] *=
        (bpmem.texcoords[coordNum].t.scale_minus_1 + 1);
  }
}
}
