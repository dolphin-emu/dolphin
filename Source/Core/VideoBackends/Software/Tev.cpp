...
void Tev::DrawColorRegular(TevStageCombiner::ColorCombiner &cc,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    s32 result =
        ((InputReg.d + m_BiasLUT[cc.bias]) << m_ScaleLShiftLUT[cc.shift]) +
        temp;
...
void Tev::DrawColorCompare(TevStageCombiner::ColorCombiner &cc,
  ...
  for (int i = BLU_C; i <= RED_C; i++) {
    ...
    case TEVCMP_R8_GT:
      Reg[cc.dest][i] =
          inputs[i].d + ((inputs[RED_C].a > inputs[RED_C].b) ? inputs[i].c : 0);
      break;

    case TEVCMP_R8_EQ:
      Reg[cc.dest][i] =
          inputs[i].d +
          ((inputs[RED_C].a == inputs[RED_C].b) ? inputs[i].c : 0);
      break;
    ...
    case TEVCMP_BGR24_GT: {
      u32 a =
          (inputs[BLU_C].a << 16) | (inputs[GRN_C].a << 8) | inputs[RED_C].a;
      u32 b =
          (inputs[BLU_C].b << 16) | (inputs[GRN_C].b << 8) | inputs[RED_C].b;
      Reg[cc.dest][i] = inputs[i].d + ((a > b) ? inputs[i].c : 0);
    } break;
    ...
    case TEVCMP_RGB8_GT:
      Reg[cc.dest][i] =
          inputs[i].d + ((inputs[i].a > inputs[i].b) ? inputs[i].c : 0);
      break;
...
void Tev::DrawAlphaRegular(TevStageCombiner::AlphaCombiner &ac,
  ...
  s32 result =
      ((InputReg.d + m_BiasLUT[ac.bias]) << m_ScaleLShiftLUT[ac.shift]) + temp;
...
void Tev::DrawAlphaCompare(TevStageCombiner::AlphaCombiner &ac,
  ...
  case TEVCMP_R8_GT:
    Reg[ac.dest][ALP_C] =
        inputs[ALP_C].d +
        ((inputs[RED_C].a > inputs[RED_C].b) ? inputs[ALP_C].c : 0);
    break;
  ...
  case TEVCMP_A8_GT:
    Reg[ac.dest][ALP_C] =
        inputs[ALP_C].d +
        ((inputs[ALP_C].a > inputs[ALP_C].b) ? inputs[ALP_C].c : 0);
    break;
...
static bool TevAlphaTest(int alpha) {
  bool comp0 =
      AlphaCompare(alpha, bpmem.alpha_test.ref0, bpmem.alpha_test.comp0);
  bool comp1 =
      AlphaCompare(alpha, bpmem.alpha_test.ref1, bpmem.alpha_test.comp1);
...
void Tev::Indirect(unsigned int stageNum, s32 s, s32 t) {
  ...
  if (indmtxid) {
    ...
    int scale = ((u32)indmtx.col0.s0 << 0) | ((u32)indmtx.col1.s1 << 2) |
    ...
    case 0:
      ...
      shift = (17 - scale);
      indtevtrans[0] =
          (indmtx.col0.ma * indcoord[0] + indmtx.col1.mc * indcoord[1] +
           indmtx.col2.me * indcoord[2]) >>
          3;
      indtevtrans[1] =
          (indmtx.col0.mb * indcoord[0] + indmtx.col1.md * indcoord[1] +
    ...
    indtevtrans[0] = shift >= 0 ? indtevtrans[0] >> shift : indtevtrans[0]
                                                                << -shift;
    indtevtrans[1] = shift >= 0 ? indtevtrans[1] >> shift : indtevtrans[1]
                                                                << -shift;
  }
...
void Tev::Draw() {
  ...
  for (unsigned int stageNum = 0; stageNum < bpmem.genMode.numindstages;
       stageNum++) {
    ...
    TextureSampler::Sample(Uv[texcoordSel].s >> scaleS,
                           Uv[texcoordSel].t >> scaleT, IndirectLod[stageNum],
                           IndirectLinear[stageNum], texmap,
  ...
  for (unsigned int stageNum = 0; stageNum <= bpmem.genMode.numtevstages;
       stageNum++) {
    int stageNum2 = stageNum >> 1;
    int stageOdd = stageNum & 1;
...
#if ALLOW_TEV_DUMPS
    if (g_ActiveConfig.bDumpTevStages) {
      u8 stage[4] = {(u8)Reg[0][RED_C], (u8)Reg[0][GRN_C], (u8)Reg[0][BLU_C],
                     (u8)Reg[0][ALP_C]};
  ...
  if (bpmem.fog.c_proj_fsel.fsel) {
    ...
    if (bpmem.fogRange.Base.Enabled) {
      ...
      // 0..1)
      float offset = (Position[0] - (s32(bpmem.fogRange.Base.Center) - 342)) /
                     (float)xfmem.viewport.wd;
      ...
      float klower =
          bpmem.fogRange.K[indexlower / 2].GetValue(indexlower % 2) * 4.f;
      float kupper =
          bpmem.fogRange.K[indexupper / 2].GetValue(indexupper % 2) * 4.f;
  ...
  if (g_ActiveConfig.bDumpTevStages) {
    for (u32 i = 0; i < bpmem.genMode.numindstages; ++i)
      DebugUtil::CopyTempBuffer(Position[0], Position[1], INDIRECT, i,
                                "Indirect");
  ...
  if (g_ActiveConfig.bDumpTevTextureFetches) {
    for (u32 i = 0; i <= bpmem.genMode.numtevstages; ++i) {
      ...
      if (order.getEnable(i & 1))
        DebugUtil::CopyTempBuffer(Position[0], Position[1], DIRECT_TFETCH, i,
                                  "TFetch");
    }
  }
#endif
