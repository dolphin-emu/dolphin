...
// box filter sampling averages 4 samples with the source texel being the top
// left of the box
// components are scaled to the range 0-255 after all samples are taken

static inline void BoxfilterRGBA_to_RGBA8(const u8 *src, u8 *r, u8 *g, u8 *b,
                                          u8 *a) {
...
static inline void BoxfilterRGBA_to_xx8(const u8 *src, u8 *x1, u8 *x2,
...
static inline void BoxfilterRGB_to_xx8(const u8 *src, u8 *x1, u8 *x2, int comp1,
                                       int comp2) {
  u16 x16_1 = 0;
  u16 x16_2 = 0;
...
static void SetBlockDimensions(int blkWidthLog2, int blkHeightLog2,
                               u16 *sBlkCount, u16 *tBlkCount, u16 *sBlkSize,
                               u16 *tBlkSize) {
...
static void SetSpans(int sBlkSize, int tBlkSize, s32 *tSpan, s32 *sBlkSpan,
                     s32 *tBlkSpan, s32 *writeStride) {
  ...
  *tSpan = (640 - sBlkSize) * readStride; // bytes to advance src pointer after
                                          // each row of texels in a block
  *sBlkSpan = ((-640 * tBlkSize) + sBlkSize) *
              readStride; // bytes to advance src pointer after each block
  *tBlkSpan =
      ((640 * tBlkSize) - alignedWidth) *
...
static void EncodeRGBA6(u8 *dst, u8 *src, u32 format) {
  ...
  case GX_TF_RGB565:
    ...
    ENCODE_LOOP_BLOCKS {
      ...
      u16 val = ((srcColor >> 8) & 0xf800) | ((srcColor >> 7) & 0x07e0) |
                ((srcColor >> 7) & 0x001e);
  ...
  case GX_TF_RGB5A3:
    ...
    ENCODE_LOOP_BLOCKS {
      ...
      if (alpha == 0x7000) // 555
        ...
        val = alpha | ((srcColor >> 12) & 0x0f00) |
              ((srcColor >> 10) & 0x00f0) | ((srcColor >> 8) & 0x000f);
...
static void EncodeRGBA6halfscale(u8 *dst, u8 *src, u32 format) {
  ...
  case GX_TF_RGB5A3:
    ...
    ENCODE_LOOP_BLOCKS {
      ...
      u16 val;
      if (a >= 224) // 5551
        val = 0x8000 | ((r << 7) & 0x7c00) | ((g << 2) & 0x03e0) |
              ((b >> 3) & 0x001e);
      else // 4443
        val = ((a << 7) & 0x7000) | ((r << 4) & 0x0f00) | (g & 0x00f0) |
              ((b >> 4) & 0x000f);
...
static void EncodeRGB8(u8 *dst, u8 *src, u32 format) {
  ...
  case GX_TF_RGB565:
    ...
    SetSpans(sBlkSize, tBlkSize, &tSpan, &sBlkSpan, &tBlkSpan, &writeStride);
    ENCODE_LOOP_BLOCKS {
      u16 val = ((src[2] << 8) & 0xf800) | ((src[1] << 3) & 0x07e0) |
                ((src[0] >> 3) & 0x001e);
      *(u16 *)dst = Common::swap16(val);
      src += readStride;
      dst += 2;
  ...
  case GX_TF_RGB5A3:
    ...
    SetSpans(sBlkSize, tBlkSize, &tSpan, &sBlkSpan, &tBlkSpan, &writeStride);
    ENCODE_LOOP_BLOCKS {
      u16 val = 0x8000 | ((src[2] << 7) & 0x7c00) | ((src[1] << 2) & 0x03e0) |
                ((src[0] >> 3) & 0x001e);
      *(u16 *)dst = Common::swap16(val);
      src += readStride;
      dst += 2;
...
static void EncodeRGB8halfscale(u8 *dst, u8 *src, u32 format) {
  ...
  case GX_TF_RGB5A3:
    ...
    ENCODE_LOOP_BLOCKS {
      BoxfilterRGB_to_RGB8(src, &r, &g, &b);
      u16 val = 0x8000 | ((r << 7) & 0x7c00) | ((g << 2) & 0x03e0) |
                ((b >> 3) & 0x001e);
      *(u16 *)dst = Common::swap16(val);
      src += readStride;
      dst += 2;
...
void Encode(u8 *dest_ptr) {
  ...
  if (bFromZBuffer) {
    ...
    if (copyfmt == 11)
      ...
      format |= _GX_TF_CTF;
  } else if (copyfmt > GX_TF_RGBA8 ||
             (copyfmt < GX_TF_RGB565 && !bIsIntensityFmt))
    format |= _GX_TF_CTF;

  u8 *src = EfbInterface::GetPixelPointer(bpmem.copyTexSrcXY.x,
                                          bpmem.copyTexSrcXY.y, bFromZBuffer);
