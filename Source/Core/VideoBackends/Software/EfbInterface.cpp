...
static void SetPixelAlphaOnly(u32 offset, u8 a) {
  ...
  default:
    ERROR_LOG(VIDEO, "Unsupported pixel format: %i",
...
static u32 GetSourceFactor(u8 *srcClr, u8 *dstClr,
                           BlendMode::BlendFactor mode) {
  switch (mode) {
  case BlendMode::ZERO:
    return 0;
  case BlendMode::ONE:
...
static u32 GetDestinationFactor(u8 *srcClr, u8 *dstClr,
                                BlendMode::BlendFactor mode) {
  switch (mode) {
  case BlendMode::ZERO:
    return 0;
...
static void BlendColor(u8 *srcClr, u8 *dstClr) {
  ...
  u32 dstFactor =
      GetDestinationFactor(srcClr, dstClr, bpmem.blendmode.dstfactor);
...
static void Dither(u16 x, u16 y, u8 *color) {
  ...
  if (!bpmem.blendmode.dither ||
      bpmem.zcontrol.pixel_format != PEControl::PixelFormat::RGBA6_Z24)
    return;
...
// For internal used only, return a non-normalized value, which saves work
// later.
void GetColorYUV(u16 x, u16 y, yuv444 *out) {
  ...
  out->Y = (u8)(0.257f * color[RED_C] + 0.504f * color[GRN_C] +
                0.098f * color[BLU_C]);
  out->U = (u8)(-0.148f * color[RED_C] + -0.291f * color[GRN_C] +
                0.439f * color[BLU_C]);
  out->V = (u8)(0.439f * color[RED_C] + -0.368f * color[GRN_C] +
                -0.071f * color[BLU_C]);
}
...
void CopyToXFB(yuv422_packed *xfb_in_ram, u32 fbWidth, u32 fbHeight,
  ...
  // our internal yuv444 type is not normalized, so black is {0, 0, 0} instead
  // of {16, 128, 128}
  yuv444 black;
  black.Y = 0;
  black.U = 0;
  ...
  for (u16 y = sourceRc.top; y < sourceRc.bottom; y++) {
    ...
    for (int i = 1, x = left; x < right; i += 2, x += 2) {
      ...
      // we mix our color differences in 10 bit space so it will round more
      // accurately
      // U[i] = 1/4 * U[i-1] + 1/2 * U[i] + 1/4 * U[i+1]
      xfb_in_ram[x].UV =
          128 +
          ((scanline[i - 1].U + (scanline[i].U << 1) + scanline[i + 1].U) >> 2);
      ...
      xfb_in_ram[x + 1].UV =
          128 +
          ((scanline[i].V + (scanline[i + 1].V << 1) + scanline[i + 2].V) >> 2);
    }
    xfb_in_ram += fbWidth;
  }
}

// Like CopyToXFB, but we copy directly into the OpenGL color texture without
// going via GameCube
// main memory or doing a yuyv conversion
void BypassXFB(u8 *texture, u32 fbWidth, u32 fbHeight,
               const EFBRectangle &sourceRc, float Gamma) {
