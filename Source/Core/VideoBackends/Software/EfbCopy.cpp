...
namespace EfbCopy {
static void CopyToXfb(u32 xfbAddr, u32 fbWidth, u32 fbHeight,
                      const EFBRectangle &sourceRc, float Gamma) {
  INFO_LOG(VIDEO, "xfbaddr: %x, fbwidth: %i, fbheight: %i, source: (%i, %i, "
                  "%i, %i), Gamma %f",
           xfbAddr, fbWidth, fbHeight, sourceRc.top, sourceRc.left,
           sourceRc.bottom, sourceRc.right, Gamma);
...
void CopyEfb() {
  ...
  // flipper represents the widths internally as last pixel minus starting
  // pixel, so
  ...
  if (!Fifo::WillSkipCurrentFrame()) {
    if (bpmem.triggerEFBCopy.copy_to_xfb) {
      ...
      if ((u32)xfbLines > MAX_XFB_HEIGHT) {
        INFO_LOG(VIDEO, "Tried to scale EFB to too many XFB lines (%f)",
                 xfbLines);
        xfbLines = MAX_XFB_HEIGHT;
      }

      CopyToXfb(bpmem.copyTexDest << 5, bpmem.copyMipMapStrideChannels << 4,
                (u32)xfbLines, rc, s_gammaLUT[bpmem.triggerEFBCopy.gamma]);
    } else {
      CopyToRam(); // FIXME: should use the rectangle we have already created
                   // above
    }
  }
}
