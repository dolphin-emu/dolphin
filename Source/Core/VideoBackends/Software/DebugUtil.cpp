...
void DumpActiveTextures() {
  for (unsigned int stageNum = 0; stageNum < bpmem.genMode.numindstages;
    ...
    for (s32 mip = 0; mip <= maxLod; ++mip) {
      SaveTexture(StringFromFormat(
                      "%star%i_ind%i_map%i_mip%i.png",
  ...
  for (unsigned int stageNum = 0; stageNum <= bpmem.genMode.numtevstages;
    ...
    for (s32 mip = 0; mip <= maxLod; ++mip) {
      SaveTexture(StringFromFormat(
                      "%star%i_stage%i_map%i_mip%i.png",
                      File::GetUserPath(D_DUMPTEXTURES_IDX).c_str(),
                      stats.thisFrame.numDrawnObjects, stageNum, texmap, mip),
                  texmap, mip);
    }
  ...
  if (!Fifo::WillSkipCurrentFrame()) {
    ...
    for (int i = 0; i < NUM_OBJECT_BUFFERS; i++) {
      if (DrawnToBuffer[i]) {
        DrawnToBuffer[i] = false;
        std::string filename =
            StringFromFormat("%sobject%i_%s(%i).png",
                             File::GetUserPath(D_DUMPFRAMES_IDX).c_str(),
                             stats.thisFrame.numDrawnObjects,
                             ObjectBufferName[i], i - BufferBase[i]);
