...
TextureCache::TCacheEntry::~TCacheEntry() { texture->Release(); }
...
bool TextureCache::TCacheEntry::Save(const std::string &filename,
                                     unsigned int level) {
  // TODO: Somehow implement this (D3DX11 doesn't support dumping individual
  // LODs)
  ...
  if (SUCCEEDED(hr) && pNewTexture) {
    ...
    if (SUCCEEDED(hr)) {
      saved_png = TextureToPng((u8 *)map.pData, map.RowPitch, filename,
...
void TextureCache::TCacheEntry::CopyRectangleFromTexture(
  ...
  if (srcrect.GetWidth() == dstrect.GetWidth() &&
    ...
    D3D::context->CopySubresourceRegion(
        texture->GetTex(), 0, dstrect.left, dstrect.top, 0,
  ...
  const D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(float(dstrect.left), float(dstrect.top),
...
void TextureCache::TCacheEntry::Load(unsigned int width, unsigned int height,
  ...
  D3D::ReplaceRGBATexture2D(texture->GetTex(), TextureCache::temp, width,
                            height, src_pitch, level, usage);
}
...
TextureCache::CreateTexture(const TCacheEntryConfig &config) {
  if (config.rendertarget) {
    ...
    const D3D11_TEXTURE2D_DESC texdesc = CD3D11_TEXTURE2D_DESC(
        DXGI_FORMAT_R8G8B8A8_UNORM, config.width, config.height, 1,
    ...
    TCacheEntry *const entry = new TCacheEntry(
        config, new D3DTexture2D(pTexture, D3D11_BIND_SHADER_RESOURCE));
    entry->usage = usage;
    ...
    D3D::SetDebugObjectName(
        (ID3D11DeviceChild *)entry->texture->GetSRV(),
...
void TextureCache::TCacheEntry::FromRenderTarget(
    u8 *dst, PEControl::PixelFormat srcFormat, const EFBRectangle &srcRect,
  ...
  ID3D11ShaderResourceView *efbTexSRV =
  ...
  const D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(0.f, 0.f, (float)config.width, (float)config.height);
  D3D::context->RSSetViewports(1, &vp);

  // set transformation
  if (nullptr == efbcopycbuf[cbufid]) {
    const D3D11_BUFFER_DESC cbdesc = CD3D11_BUFFER_DESC(
        28 * sizeof(float), D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DEFAULT);
    ...
    D3D::SetDebugObjectName(
        (ID3D11DeviceChild *)efbcopycbuf[cbufid],
  ...
  const D3D11_RECT sourcerect =
      CD3D11_RECT(targetSource.left, targetSource.top, targetSource.right,
                  targetSource.bottom);
  ...
  D3D::drawShadedTexQuad(
      efbTexSRV, &sourcerect, Renderer::GetTargetWidth(),
      Renderer::GetTargetHeight(),
      srcFormat == PEControl::Z24
          ? PixelShaderCache::GetDepthMatrixProgram(multisampled)
...
void TextureCache::CopyEFB(u8 *dst, u32 format, u32 native_width,
                           ...
                           bool scaleByHalf) {
  g_encoder->Encode(dst, format, native_width, bytes_per_row, num_blocks_y,
...
void TextureCache::ConvertTexture(TCacheEntryBase *entry,
  ...
  // stretch picture with increased internal resolution
  const D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(0.f, 0.f, (float)unconverted->config.width,
  ...
  // TODO: Add support for C14X2 format.  (Different multiplier, more palette
  // entries.)
  ...
  D3D::stateman->UnsetTexture(
      static_cast<TCacheEntry *>(entry)->texture->GetSRV());
  D3D::stateman->Apply();

  D3D::context->OMSetRenderTargets(
      1, &static_cast<TCacheEntry *>(entry)->texture->GetRTV(), nullptr);

  // Create texture copy
  D3D::drawShadedTexQuad(
      static_cast<TCacheEntry *>(unconverted)->texture->GetSRV(), &sourcerect,
...
TextureCache::TextureCache() {
  ...
  auto lutBd =
      CD3D11_BUFFER_DESC(sizeof(u16) * 256, D3D11_BIND_SHADER_RESOURCE);
  ...
  // TODO: C14X2 format.
  auto outlutUavDesc = CD3D11_SHADER_RESOURCE_VIEW_DESC(
      palette_buf, DXGI_FORMAT_R16_UINT, 0, 256, 0);
  hr = D3D::device->CreateShaderResourceView(palette_buf, &outlutUavDesc,
                                             &palette_buf_srv);
  ...
  D3D::SetDebugObjectName(
      (ID3D11DeviceChild *)palette_uniform,
      "a constant buffer used in TextureCache::CopyRenderTargetToTexture");
}
