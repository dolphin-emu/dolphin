...
TextureCacheBase::TCacheEntryBase* TextureCache::CreateTexture(const TCacheEntryConfig& config)
{
  ...
  else
  {
    ...
    TCacheEntry* const entry = new TCacheEntry(config, new D3DTexture2D(pTexture,
D3D11_BIND_SHADER_RESOURCE));
    entry->usage = usage;

    // TODO: better debug names
    D3D::SetDebugObjectName((ID3D11DeviceChild*)entry->texture->GetTex(), "a texture of the
TextureCache");
    D3D::SetDebugObjectName((ID3D11DeviceChild*)entry->texture->GetSRV(), "shader resource view of a
texture of the TextureCache");
...
void TextureCache::TCacheEntry::FromRenderTarget(u8* dst, PEControl::PixelFormat srcFormat, const
...
D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DEFAULT);
    ...
    D3D::SetDebugObjectName((ID3D11DeviceChild*)efbcopycbuf[cbufid], "a constant buffer used in
TextureCache::CopyRenderTargetToTexture");
  }
                                  ...
                                  TlutFormat format)
{
  ...
  const D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(0.f, 0.f, (float)unconverted->width(), (float)unconverted->height());
  D3D::context->RSSetViewports(1, &vp);
  ...
  const D3D11_RECT sourcerect = CD3D11_RECT(0, 0, unconverted->width(), unconverted->height());
  ...
  //		&sourcerect, unconverted->width(), unconverted->height(),
