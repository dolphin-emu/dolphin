...
ID3D11PixelShader *PixelShaderCache::ReinterpRGBA6ToRGB8(bool multisampled) {
  if (!multisampled || g_ActiveConfig.iMultisamples <= 1) {
    if (!s_rgba6_to_rgb8[0]) {
      s_rgba6_to_rgb8[0] =
          D3D::CompileAndCreatePixelShader(reint_rgba6_to_rgb8);
    ...
    std::string buf = StringFromFormat(reint_rgba6_to_rgb8_msaa,
    ...
    D3D::SetDebugObjectName(s_rgba6_to_rgb8[1],
                            "RGBA6 to RGB8 MSAA pixel shader");
  }
...
ID3D11PixelShader *PixelShaderCache::ReinterpRGB8ToRGBA6(bool multisampled) {
  if (!multisampled || g_ActiveConfig.iMultisamples <= 1) {
    if (!s_rgb8_to_rgba6[0]) {
      s_rgb8_to_rgba6[0] =
          D3D::CompileAndCreatePixelShader(reint_rgb8_to_rgba6);
    ...
    // create MSAA shader for current AA mode
    std::string buf = StringFromFormat(reint_rgb8_to_rgba6_msaa,
                                       g_ActiveConfig.iMultisamples);
    s_rgb8_to_rgba6[1] = D3D::CompileAndCreatePixelShader(buf);

    CHECK(s_rgb8_to_rgba6[1], "Create RGB8 to RGBA6 MSAA pixel shader");
    D3D::SetDebugObjectName(s_rgb8_to_rgba6[1],
                            "RGB8 to RGBA6 MSAA pixel shader");
  }
...
ID3D11PixelShader *PixelShaderCache::GetColorCopyProgram(bool multisampled) {
  if (!multisampled || g_ActiveConfig.iMultisamples <= 1) {
    ...
    // create MSAA shader for current AA mode
    std::string buf = StringFromFormat(color_copy_program_code_msaa,
                                       g_ActiveConfig.iMultisamples);
    s_ColorCopyProgram[1] = D3D::CompileAndCreatePixelShader(buf);
    CHECK(s_ColorCopyProgram[1] != nullptr,
          "Create color copy MSAA pixel shader");
...
ID3D11PixelShader *PixelShaderCache::GetColorMatrixProgram(bool multisampled) {
  if (!multisampled || g_ActiveConfig.iMultisamples <= 1) {
    ...
    // create MSAA shader for current AA mode
    std::string buf = StringFromFormat(color_matrix_program_code_msaa,
                                       g_ActiveConfig.iMultisamples);
    s_ColorMatrixProgram[1] = D3D::CompileAndCreatePixelShader(buf);
    CHECK(s_ColorMatrixProgram[1] != nullptr,
          "Create color matrix MSAA pixel shader");
...
ID3D11PixelShader *PixelShaderCache::GetDepthMatrixProgram(bool multisampled) {
  if (!multisampled || g_ActiveConfig.iMultisamples <= 1) {
    ...
    // create MSAA shader for current AA mode
    std::string buf = StringFromFormat(depth_matrix_program_msaa,
                                       g_ActiveConfig.iMultisamples);
    s_DepthMatrixProgram[1] = D3D::CompileAndCreatePixelShader(buf);
    CHECK(s_DepthMatrixProgram[1] != nullptr,
          "Create depth matrix MSAA pixel shader");
...
ID3D11PixelShader *PixelShaderCache::GetDepthResolveProgram() {
  ...
  // create MSAA shader for current AA mode
  std::string buf =
      StringFromFormat(depth_resolve_program, g_ActiveConfig.iMultisamples);
  s_DepthResolveProgram = D3D::CompileAndCreatePixelShader(buf);
  CHECK(s_DepthResolveProgram != nullptr,
        "Create depth matrix MSAA pixel shader");
  D3D::SetDebugObjectName((ID3D11DeviceChild *)s_DepthResolveProgram,
...
ID3D11Buffer *&PixelShaderCache::GetConstantBuffer() {
  ...
  if (PixelShaderManager::dirty) {
    ...
    memcpy(map.pData, &PixelShaderManager::constants,
           sizeof(PixelShaderConstants));
    D3D::context->Unmap(pscbuf, 0);

    // Again, this is hacky.
    D3D::context->Map(uber_bufffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
    memcpy(map.pData, &PixelShaderManager::more_constants,
...
class PixelShaderCacheInserter
    : public LinearDiskCacheReader<UberShader::PixelShaderUid, u8> {
public:
  void Read(const UberShader::PixelShaderUid &key, const u8 *value,
            u32 value_size) {
...
void PixelShaderCache::Init() {
  unsigned int cbsize =
      ROUND_UP(sizeof(PixelShaderConstants), 16); // must be a multiple of 16
  D3D11_BUFFER_DESC cbdesc =
      CD3D11_BUFFER_DESC(cbsize, D3D11_BIND_CONSTANT_BUFFER,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)s_ClearProgram,
                          "clear pixel shader");
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)s_ColorMatrixProgram[0],
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)s_DepthMatrixProgram[0],
  ...
  std::string cache_filename = StringFromFormat(
      "%sdx11-%s-ps.cache", File::GetUserPath(D_SHADERCACHE_IDX).c_str(),
...
bool PixelShaderCache::SetShader(DSTALPHA_MODE dstAlphaMode) {
  ...
  // uid.GetUidData());
  ShaderCode code = UberShader::GenPixelShader(
      dstAlphaMode, API_D3D, false, g_ActiveConfig.iMultisamples > 1,
...
bool PixelShaderCache::InsertByteCode(const UberShader::PixelShaderUid &uid,
                                      const void *bytecode,
                                      unsigned int bytecodelen) {
  ID3D11PixelShader *shader =
      D3D::CreatePixelShaderFromByteCode(bytecode, bytecodelen);
  if (shader == nullptr)
    return false;
