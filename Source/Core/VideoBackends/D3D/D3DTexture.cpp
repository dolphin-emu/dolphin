...
void ReplaceRGBATexture2D(ID3D11Texture2D *pTexture, const u8 *buffer,
  ...
  if (usage == D3D11_USAGE_DYNAMIC || usage == D3D11_USAGE_STAGING) {
    ...
    if (src_pitch == map.RowPitch) {
      ...
      // Source row size is aligned to texture block size. This can result in a
      // different
      // pitch to what the driver returns, so copy whichever is smaller.
      unsigned int copy_size = std::min(src_pitch, map.RowPitch);
      for (unsigned int y = 0; y < height; ++y)
        memcpy((u8 *)map.pData + y * map.RowPitch, buffer + y * src_pitch,
               copy_size);
    }
    D3D::context->Unmap(pTexture, level);
  } else {
    D3D11_BOX dest_region = CD3D11_BOX(0, 0, 0, width, height, 1);
    D3D::context->UpdateSubresource(pTexture, level, &dest_region, buffer,
                                    src_pitch, src_pitch * height);
  }
}
...
D3DTexture2D *D3DTexture2D::Create(unsigned int width, unsigned int height,
                                   ...
                                   DXGI_FORMAT fmt, unsigned int levels,
                                   unsigned int slices,
  ...
  if (usage == D3D11_USAGE_STAGING)
    cpuflags = (D3D11_CPU_ACCESS_FLAG)((int)D3D11_CPU_ACCESS_WRITE |
  ...
  D3D11_TEXTURE2D_DESC texdesc = CD3D11_TEXTURE2D_DESC(
      fmt, width, height, slices, levels, bind, usage, cpuflags);
...
D3DTexture2D::D3DTexture2D(ID3D11Texture2D *texptr, D3D11_BIND_FLAG bind,
  ...
  D3D11_DEPTH_STENCIL_VIEW_DESC dsv_desc =
      CD3D11_DEPTH_STENCIL_VIEW_DESC(dsv_dim, dsv_format);
  D3D11_RENDER_TARGET_VIEW_DESC rtv_desc =
      CD3D11_RENDER_TARGET_VIEW_DESC(rtv_dim, rtv_format);
