...
ID3D11UnorderedAccessView *&BBox::GetUAV() { return s_bbox_uav; }
...
void BBox::Init() {
  if (g_ActiveConfig.backend_info.bSupportsBBox) {
    ...
    D3D::SetDebugObjectName(s_bbox_staging_buffer,
                            "BoundingBox Staging Buffer");
    ...
    hr = D3D::device->CreateUnorderedAccessView(s_bbox_buffer, &UAVdesc,
                                                &s_bbox_uav);
...
int BBox::Get(int index) {
  ...
  HRESULT hr =
      D3D::context->Map(s_bbox_staging_buffer, 0, D3D11_MAP_READ, 0, &map);
  if (SUCCEEDED(hr)) {
