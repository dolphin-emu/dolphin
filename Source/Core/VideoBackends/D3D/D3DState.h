...
public:
  ...
  void SetPixelConstants(ID3D11Buffer *buffer0,
                         ID3D11Buffer *buffer1 = nullptr) {
    if (m_current.pixelConstants[0] != buffer0 ||
        m_current.pixelConstants[1] != buffer1)
  ...
  void SetPixelShaderDynamic(ID3D11PixelShader *shader,
  ...
  // removes currently set texture from all slots, returns mask of previously
  // bound slots
  u32 UnsetTexture(ID3D11ShaderResourceView *srv);
  void SetTextureByMask(u32 textureSlotMask, ID3D11ShaderResourceView *srv);

  // call this immediately before any drawing operation or to explicitly apply
  // pending resource
  // state changes
  void Apply();
