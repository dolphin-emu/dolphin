...
std::vector<DXGI_SAMPLE_DESC>
    aa_modes; // supported AA modes of the current adapter
...
HRESULT LoadDXGI() {
  ...
  if (!hDXGIDll) {
    MessageBoxA(nullptr, "Failed to load dxgi.dll", "Critical error",
                MB_OK | MB_ICONERROR);
    --dxgi_dll_ref;
    return E_FAIL;
  }
  PCreateDXGIFactory =
      (CREATEDXGIFACTORY)GetProcAddress(hDXGIDll, "CreateDXGIFactory");
  if (PCreateDXGIFactory == nullptr)
    MessageBoxA(nullptr, "GetProcAddress failed for CreateDXGIFactory!",
...
HRESULT LoadD3D() {
  ...
  if (!hD3DDll) {
    MessageBoxA(nullptr, "Failed to load d3d11.dll", "Critical error",
                MB_OK | MB_ICONERROR);
    --d3d_dll_ref;
    return E_FAIL;
  }
  PD3D11CreateDevice =
      (D3D11CREATEDEVICE)GetProcAddress(hD3DDll, "D3D11CreateDevice");
  if (PD3D11CreateDevice == nullptr)
    MessageBoxA(nullptr, "GetProcAddress failed for D3D11CreateDevice!",
                "Critical error", MB_OK | MB_ICONERROR);

  PD3D11CreateDeviceAndSwapChain =
      (D3D11CREATEDEVICEANDSWAPCHAIN)GetProcAddress(
          hD3DDll, "D3D11CreateDeviceAndSwapChain");
  if (PD3D11CreateDeviceAndSwapChain == nullptr)
    MessageBoxA(nullptr,
                "GetProcAddress failed for D3D11CreateDeviceAndSwapChain!",
                "Critical error", MB_OK | MB_ICONERROR);
...
HRESULT LoadD3DCompiler() {
  ...
  // try to load D3DCompiler first to check whether we have proper runtime
  // support
  // try to use the dll the backend was compiled against first - don't bother
  // about debug runtimes
  hD3DCompilerDll = LoadLibraryA(D3DCOMPILER_DLL_A);
  if (!hD3DCompilerDll) {
    // if that fails, use the dll which should be available in every SDK which
    // officially supports
    // DX11.
    ...
    if (!hD3DCompilerDll) {
      ...
      NOTICE_LOG(VIDEO, "Successfully loaded D3DCompiler_42.dll. If you're "
                        "having trouble, try "
  ...
  if (PD3DReflect == nullptr)
    MessageBoxA(nullptr, "GetProcAddress failed for D3DReflect!",
                "Critical error", MB_OK | MB_ICONERROR);
  PD3DCompile = (pD3DCompile)GetProcAddress(hD3DCompilerDll, "D3DCompile");
  if (PD3DCompile == nullptr)
    MessageBoxA(nullptr, "GetProcAddress failed for D3DCompile!",
                "Critical error", MB_OK | MB_ICONERROR);
...
std::vector<DXGI_SAMPLE_DESC> EnumAAModes(IDXGIAdapter *adapter) {
  ...
  // NOTE: D3D 10.0 doesn't support multisampled resources which are bound as
  // depth buffers AND
  // shader resources.
  ...
  HRESULT hr =
      PD3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr,
  ...
  if (FAILED(hr) || feat_level == D3D_FEATURE_LEVEL_10_0) {
    ...
    for (int samples = 0; samples < D3D11_MAX_MULTISAMPLE_SAMPLE_COUNT;
         ++samples) {
...
HRESULT Create(HWND wnd) {
  ...
  if (FAILED(hr)) {
    ...
    if (FAILED(hr))
      MessageBox(
          ...
          _T("This usually happens when you've set your video adapter to the ")
          _T("Nvidia ")
          _T("GPU in an Optimus-equipped system.\n")
          _T("Set Dolphin to use the high-performance graphics in Nvidia's ")
          _T("drivers ")
  ...
  if (std::find_if(aa_modes.begin(), aa_modes.end(),
  ...
  hr = output->FindClosestMatchingMode(&mode_desc, &swap_chain_desc.BufferDesc,
                                       nullptr);
  if (FAILED(hr))
  ...
  // Creating debug devices can sometimes fail if the user doesn't have the
  // correct
  // version of the DirectX SDK. If it does, simply fallback to a non-debug
  // device.
  {
    hr = PD3D11CreateDeviceAndSwapChain(
        ...
        supported_feature_levels, NUM_SUPPORTED_FEATURE_LEVELS,
        D3D11_SDK_VERSION, &swap_chain_desc, &swapchain, &device, &featlevel,
        &context);
    // Debugbreak on D3D error
    if (SUCCEEDED(hr) && SUCCEEDED(device->QueryInterface(__uuidof(ID3D11Debug),
                                                          (void **)&debug))) {
...
#endif
  {
    hr = PD3D11CreateDeviceAndSwapChain(
        ...
        NUM_SUPPORTED_FEATURE_LEVELS, D3D11_SDK_VERSION, &swap_chain_desc,
        &swapchain, &device, &featlevel, &context);
  }

  if (FAILED(hr)) {
    MessageBox(wnd, _T("Failed to initialize Direct3D.\nMake sure your video ")
                    _T("card supports at least D3D 10.0"),
               _T("Dolphin Direct3D 11 backend"), MB_OK | MB_ICONERROR);
    SAFE_RELEASE(device);
  ...
  // prevent DXGI from responding to Alt+Enter, unfortunately
  // DXGI_MWA_NO_ALT_ENTER
  // does not work so we disable all monitoring of window messages. However this
  // may make it more difficult for DXGI to handle display mode changes.
  hr = factory->MakeWindowAssociation(wnd, DXGI_MWA_NO_WINDOW_CHANGES);
  if (FAILED(hr))
    MessageBox(wnd, _T("Failed to associate the window"),
               _T("Dolphin Direct3D 11 backend"), MB_OK | MB_ICONERROR);
  ...
  if (FAILED(hr)) {
    MessageBox(wnd, _T("Failed to get swapchain buffer"),
               _T("Dolphin Direct3D 11 backend"), MB_OK | MB_ICONERROR);
    SAFE_RELEASE(device);
    SAFE_RELEASE(context);
    SAFE_RELEASE(swapchain);
  ...
  SetDebugObjectName((ID3D11DeviceChild *)backbuf->GetTex(),
                     "backbuffer texture");
  SetDebugObjectName((ID3D11DeviceChild *)backbuf->GetRTV(),
                     "backbuffer render target view");

  context->OMSetRenderTargets(1, &backbuf->GetRTV(), nullptr);

  // BGRA textures are easier to deal with in TextureCache, but might not be
  // supported by the
  // hardware
  UINT format_support;
  device->CheckFormatSupport(DXGI_FORMAT_B8G8R8A8_UNORM, &format_support);
  bgra_textures_supported =
...
#if defined(_DEBUG) || defined(DEBUGFAST)
  if (debug) {
    --references; // the debug interface increases the refcount of the device,
                  // subtract that.
    if (references) {
      // print out alive objects, but only if we actually have pending
      // references
      // note this will also print out internal live objects to the debug
      // console
...
// Returns the maximum width/height of a texture. This value only depends upon
// the feature level in
// DX11
...
void Reset() {
  ...
  if (FAILED(hr)) {
    MessageBox(hWnd, _T("Failed to get swapchain buffer"),
               _T("Dolphin Direct3D 11 backend"), MB_OK | MB_ICONERROR);
    SAFE_RELEASE(device);
    SAFE_RELEASE(context);
    SAFE_RELEASE(swapchain);
