...
public:
  ...
  //     either create an ID3D11Texture2D object, pass it to the constructor and
  //     specify what views
  //     to create
  ...
  static D3DTexture2D *Create(unsigned int width, unsigned int height,
                              D3D11_BIND_FLAG bind, D3D11_USAGE usage,
                              DXGI_FORMAT, unsigned int levels = 1,
                              unsigned int slices = 1,
                              D3D11_SUBRESOURCE_DATA *data = nullptr);

  // reference counting, use AddRef() when creating a new reference and
