...
namespace DX11
{
	...
	bool AvatarDrawer::CalculateTrackingSpaceToViewSpaceMatrix(int kind, Matrix44 &look_matrix)
	{
		...
		if (bIsPerspective && !bIsHudElement && !bIsOffscreen)
		{
			...
			Matrix44 A, B;
			Matrix44::Multiply(head_position_matrix, rotation_matrix, A);
			Matrix44::Multiply(lean_back_matrix, A, B);
			Matrix44::Multiply(free_look_matrix, B, A);
			Matrix44::Multiply(camera_pitch_matrix, A, B);
			Matrix44::Multiply(camera_position_matrix, B, A);
			Matrix44::Multiply(camera_forward_matrix, A, look_matrix);
		}
		else
		{
			float HudWidth, HudHeight, HudThickness, HudDistance, HudUp, CameraForward, AimDistance;
			...
			Matrix44::Multiply(scale_matrix, position_matrix, box_matrix);
