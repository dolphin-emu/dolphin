...
void Television::Init()
{
  ...
  D3D11_SHADER_RESOURCE_VIEW_DESC srvd = CD3D11_SHADER_RESOURCE_VIEW_DESC(
      m_yuyvTexture.Get(), D3D11_SRV_DIMENSION_TEXTURE2D, DXGI_FORMAT_G8R8_G8B8_UNORM);
  hr = D3D::device->CreateShaderResourceView(m_yuyvTexture.Get(), &srvd, &m_yuyvTextureSRV);
...
void Television::Submit(u32 xfbAddr, u32 stride, u32 width, u32 height)
{
  ...
  D3D::context->UpdateSubresource(m_yuyvTexture.Get(), 0, &box, yuyvSrc, 2 * stride,
                                  2 * stride * height);
}
...
void Television::Render()
{
  if (g_ActiveConfig.bUseRealXFB && g_ActiveConfig.bUseXFB)
  {
    ...
    D3D::drawShadedTexQuad(m_yuyvTextureSRV.Get(), &sourceRc, MAX_XFB_WIDTH, MAX_XFB_HEIGHT,
                           m_pShader.Get(), VertexShaderCache::GetSimpleVertexShader(),
