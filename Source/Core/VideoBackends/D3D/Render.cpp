...
static void SetupDeviceObjects() {
  ...
  D3D::SetDebugObjectName(
      (ID3D11DeviceChild *)cleardepthstates[0],
  ...
  D3D11_RASTERIZER_DESC rastdesc =
      CD3D11_RASTERIZER_DESC(D3D11_FILL_SOLID, D3D11_CULL_NONE, false, 0, 0.f,
...
static void CreateScreenshotTexture() {
  ...
  D3D11_TEXTURE2D_DESC scrtex_desc = CD3D11_TEXTURE2D_DESC(
      DXGI_FORMAT_R8G8B8A8_UNORM, D3D::GetBackBufferWidth(),
      D3D::GetBackBufferHeight(), 1, 1, 0, D3D11_USAGE_STAGING,
      D3D11_CPU_ACCESS_READ | D3D11_CPU_ACCESS_WRITE);
  HRESULT hr = D3D::device->CreateTexture2D(&scrtex_desc, nullptr,
                                            &s_screenshot_texture);
  CHECK(hr == S_OK, "Create screenshot staging texture");
  D3D::SetDebugObjectName((ID3D11DeviceChild *)s_screenshot_texture,
                          "staging screenshot texture");
}
...
static D3D11_BOX GetScreenshotSourceBox(const TargetRectangle &targetRc) {
  ...
  return CD3D11_BOX(
      std::max(targetRc.left, 0), std::max(targetRc.top, 0), 0,
...
static void Create3DVisionTexture(int width, int height) {
  ...
  LPNVSTEREOIMAGEHEADER header = (LPNVSTEREOIMAGEHEADER)(
      (u8 *)sysData.pSysMem + height * sysData.SysMemPitch);
  ...
  s_3d_vision_texture = D3DTexture2D::Create(
      width * 2, height + 1, D3D11_BIND_RENDER_TARGET, D3D11_USAGE_DEFAULT,
...
Renderer::Renderer(void *&window_handle) {
  ...
  D3D::context->ClearRenderTargetView(
      FramebufferManager::GetEFBColorTexture()->GetRTV(), ClearColor);
  D3D::context->ClearDepthStencilView(
      FramebufferManager::GetEFBDepthTexture()->GetDSV(), D3D11_CLEAR_DEPTH,
      0.f, 0);

  D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(0.f, 0.f, (float)s_target_width, (float)s_target_height);
...
void Renderer::RenderText(const std::string &text, int left, int top,
                          u32 color) {
  D3D::font.DrawTextScaled((float)(left + 1), (float)(top + 1), 20.f, 0.0f,
...
u32 Renderer::AccessEFB(EFBAccessType type, u32 x, u32 y, u32 poke_data) {
  ...
  D3D::drawShadedTexQuad(source_tex->GetSRV(), &RectToLock,
  ...
  D3D::context->CopySubresourceRegion(staging_tex, 0, 0, 0, 0,
                                      read_tex->GetTex(), 0, &box);
  ...
  if (type == PEEK_COLOR) {
    ...
    val =
        ((val & 0xFF00FF00) | ((val >> 16) & 0xFF) | ((val << 16) & 0xFF0000));
  ...
  } else                        // type == PEEK_Z
  {
    ...
    if (bpmem.zcontrol.pixel_format == PEControl::RGB565_Z16) {
      ...
      ret = MathUtil::Clamp<u32>(static_cast<u32>(val * 16777216.0f), 0,
                                 0xFFFFFF);
    }
  }
...
void Renderer::PokeEFB(EFBAccessType type, const EfbPokeData *points,
  ...
  if (type == POKE_COLOR) {
    D3D11_VIEWPORT vp = CD3D11_VIEWPORT(0.0f, 0.0f, (float)GetTargetWidth(),
                                        (float)GetTargetHeight());
    D3D::context->RSSetViewports(1, &vp);
    D3D::context->OMSetRenderTargets(
        1, &FramebufferManager::GetEFBColorTexture()->GetRTV(), nullptr);
  } else // if (type == POKE_Z)
  {
...
void Renderer::SetViewport() {
  ...
  float X = Renderer::EFBToScaledXf(xfmem.viewport.xOrig - xfmem.viewport.wd -
                                    scissorXOff);
  float Y = Renderer::EFBToScaledYf(xfmem.viewport.yOrig + xfmem.viewport.ht -
                                    scissorYOff);
  ...
  D3D11_VIEWPORT vp = CD3D11_VIEWPORT(
      X, Y, Wd, Ht,
      1.0f -
          MathUtil::Clamp<float>(xfmem.viewport.farZ, 0.0f, 16777215.0f) /
              16777216.0f,
      1.0f -
          MathUtil::Clamp<float>(
              xfmem.viewport.farZ -
                  MathUtil::Clamp<float>(xfmem.viewport.zRange, 0.0f,
                                         16777216.0f),
              0.0f, 16777215.0f) /
              16777216.0f);
  D3D::context->RSSetViewports(1, &vp);
...
void Renderer::ClearScreen(const EFBRectangle &rc, bool colorEnable,
  ...
  D3D11_VIEWPORT vp = CD3D11_VIEWPORT((float)targetRc.left, (float)targetRc.top,
                                      (float)targetRc.GetWidth(),
                                      (float)targetRc.GetHeight(), 0.f, 1.f);
  D3D::context->RSSetViewports(1, &vp);

  // Color is passed in bgra mode so we need to convert it to rgba
  u32 rgbaColor = (color & 0xFF00FF00) | ((color >> 16) & 0xFF) |
                  ((color << 16) & 0xFF0000);
...
void Renderer::ReinterpretPixelData(unsigned int convtype) {
  // TODO: MSAA support..
  D3D11_RECT source = CD3D11_RECT(0, 0, g_renderer->GetTargetWidth(),
  ...
  D3D11_VIEWPORT vp =
      CD3D11_VIEWPORT(0.f, 0.f, (float)g_renderer->GetTargetWidth(),
                      (float)g_renderer->GetTargetHeight());
  D3D::context->RSSetViewports(1, &vp);

  D3D::context->OMSetRenderTargets(
      1, &FramebufferManager::GetEFBColorTempTexture()->GetRTV(), nullptr);
  D3D::SetPointCopySampler();
  D3D::drawShadedTexQuad(FramebufferManager::GetEFBColorTexture()->GetSRV(),
                         &source, g_renderer->GetTargetWidth(),
                         g_renderer->GetTargetHeight(), pixel_shader,
                         VertexShaderCache::GetSimpleVertexShader(),
                         VertexShaderCache::GetSimpleInputLayout(),
...
void Renderer::SetBlendMode(bool forceUpdate) {
  ...
  const D3D11_BLEND d3dSrcFactors[8] = {
      D3D11_BLEND_ZERO, D3D11_BLEND_ONE, D3D11_BLEND_DEST_COLOR,
      D3D11_BLEND_INV_DEST_COLOR, D3D11_BLEND_SRC_ALPHA,
...
bool Renderer::SaveScreenshot(const std::string &filename,
  ...
  D3D::context->CopySubresourceRegion(
      ...
      (ID3D11Resource *)D3D::GetBackBuffer()->GetTex(), 0, &source_box);

  D3D11_MAPPED_SUBRESOURCE map;
  D3D::context->Map(s_screenshot_texture, 0, D3D11_MAP_READ_WRITE, 0, &map);

  bool saved_png = TextureToPng((u8 *)map.pData, map.RowPitch, filename,
...
void Renderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbStride, u32 fbHeight,
  ...
  if ((!xfbSourceList || xfbCount == 0) && g_ActiveConfig.bUseXFB &&
  ...
  float ClearColor[4] = {0.f, 0.f, 0.f, 1.f};
  D3D::context->ClearRenderTargetView(D3D::GetBackBuffer()->GetRTV(),
                                      ClearColor);
  ...
  if (g_ActiveConfig.bUseXFB && g_ActiveConfig.bUseRealXFB) {
    ...
    D3D11_VIEWPORT vp = CD3D11_VIEWPORT(
        (float)targetRc.left, (float)targetRc.top, (float)targetRc.GetWidth(),
        (float)targetRc.GetHeight());
    D3D::context->RSSetViewports(1, &vp);
    ...
    for (u32 i = 0; i < xfbCount; ++i) {
      ...
      drawRc.top =
          targetRc.top + hOffset * targetRc.GetHeight() / (s32)fbHeight;
      drawRc.bottom =
          targetRc.top +
          (hOffset + xfbHeight) * targetRc.GetHeight() / (s32)fbHeight;
      drawRc.left = targetRc.left +
                    (targetRc.GetWidth() -
                     xfbWidth * targetRc.GetWidth() / (s32)fbStride) /
                        2;
      ...
      BlitScreen(sourceRc, drawRc, xfbSource->tex, xfbSource->texWidth,
                 xfbSource->texHeight, Gamma);
    }
  } else {
    ...
    // texture as source
    D3DTexture2D *read_texture =
        FramebufferManager::GetResolvedEFBColorTexture();
    BlitScreen(sourceRc, targetRc, read_texture, GetTargetWidth(),
               GetTargetHeight(), Gamma);
  }
  ...
  if (SConfig::GetInstance().m_DumpFrames) {
    ...
    if (!bLastFrameDumped) {
      ...
      bAVIDumping = AVIDump::Start(s_recordWidth, s_recordHeight,
                                   AVIDump::DumpFormat::FORMAT_BGR);
      if (!bAVIDumping) {
        PanicAlert("Error dumping frames to AVI.");
      } else {
        std::string msg = StringFromFormat(
            "Dumping Frames to \"%sframedump0.avi\" (%dx%d RGB24)",
            File::GetUserPath(D_DUMPFRAMES_IDX).c_str(), s_recordWidth,
            s_recordHeight);
    ...
    if (bAVIDumping) {
      ...
      formatBufferDump((u8 *)map.pData, &frame_data[0], source_width,
                       source_height, map.RowPitch);
  ...
  D3D11_VIEWPORT vp = CD3D11_VIEWPORT(0.0f, 0.0f, (float)GetBackbufferWidth(),
  ...
  if (CalculateTargetSize(s_backbuffer_width, s_backbuffer_height) ||
      xfbchanged || windowResized || fullscreen_changed ||
      s_last_efb_scale != g_ActiveConfig.iEFBScale ||
      s_last_multisamples != g_ActiveConfig.iMultisamples ||
      s_last_stereo_mode != (g_ActiveConfig.iStereoMode > 0)) {
    s_last_xfb_mode = g_ActiveConfig.bUseRealXFB;
    ...
    D3D::context->OMSetRenderTargets(1, &D3D::GetBackBuffer()->GetRTV(),
                                     nullptr);
    ...
    D3D::context->ClearRenderTargetView(
        FramebufferManager::GetEFBColorTexture()->GetRTV(), clear_col);
    D3D::context->ClearDepthStencilView(
        FramebufferManager::GetEFBDepthTexture()->GetDSV(), D3D11_CLEAR_DEPTH,
        0.f, 0);
  }
...
void Renderer::ApplyState(bool bUseDstAlpha) {
  ...
  for (unsigned int stage = 0; stage < 8; stage++) {
    ...
    gx_state.sampler[stage].max_anisotropy = UINT64_C(1)
                                             << g_ActiveConfig.iMaxAnisotropy;
    D3D::stateman->SetSampler(stage,
                              gx_state_cache.Get(gx_state.sampler[stage]));
  }
...
void Renderer::RestoreCull() { D3D::stateman->PopRasterizerState(); }

void Renderer::SetGenerationMode() {
  const D3D11_CULL_MODE d3dCullModes[4] = {D3D11_CULL_NONE, D3D11_CULL_BACK,
                                           D3D11_CULL_FRONT, D3D11_CULL_BACK};
...
void Renderer::SetDepthMode() { gx_state.zmode.hex = bpmem.zmode.hex; }
...
void Renderer::SetSamplerState(int stage, int texindex, bool custom_tex) {
  ...
  if (g_ActiveConfig.bForceFiltering) {
    ...
    gx_state.sampler[stage].min_filter =
        SamplerCommon::AreBpTexMode0MipmapsEnabled(tm0) ? 6 : 4;
...
int Renderer::GetMaxTextureSize() { return DX11::D3D::GetMaxTextureSize(); }
...
void Renderer::BlitScreen(TargetRectangle src, TargetRectangle dst,
                          D3DTexture2D *src_texture, u32 src_width,
                          u32 src_height, float Gamma) {
  if (g_ActiveConfig.iStereoMode == STEREO_SBS ||
    ...
    D3D11_VIEWPORT leftVp =
        CD3D11_VIEWPORT((float)leftRc.left, (float)leftRc.top,
                        (float)leftRc.GetWidth(), (float)leftRc.GetHeight());
    D3D11_VIEWPORT rightVp =
        CD3D11_VIEWPORT((float)rightRc.left, (float)rightRc.top,
    ...
    D3D11_VIEWPORT leftVp =
        CD3D11_VIEWPORT((float)dst.left, (float)dst.top, (float)dst.GetWidth(),
                        (float)dst.GetHeight());
    D3D11_VIEWPORT rightVp =
        CD3D11_VIEWPORT((float)(dst.left + s_backbuffer_width), (float)dst.top,
                        (float)dst.GetWidth(), (float)dst.GetHeight());

    // Render to staging texture which is double the width of the backbuffer
    D3D::context->OMSetRenderTargets(1, &s_3d_vision_texture->GetRTV(),
                                     nullptr);
    ...
    D3D11_BOX box =
        CD3D11_BOX(0, 0, 0, s_backbuffer_width, s_backbuffer_height, 1);
    D3D::context->CopySubresourceRegion(D3D::GetBackBuffer()->GetTex(), 0, 0, 0,
                                        0, s_3d_vision_texture->GetTex(), 0,
                                        &box);
