...
struct CD3D11_VIEWPORT : public D3D11_VIEWPORT
{
  CD3D11_VIEWPORT(FLOAT x, FLOAT y, FLOAT w, FLOAT h,
                  FLOAT minD = D3D11_MIN_DEPTH, FLOAT maxD = D3D11_MAX_DEPTH)
  {
...
struct CD3D11_BOX : public D3D11_BOX
{
  ...
  {
    left = l; top = t; front = f; right = r; bottom = b; back = ba;
  }
};
...
struct CD3D11_UNORDERED_ACCESS_VIEW_DESC : public D3D11_UNORDERED_ACCESS_VIEW_DESC
{
  ...
  CD3D11_UNORDERED_ACCESS_VIEW_DESC(DXGI_FORMAT fmt,
  ...
  CD3D11_UNORDERED_ACCESS_VIEW_DESC(ID3D11Texture2D* pTex2D,
                                    D3D11_UAV_DIMENSION dim,
                                    ...
                                    UINT arraySize = (UINT)-1)
  {
    ...
    if (pTex2D) pTex2D->GetDesc(&tex);

    if (fmt == DXGI_FORMAT_UNKNOWN) fmt = tex.Format;
    if (arraySize == (UINT)-1 &&
        (dim == D3D11_UAV_DIMENSION_TEXTURE2DARRAY))
...
struct CD3D11_DEPTH_STENCIL_VIEW_DESC : public D3D11_DEPTH_STENCIL_VIEW_DESC
{
  ...
  CD3D11_DEPTH_STENCIL_VIEW_DESC(ID3D11Texture2D* pTex2D,
                                 ...
                                 UINT flags = 0)
  {
    ...
    if (format == DXGI_FORMAT_UNKNOWN) format = tex.Format;
    if (arraySize == (UINT)-1 &&
