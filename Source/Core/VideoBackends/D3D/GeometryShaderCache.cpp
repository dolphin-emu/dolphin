...
UidChecker<GeometryShaderUid, ShaderCode>
    GeometryShaderCache::geometry_uid_checker;
...
ID3D11Buffer *&GeometryShaderCache::GetConstantBuffer() {
  ...
  if (GeometryShaderManager::dirty) {
    ...
    memcpy(map.pData, &GeometryShaderManager::constants,
...
class GeometryShaderCacheInserter
    : public LinearDiskCacheReader<GeometryShaderUid, u8> {
public:
...
void GeometryShaderCache::Init() {
  unsigned int gbsize =
      ROUND_UP(sizeof(GeometryShaderConstants), 16); // must be a multiple of 16
  D3D11_BUFFER_DESC gbdesc =
      CD3D11_BUFFER_DESC(gbsize, D3D11_BIND_CONSTANT_BUFFER,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)ClearGeometryShader,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)CopyGeometryShader,
  ...
  std::string cache_filename = StringFromFormat(
      "%sdx11-%s-gs.cache", File::GetUserPath(D_SHADERCACHE_IDX).c_str(),
...
bool GeometryShaderCache::SetShader(u32 primitive_type) {
  ...
  if (g_ActiveConfig.bEnableShaderDebugging) {
    ShaderCode code =
        GenerateGeometryShaderCode(primitive_type, API_D3D, uid.GetUidData());
...
bool GeometryShaderCache::InsertByteCode(const GeometryShaderUid &uid,
                                         const void *bytecode,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)shader,
                          "a pixel shader of GeometryShaderCache");
