...
public:
  UtilVertexBuffer(int size) : buf(nullptr), offset(0), max_size(size) {
    D3D11_BUFFER_DESC desc =
        CD3D11_BUFFER_DESC(max_size, D3D11_BIND_VERTEX_BUFFER,
...
inline FONT2DVERTEX InitFont2DVertex(float x, float y, u32 color, float tu,
                                     float tv) {
  FONT2DVERTEX v;
  v.x = x;
  v.y = y;
...
int CD3DFont::Init() {
  ...
  D3D11_TEXTURE2D_DESC texdesc = CD3D11_TEXTURE2D_DESC(
      DXGI_FORMAT_R8G8B8A8_UNORM, m_dwTexWidth, m_dwTexHeight, 1, 1,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)buftex,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)m_pshader,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)m_vshader,
  ...
  hr = D3D::device->CreateInputLayout(desc, 3, vsbytecode->Data(),
                                      vsbytecode->Size(), &m_InputLayout);
  if (FAILED(hr))
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)m_blendstate,
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)m_raststate,
                          "rasterizer state of a CD3DFont object");

  D3D11_BUFFER_DESC vbdesc = CD3D11_BUFFER_DESC(
      MAX_NUM_VERTICES * sizeof(FONT2DVERTEX), D3D11_BIND_VERTEX_BUFFER,
      D3D11_USAGE_DYNAMIC, D3D11_CPU_ACCESS_WRITE);
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)m_pVB,
...
void InitUtils() {
  ...
  if (FAILED(hr))
    ...
    SetDebugObjectName((ID3D11DeviceChild *)point_copy_sampler,
  ...
  if (FAILED(hr))
    ...
    SetDebugObjectName((ID3D11DeviceChild *)linear_copy_sampler,
  ...
  // make sure to properly load the vertex data whenever the corresponding
  // functions get called the
  // first time
...
void SetPointCopySampler() { D3D::stateman->SetSampler(0, point_copy_sampler); }
...
void drawShadedTexQuad(ID3D11ShaderResourceView *texture,
                       const D3D11_RECT *rSource, int SourceWidth,
                       int SourceHeight, ID3D11PixelShader *PShader,
  ...
  if (stq_observer || tex_quad_data.u1 != u1 || tex_quad_data.v1 != v1 ||
      tex_quad_data.u2 != u2 || tex_quad_data.v2 != v2 ||
      tex_quad_data.S != S || tex_quad_data.G != G) {
    stq_offset =
        util_vbuf->AppendData(coords, sizeof(coords), sizeof(STQVertex));
    stq_observer = false;
...
void drawColorQuad(u32 Color, float z, float x1, float y1, float x2, float y2) {
  ...
  if (cq_observer || draw_quad_data.x1 != x1 || draw_quad_data.y1 != y1 ||
    ...
    cq_offset =
        util_vbuf->AppendData(coords, sizeof(coords), sizeof(ColVertex));
    cq_observer = false;
...
void drawClearQuad(u32 Color, float z) {
  ...
  if (clearq_observer || clear_quad_data.col != Color ||
      clear_quad_data.z != z) {
    clearq_offset =
        util_vbuf->AppendData(coords, sizeof(coords), sizeof(ClearVertex));
...
void DrawEFBPokeQuads(EFBAccessType type, const EfbPokeData *points,
  ...
  while (current_point_index < num_points) {
    ...
    void *buffer_ptr;
    int base_vertex_index = util_vbuf->BeginAppendData(
        &buffer_ptr, (int)required_bytes, sizeof(ColVertex));
