...
public:
  ...
  int AppendData(void* data, unsigned int size, unsigned int vertex_size)
  {
    ...
    if (offset + size >= max_size)
    {
      ...
      offset = 0;
      context->Map(buf.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
    ...
    {
      context->Map(buf.Get(), 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &map);
    }
  ...
  void EndAppendData() { context->Unmap(buf.Get(), 0); }
  void AddWrapObserver(bool* observer) { observers.push_back(observer); }
  inline ID3D11Buffer* GetBuffer() { return buf.Get(); }
private:
...
int CD3DFont::Init()
{
  ...
  hr = D3D::device->CreateShaderResourceView(buftex.Get(), nullptr, &m_pTexture);
  if (FAILED(hr))
...
ComPtr<ID3D11SamplerState> linear_copy_sampler;
ComPtr<ID3D11SamplerState> point_copy_sampler;
...
void InitUtils()
{
  ...
  if (FAILED(hr))
    ...
    SetDebugObjectName(point_copy_sampler.Get(), "point copy sampler state");
  ...
  if (FAILED(hr))
    ...
    SetDebugObjectName(linear_copy_sampler.Get(), "linear copy sampler state");
