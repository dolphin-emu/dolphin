...
int CD3DFont::Init()
{
	...
	{
		u32* pDst32 = reinterpret_cast<u32*>(static_cast<u8*>(texmap.pData) + y * texmap.RowPitch);
...
int CD3DFont::DrawTextScaled(float x, float y, float size, float spacing, u32 dwColor, const std::string& text)
{
	...
	float scalex = 1 / static_cast<float>(GetBackBufferWidth()) * 2.f;
	float scaley = 1 / static_cast<float>(GetBackBufferHeight()) * 2.f;
	float sizeratio = size / static_cast<float>(m_LineHeight);
	...
	for (char c : text)
	{
		...
		float w = static_cast<float>(tx2-tx1) * m_dwTexWidth * scalex * sizeratio;
		float h = static_cast<float>(ty1-ty2) * m_dwTexHeight * scaley * sizeratio;
...
void InitUtils()
{
	...
	HRESULT hr = device->CreateSamplerState(&samDesc, &point_copy_sampler);
...
void drawShadedTexQuad(ID3D11ShaderResourceView* texture,
						...
						u32 slice)
{
	float sw = 1.0f /static_cast<float>(SourceWidth);
	float sh = 1.0f /static_cast<float>(SourceHeight);
