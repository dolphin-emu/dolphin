...
ID3D11DepthStencilState* StateCache::Get(DepthState state)
{
  ...
  const D3D11_COMPARISON_FUNC d3dInvFuncs[9] = {
      D3D11_COMPARISON_NEVER,     D3D11_COMPARISON_NEVER,         D3D11_COMPARISON_GREATER,
  ...
  if (state.testenable)
  {
    ...
    depthdc.DepthFunc = state.reversed_depth ? d3dInvFuncs[state.func] : state.func;
  }
  else
  {
  ...
  m_depth.emplace(state.packed, res);
