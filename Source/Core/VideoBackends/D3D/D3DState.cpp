...
void StateManager::Apply()
{
	if (!m_blendStates.empty())
	{
		...
		{
			m_currentBlendState = const_cast<ID3D11BlendState*>(m_blendStates.top().GetPtr());
			context->OMSetBlendState(m_currentBlendState, nullptr, 0xFFFFFFFF);
		}
	}
	...
	if (!m_depthStates.empty())
	{
		...
		{
			m_currentDepthState = const_cast<ID3D11DepthStencilState*>(m_depthStates.top().GetPtr());
			context->OMSetDepthStencilState(m_currentDepthState, 0);
		}
	}
	...
	if (!m_rasterizerStates.empty())
	{
		...
		{
			m_currentRasterizerState = const_cast<ID3D11RasterizerState*>(m_rasterizerStates.top().GetPtr());
			context->RSSetState(m_currentRasterizerState);
		}
	}
	...
	auto textureMaskShift = LeastSignificantSetBit(static_cast<u32>(DirtyFlag_Texture0));
	auto samplerMaskShift = LeastSignificantSetBit(static_cast<u32>(DirtyFlag_Sampler0));

	auto dirtyTextures = (m_dirtyFlags & (DirtyFlag_Texture0 | DirtyFlag_Texture1 | DirtyFlag_Texture2 | DirtyFlag_Texture3
		| DirtyFlag_Texture4 | DirtyFlag_Texture5 | DirtyFlag_Texture6 | DirtyFlag_Texture7)) >> textureMaskShift;
	auto dirtySamplers = (m_dirtyFlags & (DirtyFlag_Sampler0 | DirtyFlag_Sampler1 | DirtyFlag_Sampler2 | DirtyFlag_Sampler3
		| DirtyFlag_Sampler4 | DirtyFlag_Sampler5 | DirtyFlag_Sampler6 | DirtyFlag_Sampler7)) >> samplerMaskShift;
	auto dirtyConstants = m_dirtyFlags & (DirtyFlag_PixelConstants | DirtyFlag_VertexConstants | DirtyFlag_GeometryConstants);
	auto dirtyShaders = m_dirtyFlags & (DirtyFlag_PixelShader | DirtyFlag_VertexShader | DirtyFlag_GeometryShader);
	auto dirtyBuffers = m_dirtyFlags & (DirtyFlag_VertexBuffer | DirtyFlag_IndexBuffer);
	...
	if (dirtyConstants)
	{
		...
		{
			context->PSSetConstantBuffers(0, m_pending.pixelConstants[1] ? 2 : 1, m_pending.pixelConstants);
	...
	if (dirtyBuffers || (m_dirtyFlags & DirtyFlag_InputAssembler))
	{
		...
		{
			context->IASetVertexBuffers(0, 1, &m_pending.vertexBuffer, &m_pending.vertexBufferStride, &m_pending.vertexBufferOffset);
	...
	while (dirtyTextures)
	{
		auto index = LeastSignificantSetBit(dirtyTextures);
		if (m_current.textures[index] != m_pending.textures[index])
		{
			context->PSSetShaderResources(index, 1, &m_pending.textures[index]);
	...
	while (dirtySamplers)
	{
		auto index = LeastSignificantSetBit(dirtySamplers);
		if (m_current.samplers[index] != m_pending.samplers[index])
		{
			context->PSSetSamplers(index, 1, &m_pending.samplers[index]);
...
void StateManager::SetTextureByMask(u32 textureSlotMask, ID3D11ShaderResourceView* srv)
{
	...
	{
		auto index = LeastSignificantSetBit(textureSlotMask);
...
ID3D11SamplerState* StateCache::Get(SamplerState state)
{
	...
	auto mip = d3dMipFilters[state.min_filter & 3];

	if (state.max_anisotropy > 1)
	{
		sampdc.Filter = D3D11_FILTER_ANISOTROPIC;
		sampdc.MaxAnisotropy = static_cast<u32>(state.max_anisotropy);
	}
	...
	auto hr = D3D::device->CreateSamplerState(&sampdc, &res);
	if (FAILED(hr))
		PanicAlert("Fail %s %d\n", __FILE__, __LINE__);

	D3D::SetDebugObjectName(static_cast<ID3D11DeviceChild*>(res), "sampler state used to emulate the GX pipeline");
...
ID3D11BlendState* StateCache::Get(BlendState state)
{
	...
	blenddc.RenderTarget[0].RenderTargetWriteMask = static_cast<u32>(state.write_mask);
	...
	auto hr = D3D::device->CreateBlendState(&blenddc, &res);
	if (FAILED(hr))
		PanicAlert("Failed to create blend state at %s %d\n", __FILE__, __LINE__);

	D3D::SetDebugObjectName(static_cast<ID3D11DeviceChild*>(res), "blend state used to emulate the GX pipeline");
...
ID3D11RasterizerState* StateCache::Get(RasterizerState state)
{
	...
	auto hr = D3D::device->CreateRasterizerState(&rastdc, &res);
	if (FAILED(hr))
		PanicAlert("Failed to create rasterizer state at %s %d\n", __FILE__, __LINE__);

	D3D::SetDebugObjectName(static_cast<ID3D11DeviceChild*>(res), "rasterizer state used to emulate the GX pipeline");
...
ID3D11DepthStencilState* StateCache::Get(ZMode state)
{
	...
	auto hr = D3D::device->CreateDepthStencilState(&depthdc, &res);
	if (SUCCEEDED(hr))
		D3D::SetDebugObjectName(static_cast<ID3D11DeviceChild*>(res), "depth-stencil state used to emulate the GX pipeline");
	else
