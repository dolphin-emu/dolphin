...
StateManager::StateManager()
    : m_currentBlendState(nullptr), m_currentDepthState(nullptr),
      m_currentRasterizerState(nullptr), m_dirtyFlags(~0u), m_pending(),
      m_current() {}
...
void StateManager::Apply() {
  ...
  if (!m_depthStates.empty()) {
    if (m_currentDepthState != m_depthStates.top().GetPtr()) {
      m_currentDepthState =
          (ID3D11DepthStencilState *)m_depthStates.top().GetPtr();
  ...
  if (!m_rasterizerStates.empty()) {
    if (m_currentRasterizerState != m_rasterizerStates.top().GetPtr()) {
      m_currentRasterizerState =
          (ID3D11RasterizerState *)m_rasterizerStates.top().GetPtr();
  ...
  u32 dirtyTextures =
      (m_dirtyFlags &
       (DirtyFlag_Texture0 | DirtyFlag_Texture1 | DirtyFlag_Texture2 |
        DirtyFlag_Texture3 | DirtyFlag_Texture4 | DirtyFlag_Texture5 |
        DirtyFlag_Texture6 | DirtyFlag_Texture7)) >>
      textureMaskShift;
  u32 dirtySamplers =
      (m_dirtyFlags &
       (DirtyFlag_Sampler0 | DirtyFlag_Sampler1 | DirtyFlag_Sampler2 |
        DirtyFlag_Sampler3 | DirtyFlag_Sampler4 | DirtyFlag_Sampler5 |
        DirtyFlag_Sampler6 | DirtyFlag_Sampler7)) >>
      samplerMaskShift;
  u32 dirtyConstants =
      m_dirtyFlags & (DirtyFlag_PixelConstants | DirtyFlag_VertexConstants |
                      DirtyFlag_GeometryConstants);
  u32 dirtyShaders =
      m_dirtyFlags & (DirtyFlag_PixelShader | DirtyFlag_VertexShader |
                      DirtyFlag_GeometryShader);
  u32 dirtyBuffers =
      m_dirtyFlags & (DirtyFlag_VertexBuffer | DirtyFlag_IndexBuffer);
  ...
  if (dirtyBuffers || (m_dirtyFlags & DirtyFlag_InputAssembler)) {
    if (m_current.vertexBuffer != m_pending.vertexBuffer ||
      ...
      D3D::context->IASetVertexBuffers(0, 1, &m_pending.vertexBuffer,
                                       &m_pending.vertexBufferStride,
    ...
    if (m_current.indexBuffer != m_pending.indexBuffer) {
      D3D::context->IASetIndexBuffer(m_pending.indexBuffer,
...
void StateManager::SetTextureByMask(u32 textureSlotMask,
...
ID3D11SamplerState *StateCache::Get(SamplerState state) {
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)res,
                          "sampler state used to emulate the GX pipeline");
...
ID3D11BlendState *StateCache::Get(BlendState state) {
  ...
  D3D::SetDebugObjectName((ID3D11DeviceChild *)res,
                          "blend state used to emulate the GX pipeline");
...
ID3D11RasterizerState *StateCache::Get(RasterizerState state) {
  ...
  D3D11_RASTERIZER_DESC rastdc =
      CD3D11_RASTERIZER_DESC(D3D11_FILL_SOLID, state.cull_mode, false, 0, 0.f,
...
ID3D11DepthStencilState *StateCache::Get(ZMode state) {
  ...
  const D3D11_COMPARISON_FUNC d3dCmpFuncs[8] = {
      D3D11_COMPARISON_NEVER,      D3D11_COMPARISON_GREATER,
      D3D11_COMPARISON_EQUAL,      D3D11_COMPARISON_GREATER_EQUAL,
      D3D11_COMPARISON_LESS,       D3D11_COMPARISON_NOT_EQUAL,
      D3D11_COMPARISON_LESS_EQUAL, D3D11_COMPARISON_ALWAYS};

  if (state.testenable) {
    depthdc.DepthEnable = TRUE;
    depthdc.DepthWriteMask = state.updateenable ? D3D11_DEPTH_WRITE_MASK_ALL
