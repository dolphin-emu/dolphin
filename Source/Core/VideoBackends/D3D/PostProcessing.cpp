...
bool PostProcessingShader::CreatePasses()
{
	...
	for (const auto& pass_config : m_config->GetPasses())
	{
		...
		{
			InputBinding input;
			if (!CreatePassInput(pass_config, input_config, &input))
				return false;
...
bool PostProcessingShader::CreatePassInput(const PostProcessingShaderConfiguration::RenderPass& pass_config,
                                           const PostProcessingShaderConfiguration::RenderPass::Input& input_config,
                                           InputBinding* input)
{
	...
	for (size_t pass_index = 0; pass_index < m_passes.size(); pass_index++)
	{
		...
		for (size_t i = 0; i < pass.inputs.size(); i++)
		{
			...
			case POST_PROCESSING_INPUT_TYPE_DEPTH_BUFFER:
				input_srv = src_depth_texture ? src_depth_texture->GetSRV() : nullptr;
				input_sizes[i] = src_size;
				break;
