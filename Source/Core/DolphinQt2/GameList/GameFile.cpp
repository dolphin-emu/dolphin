...
{
	QList<DiscIO::IVolume::ELanguage> languages;
	for (DiscIO::IVolume::ELanguage language : m_long_names.keys())
	{
...
bool GameFile::TryLoadVolume()
{
	...
	m_maker = QString::fromStdString(DiscIO::GetCompanyFromID(volume->GetMakerID()));
...
QString GameFile::GetPlatform() const
{
	switch(m_platform)
	{
		...
		case DiscIO::IVolume::WII_WAD:       return QObject::tr("Wii");
