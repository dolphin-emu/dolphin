...
void PathDialog::Browse() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("Select a Directory"), QDir::currentPath());
  if (!dir.isEmpty()) {
    Settings settings;
...
void PathDialog::BrowseDVDRoot() {
  QString dir = QFileDialog::getExistingDirectory(this, tr("Select DVD Root"),
                                                  QDir::currentPath());
  if (!dir.isEmpty()) {
...
void PathDialog::BrowseApploader() {
  QString file = QFileDialog::getOpenFileName(this, tr("Select an Apploader"),
                                              QDir::currentPath(),
                                              tr("Apploaders (*.img)"));
  if (!file.isEmpty()) {
...
void PathDialog::BrowseWiiNAND() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("Select Wii NAND Root"), QDir::currentPath());
  if (!dir.isEmpty()) {
...
QGridLayout *PathDialog::MakePathsLayout() {
  ...
  connect(game_open, &QPushButton::clicked, this,
          &PathDialog::BrowseDefaultGame);
