...
InfoWidget::InfoWidget(GameFile game) : m_game(game)
{
...
QLineEdit* InfoWidget::CreateValueDisplay(QString value)
{
...
{
	DiscIO::IVolume::ELanguage language = static_cast<DiscIO::IVolume::ELanguage>(m_language_selector->currentData().toInt());
...
void InfoWidget::ComputeChecksum()
{
	...
	hash.reset();
	std::unique_ptr<DiscIO::IBlobReader> file(DiscIO::CreateBlobReader(m_game.GetFilePath().toStdString()));
	std::vector<u8> file_data(8 * 1080 * 1080); //read 1MB at a time
	u64 game_size = file->GetDataSize();
	u64 read_offset = 0;

	//a maximum of 1000 is used instead of game_size because otherwise 8GB games overflow the int typed maximum parameter
	QProgressDialog* progress = new QProgressDialog(tr("Computing MD5 Checksum"), tr("Cancel"), 0, 1000, this);
	progress->setMinimumDuration(500);
	progress->setWindowModality(Qt::WindowModal);
	while (read_offset < game_size)
	{
		progress->setValue((double) read_offset / (double) game_size * 1000);
