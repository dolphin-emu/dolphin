...
void MenuBar::AddEmulationMenu() {
  ...
  m_fullscreen_action =
      emu_menu->addAction(tr("Fullscreen"), this, SIGNAL(Fullscreen()));
  m_frame_advance_action =
      emu_menu->addAction(tr("Frame Advance"), this, SIGNAL(FrameAdvance()));
  m_screenshot_action =
      emu_menu->addAction(tr("Take Screenshot"), this, SIGNAL(Screenshot()));
...
void MenuBar::AddStateLoadMenu(QMenu *emu_menu) {
  ...
  m_state_load_menu->addAction(tr("Load State from File"), this,
                               SIGNAL(StateLoad()));
  m_state_load_menu->addAction(tr("Load State from Selected Slot"), this,
                               SIGNAL(StateLoadSlot()));
  m_state_load_slots_menu =
      m_state_load_menu->addMenu(tr("Load State from Slot"));
  m_state_load_menu->addAction(tr("Undo Load State"), this,
                               SIGNAL(StateLoadUndo()));
...
void MenuBar::AddStateSaveMenu(QMenu *emu_menu) {
  ...
  m_state_save_menu->addAction(tr("Save State to File"), this,
                               SIGNAL(StateSave()));
  m_state_save_menu->addAction(tr("Save State to Selected Slot"), this,
                               SIGNAL(StateSaveSlot()));
  m_state_save_menu->addAction(tr("Save State to Oldest Slot"), this,
                               SIGNAL(StateSaveOldest()));
  m_state_save_slots_menu =
      m_state_save_menu->addMenu(tr("Save State to Slot"));
  m_state_save_menu->addAction(tr("Undo Save State"), this,
                               SIGNAL(StateSaveUndo()));
...
void MenuBar::AddHelpMenu() {
  ...
  connect(documentation, &QAction::triggered, this, [=]() {
    QDesktopServices::openUrl(
        QUrl(QStringLiteral("https://dolphin-emu.org/docs/guides")));
  });
...
void MenuBar::AddTableColumnsMenu(QMenu *view_menu) {
  ...
  QStringList col_names{tr("Platform"), tr("ID"),          tr("Banner"),
                        tr("Title"),    tr("Description"), tr("Maker"),
                        tr("Size"),     tr("Country"),     tr("Quality")};
  for (int i = 0; i < col_names.count(); i++) {
    QAction *action =
        column_group->addAction(cols_menu->addAction(col_names[i]));
    action->setCheckable(true);
  }
}
