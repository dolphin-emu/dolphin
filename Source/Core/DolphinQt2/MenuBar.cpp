...
void MenuBar::AddStateSaveMenu(QMenu* emu_menu)
{
	...
	for (int i = 1; i <= 10; i++)
	{
		action = m_state_save_slots_menu->addAction(QStringLiteral(""));
...
void MenuBar::AddStateSlotMenu(QMenu* emu_menu)
{
	...
	for (int i = 1; i <= 10; i++)
	{
		action = m_state_slot_menu->addAction(QStringLiteral(""));
...
void MenuBar::UpdateStateSlotMenu()
{
	...
	for (int i = 0; i < actions_slot.length(); i++)
	{
		...
		actions_save.at(i)->setText(QStringLiteral("Save to") + action_string);
		actions_slot.at(i)->setText(QStringLiteral("Select") + action_string);
	}
}
