...
public:
  ...
  public:
    class Control {
    protected:
      Control(ControllerInterface::ControlReference *const _ref,
    ...
    public:
      Setting(const std::string &_name, const ControlState def_value,
          ...
          : name(_name), value(def_value), default_value(def_value), low(_low),
            high(_high), is_virtual(false), is_iterate(false) {}
    ...
    ControlGroup(const std::string &_name,
    ...
    virtual void LoadConfig(IniFile::Section *sec,
                            const std::string &defdev = "",
                            const std::string &base = "");
    virtual void SaveConfig(IniFile::Section *sec,
                            const std::string &defdev = "",
                            const std::string &base = "");
  ...
  public:
    ...
    AnalogStick(const char *const _name, const char *const _ui_name,
                ControlState default_radius);

    void GetState(ControlState *const x, ControlState *const y) {
      ControlState yy =
          controls[0]->control_ref->State() - controls[1]->control_ref->State();
      ControlState xx =
          controls[3]->control_ref->State() - controls[2]->control_ref->State();
  ...
  public:
    ...
    template <typename C> void GetState(C *const buttons, const C *bitmasks) {
  ...
  public:
    ...
    void GetState(u16 *const digital, const u16 *bitmasks,
                  ControlState *analog) {
  ...
  public:
    ...
    void GetState(ControlState *analog) {
      ...
      for (unsigned int i = 0; i < trig_count; ++i, ++analog)
        *analog = std::max(controls[i]->control_ref->State() - deadzone, 0.0) /
                  (1 - deadzone);
    }
  };
  ...
  public:
    ...
    void GetState(ControlState *axis) {
      ...
      for (unsigned int i = 0; i < 6; i += 2) {
        ControlState tmpf = 0;
        const ControlState state = controls[i + 1]->control_ref->State() -
                                   controls[i]->control_ref->State();
        if (fabs(state) > deadzone)
  ...
  public:
    ...
    void GetState(ControlState *const x, ControlState *const y,
      ...
      ControlState square_full = std::min(ang_sin ? 1 / fabs(ang_sin) : 2,
                                          ang_cos ? 1 / fabs(ang_cos) : 2);
  ...
  public:
    ...
    void GetState(ControlState *const x, ControlState *const y,
                  ControlState *const z, const bool adjusted = false) {
      const ControlState zz =
          controls[4]->control_ref->State() - controls[5]->control_ref->State();
      ...
      *z = m_z;
      ...
      if (controls[6]->control_ref->State() > 0.5) {
        ...
        *y = 0;
        ...
        ControlState yy = controls[0]->control_ref->State() -
                          controls[1]->control_ref->State();
        ControlState xx = controls[3]->control_ref->State() -
                          controls[2]->control_ref->State();
  ...
  public:
    Extension(const std::string &_name)
        : ControlGroup(_name, GROUP_TYPE_EXTENSION), switch_extension(0),
          active_extension(0) {}
