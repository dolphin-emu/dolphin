...
void StartScanThread()
{
  if (s_adapter_detect_thread_running.IsSet())
    return;
  if (/*!s_libusb_context->IsValid()*/ !s_libusb_context.IsValid())
    return;
...
void Shutdown()
{
  StopScanThread();
#if defined(LIBUSB_API_VERSION) && LIBUSB_API_VERSION >= 0x01000102
  if (/*s_libusb_context->IsValid()*/ s_libusb_context.IsValid() && s_libusb_hotplug_enabled)
    libusb_hotplug_deregister_callback(/**s_libusb_context*/s_libusb_context, s_hotplug_handle);
#endif
