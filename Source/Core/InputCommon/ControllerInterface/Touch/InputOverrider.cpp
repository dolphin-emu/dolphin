...
using ControlsMap = std::map<std::pair<std::string, std::string>, ControlID>;
                                                              ...
                                                              size_t i)
{
  ...
  return [&](const std::string& group_name, const std::string& control_name,
...
void UnregisterGameCubeInputOverrider(int controller_index)
{
  ...
  for (size_t i = ControlID::FIRST_GC_CONTROL; i <= ControlID::LAST_GC_CONTROL; ++i)
    s_state_arrays[i][controller_index].overriding = false;
}
