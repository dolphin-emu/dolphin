  ...
  else
  {
    ...
    if (gate_enabled && Host_RendererHasFocus() && ::Core::GetState() == ::Core::State::Running)
    {
      const auto locked = m_mouse_stick_gate.LockMouseInGate(
          static_cast<int>(0), static_cast<int>(win_width), static_cast<int>(0),
          static_cast<int>(win_height), static_cast<int>(win_x), static_cast<int>(win_y),
      ...
      if (locked)
      {
        ...
        const auto relative = m_mouse_stick_gate.CalculateRelativeCursorPosition(
            static_cast<int>(win_x), static_cast<int>(win_y), scale_x, scale_y);
        m_state.cursor.x = relative.x;
        m_state.cursor.y = relative.y;
        return;
      }
    }
  }
