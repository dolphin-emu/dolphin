...
private:
  ...
  static void TrackerCallback(void* userdata, const OSVR_TimeValue* timestamp,
...
private:
  ...
  public:
    std::string GetName() const;
    Axis(u8 index, const double& axis, bool negative)
        : m_index(index), m_axis(axis), m_negative(negative)
    {
    }
    ControlState GetState() const;
  ...
  static void ButtonCallback(void* userdata, const struct OSVR_TimeValue* timestamp,
                             const struct OSVR_ButtonReport* report);
  static void AnalogCallback(void* userdata, const struct OSVR_TimeValue* timestamp,
