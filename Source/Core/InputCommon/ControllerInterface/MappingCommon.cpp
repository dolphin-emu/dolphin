...
DirectionalInputMappingsDetector::DirectionalInputMappingsDetector(
    const Core::DeviceContainer& container, const std::vector<std::string>& device_strings)
{
  m_input_detector.Start(container, device_strings);
}

bool DirectionalInputMappingsDetector::Update()
{
...
float DirectionalInputMappingsDetector::GetCurrentAngle() const
{
...
bool DirectionalInputMappingsDetector::IsComplete() const
{
  return m_input_detector.GetResults().size() >= REQUIRED_INPUT_COUNT ||
         m_input_detector.IsComplete();
}

bool DirectionalInputMappingsDetector::IsCalibrationNeeded() const
{
...
bool DirectionalInputMappingsDetector::ApplyResults(ControllerEmu::EmulatedController* controller,
