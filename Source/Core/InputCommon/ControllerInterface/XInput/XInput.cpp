...
static const char *const named_axes[] = {"Left X", "Left Y", "Right X",
                                         "Right Y"};
...
void Init(std::vector<Core::Device *> &devices) {
  if (!hXInput) {
    // Try for the most recent version we were compiled against (will only work
    // if running on Win8+)
    ...
    PXInputGetCapabilities = (XInputGetCapabilities_t)::GetProcAddress(
        hXInput, "XInputGetCapabilities");
    PXInputSetState =
        (XInputSetState_t)::GetProcAddress(hXInput, "XInputSetState");

    // Ordinal 100 is the same as XInputGetState, except it doesn't dummy out
    // the guide
    ...
    if (PXInputGetState)
      ...
      PXInputGetState =
          (XInputGetState_t)::GetProcAddress(hXInput, "XInputGetState");
...
Device::Device(const XINPUT_CAPABILITIES &caps, u8 index)
  ...
  for (int i = 0; i != sizeof(named_triggers) / sizeof(*named_triggers); ++i) {
    // BYTE val = (&caps.Gamepad.bLeftTrigger)[i];  // should be max value /
    // MSDN lies
  ...
  for (int i = 0; i != sizeof(named_axes) / sizeof(*named_axes); ++i) {
    // SHORT val = (&caps.Gamepad.sThumbLX)[i];  // xinput doesn't give the
    // range / MSDN is a liar
  ...
  for (int i = 0; i != sizeof(named_motors) / sizeof(*named_motors); ++i) {
    // WORD val = (&caps.Vibration.wLeftMotorSpeed)[i]; // should be max value /
    // nope, more lies
...
int Device::GetId() const { return m_index; }

std::string Device::GetSource() const { return "XInput"; }

// Update I/O

void Device::UpdateInput() { PXInputGetState(m_index, &m_state_in); }

void Device::UpdateMotors() {
  // this if statement is to make rumble work better when multiple
  // ControllerInterfaces are using
  // the device
...
std::string Device::Trigger::GetName() const { return named_triggers[m_index]; }

std::string Device::Motor::GetName() const { return named_motors[m_index]; }
