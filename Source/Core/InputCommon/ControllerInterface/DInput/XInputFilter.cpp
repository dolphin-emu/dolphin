...
// This function is contained in a separate file because WbemIdl.h pulls in
// files which break on
// /Zc:strictStrings, so this compilation unit is compiled without
// /Zc:strictStrings.
...
namespace DInput {
//-----------------------------------------------------------------------------
// Modified some MSDN code to get all the XInput device GUID.Data1 values in a
// vector,
// faster than checking all the devices for each DirectInput device, like MSDN
// says to do
//-----------------------------------------------------------------------------
void GetXInputGUIDS(std::vector<DWORD> *guids) {
  ...
  // Connect to WMI
  hr = pIWbemLocator->ConnectServer(bstrNamespace, nullptr, nullptr, 0L, 0L,
