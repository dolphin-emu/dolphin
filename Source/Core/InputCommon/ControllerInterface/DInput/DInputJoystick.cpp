...
void InitJoystick(IDirectInput8 *const idi8,
  ...
  idi8->EnumDevices(DI8DEVCLASS_GAMECTRL, DIEnumDevicesCallback,
                    (LPVOID)&joysticks, DIEDFL_ATTACHEDONLY);
  ...
  for (DIDEVICEINSTANCE &joystick : joysticks) {
    // skip XInput Devices
    if (std::find(xinput_guids.begin(), xinput_guids.end(),
                  joystick.guidProduct.Data1) != xinput_guids.end()) {
      continue;
    }

    LPDIRECTINPUTDEVICE8 js_device;
    if (SUCCEEDED(
            idi8->CreateDevice(joystick.guidInstance, &js_device, nullptr))) {
      if (SUCCEEDED(js_device->SetDataFormat(&c_dfDIJoystick))) {
        if (FAILED(js_device->SetCooperativeLevel(GetAncestor(hwnd, GA_ROOT),
          ...
          if (FAILED(js_device->SetCooperativeLevel(
                  nullptr, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE))) {
        ...
        Joystick *js = new Joystick(/*&*i, */ js_device,
                                    name_counts[joystick.tszInstanceName]++);
...
Joystick::Joystick(
    /*const LPCDIDEVICEINSTANCE lpddi, */ const LPDIRECTINPUTDEVICE8 device,
...
//, m_name(TStringToString(lpddi->tszInstanceName))
{
  ...
  // screw EnumObjects, just go through all the axis offsets and try to
  // GetProperty
  // this should be more foolproof, less code, and probably faster
  for (unsigned int offset = 0; offset < DIJOFS_BUTTON(0) / sizeof(LONG);
       ++offset) {
    range.diph.dwObj = offset * sizeof(LONG);
    // try to set some nice power of 2 values (128) to match the GameCube
    // controls
    range.lMin = -(1 << 7);
    range.lMax = (1 << 7);
  ...
  if (SUCCEEDED(m_device->EnumObjects(DIEnumDeviceObjectsCallback,
                                      (LPVOID)&objects, DIDFT_AXIS))) {
...
std::string Joystick::GetName() const { return GetDeviceName(m_device); }

int Joystick::GetId() const { return m_index; }

std::string Joystick::GetSource() const { return DINPUT_SOURCE_NAME; }
...
void Joystick::UpdateInput() {
  ...
  if (m_buffered) {
    ...
    if (SUCCEEDED(hr)) {
      for (LPDIDEVICEOBJECTDATA evt = evtbuf; evt != (evtbuf + numevents);
           ++evt) {
