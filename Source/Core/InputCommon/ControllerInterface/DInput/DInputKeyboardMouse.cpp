...
void InitKeyboardMouse(IDirectInput8 *const idi8,
                       std::vector<Core::Device *> &devices, HWND _hwnd) {
  m_hwnd = _hwnd;
  ...
  // inputs/outputs from
  // other devices
  // so there can be a separated Keyboard and mouse, as well as combined
  // KeyboardMouse
  ...
  if (SUCCEEDED(idi8->CreateDevice(GUID_SysKeyboard, &kb_device, nullptr))) {
    if (SUCCEEDED(kb_device->SetDataFormat(&c_dfDIKeyboard))) {
      if (SUCCEEDED(kb_device->SetCooperativeLevel(
        ...
        if (SUCCEEDED(idi8->CreateDevice(GUID_SysMouse, &mo_device, nullptr))) {
          if (SUCCEEDED(mo_device->SetDataFormat(&c_dfDIMouse2))) {
            if (SUCCEEDED(mo_device->SetCooperativeLevel(
                    nullptr, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE))) {
...
void GetMousePos(ControlState *const x, ControlState *const y) {
  ...
  if (processId == GetCurrentProcessId()) {
    ...
    // Get the size of the current window. (In my case Rect.top and Rect.left
    // was zero.)
    RECT rect;
...
void KeyboardMouse::UpdateInput() {
  ...
  HRESULT kb_hr = m_kb_device->GetDeviceState(sizeof(m_state_in.keyboard),
                                              &m_state_in.keyboard);
  ...
  if (SUCCEEDED(kb_hr) && SUCCEEDED(mo_hr)) {
    ...
    memcpy(m_state_in.mouse.rgbButtons, tmp_mouse.rgbButtons,
           sizeof(m_state_in.mouse.rgbButtons));
...
// get/set state
ControlState KeyboardMouse::Key::GetState() const { return (m_key != 0); }

ControlState KeyboardMouse::Button::GetState() const { return (m_button != 0); }
