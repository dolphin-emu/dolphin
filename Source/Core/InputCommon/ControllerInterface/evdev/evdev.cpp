...
void Init(std::vector<Core::Device *> &controllerDevices) {
  ...
  // We use Udev to find any devices. In the future this will allow for
  // hotplugging.
  // But for now it is essentially iterating over /dev/input/event0 to event31.
  // However if the
  // naming scheme is ever updated in the future, this *should* be forwards
  // compatable.
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    if (devnode && access(devnode, W_OK) == 0) {
      // Unfortunately udev gives us no way to filter out the non event device
      // interfaces.
      ...
      if (input->IsInteresting()) {
        ...
        // Either it wasn't a evdev device, or it didn't have at least 8 buttons
        // or two axis.
        delete input;
      }
    }
...
evdevDevice::evdevDevice(const std::string &devnode, int id)
    : m_devfile(devnode), m_id(id) {
  // The device file will be read on one of the main threads, so we open in
  // non-blocking mode.
  m_fd = open(devnode.c_str(), O_RDWR | O_NONBLOCK);
  int ret = libevdev_new_from_fd(m_fd, &m_dev);

  if (ret != 0) {
    // This useally fails because the device node isn't an evdev device, such as
    // /dev/input/js0
    m_initialized = false;
    close(m_fd);
    return;
...
void evdevDevice::UpdateInput() {
  ...
  // libevdev will keep track of the actual controller state internally which
  // can be queried
...
std::string evdevDevice::Button::GetName() const {
  ...
  // But controllers use codes above 0x100, and the standard label often doesn't
  // match.
...
void evdevDevice::ForceFeedback::SetState(ControlState state) {
  ...
  if (state > 0) // Upload and start an effect.
  {
    ...
    // This is the the interface that XInput uses, with 2 motors of differing
    // sizes/frequencies that
    // are controlled seperatally
    if (m_type == FF_RUMBLE) {
      effect.type = FF_RUMBLE;
      // max ranges tuned to 'feel' similar in magnitude to triangle/sine on
      // xbox360 controller
