...
static std::map<std::string, int> s_name_counts;
...
static void HotplugThreadFunc()
{
  ...
  while (s_hotplug_thread_running.IsSet())
  {
    ...
    if (strcmp(action, "add") == 0 && access(devnode, W_OK) == 0 && !name.empty())
    {
      INFO_LOG(SERIALINTERFACE, "Found new device: %s (%s)", devnode, name.c_str());
      // Try to find a device that was unplugged then replugged
      // We check for a device which has the same source, same name, and does not work anymore
      // If found, update the fd on the libevdev device, so we can get input from the device again.
      auto matched_device = g_controller_interface.FindDevice([&name](const auto* device) {
        if (device->GetSource() != "evdev" || device->GetName() != name)
          return false;
        auto* d = static_cast<const evdevDevice*>(device);
        return !d->IsStillValid();
      });

      if (matched_device != nullptr)
      {
        auto device = std::static_pointer_cast<evdevDevice>(matched_device);
...
void Init()
{
  ...
  udev_list_entry_foreach(dev_list_entry, devices)
  {
    ...
    if (devnode && access(devnode, W_OK) == 0)
    {
      ...
      std::string name = GetName(devnode);
      auto input = std::make_shared<evdevDevice>(devnode, s_name_counts[name]++);
...
bool evdevDevice::IsStillValid() const
{
