...
static CFRunLoopRef s_hotplug_runloop;
...
static std::string GetDeviceRefName(IOHIDDeviceRef inIOHIDDeviceRef)
{
  ...
  return (pName != nullptr) ? StripSpaces([pName UTF8String]) : "Unknown device";
}

static void DeviceRemoval_callback(void* inContext, IOReturn inResult, void* inSender,
                                   IOHIDDeviceRef inIOHIDDeviceRef)
{
  g_controller_interface.RemoveDevice([&inIOHIDDeviceRef](const auto* device) {
    ...
    if (joystick && joystick->m_device == inIOHIDDeviceRef)
      return true;

    const Keyboard* keyboard = dynamic_cast<const Keyboard*>(device);
    if (keyboard && keyboard->m_device == inIOHIDDeviceRef)
      return true;
...
void Init(void* window)
{
  ...
  IOHIDManagerRegisterDeviceRemovalCallback(HIDManager, DeviceRemoval_callback, nullptr);
  ...
  s_hotplug_thread = std::thread([] {
    ...
    s_hotplug_runloop = CFRunLoopGetCurrent();
    IOHIDManagerScheduleWithRunLoop(HIDManager, s_hotplug_runloop, OurRunLoop);
    CFRunLoopRunInMode(OurRunLoop, FOREVER, FALSE);
    IOHIDManagerUnscheduleFromRunLoop(HIDManager, s_hotplug_runloop, OurRunLoop);
...
void DeInit()
{
  CFRunLoopStop(s_hotplug_runloop);
