...
//
void ControllerInterface::RegisterDeviceCallback(std::function<void(void)> callback)
{
	m_device_callbacks.push_back(callback);
}
...
{
	for (std::function<void(void)> callback : m_device_callbacks)
		callback();
}
