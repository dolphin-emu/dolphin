...
void ControllerEmu::ControlGroup::LoadConfig(IniFile::Section *sec,
                                             const std::string &defdev,
...
void ControllerEmu::ControlGroup::SaveConfig(IniFile::Section *sec,
                                             const std::string &defdev,
                                             const std::string &base) {
  std::string group(base + name + "/");
  ...
  if (type == GROUP_TYPE_EXTENSION) {
    Extension *const ext = (Extension *)this;
    sec->Set(base + name, ext->attachments[ext->switch_extension]->GetName(),
             "None");
...
void ControllerEmu::ControlGroup::SetControlExpression(
...
ControllerEmu::AnalogStick::AnalogStick(const char *const _name,
  ...
  settings.emplace_back(
      std::make_unique<Setting>(_trans("Radius"), default_radius, 0, 100));
  settings.emplace_back(
      std::make_unique<Setting>(_trans("Dead Zone"), 0, 0, 50));
}

ControllerEmu::Buttons::Buttons(const std::string &_name)
    : ControlGroup(_name, GROUP_TYPE_BUTTONS) {
...
ControllerEmu::Slider::Slider(const std::string &_name)
    : ControlGroup(_name, GROUP_TYPE_SLIDER) {
...
ControllerEmu::Force::Force(const std::string &_name)
    : ControlGroup(_name, GROUP_TYPE_FORCE) {
...
ControllerEmu::Tilt::Tilt(const std::string &_name)
  ...
  settings.emplace_back(
      std::make_unique<Setting>(_trans("Angle"), 0.9, 0, 180));
}
