...
{
  controls.emplace_back(std::make_unique<InputReference>(_trans("Up")));
  controls.emplace_back(std::make_unique<InputReference>(_trans("Down")));
...
void Force::GetState(ControlState* axis)
{
  ...
  {
    ControlState tmpf = 0;
    const ControlState state = controls[i + 1]->State() - controls[i]->State();
