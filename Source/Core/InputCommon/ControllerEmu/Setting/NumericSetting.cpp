...
void NumericSetting<T>::SetExpressionFromValue()
{
  ...
  // We must use "classic" locale. ExpressionParser expects dot decimal separator.
  m_value.m_input.SetExpression(
      fmt::format(std::locale::classic(), "{:+g}", ControlState(GetValue())));
}
