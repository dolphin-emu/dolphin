...
void HotkeyScheduler::Run()
{
  ...
  while (!m_stop_requested.IsSet())
  {
    ...
    if (GetState(system) != Core::State::Stopping)
    {
      ...
      {
        const bool enable_dumping = !Get(Config::GFX_DUMP_TEXTURES);
        SetCurrent(Config::GFX_DUMP_TEXTURES, enable_dumping);
        OSD::AddMessage(
      ...
      if (IsHotkey(HK_TOGGLE_THROTTLE, true) && !Get(Config::MAIN_AUDIO_MUTED) &&
          Get(Config::MAIN_AUDIO_MUTE_ON_DISABLED_SPEED_LIMIT))
      {
        SetCurrent(Config::MAIN_AUDIO_MUTED, true);
        AudioCommon::UpdateSoundStream(system);
      }
      else if (!IsHotkey(HK_TOGGLE_THROTTLE, true) && Get(Config::MAIN_AUDIO_MUTED) &&
               GetActiveLayerForConfig(Config::MAIN_AUDIO_MUTED) ==
                   Config::LayerType::CurrentRun)
      {
        DeleteKey(Config::LayerType::CurrentRun, Config::MAIN_AUDIO_MUTED);
        AudioCommon::UpdateSoundStream(system);
      }
