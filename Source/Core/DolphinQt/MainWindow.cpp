...
void MainWindow::Play(const std::optional<std::string>& savestate_path)
{
  ...
  if (GetState(m_system) == Core::State::Paused)
  {
    SetState(m_system, Core::State::Running);
  }
  else
  {
...
{
  SetState(m_system, Core::State::Paused);
}
...
bool MainWindow::RequestStop()
{
  ...
  if (Get(Config::MAIN_CONFIRM_ON_STOP))
  {
    ...
    const Core::State state = GetState(m_system);
...
void MainWindow::UpdateScreenSaverInhibition()
{
  const bool inhibit = Get(Config::MAIN_DISABLE_SCREENSAVER) &&
                       (GetState(m_system) == Core::State::Running);
...
{
  auto& movie = m_system.GetMovie();
  if (GetState(m_system) == Core::State::Starting ||
      GetState(m_system) == Core::State::Stopping || movie.IsRecordingInput() ||
...
void MainWindow::ShowTASInput()
{
  ...
  for (int i = 0; i < num_wii_controllers; i++)
  {
    if (Get(Config::GetInfoForWiimoteSource(i)) == WiimoteSource::Emulated &&
        (!IsRunning(m_system) || m_system.IsWii()))
    {
