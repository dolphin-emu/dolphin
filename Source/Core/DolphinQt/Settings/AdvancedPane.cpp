...
void AdvancedPane::CreateLayout()
{
  ...
  auto* vi_clock_override = new QGroupBox(tr("VI Clock Override"));
  ...
  m_vi_clock_override_checkbox = new QCheckBox(tr("Enable VI Overclocking"));
  ...
  m_vi_clock_override_slider->setRange(0, 150);
  ...
  auto* vi_clock_override_description = new QLabel(
      tr("Adjusts the emulated VI clock rate.\n\n"
...
void AdvancedPane::ConnectLayout()
{
  ...
  connect(m_vi_clock_override_slider, &QSlider::valueChanged, [this](int oc_factor) {
    // Vaguely exponential scaling?
    const float factor = std::exp2f((m_vi_clock_override_slider->value() - 100.f) / 25.f);
...
void AdvancedPane::Update()
{
...

  {
    ...
    m_vi_clock_override_slider->setValue(static_cast<int>(
        std::round(std::log2f(Config::Get(Config::MAIN_VI_OVERCLOCK)) * 25.f + 100.f)));
  }
