...
void AudioPane::CreateWidgets()
{
  ...
  m_turbo_mute_enable = new QCheckBox(tr("Mute Audio On Turbo"));
  m_turbo_mute_enable->setToolTip(
      tr("When overriding the emulation speed limit (default hotkey: Tab), mute the audio."));

  misc_layout->addWidget(m_turbo_mute_enable, 0, 0, 1, 1);
...
void AudioPane::ConnectWidgets()
{
  ...
  connect(m_turbo_mute_enable, &QCheckBox::toggled, this, &AudioPane::SaveSettings);
...
void AudioPane::LoadSettings()
{
  ...
  // Misc
  m_turbo_mute_enable->setChecked(Config::Get(Config::MAIN_AUDIO_MUTE_ON_TURBO));
...
void AudioPane::SaveSettings()
{
  ...
  // Misc
  Config::SetBaseOrCurrent(Config::MAIN_AUDIO_MUTE_ON_TURBO, m_turbo_mute_enable->isChecked());
