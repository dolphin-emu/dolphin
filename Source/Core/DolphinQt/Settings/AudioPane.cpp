...
void AudioPane::CreateWidgets()
{
  ...
  dsp_layout->addWidget(m_dsp_combo);
...
void AudioPane::OnDspChanged()
{
  ...
  const bool enabled =
      AudioCommon::SupportsDPL2Decoder(backend) && m_dsp_combo->currentIndex() != DSP_HLE_INDEX;
...
void AudioPane::OnEmulationStateChanged(bool running)
{
  ...
  if (AudioCommon::SupportsDPL2Decoder(Config::Get(Config::MAIN_AUDIO_BACKEND)) &&
      m_dsp_combo->currentIndex() != DSP_HLE_INDEX)
  {
...
void AudioPane::AddDescriptions()
{
  static const char TR_DSP_DESCRIPTION[] = QT_TR_NOOP(
      "HLE - High Level Emulation of the DSP. Fast, but not always accurate. Lacks Dolby Pro Logic "
      "II decoding.<br><br>"
      "LLE - Low Level Emulation of the DSP, via a recompiler. Slower, but more accurate. "
      "Enables Dolby Pro Logic II decoding on certain audio backends.<br><br>"
      "Interpreter - Low Level Emulation of the DSP, via an interpreter. Slowest, for debugging "
      "purposes only. Not recommended; consider using DSP HLE "
      "or DSP LLE Recompiler instead.<br><br><dolphin_emphasis>If unsure, "
      "select HLE.</dolphin_emphasis>");
  ...
  m_dsp_combo->SetTitle(tr("DSP Choice"));
