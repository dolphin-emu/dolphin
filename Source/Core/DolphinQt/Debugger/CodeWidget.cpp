...
void CodeWidget::UpdateSymbols()
{
  ...
  for (const auto& symbol : m_ppc_symbol_db.Symbols())
  {
    ...
    if (symbol.second.has_object_name)
    {
  ...
  for (const auto& call : symbol->calls)
  {
    ...
    if (call_symbol)
    {
      const QString name = QString::fromStdString(fmt::format("> {} ({}{:08x})", call_symbol->name,
          call_symbol->has_object_name ? fmt::format("{}, ", call_symbol->object_name) : "", addr));
      if (!name.contains(filter, Qt::CaseInsensitive))
        continue;
  ...
  for (const auto& caller : symbol->callers)
  {
    ...
    if (caller_symbol)
    {
      const QString name = QString::fromStdString(fmt::format("< {} ({}{:08x})", caller_symbol->name,
          caller_symbol->has_object_name ? fmt::format("{}, ",  caller_symbol->object_name) : "", addr));
