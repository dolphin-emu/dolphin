...
bool DeleteAddressEventFilter::eventFilter(QObject* object, QEvent* event)
{
  if (event->type() == QEvent::KeyPress)
  {
    ...
    if (key_event->key() == Qt::Key_Delete && m_combo_box->view()->isVisible())
    {
      const int row = m_combo_box->view()->currentIndex().row();
      m_combo_box->removeItem(row);
      return true;
    }
    // Disable up/down key navigation when the ComboBox is closed. Alt/Shift up/down are still
    // allowed.
    if (key_event->key() == Qt::Key_Down || key_event->key() == Qt::Key_Up)
      if (!m_combo_box->view()->isVisible() && key_event->modifiers() == Qt::NoModifier)
        return true;
  }
...
void CodeWidget::CreateWidgets()
{
  ...
  auto* search_address_filter = new DeleteAddressEventFilter(m_search_address);
  m_search_address->installEventFilter(search_address_filter);
  m_search_address->view()->installEventFilter(search_address_filter);
  ...
  m_search_address->view()->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
  ...
  m_save_address_btn->setToolTip(tr("Save address to the address box dropdown. Highlight an "
                                    "address and press Delete to clear it."));
