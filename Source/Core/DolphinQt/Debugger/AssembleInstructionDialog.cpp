...
void AssembleInstructionDialog::OnEditChanged()
{
  ...
  if (IsFailure(asm_result))
  {
    ...
    m_error_loc_label->setText(HtmlFormatErrorLoc(GetFailure(asm_result)));
    m_error_line_label->setText(HtmlFormatErrorLine(GetFailure(asm_result)));
    m_msg_label->setText(QString::fromStdString(GetFailure(asm_result).message).toHtmlEscaped());
  }
