...
void JitBlockTableModel::DisconnectSlots()
{
  ...
  disconnect(
      host, &Host::PPCBreakpointsChanged, this, &JitBlockTableModel::OnPPCBreakpointsChanged);
  auto* const settings = &Settings::Instance();
...
QVariant JitBlockTableModel::DisplayRoleData(const QModelIndex& index) const
{
  ...
  case Column::CyclesPercent:
    ...
    return QStringLiteral("%1%").arg(
        100.0 * profile_data->cycles_spent / m_overall_cycles_spent, 10, 'f', 6);
...
JitBlockTableModel::JitBlockTableModel(
    Core::System& system, JitInterface& jit_interface, PPCSymbolDB& ppc_symbol_db, QObject* parent)
    : QAbstractTableModel(parent)
    , m_system(system)
    , m_jit_interface(jit_interface)
