...
void JitBlockTableModel::OnBreakpointsChanged()
{
  // This slot is unfortunate, because it must always be connected even when the JIT Widget is
  // hidden due to design elsewhere in DolphinQt. We don't want the table model doing any work
  // it doesn't need to, so this condition is the best solution I came up with to achieve that.
  if (m_jit_blocks.empty())
    return;
  Update(Core::GetState(m_system));
}
