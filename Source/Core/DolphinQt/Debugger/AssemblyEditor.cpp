...
AsmEditor::AsmEditor(const QString& path, int editor_num, QWidget* parent)
    : QPlainTextEdit(parent), m_path(path), m_base_address(QStringLiteral()),
      m_editor_num(editor_num), m_dirty(false)
{
  if (!m_path.isEmpty())
  {
    m_filename =
  ...
  m_highlighter = new GekkoSyntaxHighlight(document());
...
bool AsmEditor::LoadFromPath()
{
  ...
  std::string base_addr_line = file.readLine().toStdString();
  ...
  {
    if (base_address.back() == '\n')
    {
      base_address.pop_back();
    }
