...
void AssemblerWidget::CreateWidgets()
{
  ...
  m_new->setIcon(Resources::GetThemeIcon("new"));
  m_open->setIcon(Resources::GetThemeIcon("openasm"));
  m_save->setIcon(Resources::GetThemeIcon("save"));
  m_assemble->setIcon(Resources::GetThemeIcon("assemble"));
  m_inject->setIcon(Resources::GetThemeIcon("inject"));
  ...
  m_output_type->addItem(tr("Gecko (Write)"));
  m_output_type->addItem(tr("Gecko (Execute)"));
  m_output_type->addItem(tr("Gecko (Trampoline)"));
  m_copy_output_button->setIcon(Resources::GetThemeIcon("clipboard"));
...
void AssemblerWidget::OnAssemble(std::vector<CodeBlock>& asm_out)
{
  ...
  if (IsFailure(result))
  {
    ...
    m_error_box->append(HtmlFormatErrorLoc(GetFailure(result)));
    m_error_box->append(HtmlFormatErrorLine(GetFailure(result)));
    m_error_box->append(HtmlFormatMessage(GetFailure(result)));
    asm_out.clear();
    return;
  }
...
QString AssemblerWidget::TabTextForEditor(AsmEditor* editor, bool with_dirty)
{
  ...
  return tr("%1%2").arg(editor->FileName()).arg(dirtyFlag);
}
