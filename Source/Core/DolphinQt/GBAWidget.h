...
class GBAWidgetController : public QObject
{
  Q_OBJECT
public:
  explicit GBAWidgetController();
  ~GBAWidgetController();

public slots:
  void Create(std::weak_ptr<HW::GBA::Core> core, int device_number, std::string game_title,
              int width, int height);
  void GameChanged(std::string game_title, int width, int height);
