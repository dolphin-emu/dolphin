...
void IRWidget::paintEvent(QPaintEvent* event)
{
  ...
  const int x = PADDING + (w - (m_x * w) / ir_max_x);
  const int y = PADDING + ((m_y * h) / ir_max_y);
...
void IRWidget::handleMouseEvent(QMouseEvent* event)
{
  ...
  else
  {
    ...
    const int new_x = ir_max_x - (event->x() * ir_max_x) / width();
    const int new_y = (event->y() * ir_max_y) / height();
