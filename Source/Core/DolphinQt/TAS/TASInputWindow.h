...
public:
  ...
  void AddFunction(std::string group_name, std::string control_name, OverrideFunction function);

  ControllerEmu::InputOverrideFunction GetInputOverrideFunction() const;

private:
  std::map<std::pair<std::string, std::string>, OverrideFunction> m_functions;
};
...
protected:
  TASCheckBox* CreateButton(const QString& text, std::string group_name, std::string control_name,
                            InputOverrider* overrider);
  QGroupBox* CreateStickInputs(const QString& text, std::string group_name,
                               InputOverrider* overrider, QSpinBox*& x_value, QSpinBox*& y_value,
                               u16 max_x, u16 max_y, Qt::Key x_shortcut_key,
                               Qt::Key y_shortcut_key);
  QBoxLayout* CreateSliderValuePairLayout(const QString& text, std::string group_name,
                                          std::string control_name, InputOverrider* overrider,
                                          QSpinBox*& value, u16 zero, int default_, u16 max,
                                          Qt::Key shortcut_key, QWidget* shortcut_widget,
                                          std::optional<ControlState> scale = {});
  QSpinBox* CreateSliderValuePair(std::string group_name, std::string control_name,
                                  InputOverrider* overrider, QBoxLayout* layout, u16 zero,
                                  int default_, u16 max, QKeySequence shortcut_key_sequence,
...
private:
  ...
  std::optional<ControlState> GetSpinBox(QSpinBox* spin, u16 zero, u16 max,
                                         ControlState controller_state);
  std::optional<ControlState> GetSpinBox(QSpinBox* spin, u16 zero, u16 max,
                                         ControlState controller_state, ControlState scale);
