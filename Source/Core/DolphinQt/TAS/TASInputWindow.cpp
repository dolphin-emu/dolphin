...
void InputOverrider::AddFunction(std::string group_name, std::string control_name,
                                 OverrideFunction function)
{
  m_functions.emplace(std::make_pair(std::move(group_name), std::move(control_name)),
                      std::move(function));
}

ControllerEmu::InputOverrideFunction InputOverrider::GetInputOverrideFunction() const
{
  return [this](const std::string& group_name, const std::string& control_name,
...
TASCheckBox* TASInputWindow::CreateButton(const QString& text, std::string group_name,
                                          std::string control_name, InputOverrider* overrider)
{
  TASCheckBox* checkbox = new TASCheckBox(text, this);

  overrider->AddFunction(std::move(group_name), std::move(control_name),
                         [this, checkbox](ControlState controller_state) {
...
QGroupBox* TASInputWindow::CreateStickInputs(const QString& text, std::string group_name,
                                             ...
                                             Qt::Key x_shortcut_key, Qt::Key y_shortcut_key)
{
  ...
  overrider->AddFunction(group_name, ControllerEmu::ReshapableInput::X_INPUT_OVERRIDE,
                         [this, x_value, x_default, max_x](ControlState controller_state) {
                           return GetSpinBox(x_value, x_default, max_x, controller_state);
                         });

  overrider->AddFunction(std::move(group_name), ControllerEmu::ReshapableInput::Y_INPUT_OVERRIDE,
                         [this, y_value, y_default, max_y](ControlState controller_state) {
                           return GetSpinBox(y_value, y_default, max_y, controller_state);
                         });
...
QBoxLayout* TASInputWindow::CreateSliderValuePairLayout(
    const QString& text, std::string group_name, std::string control_name,
    InputOverrider* overrider, QSpinBox*& value, u16 zero, int default_, u16 max,
    Qt::Key shortcut_key, QWidget* shortcut_widget, std::optional<ControlState> scale)
{
  ...
  value = CreateSliderValuePair(std::move(group_name), std::move(control_name), overrider, layout,
                                zero, default_, max, shortcut_key_sequence, Qt::Horizontal,
...
QSpinBox* TASInputWindow::CreateSliderValuePair(
    std::string group_name, std::string control_name, InputOverrider* overrider, QBoxLayout* layout,
    u16 zero, int default_, u16 max, QKeySequence shortcut_key_sequence,
    Qt::Orientation orientation, QWidget* shortcut_widget, std::optional<ControlState> scale)
{
  ...
  if (scale)
  {
    func = [this, value, zero, max, scale](ControlState controller_state) {
      return GetSpinBox(value, zero, max, controller_state, *scale);
    };
  }
  else
  {
    func = [this, value, zero, max](ControlState controller_state) {
      return GetSpinBox(value, zero, max, controller_state);
    };
  }

  overrider->AddFunction(std::move(group_name), std::move(control_name), std::move(func));
...
std::optional<ControlState> TASInputWindow::GetButton(TASCheckBox* checkbox,
                                                      ControlState controller_state)
{
  const bool pressed = std::lround(controller_state) > 0.0;
...
std::optional<ControlState> TASInputWindow::GetSpinBox(QSpinBox* spin, u16 zero, u16 max,
                                                       ControlState controller_state)
{
  ...
  return ControllerEmu::EmulatedController::MapToFloat<ControlState, u16>(spin->value(), zero, 0,
                                                                          max);
}
...
std::optional<ControlState> TASInputWindow::GetSpinBox(QSpinBox* spin, u16 zero, u16 max,
                                                       ...
                                                       ControlState scale)
{
  const u16 controller_value = static_cast<u16>(std::lround(controller_state * scale + zero));
