...
void FilesystemWidget::PopulateDirectory(
    int partition_id, QStandardItem* root, const DiscIO::Partition& partition)
{
...
void FilesystemWidget::PopulateDirectory(
    int partition_id, QStandardItem* root, const DiscIO::FileInfo& directory)
{
  for (const auto& info : directory)
  {
    ...
    item->setData(
        QVariant::fromValue(info.IsDirectory() ? EntryType::Dir : EntryType::File), ENTRY_TYPE);
...
void FilesystemWidget::ShowContextMenu(const QPoint&)
{
        ...
        [this, partition]
        {
          ...
          if (ExtractSystemData(partition, folder))
            ModalMessageBox::information(
                this, tr("Success"), tr("Successfully extracted system data."));
          else
  ...
  case EntryType::File:
    menu->addAction(tr("Extract File..."), this,
        [this, partition, path]
        {
          auto dest = DolphinFileDialog::getSaveFileName(
              this, tr("Save File To"), QFileInfo(path).fileName());
...
void FilesystemWidget::ExtractDirectory(
...
void FilesystemWidget::ExtractFile(
    const DiscIO::Partition& partition, const QString& path, const QString& out)
{
  const DiscIO::FileSystem* filesystem = m_volume->GetFileSystem(partition);
  if (!filesystem)
