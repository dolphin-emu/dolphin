...
void CalibrationWidget::DrawInProgressCalibration(QPainter& p, Common::DVec2 point)
{
  ...
  // Show the red dot only if it's approaching the calibration.
  // The cool spinning stick is otherwise uglified by the red dot always being show.
  const auto near_calibration =
      0.5 * m_calibrator->GetCalibrationRadiusAtAngle(std::atan2(point.y, point.x) + MathUtil::TAU);
  if (Common::DVec2{point.x, point.y}.LengthSquared() > (near_calibration * near_calibration))
  {
...
void ReshapableInputIndicator::Update(float elapsed_seconds)
{
  if (m_calibration_widget != nullptr)
    m_calibration_widget->StepAnimation(elapsed_seconds);
}
...
void CalibrationWidget::StartMappingAndCalibration()
{
  ...
  m_mapper = std::make_unique<ciface::MappingCommon::DirectionalInputMappingsDetector>(
...
void CalibrationWidget::StartCalibration(std::optional<Common::DVec2> center)
{
  ...
  // i18n: A button to stop a game controller calibration process.
