...
void GeneralWidget::CreateWidgets()
{
  ...
  m_aspect_combo = new ConfigChoice(
      {tr("Auto"), tr("Force 16:9"), tr("Force 4:3"), tr("Custom"), tr("Stretch to Window")},
...
void GeneralWidget::ConnectWidgets()
{
  ...
  connect(m_aspect_combo, qOverload<int>(&QComboBox::currentIndexChanged), this, [&](int index) {
    const bool is_custom_aspect_ratio = (index == (int)AspectMode::Custom);
...
void GeneralWidget::AddDescriptions()
{
  ...
  static const char TR_ASPECT_RATIO_DESCRIPTION[] =
      QT_TR_NOOP("Selects which aspect ratio to use when rendering.<br>"
                 "Each game can have a slightly different default aspect ratio."
                 ...
                 "<br>Custom: For games running with specific custom aspect ratio cheats."
                 "<br>Stretch to Window: Stretches the picture to the window size.<br><br>"
