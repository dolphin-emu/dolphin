...
ConvertDialog::ConvertDialog(
    QList<std::shared_ptr<const UICommon::GameFile>> files, QWidget* parent)
    : QDialog(parent)
    , m_files(std::move(files))
{
  ...
  connect(
      m_compression, &QComboBox::currentIndexChanged, this, &ConvertDialog::OnCompressionChanged);
...
void ConvertDialog::AddToCompressionComboBox(
    const QString& name, DiscIO::WIARVZCompressionType type)
{
...
void ConvertDialog::OnFormatChanged()
{
  ...
  case DiscIO::BlobType::RVZ:
  {
    ...
    AddToCompressionComboBox(
        slow.arg(QStringLiteral("bzip2")), DiscIO::WIARVZCompressionType::Bzip2);

    AddToCompressionComboBox(slow.arg(QStringLiteral("LZMA")), DiscIO::WIARVZCompressionType::LZMA);

    AddToCompressionComboBox(
        slow.arg(QStringLiteral("LZMA2")), DiscIO::WIARVZCompressionType::LZMA2);
    ...
    if (format == DiscIO::BlobType::RVZ)
    {
      ...
      AddToCompressionComboBox(
          recommended.arg(QStringLiteral("Zstandard")), DiscIO::WIARVZCompressionType::Zstd);
...
void ConvertDialog::Convert()
{
  ...
  for (const auto& file : m_files)
  {
    ...
    if (std::filesystem::exists(StringToPath(dst_path.toStdString())))
    {
      ...
      if (std::filesystem::equivalent(
              StringToPath(dst_path.toStdString()), StringToPath(original_path), ec))
      {
    ...
    else
    {
      ...
      case DiscIO::BlobType::PLAIN:
        success = std::async(std::launch::async,
            [&]
            {
              const bool good = DiscIO::ConvertToPlain(
                  blob_reader.get(), original_path, dst_path.toStdString(), callback);
      ...
      if (!success.get())
      {
        ModalMessageBox::critical(
            this, tr("Error"), tr("Dolphin failed to complete the requested action."));
        return;
      }
  ...
  ModalMessageBox::information(
      this, tr("Success"), tr("Successfully converted %n image(s).", "", success_count));
