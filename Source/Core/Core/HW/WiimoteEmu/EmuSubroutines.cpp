...
void Wiimote::InvokeHandler(H&& handler, const WiimoteCommon::OutputReportGeneric& rpt, u32 size)
{
  ...
  (this->*handler)(Common::BitCastPtr<T>(&rpt.rpt_stub.storage));
}
...
void Wiimote::HandleWriteData(const OutputReportWriteData& wd)
{
  ...
  DEBUG_LOG_FMT(WIIMOTE, "Wiimote::WriteData: {:#04x} @ {:#04x} @ {:#04x} ({})",
                (u8)wd.space().Get(), wd.slave_address(), address, wd.size);
  ...
  default:
    WARN_LOG_FMT(WIIMOTE, "WriteData: invalid address space: {:#x}", (u8)wd.space().Get());
