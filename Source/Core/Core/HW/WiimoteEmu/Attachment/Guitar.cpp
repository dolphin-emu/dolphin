...
Guitar::Guitar(ExtensionReg& reg) : Attachment(_trans("Guitar"), reg)
{
  ...
  for (auto& guitar_fret_name : guitar_fret_names)
    m_frets->controls.emplace_back(new ControllerEmu::InputReference(guitar_fret_name));

  // strum
  groups.emplace_back(m_strum = new ControllerEmu::Buttons(_trans("Strum")));
  m_strum->controls.emplace_back(new ControllerEmu::InputReference(_trans("Up")));
  m_strum->controls.emplace_back(new ControllerEmu::InputReference(_trans("Down")));
  ...
  m_whammy->controls.emplace_back(new ControllerEmu::InputReference(_trans("Bar")));
...
void Guitar::GetState(u8* const data)
{
  ...
  // slider bar
  if (m_slider_bar->controls[0]->BoundCount())
  {
