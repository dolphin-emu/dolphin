...
Wiimote::Wiimote(const unsigned int index) : m_index(index)
{
  ...
  for (auto& named_button : {A_BUTTON, B_BUTTON, ONE_BUTTON, TWO_BUTTON, PLUS_BUTTON, MINUS_BUTTON})
  {
...
void Wiimote::UpdateButtonsStatus()
{
  ...
  m_buttons->GetState(&m_status.buttons.hex, button_bitmasks, m_input_override_function);
...
bool Wiimote::IsButtonPressed()
{
  u16 buttons = 0;
  const auto lock = GetStateLock();
  m_buttons->GetState(&buttons, button_bitmasks, m_input_override_function);
  m_dpad->GetState(&buttons, dpad_bitmasks, m_input_override_function);
