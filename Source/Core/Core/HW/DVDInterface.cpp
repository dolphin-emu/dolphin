...
constexpr u64 BUFFER_TRANSFER_RATE = 1024 * 1024 * 16;

// The size of a Wii disc layer in bytes (2294912 sectors per layer)
...
constexpr double GC_DISC_INNER_READ_SPEED = 1024 * 1024 * 2.1;
constexpr double GC_DISC_OUTER_READ_SPEED = 1024 * 1024 * 3.325;
constexpr double WII_DISC_INNER_READ_SPEED = 1024 * 1024 * 3.5;
constexpr double WII_DISC_OUTER_READ_SPEED = 1024 * 1024 * 8.45;

// Experimentally measured seek constants. The time to seek appears to be
// linear, but short seeks appear to be lower velocity.
constexpr double SHORT_SEEK_DISTANCE = 0.002;
...
static int s_current_read_callback_event_type;
...
void ScheduleReads(u64 dvd_offset, u32 length, bool decrypt, u32 output_address, int callback_event_type);
...
static void FinishRead(u64 userdata, int cyclesLate)
{
	...
	if (offset + length == s_current_read_length)
	{
		DEBUG_LOG(DVDINTERFACE, "Read complete");
		DVDThread::Cleanup();
		CoreTiming::ScheduleEvent_Immediate(s_current_read_callback_event_type, DVDInterface::INT_TCINT);
	}
}
                        ...
                        int callback_event_type, DIInterruptType* interrupt_type)
{
	...
	ScheduleReads(DVD_offset, DVD_length, decrypt, output_address, callback_event_type);
	return true;
}
...
void ScheduleReads(u64 dvd_offset, u32 dvd_length, bool decrypt, u32 output_address, int callback_event_type)
{
	...
	// up to 16 MiB/s.
	//
	// Metroid Prime is a good example of a game that's sensitive to all of
	// these details; if there isn't enough latency in the right places, doors
	...
	s_current_read_callback_event_type = callback_event_type;
		...
		else
		{
			...
			else
			{
				...
				buffer_end = s_read_buffer_start_offset
					         ...
					         / (s_read_buffer_end_time - s_read_buffer_start_time), DVD_ECC_BLOCK_SIZE);
			}
	...
	DEBUG_LOG(DVDINTERFACE, "Buffer: start=0x%" PRIx64 " end=0x%" PRIx64,
	          buffer_start, buffer_end);

	u32 offset = 0;

	DEBUG_LOG(DVDINTERFACE, "Schedule reads: offset=0x%" PRIx64 " length=0x%" PRIx32 " address=0x%" PRIx32,
	...
	while (dvd_length > 0)
	{
		...
		if (rounded_offset >= buffer_start && rounded_offset < buffer_end)
		{
			...
			ticks_until_completion += ((u64)length * SystemTimers::GetTicksPerSecond())
		...
		CoreTiming::ScheduleEvent((int)ticks_until_completion, s_finish_read, ((u64)offset << 32) | length);
	...
	else
	{
		...
		s_read_buffer_start_time = ticks_until_completion;
		s_read_buffer_end_time = ticks_until_completion
		                         + CalculateRawDiscReadTime(s_read_buffer_start_offset,
		                                                   (u32)(s_read_buffer_end_offset - s_read_buffer_start_offset));
	}

	DEBUG_LOG(DVDINTERFACE, "Schedule reads: ecc blocks unbuffered=%d buffered=%d, ticks=%" PRId64 ", time=%" PRId64 "us",
...
double CalculatePhysicalDiscPosition(u64 offset)
{
	// Assumption: the layout on the second disc layer is identical to the
	// first, and that a multi-layer disc layer is identical to a single-layer
	// disc's layer.
	offset %= WII_DISC_LAYER_SIZE;
	...
	return std::sqrt((double)offset / WII_DISC_LAYER_SIZE
...
u64 CalculateSeekTime(u64 offset_from, u64 offset_to)
{
	...
	if (distance < SHORT_SEEK_DISTANCE)
		time = (SHORT_SEEK_CONSTANT + distance * SHORT_SEEK_VELOCITY_INVERSE) * SystemTimers::GetTicksPerSecond();
	else
		time = (LONG_SEEK_CONSTANT + distance * LONG_SEEK_VELOCITY_INVERSE) * SystemTimers::GetTicksPerSecond();
...
// drive and varies between ~3-8MiB/s. Note that there is technically a DMA
// delay on top of this, but we model that as part of this read time.ß
u64 CalculateRawDiscReadTime(u64 offset, u32 length)
{
	// The Wii is a CAV drive and the data has a constant pit length
	...
	double physical_offset = CalculatePhysicalDiscPosition(offset);

	double speed;
	if (s_inserted_volume->GetVolumeType() == DiscIO::IVolume::WII_DISC)
	{
		speed = (physical_offset - DVD_INNER_RADIUS)
		         / (WII_DVD_OUTER_RADIUS - DVD_INNER_RADIUS)
	...
	{
		speed = (physical_offset - DVD_INNER_RADIUS)
		         / (GC_DVD_OUTER_RADIUS - DVD_INNER_RADIUS)
		         * (GC_DISC_OUTER_READ_SPEED - GC_DISC_INNER_READ_SPEED)
		         + GC_DISC_INNER_READ_SPEED;
	}

	DEBUG_LOG(DVDINTERFACE, "Read 0x%" PRIx32 " @ 0x%" PRIx64 " @%lfmm: %lf us, %lf MiB/s",
		      length, offset, physical_offset * 1000, length / speed * 1000 * 1000, speed / 1024 / 1024);
