                    ...
                    bool write_to_DIIMMBUF, int callback_event_type)
{
	...
	case DVDLowRead:
		INFO_LOG(DVDINTERFACE, "DVDLowRead: DVDAddr: 0x%09lx, Size: 0x%x", (u64)command_2 << 2, command_1);
	...
	case DVDLowUnencryptedRead:
		INFO_LOG(DVDINTERFACE, "DVDLowUnencryptedRead: DVDAddr: 0x%09lx, Size: 0x%x", (u64)command_2 << 2, command_1);
		...
		{
			WARN_LOG(DVDINTERFACE, "DVDLowUnencryptedRead: trying to read out of bounds @ %09lx", (u64)command_2 << 2);
		...
		case 0x00: // Read Sector
			{
				...
				INFO_LOG(DVDINTERFACE, "Read: DVDOffset=%08lx, DMABuffer = %08x, SrcLength = %08x, DMALength = %08x",
				...
				if (GCAM)
				{
					if (iDVDOffset & 0x80000000) // read request to hardware buffer
					{
						...
						case 0x84000020:
							...
							break;
						default:
							ERROR_LOG(DVDINTERFACE, "GC-AM: UNKNOWN MEDIA BOARD LOCATION %lx", iDVDOffset);
							break;
						}
						break;
					}
	...
	case 0xAA:
		if (GCAM)
		{
			...
			if ((offset < 0) || ((offset + len) > 0x40) || len > 0x40)
			{
				...
				while (len >= 4)
				{
					ERROR_LOG(DVDINTERFACE, "GC-AM Media Board WRITE (0xAA): %08lx: %08x", iDVDOffset, Memory::Read_U32(addr));
					addr += 4;
					len -= 4;
					iDVDOffset += 4;
				}
	...
	case DVDLowSeek:
		if (!GCAM)
		{
			...
			INFO_LOG(DVDINTERFACE, "Seek: offset=%09lx (ignoring)", (u64)command_1 << 2);
		}
		else
		{
	...
	case 0xE1:
		{
			...
			INFO_LOG(DVDINTERFACE, "(Audio) Stream cmd: %08x offset: %08lx length: %08x",
...
u64 SimulateDiscReadTime(u64 offset, u32 length)
{
	...
	if (offset + length - g_last_read_offset > 1024 * 1024)
	{
		...
		DEBUG_LOG(DVDINTERFACE, "Seeking %ld bytes",
	...
	else
	{
		...
		{
			DEBUG_LOG(DVDINTERFACE, "Fast buffer read at %lx", offset);
		...
		{
			DEBUG_LOG(DVDINTERFACE, "Slow buffer read at %lx", offset);
		...
		{
			DEBUG_LOG(DVDINTERFACE, "Short seek %ld bytes",
			          s64(g_last_read_offset) - s64(offset));
			ticks_until_completion = disk_read_duration;
			g_last_read_time = current_time + ticks_until_completion;
		}
