	...
	u32 DVD_length, u32 output_length, bool raw = false)
{
	...
	if (raw)
	{
		...
		{
			WARN_LOG(DVDINTERFACE, "DVDLowUnencryptedRead: trying to read out of bounds @ %x", DVD_offset_32);
                                ...
                                u32 output_address, u32 output_length, bool write_to_DIIMMBUF)
{
	...
	// Only used from WII_IPC, not through direct access
	...
	// Only used from WII_IPC, not through direct access
	case DVDLowRead:
		INFO_LOG(DVDINTERFACE, "DVDLowRead: DVDAddr: 0x%08" PRIx64 ", Size: 0x%x", (u64)command_2 << 2, command_1);
	...
	case DVDLowUnencryptedRead:
		INFO_LOG(DVDINTERFACE, "DVDLowUnencryptedRead: DVDAddr: 0x%08" PRIx64 ", Size: 0x%x", (u64)command_2 << 2, command_1);
			...
			case 0x00: // Read Sector
				{
					...
					INFO_LOG(DVDINTERFACE, "Read: DVDOffset=%08x, DMABuffer=%08x, SrcLength=%08x, DMALength=%08x",
	...
	case DVDLowSeek:
		if (!GCAM)
		{
			...
			INFO_LOG(DVDINTERFACE, "Seek: offset=%08x (ignoring)", (u64)command_1 << 2);
		}
		else
		{
	...
	case 0xE1:
		{
			...
			INFO_LOG(DVDINTERFACE, "(Audio) Stream cmd: %08x offset: %08x length: %08x",
...
u64 SimulateDiscReadTime(u64 offset, u32 length)
{
	...
	else
	{
		...
		{
			DEBUG_LOG(DVDINTERFACE, "Fast buffer read at %" PRId64, s64(offset));
		...
		{
			DEBUG_LOG(DVDINTERFACE, "Slow buffer read at %" PRId64, s64(offset));
