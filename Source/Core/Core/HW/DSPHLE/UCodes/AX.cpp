  ...
  while (pb_addr)
  {
    ...
    for (int curr_ms = 0; curr_ms < 5; ++curr_ms)
    {
      ...
      ProcessVoice(pb, buffers, spms, ConvertMixerControl(pb.mixer_control),
                   m_coeffs_available ? m_coeffs.data() : nullptr);
...
void AXUCode::DoAXState(PointerWrap& p)
{
  ...
  bool old_available = m_coeffs_available;
  u32 old_checksum = m_coeffs_checksum;
  p.Do(m_coeffs_available);
  p.Do(m_coeffs_checksum);

  if (p.GetMode() == PointerWrap::MODE_READ && m_coeffs_available &&
      (!old_available || old_checksum != m_coeffs_checksum))
  {
