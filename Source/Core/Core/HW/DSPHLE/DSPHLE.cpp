...
void DSPHLE::DoState(PointerWrap& p)
{
  ...
  // Move all ucodes back if they were the same, or keep the new ones when loading a state.
  if (same_ucode || p.GetMode() == PointerWrap::MODE_READ)
    m_ucode = std::move(ucode);
  if (same_last_ucode || p.GetMode() == PointerWrap::MODE_READ)
