...
typedef std::pair<ReadRequest, std::vector<u8>> ReadResult;
...
void Stop()
{
  ...
  // This isn't actually necessary, but it feels a bit cleaner to me
  s_next_id = 0;
}
...
void DoState(PointerWrap& p)
{
  ...
  while (s_result_queue.Pop(result))
  {
    ...
    s_result_map.insert(std::make_pair(id, std::move(result)));
  }
