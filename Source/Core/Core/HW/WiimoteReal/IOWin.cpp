...
namespace
{
// TODO C++23: Replace usages with std::optional<>::transform
template <class Opt, class F>
auto optional_transform(Opt&& opt, F&& func)
{
  using T = std::decay_t<Opt>::value_type;
  using Result = std::optional<std::decay_t<std::invoke_result_t<F, T&>>>;
  return opt ? Result(std::invoke(std::forward<F>(func), *std::forward<Opt>(opt))) : Result{};
}
}  // namespace

namespace WiimoteReal
{
enum class BluetoothQueryType : bool
{
  ExistingDevices,
  NewScan,
};
...
void EnumerateBluetoothWiimotes(BluetoothQueryType query_type, auto&& enumeration_callback)
{
  EnumerateBluetoothDevices(query_type,
...
void EnumerateBluetoothDevices(BluetoothQueryType query_type, auto&& enumeration_callback)
{
  BLUETOOTH_DEVICE_SEARCH_PARAMS search_params{
      ...
      .fIssueInquiry = query_type == BluetoothQueryType::NewScan,
  ...
  EnumerateRadios([&](HANDLE radio_handle) {
    ...
    {
      ERROR_LOG_FMT(WIIMOTE, "BluetoothFindFirstDevice: {}", Common::GetLastErrorString());
      return EnumerationControl::Continue;
    }
...
u32 RemoveWiimoteBluetoothDevices(std::invocable<BLUETOOTH_DEVICE_INFO> auto&& should_remove_filter)
{
  ...
  EnumerateBluetoothWiimotes(
      BluetoothQueryType::ExistingDevices,
      [&](HANDLE radio_handle, const auto& radio_info, auto* btdi) {
...
u32 DiscoverAndPairWiimotes(std::optional<AuthenticationMethod> auth_method = std::nullopt)
{
  RemoveUnusableWiimoteBluetoothDevices();

  u32 success_count = 0;
  EnumerateBluetoothWiimotes(
      BluetoothQueryType::NewScan, [&](HANDLE radio_handle, const auto& radio_info, auto* btdi) {
        if (btdi->fConnected)
          return EnumerationControl::Continue;
...
u32 WiimoteScannerWindows::HostPerformWiimoteSync()
{
  ...
  return DiscoverAndPairWiimotes(AuthenticationMethod::SyncButton);
}

u32 WiimoteScannerWindows::HostPerformWiimoteReset()
{
  return RemoveWiimoteBluetoothDevices(&BLUETOOTH_DEVICE_INFO::fRemembered);
}
...
int WiimoteWindows::OverlappedRead(u8* data, DWORD size)
{
  ...
  // This is okay. It happens on read timoeut or IOWakeup.
  if (overlapped_error == ERROR_OPERATION_ABORTED)
    return -1;

  ERROR_LOG_FMT(WIIMOTE, "GetOverlappedResult: {}", Common::GetWin32ErrorString(overlapped_error));
  return 0;
}
...
int WiimoteWindows::OverlappedWrite(const u8* data, DWORD size)
{
  ...
  ERROR_LOG_FMT(WIIMOTE, "OverlappedWrite GetOverlappedResult: {}", Common::GetLastErrorString());
  return 0;
}
                                                  ...
                                                  Wiimote*& found_board)
{
       ...
       hid_iface += wcslen(hid_iface) + 1)
  {
    ...
    // The Mayflash DolphinBar does not advertise the RVL- names,
    //  though I have only tested one hardware revision.
    // I am not sure if *every* DolphinBar advertises exactly the same description.
    ...
    // TODO: This might be unnecessary. Checking the description should be enough?
    ...
    if (!is_relevant_description)
    {
      ...
      // TODO: Does the Balance Board have a different PID?
      // TODO: Our hidapi code checks an additional PID.
      constexpr u16 NINTENDO_WIIMOTE_PRODUCT_ID = 0x0306;

      constexpr auto wiimote_dev_info =
          USBUtils::DeviceInfo{NINTENDO_VENDOR_ID, NINTENDO_WIIMOTE_PRODUCT_ID};
    ...
    {
      WARN_LOG_FMT(WIIMOTE, "ConnectInternal failed");
      continue;
    }
...
void WiimoteScannerWindows::FindAlreadyConnectedWiimote(std::vector<Wiimote*>& found_wiimotes,
                                                        Wiimote*& found_board)
{
