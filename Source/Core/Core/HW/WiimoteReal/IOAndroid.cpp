...
void WiimoteScanner::FindWiimotes(std::vector<Wiimote*> & found_wiimotes, Wiimote* & found_board)
{
	...
	JNIEnv* env;
	int get_env_status = g_java_vm->GetEnv((void **)&env, JNI_VERSION_1_6);
...
{
	g_java_vm->AttachCurrentThread(&m_env, NULL);
...
int WiimoteAndroid::IOWrite(u8 const* buf, size_t len)
{
	...
	jbyte* output = m_env->GetByteArrayElements(output_array, NULL);
