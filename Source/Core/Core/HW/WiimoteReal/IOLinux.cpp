...
static std::string BDAddrToString(bdaddr_t bdaddr)
{
  std::ranges::reverse(bdaddr.b);
  return Common::MacAddressToString(std::bit_cast<Common::MACAddress>(bdaddr));
}

static std::optional<bdaddr_t> StringToBDAddr(std::string_view str)
{
  auto result = Common::StringToMacAddress(str);
  if (!result)
    return std::nullopt;
  std::ranges::reverse(*result);
  return std::bit_cast<bdaddr_t>(*result);
}
...
static void AddAutoConnectAddresses(std::vector<Wiimote*>& found_wiimotes)
{
  ...
  {
    const auto bt_addr = StringToBDAddr(bt_address_str);
...
void WiimoteScannerLinux::FindWiimotes(std::vector<Wiimote*>& found_wiimotes, Wiimote*& found_board)
{
  ...
  {
    const auto bdaddr_str = BDAddrToString(scan_info.bdaddr);
    ...
    // We won't wait quite so long since we're doing this in a blocking manner.
...
WiimoteLinux::WiimoteLinux(bdaddr_t bdaddr)
...
{
  return BDAddrToString(m_bdaddr);
}
