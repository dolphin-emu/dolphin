...
void LoadSettings()
{
  for (unsigned int i = 0; i < MAX_WIIMOTES; ++i)
  {
    Config::Section* wiimote =
        Config::GetOrCreateSection(Config::System::WiiPad, StringFromFormat("Wiimote%d", i + 1));
    wiimote->Get("Source", &g_wiimote_sources[i], i ? WIIMOTE_SRC_NONE : WIIMOTE_SRC_EMU);
  }

  Config::Section* balanceboard =
      Config::GetOrCreateSection(Config::System::WiiPad, "BalanceBoard");
