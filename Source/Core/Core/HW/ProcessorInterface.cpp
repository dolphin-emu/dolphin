...
void Init()
{
	...
	*/
	...
	iosNotifyResetButton = CoreTiming::RegisterEvent("IOSNotifyResetButton", IOSNotifyResetButtonCallback);
}
...
static void IOSNotifyResetButtonCallback(u64 userdata, s64 cyclesLate) {
	...
	{
		std::shared_ptr<IWII_IPC_HLE_Device> stm = WII_IPC_HLE_Interface::GetDeviceByName("/dev/stm/eventhook");
		if (stm)
