...
static void AMMBCommandRecv(u32 parameter_offset)
{
  ...
  {
    ERROR_LOG_FMT(AMMEDIABOARD_NET, "AMMBCommandRecv: Bad data offset: {:08x}", off);
    return;
  }
...
static void AMMBCommandSend(u32 parameter_offset)
{
  ...
  if (data_span.size() < len)
  {
    ERROR_LOG_FMT(AMMEDIABOARD_NET, "AMMBCommandSend: Bad data offset: {:08x}", off);
    return;
  }
...
static void AMMBCommandAccept(u32 parameter_offset)
{
  ...
  {
    const u32 addrlen = Common::BitCastPtr<u32>(addrlen_span.data());
    ...
    {
      ERROR_LOG_FMT(AMMEDIABOARD_NET, "AMMBCommandAccept: Bad addr offset: {:08x}", addr_off);
    }
  }
  else
  {
    ERROR_LOG_FMT(AMMEDIABOARD_NET, "AMMBCommandAccept: Bad addrlen offset: {:08x}", addrlen_off);
  }
...
static void AMMBCommandSetSockOpt(u32 parameter_offset)
{
  ...
  {
    ERROR_LOG_FMT(AMMEDIABOARD_NET, "AMMBCommandSetSockOpt: Bad optval offset: {:08x}",
                  optval_offset);
    return;
  }
