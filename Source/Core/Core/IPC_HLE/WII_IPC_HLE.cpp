...
void DoState(PointerWrap &p)
{
	...
	for (auto &descripter : g_FdMap)
	{
		if (descripter)
			descripter->PrepareForState(p.GetMode());
	}
	...
	else
	{
		for (auto& descripter : g_FdMap)
		{
			u32 exists = descripter ? 1 : 0;
			p.Do(exists);
			if (exists)
			{
				u32 isHw = descripter->IsHardware() ? 1 : 0;
				p.Do(isHw);
				if (isHw)
				{
					u32 hwId = descripter->GetDeviceID();
					p.Do(hwId);
				}
				else
				{
					descripter->DoState(p);
				}
			}
		}
