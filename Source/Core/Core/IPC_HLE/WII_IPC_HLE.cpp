...
static s32 OpenDevice(const u32 address)
{
  ...
  Memory::Write_U32(new_fd, address + 0x4);
  device->Open(address, open_mode);
  const s32 open_return_code = Memory::Read_U32(address + 0x4);
...
static IPCCommandResult HandleCommand(const u32 address)
{
  ...
  const s32 fd = Memory::Read_U32(address + 0x8);
  ...
  case IPC_CMD_CLOSE:
    ...
    Memory::Write_U32(FS_SUCCESS, address + 0x4);
