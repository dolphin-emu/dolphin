...
IPCCommandResult CWII_IPC_HLE_Device_usb_oh1_57e_305_real::IOCtlV(u32 command_address)
{
  ...
  case USBV0_IOCTL_CTRLMSG:
  {
    ...
    const u16 opcode = *reinterpret_cast<u16*>(Memory::GetPointer(message.payload_addr));
    switch (opcode)
    {
    case 0xFC4C:
    case 0xFC4F:
...
void CWII_IPC_HLE_Device_usb_oh1_57e_305_real::FakeEventCommandComplete(const u16 opcode,
                                                                        const void* data,
                                                                        const u32 data_size,
