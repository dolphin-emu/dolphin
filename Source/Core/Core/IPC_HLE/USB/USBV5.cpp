...
USBV5IntrMessage::USBV5IntrMessage(const SIOCtlVBuffer& cmd_buffer)
{
  ...
  INFO_LOG(WII_IPC_USB, "[INTR] payload: %x length: %x endpoint: %x", data_addr, length, endpoint);
}
...
USBV5IsoMessage::USBV5IsoMessage(const SIOCtlVBuffer& cmd_buffer)
{
  ...
  const u16* sizes = reinterpret_cast<const u16*>(
      Memory::GetPointer(Memory::Read_U32(cmd_buffer.InBuffer[0].m_Address + 12)));
