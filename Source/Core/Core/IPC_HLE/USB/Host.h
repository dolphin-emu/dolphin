...
protected:
  ...
  using DeviceChangeHooks = std::map<std::shared_ptr<USBDevice>, ChangeEvent>;

  std::map<u64, std::shared_ptr<USBDevice>> m_devices;
  mutable std::mutex m_devices_mutex;
  Common::Event m_ready_to_trigger_hooks;

  void UpdateDevices();
  std::set<u64> AddNewDevices(DeviceChangeHooks* hooks);
  ...
  bool AddDevice(std::unique_ptr<USBDevice> device);
  std::shared_ptr<USBDevice> GetDeviceById(u64 device_id) const;
  std::shared_ptr<USBDevice> GetDeviceByVidPid(u16 vid, u16 pid) const;
  virtual void OnDeviceChange(ChangeEvent event, std::shared_ptr<USBDevice> changed_device) = 0;
