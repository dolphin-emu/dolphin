...
CWII_IPC_HLE_Device_usb_hid_v4::CWII_IPC_HLE_Device_usb_hid_v4(const u32 device_id,
...
std::shared_ptr<USBDevice> CWII_IPC_HLE_Device_usb_hid_v4::GetDeviceByIOSID(const s32 ios_id) const
{
...
void CWII_IPC_HLE_Device_usb_hid_v4::OnDeviceChange(ChangeEvent event,
...
std::vector<u32> CWII_IPC_HLE_Device_usb_hid_v4::GetDeviceEntry(USBDevice& device) const
{
  std::lock_guard<std::mutex> id_map_lock{m_id_map_mutex};
  const std::vector<u8> descriptors = device.GetIOSDescriptors(IOSVersion::USBV4);
