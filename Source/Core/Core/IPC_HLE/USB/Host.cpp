...
{
  // Wake up the scan thread in case it's still waiting
...
s32 CWII_IPC_HLE_Device_usb_host::GetUSBDeviceId(libusb_device* device)
{
#ifdef __LIBUSB__
  libusb_device_descriptor device_descriptor;
  libusb_get_device_descriptor(device, &device_descriptor);
  return static_cast<u64>(device_descriptor.idVendor) << 32 |
...
void CWII_IPC_HLE_Device_usb_host::UpdateDevices()
{
  ...
  for (ssize_t i = 0; i < cnt; ++i)
  {
    ...
    const s32 main_device_id = GetUSBDeviceId(device);
    const auto config_descr = std::make_unique<LibusbConfigDescriptor>(device);
    if (!config_descr->IsValid())
    {
      ERROR_LOG(WII_IPC_USB, "Failed to get config descriptor for device %x", main_device_id);
      continue;
    }
    ...
    {
      auto dev = std::make_unique<LibusbDevice>(device, main_device_id, interface);
  ...
  {
    INFO_LOG(WII_IPC_USB, "Dispatching hook: device %x %s", hook.device->GetId(),
             hook.event == ChangeEvent::Inserted ? "inserted" : "removed");
    OnDeviceChange(hook.event, hook.device);
  }
}

void CWII_IPC_HLE_Device_usb_host::StartThread()
{
...
void CWII_IPC_HLE_Device_usb_host::StopThread()
{
