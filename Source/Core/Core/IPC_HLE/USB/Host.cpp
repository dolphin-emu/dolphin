...
CWII_IPC_HLE_Device_usb_host::CWII_IPC_HLE_Device_usb_host(const u32 device_id,
...
bool CWII_IPC_HLE_Device_usb_host::AddDevice(std::unique_ptr<USBDevice> device)
{
...
std::shared_ptr<USBDevice> CWII_IPC_HLE_Device_usb_host::GetDeviceById(const u64 device_id) const
{
  std::lock_guard<std::mutex> lk(m_devices_mutex);
...
std::shared_ptr<USBDevice> CWII_IPC_HLE_Device_usb_host::GetDeviceByVidPid(const u16 vid,
                                                                           const u16 pid) const
{
  std::lock_guard<std::mutex> lk(m_devices_mutex);
...
void CWII_IPC_HLE_Device_usb_host::UpdateDevices()
{
  ...
  const std::set<u64> plugged_devices = AddNewDevices(&hooks);
  DetectRemovedDevices(plugged_devices, &hooks);
  DispatchHooks(hooks);
}

std::set<u64> CWII_IPC_HLE_Device_usb_host::AddNewDevices(DeviceChangeHooks* hooks)
{
  ...
  for (ssize_t i = 0; i < cnt; ++i)
  {
    ...
    const u8 num_interfaces =
        ShouldExposeInterfacesAsDevices() ? config_descr->m_descriptor->bNumInterfaces : 1;
...
void CWII_IPC_HLE_Device_usb_host::DetectRemovedDevices(const std::set<u64>& plugged_devices,
                                                        DeviceChangeHooks* hooks)
{
  std::lock_guard<std::mutex> lk(m_devices_mutex);
...
void CWII_IPC_HLE_Device_usb_host::DispatchHooks(const DeviceChangeHooks& hooks)
{
  for (const auto& hook : hooks)
  {
    INFO_LOG(WII_IPC_USB, "%s - Dispatching hook: device %04x:%04x %s", GetDeviceName().c_str(),
             hook.first->GetVid(), hook.first->GetPid(),
             hook.second == ChangeEvent::Inserted ? "inserted" : "removed");
