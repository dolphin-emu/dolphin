...
{
  std::map<std::shared_ptr<Device>, ChangeEvent> pending_hooks;
  ...
  for (ssize_t i = 0; i < cnt; ++i)
  {
    ...
    for (u8 interface = 0; interface < num_interfaces; ++interface)
    {
      ...
      if (AddDevice(std::move(dev)))
        pending_hooks.emplace(GetDeviceById(id), ChangeEvent::Inserted);
    }
  }
  ...
  // Detect and remove devices which have been unplugged
  {
    ...
    for (auto it = m_devices.begin(); it != m_devices.end();)
    {
      ...
      {
        pending_hooks.emplace(it->second, ChangeEvent::Removed);
  ...
  {
    INFO_LOG(WII_IPC_USB, "%s - Dispatching hook: device %04x:%04x (%x) %s",
             GetDeviceName().c_str(), hook.first->GetVid(), hook.first->GetPid(),
             hook.first->GetId(), hook.second == ChangeEvent::Inserted ? "inserted" : "removed");
