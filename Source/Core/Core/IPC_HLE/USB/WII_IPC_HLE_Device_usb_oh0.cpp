...
CWII_IPC_HLE_Device_usb_oh0::CWII_IPC_HLE_Device_usb_oh0(u32 dev_id, const std::string& dev_name)
                                                           ...
                                                           const u32 command_address)
{
  ...
  {
    int ret = -1;
    if (cmd_buffer.Parameter == USBV0_IOCTL_CTRLMSG)
      ret = device->SubmitTransfer(std::make_unique<USBV0CtrlMessage>(cmd_buffer));
    else if (cmd_buffer.Parameter == USBV0_IOCTL_BLKMSG)
      ret = device->SubmitTransfer(std::make_unique<USBV0BulkMessage>(cmd_buffer));
    else if (cmd_buffer.Parameter == USBV0_IOCTL_INTRMSG)
      ret = device->SubmitTransfer(std::make_unique<USBV0IntrMessage>(cmd_buffer));
    else if (cmd_buffer.Parameter == USBV0_IOCTL_ISOMSG)
      ret = device->SubmitTransfer(std::make_unique<USBV0IsoMessage>(cmd_buffer));
  ...
  default:
    ERROR_LOG(WII_IPC_USB, "[%x] Unknown IOCtlV: %02x", device->GetId(), cmd_buffer.Parameter);
    DumpAsync(cmd_buffer.BufferVector, cmd_buffer.NumberInBuffer, cmd_buffer.NumberPayloadBuffer,
              LogTypes::WII_IPC_USB, LogTypes::LERROR);
  }
  Memory::Write_U32(0, command_address + 4);
...
static void TriggerHook(std::map<DeviceInfo, u32>& hooks, u16 vid, u16 pid)
{
...
void CWII_IPC_HLE_Device_usb_oh0::OnDeviceChange(ChangeEvent event, std::shared_ptr<Device> device)
{
  std::lock_guard<std::mutex> lk(m_devices_mutex);
...
IPCCommandResult CWII_IPC_HLE_Device_usb_oh0_device::Open(u32 command_address, u32 mode)
{
  if (m_Active)
  {
...
IPCCommandResult CWII_IPC_HLE_Device_usb_oh0_device::Close(u32 command_address, bool force)
{
  if (!force)
    Memory::Write_U32(0, command_address + 4);
  m_Active = false;
...
IPCCommandResult CWII_IPC_HLE_Device_usb_oh0_device::IOCtl(u32 command_address)
{
  return m_oh0->DeviceIOCtl(m_vid, m_pid, command_address);
}

IPCCommandResult CWII_IPC_HLE_Device_usb_oh0_device::IOCtlV(u32 command_address)
{
