...
private:
  ...
  std::map<DeviceInfo, u32> m_insertion_hooks;
  std::map<DeviceInfo, u32> m_removal_hooks;
  ...
  {
    return CWII_IPC_HLE_Device_usb_host::GetUSBDeviceId(device);
  }
  void OnDeviceChange(ChangeEvent event, std::shared_ptr<Device> device) override;
