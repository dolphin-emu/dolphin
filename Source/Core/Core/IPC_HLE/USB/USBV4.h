...
struct USBV4Request
{
  USBV4Request(const IOCtlBuffer& cmd_buffer);
  u32 cmd_address;
  s32 device_id;
};

struct USBV4CtrlMessage : CtrlMessage
{
  USBV4CtrlMessage(const IOCtlBuffer& cmd_buffer);
};

struct USBV4GetUSStringMessage : CtrlMessage
{
  USBV4GetUSStringMessage(const IOCtlBuffer& cmd_buffer);
};

struct USBV4IntrMessage : IntrMessage
{
  USBV4IntrMessage(const IOCtlBuffer& cmd_buffer);
};
