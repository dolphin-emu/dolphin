...
struct USBV0CtrlMessage final : CtrlMessage
{
  explicit USBV0CtrlMessage(const SIOCtlVBuffer& cmd_buffer);
};

struct USBV0BulkMessage final : BulkMessage
{
  USBV0BulkMessage() = default;
  explicit USBV0BulkMessage(const SIOCtlVBuffer& cmd_buffer);
};

struct USBV0IntrMessage final : IntrMessage
{
  USBV0IntrMessage() = default;
  explicit USBV0IntrMessage(const SIOCtlVBuffer& cmd_buffer);
};

struct USBV0IsoMessage final : IsoMessage
{
  explicit USBV0IsoMessage(const SIOCtlVBuffer& cmd_buffer);
};
