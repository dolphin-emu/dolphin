...
IPCCommandResult CWII_IPC_HLE_Device_sdio_slot0::IOCtl(u32 _CommandAddress)
{
  ...
  case IOCTL_WRITEHCR:
  {
    ...
    if (reg >= 0x200)
    {
      INFO_LOG(WII_IPC_SD, "IOCTL_WRITEHCR out of range");
      break;
    }
  ...
  case IOCTL_READHCR:
  {
    ...
    if (reg >= 0x200)
    {
      INFO_LOG(WII_IPC_SD, "IOCTL_READHCR out of range");
      break;
    }
                                                   ...
                                                   u32 _BufferOutSize)
{
  ...
  case READ_MULTIPLE_BLOCK:
  {
    ...
    if (m_Card)
    {
      ...
      {
        INFO_LOG(WII_IPC_SD, "Outbuffer size %i got %i", _rwBufferSize, size);
      }
      else
      {
