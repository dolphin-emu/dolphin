...
CWII_IPC_HLE_Device_sdio_slot0::CWII_IPC_HLE_Device_sdio_slot0(u32 DeviceID, const std::string& rDeviceName)
...
bool CWII_IPC_HLE_Device_sdio_slot0::Open(u32 CommandAddress, u32 Mode)
{
	INFO_LOG(WII_IPC_SD, "Open");

	OpenInternal();

	Memory::Write_U32(GetDeviceID(), CommandAddress + 0x4);
	memset(m_Registers, 0, sizeof(m_Registers));
	m_Active = true;
	return true;
}

bool CWII_IPC_HLE_Device_sdio_slot0::Close(u32 CommandAddress, bool bForce)
{
...
// The front SD slot
bool CWII_IPC_HLE_Device_sdio_slot0::IOCtl(u32 CommandAddress)
{
	u32 Cmd = Memory::Read_U32(CommandAddress + 0xC);
	...
	case IOCTL_SENDCMD:
		INFO_LOG(WII_IPC_SD, "IOCTL_SENDCMD %x IPC:%08x",
			Memory::Read_U32(BufferIn), CommandAddress);
...
bool CWII_IPC_HLE_Device_sdio_slot0::IOCtlV(u32 CommandAddress)
{
...
u32 CWII_IPC_HLE_Device_sdio_slot0::ExecuteCommand(u32 BufferIn, u32 BufferInSize,
                                                   u32 rwBuffer, u32 rwBufferSize,
                                                   u32 BufferOut, u32 BufferOutSize)
{
	...
	req.command = Memory::Read_U32(BufferIn + 0);
	req.type    = Memory::Read_U32(BufferIn + 4);
	req.resp    = Memory::Read_U32(BufferIn + 8);
	req.arg     = Memory::Read_U32(BufferIn + 12);
	req.blocks  = Memory::Read_U32(BufferIn + 16);
	req.bsize   = Memory::Read_U32(BufferIn + 20);
	req.addr    = Memory::Read_U32(BufferIn + 24);
	req.isDMA   = Memory::Read_U32(BufferIn + 28);
	req.pad0    = Memory::Read_U32(BufferIn + 32);

	// Note: req.addr is the virtual address of rwBuffer
	...
	case SEND_RELATIVE_ADDR:
		...
		Memory::Write_U32(0x9f62, BufferOut);
		break;
	...
	case SELECT_CARD:
		...
		Memory::Write_U32((req.arg>>16) ? 0x700 : 0x900, BufferOut);
		break;
	...
	case SEND_IF_COND:
		...
		Memory::Write_U32(req.arg, BufferOut);
		break;
	...
	case SEND_CSD:
		...
		Memory::Write_U32(0x80168000, BufferOut);
		Memory::Write_U32(0xa9ffffff, BufferOut + 4);
		Memory::Write_U32(0x325b5a83, BufferOut + 8);
		Memory::Write_U32(0x00002e00, BufferOut + 12);
		break;

	case ALL_SEND_CID:
	case SEND_CID:
		DEBUG_LOG(WII_IPC_SD, "(ALL_)SEND_CID");
		Memory::Write_U32(0x80114d1c, BufferOut);
		Memory::Write_U32(0x80080000, BufferOut + 4);
		Memory::Write_U32(0x8007b520, BufferOut + 8);
		Memory::Write_U32(0x80080000, BufferOut + 12);
		break;

	case SET_BLOCKLEN:
		m_BlockLength = req.arg;
		Memory::Write_U32(0x900, BufferOut);
		break;

	case APP_CMD_NEXT:
		// Next cmd is going to be ACMD_*
		Memory::Write_U32(0x920, BufferOut);
		break;
	...
	case READ_MULTIPLE_BLOCK:
		...
		if (m_Card)
		{
			...
			if (m_Card.ReadBytes(buffer, req.bsize * req.blocks))
			{
				...
				DEBUG_LOG(WII_IPC_SD, "Outbuffer size %i got %i", rwBufferSize, i);
			}
			else
			{
