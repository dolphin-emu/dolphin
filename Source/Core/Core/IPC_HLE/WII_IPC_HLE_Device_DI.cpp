...
CWII_IPC_HLE_Device_di::CWII_IPC_HLE_Device_di(u32 DeviceID, const std::string& rDeviceName )
...
bool CWII_IPC_HLE_Device_di::Open(u32 CommandAddress, u32 Mode)
{
...
bool CWII_IPC_HLE_Device_di::Close(u32 CommandAddress, bool bForce)
{
	if (m_pFileSystem)
	{
		delete m_pFileSystem;
...
bool CWII_IPC_HLE_Device_di::IOCtl(u32 CommandAddress)
{
...
bool CWII_IPC_HLE_Device_di::IOCtlV(u32 CommandAddress)
{
...
u32 CWII_IPC_HLE_Device_di::ExecuteCommand(u32 BufferIn, u32 BufferInSize, u32 BufferOut, u32 BufferOutSize)
{
	u32 Command = Memory::Read_U32(BufferIn) >> 24;
	...
	case DVDLowInquiry:
		{
			...
			Memory::Write_U32(0x00000002, BufferOut);
			Memory::Write_U32(0x20060526, BufferOut + 4);
			...
			Memory::Write_U32(0x41000000, BufferOut + 8);

			INFO_LOG(WII_IPC_DVD, "DVDLowInquiry (Buffer 0x%08x, 0x%x)",
				BufferOut, BufferOutSize);
		}
		break;

	case DVDLowReadDiskID:
		{
			VolumeHandler::RAWReadToPtr(Memory::GetPointer(BufferOut), 0, BufferOutSize);

			INFO_LOG(WII_IPC_DVD, "DVDLowReadDiskID %s",
				ArrayToString(Memory::GetPointer(BufferOut), BufferOutSize, BufferOutSize).c_str());
		}
		break;
	...
	case DVDLowRead:
		{
			if (BufferOut == 0)
			{
				PanicAlert("DVDLowRead : BufferOut == 0");
				return 0;
			}
			u32 Size = Memory::Read_U32(BufferIn + 0x04);
			u64 DVDAddress = (u64)Memory::Read_U32(BufferIn + 0x08) << 2;
	...
	case DVDLowGetCoverReg:
		Memory::Write_U32(m_CoverStatus, BufferOut);

		INFO_LOG(WII_IPC_DVD, "DVDLowGetCoverReg 0x%08x", Memory::Read_U32(BufferOut));
		break;
	...
	case DVDLowGetCoverStatus:
		Memory::Write_U32(IsDiscInside() ? 2 : 1, BufferOut);
	...
	case DVDLowUnencryptedRead:
		{
			...
			u32 DVDAddress32 = Memory::Read_U32(BufferIn + 0x08);
			...
			if (!VolumeHandler::RAWReadToPtr(Memory::GetPointer(BufferOut), DVDAddress, Size))
			{
	...
	case DVDLowSeek:
		{
			u64 DVDAddress = Memory::Read_U32(BufferIn + 0x4) << 2;
			if (m_pFileSystem)
			{
				const std::string filename = m_pFileSystem->GetFileName(DVDAddress);
	...
	case DVDLowReadDiskBca:
		...
		Memory::Write_U32(1, BufferOut + 0x30);
		break;
	...
	case DVDLowRequestError:
		...
		Memory::Write_U32(m_ErrorStatus, BufferOut);
...
// Ex commands are immediate and respond with 4 bytes
	...
	case 0x96:
		WARN_LOG(WII_IPC_DVD, "Unimplemented command 0x%08x (Buffer 0x%08x, 0x%x)",
			Command, BufferOut, BufferOutSize);
		break;
...
int CWII_IPC_HLE_Device_di::GetCmdDelay(u32 CommandAddress)
{
	u32 BufferIn = Memory::Read_U32(CommandAddress + 0x10);
	u32 Command  = Memory::Read_U32(BufferIn) >> 24;
