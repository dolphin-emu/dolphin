...
bool BootCore(const std::string& _rFilename)
{
  ...
  // Apply overrides
  // Some NTSC GameCube games such as Baten Kaitos react strangely to language
  // settings that would be invalid on an NTSC system
  if (!StartUp.bOverrideGCLanguage && StartUp.bNTSC)
    StartUp.SelectedLanguage = 0;

  // Some NTSC Wii games such as Doc Louis's Punch-Out!! and 1942 (Virtual
  // Console) crash if the PAL60 option is enabled
