...
MMU::MMU(Core::System& system, Memory::MemoryManager& memory, PowerPC::PowerPCManager& power_pc)
    ...
    , m_memory(memory)
    , m_power_pc(power_pc)
    , m_ppc_state(power_pc.GetPPCState())
{
}
...
T MMU::ReadFromHardware(u32 em_address)
{
  ...
  if (m_memory.GetFakeVMEM() && ((em_address & 0xFE000000) == 0x7E000000))
  {
    T value;
    std::memcpy(
        &value, &m_memory.GetFakeVMEM()[em_address & m_memory.GetFakeVMemMask()], sizeof(T));
    return bswap(value);
  }
...
void MMU::WriteToHardware(u32 em_address, const u32 data, const u32 size)
{
      ...
      (em_address & 0x0FFFFFFF) < m_memory.GetExRamSizeReal())
  {
    ...
    if (m_ppc_state.m_enable_dcache && !wi)
    {
      m_ppc_state.dCache.Write(
          m_memory, em_address + 0x10000000, &swapped_data, size, HID0(m_ppc_state).DLOCK);
    }
  ...
  {
    std::memcpy(
        &m_memory.GetFakeVMEM()[em_address & m_memory.GetFakeVMemMask()], &swapped_data, size);
    return;
  }
...
std::optional<ReadResult<u32>> MMU::HostTryReadInstruction(
...
template <typename T>
std::optional<ReadResult<T>> MMU::HostTryReadUX(
    const Core::CPUThreadGuard& guard, const u32 address, RequestedAddressSpace space)
{
...
std::optional<ReadResult<u8>> MMU::HostTryReadU8(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
  return HostTryReadUX<u8>(guard, address, space);
}

std::optional<ReadResult<u16>> MMU::HostTryReadU16(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
  return HostTryReadUX<u16>(guard, address, space);
}

std::optional<ReadResult<u32>> MMU::HostTryReadU32(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
  return HostTryReadUX<u32>(guard, address, space);
}

std::optional<ReadResult<u64>> MMU::HostTryReadU64(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
  return HostTryReadUX<u64>(guard, address, space);
}

std::optional<ReadResult<float>> MMU::HostTryReadF32(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
...
std::optional<ReadResult<double>> MMU::HostTryReadF64(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
...
std::optional<ReadResult<std::string>> MMU::HostTryReadString(
    const Core::CPUThreadGuard& guard, u32 address, size_t size, RequestedAddressSpace space)
{
...
bool MMU::HostIsInstructionRAMAddress(
    const Core::CPUThreadGuard& guard, u32 address, RequestedAddressSpace space)
{
