...
template <std::unsigned_integral T>
void MMU::Write(const make_atleast_u32<T> var, const u32 address)
{
...
template <std::unsigned_integral T>
void MMU::HostWrite(const Core::CPUThreadGuard& guard, const make_atleast_u32<T> var,
                    const u32 address)
{
  auto& mmu = guard.GetSystem().GetMMU();
...
std::optional<WriteResult> MMU::HostTryWrite(const Core::CPUThreadGuard& guard,
                                             const make_atleast_u32<T> var, const u32 address,
                                             RequestedAddressSpace space)
{
