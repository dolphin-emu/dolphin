...
void MMU::PageTableUpdated(std::span<const u8> page_table)
{
  ...
  const auto get_page_index = [this](UPTE_Lo pte1, u32 hash) -> std::optional<EffectiveAddress> {
    ...
    // If the hash mask is large enough that one or more specified in pte1.API can also be obtained
    // from page_index_from_hash, check that those bits match.
    const u32 api_mask =
        (((m_ppc_state.pagetable_hashmask << 6) & ~m_ppc_state.pagetable_base) >> 16) & 0x3f;
  ...
  const auto fixup_shadowed_mappings = [this, old_page_table, new_page_table](
    ...
    while (true)
    {
      ...
      else
      {
        ...
        pte1.H = 1;
        page_table_offset =
            ((~pteg_start & (m_ppc_state.pagetable_hashmask << 6)) | m_ppc_state.pagetable_base) -
  ...
  {
    if ((i & (m_ppc_state.pagetable_hashmask << 6)) != i || (i & m_ppc_state.pagetable_base) != 0)
      continue;
