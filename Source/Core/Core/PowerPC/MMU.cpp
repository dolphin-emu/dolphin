...
T MMU::ReadFromHardware(const u32 effective_address)
{
  ...
  if (m_memory.GetRAM() && (physical_address & 0xF8000000) == 0x00000000)
  {
    ...
    {
      m_ppc_state.dCache.Read(physical_address, &value, sizeof(T),
      ...
      (physical_address & 0x0FFFFFFF) < m_memory.GetExRamSizeReal())
  {
    ...
    {
      m_ppc_state.dCache.Read(physical_address + 0x10000000, &value, sizeof(T),
                              HID0(m_ppc_state).DLOCK || flag != XCheckTLBFlag::Read);
    }
...
void MMU::WriteToHardware(const u32 effective_address, const u32 data, const u32 size)
{
  ...
  if (m_memory.GetRAM() && (physical_address & 0xF8000000) == 0x00000000)
  {
    ...
    if (m_ppc_state.m_enable_dcache && !wi)
      m_ppc_state.dCache.Write(physical_address, &swapped_data, size, HID0(m_ppc_state).DLOCK);
      ...
      (physical_address & 0x0FFFFFFF) < m_memory.GetExRamSizeReal())
  {
    ...
    if (m_ppc_state.m_enable_dcache && !wi)
    {
      m_ppc_state.dCache.Write(physical_address + 0x10000000, &swapped_data, size,
