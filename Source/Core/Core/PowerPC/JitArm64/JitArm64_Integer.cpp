...
void JitArm64::reg_imm(u32 d, u32 a, u32 value, Operation do_op,
                       void (ARM64XEmitter::*op)(ARM64Reg, ARM64Reg, ARM64Reg,
                                                 ArithOption),
                       bool Rc) {
  if (gpr.IsImm(a)) {
...
void JitArm64::extsXx(UGeckoInstruction inst) {
  ...
  if (gpr.IsImm(s)) {
    gpr.SetImmediate(
        a, (u32)(s32)(size == 16 ? (s16)gpr.GetImm(s) : (s8)gpr.GetImm(s)));
    if (inst.Rc)
      ComputeRC(gpr.GetImm(a), 0);
  } else {
    gpr.BindToRegister(a, a == s);
...
void JitArm64::slwx(UGeckoInstruction inst) {
  ...
  if (gpr.IsImm(b) && gpr.IsImm(s)) {
    ...
    // AArch64 it will use a mask of the register size for determining what
    // shift amount
    // So if we use a 64bit so the bits will end up in the high 32bits, and
    // Later instructions will just eat high 32bits since it'll run 32bit
    // operations for everything.
    LSLV(EncodeRegTo64(gpr.R(a)), EncodeRegTo64(gpr.R(s)),
         EncodeRegTo64(gpr.R(b)));
...
void JitArm64::srwx(UGeckoInstruction inst) {
  ...
  if (gpr.IsImm(b) && gpr.IsImm(s)) {
    ...
    // wipe upper bits. TODO: get rid of it, but then no instruction is allowed
    // to emit some higher
    // bits.
    MOV(gpr.R(s), gpr.R(s));

    LSRV(EncodeRegTo64(gpr.R(a)), EncodeRegTo64(gpr.R(s)),
         EncodeRegTo64(gpr.R(b)));
