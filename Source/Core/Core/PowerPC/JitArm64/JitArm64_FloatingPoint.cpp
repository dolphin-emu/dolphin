...
void JitArm64::Force25BitPrecision(ARM64Reg output, ARM64Reg input, ARM64Reg temp)
{
  ...
  auto [constant1, constant2] = fpr.GetTwoConstants(
      0x0000'0000'0800'0000ULL, true, IsQuad(input), true,
      [](ARM64Reg reg, ARM64FloatEmitter* float_emit) {
        float_emit->MOVI(64, reg, 0x0000'0000'FF00'0000ULL);
        float_emit->BIC(32, reg, 0xF7, 24);
      },
      0xFFFF'FFFF'F800'0000ULL, true, IsQuad(input), true,
      [](ARM64Reg reg, ARM64FloatEmitter* float_emit) {
        float_emit->MOVI(64, reg, 0xFFFF'FFFF'0000'0000ULL);
        float_emit->ORR(32, reg, 0xF8, 24);
      },
      [](ARM64Reg reg_1, ARM64Reg reg_2, ARM64FloatEmitter* float_emit) {
        float_emit->MOVI(64, reg_1, 0x0000'0000'FF00'0000ULL);
        float_emit->MOVI(64, reg_2, 0xFFFF'FFFF'0000'0000ULL);
        float_emit->BIC(32, reg_1, 0xF7, 24);
        float_emit->ORR(32, reg_2, 0xF8, 24);
      });
