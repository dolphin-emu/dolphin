...
static const ARM64Reg DISPATCHER_PC =
    W26; // register for PC when calling the dispatcher

#define PPCSTATE_OFF(elem) (offsetof(PowerPC::PowerPCState, elem))

// Some asserts to make sure we will be able to load everything
static_assert(PPCSTATE_OFF(spr[1023]) <= 16380,
              "LDR(32bit) can't reach the last SPR");
...
enum RegType {
  ...
  REG_DUP, // The lower reg is the same as the upper one (physical upper doesn't
           // actually have the
           // duplicated value)
...
public:
  OpArg()
      : m_type(REG_NOTLOADED), m_reg(INVALID_REG), m_value(0), m_last_used(0) {}
...
public:
  Arm64RegCache()
      : m_emit(nullptr), m_float_emit(nullptr), m_reg_stats(nullptr){};
  ...
  template <typename T = ARM64Reg, typename... Args> void Lock(Args... args) {
    for (T reg : {args...}) {
      FlushByHost(reg);
      LockRegister(reg);
    }
  ...
  template <typename T = ARM64Reg, typename... Args> void Unlock(Args... args) {
    for (T reg : {args...}) {
      FlushByHost(reg);
      UnlockRegister(reg);
    }
...
public:
  ...
  bool IsImm(u32 reg) const {
    return m_guest_registers[reg].GetType() == REG_IMM;
  }
