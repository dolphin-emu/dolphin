...
void JitArm64::lfXX(UGeckoInstruction inst) {
  ...
  RegType type = !!(flags & BackPatchInfo::FLAG_SIZE_F64) ? REG_LOWER_PAIR
                                                          : REG_DUP_SINGLE;
  ...
  if (is_immediate && PowerPC::IsOptimizableRAMAddress(imm_addr)) {
    ...
    EmitBackpatchRoutine(flags, jo.fastmem, jo.fastmem, VD, XA, regs_in_use,
                         fprs_in_use);
  }
...
void JitArm64::stfXX(UGeckoInstruction inst) {
  ...
  bool single =
      (flags & BackPatchInfo::FLAG_SIZE_F32) && fpr.IsSingle(inst.FS, true);
  ...
  if (is_immediate &&
      !(jit->jo.optimizeGatherPipe &&
        PowerPC::IsOptimizableGatherPipeWrite(imm_addr))) {
    MOVI2R(XA, imm_addr);
  ...
  if (is_immediate) {
    if (jit->jo.optimizeGatherPipe &&
      ...
      u64 base_ptr =
          std::min((u64)&GPFifo::m_gatherPipeCount, (u64)&GPFifo::m_gatherPipe);
      ...
      if (pipe_off) {
        m_float_emit.STR(accessSize, accessSize == 64 ? Q0 : D0, X1,
                         ArithOption(X0));
      } else {
        m_float_emit.STR(accessSize, accessSize == 64 ? Q0 : D0, X30,
                         ArithOption(X0));
      }
      ...
      EmitBackpatchRoutine(flags, true, false, V0, XA, BitSet32(0),
                           BitSet32(0));
    } else {
      EmitBackpatchRoutine(flags, false, false, V0, XA, regs_in_use,
                           fprs_in_use);
    }
  } else {
    EmitBackpatchRoutine(flags, jo.fastmem, jo.fastmem, V0, XA, regs_in_use,
                         fprs_in_use);
  }
