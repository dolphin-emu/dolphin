...
static void DoBacktrace(uintptr_t access_address, SContext *ctx) {
  for (int i = 0; i < 30; i += 2)
    ERROR_LOG(DYNA_REC, "R%d: 0x%016llx\tR%d: 0x%016llx", i, ctx->CTX_REG(i),
              i + 1, ctx->CTX_REG(i + 1));

  ERROR_LOG(DYNA_REC, "R30: 0x%016llx\tSP: 0x%016llx", ctx->CTX_REG(30),
            ctx->CTX_SP);
  ...
  for (u64 pc = (ctx->CTX_PC - 32); pc < (ctx->CTX_PC + 32); pc += 16) {
    pc_memory += StringFromFormat(
        "%08x%08x%08x%08x", Common::swap32(*(u32 *)pc),
        Common::swap32(*(u32 *)(pc + 4)), Common::swap32(*(u32 *)(pc + 8)),
        Common::swap32(*(u32 *)(pc + 12)));

    ERROR_LOG(DYNA_REC, "0x%016lx: %08x %08x %08x %08x", pc, *(u32 *)pc,
              *(u32 *)(pc + 4), *(u32 *)(pc + 8), *(u32 *)(pc + 12));
  }
...
bool JitArm64::HandleFault(uintptr_t access_address, SContext *ctx) {
  if (!IsInSpace((u8 *)ctx->CTX_PC)) {
    ERROR_LOG(DYNA_REC,
              "Backpatch location not within codespace 0x%016llx(0x%08x)",
              ctx->CTX_PC, Common::swap32(*(u32 *)ctx->CTX_PC));
  ...
  if (!(access_address >= (uintptr_t)Memory::physical_base &&
    ...
    ERROR_LOG(DYNA_REC, "Exception handler - access below memory space. PC: "
                        "0x%016llx 0x%016lx < 0x%016lx",
  ...
  auto slow_handler_iter =
      m_fault_to_handler.upper_bound((const u8 *)ctx->CTX_PC);
  slow_handler_iter--;
  ...
  if ((const u8 *)ctx->CTX_PC - slow_handler_iter->first >
      slow_handler_iter->second.length)
    return false;
