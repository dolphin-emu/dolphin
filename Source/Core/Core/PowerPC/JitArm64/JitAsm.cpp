...
void JitArm64::GenerateAsm()
{
  ...
  if (assembly_dispatcher)
  {
    ...
    MOVI2R(block, (u64)jit->GetBlockCache()->GetBlocks());
  ...
  MOVI2R(X0, (u64)GetBlockCache());
  MOV(X1, DISPATCHER_PC);
  LDR(INDEX_UNSIGNED, X2, PPC_REG, PPCSTATE_OFF(msr));
  MOVI2R(X30, (u64)(void*)&JitBaseBlockCache::Dispatch);
  BLR(X30);
