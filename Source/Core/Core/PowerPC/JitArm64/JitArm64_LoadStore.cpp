...
void JitArm64::SafeLoadToReg(u32 dest, s32 addr, s32 offsetReg, u32 flags,
  ...
  if (is_immediate && PowerPC::IsOptimizableRAMAddress(imm_addr)) {
    EmitBackpatchRoutine(flags, true, false, dest_reg, XA, BitSet32(0),
                         BitSet32(0));
  } else if (mmio_address) {
    MMIOLoadToReg(Memory::mmio_mapping.get(), this, regs_in_use, fprs_in_use,
                  dest_reg, mmio_address, flags);
  } else {
    EmitBackpatchRoutine(flags, jo.fastmem, jo.fastmem, dest_reg, XA,
...
void JitArm64::SafeStoreFromReg(s32 dest, u32 value, s32 regOffset, u32 flags,
                                s32 offset) {
  // We want to make sure to not get LR as a temp register
  gpr.Lock(W0, W1, W30);
  ...
  if (is_immediate && jo.optimizeGatherPipe &&
    ...
    MMIOWriteRegToAddr(Memory::mmio_mapping.get(), this, regs_in_use,
                       fprs_in_use, RS, mmio_address, flags);
  } else {
    if (is_immediate)
      MOVI2R(XA, imm_addr);

    EmitBackpatchRoutine(flags, jo.fastmem, jo.fastmem, RS, XA, regs_in_use,
                         fprs_in_use);
  }
...
void JitArm64::lmw(UGeckoInstruction inst) {
  ...
  for (int i = inst.RD; i < 32; i++) {
    ...
    if (remaining >= 4) {
      ...
      ORR(EncodeRegTo64(RX2), ZR, EncodeRegTo64(RX1),
          ArithOption(EncodeRegTo64(RX1), ST_LSR, 32));
      ORR(EncodeRegTo64(RX4), ZR, EncodeRegTo64(RX3),
          ArithOption(EncodeRegTo64(RX3), ST_LSR, 32));
      i += 3;
    } else if (remaining >= 2) {
      gpr.BindToRegister(i + 1, false);
      gpr.BindToRegister(i, false);
...
void JitArm64::dcbx(UGeckoInstruction inst) {
  ...
  AND(value, addr, 32 - 10,
      28 - 10); // upper three bits and last 10 bit are masked for the bitset
                // of cachelines, 0x1ffffc00
  LSR(value, value,
      5 + 5); // >> 5 for cache line size, >> 5 for width of bitset
  ...
  LSR(addr, addr,
      5); // mask sizeof cacheline, & 0x1f is the position within the bitset
...
void JitArm64::dcbz(UGeckoInstruction inst) {
  ...
  EmitBackpatchRoutine(BackPatchInfo::FLAG_ZERO_256, true, true, W0,
