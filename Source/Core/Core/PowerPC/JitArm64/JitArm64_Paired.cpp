...
void JitArm64::ps_arith(UGeckoInstruction inst)
{
  ...
  const bool error_free_transformation = fma && !singles && m_accurate_fmadds;
...

  {
    ...
    if (error_free_transformation)
    {
      // Earlier we calculated s := a + b (with a = VA * rounded_c_reg, b = negate_b ? -VB : VB)
    ...
    if (m_accurate_nans)
    {
      const ARM64Reg nan_temp_reg = singles ? EncodeRegToSingle(V1Q) : EncodeRegToDouble(V1Q);
