...
//#define NORMALBRANCH_START FallBackToInterpreter(inst);
// ibuild.EmitInterpreterBranch(); return;
...
void JitILBase::rfi(UGeckoInstruction inst) { ibuild.EmitRFIExit(); }
...
static IREmitter::InstLoc EmitCRTest(IREmitter::IRBuilder &ibuild,
...
static IREmitter::InstLoc TestBranch(IREmitter::IRBuilder &ibuild,
                                     UGeckoInstruction inst) {
...
void JitILBase::bcx(UGeckoInstruction inst) {
  ...
  // Idle skipping:
  // The main Idle skipping is done in the LoadStore code, but there is an
  // optimization here.
  // If idle skipping is enabled, then this branch will only be reached when the
  // branch is not
  // taken.
