...
public:
  ...
  template <std::unsigned_integral T>
  static void HostWrite(const Core::CPUThreadGuard& guard, make_atleast_u32<T> var, u32 address);
  template <typename T>
  ...
  HostTryWrite(const Core::CPUThreadGuard& guard, const make_atleast_u32<T> var, const u32 address,
               RequestedAddressSpace space = RequestedAddressSpace::Effective);
  template <typename T>
  static std::optional<WriteResult>
  ...
  template <std::unsigned_integral T>
  void Write(const make_atleast_u32<T> var, const u32 address);
