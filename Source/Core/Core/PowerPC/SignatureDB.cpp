  ...
  for (const auto &entry : database) {
    ...
    if (function) {
      ...
      if (entry.second.size == (unsigned int)function->size) {
        ...
        INFO_LOG(OSHLE, "Found %s at %08x (size: %08x)!",
                 entry.second.name.c_str(), function->address, function->size);
      } else {
        function->name = entry.second.name;
        ERROR_LOG(OSHLE,
                  "Wrong size! Found %s at %08x (size: %08x instead of %08x)!",
                  entry.second.name.c_str(), function->address, function->size,
                  entry.second.size);
      }
    }
...
void SignatureDB::Initialize(PPCSymbolDB *symbol_db,
  ...
  for (const auto &symbol : symbol_db->Symbols()) {
    if ((prefix.empty() && (!symbol.second.name.empty()) &&
         symbol.second.name.substr(0, 3) != "zz_" &&
         symbol.second.name.substr(0, 1) != ".") ||
        ((!prefix.empty()) &&
         symbol.second.name.substr(0, prefix.size()) == prefix)) {
      DBFunc temp_dbfunc;
...
/*static*/ u32 SignatureDB::ComputeCodeChecksum(u32 offsetStart,
                                                u32 offsetEnd) {
  u32 sum = 0;
