...
CPUCoreBase *GetCore() { return jit; }
...
void WriteProfileResults(const std::string &filename) {
  ...
  fprintf(f.GetHandle(), "origAddr\tblkName\trunCount\tcost\ttimeCost\tpercent"
                         "\ttimePercent\tOvAlli"
  ...
  for (auto &stat : prof_stats.block_stats) {
    ...
    double timePercent =
        100.0 * (double)stat.tick_counter / (double)prof_stats.timecost_sum;
    fprintf(f.GetHandle(), "%08x\t%s\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                           "\t%.2f\t%.2f\t%.2f\t%i\n",
            stat.addr, name.c_str(), stat.run_count, stat.cost,
            stat.tick_counter, percent, timePercent,
            (double)stat.tick_counter * 1000.0 /
                (double)prof_stats.countsPerSec,
            stat.block_size);
  }
}
...
void GetProfileResults(ProfileStats *prof_stats) {
  ...
  for (int i = 0; i < jit->GetBlockCache()->GetNumBlocks(); i++) {
    ...
    // Todo: tweak.
    if (block->runCount >= 1)
      prof_stats->block_stats.emplace_back(i, block->originalAddress, cost,
                                           timecost, block->runCount,
                                           block->codeSize);
...
int GetHostCode(u32 *address, const u8 **code, u32 *code_size) {
  ...
  int block_num =
      jit->GetBlockCache()->GetBlockNumberFromStartAddress(*address);
  if (block_num < 0) {
    for (int i = 0; i < 500; i++) {
      block_num = jit->GetBlockCache()->GetBlockNumberFromStartAddress(
          *address - 4 * i);
      if (block_num >= 0)
        break;
    }
...
void CompileExceptionCheck(ExceptionType type) {
  ...
  if (PC != 0 &&
      (exception_addresses->find(PC)) == (exception_addresses->end())) {
    ...
    // Invalidate the JIT block so that it gets recompiled with the external
    // exception check
    // included.
