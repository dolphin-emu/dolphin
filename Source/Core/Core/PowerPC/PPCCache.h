...
struct Cache
{
  ...
  void Store(const Memory::MemoryManager& memory, u32 addr);
  void Invalidate(const Memory::MemoryManager& memory, u32 addr);
  void Flush(const Memory::MemoryManager& memory, u32 addr);
  void Touch(const Memory::MemoryManager& memory, u32 addr, bool store);

  void FlushAll(const Memory::MemoryManager& memory);

  std::pair<u32, u32> GetCache(const Memory::MemoryManager& memory, u32 addr, bool locked);

  void Read(const Memory::MemoryManager& memory, u32 addr, void* buffer, u32 len, bool locked);
  void Write(const Memory::MemoryManager& memory, u32 addr, const void* buffer, u32 len, bool locked);
...
struct InstructionCache : public Cache
{
  ...
  ~InstructionCache();
  u32 ReadInstruction(const Memory::MemoryManager& memory, PowerPCState& ppc_state, u32 addr);
  void Invalidate(const Memory::MemoryManager& memory, const JitInterface& jit_interface, u32 addr);
  void Init(const Memory::MemoryManager& memory);
  void Reset(const JitInterface& jit_interface);
  void RefreshConfig();
};
}  // namespace PowerPC
