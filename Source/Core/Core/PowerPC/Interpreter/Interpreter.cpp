...
static void Trace(UGeckoInstruction &instCode) {
  ...
  std::string fregs = "";
  for (int i = 0; i < 32; i++) {
    fregs += StringFromFormat("f%02d: %08" PRIx64 " %08" PRIx64 " ", i,
                              PowerPC::ppcState.ps[i][0],
                              PowerPC::ppcState.ps[i][1]);
  }

  std::string ppc_inst = GekkoDisassembler::Disassemble(instCode.hex, PC);
  DEBUG_LOG(POWERPC, "INTER PC: %08x SRR0: %08x SRR1: %08x CRval: %016lx "
                     "FPSCR: %08x MSR: %08x LR: "
                     "%08x %s %08x %s",
            PC, SRR0, SRR1, (unsigned long)PowerPC::ppcState.cr_val[0],
            PowerPC::ppcState.fpscr, PowerPC::ppcState.msr,
            PowerPC::ppcState.spr[8], regs.c_str(), instCode.hex,
            ppc_inst.c_str());
}
...
// FastRun - inspired by GCemu (to imitate the JIT so that they can be
// compared).
void Interpreter::Run() {
  while (!CPU::GetState()) {
    ...
    if (SConfig::GetInstance().bEnableDebugging) {
      ...
      // Debugging friendly version of inner loop. Tries to do the timing as
      // similarly to the
      // JIT as possible. Does not take into account that some instructions take
      // multiple cycles.
...
void Interpreter::unknown_instruction(UGeckoInstruction _inst) {
  std::string disasm =
      GekkoDisassembler::Disassemble(PowerPC::HostRead_U32(last_pc), last_pc);
  NOTICE_LOG(POWERPC, "Last PC = %08x : %s", last_pc, disasm.c_str());
  Dolphin_Debugger::PrintCallstack();
  NOTICE_LOG(POWERPC, "\nIntCPU: Unknown instruction %08x at PC = %08x  "
                      "last_PC = %08x  LR = %08x\n",
             _inst.hex, PC, last_pc, LR);
  for (int i = 0; i < 32; i += 4)
    NOTICE_LOG(POWERPC, "r%d: 0x%08x r%d: 0x%08x r%d:0x%08x r%d: 0x%08x", i,
               rGPR[i], i + 1, rGPR[i + 1], i + 2, rGPR[i + 2], i + 3,
               rGPR[i + 3]);
