...
static void FPSCRtoFPUSettings(UReg_FPSCR fp) {
  ...
  if (fp.VE || fp.OE || fp.UE || fp.ZE || fp.XE) {
    // PanicAlert("FPSCR - exceptions enabled. Please report. VE=%i OE=%i UE=%i
    // ZE=%i XE=%i",
...
void Interpreter::mfcr(UGeckoInstruction _inst) { rGPR[_inst.RD] = GetCR(); }
...
static void SetSR(int index, u32 value) {
  DEBUG_LOG(POWERPC, "%08x: MMU: Segment register %i set to %08x",
            PowerPC::ppcState.pc, index, value);
...
void Interpreter::mftb(UGeckoInstruction _inst) {
  ...
  _dbg_assert_msg_(POWERPC, (iIndex == SPR_TL) || (iIndex == SPR_TU),
                   "Invalid mftb");
  (void)iIndex;
  mfspr(_inst);
}
...
void Interpreter::mfspr(UGeckoInstruction _inst) {
  ...
  // XER LR CTR are the only ones available in user mode, time base can be read
  // too.
...
void Interpreter::mtspr(UGeckoInstruction _inst) {
  ...
  // Our DMA emulation is highly inaccurate - instead of properly emulating the
  // queue
  ...
  case SPR_HID0: // HID0
  {
    ...
    if (HID0.ICE != old_hid0.ICE) {
      INFO_LOG(POWERPC, "Instruction Cache Enable (HID0.ICE) = %d",
               (int)HID0.ICE);
    }
    if (HID0.ILOCK != old_hid0.ILOCK) {
      INFO_LOG(POWERPC, "Instruction Cache Lock (HID0.ILOCK) = %d",
               (int)HID0.ILOCK);
    }
    if (HID0.ICFI) {
      HID0.ICFI = 0;
  ...
  case SPR_WPAR:
    _assert_msg_(POWERPC, rGPR[_inst.RD] == 0x0C008000, "Gather pipe @ %08x",
                 PC);
  ...
  case SPR_DMAL:
    ...
    if (DMAL.DMA_T) {
      ...
      // INFO_LOG(POWERPC, "DMA: mem = %x, cache = %x, len = %u, LD = %d,
      // PC=%x", dwMemAddress,
...
void Interpreter::mcrfs(UGeckoInstruction _inst) {
  ...
  //	PanicAlert("msrfs at %x, CRFS = %d, CRFD = %d", PC, (int)_inst.CRFS,
  //(int)_inst.CRFD);
