...
void Interpreter::ps_nmsub(UGeckoInstruction inst)
{
  ...
  const float tmp0 = ForceSingle(FPSCR, NI_msub(FPSCR, a.PS0AsDouble(), c0, b.PS0AsDouble()).value);
  const float tmp1 = ForceSingle(FPSCR, NI_msub(FPSCR, a.PS1AsDouble(), c1, b.PS1AsDouble()).value);
...
void Interpreter::ps_nmadd(UGeckoInstruction inst)
{
  ...
  const float tmp0 = ForceSingle(FPSCR, NI_madd(FPSCR, a.PS0AsDouble(), c0, b.PS0AsDouble()).value);
  const float tmp1 = ForceSingle(FPSCR, NI_madd(FPSCR, a.PS1AsDouble(), c1, b.PS1AsDouble()).value);
