...
public:
	...
	typedef void (*interpreterInstruction)(UGeckoInstruction instCode);

	interpreterInstruction GetInstruction(UGeckoInstruction instCode);
	...
	static void CompiledBlock(UGeckoInstruction inst);
	...
	//static void fsqrtsx(UGeckoInstruction inst);
	...
	static void fsqrtx(UGeckoInstruction inst);
	...
	static void tlbia(UGeckoInstruction inst);
	...
	static void rfid(UGeckoInstruction inst);
	static void sync(UGeckoInstruction inst);
	static void isync(UGeckoInstruction inst);

	static interpreterInstruction m_opTable[64];
	static interpreterInstruction m_opTable4[1024];
	static interpreterInstruction m_opTable19[1024];
	static interpreterInstruction m_opTable31[1024];
	static interpreterInstruction m_opTable59[32];
	static interpreterInstruction m_opTable63[1024];

	// singleton
	static Interpreter* getInstance();

	static void RunTable4(UGeckoInstruction instCode);
	static void RunTable19(UGeckoInstruction instCode);
	static void RunTable31(UGeckoInstruction instCode);
	static void RunTable59(UGeckoInstruction instCode);
	static void RunTable63(UGeckoInstruction instCode);

	static u32 Helper_Carry(u32 uValue1, u32 uValue2);

private:
	// flag helper
	static void Helper_UpdateCR0(u32 uValue);
	static void Helper_UpdateCR1(double fValue);
	static void Helper_UpdateCR1(float fValue);
	static void Helper_UpdateCRx(int x, u32 uValue);
	...
	// paired helper
	static float Helper_Dequantize(const u32 Addr, const EQuantizeType quantizeType, const unsigned int uScale);
	static void  Helper_Quantize  (const u32 Addr, const double fValue, const EQuantizeType quantizeType, const unsigned uScale);
