...
void Interpreter::dcbi(UGeckoInstruction _inst) {
  // Removes a block from data cache. Since we don't emulate the data cache, we
  // don't need to do
  ...
  // The following detects a situation where the game is writing to the dcache
  // at the address being
  // DMA'd. As we do not
  // have dcache emulation, invalid data is being DMA'd causing audio glitches.
  // The following code
  // detects this and
...
void Interpreter::dcbst(UGeckoInstruction _inst) {
  // Cache line flush. Since we don't emulate the data cache, we don't need to
  // do anything.
  // Invalidate the jit block cache on dcbst in case new code has been loaded
  // via the data cache
  u32 address = Helper_Get_EA_X(_inst);
  JitInterface::InvalidateICache(address & ~0x1f, 32, false);
}
...
void Interpreter::eciwx(UGeckoInstruction _inst) {
  ...
  // 	_assert_msg_(POWERPC,0,"eciwx - fill r%i with word @ %08x from device
  // %02x",
...
void Interpreter::ecowx(UGeckoInstruction _inst) {
  ...
  // 	_assert_msg_(POWERPC,0,"ecowx - send stw request (%08x@%08x) to device
  // %02x",
...
void Interpreter::sthbrx(UGeckoInstruction _inst) {
  PowerPC::Write_U16(Common::swap16((u16)rGPR[_inst.RS]),
                     Helper_Get_EA_X(_inst));
}
