...
void Interpreter::lbz(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  const u32 temp = interpreter.m_mmu.Read<u8>(Helper_Get_EA(ppc_state, inst), inst);
...
void Interpreter::lbzu(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  const u32 temp = interpreter.m_mmu.Read<u8>(address, inst);
...
void Interpreter::lfd(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  const u64 temp = interpreter.m_mmu.Read<u64>(address, inst);
...
void Interpreter::lfs(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  const u32 temp = interpreter.m_mmu.Read<u32>(address, inst);
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp = u32(s32(s16(interpreter.m_mmu.Read<u16>(Helper_Get_EA(ppc_state, inst), inst))));
...
void Interpreter::lhau(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 address = Helper_Get_EA_U(ppc_state, inst);
  const u32 temp = u32(s32(s16(interpreter.m_mmu.Read<u16>(address, inst))));
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp = interpreter.m_mmu.Read<u16>(Helper_Get_EA(ppc_state, inst), inst);
...
void Interpreter::lhzu(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 address = Helper_Get_EA_U(ppc_state, inst);
  const u32 temp = interpreter.m_mmu.Read<u16>(address, inst);
...
void Interpreter::lmw(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  {
    const u32 temp_reg = interpreter.m_mmu.Read<u32>(address, inst);
...
void Interpreter::stmw(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  {
    interpreter.m_mmu.Write<u32>(ppc_state.gpr[i], address, inst);
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
    {
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  interpreter.m_mmu.Write<u8>(ppc_state.gpr[inst.RS], Helper_Get_EA(ppc_state, inst), inst);
}
...
void Interpreter::stbu(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u8>(ppc_state.gpr[inst.RS], address, inst);
  if (!(ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION))
  {
    ppc_state.gpr[inst.RA] = address;
  }
...
void Interpreter::stfd(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u64>(ppc_state.ps[inst.FS].PS0AsU64(), address, inst);
}
...
void Interpreter::stfs(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u32>(ConvertToSingle(ppc_state.ps[inst.FS].PS0AsU64()), address, inst);
}
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  interpreter.m_mmu.Write<u16>(ppc_state.gpr[inst.RS], Helper_Get_EA(ppc_state, inst), inst);
}
...
void Interpreter::sthu(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u16>(ppc_state.gpr[inst.RS], address, inst);
  if (!(ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION))
  {
    ppc_state.gpr[inst.RA] = address;
  }
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  interpreter.m_mmu.Write<u32>(ppc_state.gpr[inst.RS], Helper_Get_EA(ppc_state, inst), inst);
}
...
void Interpreter::stwu(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u32>(ppc_state.gpr[inst.RS], address, inst);
  if (!(ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION))
  {
    ppc_state.gpr[inst.RA] = address;
  }
...
void Interpreter::eciwx(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  ppc_state.gpr[inst.RD] = interpreter.m_mmu.Read<u32>(EA, inst);
}
...
void Interpreter::ecowx(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u32>(ppc_state.gpr[inst.RS], EA, inst);
}
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp = interpreter.m_mmu.Read<u8>(Helper_Get_EA_X(ppc_state, inst), inst);
...
void Interpreter::lhaux(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 address = Helper_Get_EA_UX(ppc_state, inst);
  const s32 temp = s32{s16(interpreter.m_mmu.Read<u16>(address, inst))};
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const s32 temp = s32{s16(interpreter.m_mmu.Read<u16>(Helper_Get_EA_X(ppc_state, inst), inst))};
...
void Interpreter::lhbrx(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp =
      Common::swap16(interpreter.m_mmu.Read<u16>(Helper_Get_EA_X(ppc_state, inst), inst));
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp = interpreter.m_mmu.Read<u16>(Helper_Get_EA_X(ppc_state, inst), inst);
...
void Interpreter::lswx(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  for (u32 n = 0; n < static_cast<u8>(ppc_state.xer_stringctrl); n++)
  {
    ...
    const u32 temp_value = interpreter.m_mmu.Read<u8>(EA, inst) << (24 - offset);
...
void Interpreter::lwbrx(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp =
      Common::swap32(interpreter.m_mmu.Read<u32>(Helper_Get_EA_X(ppc_state, inst), inst));
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  interpreter.m_mmu.Write<u8>(ppc_state.gpr[inst.RS], Helper_Get_EA_X(ppc_state, inst), inst);
}
...
void Interpreter::stfiwx(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  interpreter.m_mmu.Write<u32>(ppc_state.ps[inst.FS].PS0AsU32(), address, inst);
}
...
{
  auto& ppc_state = interpreter.m_ppc_state;
  interpreter.m_mmu.Write<u16>(ppc_state.gpr[inst.RS], Helper_Get_EA_X(ppc_state, inst), inst);
}
...
void Interpreter::lswi(Interpreter& interpreter, UGeckoInstruction inst)
{
  ...
  while (n > 0)
  {
    ...
    const u32 temp_value = interpreter.m_mmu.Read<u8>(EA, inst) << (24 - i);
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
    {
                                     ...
                                     u32 n, u32 r)
{
  ...
  if (misalignment_bytes != 0)
  {
    ...
    current_value = interpreter.m_mmu.Read<u32>(current_address, inst);
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
      return;
  ...
  if (n != 0)
  {
    ...
    current_value &= 0xFFFF'FFFF'0000'0000;
    current_value |= (interpreter.m_mmu.Read<u32>(current_address, inst) << (n * 8)) & 0xFFFF'FFFF;
    if ((ppc_state.Exceptions & ANY_LOADSTORE_EXCEPTION) != 0)
      return;
    interpreter.m_mmu.Write<u32>(static_cast<u32>(current_value >> (n * 8)), current_address, inst);
  }
}
