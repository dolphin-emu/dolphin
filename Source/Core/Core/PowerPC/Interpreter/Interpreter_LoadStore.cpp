...
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 temp = static_cast<u32>(static_cast<s32>(s16(interpreter.m_mmu.Read_U16(Helper_Get_EA(ppc_state, inst)))));
...
void Interpreter::lhau(Interpreter& interpreter, UGeckoInstruction inst)
{
  auto& ppc_state = interpreter.m_ppc_state;
  const u32 address = Helper_Get_EA_U(ppc_state, inst);
  const u32 temp = static_cast<u32>(static_cast<s32>(s16(interpreter.m_mmu.Read_U16(address))));
