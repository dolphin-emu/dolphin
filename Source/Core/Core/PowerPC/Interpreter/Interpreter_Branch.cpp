...
void Interpreter::bcx(UGeckoInstruction _inst) {
  ...
  bool condition =
      only_counter_check || (GetCRBit(_inst.BI) == u32(true_false));
...
void Interpreter::bcctrx(UGeckoInstruction _inst) {
  ...
  int condition =
      ((_inst.BO_2 >> 4) | (GetCRBit(_inst.BI_2) == ((_inst.BO_2 >> 3) & 1))) &
      1;
...
void Interpreter::rfi(UGeckoInstruction _inst) {
  ...
  // MSR[13] is set to 0.
  MSR &= 0xFFFBFFFF;
  // Here we should check if there are pending exceptions, and if their
...
// sc isn't really used for anything important in GameCube games (just for a
// write barrier) so we
