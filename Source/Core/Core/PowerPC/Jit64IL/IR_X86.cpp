...
static void DoWriteCode(IRBuilder* ibuild, JitIL* Jit, u32 exitAddress)
{
	...
	for (unsigned i = 0; i != RI.IInfo.size(); i++)
	{
		...
		case LoadSingle:
		{
			...
			RI.Jit->SafeLoadToReg(RSCRATCH2, info.first, 32, info.second, regsInUse(RI), false, 0, RSCRATCH);
		...
		case LoadDouble:
		{
			...
			RI.Jit->SafeLoadToReg(RSCRATCH2, info.first, 64, info.second, regsInUse(RI), false, 0, RSCRATCH);
