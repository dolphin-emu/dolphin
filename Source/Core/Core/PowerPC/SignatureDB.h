...
// You're not meant to keep around SignatureDB objects persistently. Use 'em,
// throw them away.
...
class SignatureDB {
  ...
  // Map from signature to function. We store the DB in this map because it
  // optimizes the
...
public:
  ...
  bool Load(const std::string &filename); // Does not clear. Remember to clear
