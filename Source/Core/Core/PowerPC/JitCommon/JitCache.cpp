...
JitBlock *JitBaseBlockCache::GetBlock(int no) { return &blocks[no]; }

int JitBaseBlockCache::GetNumBlocks() const { return num_blocks; }
...
void JitBaseBlockCache::FinalizeBlock(int block_num, bool block_link,
                                      const u8 *code_ptr) {
  ...
  for (u32 block = pAddr / 32; block <= (pAddr + (b.originalSize - 1) * 4) / 32;
       ++block)
...
void JitBaseBlockCache::DestroyBlock(int block_num, bool invalidate) {
  ...
  b.invalid = true;
  std::memcpy(GetICachePtr(b.originalAddress), &JIT_ICACHE_INVALID_WORD,
              sizeof(u32));
  ...
  // Spurious entrances from previously linked blocks can only come through
  // checkedEntry
...
void JitBaseBlockCache::InvalidateICache(u32 address, const u32 length,
  ...
  // destroy JIT blocks
  // !! this works correctly under assumption that any two overlapping blocks
  // end at the same
  // address
  if (destroy_block) {
    ...
    // If the code was actually modified, we need to clear the relevant entries
    // from the
    // FIFO write address cache, so we don't end up with FIFO checks in places
    // they shouldn't
    // be (this can clobber flags, and thus break any optimization that relies
    // on flags
