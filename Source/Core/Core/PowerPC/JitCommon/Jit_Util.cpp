...
void EmuCodeBlock::MemoryExceptionCheck() {
  if (jit->jo.memcheck && !jit->js.fastmemLoadStore &&
      !jit->js.fixupExceptionHandler) {
...
void EmuCodeBlock::UnsafeLoadRegToReg(X64Reg reg_addr, X64Reg reg_value,
...
void EmuCodeBlock::UnsafeLoadRegToRegNoSwap(X64Reg reg_addr, X64Reg reg_value,
                                            int accessSize, s32 offset,
                                            bool signExtend) {
  if (signExtend)
...
u8 *EmuCodeBlock::UnsafeLoadToReg(X64Reg reg_value, OpArg opAddress,
...
void EmuCodeBlock::MMIOLoadToReg(MMIO::Mapping *mmio, Gen::X64Reg reg_value,
  ...
  switch (access_size) {
  case 8: {
    MMIOReadCodeGenerator<u8> gen(this, registers_in_use, reg_value, address,
                                  sign_extend);
    mmio->GetHandlerForRead<u8>(address).Visit(gen);
    break;
  }
  case 16: {
    MMIOReadCodeGenerator<u16> gen(this, registers_in_use, reg_value, address,
                                   sign_extend);
    mmio->GetHandlerForRead<u16>(address).Visit(gen);
    break;
  }
  case 32: {
    MMIOReadCodeGenerator<u32> gen(this, registers_in_use, reg_value, address,
                                   sign_extend);
...
FixupBranch EmuCodeBlock::CheckIfSafeAddress(const OpArg &reg_value,
                                             X64Reg reg_addr,
  ...
  // Get ourselves a free register; try to pick one that doesn't involve
  // pushing, if we can.
  ...
  // On Gamecube games with MMU, do a little bit of extra work to make sure
  // we're not accessing the
  // 0x81800000 to 0x83FFFFFF range.
  // It's okay to take a shortcut and not check this range on non-MMU games,
  // since we're already
  // assuming they'll never do an invalid memory access.
  // The slightly more complex check needed for Wii games using the space just
  // above MEM1 isn't
  // implemented here yet, since there are no known working Wii MMU games to
  // test it with.
...
void EmuCodeBlock::SafeLoadToReg(X64Reg reg_value, const Gen::OpArg &opAddress,
                                 int accessSize, s32 offset,
  ...
  _assert_msg_(
      DYNA_REC, opAddress.IsSimpleReg(),
  ...
  if (!jit->jo.alwaysUseMemFuncs) {
    FixupBranch slow =
        CheckIfSafeAddress(R(reg_value), reg_addr, registersInUse, mem_mask);
...
u8 *EmuCodeBlock::UnsafeWriteRegToReg(OpArg reg_value, X64Reg reg_addr,
                                      int accessSize, s32 offset, bool swap) {
...
void EmuCodeBlock::UnsafeWriteGatherPipe(int accessSize) {
  ...
  // question - should we inline them instead? Pro: Lose a CALL   Con: Code
  // bloat
  switch (accessSize) {
  case 8:
...
bool EmuCodeBlock::WriteToConstAddress(int accessSize, OpArg arg, u32 address,
  ...
  if (jit->jo.optimizeGatherPipe &&
      PowerPC::IsOptimizableGatherPipeWrite(address)) {
...
void EmuCodeBlock::SafeWriteRegToReg(OpArg reg_value, X64Reg reg_addr,
                                     int accessSize, s32 offset,
  ...
  switch (accessSize) {
  case 64:
    ABI_CallFunctionRR(swap ? ((void *)&PowerPC::Write_U64)
                            : ((void *)&PowerPC::Write_U64_Swap),
                       reg, reg_addr);
    break;
  case 32:
    ABI_CallFunctionRR(swap ? ((void *)&PowerPC::Write_U32)
                            : ((void *)&PowerPC::Write_U32_Swap),
                       reg, reg_addr);
    break;
  case 16:
    ABI_CallFunctionRR(swap ? ((void *)&PowerPC::Write_U16)
                            : ((void *)&PowerPC::Write_U16_Swap),
                       reg, reg_addr);
    break;
  case 8:
...
void EmuCodeBlock::WriteToConstRamAddress(int accessSize, OpArg arg,
                                          u32 address, bool swap) {
  X64Reg reg;
  if (arg.IsImm()) {
  ...
  if (!arg.IsSimpleReg() ||
      (!cpu_info.bMOVBE && swap && arg.GetSimpleReg() != RSCRATCH)) {
    MOV(accessSize, R(RSCRATCH), arg);
    reg = RSCRATCH;
  } else {
...
void EmuCodeBlock::ForceSinglePrecision(X64Reg output, const OpArg &input,
                                        bool packed, bool duplicate) {
  // Most games don't need these. Zelda requires it though - some platforms get
  // stuck without them.
...
void EmuCodeBlock::avx_op(void (XEmitter::*avxOp)(X64Reg, X64Reg,
                                                  const OpArg &),
                          void (XEmitter::*sseOp)(X64Reg, const OpArg &),
                          X64Reg regOp, const OpArg &arg1, const OpArg &arg2,
                          bool packed, bool reversible) {
...
void EmuCodeBlock::avx_op(void (XEmitter::*avxOp)(X64Reg, X64Reg, const OpArg &,
                                                  u8),
                          void (XEmitter::*sseOp)(X64Reg, const OpArg &, u8),
                          X64Reg regOp, const OpArg &arg1, const OpArg &arg2,
                          u8 imm) {
  if (arg1.IsSimpleReg(regOp)) {
...
alignas(16) static const u64 psMantissaTruncate[2] = {0xFFFFFFFFF8000000ULL,
                                                      0xFFFFFFFFF8000000ULL};
alignas(16) static const u64 psRoundBit[2] = {0x8000000, 0x8000000};

// Emulate the odd truncation/rounding that the PowerPC does on the RHS operand
// before
// a single precision multiply. To be precise, it drops the low 28 bits of the
// mantissa,
// rounding to nearest as it does.
// It needs a temp, so let the caller pass that in.
void EmuCodeBlock::Force25BitPrecision(X64Reg output, const OpArg &input,
                                       X64Reg tmp) {
  if (jit->jo.accurateSinglePrecision) {
    ...
    if (input.IsSimpleReg() && cpu_info.bAVX) {
      ...
      if (!input.IsSimpleReg(output))
        MOVAPD(output, input);
      avx_op(&XEmitter::VPAND, &XEmitter::PAND, tmp, R(output), M(psRoundBit),
             true, true);
...
// Since the following float conversion functions are used in non-arithmetic PPC
// float instructions,
// they must convert floats bitexact and never flush denormals to zero or turn
// SNaNs into QNaNs.
// This means we can't use CVTSS2SD/CVTSD2SS :(
// The x87 FPU doesn't even support flush-to-zero so we can use FLD+FSTP even on
// denormals.
// If the number is a NaN, make sure to set the QNaN bit back to its original
// value.

// Another problem is that officially, converting doubles to single format
// results in undefined
// behavior.
// Relying on undefined behavior is a bug so no software should ever do this.
// In case it does happen, phire's more accurate implementation of
// ConvertDoubleToSingle() is
// reproduced below.

//#define MORE_ACCURATE_DOUBLETOSINGLE
#ifdef MORE_ACCURATE_DOUBLETOSINGLE

alignas(16) static const __m128i double_exponent =
    _mm_set_epi64x(0, 0x7ff0000000000000);
alignas(16) static const __m128i double_fraction =
    _mm_set_epi64x(0, 0x000fffffffffffff);
alignas(16) static const __m128i double_sign_bit =
    _mm_set_epi64x(0, 0x8000000000000000);
alignas(16) static const __m128i double_explicit_top_bit =
    _mm_set_epi64x(0, 0x0010000000000000);
alignas(16) static const __m128i double_top_two_bits =
    _mm_set_epi64x(0, 0xc000000000000000);
alignas(16) static const __m128i double_bottom_bits =
    _mm_set_epi64x(0, 0x07ffffffe0000000);
...
alignas(16) static const __m128i double_sign_bit =
    _mm_set_epi64x(0xffffffffffffffff, 0x7fffffffffffffff);
alignas(16) static const __m128i single_qnan_bit =
    _mm_set_epi64x(0xffffffffffffffff, 0xffffffffffbfffff);
alignas(16) static const __m128i double_qnan_bit =
    _mm_set_epi64x(0xffffffffffffffff, 0xfff7ffffffffffff);

// Smallest positive double that results in a normalized single.
alignas(16) static const
    double min_norm_single = std::numeric_limits<float>::min();

void EmuCodeBlock::ConvertDoubleToSingle(X64Reg dst, X64Reg src) {
  // Most games have flush-to-zero enabled, which causes the single -> double ->
  // single process here
  // to be lossy.
  ...
  // Here, check to see if the source is small enough that it will result in a
  // denormal, and pass it
  // to the x87 unit
  // if it is.
  avx_op(&XEmitter::VPAND, &XEmitter::PAND, XMM0, R(src), M(&double_sign_bit),
         true, true);
  ...
  // Clear the quiet bit of the SNaN, which was 0 (signalling) but got set to 1
  // (quiet) by
  // conversion.
  ...
  // We'd normally need to MOVDDUP here to put the single in the top half of the
  // output register
  // too, but
  // this function is only used to go directly to a following store, so we omit
  // the MOVDDUP here.
}
#endif // MORE_ACCURATE_DOUBLETOSINGLE

// Converting single->double is a bit easier because all single denormals are
// double normals.
void EmuCodeBlock::ConvertSingleToDouble(X64Reg dst, X64Reg src,
                                         bool src_is_gpr) {
...
// TODO: it might be faster to handle FPRF in the same way as CR is currently
// handled for integer,
// storing
// the result of each floating point op and calculating it when needed. This is
// trickier than for
// integers
// though, because there's 32 possible FPRF bit combinations but only 9
// categories of floating point
// values,
// which makes the whole thing rather trickier.
// Fortunately, PPCAnalyzer can optimize out a large portion of FPRF
// calculations, so maybe this
// isn't
...
void EmuCodeBlock::SetFPRF(Gen::X64Reg xmm) {
  ...
  if (cpu_info.bSSE4_1) {
    ...
    SHR(64, R(RSCRATCH),
        Imm8(63)); // Get the sign bit; almost all the branches need it.
    ...
    LEA(32, RSCRATCH, MScaled(RSCRATCH, MathUtil::PPC_FPCLASS_NINF -
    ...
    FixupBranch nan = J_CC(CC_G); // This works because if the sign bit is set,
                                  // RSCRATCH is negative
...
// Some testing shows CA is set roughly ~1/3 of the time (relative to clears),
// so
...
void EmuCodeBlock::JitClearCA() { MOV(8, PPCSTATE(xer_ca), Imm8(0)); }
