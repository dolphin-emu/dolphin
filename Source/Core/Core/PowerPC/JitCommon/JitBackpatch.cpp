...
static void BackPatchError(const std::string &text, u8 *codePtr,
                           u32 emAddress) {
...
bool Jitx86Base::HandleFault(uintptr_t access_address, SContext *ctx) {
  ...
  if (access_address >= (uintptr_t)Memory::physical_base &&
    ...
    return BackPatch((u32)(access_address - (uintptr_t)Memory::physical_base),
                     ctx);
  if (access_address >= (uintptr_t)Memory::logical_base &&
      access_address < (uintptr_t)Memory::logical_base + 0x100010000)
    return BackPatch((u32)(access_address - (uintptr_t)Memory::logical_base),
                     ctx);
...
bool Jitx86Base::BackPatch(u32 emAddress, SContext *ctx) {
  ...
  if (!DisassembleMov(codePtr, &info)) {
    BackPatchError("BackPatch - failed to disassemble MOV instruction", codePtr,
                   emAddress);
    return false;
  }
  ...
  if (!info.isMemoryWrite) {
    ...
    if (info.byteSwap || info.hasImmediate) {
      // The instruction is a MOVBE but it failed so the value is still in
  ...
  if (info.isMemoryWrite) {
    ...
    u32 pc = it3->second;
    trampoline = trampolines.GenerateWriteTrampoline(
        info, registersInUse, exceptionHandler, returnPtr, pc);
  } else {
    trampoline = trampolines.GenerateReadTrampoline(
        info, registersInUse, exceptionHandler, returnPtr);
  }
