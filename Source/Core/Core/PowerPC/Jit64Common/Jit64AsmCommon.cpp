...
#define QUANTIZED_REGS_TO_SAVE                                                 \
  (ABI_ALL_CALLER_SAVED &                                                      \
   ~BitSet32{RSCRATCH, RSCRATCH2, RSCRATCH_EXTRA, XMM0 + 16, XMM1 + 16})

#define QUANTIZED_REGS_TO_SAVE_LOAD                                            \
  (QUANTIZED_REGS_TO_SAVE | BitSet32{RSCRATCH2})
...
void CommonAsmRoutines::GenFifoWrite(int size) {
  ...
  _assert_msg_(DYNA_REC, gather_pipe <= 0x7FFFFFFF,
               "Gather pipe not in low 2GB of memory!");
...
void CommonAsmRoutines::GenFrsqrte() {
  ...
  SHL(64, R(RSCRATCH2),
      Imm8(
          52)); // exponent = ((0x3FFLL << 52) - ((exponent - (0x3FELL << 52)) /
                // 2)) & (0x7FFLL << 52);

  MOV(64, R(RSCRATCH_EXTRA), R(RSCRATCH));
  SHR(64, R(RSCRATCH_EXTRA), Imm8(48));
  AND(32, R(RSCRATCH_EXTRA), Imm8(0x1F));
  XOR(32, R(RSCRATCH_EXTRA),
      Imm8(0x10)); // int index = i / 2048 + (odd_exponent ? 16 : 0);

  SHR(64, R(RSCRATCH), Imm8(37));
  AND(32, R(RSCRATCH), Imm32(0x7FF));
  IMUL(32, RSCRATCH, MScaled(RSCRATCH_EXTRA, SCALE_4,
                             (u32)(u64)MathUtil::frsqrte_expected_dec));
  MOV(32, R(RSCRATCH_EXTRA),
      MScaled(RSCRATCH_EXTRA, SCALE_4,
              (u32)(u64)MathUtil::frsqrte_expected_base));
  SUB(32, R(RSCRATCH_EXTRA), R(RSCRATCH));
  SHL(64, R(RSCRATCH_EXTRA), Imm8(26));
  OR(64, R(RSCRATCH2),
     R(RSCRATCH_EXTRA)); // vali |= (s64)(frsqrte_expected_base[index] -
...
void CommonAsmRoutines::GenFres() {
  ...
  IMUL(32, RSCRATCH,
       MScaled(RSCRATCH2, SCALE_4, (u32)(u64)MathUtil::fres_expected_dec));
  ADD(32, R(RSCRATCH), Imm8(1));
  SHR(32, R(RSCRATCH), Imm8(1));

  MOV(32, R(RSCRATCH2),
      MScaled(RSCRATCH2, SCALE_4, (u32)(u64)MathUtil::fres_expected_base));
  SUB(32, R(RSCRATCH2), R(RSCRATCH));
  SHL(64, R(RSCRATCH2), Imm8(29));
  OR(64, R(RSCRATCH2),
     R(RSCRATCH_EXTRA)); // vali |= (s64)(fres_expected_base[i / 1024] -
...
void CommonAsmRoutines::GenMfcr() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    // FIXME: is there a better way to do this without the partial register
    // merging?
    SETcc(CC_Z, R(tmp));
...
alignas(16) static const
    float m_65535[4] = {65535.0f, 65535.0f, 65535.0f, 65535.0f};
...
// according to Intel Docs CVTPS2DQ writes 0x80000000 if the source floating
// point value is out of
// int32 range
...
void CommonAsmRoutines::GenQuantizedStores() {
  ...
  SafeWriteRegToReg(RSCRATCH, RSCRATCH_EXTRA, 64, 0, QUANTIZED_REGS_TO_SAVE,
                    SAFE_LOADSTORE_NO_SWAP | SAFE_LOADSTORE_NO_PROLOG |
                        SAFE_LOADSTORE_NO_FASTMEM);
...
void CommonAsmRoutines::GenQuantizedLoads() {
  ...
  // FIXME? This code (in non-MMU mode) assumes all accesses are directly to
  // RAM, i.e.
  // don't need hardware access handling. This will definitely crash if paired
  // loads occur
  // from non-RAM areas, but as far as I know, this never happens. I don't know
  // if this is
  // for a good reason, or merely because no game does this.
  // If we find something that actually does do this, maybe this should be
  // changed. How
  // much of a performance hit would it be?
  const u8 *loadPairedFloatTwo = AlignCode4();
  if (jit->jo.memcheck) {
    SafeLoadToReg(RSCRATCH_EXTRA, R(RSCRATCH_EXTRA), 64, 0,
                  QUANTIZED_REGS_TO_SAVE, false,
  ...
  if (jit->jo.memcheck) {
    SafeLoadToReg(RSCRATCH_EXTRA, R(RSCRATCH_EXTRA), 32, 0,
                  QUANTIZED_REGS_TO_SAVE, false,
                  SAFE_LOADSTORE_NO_FASTMEM | SAFE_LOADSTORE_NO_PROLOG);
  ...
  if (jit->jo.memcheck) {
    ...
    SafeLoadToReg(RSCRATCH_EXTRA, R(RSCRATCH_EXTRA), 16, 0,
                  QUANTIZED_REGS_TO_SAVE_LOAD, false,
                  SAFE_LOADSTORE_NO_FASTMEM | SAFE_LOADSTORE_NO_PROLOG);
  ...
  if (jit->jo.memcheck)
    SafeLoadToReg(RSCRATCH_EXTRA, R(RSCRATCH_EXTRA), 8, 0,
                  QUANTIZED_REGS_TO_SAVE_LOAD, false,
                  SAFE_LOADSTORE_NO_FASTMEM | SAFE_LOADSTORE_NO_PROLOG);
  else
    UnsafeLoadRegToRegNoSwap(RSCRATCH_EXTRA, RSCRATCH_EXTRA, 8,
                             0); // RSCRATCH_EXTRA = 0x000000xx
  ...
  if (jit->jo.memcheck)
    SafeLoadToReg(RSCRATCH_EXTRA, R(RSCRATCH_EXTRA), 8, 0,
                  QUANTIZED_REGS_TO_SAVE_LOAD, true,
                  SAFE_LOADSTORE_NO_FASTMEM | SAFE_LOADSTORE_NO_PROLOG);
  else
