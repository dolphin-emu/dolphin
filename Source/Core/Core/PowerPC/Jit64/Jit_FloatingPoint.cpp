...
void Jit64::fmaddXX(UGeckoInstruction inst)
{
  ...
  const bool want_rc_rounded = (error_free_transformation || software_fma) && round_input;
  const bool want_rc_duplicated =
      (m_accurate_nans || (error_free_transformation && !want_rc_rounded)) && (madds0 || madds1);
  ...
  else
  {
    ...
    if (want_rc_duplicated && (packed || !error_free_transformation))
    {
  ...
  if (software_fma)
  {
    ...
    {
      if ((want_rc_rounded && error_free_transformation) || ((i == 0 || madds0) && !madds1))
        MOVSD(XMM1, want_rc_rounded ? R(Rc_rounded) : Rc);
      else
  ...
  if (error_free_transformation)
  {
    ...
    BitSet32 additional_registers_to_save{XMM1 + 16, XMM2 + 16};
    if (want_rc_rounded)
    {
      Rc_rounded_duplicated = Rc_rounded;
      additional_registers_to_save[Rc_rounded] = true;
    }
    else if (want_rc_duplicated)
    {
      Rc_rounded_duplicated = Rc_duplicated;
      additional_registers_to_save[want_rc_duplicated] = true;
    }
    ...
    if (packed)
    {
      ...
      if (software_fma)
      {
        const BitSet32 registers_to_save_1 =
            CallerSavedRegistersInUse(additional_registers_to_save);
        ABI_PushRegistersAndAdjustStack(registers_to_save_1, 0);
        ...
        MOVSD(XMM0, R(Rc_rounded_duplicated));
        MOVSD(XMM1, Ra);

        ABI_CallFunction(static_cast<double (*)(double, double, double)>(&std::fma));

        ABI_PopRegistersAndAdjustStack(registers_to_save_1, 0);

        MOVSD(XMM3, R(XMM0));

        additional_registers_to_save[XMM3 + 16] = true;
        const BitSet32 registers_to_save_2 =
            CallerSavedRegistersInUse(additional_registers_to_save);
        ABI_PushRegistersAndAdjustStack(registers_to_save_2, 0);
        ...
        ABI_PopRegistersAndAdjustStack(registers_to_save_2, 0);
    ...
    else
    {
      ...
      if (software_fma)
      {
        const BitSet32 registers_to_save = CallerSavedRegistersInUse(additional_registers_to_save);
        ABI_PushRegistersAndAdjustStack(registers_to_save, 0);
        ...
        ABI_PopRegistersAndAdjustStack(registers_to_save, 0);
      }
      else if (use_fma)
      {
