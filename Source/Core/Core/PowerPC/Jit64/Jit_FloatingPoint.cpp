...
alignas(16) static const u64 psSignBits[2] = {0x8000000000000000ULL,
                                              0x0000000000000000ULL};
alignas(16) static const u64 psSignBits2[2] = {0x8000000000000000ULL,
                                               0x8000000000000000ULL};
alignas(16) static const u64 psAbsMask[2] = {0x7FFFFFFFFFFFFFFFULL,
                                             0xFFFFFFFFFFFFFFFFULL};
alignas(16) static const u64 psAbsMask2[2] = {0x7FFFFFFFFFFFFFFFULL,
                                              0x7FFFFFFFFFFFFFFFULL};
alignas(16) static const u64 psGeneratedQNaN[2] = {0x7FF8000000000000ULL,
...
X64Reg Jit64::fp_tri_op(int d, int a, int b, bool reversible, bool single,
                        ...
                        bool packed, bool preserve_inputs, bool roundRHS) {
  fpr.Lock(d, a, b);
...
// We can avoid calculating FPRF if it's not needed; every float operation
// resets it, so
// if it's going to be clobbered in a future instruction before being read, we
// can just
// not calculate it.
void Jit64::SetFPRFIfNeeded(X64Reg xmm) {
  // As far as we know, the games that use this flag only need FPRF for fmul and
  // fmadd, but
  // FPRF is fast enough in JIT that we might as well just enable it for every
  // float instruction
...
void Jit64::HandleNaNs(UGeckoInstruction inst, X64Reg xmm_out, X64Reg xmm,
  ...
  if (inst.OPCD != 4) {
    ...
    if (cpu_info.bSSE4_1) {
      avx_op(&XEmitter::VCMPPD, &XEmitter::CMPPD, clobber, R(xmm), R(xmm),
             CMP_UNORD);
      ...
      for (u32 x : inputs) {
        avx_op(&XEmitter::VCMPPD, &XEmitter::CMPPD, clobber, fpr.R(x), fpr.R(x),
               CMP_UNORD);
...
void Jit64::fp_arith(UGeckoInstruction inst) {
  ...
  // If both the inputs are known to have identical top and bottom halves, we
  // can skip the MOVDDUP
  // at the end by
  // using packed arithmetic instead.
  bool packed =
      inst.OPCD == 4 || (inst.OPCD == 59 && jit->js.op->fprIsDuplicated[a] &&
                         jit->js.op->fprIsDuplicated[arg2]);
  // Packed divides are slower than scalar divides on basically all x86, so this
  // optimization isn't
  // worth it in that case.
  // Atoms (and a few really old CPUs) are also slower on packed operations than
  // scalar ones.
  ...
  case 18:
    dest = fp_tri_op(
        d, a, b, false, single, packed ? &XEmitter::VDIVPD : &XEmitter::VDIVSD,
        packed ? &XEmitter::DIVPD : &XEmitter::DIVSD, packed, preserve_inputs);
    break;
  case 20:
    dest = fp_tri_op(
        d, a, b, false, single, packed ? &XEmitter::VSUBPD : &XEmitter::VSUBSD,
        packed ? &XEmitter::SUBPD : &XEmitter::SUBSD, packed, preserve_inputs);
    break;
  case 21:
    dest = fp_tri_op(
        d, a, b, true, single, packed ? &XEmitter::VADDPD : &XEmitter::VADDSD,
        packed ? &XEmitter::ADDPD : &XEmitter::ADDSD, packed, preserve_inputs);
    break;
  case 25:
    dest = fp_tri_op(d, a, c, true, single,
                     packed ? &XEmitter::VMULPD : &XEmitter::VMULSD,
                     packed ? &XEmitter::MULPD : &XEmitter::MULSD, packed,
                     preserve_inputs, round_input);
    break;
  default:
...
void Jit64::fmaddXX(UGeckoInstruction inst) {
  ...
  bool packed =
      inst.OPCD == 4 ||
      (!cpu_info.bAtom && single && jit->js.op->fprIsDuplicated[a] &&
  ...
  default:
    bool special =
        inst.SUBOP5 == 30 && (!cpu_info.bFMA || Core::g_want_determinism);
  ...
  // While we don't know if any games are actually affected (replays seem to
  // work with all the usual
  // suspects for desyncing), netplay and other applications need absolute
  // perfect determinism, so
  // be extra careful and don't use FMA, even if in theory it might be okay.
  // Note that FMA isn't necessarily less correct (it may actually be closer to
  // correct) compared
  // to what the Gekko does here; in deterministic mode, the important thing is
  // multiple Dolphin
  ...
  {
    // We implement nmsub a little differently ((b - a*c) instead of -(a*c -
    // b)), so handle it
    // separately.
    MOVAPD(XMM1, fpr.R(b));
    if (packed) {
...
void Jit64::fsign(UGeckoInstruction inst) {
  ...
  switch (inst.SUBOP10) {
  case 40: // neg
    avx_op(&XEmitter::VPXOR, &XEmitter::PXOR, fpr.RX(d), src,
           M(packed ? psSignBits2 : psSignBits), packed);
    break;
  case 136: // nabs
    avx_op(&XEmitter::VPOR, &XEmitter::POR, fpr.RX(d), src,
           M(packed ? psSignBits2 : psSignBits), packed);
    break;
  case 264: // abs
    avx_op(&XEmitter::VPAND, &XEmitter::PAND, fpr.RX(d), src,
           M(packed ? psAbsMask2 : psAbsMask), packed);
    break;
  default:
...
void Jit64::fselx(UGeckoInstruction inst) {
  ...
  fpr.Lock(a, b, c, d);
  PXOR(XMM0, R(XMM0));
  // This condition is very tricky; there's only one right way to handle both
  // the case of
...
void Jit64::fmrx(UGeckoInstruction inst) {
  ...
  if (fpr.IsBound(d)) {
    // We don't need to load d, but if it is loaded, we need to mark it as
    // dirty.
    fpr.BindToRegister(d);
    // We have to use MOVLPD if b isn't loaded because "MOVSD reg, mem" sets the
    // upper bits (64+)
...
void Jit64::FloatCompare(UGeckoInstruction inst, bool upper) {
  ...
  if (analyzer.HasOption(PPCAnalyst::PPCAnalyzer::OPTION_CROR_MERGE) &&
      MergeAllowedNextInstructions(1) && next.OPCD == 19 &&
      next.SUBOP10 == 449 && (next.CRBA >> 2) == crf &&
      (next.CRBB >> 2) == crf && (next.CRBD >> 2) == crf) {
