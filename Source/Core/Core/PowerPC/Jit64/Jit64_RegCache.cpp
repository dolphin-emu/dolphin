...
const std::vector<X64Reg> GPR_SCRATCH_ALLOCATION_ORDER = {RSCRATCH, RSCRATCH2};

const std::vector<X64Reg> FPU_ALLOCATION_ORDER = {XMM6,  XMM7,  XMM8,  XMM9, XMM10, XMM11, XMM12,
                                                  XMM13, XMM14, XMM15, XMM2, XMM3,  XMM4,  XMM5};

const std::vector<X64Reg> FPU_SCRATCH_ALLOCATION_ORDER = {XMM0, XMM1};
...
Native<T> Any<T>::Bind(Jit64Reg::BindMode mode)
{
  ...
  if (mode == BindMode::Read || mode == BindMode::ReadWrite ||
      mode == BindMode::ReadWriteTransactional)
  {
    const auto& ppc = m_reg->m_regs[m_data.reg];
    _assert_msg_(REGCACHE, ppc.bindWriteWithoutReadAdvise == 0,
                 "Register was bound as write-only before being bound for read");
    // we can continue here, but it may not be pretty
  }

  _assert_msg_(REGCACHE, m_data.type == RegisterType::PPC || m_data.type == RegisterType::Bind,
               "Only PPC registers can be bound");
  RealizeLock();
  RegisterData data = {};
...
void Any<T>::LoadIfNotImmediate()
{
  ...
  if (!IsRegBound() && !IsImm())
  {
    ...
    DEBUG_LOG(REGCACHE, "LoadIfNotImmediate bind %zu to %d", m_data.reg,
              m_reg->m_regs[m_data.reg].location.GetSimpleReg());
  }
}
...
void Any<T>::Lock(RegisterData& data)
{
  ...
  case RegisterType::Bind:
  {
    ...
    xreg.locked++;
    DEBUG_LOG(REGCACHE, "++ Lock bind %zu (count = %d), native %u (count = %d)", data.reg,
              ppc.locked, data.xreg, xreg.locked);
    break;
  }
  case RegisterType::Borrow:
  {
    ...
    {
      // This should happen when the lock is realized to give us the best chance at choosing unused
      // regs
    ...
    {
      auto& xreg = m_reg->m_xregs[data.xreg];
      _assert_msg_(DYNAREC, xreg.locked == 0 || xreg.IsBorrowed(),
                   "Attempting to steal a register that is bound");
...
template <Type T>
void Any<T>::LockAdvisory(const RegisterData& data)
{
  switch (data.type)
  {
  case RegisterType::Bind:
...
template <Type T>
void Any<T>::Unlock(const RegisterData& data)
{
  switch (data.type)
  {
  case RegisterType::Bind:
  {
    ...
    auto& xreg = m_reg->m_xregs[data.xreg];
    xreg.locked--;
    DEBUG_LOG(REGCACHE, "-- Unlock bind %zu (count = %d), native %u (count = %d)", data.reg,
              ppc.locked, data.xreg, xreg.locked);
    _assert_msg_(REGCACHE, xreg.ppcReg == data.reg,
                 "Register is no longer associated with PPC register");
...
template <Type T>
void Any<T>::UnlockAdvisory(const RegisterData& data)
{
  switch (data.type)
  {
  case RegisterType::Bind:
...
void Any<T>::SetImm32(u32 imm)
{
  ...
  _assert_msg_(REGCACHE, m_data.type == RegisterType::PPC,
               "SetImm32 is only valid on guest registers");
...
Any<T>::operator OpArg()
{
  ...
  if (m_data.type == RegisterType::PPC)
  {
    ...
    _assert_msg_(REGCACHE, ppc.bindAdvise == 0,
                 "Register is bound but caller attempted to use unbound register");
    return ppc.location;
  }
...
void RegisterClassBase<T>::Init()
{
  ...
  {
    ReleaseXReg(static_cast<X64Reg>(&xreg - &m_xregs[0]));
  }
  for (auto& reg : m_regs)
  {
...
size_t RegisterClassBase<T>::NumFreeRegisters() const
{
  ...
  return std::count_if(order.begin(), order.end(),
                       [&](auto reg) { return m_xregs[reg].locked == 0; });
}
...
void RegisterClassBase<T>::FlushBatch(const BitSet32& regs)
{
  ...
  for (int i : regs)
  {
    ...
    if (reg.away)
    {
      _assert_msg_(REGCACHE, reg.location.IsSimpleReg() || reg.location.IsImm(),
                   "Jit64 - Flush unhandled case, reg %u PC: %08x", i, PC);
...
BitSet32 RegisterClassBase<T>::InUse() const
{
  ...
  ForEachX64([&](auto i, const auto& xreg) {
    if (!xreg.IsFree())
      inuse[i] = true;
  });
...
int RegisterClassBase<T>::SanityCheck() const
{
  if (std::any_of(m_xregs.begin(), m_xregs.end(), [](const auto& xreg) { return xreg.locked > 0; }))
    return 1;
  if (std::any_of(m_regs.begin(), m_regs.end(),
                  [](const auto& reg) { return reg.locked > 0 || reg.lockAdvise > 0; }))
    return 2;
...
void RegisterClassBase<T>::Sync()
{
  ...
  ForEachPPC([&](auto i, const auto& reg) {
    ...
    {
      _assert_msg_(REGCACHE, reg.location.IsSimpleReg() || reg.location.IsImm(),
                   "Jit64 - Flush unhandled case, reg %lu PC: %08x", i, PC);
...
template <Type T>
X64Reg RegisterClassBase<T>::GetFreeXReg(const std::vector<X64Reg>& candidates,
                                         BitSet32 disallowed) const
{
  auto it = std::find_if(std::begin(candidates), std::end(candidates),
                         [&](auto xr) { return m_xregs[xr].IsFree() && !disallowed[xr]; });
...
X64Reg RegisterClassBase<T>::BindToRegister(reg_t preg, bool doLoad, bool makeDirty, bool tx)
{
  ...
  const auto& xreg = m_xregs[xr];
  _assert_msg_(REGCACHE, xreg.ppcReg == preg, "This reg should have been bound");
  _assert_msg_(REGCACHE, ppc.location.IsImm() || ppc.location.IsSimpleReg(),
               "We didn't end up with a simple register or immediate");
  return xr;
}
...
void RegisterClassBase<T>::StoreFromRegister(size_t preg, FlushMode mode)
{
  ...
  _assert_msg_(REGCACHE, !ppc.transaction, "Register %zu has an open transaction during store",
               preg);
  ...
  ForEachPPC([&](auto i, auto& reg) {
    if (reg.transaction)
    {
      ...
      if (reg.location.IsSimpleReg())
      {
        ...
        _assert_msg_(REGCACHE, xreg.locked == ROLLBACK_LOCK_MAGIC,
                     "Lock count for the rollback register should have been %u (was %u)",
                     ROLLBACK_LOCK_MAGIC, xreg.locked);
        xreg.locked = 0;
      }
    }
  });
