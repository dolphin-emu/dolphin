...
// TODO(ector): Tons of pshufb optimization of the loads/stores, for SSSE3+,
// possibly SSE4, only.
...
void Jit64::lXXx(UGeckoInstruction inst) {
  ...
  FALLBACK_IF(SConfig::GetInstance().bJITLoadStorelbzxOff &&
              (inst.OPCD == 31) && (inst.SUBOP10 == 87));
  ...
  // PowerPC has no 8-bit sign extended load, but x86 does, so merge extsb with
  // the load if we find
  // it.
  if (MergeAllowedNextInstructions(1) && accessSize == 8 &&
      js.op[1].inst.OPCD == 31 && js.op[1].inst.SUBOP10 == 954 &&
      js.op[1].inst.RS == inst.RD && js.op[1].inst.RA == inst.RD &&
      !js.op[1].inst.Rc) {
    js.downcountAmount++;
  ...
  // TODO(ector): Make it dynamically enable/disable idle skipping where
  // appropriate
  // Will give nice boost to dual core mode
  // (mb2): I agree,
  // IMHO those Idles should always be skipped and replaced by a more
  // controllable "native" Idle
  // methode
  ...
  if (SConfig::GetInstance().bSkipIdle &&
      CPU::GetState() != CPU::CPU_STEPPING && inst.OPCD == 32 &&
    ...
    // TODO(LinesPrower):
    // - Rewrite this!
    // It seems to be ugly and inefficient, but I don't know JIT stuff enough to
    // make it right
    ...
    SafeLoadToReg(gpr.RX(d), gpr.R(a), accessSize, offset,
                  CallerSavedRegistersInUse(), signExtend);
    ...
    // ! we must continue executing of the loop after exception handling, maybe
    // there is still 0 in
    // r0
  ...
  if (!update && !a) {
    ...
    if ((inst.OPCD != 31) && gpr.R(a).IsImm() && !jo.memcheck) {
      ...
      // If we're using reg+reg mode and b is an immediate, pretend we're using
      ...
      // Depending on whether we have an immediate and/or update, find the
      // optimum way to calculate
      // the load address.
  ...
  // A bit of an evil hack here. We need to retain the original value of this
  // register for the
  // exception path, but we'd rather not needlessly pass it around if we don't
  // have to, since
  // the exception path is very rare. So we store the value in the regcache, let
  // the load path
  // clobber it, then restore the value in the exception path.
  // TODO: no other load has to do this at the moment, since no other loads go
  // directly to the
  ...
  SafeLoadToReg(gpr.RX(d), opAddress, accessSize, loadOffset, registersInUse,
                signExtend);
...
void Jit64::dcbx(UGeckoInstruction inst) {
  ...
  LEA(32, value,
      MScaled(addr, SCALE_8, 0)); // addr << 3 (masks the first 3 bits)
  SHR(32, R(value),
      Imm8(3 + 5 + 5)); // >> 5 for cache line size, >> 5 for width of bitset
...
void Jit64::dcbt(UGeckoInstruction inst) {
  ...
  // Prefetch. Since we don't emulate the data cache, we don't need to do
  // anything.
  ...
  if (MergeAllowedNextInstructions(1) && js.op[1].inst.OPCD == 31 &&
      js.op[1].inst.SUBOP10 == 54 && js.op[1].inst.RA == inst.RA &&
      js.op[1].inst.RB == inst.RB) {
    js.skipInstructions = 1;
  }
}
...
void Jit64::dcbz(UGeckoInstruction inst) {
  ...
  // Should this code ever run? I can't find any games that use DCBZ on
  // non-physical addresses, but
  // supposedly there are, at least for some MMU titles. Let's be careful and
  // support it to be sure.
  SwitchToFarCode();
  SetJumpTarget(slow);
...
void Jit64::stX(UGeckoInstruction inst) {
  ...
  if (!a || gpr.R(a).IsImm()) {
    ...
    bool exception = WriteToConstAddress(accessSize, gpr.R(s), addr,
    ...
    gpr.BindToRegister(a, true, update);
    if (gpr.R(s).IsImm()) {
      SafeWriteRegToReg(gpr.R(s), gpr.RX(a), accessSize, offset,
                        CallerSavedRegistersInUse(),
      ...
      SafeWriteRegToReg(reg_value, gpr.RX(a), accessSize, offset,
                        CallerSavedRegistersInUse(),
                        SAFE_LOADSTORE_CLOBBER_RSCRATCH_INSTEAD_OF_ADDR);
    }
...
void Jit64::stmw(UGeckoInstruction inst) {
  ...
  for (int i = inst.RD; i < 32; i++) {
    ...
    if (gpr.R(i).IsImm()) {
      SafeWriteRegToReg(gpr.R(i), RSCRATCH, 32,
                        (i - inst.RD) * 4 + (u32)(s32)inst.SIMM_16,
                        CallerSavedRegistersInUse());
    } else {
      MOV(32, R(RSCRATCH2), gpr.R(i));
      SafeWriteRegToReg(RSCRATCH2, RSCRATCH, 32,
                        (i - inst.RD) * 4 + (u32)(s32)inst.SIMM_16,
                        CallerSavedRegistersInUse());
    }
  }
