...
void Jit64::ClearCRFieldBit(int field, int bit) {
  ...
  // We don't need to set bit 32; the cases where that's needed only come up
  // when setting bits, not
  // clearing.
}
...
static void DoICacheReset() { PowerPC::ppcState.iCache.Reset(); }
...
void Jit64::mfspr(UGeckoInstruction inst) {
  ...
  case SPR_TL:
  case SPR_TU: {
    // TODO: we really only need to call GetFakeTimeBase once per JIT block;
    // this matters because
    // typical use of this instruction is to call it three times, e.g.
    // mftbu/mftbl/mftbu/cmpw/bne
    // to deal with possible timer wraparound. This makes the second two (out of
    // three) completely
    ...
    CVTSS2SI(RDX,
             R(XMM0)); // RDX is downcount scaled by the overclocking factor
    MOV(32, R(RAX), M(&CoreTiming::g_slicelength));
    SUB(64, R(RAX), R(RDX)); // cycles since the last CoreTiming::Advance()
                             // event is (slicelength -
                             // Scaled_downcount)
...
void Jit64::mtmsr(UGeckoInstruction inst) {
  ...
  TEST(32, PPCSTATE(Exceptions),
       Imm32(EXCEPTION_EXTERNAL_INT | EXCEPTION_PERFORMANCE_MONITOR |
             EXCEPTION_DECREMENTER));
  FixupBranch noExceptionsPending = J_CC(CC_Z);

  // Check if a CP interrupt is waiting and keep the GPU emulation in sync
  // (issue 4336)
