...
public:
  ...
  void Flush(FlushMode mode = FLUSH_ALL,
             BitSet32 regsToFlush = BitSet32::AllTrue(32));
  ...
  template <typename T> void Lock(T p) { regs[p].locked = true; }
  template <typename T, typename... Args> void Lock(T first, Args... args) {
    Lock(first);
    Lock(args...);
  }

  // these are x64 reg indices
  template <typename T> void LockX(T x) {
    if (xregs[x].locked)
      PanicAlert("RegCache: x %i already locked!", x);
    xregs[x].locked = true;
  }
  template <typename T, typename... Args> void LockX(T first, Args... args) {
    LockX(first);
    LockX(args...);
  }

  template <typename T> void UnlockX(T x) {
    if (!xregs[x].locked)
      PanicAlert("RegCache: x %i already unlocked!", x);
    xregs[x].locked = false;
  }
  template <typename T, typename... Args> void UnlockX(T first, Args... args) {
    UnlockX(first);
    UnlockX(args...);
  }
