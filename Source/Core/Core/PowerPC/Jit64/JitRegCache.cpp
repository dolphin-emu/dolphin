...
void RegCache::Start() {
  ...
  for (int i = 0; i < 32; i++)
  {
    ...
    {
      LoadToX64(i, true, false); //stats.firstRead[i] <= stats.firstWrite[i],
  false);
      maxPreload--;
      if (!maxPreload)
        break;
...
BitSet32 GPRRegCache::GetRegUtilization() { return jit->js.op->gprInReg; }

BitSet32 FPURegCache::GetRegUtilization() { return jit->js.op->gprInReg; }
...
// Estimate roughly how bad it would be to de-allocate this register. Higher
// score
// means more bad.
float RegCache::ScoreRegister(X64Reg xr) {
  ...
  // If it's not dirty, we don't need a store to write it back to the register
  // file, so
  // bias a bit against dirty registers. Testing shows that a bias of 2 seems
  // roughly
  // right: 3 causes too many extra clobbers, while 1 saves very few clobbers
  // relative
  // to the number of extra stores it causes.
  if (xregs[xr].dirty)
    score += 2;

  // If the register isn't actually needed in a physical register for a later
  // instruction,
  // writing it back to the register file isn't quite as bad.
  if (GetRegUtilization()[preg]) {
    // Don't look too far ahead; we don't want to have quadratic compilation
    // times for
    ...
    // Count how many other registers are going to be used before we need this
    // one again.
    u32 regs_in_count = CountRegsIn(preg, lookahead).Count();
    // Totally ad-hoc heuristic to bias based on how many other registers we'll
    // need
...
X64Reg RegCache::GetFreeXReg() {
  ...
  // Okay, not found; run the register allocator heuristic and figure out which
  // register we should
  // clobber.
...
// R12, when used as base register, for example in a LEA, can generate bad code!
// Need to look into
// this.
#ifdef _WIN32
...
const X64Reg *FPURegCache::GetAllocationOrder(size_t *count) {
  static const X64Reg allocationOrder[] = {XMM6,  XMM7,  XMM8,  XMM9,  XMM10,
