...
private:
	...
	const Gen::OpArg Location() const
	{
...
protected:
	...
	void ForEachPPC(const std::function<void(reg_t, PPCCachedReg&)> fn)
	{
		reg_t i = 0;
		for (auto& reg : m_regs)
			fn(i++, reg);
	}
	void ForEachPPCConst(const std::function<void(reg_t, const PPCCachedReg&)> fn) const
	{
		reg_t i = 0;
		for (auto& reg : m_regs)
			fn(i++, reg);
	}
	void ForEachX64(const std::function<void(X64Reg, X64CachedReg&)> fn)
	{
		int xr = 0;
		for (auto& xreg : m_xregs)
			fn((X64Reg)xr++, xreg);
	}
	void ForEachX64Const(const std::function<void(X64Reg, const X64CachedReg&)> fn) const
	{
		int xr = 0;
		for (auto& xreg : m_xregs)
			fn((X64Reg)xr++, xreg);
	...
	RegisterClassBase(Gen::XEmitter* emit, JitBase* jitBase): m_emit(emit), m_jit(jitBase) {}
...
public:
	...
	void Commit() { gpr.Commit(); fpu.Commit(); }
	void Rollback() { gpr.Rollback(); fpu.Rollback(); }
