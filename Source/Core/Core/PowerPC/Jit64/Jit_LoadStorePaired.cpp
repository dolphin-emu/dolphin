...
// The big problem is likely instructions that set the quantizers in the same
// block.
...
void Jit64::psq_stXX(UGeckoInstruction inst) {
  ...
  bool update =
      (inst.OPCD == 61 && offset) || (inst.OPCD == 4 && !!(inst.SUBOP6 & 32));
  int a = inst.RA;
  ...
  if (js.assumeNoPairedQuantize) {
    ...
    // TODO: this is kind of ugly :/ we should probably create a universal
    // load/store address
    // calculation
    // function that handles all these weird cases, e.g. how non-fastmem
    // loadstores clobber
    // addresses.
  ...
  // Some games (e.g. Dirt 2) incorrectly set the unused bits which breaks the
  // lookup table code.
  // Hence, we need to mask out the unused bits. The layout of the GQR register
  // is
  // UU[SCALE]UUUUU[TYPE] where SCALE is 6 bits and TYPE is 3 bits, so we have
  // to AND with
  ...
  if (w) {
    // One value
    CVTSD2SS(XMM0, fpr.R(s));
    CALLptr(MScaled(RSCRATCH, SCALE_8,
                    (u32)(u64)asm_routines.singleStoreQuantized));
  } else {
    // Pair of values
    CVTPD2PS(XMM0, fpr.R(s));
    CALLptr(MScaled(RSCRATCH, SCALE_8,
                    (u32)(u64)asm_routines.pairedStoreQuantized));
  }
...
void Jit64::psq_lXX(UGeckoInstruction inst) {
  ...
  bool indexed = inst.OPCD == 4;
  bool update =
      (inst.OPCD == 57 && offset) || (inst.OPCD == 4 && !!(inst.SUBOP6 & 32));
  int a = inst.RA;
  int b = indexed ? inst.RB : a;
  int s = inst.FS;
  int i = indexed ? inst.Ix : inst.I;
  ...
  if (js.assumeNoPairedQuantize) {
    ...
    // Let's mirror the JitAsmCommon code and assume all non-MMU loads go to
    // RAM.
    if (!jo.memcheck) {
      if (w) {
        if (cpu_info.bSSSE3) {
      ...
      if (update)
        registersInUse[addr] = true;
      SafeLoadToReg(RSCRATCH, R(addr), w ? 32 : 64, loadOffset, registersInUse,
                    false);
      MemoryExceptionCheck();
      if (w) {
        MOVD_xmm(XMM0, R(RSCRATCH));
  ...
  CALLptr(MScaled(RSCRATCH, SCALE_8,
                  (u32)(u64)(&asm_routines.pairedLoadQuantized[w * 8])));
