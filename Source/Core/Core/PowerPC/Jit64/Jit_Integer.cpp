...
// We could do overflow branchlessly, but unlike carry it seems to be quite a
// bit rarer.
void Jit64::GenerateOverflow() {
  ...
  SetJumpTarget(jno);
  // XER[OV] = 0
  // We need to do this without modifying flags so as not to break stuff that
  // assumes flags
  // aren't clobbered (carry, branch merging): speed doesn't really matter here
  // (this is really
  // rare).
...
void Jit64::FinalizeCarry(CCFlags cond) {
  ...
  if (js.op->wantsCA) {
    // Not actually merging instructions, but the effect is equivalent (we can't
    // have
...
void Jit64::FinalizeCarryOverflow(bool oe, bool inv) {
  if (oe) {
    // Make sure not to lose the carry flags (not a big deal, this path is
    // rare).
    PUSHF();
    // XER[OV] = 0
    AND(8, PPCSTATE(xer_so_ov), Imm8(~XER_OV_MASK));
...
// Be careful; only set needs_test to false if we can be absolutely sure flags
// don't need
// to be recalculated and haven't been clobbered. Keep in mind not all
// instructions set
// sufficient flags -- for example, the flags from SHL/SHR are *not* sufficient
// for LT/GT
// branches, only EQ.
// The flags from any instruction that may set OF (such as ADD/SUB) can not be
// used for
// LT/GT either.
void Jit64::ComputeRC(const OpArg &arg, bool needs_test, bool needs_sext) {
  _assert_msg_(DYNA_REC, arg.IsSimpleReg() || arg.IsImm(),
               "Invalid ComputeRC operand");
  if (arg.IsImm()) {
  ...
  if (CheckMergedBranch(0)) {
    if (arg.IsImm()) {
      ...
      if (needs_test) {
        ...
        // If an operand to the cmp/rc op we're merging with the branch isn't
        // used anymore, it'd be
        // better to flush it here so that we don't have to flush it on both
        // sides of the branch.
        // We don't want to do this if a test is needed though, because it would
        // interrupt macro-op
        // fusion.
...
// we can't do this optimization in the emitter because MOVZX and AND have
...
void Jit64::regimmop(int d, int a, bool binary, u32 value, Operation doop,
                     void (XEmitter::*op)(int, const OpArg &, const OpArg &),
                     bool Rc, bool carry) {
  ...
  if (a || binary || carry) {
    ...
    if (gpr.R(a).IsImm() && !carry) {
      ...
      (this->*op)(32, gpr.R(d),
                  Imm32(value)); // m_GPR[d] = m_GPR[_inst.RA] + _inst.SIMM_16;
    } else {
...
void Jit64::reg_imm(UGeckoInstruction inst) {
  ...
  case 14: // addi
    ...
    if (gpr.R(a).IsImm() && d != a && a != 0) {
      ...
      regimmop(d, a, false, (u32)(s32)inst.SIMM_16, Add,
               &XEmitter::ADD); // addi
    }
    break;
  case 15: // addis
  ...
  case 24:                                              // ori
    ...
    {
      // Make the nop visible in the generated code. not much use but
      // interesting if we see one.
      NOP();
      return;
    }
  ...
  case 27: // xoris
    ...
    break;
  case 12: // addic
    regimmop(d, a, false, (u32)(s32)inst.SIMM_16, Add, &XEmitter::ADD, false,
             true);
    break;
  case 13: // addic_rc
    regimmop(d, a, true, (u32)(s32)inst.SIMM_16, Add, &XEmitter::ADD, true,
             true);
    break;
  default:
    FALLBACK_IF(true);
...
bool Jit64::CheckMergedBranch(int crf) {
  ...
  return (((next.OPCD == 16 /* bcx */) ||
          ...
          !(next.BO & BO_DONT_CHECK_CONDITION) && (next.BI >> 2) == crf);
}
...
void Jit64::DoMergedBranchCondition() {
  ...
  if (test_bit & 8)
    pDontBranch =
        J_CC(condition ? CC_GE : CC_L, true); // Test < 0, so jump over if >= 0.
  else if (test_bit & 4)
    pDontBranch =
        J_CC(condition ? CC_LE : CC_G, true); // Test > 0, so jump over if <= 0.
  else if (test_bit & 2)
    pDontBranch =
        J_CC(condition ? CC_NE : CC_E, true); // Test = 0, so jump over if != 0.
...
void Jit64::cmpXX(UGeckoInstruction inst) {
  ...
  if (gpr.R(a).IsImm() && comparand.IsImm()) {
    // Both registers contain immediate values, so we can pre-compile the
    // compare result
    s64 compareResult = signedCompare
                            ? (s64)gpr.R(a).SImm32() - (s64)comparand.SImm32()
...
void Jit64::extsXx(UGeckoInstruction inst) {
  ...
  if (gpr.R(s).IsImm()) {
    gpr.SetImmediate32(a, (u32)(s32)(size == 16 ? (s16)gpr.R(s).Imm32()
                                                : (s8)gpr.R(s).Imm32()));
  } else {
    gpr.Lock(a, s);
...
void Jit64::MultiplyImmediate(u32 imm, int a, int d, bool overflow) {
  ...
  if (!overflow) {
    ...
    // We could handle factors of 2^N*3, 2^N*5, and 2^N*9 using lea+shl, but
    // testing shows
...
void Jit64::mulhwXx(UGeckoInstruction inst) {
  ...
  if (gpr.R(a).IsImm() && gpr.R(b).IsImm()) {
    if (sign)
      gpr.SetImmediate32(
          d, (u32)((u64)(((s64)gpr.R(a).SImm32() * (s64)gpr.R(b).SImm32())) >>
                   32));
    else
      gpr.SetImmediate32(
          d, (u32)(((u64)gpr.R(a).Imm32() * (u64)gpr.R(b).Imm32()) >> 32));
  } else if (sign) {
    gpr.Lock(a, b, d);
    // no register choice
    ...
    gpr.Lock(a, b, d);
    // We need to bind everything to registers since the top 32 bits need to be
    // zero.
    int src = d == b ? a : b;
    gpr.BindToRegister(d, d == a || d == b, true);
  ...
  if (gpr.R(a).IsImm() && gpr.R(b).IsImm()) {
    ...
    if (divisor == 0) {
      ...
      if (divisor == (u32)(1 << shift)) {
        ...
        if (((u64)(magic + 1) * (max_quotient * divisor - 1)) >> (shift + 32) !=
            max_quotient - 1) {
...
void Jit64::arithXex(UGeckoInstruction inst) {
  ...
  if (same_input_sub) {
    ...
    OpArg source =
        regsource ? gpr.R(d == b ? a : b) : Imm32(mex ? 0xFFFFFFFF : 0);
    if (d != a && d != b)
      MOV(32, gpr.R(d), gpr.R(a));
    if (!add)
      NOT(32, gpr.R(d));
    // if the source is an immediate, we can invert carry by going from add ->
    // sub and doing src =
    // -1 - src
...
void Jit64::rlwinmx(UGeckoInstruction inst) {
  ...
  if (gpr.R(s).IsImm()) {
    ...
    // If we know the high bit can't be set, we can avoid doing a sign extend
    // for flag storage.
    ...
    // common optimized case: byte/word extract
      ...
      MOVZX(32, mask_size, gpr.RX(a),
            ExtractFromReg(s, inst.SH ? (32 - inst.SH) >> 3 : 0));
      needs_sext = false;
    }
...
void Jit64::rlwimix(UGeckoInstruction inst) {
  ...
  if (gpr.R(a).IsImm() && gpr.R(s).IsImm()) {
    ...
    gpr.SetImmediate32(a, (gpr.R(a).Imm32() & ~mask) |
                              (_rotl(gpr.R(s).Imm32(), inst.SH) & mask));
    if (inst.Rc)
      ComputeRC(gpr.R(a));
  } else {
    gpr.Lock(a, s);
...
void Jit64::rlwnmx(UGeckoInstruction inst) {
  ...
  if (gpr.R(b).IsImm() && gpr.R(s).IsImm()) {
    gpr.SetImmediate32(a,
                       _rotl(gpr.R(s).Imm32(), gpr.R(b).Imm32() & 0x1F) & mask);
  } else {
    // no register choice
    gpr.FlushLockX(ECX);
    gpr.Lock(a, b, s);
...
void Jit64::srwx(UGeckoInstruction inst) {
  ...
  if (gpr.R(b).IsImm() && gpr.R(s).IsImm()) {
    ...
    gpr.SetImmediate32(
        a, (amount & 0x20) ? 0 : (gpr.R(s).Imm32() >> (amount & 0x1f)));
  } else {
    // no register choice
    gpr.FlushLockX(ECX);
    gpr.Lock(a, b, s);
...
void Jit64::slwx(UGeckoInstruction inst) {
  ...
  if (gpr.R(b).IsImm() && gpr.R(s).IsImm()) {
    u32 amount = gpr.R(b).Imm32();
    gpr.SetImmediate32(a, (amount & 0x20) ? 0 : gpr.R(s).Imm32()
                                                    << (amount & 0x1f));
    if (inst.Rc)
      ComputeRC(gpr.R(a));
  } else {
    ...
    if (!js.op->wantsCA) {
      ...
      if (amount == 1) {
        ...
        MOV(8, PPCSTATE(xer_ca),
            R(RSCRATCH)); // XER.CA = sign && carry, aka (input&0x80000001) ==
                          // 0x80000001
      } else {
        SAR(32, gpr.R(a), Imm8(amount));
