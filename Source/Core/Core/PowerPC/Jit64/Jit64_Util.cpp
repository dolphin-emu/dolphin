                     ...
                     int accessSize, bool signExtend, bool swap, bool update)
{
  ...
  if (reg_addr.IsImm() && offset.IsImm())
  {
    ...
    if (reg_value.HasPPCRegister())
    {
      // We have to use a scratch register here as reg_value will get clobbered if we pass a
      // constant
...
void Jit64::UnsafeReadFromMemory(int accessSize, GPRNative& value, OpArg target, bool signExtend,
                                 bool swap)
{
  if (swap)
  {
