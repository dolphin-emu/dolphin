...
// Unfeatures:
// * Does not recompile all instructions - sometimes falls back to inserting a
// CALL to the
...
// IMPORTANT:
// Make sure that all generated code and all emulator state sits under the 2GB
// boundary so that
// RIP addressing can be used easily. Windows will always allocate static code
// under the 2GB
// boundary.
// Also make sure to use VirtualAlloc and specify EXECUTE permission.

// Open questions
// * Should there be any statically allocated registers? r3, r4, r5, r8, r0 come
// to mind.. maybe sp
// * Does it make sense to finish off the remaining non-jitted instructions?
// Seems we are hitting
// diminishing returns.

// Other considerations
//
// We support block linking. Reserve space at the exits of every block for a
// full 5-byte jmp. Save
// 16-bit offsets
// from the starts of each block, marking the exits so that they can be nicely
// patched at any time.
//
// Blocks do NOT use call/ret, they only jmp to each other and to the dispatcher
// when necessary.
//
// All blocks that can be precompiled will be precompiled. Code will be memory
// protected - any write
// will mark
// the region as non-compilable, and all links to the page will be torn out and
// replaced with
// dispatcher jmps.
//
// Alternatively, icbi instruction SHOULD mark where we can't compile
//
// Seldom-happening events is handled by adding a decrement of a counter to all
// blr instructions
// (which are
// expensive anyway since we need to return to dispatcher, except when they can
// be predicted).

// TODO: SERIOUS synchronization problem with the video backend setting tokens
// and breakpoints in
// dual core mode!!!
//       Somewhat fixed by disabling idle skipping when certain interrupts are
//       enabled
//       This is no permanent reliable fix
// TODO: Zeldas go whacko when you hang the gfx thread

// Idea - Accurate exception handling
// Compute register state at a certain instruction by running the JIT in "dry
// mode", and stopping at
// the right place.
...
/*
  ...
  * Assume all floating point loads and double precision loads+stores are
  to/from main ram
    (single precision stores can be used in write gather pipe, specialized fast
  check added)
  * AMD only - use movaps instead of movapd when loading ps from memory?
  * HLE functions like floorf, sin, memcpy, etc - they can be much faster
  * ABI optimizations - drop F0-F13 on blr, for example. Watch out for context
  switching.
  ...
  * Track which registers a block clobbers without using, then take advantage of
  this knowledge
    when compiling a block that links to that block.
  * Track more dependencies between instructions, e.g. avoiding PPC_FP code,
  single/double
    conversion, movddup on non-paired singles, etc where possible.
  * Support loads/stores directly from xmm registers in jit_util and the
  backpatcher; this might
    help AMD a lot since gpr/xmm transfers are slower there.
  * Smarter register allocation in general; maybe learn to drop values once we
  know they won't be
    used again before being overwritten?
  * More flexible reordering; there's limits to how far we can go because of
  exception handling
    and such, but it's currently limited to integer ops only. This can
...
// doesn't look like there is a way to hook into a guard page fault and
// implement
// our own logic.
...
enum {
  ...
  GUARD_SIZE =
      0x10000, // two guards - bottom (permanent) and middle (see above)
...
#else
  // For windows we just keep using the system stack and reserve a large amount
...
bool Jit64::HandleStackFault() {
  // It's possible the stack fault might have been caused by something other
  // than
  // the BLR optimization. If the fault was triggered from another thread, or
  // when BLR optimization isn't enabled then there is nothing we can do about
  // the fault.
  // Return false so the regular stack overflow handler can trigger (which
  // crashes)
  if (!m_enable_blr_optimization || !Core::IsCPUThread())
    return false;

  WARN_LOG(POWERPC,
           "BLR cache disabled due to excessive BL in the emulated program.");
...
bool Jit64::HandleFault(uintptr_t access_address, SContext *ctx) {
  ...
  // In the trap region?
  if (m_enable_blr_optimization && diff >= GUARD_OFFSET &&
      diff < GUARD_OFFSET + GUARD_SIZE)
...
void Jit64::Init() {
  ...
  trampolines.Init(jo.memcheck ? TRAMPOLINE_CODE_SIZE_MMU
                               : TRAMPOLINE_CODE_SIZE);
  AllocCodeSpace(CODE_SIZE);

  // BLR optimization has the same consequences as block linking, as well as
  // depending on the fault handler to be safe in the event of excessive BL.
  m_enable_blr_optimization = jo.enableBlocklink &&
  ...
  // important: do this *after* generating the global asm routines, because we
  // can't use farcode in
  // them.
...
bool Jit64::Cleanup() {
  ...
  if (MMCR0.Hex || MMCR1.Hex) {
    ABI_PushRegistersAndAdjustStack({}, 0);
    ABI_CallFunctionCCC((void *)&PowerPC::UpdatePerformanceMonitor,
                        js.downcountAmount, js.numLoadStoreInst,
...
void Jit64::JustWriteExit(u32 destination, bool bl, u32 after) {
  // If nobody has taken care of this yet (this can be removed when all branches
  // are done)
  ...
  if (jo.enableBlocklink &&
      (block = blocks.GetBlockNumberFromStartAddress(destination)) >= 0) {
...
void Jit64::Trace() {
  ...
  DEBUG_LOG(DYNA_REC, "JIT64 PC: %08x SRR0: %08x SRR1: %08x FPSCR: %08x MSR: "
                      "%08x LR: %08x %s %s",
...
void Jit64::Jit(u32 em_address) {
  ...
  if (SConfig::GetInstance().bEnableDebugging) {
    // We can link blocks as long as we are not single stepping and there are no
    // breakpoints here
    EnableBlockLink();
    EnableOptimization();
  ...
  // Analyze the block, collect all instructions it is made of (including
  // inlining,
  // if that is enabled), reorder instructions for optimal performance, and join
  // joinable
  // instructions.
  u32 nextPC =
      analyzer.Analyze(em_address, &code_block, &code_buffer, blockSize);
...
const u8 *Jit64::DoJit(u32 em_address, PPCAnalyst::CodeBuffer *code_buf,
                       JitBlock *b, u32 nextPC) {
  ...
  const u8 *start = AlignCode4(); // TODO: Test if this or AlignCode16 make a
                                  // difference from GetCodePtr
  b->checkedEntry = start;
  b->runCount = 0;

  // Downcount flag check. The last block decremented downcounter, and the flag
  // should still be
  // available.
  ...
  if (ImHereDebug) {
    ABI_PushRegistersAndAdjustStack({}, 0);
    ABI_CallFunction((void *)&ImHere); // Used to get a trace of the last few
                                       // blocks before a crash,
  ...
  // If the block only uses one GQR and the GQR is zero at compile time, make a
  // guess that the block
  // never uses quantized loads/stores. Many paired-heavy games use largely
  // float loads and stores,
  // which are significantly faster when inlined (especially in MMU mode, where
  // this lets them use
  // fastmem).
  // Insert a check that the GQR is still zero at the start of the block in case
  // our guess turns out
  // wrong.
  // TODO: support any other constant GQR value, not merely zero/unquantized: we
  // can optimize
  // quantized
  // loadstores too, it'd just be more code.
  if (code_block.m_gqr_used.Count() == 1 &&
      js.pairedQuantizeAddresses.find(js.blockStart) ==
          js.pairedQuantizeAddresses.end()) {
    ...
    if (!code_block.m_gqr_modified[gqr] && !GQR(gqr)) {
      ...
      ABI_PushRegistersAndAdjustStack({}, 0);
      ABI_CallFunctionC(
          (void *)&JitInterface::CompileExceptionCheck,
  ...
  for (u32 i = 0; i < code_block.m_num_instructions; i++) {
    ...
    // Gather pipe writes using a non-immediate address are discovered by
    // profiling.
    bool gatherPipeIntCheck = js.fifoWriteAddresses.find(ops[i].address) !=
                              js.fifoWriteAddresses.end();
    ...
    if (gatherPipeIntCheck) {
      ...
      TEST(32, M(&ProcessorInterface::m_InterruptCause),
           Imm32(ProcessorInterface::INT_CAUSE_CP |
    ...
    if (!ops[i].skip) {
      ...
      if (SConfig::GetInstance().bEnableDebugging &&
        ...
        // Turn off block linking if there are breakpoints so that the Step Over
        // command does not
        // link this block.
        jo.enableBlocklink = false;
      ...
      // If we have an input register that is going to be used again, load it
      // pre-emptively,
      // even if the instruction doesn't strictly need it in a register, to
      // avoid redundant
      // loads later. Of course, don't do this if we're already out of
      // registers.
      // As a bit of a heuristic, make sure we have at least one register left
      // over for the
      // output, which needs to be bound in the actual instruction compilation.
      // TODO: make this smarter in the case that we're actually
      // register-starved, i.e.
      ...
      if (jo.memcheck && (opinfo->flags & FL_LOADSTORE)) {
        // If we have a fastmem loadstore, we can omit the exception check and
        // let fastmem handle
        // it.
        FixupBranch memException;
        _assert_msg_(DYNA_REC,
                     !(js.fastmemLoadStore && js.fixupExceptionHandler),
                     "Fastmem loadstores shouldn't have exception handler "
                     "fixups (PC=%x)!",
                     ops[i].address);
        ...
        if (!js.fastmemLoadStore) {
          ...
          SetJumpTarget(js.fixupExceptionHandler ? js.exceptionHandler
                                                 : memException);
        } else {
    ...
    if (gpr.SanityCheck() || fpr.SanityCheck()) {
      std::string ppc_inst =
          GekkoDisassembler::Disassemble(ops[i].inst.hex, em_address);
