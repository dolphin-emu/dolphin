...
//TODO
	...
	for (u32 i = 0; i < code_block.m_num_instructions; i++)
	{
		...
		if (!ops[i].skip)
		{
			...
			// TODO: support this in float mode; it will work now, but it can hurt because if we're
			// doing a store, we don't want to load an XMM register only to pass it back to a GPR
			// for the store (slower, more instructions).
