...
enum InstructionFlags : u64
{
  ...
  FL_IN_S = (1ull << 11),  // Uses rS as an input.
  FL_IN_AB = FL_IN_A | FL_IN_B,
  FL_IN_SB = FL_IN_S | FL_IN_B,
  FL_IN_A0B = FL_IN_A0 | FL_IN_B,
  FL_OUT_D = (1ull << 12),  // rD is used as a destination.
  FL_OUT_A = (1ull << 13),  // rA is used as a destination.
  FL_OUT_AD = FL_OUT_A | FL_OUT_D,
  FL_TIMER = (1ull << 14),            // Used only for mftb.
  FL_CHECKEXCEPTIONS = (1ull << 15),  // Used with rfi/rfid.
  FL_EVIL =
      (1ull << 16),  // Historically used to refer to instructions that messed up Super Monkey Ball.
  FL_USE_FPU = (1ull << 17),     // Used to indicate a floating point instruction.
  FL_LOADSTORE = (1ull << 18),   // Used to indicate a load/store instruction.
  FL_SET_FPRF = (1ull << 19),    // Sets bits in the FPRF.
  FL_READ_FPRF = (1ull << 20),   // Reads bits from the FPRF.
  FL_SET_OE = (1ull << 21),      // Sets the overflow flag.
  FL_IN_FLOAT_A = (1ull << 22),  // frA is used as an input.
  FL_IN_FLOAT_B = (1ull << 23),  // frB is used as an input.
  FL_IN_FLOAT_C = (1ull << 24),  // frC is used as an input.
  FL_IN_FLOAT_S = (1ull << 25),  // frS is used as an input.
  FL_IN_FLOAT_D = (1ull << 26),  // frD is used as an input.
  ...
  FL_OUT_FLOAT_D = (1ull << 27),  // frD is used as a destination.
  // Used in the case of double ops (they don't modify the top half of the output)
  FL_INOUT_FLOAT_D = FL_IN_FLOAT_D | FL_OUT_FLOAT_D,
  FL_IN_FLOAT_A_BITEXACT = (1ull << 28),  // The output is based on the exact bits in frA.
  FL_IN_FLOAT_B_BITEXACT = (1ull << 29),  // The output is based on the exact bits in frB.
  FL_IN_FLOAT_C_BITEXACT = (1ull << 30),  // The output is based on the exact bits in frC.
  ...
  FL_PROGRAMEXCEPTION = (1ull << 31),  // May generate a program exception (not floating point).
  FL_FLOAT_EXCEPTION = (1ull << 32),   // May generate a program exception (floating point).
  FL_FLOAT_DIV = (1ull << 33),  // May generate a program exception (FP) due to division by 0.
};
