...
enum {
  ...
  FL_RC_BIT_F = (1 << 6), // Sets the record bit. Used for floating point
                          // instructions that do this.
  FL_ENDBLOCK = (1 << 7), // Specifies that the instruction can be used as an
                          // exit point for a JIT block.
  FL_IN_A = (1 << 8),     // Uses rA as an input.
  FL_IN_A0 = (1 << 9), // Uses rA as an input. Indicates that if rA is zero, the
                       // value zero is
  ...
  FL_EVIL = (1 << 17), // Historically used to refer to instructions that messed
                       // up Super Monkey Ball.
  ...
  // Used in the case of double ops (they don't modify the top half of the
  // output)
