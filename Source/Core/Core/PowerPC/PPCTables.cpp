...
const u64 m_crTable[16] = {
    PPCCRToInternal(0x0), PPCCRToInternal(0x1), PPCCRToInternal(0x2),
    PPCCRToInternal(0x3), PPCCRToInternal(0x4), PPCCRToInternal(0x5),
    PPCCRToInternal(0x6), PPCCRToInternal(0x7), PPCCRToInternal(0x8),
    PPCCRToInternal(0x9), PPCCRToInternal(0xA), PPCCRToInternal(0xB),
    PPCCRToInternal(0xC), PPCCRToInternal(0xD), PPCCRToInternal(0xE),
    PPCCRToInternal(0xF),
};
...
GekkoOPInfo *GetOpInfo(UGeckoInstruction _inst) {
  ...
  if (info->type == OPTYPE_SUBTABLE) {
    ...
    default:
      _assert_msg_(POWERPC, 0, "GetOpInfo - invalid subtable op %08x @ %08x",
                   _inst.hex, PC);
      return nullptr;
    }
  } else {
    if (info->type == OPTYPE_INVALID) {
      _assert_msg_(POWERPC, 0, "GetOpInfo - invalid op %08x @ %08x", _inst.hex,
                   PC);
      return nullptr;
    }
  ...
  if (info->type == OPTYPE_SUBTABLE) {
    ...
    default:
      _assert_msg_(POWERPC, 0,
                   "GetInterpreterOp - invalid subtable op %08x @ %08x",
                   _inst.hex, PC);
      return nullptr;
    }
  } else {
    if (info->type == OPTYPE_INVALID) {
      _assert_msg_(POWERPC, 0, "GetInterpreterOp - invalid op %08x @ %08x",
                   _inst.hex, PC);
      return nullptr;
    }
...
void PrintInstructionRunCounts() {
  ...
  std::sort(temp.begin(), temp.end(), [](const OpInfo &a, const OpInfo &b) {
...
void LogCompiledInstructions() {
  ...
  File::IOFile f(StringFromFormat("%sinst_log%i.txt",
                                  File::GetUserPath(D_LOGS_IDX).c_str(), time),
                 "w");
  for (int i = 0; i < m_numInstructions; i++) {
    GekkoOPInfo *pInst = m_allInstructions[i];
  ...
  f.Open(StringFromFormat("%sinst_not%i.txt",
                          File::GetUserPath(D_LOGS_IDX).c_str(), time),
         "w");
  for (int i = 0; i < m_numInstructions; i++) {
    GekkoOPInfo *pInst = m_allInstructions[i];
...
#ifdef OPLOG
  f.Open(StringFromFormat("%s" OP_TO_LOG "_at%i.txt",
                          File::GetUserPath(D_LOGS_IDX).c_str(), time),
         "w");
