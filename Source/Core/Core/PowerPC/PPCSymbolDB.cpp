...
void PPCSymbolDB::AddKnownSymbol(u32 startAddr, u32 size,
  ...
  if (iter != functions.end()) {
    ...
    tempfunc->name = name;
    tempfunc->hash =
        SignatureDB::ComputeCodeChecksum(startAddr, startAddr + size - 4);
    tempfunc->type = type;
    tempfunc->size = size;
  } else {
  ...
  for (auto &entry : functions) {
    ...
    for (const SCall &call : f.calls) {
      ...
      if (FuncIterator != functions.end()) {
        ...
        // LOG(OSHLE, "FillInCallers tries to fill data in an unknown function
        // 0x%08x.",
        // FunctionAddress);
...
void PPCSymbolDB::PrintCalls(u32 funcAddr) const {
  ...
  if (iter != functions.end()) {
    ...
    INFO_LOG(OSHLE, "The function %s at %08x calls:", f.name.c_str(),
             f.address);
    for (const SCall &call : f.calls) {
      XFuncMap::const_iterator n = functions.find(call.function);
      if (n != functions.end()) {
        INFO_LOG(CONSOLE, "* %08x : %s", call.callAddress,
                 n->second.name.c_str());
      }
    }
  } else {
...
void PPCSymbolDB::PrintCallers(u32 funcAddr) const {
  ...
  if (iter != functions.end()) {
    const Symbol &f = iter->second;
    INFO_LOG(CONSOLE, "The function %s at %08x is called by:", f.name.c_str(),
             f.address);
    for (const SCall &caller : f.callers) {
      XFuncMap::const_iterator n = functions.find(caller.function);
      if (n != functions.end()) {
        INFO_LOG(CONSOLE, "* %08x : %s", caller.callAddress,
                 n->second.name.c_str());
      }
    }
  }
...
// The use case for handling bad map files is when you have a game with a map
// file on the disc,
// but you can't tell whether that map file is for the particular release
// version used in that game,
// or when you know that the map file is not for that build, but perhaps half
// the functions in the
// map file are still at the correct locations. Which are both common
// situations. It will load any
// function names and addresses that have a BLR before the start and at the end,
// but ignore any that
// don't, and then tell you how many were good and how many it ignored. That way
// you either find out
// it is all good and use it, find out it is partly good and use the good part,
// or find out that
// only
// a handful of functions lined up by coincidence and then you can clear the
// symbols. In the future
// I
// want to make it smarter, so it checks that there are no BLRs in the middle of
// the function
// (by checking the code length), and also make it cope with added functions in
// the middle or work
// based on the order of the functions and their approximate length. Currently
// that process has to
// be
// done manually and is very tedious.
// The use case for separate handling of map files that aren't bad is that you
// usually want to also
// load names that aren't functions(if included in the map file) without them
// being rejected as
// invalid.
...
// bad=true means carefully load map files that might not be from exactly the
// right version
bool PPCSymbolDB::LoadMap(const std::string &filename, bool bad) {
  ...
  // four columns are used in American Mensa Academy map files and perhaps other
  // games
  ...
  while (fgets(line, 512, f.GetHandle())) {
    ...
    if (length == 34 &&
        strcmp(line, "  address  Size   address  offset\n") == 0) {
      four_columns = true;
      continue;
    }
    ...
    if (four_columns) {
      // sometimes there is no unknown number, and sometimes it is because it is
      // an entry of
      // something else
      if (length > 37 && line[37] == ' ') {
        unknown = 0;
        sscanf(line, "%08x %08x %08x %08x %511s", &address, &size, &vaddress,
               &offset, name);
        ...
        sscanf(line, "%08x %08x %08x %08x %i %511s", &address, &size, &vaddress,
               &offset, &unknown, name);
      }
    }
    // some entries in the table have a function name followed by " (entry of "
    // followed by a
    // container name, followed by ")"
    // instead of a space followed by a number followed by a space followed by a
    // name
      ...
      sscanf(line, "%08x %08x %08x %i %511s", &address, &size, &vaddress,
             &unknown, name);
    }
    ...
    // we want the function names only .... TODO: or do we really? aren't we
    // wasting information
    // here?
  ...
  Index();
  if (bad)
    SuccessAlertT("Loaded %d good functions, ignored %d bad functions.",
...
bool PPCSymbolDB::SaveMap(const std::string &filename, bool WithCodes) const {
  ...
  if (functions.size() == 0) {
    if (!AskYesNo(StringFromFormat("No symbol names are generated. Do you want "
                                   "to replace '%s' with a blank file?",
                                   mapFile.c_str())
                      .c_str(),
                  "Confirm", wxYES_NO))
      return false;
  ...
  while (itr != functions.end()) {
    ...
    if (!WithCodes) {
      fprintf(f.GetHandle(), "%08x %08x %08x %i %s\n", rSymbol.address,
              rSymbol.size, rSymbol.address, 0, rSymbol.name.c_str());
      ++itr;
    }
    ...
    // Save a code file
      ...
      /* To make nice straight lines we fill out the name with spaces, we also
         cut off
