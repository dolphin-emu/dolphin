...
static void ChangeStateFromRunning(CPUState new_state, bool synchronize)
{
	...
	{
		// The CPU Thread is still inside RunLoop so we need to wait for it to leave.
		unsigned int cookie = s_state_run_loop_leave_counter;
		// FIXME: Timeout is a hack to contend with the Core deadlocking itself or
		//   using MsgHandler to deadlock against the GUI Thread. It should be removed.
		bool success = s_state_change_cvar.wait_for(guard, std::chrono::seconds(5), [&]
		{
			return s_state_run_loop_leave_counter != cookie;
		});
		if (!success)
			ERROR_LOG(POWERPC, "Gave up waiting for CPU Thread early.");
	}
}
