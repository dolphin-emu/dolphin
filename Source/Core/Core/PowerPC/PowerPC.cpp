...
void DoState(PointerWrap &p) {
  ...
  // it changes registers even in MODE_MEASURE (which is suspicious and seems
  // like it could cause
  // desyncs)
  // and because the values it's changing have been added to
  // CoreTiming::DoState, so it might
  // conflict to mess with them here.

  // rSPR(SPR_DEC) = SystemTimers::GetFakeDecrementer();
  // *((u64 *)&TL) = SystemTimers::GetFakeTimeBase(); //works since we are
  // little endian and TL
  // comes first :)
...
static void ResetRegisters() {
  ...
  */
  ...
  // MSR should be 0x40, but we don't emulate BS1, so it would never be turned
  // off :}
  ppcState.msr = 0;
  rDEC = 0xFFFFFFFF;
  SystemTimers::DecrementerSet();
...
void Init(int cpu_core) {
  ...
  default:
    ...
    {
      WARN_LOG(POWERPC, "Jit core %d not available. Defaulting to interpreter.",
               cpu_core);
      s_cpu_core_base = s_interpreter;
    }
    break;
...
static void ApplyMode() {
  ...
  case MODE_JIT: // Switching from interpreter to JIT.
    ...
    if (!s_cpu_core_base) // Has a chance to not get a working JIT core if one
                          // isn't active on host
      s_cpu_core_base = s_interpreter;
    break;
  }
}
...
const char *GetCPUName() { return s_cpu_core_base->GetName(); }
...
void SingleStep() { s_cpu_core_base->SingleStep(); }
...
void UpdatePerformanceMonitor(u32 cycles, u32 num_load_stores,
                              u32 num_fp_inst) {
  ...
  if ((MMCR0.PMC1INTCONTROL &&
       (PowerPC::ppcState.spr[SPR_PMC1] & 0x80000000) != 0) ||
      (MMCR0.PMCINTCONTROL &&
       (PowerPC::ppcState.spr[SPR_PMC2] & 0x80000000) != 0) ||
      (MMCR0.PMCINTCONTROL &&
       (PowerPC::ppcState.spr[SPR_PMC3] & 0x80000000) != 0) ||
      (MMCR0.PMCINTCONTROL &&
       (PowerPC::ppcState.spr[SPR_PMC4] & 0x80000000) != 0))
...
void CheckExceptions() {
  ...
  // MSR &= ~0x04EF36; // 0x04FF36 also clears ME (only for machine check
  // exception)
  ...
  else if (MSR & 0x0008000) // Handling is delayed until MSR.EE=1.
  {
    if (exceptions & EXCEPTION_EXTERNAL_INT) {
      ...
      _dbg_assert_msg_(POWERPC, (SRR1 & 0x02) != 0,
...
void CheckExternalExceptions() {
  ...
  if (exceptions && (MSR & 0x0008000)) // Handling is delayed until MSR.EE=1.
  {
    if (exceptions & EXCEPTION_EXTERNAL_INT) {
      ...
      _dbg_assert_msg_(POWERPC, 0, "Unknown EXT interrupt: Exceptions == %08x",
                       exceptions);
      ERROR_LOG(POWERPC,
                "Unknown EXTERNAL INTERRUPT exception: Exceptions == %08x",
                exceptions);
    }
  }
}
