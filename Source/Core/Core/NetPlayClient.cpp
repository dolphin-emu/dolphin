...
// called from ---GUI--- thread
NetPlayClient::NetPlayClient(const std::string &address, const u16 port,
                             NetPlayUI *dialog, const std::string &name,
                             bool traversal, const std::string &centralServer,
                             u16 centralPort)
...
bool NetPlayClient::Connect() {
  ...
  ENetEvent netEvent;
  if (enet_host_service(m_client, &netEvent, 5000) > 0 &&
...
void NetPlayClient::GetPlayerList(std::string &list,
  ...
  const auto enumerate_player_controller_mappings =
      [&ss](const PadMappingArray &mappings, const Player &player) {
  ...
  for (const auto &entry : m_players) {
    ...
    ss << player.name << "[" << static_cast<int>(player.pid)
       << "] : " << player.revision << " | ";
...
void NetPlayClient::SendPadState(const PadMapping in_game_pad,
                                 const GCPadStatus &pad) {
  auto spac = std::make_unique<sf::Packet>();
  *spac << static_cast<MessageId>(NP_MSG_PAD_DATA);
  *spac << in_game_pad;
  *spac << pad.button << pad.analogA << pad.analogB << pad.stickX << pad.stickY
        << pad.substickX << pad.substickY << pad.triggerLeft
        << pad.triggerRight;
...
// called from ---CPU--- thread
void NetPlayClient::SendWiimoteState(const PadMapping in_game_pad,
                                     const NetWiimote &nw) {
  auto spac = std::make_unique<sf::Packet>();
...
bool NetPlayClient::StartGame(const std::string &path) {
  ...
  if (SConfig::GetInstance().bWii) {
    for (unsigned int i = 0; i < 4; ++i)
      WiimoteReal::ChangeWiimoteSource(
          i, m_wiimote_map[i] > 0 ? WIIMOTE_SRC_EMU : WIIMOTE_SRC_NONE);

    // Needed to prevent locking up at boot if (when) the wiimotes connect out
    // of order.
    NetWiimote nw;
    nw.resize(4, 0);
...
// called from ---GUI--- thread
bool NetPlayClient::ChangeGame(const std::string &) { return true; }
...
void NetPlayClient::UpdateDevices() {
  ...
  for (auto player_id : m_pad_map) {
    ...
    if (player_id == m_local_player->pid) {
      if (SIDevice_IsGCController(
              SConfig::GetInstance().m_SIDevice[local_pad])) {
        SerialInterface::AddDevice(SConfig::GetInstance().m_SIDevice[local_pad],
                                   pad);
      } else {
...
void NetPlayClient::OnTraversalStateChanged() {
  if (m_connection_state ==
          ConnectionState::WaitingForTraversalClientConnection &&
...
bool NetPlayClient::WiimoteUpdate(int _number, u8 *data, const u8 size) {
  ...
  while (m_wiimote_current_data_size[_number] == size &&
  ...
  if (nw.size() != size) {
    ...
    if (size != nw.size()) {
      PanicAlertT(
          "Netplay has desynced. There is no way to recover from this.");
      return false;
    }
  }
...
bool NetPlayClient::LocalPlayerHasControllerMapped() const {
  ...
  return std::any_of(m_pad_map.begin(), m_pad_map.end(),
                     mapping_matches_player_id) ||
         std::any_of(m_wiimote_map.begin(), m_wiimote_map.end(),
                     mapping_matches_player_id);
}
...
u8 NetPlayClient::NumLocalPads() const {
  return static_cast<u8>(
      std::count_if(m_pad_map.begin(), m_pad_map.end(), [this](auto mapping) {
        return mapping == m_local_player->pid;
      }));
}
...
bool CSIDevice_GCController::NetPlay_GetInput(u8 numPAD,
...
bool WiimoteEmu::Wiimote::NetPlay_GetWiimoteData(int wiimote, u8 *data,
                                                 u8 size) {
  std::lock_guard<std::mutex> lk(crit_netplay_client);
...
bool NetPlay::IsNetPlayRunning() { return netplay_client != nullptr; }
