...
enum PlayMode { MODE_NONE = 0, MODE_RECORDING, MODE_PLAYING };
...
static_assert(sizeof(ControllerState) == 8,
              "ControllerState should be 8 bytes");
#pragma pack(pop)
...
bool PlayWiimote(int wiimote, u8 *data,
                 const struct WiimoteEmu::ReportFeatures &rptf, int ext,
...
typedef void (*WiiManipFunction)(u8 *, WiimoteEmu::ReportFeatures, int, int,
                                 wiimote_key);

void SetGCInputManip(GCManipFunction);
void SetWiiInputManip(WiiManipFunction);
void CallGCInputManip(GCPadStatus *PadStatus, int controllerID);
void CallWiiInputManip(u8 *core, WiimoteEmu::ReportFeatures rptf,
