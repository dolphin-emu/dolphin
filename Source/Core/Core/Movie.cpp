...
static bool IsMovieHeader(u8 magic[4]) {
  return magic[0] == 'D' && magic[1] == 'T' && magic[2] == 'M' &&
         magic[3] == 0x1A;
}

static std::array<u8, 20>
ConvertGitRevisionToBytes(const std::string &revision) {
  std::array<u8, 20> revision_bytes{};

  if (revision.size() % 2 == 0 &&
      std::all_of(revision.begin(), revision.end(), ::isxdigit)) {
    ...
    size_t bytes_to_write =
        std::min(revision.size() / 2, revision_bytes.size());
    unsigned int temp;
    ...
    std::copy_n(std::begin(revision), bytes_to_write,
                std::begin(revision_bytes));
  }
...
void Init() {
  ...
  if (IsPlayingInput()) {
    ...
    md5thread.detach();
    if (strncmp(tmpHeader.gameID, SConfig::GetInstance().GetUniqueID().c_str(),
                6)) {
      PanicAlertT(
          "The recorded game (%s) is not the same as the selected game (%s)",
...
// NOTE: CPU Thread
void SetPolledDevice() { s_bPolled = true; }
...
void SetReadOnly(bool bEnabled) {
  if (s_bReadOnly != bEnabled)
    Core::DisplayMessage(bEnabled ? "Read-only mode." : "Read+Write mode.",
                         1000);
...
void FrameSkipping() {
  ...
  if (!IsMovieActive() || NetPlay::IsNetPlayRunning()) {
    ...
    s_frameSkipCounter++;
    if (s_frameSkipCounter > s_framesToSkip ||
        Core::ShouldSkipFrame(s_frameSkipCounter) == false)
      s_frameSkipCounter = 0;
...
bool IsRecordingInput() { return (s_playMode == MODE_RECORDING); }

bool IsRecordingInputFromSaveState() { return s_bRecordingFromSaveState; }
...
bool IsJustStartingPlayingInputFromSaveState() {
  return IsRecordingInputFromSaveState() && g_currentFrame == 1 &&
         IsPlayingInput();
}

bool IsPlayingInput() { return (s_playMode == MODE_PLAYING); }

bool IsMovieActive() { return s_playMode != MODE_NONE; }

bool IsReadOnly() { return s_bReadOnly; }

u64 GetRecordingStartTime() { return s_recordingStartTime; }
...
bool IsConfigSaved() { return s_bSaveConfig; }
bool IsDualCore() { return s_bDualCore; }

bool IsProgressive() { return s_bProgressive; }

bool IsPAL60() { return s_bPAL60; }

bool IsSkipIdle() { return s_bSkipIdle; }

bool IsDSPHLE() { return s_bDSPHLE; }

bool IsFastDiscSpeed() { return s_bFastDiscSpeed; }

int GetCPUMode() { return s_iCPUCore; }

u8 GetLanguage() { return s_language; }

bool IsStartingFromClearSave() { return g_bClearSave; }

bool IsUsingMemcard(int memcard) { return (s_memcards & (1 << memcard)) != 0; }
bool IsSyncGPU() { return s_bSyncGPU; }

bool IsNetPlayRecording() { return s_bNetPlay; }
  ...
  for (int i = 0; i < MAX_SI_CHANNELS; ++i) {
    ...
    if (IsUsingPad(i)) {
      if (SIDevice_IsGCController(SConfig::GetInstance().m_SIDevice[i]))
        ...
        device =
            IsUsingBongo(i) ? SIDEVICE_GC_TARUKONGA : SIDEVICE_GC_CONTROLLER;
    }
...
void ChangeWiiPads(bool instantly) {
  ...
  // This is important for Wiimotes, because they can desync easily if they get
  // re-activated
  ...
  for (int i = 0; i < MAX_WIIMOTES; ++i) {
    g_wiimote_sources[i] =
        IsUsingWiimote(i) ? WIIMOTE_SRC_EMU : WIIMOTE_SRC_NONE;
...
bool BeginRecordingInput(int controllers) {
  ...
  if (Core::IsRunningAndStarted()) {
    const std::string save_path =
        File::GetUserPath(D_STATESAVES_IDX) + "dtm.sav";
    ...
    // This is only done here if starting from save state because otherwise we
    // won't have the
    // titleid. Otherwise it's set in WII_IPC_HLE_Device_es.cpp.
    // TODO: find a way to GetTitleDataPath() from Movie::Init()
    if (SConfig::GetInstance().bWii) {
      if (File::Exists(
              Common::GetTitleDataPath(g_titleID, Common::FROM_SESSION_ROOT) +
              "banner.bin"))
...
static std::string Analog2DToString(u8 x, u8 y, const std::string &prefix,
  ...
  if ((x <= 1 || x == center || x >= range) &&
    ...
    if (x != center || y != center) {
      if (x != center && y != center) {
        return StringFromFormat("%s:%s,%s", prefix.c_str(),
                                x < center ? "LEFT" : "RIGHT",
                                y < center ? "DOWN" : "UP");
      } else if (x != center) {
        return StringFromFormat("%s:%s", prefix.c_str(),
                                x < center ? "LEFT" : "RIGHT");
      } else {
        return StringFromFormat("%s:%s", prefix.c_str(),
                                y < center ? "DOWN" : "UP");
...
static void SetInputDisplayString(ControllerState padState, int controllerID) {
  ...
  display_str +=
      Analog2DToString(padState.AnalogStickX, padState.AnalogStickY, " ANA");
...
static void SetWiiInputDisplayString(int remoteID, u8 *const data,
  ...
  if (accelData) {
    ...
    display_str += StringFromFormat(
        " ACC:%d,%d,%d", dt->x << 2 | ((wm_buttons *)coreData)->acc_x_lsb,
  ...
  if (extData && ext == 1) {
    ...
    std::string accel = StringFromFormat(
        ...
        (nunchuk.ay << 2) | nunchuk.bt.acc_y_lsb,
        (nunchuk.az << 2) | nunchuk.bt.acc_z_lsb);
  ...
  if (extData && ext == 2) {
    ...
    display_str +=
        Analog2DToString(cc.regular_data.lx, cc.regular_data.ly, " ANA", 63);
    display_str += Analog2DToString(cc.rx1 | (cc.rx2 << 1) | (cc.rx3 << 3),
                                    cc.ry, " R-ANA", 31);
  }
...
// NOTE: CPU Thread
void CheckWiimoteStatus(int wiimote, u8 *data,
...
void DoState(PointerWrap &p) {
  ...
  // other variables (such as s_totalBytes and g_totalFrames) are set in
  // LoadInput
}
...
void LoadInput(const std::string &filename) {
  ...
  if (!IsMovieHeader(tmpHeader.filetype)) {
    PanicAlertT("Savestate movie %s is corrupted, movie recording stopping...",
                filename.c_str());
    EndPlayInput(false);
    return;
  }
  ...
  if (s_currentByte > totalSavedBytes) {
    PanicAlertT("Warning: You loaded a save whose movie ends before the "
                ...
                "before continuing.",
                (u32)totalSavedBytes + 256, (u32)s_currentByte + 256,
                (u32)tmpHeader.frameCount, (u32)g_currentFrame);
    afterEnd = true;
  }
  ...
  if (!s_bReadOnly || tmpInput == nullptr) {
    ...
    if (s_currentByte > totalSavedBytes) {
      ...
      afterEnd = true;
      PanicAlertT("Warning: You loaded a save that's after the end of the "
                  "current movie. (byte %u "
                  "> %u) (frame %u > %u). You should load another save before "
                  "continuing, or load "
                  "this state with read-only mode off.",
                  (u32)s_currentByte + 256, (u32)s_totalBytes + 256,
                  (u32)g_currentFrame, (u32)g_totalFrames);
      ...
      for (u32 i = 0; i < len; ++i) {
        if (movInput[i] != tmpInput[i]) {
          ...
          // we'll try to say what's going on in excruciating detail, otherwise
          // the user might not
          // believe us.
          if (IsUsingWiimote(0)) {
            // TODO: more detail
            PanicAlertT(
                "Warning: You loaded a save whose movie mismatches on byte %d "
                "(0x%X). You "
                "should load another save before continuing, or load this "
                "state with "
            ...
            PanicAlertT(
                "Warning: You loaded a save whose movie mismatches on frame "
                "%d. You should load "
                "another save before continuing, or load this state with "
                "read-only mode off. "
                "Otherwise you'll probably get a desync.\n\n"
                "More information: The current movie is %d frames long and the "
                "savestate's movie "
                "is %d frames long.\n\n"
                "On frame %d, the current movie presses:\n"
                "Start=%d, A=%d, B=%d, X=%d, Y=%d, Z=%d, DUp=%d, DDown=%d, "
                "DLeft=%d, DRight=%d, "
                ...
                "Start=%d, A=%d, B=%d, X=%d, Y=%d, Z=%d, DUp=%d, DDown=%d, "
                "DLeft=%d, DRight=%d, "
                "L=%d, R=%d, LT=%d, RT=%d, AnalogX=%d, AnalogY=%d, CX=%d, "
                "CY=%d",
                (int)frame, (int)g_totalFrames, (int)tmpHeader.frameCount,
                (int)frame, (int)curPadState.Start, (int)curPadState.A,
                (int)curPadState.B, (int)curPadState.X, (int)curPadState.Y,
                (int)curPadState.Z, (int)curPadState.DPadUp,
                (int)curPadState.DPadDown, (int)curPadState.DPadLeft,
                (int)curPadState.DPadRight, (int)curPadState.L,
                (int)curPadState.R, (int)curPadState.TriggerL,
                (int)curPadState.TriggerR, (int)curPadState.AnalogStickX,
                (int)curPadState.AnalogStickY, (int)curPadState.CStickX,
                (int)curPadState.CStickY, (int)frame, (int)movPadState.Start,
                (int)movPadState.A, (int)movPadState.B, (int)movPadState.X,
                (int)movPadState.Y, (int)movPadState.Z, (int)movPadState.DPadUp,
                (int)movPadState.DPadDown, (int)movPadState.DPadLeft,
                (int)movPadState.DPadRight, (int)movPadState.L,
                (int)movPadState.R, (int)movPadState.TriggerL,
                (int)movPadState.TriggerR, (int)movPadState.AnalogStickX,
                (int)movPadState.AnalogStickY, (int)movPadState.CStickX,
                (int)movPadState.CStickY);
          }
          break;
        }
      }
...
static void CheckInputEnd() {
  if (g_currentFrame > g_totalFrames || s_currentByte >= s_totalBytes ||
      (CoreTiming::GetTicks() > s_totalTickCount &&
...
void PlayController(GCPadStatus *PadStatus, int controllerID) {
  ...
  if (s_currentByte + 8 > s_totalBytes) {
    PanicAlertT("Premature movie end in PlayController. %u + 8 > %u",
                (u32)s_currentByte, (u32)s_totalBytes);
    EndPlayInput(!s_bReadOnly);
    return;
  }
  ...
  if (s_padState.disc) {
    ...
    // to change more than
    // that will cause
    // it to load the last disc every time. As far as i know though, there are
    // no 3+ disc games, so
    // this should be fine.
    CPU::Break();
    bool found = false;
...
bool PlayWiimote(int wiimote, u8 *data, const WiimoteEmu::ReportFeatures &rptf,
  ...
  if (s_currentByte > s_totalBytes) {
    PanicAlertT("Premature movie end in PlayWiimote. %u > %u",
                (u32)s_currentByte, (u32)s_totalBytes);
    EndPlayInput(!s_bReadOnly);
    return false;
  }
  ...
  if (size != sizeInMovie) {
    PanicAlertT(
        "Fatal desync. Aborting playback. (Error in PlayWiimote: %u != %u, "
        "byte %u.)%s",
  ...
  if (s_currentByte + size > s_totalBytes) {
    PanicAlertT("Premature movie end in PlayWiimote. %u + %d > %u",
                (u32)s_currentByte, size, (u32)s_totalBytes);
    EndPlayInput(!s_bReadOnly);
    return false;
  }
...
void SaveRecording(const std::string &filename) {
  ...
  header.numControllers =
      s_numPads & (SConfig::GetInstance().bWii ? 0xFF : 0x0F);
  ...
  strncpy((char *)header.videoBackend, s_videoBackend.c_str(),
          ArraySize(header.videoBackend));
  ...
  strncpy((char *)header.discChange, g_discChange.c_str(),
          ArraySize(header.discChange));
  ...
  if (success && s_bRecordingFromSaveState) {
    ...
    success = File::Copy(File::GetUserPath(D_STATESAVES_IDX) + "dtm.sav",
                         stateFilename);
  }

  if (success)
    Core::DisplayMessage(StringFromFormat("DTM %s saved", filename.c_str()),
                         2000);
  else
    Core::DisplayMessage(
        StringFromFormat("Failed to save %s", filename.c_str()), 2000);
}

void SetGCInputManip(GCManipFunction func) { gcmfunc = func; }
void SetWiiInputManip(WiiManipFunction func) { wiimfunc = func; }
...
// NOTE: CPU Thread
void CallWiiInputManip(u8 *data, WiimoteEmu::ReportFeatures rptf,
...
static const mbedtls_md_info_t *s_md5_info =
...
void CheckMD5() {
  ...
  mbedtls_md_file(s_md5_info, SConfig::GetInstance().m_strFilename.c_str(),
                  gameMD5);

  if (memcmp(gameMD5, s_MD5, 16) == 0)
    Core::DisplayMessage("Checksum of current game matches the recorded game.",
                         2000);
  else
    Core::DisplayMessage(
        "Checksum of current game does not match the recorded game!", 3000);
}
...
// NOTE: EmuThread
void Shutdown() {
  g_currentInputCount = g_totalInputCount = g_totalFrames = s_totalBytes =
      s_tickCountAtLastInput = 0;
  delete[] tmpInput;
  tmpInput = nullptr;
  tmpInputAllocated = 0;
