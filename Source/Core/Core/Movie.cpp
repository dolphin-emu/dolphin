	...
	for (int i = 0; i < MAX_SI_CHANNELS; ++i)
		if (SIDevice_IsGCController(SConfig::GetInstance().m_SIDevice[i]))
		{
			if (instantly) // Changes from savestates need to be instantaneous
				SerialInterface::AddDevice(IsUsingPad(i) ? SConfig::GetInstance().m_SIDevice[i] : SIDEVICE_NONE, i);
			else
				SerialInterface::ChangeDevice(IsUsingPad(i) ? SConfig::GetInstance().m_SIDevice[i] : SIDEVICE_NONE, i);
		}
		else
		{
