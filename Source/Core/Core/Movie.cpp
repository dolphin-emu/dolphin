...
void ChangePads(bool instantly)
{
	...
	for (int i = 0; i < MAX_SI_CHANNELS; ++i)
	{
		SIDevices device;
		if (SIDevice_IsGCController(SConfig::GetInstance().m_SIDevice[i]))
			device = IsUsingPad(i) ? SConfig::GetInstance().m_SIDevice[i] : SIDEVICE_NONE;
		else
			device = IsUsingPad(i) ? (IsUsingBongo(i) ? SIDEVICE_GC_TARUKONGA : SIDEVICE_GC_CONTROLLER) : SIDEVICE_NONE;
