...
static bool LoadFirmware(LibUSBBluetoothAdapter& adapter, std::span<const char> data)
{
  ...
  while (data.end() - block_iter > rtl_download_cmd::DATA_LEN)
  {
    ...
    std::ranges::copy(block_iter, block_end, payload.command.data.data());
  ...
  const auto bytes_short_of_full_block = payload.command.DATA_LEN - last_block_size;

  // Mark data end.
  payload.command.index |= 0x80u;
  payload.header.length -= bytes_short_of_full_block;
  std::ranges::copy(block_iter, data.end(), payload.command.data.data());
