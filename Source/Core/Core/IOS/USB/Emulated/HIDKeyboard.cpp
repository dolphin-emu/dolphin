  ...
  case USBHDR(DIR_HOST2DEVICE, TYPE_CLASS, REC_INTERFACE, HIDRequestCodes::SET_IDLE):
  {
    ...
    if (duration == 0)
    {
      INFO_LOG_FMT(IOS_USB, "SET_IDLE duration to indefinite (report_id={:02x}, index={})",
                   report_id, cmd->index);
      const std::lock_guard lock(m_pending_lock);
      m_idle_duration.reset();
    }
    else
    {
      const auto idle_duration = 4 * duration;  // 4 millisecond resolution (see HID specification)
...
{
  // Don't grab pressed keys when out of focus.
  //
  // Note: GetInputGate relies on a thread_local variable, thus,
  // it cannot be used in the worker thread to check if backgroud input is on.
  if (!ControlReference::GetInputGate())
  {
    PendingTransfer(std::move(cmd)).Do({});
    return IPC_SUCCESS;
  }
...
void HIDKeyboard::HandlePendingTransfer(std::shared_ptr<PendingTransfer> transfer)
{
  ...
  Common::HIDPressedState state{m_keyboard_context->GetPressedState()};
