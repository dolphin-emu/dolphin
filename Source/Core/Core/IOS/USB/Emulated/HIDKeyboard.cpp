...
{
  static constexpr u32 POLLING_RATE = 8000;  // 8ms (i.e. 125Hz)
  static auto start_time = std::chrono::steady_clock::now();
  const auto current_time = std::chrono::steady_clock::now();
  const bool should_poll = (current_time - start_time) >= std::chrono::microseconds(POLLING_RATE) &&
  ...
  cmd->FillBuffer(reinterpret_cast<const u8*>(&state), sizeof(state));
  cmd->ScheduleTransferCompletion(IPC_SUCCESS, POLLING_RATE);
