...
namespace IOS::HLE::USB
{
  ...
  u16 SkylanderCrypt::ComputeCRC16(u16 init_value, const u8* buffer, u32 size)
{
...
// CRC-64 algorithm that is limited to 48 bits every itteration
u64 SkylanderCrypt::ComputeCRC48(const u8* data, u32 size)
{
...
u64 SkylanderCrypt::CalculateKeyA(u8 sector, const u8* nuid, u32 size)
{
  if (sector == 0)
  {
  ...
  if (size != 4)
  {
    throw new std::invalid_argument("Size of NUID should be 4.");
  }

  u8 data[5] = {nuid[0], nuid[1], nuid[2], nuid[3], sector};

  u64 bigEndianCRC = ComputeCRC48(data, 5);
