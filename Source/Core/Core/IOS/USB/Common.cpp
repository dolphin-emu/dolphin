...
{
  const auto ticks = m_ios.GetSystem().GetSystemTimers().GetTicksPerSecond();
  const s64 cycles_in_future = static_cast<s64>((static_cast<u64>(ticks) * expected_time_us) / 1'000'000);
