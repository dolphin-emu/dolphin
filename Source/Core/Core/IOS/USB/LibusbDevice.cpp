...
namespace USB
{
LibusbDevice::LibusbDevice(libusb_device* device) : m_device(device)
{
  libusb_ref_device(m_device);

  libusb_device_descriptor device_descriptor;
  libusb_get_device_descriptor(device, &device_descriptor);
  m_vid = device_descriptor.idVendor;
  m_pid = device_descriptor.idProduct;
  ...
  for (u8 i = 0; i < device_descriptor.bNumConfigurations; ++i)
  ...
  case USBHDR(DIR_HOST2DEVICE, TYPE_STANDARD, REC_INTERFACE, REQUEST_SET_INTERFACE):
  {
    if (static_cast<u8>(cmd->index) != m_active_interface)
    {
      ...
      if (ret < 0)
      {
        ...
        EnqueueReply(cmd->ios_request, IPC_EINVAL);
        return ret;
      }
    }

    const int ret = SetAltSetting(static_cast<u8>(cmd->value));
    EnqueueReply(cmd->ios_request, ret == 0 ? cmd->length : IPC_EINVAL);
    return ret;
  }
                                                    ...
                                                    std::function<s32(const TransferCommand&)> fn)
{
  ...
  const auto& cmd = *m_transfers.at(transfer);
