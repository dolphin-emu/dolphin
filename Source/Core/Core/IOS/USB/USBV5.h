...
protected:
  ...
  IPCReply SetAlternateSetting(const USBV5Device& device, const IOCtlRequest& request) const;
  IPCReply Shutdown(const IOCtlRequest& request);
  IPCReply SuspendResume(const USBV5Device& device, const IOCtlRequest& request) const;
