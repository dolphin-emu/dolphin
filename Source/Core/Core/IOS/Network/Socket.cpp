        ...
        if (IsSSLIDValid(sslID))
        {
          ...
          case IOCTLV_NET_SSL_DOHANDSHAKE:
          {
            ...
            if (Config::Get(Config::MAIN_NETWORK_SSL_DUMP_PEER_CERT) &&
                ctx->private_session_negotiate != nullptr)
            {
              const mbedtls_x509_crt* cert = ctx->private_session_negotiate->private_peer_cert;
              if (cert != nullptr)
              {
                std::string filename =
                    File::GetUserPath(D_DUMPSSL_IDX) +
                    ((ctx->private_hostname != nullptr) ? ctx->private_hostname : "") +
                    "_peercert.der";
