...
static std::vector<InterfaceRouting> GetSystemInterfaceRouting()
{
  ...
  {
    ERROR_LOG_FMT(
        IOS_NET, "Failed to set netlink socket recv timeout: {}", Common::StrNetworkError());
    return {};
  }
...
std::optional<IPCReply> NetIPTopDevice::HandleDoSockRequest(const IOCtlRequest& request)
{
  ...
  const u32 fd = memory.Read_U32(request.buffer_in);
  GetEmulationKernel().GetSocketManager()->DoSock(
      fd, request, static_cast<NET_IOCTL>(request.request));
  return std::nullopt;
}
...
IPCReply NetIPTopDevice::HandleGetInterfaceOptRequest(const IOCtlVRequest& request)
{
      ...
      if (RetVal == NO_ERROR)
      {
        ...
        if (GetBestInterface(dwDestAddr, &dwBestIfIndex) == NO_ERROR)
        {
          while (AdapterList)
          {
            ...
            {
              INFO_LOG_FMT(
                  IOS_NET, "Name of valid interface: {}", WStringToUTF8(AdapterList->FriendlyName));
  ...
  case 0x4006:  // get routing table
  {
    ...
    for (InterfaceRouting route : interface.routing_table)
    {
      ...
      memory.Write_U32(
          route.gateway.s_addr == 0 ? 0 : 1, request.io_vectors[0].address + param5 + 12);
...
IPCReply NetIPTopDevice::HandleGetAddressInfoRequest(const IOCtlVRequest& request)
{
  ...
  if (ret == 0)
  {
    ...
    for (addrinfo* result_iter = result; result_iter != nullptr; result_iter = result_iter->ai_next)
    {
      ...
      if (result_iter->ai_addr)
      {
        ...
        memory.CopyToEmu(
            sockoffset + 0x2, result_iter->ai_addr->sa_data, sizeof(result_iter->ai_addr->sa_data));
        sockoffset += 0x1C;
      }
      else
      {
  ...
  request.Dump(
      system, GetDeviceName(), Common::Log::LogType::IOS_NET, Common::Log::LogLevel::LINFO);
  return IPCReply(ret);
}
...
IPCReply NetIPTopDevice::HandleICMPPingRequest(const IOCtlVRequest& request)
{
  ...
  {
    ret = icmp_echo_rep(
        socket_manager->GetHostSocket(fd), &addr, static_cast<u32>(timeout), icmp_length);
  }
