...
static std::vector<u8> ReadCertFile(
    const std::string& path, const std::array<u8, 32>& correct_hash, bool silent)
{
...
std::optional<IPCReply> NetSSLDevice::IOCtlV(const IOCtlVRequest& request)
{
  ...
  case IOCTLV_NET_SSL_NEW:
  {
    ...
    if (freeSSL)
    {
      ...
      mbedtls_ssl_conf_max_version(
          &ssl->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_2);
  ...
  case IOCTLV_NET_SSL_DOHANDSHAKE:
  {
    ...
    if (IsSSLIDValid(sslID))
    {
      GetEmulationKernel().GetSocketManager()->DoSock(
          _SSL[sslID].sockfd, request, IOCTLV_NET_SSL_DOHANDSHAKE);
      return std::nullopt;
    }
    else
    {
  ...
  case IOCTLV_NET_SSL_WRITE:
  {
    ...
    {
      GetEmulationKernel().GetSocketManager()->DoSock(
          _SSL[sslID].sockfd, request, IOCTLV_NET_SSL_WRITE);
      return std::nullopt;
    }
    else
    {
  ...
  case IOCTLV_NET_SSL_READ:
  {
    ...
    {
      GetEmulationKernel().GetSocketManager()->DoSock(
          _SSL[sslID].sockfd, request, IOCTLV_NET_SSL_READ);
      return std::nullopt;
    }
    else
    {
