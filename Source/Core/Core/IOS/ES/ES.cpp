...
ReturnCode ES::WriteNewCertToStore(const IOS::ES::CertReader& cert)
{
  ...
  std::vector<u8> certs_bytes(store_file.GetSize());
...
{
  if (SConfig::GetInstance().m_disable_signature_checks)
    return IPC_SUCCESS;
  ...
  if (mode == VerifyMode::UpdateCertStore)
  {
    ...
    if (ret != IPC_SUCCESS)
      ERROR_LOG(IOS_ES, "VerifyContainer: WriteNewCertToStore failed with return code %d", ret);
  }
