...
bool HostFileSystem::FstEntry::CheckPermission(
    Uid caller_uid, Gid caller_gid, Mode requested_mode) const
{
  if (caller_uid == 0)
    return true;
...
HostFileSystem::HostFileSystem(
    const std::string& root_path, std::vector<NandRedirect> nand_redirects)
...
ResultCode HostFileSystem::CreateFileOrDirectory(
    Uid uid, Gid gid, const std::string& path, FileAttribute attr, Modes modes, bool is_file)
{
...
ResultCode HostFileSystem::CreateFile(
    Uid uid, Gid gid, const std::string& path, FileAttribute attr, Modes modes)
{
  return CreateFileOrDirectory(uid, gid, path, attr, modes, true);
}

ResultCode HostFileSystem::CreateDirectory(
    Uid uid, Gid gid, const std::string& path, FileAttribute attr, Modes modes)
{
...
{
  return std::ranges::any_of(m_handles,
      [&path](const Handle& handle) { return handle.opened && handle.wii_path == path; });
}

bool HostFileSystem::IsDirectoryInUse(const std::string& path) const
{
  return std::ranges::any_of(m_handles,
      [&path](const Handle& handle) { return handle.opened && handle.wii_path.starts_with(path); });
}
...
ResultCode HostFileSystem::Rename(
    Uid uid, Gid gid, const std::string& old_path, const std::string& new_path)
{
  ...
  if (!File::Rename(host_old_path, host_new_path))
  {
    if (host_old_info.is_redirect || host_new_info.is_redirect)
    {
      ...
      {
        ERROR_LOG_FMT(
            IOS_FS, "Copying {} to {} in Rename fallback failed", host_old_path, host_new_path);
        return ResultCode::NotFound;
      }
...
Result<std::vector<std::string>> HostFileSystem::ReadDirectory(
    Uid uid, Gid gid, const std::string& path)
{
  if (!IsValidPath(path))
    return ResultCode::Invalid;
...
ResultCode HostFileSystem::SetMetadata(
    Uid caller_uid, const std::string& path, Uid uid, Gid gid, FileAttribute attr, Modes modes)
{
  if (!IsValidPath(path))
    return ResultCode::Invalid;
...
Result<ExtendedDirectoryStats> HostFileSystem::GetExtendedDirectoryStats(
