...
bool SConfig::AutoSetup(EBootBS2 _BootBS2)
{
  ...
  case BOOT_DEFAULT:
  {
    ...
    else if (DiscIO::CNANDContentManager::Access().GetNANDLoader(m_strFilename).IsValid())
    {
      ...
      // Use the TitleIDhex for name and/or unique ID if launching from nand
      // folder
      // or if it is not ascii characters (specifically sysmenu could
      // potentially apply to other
      // things)
...
DiscIO::IVolume::ELanguage SConfig::GetCurrentLanguage(bool wii) const
{
  ...
  // Get rid of invalid values (probably doesn't matter, but might as well do
  // it)
