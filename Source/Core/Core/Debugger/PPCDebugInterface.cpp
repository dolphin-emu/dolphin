...
{
  u8 data[] = {value};
  ApplyMemoryPatch(guard, data, address, store_existing_value);
}
void ApplyMemoryPatch(const Core::CPUThreadGuard& guard, u16 value, const u32 address,
                      bool store_existing_value)
{
  u8 data[] = {static_cast<u8>(value >> 8), static_cast<u8>(value)};
  ApplyMemoryPatch(guard, data, address, store_existing_value);
}
void ApplyMemoryPatch(const Core::CPUThreadGuard& guard, u32 value, const u32 address,
                      bool store_existing_value)
{
  u8 data[] = {static_cast<u8>(value >> 24), static_cast<u8>(value >> 16),
