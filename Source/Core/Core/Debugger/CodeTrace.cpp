...
bool CompareMemoryTargetToTracked(
    const std::string& instr, const u32 mem_target, const std::set<u32>& mem_tracked)
{
...
AutoStepResults CodeTrace::AutoStepping(
    const Core::CPUThreadGuard& guard, bool continue_previous, AutoStop stop_on)
{
...
HitType CodeTrace::TraceLogic(const TraceOutput& current_instr, bool first_hit)
{
  ...
  {
    mem_hit = CompareMemoryTargetToTracked(
        current_instr.instruction, *current_instr.memory_target, m_mem_autotrack);
  }
