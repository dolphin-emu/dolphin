...
std::vector<u8> Cheats::GetValueAsByteVector(const Cheats::SearchValue& value)
{
  ...
  case Cheats::DataType::S8:
    return {__builtin_bit_cast(u8, std::get<s8>(value.m_value))};
  case Cheats::DataType::S16:
    return ToByteVector(Common::swap16(__builtin_bit_cast(u16, std::get<s16>(value.m_value))));
  case Cheats::DataType::S32:
    return ToByteVector(Common::swap32(__builtin_bit_cast(u32, std::get<s32>(value.m_value))));
  case Cheats::DataType::S64:
    return ToByteVector(Common::swap64(__builtin_bit_cast(u64, std::get<s64>(value.m_value))));
  case Cheats::DataType::F32:
    return ToByteVector(Common::swap32(__builtin_bit_cast(u32, std::get<float>(value.m_value))));
  case Cheats::DataType::F64:
    return ToByteVector(Common::swap64(__builtin_bit_cast(u64, std::get<double>(value.m_value))));
  default:
    assert(0);
    return {};
  }
...
TryReadValueFromEmulatedMemory(u32 addr, PowerPC::RequestedAddressSpace space)
{
  ...
  if (!tmp)
    return std::nullopt;
  return PowerPC::ReadResult<s8>(tmp->translated, __builtin_bit_cast(s8, tmp->value));
}
...
TryReadValueFromEmulatedMemory(u32 addr, PowerPC::RequestedAddressSpace space)
{
  ...
  if (!tmp)
    return std::nullopt;
  return PowerPC::ReadResult<s16>(tmp->translated, __builtin_bit_cast(s16, tmp->value));
}
...
TryReadValueFromEmulatedMemory(u32 addr, PowerPC::RequestedAddressSpace space)
{
  ...
  if (!tmp)
    return std::nullopt;
  return PowerPC::ReadResult<s32>(tmp->translated, __builtin_bit_cast(s32, tmp->value));
}
...
TryReadValueFromEmulatedMemory(u32 addr, PowerPC::RequestedAddressSpace space)
{
  ...
  if (!tmp)
    return std::nullopt;
  return PowerPC::ReadResult<s64>(tmp->translated, __builtin_bit_cast(s64, tmp->value));
}
...
std::string Cheats::CheatSearchSession<T>::GetResultValueAsString(size_t index, bool hex) const
{
  ...
  if (hex)
  {
    if constexpr (std::is_same_v<T, float>)
      return fmt::format("0x{0:08x}", __builtin_bit_cast(u32, m_search_results[index].m_value));
    else if constexpr (std::is_same_v<T, double>)
      return fmt::format("0x{0:016x}", __builtin_bit_cast(u64, m_search_results[index].m_value));
    else
