...
// NOTE: Advance() must be called before executing the first cycle to get the correct downcount.
// This is because the time before the first cycle is treated as a large, single slice boundary,
// i.e. all the initialization code is running inside a theoretical callback. A call to Advance()
// is required to declare the end of that slice.
void Init();
void Shutdown();
