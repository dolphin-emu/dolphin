...
bool NetPlayServer::initUPnP()
{
  ...
  for (const UPNPDev* dev : igds)
  {
    ...
    descXML.reset(static_cast<char*>(miniwget_getaddr(dev->descURL, &descXMLsize, cIP, 20, 0, &statusCode)));
#else
    descXML.reset(static_cast<char*>(miniwget_getaddr(dev->descURL, &descXMLsize, cIP, 20, 0)));
#endif
