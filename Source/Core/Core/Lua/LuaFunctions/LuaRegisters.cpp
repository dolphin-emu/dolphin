...
{
  LuaRegister** lua_register_ptr_ptr = (LuaRegister**)lua_newuserdata(lua_state, sizeof(LuaRegister*));
  *lua_register_ptr_ptr = GetLuaRegisterInstance();
  ...
  AddLatestFunctionsForVersion(lua_registers_functions_with_versions_attached, lua_api_version, deprecated_functions_map, lua_state);
...
u8* GetAddressForRegister(RegisterObject register_object, lua_State* lua_state, const char* func_name)
{
  ...
  case RegisterObject::RegisterType::GeneralPurposeRegister:
    ...
    address = (u8*)(rGPR + register_number);
    return address;
...
void PushValueFromAddress(lua_State* lua_state, u8* memory_location, NumberType return_type, u8 register_size, u8 offset_bytes)
{
  u8 return_type_size = GetMaxSize(return_type);
  if (return_type_size > register_size)
    luaL_error(lua_state, "Error: in getRegister(), user requested a return type that was larger than the register!");
  else if (return_type_size + offset_bytes > register_size)
    luaL_error(lua_state, "Error: in getRegister(), there was not enough room after offsetting the specified number of bytes in order to get the specified data type.");
...
int GetRegister(lua_State* lua_state)
{
  ...
  if (return_type == NumberType::Undefined)
    luaL_error(lua_state, "Error: undefined type string was passed as an argument to getRegister()");
  s64 offset_bytes = 0;
  u8 register_size = 4;
  ...
  if (offset_bytes < 0 || offset_bytes > 15)
    luaL_error(lua_state, "Error: in getRegister(), number of bytes to offset from left was either less than 0 or more than 15");
...
int PushByteArrayFromAddressHelperFunction(lua_State* lua_state, bool is_unsigned, const char* func_name)
{
  const char* register_string = luaL_checkstring(lua_state, 2);
  RegisterObject register_object = ParseRegister(register_string);
  u8 register_size = 4;
  ...
  if (array_size <= 0)
    luaL_error(lua_state, (std::string("Error: in ") + func_name + ", arraySize was <= 0.").c_str());
  s64 offset_bytes = 0;
  if (lua_gettop(lua_state) >= 4)
    offset_bytes = luaL_checkinteger(lua_state, 4);
  if (offset_bytes < 0)
    luaL_error(lua_state, (std::string("Error: in ") + func_name + ", offset value was less than 0.").c_str());
  lua_createtable(lua_state, array_size, 0);
  s8 signed8 = 0;
  u8 unsigned8 = 0;

  if (array_size + offset_bytes > register_size)
    luaL_error(lua_state, (std::string("Error: in ") + func_name + ", attempt to read past the end of the register occured.").c_str());
...
{
  LuaColonOperatorTypeCheck(lua_state, "getRegisterAsUnsignedByteArray", "registers:getRegisterAsUnsignedByteArray(\"r4\", 2, 6)");
  return PushByteArrayFromAddressHelperFunction(lua_state, true, "getRegisterAsUnsignedByteArray()");
}

int GetRegisterAsSignedByteArray(lua_State* lua_state)
{
  LuaColonOperatorTypeCheck(lua_state, "getRegisterAsSignedByteArray", "registers:getRegisterAsSignedByteArray(\"r4\", 2, 6)");
  return PushByteArrayFromAddressHelperFunction(lua_state, false, "getRegisterAsSignedByteArray()");
}

void WriteValueToAddress(lua_State* lua_state, u8* memory_location, NumberType value_type, u8 register_size, u8 offset_bytes)
{
  u8 value_type_size = GetMaxSize(value_type);
  if (value_type_size > register_size)
    luaL_error(lua_state, "Error: in setRegister(), user tried to write a number that was larger than the register!");
  else if (offset_bytes >= register_size)
    luaL_error(lua_state, "Error: in setRegister(), user tried to use an offset that was larger than the register!");
  else if (value_type_size + offset_bytes > register_size)
    luaL_error(lua_state, "Error: in setRegister(), there was not enough room after offsetting the specified number of bytes in order to write the specified value.");
  ...
  case NumberType::Signed32:
    ...
    return;
  case NumberType::Unsigned64:
    luaL_error(lua_state, "Error: UNSIGNED_64 is not a valid type for setRegister(), since Lua only "
...
{
  LuaColonOperatorTypeCheck(lua_state, "setRegister", "registers:setRegister(\"r4\", \"u32\", 45, 4)");
  const char* register_string = luaL_checkstring(lua_state, 2);
  RegisterObject register_object = ParseRegister(register_string);
  ...
  if (value_type == NumberType::Undefined)
    luaL_error(lua_state, "Error: undefined type string was passed as an argument to setRegister()");
  u8 register_size = 4;
  if (register_object.register_type == RegisterObject::RegisterType::FloatingPointRegister)
    register_size = 16;
  ...
  if (offset_bytes < 0 || offset_bytes > 15)
    luaL_error(lua_state, "Error: in setRegister(), number of bytes to offset from left was either less than 0 or more than 15");
...
{
  LuaColonOperatorTypeCheck(lua_state, "setRegisterFromByteArray", "registers:setRegisterFromByteArray(\"r4\", byteTable, 2)");
  const char* register_string = luaL_checkstring(lua_state, 2);
  RegisterObject register_object = ParseRegister(register_string);
  u8 register_size = 4;
  if (register_object.register_type == RegisterObject::RegisterType::FloatingPointRegister)
  ...
  {
    luaL_error(lua_state, "Error: in setRegisterFromByteArray(), size of offset was too large for the register!");
  }
  ...
  for (int i = 0; i < sorted_byte_table.size(); ++i)
  {
    ...
    {
      luaL_error(lua_state, "Error: in setRegisterFromByteArray(), attempt to write byte array past the end of the register occured!");
    }

    signed64 = sorted_byte_table[i].value;

    if (signed64 < -128)
      luaL_error(lua_state, "Error: in setRegisterFromByteArray(), attempted to write value from byte array which could not be represented with 1 byte (value was less than -128)");
    else if (signed64 > 255)
      luaL_error(lua_state, "Error: in setRegisterFromByteArray(), attempted to write value from byte array which could not be represented with 1 byte (value was greater than 255)");
