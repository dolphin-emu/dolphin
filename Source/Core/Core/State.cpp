...
// Counter is incremented in state save calls that will write to the filesystem.
// It won't again reach zero until the task is done writing to the filesystem.
static Common::WaitableCounter<u32> s_state_saves_in_progress;

// Protect concurrent access of states on the filesystem.
// Note: Only the worker thread writes to these files.
static std::mutex s_state_fs_mutex;

struct CompressAndDumpStateArgs
{
  // Note: The buffer is larger than necessary to skip PointerWrap measurement.
  Common::UniqueBuffer<u8> oversized_buffer;
  std::size_t required_size;

  std::string filename;
  std::unique_lock<decltype(s_state_saves_in_progress)> write_lock;
};

// Queue for compressing and writing savestates to disk.
static Common::WorkQueueThreadSP<CompressAndDumpStateArgs> s_state_write_thread;
...
// Acquired when starting a task that will write state saves to the filesystem.
// It's not a real mutex, it can be moved to other threads.
[[nodiscard]] static auto GetStateSaveTaskLock()
{
  return std::unique_lock{s_state_saves_in_progress};
}

// Waits for previously requested state saves to reach the filesystem.
static void WaitForStateSaveTaskCompletion()
{
  // Note: Because CPU-thread might be in a task itself, we instead wait for the worker.
  // Being the CPU-thread, we would have already queued previous work.
  if (Core::IsCPUThread())
    s_state_write_thread.WaitForCompletion();
  else
    s_state_saves_in_progress.WaitForValue(0);
}

static Common::EventHook s_flush_data_hook =
    Core::FlushUnsavedDataEvent::Register(&WaitForStateSaveTaskCompletion, "State flush");
...
// Returns the require size, or 0 on failure.
...
{
  const size_t buffer_size = save_args.required_size;
  const auto buffer = std::span(save_args.oversized_buffer).first(buffer_size);
...
static void SaveAsFromCore(Core::System& system, std::string filename)
{
  ...
  if (const auto required_size = SaveToBuffer(system, buffer))
  {
    CompressAndDumpStateArgs dump_args{
        .oversized_buffer = std::move(buffer),
        .required_size = required_size,
        .filename = std::move(filename),
        .write_lock = GetStateSaveTaskLock(),
    };
    Core::DisplayMessage("Saving State...", 1000);
    s_state_write_thread.EmplaceItem(std::move(dump_args));
  }
  else
  {
...
void SaveAs(Core::System& system, std::string filename)
{
  Core::RunOnCPUThread(
      system,
      [&system, filename = std::move(filename), lock = GetStateSaveTaskLock()] {
...
{
  WaitForStateSaveTaskCompletion();
  std::lock_guard lk{s_state_fs_mutex};
...
{
  s_state_write_thread.Reset("Savestate Worker",
                             std::bind_front(&CompressAndDumpState, std::ref(system)));
}

void Shutdown()
{
  s_state_write_thread.Shutdown();
