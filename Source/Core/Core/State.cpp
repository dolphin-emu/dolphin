...
void EnableCompression(bool compression) { g_use_compression = compression; }

// Returns true if state version matches current Dolphin state version, false
// otherwise.
  ...
  *version_created_by = scm_rev_str;
  ...
  if (version != STATE_VERSION) {
    if (version_created_by->empty() && s_old_versions.count(version)) {
      ...
      std::pair<std::string, std::string> version_range =
          s_old_versions.find(version)->second;
      std::string oldest_version = version_range.first;
      std::string newest_version = version_range.second;

      *version_created_by =
          "Dolphin " + oldest_version + " - " + newest_version;
    }
...
static std::string DoState(PointerWrap &p) {
  ...
  if (!DoStateVersion(p, &version_created_by)) {
    ...
    // this will trigger an OSD message like "Can't load state from other
    // revisions"
    // we could use the version numbers to maintain some level of backward
    // compatibility, but
    // currently don't.
  ...
  // Begin with video backend, so that it gets a chance to clear its caches and
  // writeback modified
  // things to RAM
  ...
  // the controller code might need to schedule an event if the controller has
  // changed.
...
static void CompressAndDumpState(CompressAndDumpState_args save_args) {
  ...
  // ScopeGuard is used here to ensure that
  // g_compressAndDumpStateSyncEvent.Set()
  // will be called and that it will happen after the IOFile is closed.
  // Both ScopeGuard's and IOFile's finalization occur at respective object
  // destruction time.
  // As Local (stack) objects are destructed in the reverse order of
  // construction and "ScopeGuard
  // on_exit"
  // is created before the "IOFile f", it is guaranteed that the file will be
  // finalized before
  // the ScopeGuard's finalization (i.e. "g_compressAndDumpStateSyncEvent.Set()"
  // call).
  Common::ScopeGuard on_exit([]() { g_compressAndDumpStateSyncEvent.Set(); });
  // If it is not required to wait, we call finalizer early (and it won't be
  // called again at
  // destruction).
  if (!save_args.wait)
    on_exit.Exit();
  ...
  if (File::Exists(filename)) {
    ...
    if (!File::Rename(filename,
                      File::GetUserPath(D_STATESAVES_IDX) + "lastState.sav"))
      Core::DisplayMessage("Failed to move previous state to state undo backup",
                           1000);
    else
  ...
  if (header.size != 0) // non-zero header size means the state is compressed
  {
    ...
    while (true) {
      ...
      if (lzo1x_1_compress(buffer_data + i, cur_len, out, &out_len, wrkmem) !=
          LZO_E_OK)
  ...
  Core::DisplayMessage(StringFromFormat("Saved State to %s", filename.c_str()),
                       2000);
...
static void LoadFileStateData(const std::string &filename,
                              std::vector<u8> &ret_data) {
  Flush();
  File::IOFile f(filename, "rb");
  if (!f) {
    Core::DisplayMessage("State not found", 2000);
  ...
  if (strncmp(SConfig::GetInstance().GetUniqueID().c_str(), header.gameID, 6)) {
    Core::DisplayMessage(
        StringFromFormat("State belongs to a different game (ID %.*s)", 6,
                         header.gameID),
        2000);
    return;
  }
  ...
  if (header.size != 0) // non-zero size means the state is compressed
  {
    ...
    while (true) {
      ...
      const int res =
          lzo1x_decompress(out, cur_len, &buffer[i], &new_len, nullptr);
      if (res != LZO_E_OK) {
...
void LoadAs(const std::string &filename) {
  ...
  if (loaded) {
    if (loadedSuccessfully) {
      Core::DisplayMessage(
          StringFromFormat("Loaded state from %s", filename.c_str()), 2000);
      ...
      // failed to load
      Core::DisplayMessage(
          "Unable to load: Can't load state from other versions!", 4000);
      if (!version_created_by.empty())
        Core::DisplayMessage(
            "The savestate was created using " + version_created_by, 4000);

      // since we could be in an inconsistent state now (and might crash or
      // whatever), undo.
      if (g_loadDepth < 2)
        UndoLoadState();
    }
...
void VerifyAt(const std::string &filename) {
  ...
  if (!buffer.empty()) {
    ...
    if (p.GetMode() == PointerWrap::MODE_VERIFY)
      Core::DisplayMessage(
          StringFromFormat("Verified state at %s", filename.c_str()), 2000);
    else
      Core::DisplayMessage(
          "Unable to Verify : Can't verify state from other revisions !", 4000);
  }
...
void Shutdown() {
  ...
  // this gives a better guarantee to free the allocated memory right NOW (as
  // opposed to, actually,
  // never)
  {
    std::lock_guard<std::mutex> lk(g_cs_current_buffer);
...
static std::string MakeStateFilename(int number) {
  return StringFromFormat("%s%s.s%02i",
                          File::GetUserPath(D_STATESAVES_IDX).c_str(),
                          SConfig::GetInstance().GetUniqueID().c_str(), number);
}

void Save(int slot, bool wait) { SaveAs(MakeStateFilename(slot), wait); }

void Load(int slot) { LoadAs(MakeStateFilename(slot)); }

void Verify(int slot) { VerifyAt(MakeStateFilename(slot)); }
...
void UndoLoadState() {
  ...
  if (!g_undo_load_buffer.empty()) {
    if (File::Exists(File::GetUserPath(D_STATESAVES_IDX) + "undo.dtm") ||
      ...
      PanicAlertT("No undo.dtm found, aborting undo load state to prevent "
                  "movie desyncs");
    }
  } else {
