...
// Multiple "shared" locks are acquired for each state save task.
// Tasks generally transition from: Host -> Core -> Compression thread.
...
// Only the "Core" thread manipulates this worker.
...
static Common::EventHook s_flush_data_hook = Core::FlushUnsavedDataEvent::Register(
    [] {
...
{
  const size_t buffer_size = save_args.actual_size;
...
void UndoLoadState(Core::System& system)
{
  ...
  Core::RunOnCPUThread(
      ...
      [&system] {
        ...
        if (movie.IsMovieActive())
        {
          // Note: Only "Core" thread writes to "undo.dtm".
          const std::string dtmpath = File::GetUserPath(D_STATESAVES_IDX) + "undo.dtm";
