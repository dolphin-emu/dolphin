...
void MovieConfigLayerLoader::Load(OnionConfig::BloomLayer *config_layer) {
  OnionConfig::OnionPetal *core = config_layer->GetOrCreatePetal(
      OnionConfig::OnionSystem::SYSTEM_MAIN, "Core");
  OnionConfig::OnionPetal *display = config_layer->GetOrCreatePetal(
      OnionConfig::OnionSystem::SYSTEM_MAIN, "Display");
  OnionConfig::OnionPetal *video_settings = OnionConfig::GetOrCreatePetal(
      OnionConfig::OnionSystem::SYSTEM_GFX, "Settings");
  OnionConfig::OnionPetal *video_hacks = OnionConfig::GetOrCreatePetal(
      OnionConfig::OnionSystem::SYSTEM_GFX, "Hacks");
  ...
  video_hacks->Set("EFBEmulateFormatChanges",
...
void MovieConfigLayerLoader::Save(OnionConfig::BloomLayer *config_layer) {
  ...
  core->Get("GFXBackend", &video_backend, "OGL");
  ...
  video_hacks->Get("EFBEmulateFormatChanges",
                   &m_header->bEFBEmulateFormatChanges);
  ...
  strncpy((char *)m_header->videoBackend, video_backend.c_str(),
          ArraySize(m_header->videoBackend));
}

// Loader generation
OnionConfig::ConfigLayerLoader *
GenerateMovieConfigLoader(Movie::DTMHeader *header) {
