                         ...
                         const std::vector<DiscIO::Riivolution::Patch>& riivolution_patches)
{
  ...
  const u32 iAppLoaderInit = mmu.Read<u32>(iAppLoaderFuncAddr + 0, UGeckoInstruction{});
  const u32 iAppLoaderMain = mmu.Read<u32>(iAppLoaderFuncAddr + 4, UGeckoInstruction{});
  const u32 iAppLoaderClose = mmu.Read<u32>(iAppLoaderFuncAddr + 8, UGeckoInstruction{});
  ...
  {
    const u32 ram_address = mmu.Read<u32>(0x81300004, UGeckoInstruction{});
    const u32 length = mmu.Read<u32>(0x81300008, UGeckoInstruction{});
    const u32 dvd_offset = mmu.Read<u32>(0x8130000c, UGeckoInstruction{}) << (is_wii ? 2 : 0);
