...
bool CBoot::BootUp()
{
  ...
  case SConfig::BOOT_ISO:
  {
    ...
    if ((pVolume.GetVolumeType() == DiscIO::EPlatform::WII_DISC) != _StartupPara.bWii)
    {
    ...
    _StartupPara.bWii = pVolume.GetVolumeType() == DiscIO::EPlatform::WII_DISC;
  ...
  case SConfig::BOOT_ELF:
  {
    ...
    if (_StartupPara.bWii)
      SetupWiiMemory(DiscIO::ECountry::COUNTRY_UNKNOWN);
    else
      EmulatedBS2_GC(true);
