                                 ...
                                 const std::optional<std::string>& savestate_path)
{
  ...
  if (extension == ".json")
  {
    ...
    if (descriptor)
    {
      ...
      auto boot_params = GenerateFromFile(descriptor->m_base_file, savestate_path);
      if (boot_params)
      {
        if (descriptor->m_riivolution && std::holds_alternative<Disc>(boot_params->parameters))
        {
          ...
          boot_params->riivolution_patches =
              DiscIO::Riivolution::GenerateRiivolutionPatchesFromGameModDescriptor(
                  *descriptor->m_riivolution, volume.GetGameID(),
                  volume.GetRevision() ? static_cast<int>(*volume.GetRevision()) : -1,
                  volume.GetDiscNumber() ? static_cast<int>(*volume.GetDiscNumber()) : -1);
      ...
      {
        PanicAlertFmtT("Could not recognize file {0}", descriptor->m_base_file);
        return nullptr;
      }
    }
  }
