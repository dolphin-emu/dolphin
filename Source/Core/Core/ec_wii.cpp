...
static u8 default_NG_priv[] = {
    0x00, 0xAB, 0xEE, 0xC1, 0xDD, 0xB4, 0xA6, 0x16, 0x6B, 0x70,
...
static u8 default_NG_sig[] = {
    // R
    0x00, 0xD8, 0x81, 0x63, 0xB2, 0x00, 0x6B, 0x0B, 0x54, 0x82, 0x88, 0x63,
    0x81, 0x1C, 0x00, 0x71, 0x12, 0xED, 0xB7, 0xFD, 0x21, 0xAB, 0x0E, 0x50,
    0x0E, 0x1F, 0xBF, 0x78, 0xAD, 0x37,
    // S
    0x00, 0x71, 0x8D, 0x82, 0x41, 0xEE, 0x45, 0x11, 0xC7, 0x3B, 0xAC, 0x08,
    0xB6, 0x83, 0xDC, 0x05, 0xB8, 0xA8, 0x90, 0x1F, 0xA8, 0x2A, 0x0E, 0x4E,
    0x76, 0xEF, 0x44, 0x72, 0x99, 0xF8,
};

// get_ng_cert

// ng_cert_out is a pointer to a 0x180 byte buffer that will contain the
...
// if NG_priv iis nullptr or NG_sig is nullptr or NG_id is 0 or NG_key_id is 0,
// default values
// will be used for all of them
void get_ng_cert(u8 *ng_cert_out, u32 NG_id, u32 NG_key_id, const u8 *NG_priv,
                 const u8 *NG_sig) {
  char name[64];
  if ((NG_id == 0) || (NG_key_id == 0) || (NG_priv == nullptr) ||
      (NG_sig == nullptr)) {
    NG_id = default_NG_id;
  ...
  make_blanksig_ec_cert(ng_cert_out, "Root-CA00000001-MS00000002", name,
                        NG_priv, NG_key_id);
...
// sig_out is a pointer to a 0x3c byte buffer which will be filled with the data
// payload's signature
// ap_cert_out is a pointer to a 0x180 byte buffer which will be filled with the
// temporal AP
// certificate
...
void get_ap_sig_and_cert(u8 *sig_out, u8 *ap_cert_out, u64 title_id, u8 *data,
                         u32 data_size, const u8 *NG_priv, u32 NG_id) {
  u8 hash[20];
  u8 ap_priv[30];
  char signer[64];
  char name[64];
  ...
  sprintf(name, "AP%08x%08x", (u32)(title_id >> 32),
  ...
  if (File::Exists(keys_path)) {
    ...
    if (keys_f.IsOpen()) {
      if (keys_f.ReadBytes(&BootMiiKeysBin, sizeof(BootMiiKeysBin))) {
        ...
        INFO_LOG(WII_IPC_ES, "Successfully loaded keys.bin created by: %s",
                 BootMiiKeysBin.creator);
      } else {
        ERROR_LOG(WII_IPC_ES, "Failed to read keys.bin, check it is the "
                              "correct size of %08zX bytes.",
                  sizeof(BootMiiKeysBin));
      }
    } else {
      ERROR_LOG(WII_IPC_ES, "Failed to open keys.bin, maybe a permissions "
                            "error or it is in use?");
    }
  } else {
    ERROR_LOG(WII_IPC_ES, "%s could not be found. Using default values. We "
                          "recommend you grab keys.bin from BootMii.",
              keys_path.c_str());
  }
...
void EcWii::InitDefaults() {
  ...
  memcpy(BootMiiKeysBin.ng_priv, default_NG_priv,
         sizeof(BootMiiKeysBin.ng_priv));
