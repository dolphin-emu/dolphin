...
namespace prime {
  ...
  // Overwrites the normal beam change check by looking at 0x804a79f0 and checking if it is flagged at 1
  // if 1, reset it to 0 and read out the desired beam from 0x804a79f4. These two addresses are set by
  // the mod.
  ...
  void MP1::run_mod() {
    ...
    int dx = g_mouse_input->GetDeltaHorizontalAxis(),
      dy = g_mouse_input->GetDeltaVerticalAxis();
  ...
  MP1NTSC::MP1NTSC() {
    // This instruction change is used in all 3 games, all 3 regions. It's an update to what I believe to be interpolation for player camera pitch
    // The change is from fmuls f0, f0, f1 (0xec000072) to fmuls f0, f1, f1 (0xec010072), where f0 is the output. The higher f0 is, the faster the
    // pitch *can* change in-game, and squaring f1 seems to do the job.
  ...
  uint32_t MP1NTSC::lockon_address() const { return 0x804c00b3; }
  uint32_t MP1NTSC::yaw_vel_address() const { return 0x804d3d38; }
  uint32_t MP1NTSC::pitch_address() const { return 0x804d3ffc; }
  uint32_t MP1NTSC::pitch_goal_address() const { return 0x804c10ec; }
  uint32_t MP1NTSC::avel_limiter_address() const { return 0x804d3d74; }
  uint32_t MP1NTSC::new_beam_address() const { return 0x804a79f4; }
  uint32_t MP1NTSC::beamchange_flag_address() const { return 0x804a79f0; }
  uint32_t MP1NTSC::visor_base_address() const { return 0x804bfcd4; }
  uint32_t MP1NTSC::camera_pointer_address() const { return 0x804bfc30; }
  uint32_t MP1NTSC::active_camera_offset_address() const { return 0x804c4a08; }
  uint32_t MP1NTSC::global_fov1() const { return 0x805c0e38; }
  uint32_t MP1NTSC::global_fov2() const { return 0x805c0e3c; }
  ...
  uint32_t MP1PAL::lockon_address() const { return 0x804c3ff3; }
  uint32_t MP1PAL::yaw_vel_address() const { return 0x804d7c78; }
  uint32_t MP1PAL::pitch_address() const { return 0x804d7f3c; }
  uint32_t MP1PAL::pitch_goal_address() const { return 0x804c502c; }
  uint32_t MP1PAL::avel_limiter_address() const { return 0x804d7cb4; }
  uint32_t MP1PAL::new_beam_address() const { return 0x804a79f4; }
  uint32_t MP1PAL::beamchange_flag_address() const { return 0x804a79f0; }
  uint32_t MP1PAL::visor_base_address() const { return 0x804c3c14; }
  uint32_t MP1PAL::camera_pointer_address() const { return 0x804c3b70; }
  uint32_t MP1PAL::active_camera_offset_address() const { return 0x804c8948; }
  uint32_t MP1PAL::global_fov1() const { return 0x805c5178; }
  uint32_t MP1PAL::global_fov2() const { return 0x805c517c; }
  ...
  void MP2::run_mod() {
    ...
    u32 camera_offset = ((PowerPC::HostRead_U32(PowerPC::HostRead_U32(camera_offset_address())) >> 16) & 0x3ff) << 3;
  ...
  uint32_t MP2NTSC::camera_ctl_address() const { return 0x804e87dc; }
  uint32_t MP2NTSC::lockon_address() const { return 0x804e894f; }
  uint32_t MP2NTSC::new_beam_address() const { return 0x804cd254; }
  uint32_t MP2NTSC::beamchange_flag_address() const { return 0x804cd250; }
  uint32_t MP2NTSC::camera_ptr_address() const { return 0x804e7af8; }
  uint32_t MP2NTSC::camera_offset_address() const { return 0x804eb9ac; }
  ...
  uint32_t MP2PAL::load_state_address() const { return 0x804efc74; }
  uint32_t MP2PAL::camera_ctl_address() const { return 0x804efc2c; }
  uint32_t MP2PAL::lockon_address() const { return 0x804efd9f; }
  uint32_t MP2PAL::new_beam_address() const { return 0x804cd254; }
  uint32_t MP2PAL::beamchange_flag_address() const { return 0x804cd250; }
  uint32_t MP2PAL::camera_ptr_address() const { return 0x804eef48; }
  uint32_t MP2PAL::camera_offset_address() const { return 0x804f2f4c; }

  // IIRC, this hook will be executed upon a control scheme (of some sort) being changed. The new ctl scheme (pointer, offset, forgot?) is in r6
  // If the entity having this scheme being updated is the player, it's happening because we're going into locked-camera-puzzle-mode (or in ship)
  // so if we cause the action of updating the scheme to not happen for the player, we can stick with free-camera mode, and the game won't jack
  // the controls from us (meaning we can use the same addresses to write yaw velocity and pitch values).
  void MP3::control_state_hook(u32 base_offset, Region region) {
    if (region == Region::NTSC) {
      ...
      code_changes.emplace_back(base_offset + 0x04, 0x38636c40); // addi r3, r3, 0x6c40  ; load 0x805c6c40 into r3 (indirect pointer to player camera control)
    }
    ...
    code_changes.emplace_back(base_offset + 0x10, 0x80632184); // lwz  r3, 0x2184(r3)    ; should have r31 equal to the object which is being modified
    code_changes.emplace_back(base_offset + 0x14, 0x7c03f800); // cmpw r3, r31           ; if r31 is player camera control (in r3)
    code_changes.emplace_back(base_offset + 0x18, 0x4d820020); // beqlr                  ; then DON'T store the value of r6 into 0x78+(player camera control)
    code_changes.emplace_back(base_offset + 0x1c, 0x7fe3fb78); // mr   r3, r31           ; otherwise do it
    code_changes.emplace_back(base_offset + 0x20, 0x90c30078); // stw  r6, 0x78(r3)      ; this is the normal action which was overwritten by a bl to this mini-function
    code_changes.emplace_back(base_offset + 0x24, 0x4e800020); // blr                    ; LR wasn't changed, so we're safe here (same case as beqlr)
  }
  ...
  void MP3::run_mod() {
    ...
    if (PowerPC::HostRead_U8(grapple_hook_address()) ||
      PowerPC::HostRead_U8(base_address + 0x378) ||
  ...
  uint32_t MP3NTSC::lockon_address() const { return 0x805c6db7; }
  uint32_t MP3NTSC::camera_pointer_address() const { return 0x805c6c68; }
  ...
  uint32_t MP3PAL::lockon_address() const { return 0x805ca237; }
  uint32_t MP3PAL::camera_pointer_address() const { return 0x805ca0e8; }
