...
{
  std::unordered_map<std::string, std::string> deprecated_functions_map;
  return GetFunctionForVersion(all_on_wii_input_polled_callback_functions_metadata_list, api_version, function_name,
...
{
  bool return_value = current_script->UnregisterOnWiiInputPolledCallbacks(args_list[0].void_pointer_val);
  if (!return_value)
