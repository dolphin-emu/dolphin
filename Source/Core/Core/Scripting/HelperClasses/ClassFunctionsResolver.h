...
void Send_ClassMetadataForVersion_To_DLL_Buffer_impl(
    void*, const char*,
    const char*);  // Takes a ScriptContext* as 1st param, an input module name as 2nd param,
                   // andversion number as 3rd param. Calls the dll-specific
                   // DLLClassMetadataCopyHook function contained in the ScriptContext*, passing in
                   // the resulting opaque ClassMetadata* as a handle (allows for its fields to be
                   // copied to the DLL-side).
