...
void* Vector_Get(Vector* __this, int index)
{
  ...
  {
    fprintf(stderr,
            "Error: Attempted to access an element passed the end of the vector in Vector_Get()\n");
    return NULL;
  }
  else
  {
...
void* Vector_RemoveByIndex(Vector* __this, int index)
{
  ...
  if (index < __this->length - 1)
    memmove(__this->arr + index, __this->arr + index + 1,
            (__this->length - index - 1) * sizeof(void*));
...
{
  // If the destructor parameter passed into Vector_Initializer was NULL, then that means that this
  // represents an array of primitive types which don't have a destructor.
  // Otherwise, the destructor_function represents the function that acts as the destructor for each
  // object in the array, and each object in the array is the address of an object (we then invoke
  // the destructor on each object). In either case, we free the memory allocated for the array.
  if (__this->size <= 0)
    return;
