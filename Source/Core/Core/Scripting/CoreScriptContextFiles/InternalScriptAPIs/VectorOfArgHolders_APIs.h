...
extern "C" {
  ...
  // IMPORTANT NOTE: CreateNewVectorOfArgHolders allocates a new Vector on the heap. As such, Delete_VectorOfArgHolders needs to be called when done using the vector by the DLL in order to prevent memory leaks.
  // Also, if the Delete_VectorOfArgHolders() function is called, then the delete function for ArgHolders MAY NOT be called on individual ArgHolders, as this would cause a double-free!
typedef struct VectorOfArgHolders_APIs
{
  ...
  void (*Delete_VectorOfArgHolders)(void*); // Deletes the VectorOfArgHolders* passed into it (including calling delete on each individual ArgHolder*).
  // WARNING: The user may not call delete on an ArgHolder directly if it has been added to a VectorOfArgHolders. Instead, they need to use the above delete method, which frees up the vector and all ArgHolders in it.
