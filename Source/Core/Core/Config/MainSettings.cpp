...
std::unordered_map<Config::ValueType, std::vector<const void*>> all_main_settings = {{
    ValueType::BOOLEAN,
     {
         &MAIN_SKIP_IPL, &MAIN_JIT_FOLLOW_BRANCH, &MAIN_FASTMEM, &MAIN_ACCURATE_CPU_CACHE,
         &MAIN_DSP_HLE, &MAIN_CPU_THREAD, &MAIN_SYNC_ON_SKIP_IDLE, &MAIN_ENABLE_CHEATS,
         &MAIN_OVERRIDE_REGION_SETTINGS, &MAIN_DPL2_DECODER, &MAIN_AUDIO_STRETCH,
         &MAIN_BBA_XLINK_CHAT_OSD, &GetInfoForAdapterRumble(0), &GetInfoForAdapterRumble(1),
         &GetInfoForAdapterRumble(2), &GetInfoForAdapterRumble(3),
         &GetInfoForSimulateKonga(0), &GetInfoForSimulateKonga(1), &GetInfoForSimulateKonga(2), &GetInfoForSimulateKonga(3),
         &MAIN_WII_SD_CARD, &MAIN_WII_SD_CARD_ENABLE_FOLDER_SYNC, &MAIN_WII_KEYBOARD, &MAIN_WIIMOTE_CONTINUOUS_SCANNING,
         &MAIN_CONNECT_WIIMOTES_FOR_CONTROLLER_INTERFACE, &MAIN_MMU, &MAIN_PAUSE_ON_PANIC, &MAIN_SYNC_GPU,
         &MAIN_FAST_DISC_SPEED, &MAIN_LOW_DCBZ_HACK, &MAIN_FLOAT_EXCEPTIONS, &MAIN_DIVIDE_BY_ZERO_EXCEPTIONS,
         &MAIN_FPRF, &MAIN_ACCURATE_NANS, &MAIN_DISABLE_ICACHE, &MAIN_OVERCLOCK_ENABLE, &MAIN_RAM_OVERRIDE_ENABLE,
         &MAIN_CUSTOM_RTC_ENABLE, &MAIN_AUTO_DISC_CHANGE, &MAIN_ALLOW_SD_WRITES, &MAIN_ENABLE_SAVESTATES,
         &MAIN_REAL_WII_REMOTE_REPEAT_REPORTS, &MAIN_DSP_THREAD, &MAIN_DSP_CAPTURE_LOG, &MAIN_DSP_JIT,
         &MAIN_DUMP_AUDIO, &MAIN_DUMP_AUDIO_SILENT, &MAIN_DUMP_UCODE, &MAIN_AUDIO_MUTED, &MAIN_FULLSCREEN,
         &MAIN_RENDER_TO_MAIN, &MAIN_RENDER_WINDOW_AUTOSIZE, &MAIN_KEEP_WINDOW_ON_TOP, &MAIN_DISABLE_SCREENSAVER,
         &MAIN_SHOW_LAG, &MAIN_SHOW_FRAME_COUNT, &MAIN_GBA_SAVES_IN_ROM_PATH, &MAIN_GBA_THREADS, &MAIN_NETWORK_SSL_DUMP_READ,
         &MAIN_NETWORK_SSL_DUMP_WRITE, &MAIN_NETWORK_SSL_VERIFY_CERTIFICATES, &MAIN_NETWORK_SSL_DUMP_ROOT_CA,
         &MAIN_NETWORK_SSL_DUMP_PEER_CERT, &MAIN_NETWORK_DUMP_BBA, &MAIN_NETWORK_DUMP_AS_PCAP, &MAIN_USE_HIGH_CONTRAST_TOOLTIPS,
         &MAIN_USE_PANIC_HANDLERS, &MAIN_ABORT_ON_PANIC_ALERT, &MAIN_OSD_MESSAGES, &MAIN_SKIP_NKIT_WARNING, &MAIN_CONFIRM_ON_STOP,
         &MAIN_LOCK_CURSOR, &MAIN_SHOW_ACTIVE_TITLE, &MAIN_USE_BUILT_IN_TITLE_DATABASE, &MAIN_PAUSE_ON_FOCUS_LOST, &MAIN_ENABLE_DEBUGGING,
         &MAIN_ANALYTICS_ENABLED, &MAIN_ANALYTICS_PERMISSION_ASKED, &MAIN_GAMELIST_LIST_DRIVES, &MAIN_GAMELIST_LIST_WAD,
         &MAIN_GAMELIST_LIST_ELF_DOL, &MAIN_GAMELIST_LIST_WII, &MAIN_GAMELIST_LIST_GC, &MAIN_GAMELIST_LIST_JPN, &MAIN_GAMELIST_LIST_PAL,
         &MAIN_GAMELIST_LIST_USA, &MAIN_GAMELIST_LIST_AUSTRALIA, &MAIN_GAMELIST_LIST_FRANCE, &MAIN_GAMELIST_LIST_GERMANY, &MAIN_GAMELIST_LIST_ITALY,
         &MAIN_GAMELIST_LIST_KOREA, &MAIN_GAMELIST_LIST_NETHERLANDS, &MAIN_GAMELIST_LIST_RUSSIA, &MAIN_GAMELIST_LIST_SPAIN, &MAIN_GAMELIST_LIST_TAIWAN,
         &MAIN_GAMELIST_LIST_WORLD, &MAIN_GAMELIST_LIST_UNKNOWN, &MAIN_GAMELIST_COLUMN_PLATFORM, &MAIN_GAMELIST_COLUMN_DESCRIPTION,
         &MAIN_GAMELIST_COLUMN_BANNER, &MAIN_GAMELIST_COLUMN_TITLE, &MAIN_GAMELIST_COLUMN_MAKER, &MAIN_GAMELIST_COLUMN_FILE_NAME,
         &MAIN_GAMELIST_COLUMN_FILE_PATH, &MAIN_GAMELIST_COLUMN_GAME_ID, &MAIN_GAMELIST_COLUMN_REGION, &MAIN_GAMELIST_COLUMN_FILE_SIZE,
         &MAIN_GAMELIST_COLUMN_FILE_FORMAT, &MAIN_GAMELIST_COLUMN_BLOCK_SIZE, &MAIN_GAMELIST_COLUMN_COMPRESSION, &MAIN_GAMELIST_COLUMN_TAGS,
         &MAIN_FIFOPLAYER_LOOP_REPLAY, &MAIN_FIFOPLAYER_EARLY_MEMORY_UPDATES, &MAIN_MOVIE_PAUSE_MOVIE, &MAIN_MOVIE_DUMP_FRAMES,
         &MAIN_MOVIE_DUMP_FRAMES_SILENT, &MAIN_MOVIE_SHOW_INPUT_DISPLAY, &MAIN_MOVIE_SHOW_RTC, &MAIN_MOVIE_SHOW_RERECORD, &MAIN_INPUT_BACKGROUND_INPUT,
         &MAIN_DEBUG_JIT_OFF, &MAIN_DEBUG_JIT_LOAD_STORE_OFF, &MAIN_DEBUG_JIT_LOAD_STORE_LXZ_OFF, &MAIN_DEBUG_JIT_LOAD_STORE_LWZ_OFF,
         &MAIN_DEBUG_JIT_LOAD_STORE_LBZX_OFF, &MAIN_DEBUG_JIT_LOAD_STORE_FLOATING_OFF, &MAIN_DEBUG_JIT_LOAD_STORE_PAIRED_OFF,
         &MAIN_DEBUG_JIT_FLOATING_POINT_OFF, &MAIN_DEBUG_JIT_INTEGER_OFF, &MAIN_DEBUG_JIT_PAIRED_OFF, &MAIN_DEBUG_JIT_SYSTEM_REGISTERS_OFF,
         &MAIN_DEBUG_JIT_BRANCH_OFF, &MAIN_DEBUG_JIT_REGISTER_CACHE_OFF, &MAIN_BLUETOOTH_PASSTHROUGH_ENABLED, &MAIN_EMULATE_SKYLANDER_PORTAL
     }
     },
    {ValueType::INT, {
      &MAIN_MAX_FALLBACK, &MAIN_TIMING_VARIANCE, &MAIN_GC_LANGUAGE, &MAIN_AUDIO_LATENCY, &MAIN_AUDIO_STRETCH_LATENCY,
      &MAIN_MEMORY_CARD_SIZE, &MAIN_BB_DUMP_PORT, &MAIN_SYNC_GPU_MAX_DISTANCE, &MAIN_SYNC_GPU_MIN_DISTANCE, &MAIN_AUDIO_VOLUME,
      &MAIN_RENDER_WINDOW_XPOS, &MAIN_RENDER_WINDOW_YPOS, &MAIN_RENDER_WINDOW_WIDTH, &MAIN_RENDER_WINDOW_HEIGHT, &MAIN_GDB_PORT,
      &MAIN_ISO_PATH_COUNT, &MAIN_NETWORK_TIMEOUT, &MAIN_GAMELIST_LIST_SORT, &MAIN_GAMELIST_LIST_SORT_SECONDARY
}},
    {ValueType::U32, {&MAIN_MEM1_SIZE, &MAIN_MEM2_SIZE, &MAIN_ARAM_EXPANSION_SIZE, &MAIN_CUSTOM_RTC_VALUE }
},
    {ValueType::FLOAT, {&MAIN_SYNC_GPU_OVERCLOCK, &MAIN_EMULATION_SPEED, &MAIN_OVERCLOCK}},
    {ValueType::S32, {&MAIN_OVERRIDE_BOOT_IOS}},
    {ValueType::STRING, {&MAIN_DEFAULT_ISO, &MAIN_MEMCARD_A_PATH, &MAIN_MEMCARD_B_PATH, &MAIN_AGP_CART_A_PATH, &MAIN_AGP_CART_B_PATH,
      &MAIN_GCI_FOLDER_A_PATH, &MAIN_GCI_FOLDER_B_PATH, &MAIN_GCI_FOLDER_A_PATH_OVERRIDE, &MAIN_GCI_FOLDER_B_PATH_OVERRIDE, &MAIN_BBA_MAC,
      &MAIN_BBA_XLINK_IP, &MAIN_BBA_BUILTIN_DNS, &MAIN_BBA_BUILTIN_IP, &MAIN_GFX_BACKEND, &MAIN_GPU_DETERMINISM_MODE, &MAIN_PERF_MAP_DIR,
      &MAIN_WII_NUS_SHOP_URL, &MAIN_AUDIO_BACKEND, &MAIN_WASAPI_DEVICE, &MAIN_FULLSCREEN_DISPLAY_RES, &MAIN_DUMP_PATH, &MAIN_LOAD_PATH,
      &MAIN_RESOURCEPACK_PATH, &MAIN_FS_PATH, &MAIN_WII_SD_CARD_IMAGE_PATH, &MAIN_WII_SD_CARD_SYNC_FOLDER_PATH, &MAIN_WFS_PATH,
      &MAIN_WIRELESS_MAC, &MAIN_GDB_SOCKET, &MAIN_GBA_BIOS_PATH, &MAIN_GBA_ROM_PATHS[0], &MAIN_GBA_ROM_PATHS[1], &MAIN_GBA_ROM_PATHS[2],
      &MAIN_GBA_ROM_PATHS[3], &MAIN_GBA_SAVES_PATH, &MAIN_INTERFACE_LANGUAGE, &MAIN_THEME_NAME, &MAIN_ANALYTICS_ID, &MAIN_AUTOUPDATE_UPDATE_TRACK,
      &MAIN_AUTOUPDATE_HASH_OVERRIDE, &MAIN_MOVIE_MOVIE_AUTHOR, &MAIN_BLUETOOTH_PASSTHROUGH_LINK_KEYS, &MAIN_USB_PASSTHROUGH_DEVICES}},
    {ValueType::CPU_CORE, {&MAIN_CPU_CORE}},
  {ValueType::DPL2_QUALITY, {&MAIN_DPL2_QUALITY}},
  {ValueType::EXI_DEVICE_TYPE, {&MAIN_SLOT_A, &MAIN_SLOT_B, &MAIN_SERIAL_PORT_1}},
    {ValueType::SI_DEVICES, {&GetInfoForSIDevice(0), &GetInfoForSIDevice(1), &GetInfoForSIDevice(2), &GetInfoForSIDevice(3)}},
  {ValueType::HSP_DEVICE_TYPE, {&MAIN_HSP_DEVICE}},
  {ValueType::DISCIO_REGION, {&MAIN_FALLBACK_REGION}},
    {ValueType::SHOW_CURSOR_VALUE_TYPE, {&MAIN_SHOW_CURSOR}}};
