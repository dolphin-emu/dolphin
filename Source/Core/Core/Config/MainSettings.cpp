...
static std::vector<USBUtils::DeviceInfo>
LoadUSBWhitelistFromString(const std::string& devices_string)
{
  std::vector<USBUtils::DeviceInfo> devices;
...
static std::string SaveUSBWhitelistToString(const std::vector<USBUtils::DeviceInfo>& devices)
{
  std::ostringstream oss;
  for (const auto& device : devices)
    oss << fmt::format("{:04x}:{:04x}", device.vid, device.pid) << ',';
