...
std::string GetStringVA(u32 str_reg)
{
  ...
  HLE::va_list ap(GPR(1) + 0x8, str_reg + 1);
    ...
    if (string[i] == '%')
    {
      ...
      case 's':
        result += StringFromFormat(ArgumentBuffer.c_str(),
                                   PowerPC::HostGetString(ap.get_arg_t<u32>()).c_str());
        break;
      ...
      case 'i':
      {
        if (string[i - 1] == 'l' && string[i - 2] == 'l')
          result += StringFromFormat(ArgumentBuffer.c_str(), ap.get_arg_t<u64>());
        else
          result += StringFromFormat(ArgumentBuffer.c_str(), ap.get_arg_t<u32>());
      ...
      case 'f':
      {
        result += StringFromFormat(ArgumentBuffer.c_str(), ap.get_arg_t<double>());
        break;
      }

      case 'p':
        // Override, so 64bit Dolphin prints 32bit pointers, since the ppc is 32bit :)
        result += StringFromFormat("%x", ap.get_arg_t<u32>());
        break;

      case 'n':
        PowerPC::HostWrite_U32(static_cast<u32>(result.size()), ap.get_arg_t<u32>());
