...
void Interpreter::srsh(const UDSPInstruction opc) const
{
...
// CR[0-7] | M. That is, the upper 8 bits of the address are the
// bottom 8 bits from CR, and the lower 8 bits are from the 8-bit immediate.
void Interpreter::srs(const UDSPInstruction opc) const
{
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::lrs(const UDSPInstruction opc) const
{
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::lr(const UDSPInstruction opc) const
{
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::sr(const UDSPInstruction opc) const
{
  auto& state = m_dsp_core.DSPState();
  const u8 reg = opc & 0x1F;
  const u16 addr = state.FetchInstruction();
...
void Interpreter::si(const UDSPInstruction opc) const
{
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::lrr(const UDSPInstruction opc) const
{
...
void Interpreter::lrrd(const UDSPInstruction opc) const
{
  const u8 sreg = (opc >> 5) & 0x3;
  const u8 dreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::lrri(const UDSPInstruction opc) const
{
  const u8 sreg = (opc >> 5) & 0x3;
  const u8 dreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::lrrn(const UDSPInstruction opc) const
{
  const u8 sreg = (opc >> 5) & 0x3;
  const u8 dreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::srr(const UDSPInstruction opc) const
{
...
void Interpreter::srrd(const UDSPInstruction opc) const
{
  const u8 dreg = (opc >> 5) & 0x3;
  const u8 sreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::srri(const UDSPInstruction opc) const
{
  const u8 dreg = (opc >> 5) & 0x3;
  const u8 sreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::srrn(const UDSPInstruction opc) const
{
  const u8 dreg = (opc >> 5) & 0x3;
  const u8 sreg = opc & 0x1f;
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::ilrr(const UDSPInstruction opc) const
{
...
void Interpreter::ilrrd(const UDSPInstruction opc) const
{
  const u16 reg = opc & 0x3;
  const u16 dreg = DSP_REG_ACM0 + ((opc >> 8) & 1);
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::ilrri(const UDSPInstruction opc) const
{
  const u16 reg = opc & 0x3;
  const u16 dreg = DSP_REG_ACM0 + ((opc >> 8) & 1);
  auto& state = m_dsp_core.DSPState();
...
void Interpreter::ilrrn(const UDSPInstruction opc) const
{
  const u16 reg = opc & 0x3;
  const u16 dreg = DSP_REG_ACM0 + ((opc >> 8) & 1);
  auto& state = m_dsp_core.DSPState();
