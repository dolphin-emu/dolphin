...
public:
  ...
  bool Disassemble(const std::vector<u16>& code, std::string& text) const;
  ...
  bool DisassembleOpcode(const std::vector<u16>& code, u16* pc, std::string& dest) const;
  bool DisassembleOpcode(const u16* binbuf, size_t binbuf_size, u16* pc, std::string& dest) const;

private:
  std::string DisassembleParameters(const DSPOPCTemplate& opc, u16 op1, u16 op2) const;
