...
void DSPEmitter::andcf(const UDSPInstruction opc)
{
  if (FlagsNeeded())
  {
    ...
    const FixupBranch notLogicZero = J_CC(CC_NE);
...
void DSPEmitter::cmp(const UDSPInstruction opc)
{
  if (FlagsNeeded())
  {
    ...
    constexpr X64Reg acc0 = RAX;
    get_long_acc(0, acc0);
    // const s64 acc1 = GetLongAcc(1);
    constexpr X64Reg acc1 = RDX;
    get_long_acc(1, acc1);
...
void DSPEmitter::cmpaxh(const UDSPInstruction opc)
{
  if (FlagsNeeded())
  {
    ...
    constexpr X64Reg acc = RAX;
    get_long_acc(sreg, acc);
    // s64 ax = GetAXHigh(rreg);
    constexpr X64Reg ax = RDX;
    get_ax_h(rreg, ax);
    // ax <<= 16;
    SHL(64, R(ax), Imm8(16));
...
{
  const u8 dreg = (opc >> 8) & 0x1;
  constexpr s64 subtract = 0x10000;
  // const s64 acc = GetLongAcc(dreg);
  constexpr X64Reg acc = RAX;
  get_long_acc(dreg, acc);
...
void DSPEmitter::abs(const UDSPInstruction opc)
{
  ...
  //	if (acc < 0) acc = 0 - acc;
  TEST(64, R(RAX), R(RAX));
  const FixupBranch GreaterThanOrEqual = J_CC(CC_GE);
  NEG(64, R(RAX));
  set_long_acc(dreg);
