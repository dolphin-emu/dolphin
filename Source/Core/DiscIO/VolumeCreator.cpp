...
static std::unique_ptr<IVolume>
CreateVolumeFromCryptedWiiImage(std::unique_ptr<IBlobReader> reader,
                                u32 partition_group, u32 volume_type,
                                u32 volume_number);
...
std::unique_ptr<IVolume> CreateVolumeFromFilename(const std::string &filename,
  ...
  case DISC_TYPE_WII_CONTAINER:
    return CreateVolumeFromCryptedWiiImage(std::move(reader), partition_group,
                                           0, volume_number);
  ...
  default:
    ...
    NOTICE_LOG(
        ...
        "Set Log Verbosity to Warning and attempt to load the game again to "
        "view the values",
        name.c_str());
  }
...
std::unique_ptr<IVolume> CreateVolumeFromDirectory(const std::string &directory,
                                                   bool is_wii,
                                                   const std::string &apploader,
                                                   const std::string &dol) {
  if (CVolumeDirectory::IsValidDirectory(directory))
    return std::make_unique<CVolumeDirectory>(directory, is_wii, apploader,
                                              dol);
...
void VolumeKeyForPartition(IBlobReader &_rReader, u64 offset, u8 *VolumeKey) {
  ...
  mbedtls_aes_setkey_dec(&AES_ctx, (using_korean_key == 1 && region == 'K'
                                        ? s_master_key_korean
                                        : s_master_key),
                         128);

  mbedtls_aes_crypt_cbc(&AES_ctx, MBEDTLS_AES_DECRYPT, 16, IV, SubKey,
                        VolumeKey);
}
...
CreateVolumeFromCryptedWiiImage(std::unique_ptr<IBlobReader> reader,
  ...
  u32 num_partitions;
  if (!big_endian_reader.ReadSwapped(0x40000 + (partition_group * 8),
                                     &num_partitions))
    return nullptr;
  ...
  for (SPartitionGroup &group : partition_groups) {
    for (u32 i = 0; i < num_partitions; i++) {
      ...
      if (big_endian_reader.ReadSwapped(partitions_offset + (i * 8) + 0,
                                        &partition_offset) &&
          big_endian_reader.ReadSwapped(partitions_offset + (i * 8) + 4,
                                        &partition_type)) {
        group.partitions.emplace_back((u64)partition_offset << 2,
                                      partition_type);
      }
    }
  }
  ...
  //  some partitions on SSBB use the ASCII title id of the demo VC game they
  //  hold...
  for (size_t i = 0; i < partition_groups[partition_group].partitions.size();
       i++) {
    const SPartition &partition =
        partition_groups[partition_group].partitions.at(i);

    if ((partition.type == volume_type && (int)volume_number == -1) ||
        i == volume_number) {
      u8 volume_key[16];
      VolumeKeyForPartition(*reader, partition.offset, volume_key);
      return std::make_unique<CVolumeWiiCrypted>(std::move(reader),
                                                 partition.offset, volume_key);
    }
  }
