    ...
    : m_sd_root(std::move(sd_root))
{
  ...
  // - Otherwise:
  //   - If the 'root' paramter of the current patch is not set or is empty, the path is relative to
  //     the folder the XML file is in.
  //   - If the 'root' paramter of the current patch starts with a '/', the path is relative to
  //     that folder on the SD card, starting at the SD card root.
  //   - If the 'root' paramter of the current patch starts without a '/', the path is relative to
                             ...
                             u64 raw_external_file_offset, u64 file_patch_length, bool resize)
{
  ...
  if (patch_size > 0)
  {
    ...
    if (external_filesize == 0)
    {
      BuilderContentSource padding{patch_start, patch_size, ContentFixedByte{0}};
      content.emplace(content.begin() + insert_where, std::move(padding));
    }
    else if (patch_size > external_filesize)
    {
      BuilderContentSource source = patch.m_file_data_loader->MakeContentSource(
          external_filename, external_file_offset, external_filesize, patch_start);
      ...
      content.emplace(content.begin() + insert_where + 1, std::move(padding));
    }
    else
    {
      BuilderContentSource source = patch.m_file_data_loader->MakeContentSource(
          external_filename, external_file_offset, patch_size, patch_start);
      content.emplace(content.begin() + insert_where, std::move(source));
    }
  }
  ...
  {
    if (content[i - 1].m_offset >= target_filesize)
                                  ...
                                  std::vector<DiscIO::FSTBuilderNode>* fst)
{
  ...
  {
    const std::string child_disc_path = disc_path + "/" + child.m_filename;
  ...
  for (const auto& patch : patches)
  {
    for (const auto& file : patch.m_file_patches)
    {
      if (!file.m_disc.empty() && file.m_disc[0] == '/')
      {
        ...
        DiscIO::FSTBuilderNode* node = FindFileNodeInFST(file.m_disc.substr(1), fst, file.m_create);
        if (node)
