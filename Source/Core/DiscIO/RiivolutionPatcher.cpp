                             ...
                             u64 raw_external_file_offset, u64 file_patch_length, bool resize)
{
  ...
  if (patch_size > 0)
  {
    ...
    {
      BuilderContentSource source{patch_start, external_filesize,
    ...
    {
      BuilderContentSource source{patch_start, patch_size,
                                  ContentFile{std::move(external_filename), external_file_offset}};
      content.emplace(content.begin() + insert_where, std::move(source));
    }
  }
  ...
  for (const auto& patch : patches)
  {
    ...
    for (const auto& folder : patch.m_folder_patches)
    {
      ...
      if (disc_path.empty())
        ...
        ApplyFolderPatchToFST(patch, folder, external_files, std::string(disc_path), fst);
    }
  }
...
static void ApplyMemoryPatchWithoutAddress(const Patch& patch, const Memory& memory_patch)
{
...
void ApplyPatchesToMemory(const std::vector<Patch>& patches)
{
  for (const auto& patch : patches)
  {
    for (const auto& memory : patch.m_memory_patches)
    {
      ...
      if (memory.m_search)
        ApplyMemoryPatchWithoutAddress(patch, memory);
      else
