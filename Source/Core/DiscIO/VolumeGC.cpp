...
bool CVolumeGC::Read(u64 _Offset, u64 _Length, u8 *_pBuffer,
                     bool decrypt) const {
  if (decrypt)
...
CVolumeGC::GetNames(bool prefer_long) const {
...
std::vector<u32> CVolumeGC::GetBanner(int *width, int *height) const {
  ...
  ColorUtil::decode5A3image(image_buffer.data(), m_banner_file.image,
                            GC_BANNER_WIDTH, GC_BANNER_HEIGHT);
...
IVolume::EPlatform CVolumeGC::GetVolumeType() const { return GAMECUBE_DISC; }
...
bool CVolumeGC::LoadBannerFile() const {
  ...
  if (file_size == BNR1_SIZE || file_size == BNR2_SIZE) {
    file_system->ReadFile("opening.bnr", reinterpret_cast<u8 *>(&m_banner_file),
                          file_size);

    if (file_size == BNR1_SIZE && m_banner_file.id == 0x31524e42) // "BNR1"
    {
      m_banner_file_type = BANNER_BNR1;
    } else if (file_size == BNR2_SIZE &&
               m_banner_file.id == 0x32524e42) // "BNR2"
    {
      m_banner_file_type = BANNER_BNR2;
    } else {
      m_banner_file_type = BANNER_INVALID;
      WARN_LOG(DISCIO, "Invalid opening.bnr. Type: %0x Size: %0zx",
               m_banner_file.id, file_size);
    }
  } else {
    m_banner_file_type = BANNER_INVALID;
...
CVolumeGC::ReadMultiLanguageStrings(bool description, bool prefer_long) const {
  ...
  case BANNER_BNR1: // NTSC
    ...
    start_language = is_japanese ? ELanguage::LANGUAGE_JAPANESE
