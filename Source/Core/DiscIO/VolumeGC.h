...
private:
  ...
  std::map<ELanguage, std::string>
  ReadMultiLanguageStrings(bool description, bool prefer_long = true) const;
  ...
  struct GCBannerComment {
    ...
    char longMaker[64];  // Long developer, publisher names shown in IPL game
                         // start screen
    char comment[128]; // Game description shown in IPL game start screen in two
                       // lines.
  };
  ...
  struct GCBanner {
    ...
    GCBannerComment
        comment[6]; // Comments in six languages (only one for BNR1 type)
  };

  static const size_t BNR1_SIZE =
      sizeof(GCBanner) - sizeof(GCBannerComment) * 5;
