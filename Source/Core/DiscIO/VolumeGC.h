...
private:
	...
	struct GCBannerInformation
	{
		...
		char long_maker[64];  // Long developer, publisher names shown in IPL game start screen
		char description[128];   // Game description shown in IPL game start screen in two lines.
	};
	...
	struct GCBanner
	{
		...
		GCBannerInformation information[6]; // information comes in six languages (only one for BNR1 type)
	};
	...
	void ExtractBannerInformation(GCBanner banner_file) const;
	...
	mutable std::vector<u32> image_buffer;
