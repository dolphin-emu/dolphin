...
CVolumeDirectory::CVolumeDirectory(const std::string &_rDirectory, bool _bIsWii,
    ...
    : m_totalNameSize(0), m_dataStartAddress(-1),
      m_diskHeader(DISKHEADERINFO_ADDRESS),
      m_diskHeaderInfo(std::make_unique<SDiskHeaderInfo>()), m_fst_address(0),
      m_dol_address(0) {
...
bool CVolumeDirectory::Read(u64 _Offset, u64 _Length, u8 *_pBuffer,
  ...
  if (_Offset < DISKHEADERINFO_ADDRESS) {
    WriteToBuffer(DISKHEADER_ADDRESS, DISKHEADERINFO_ADDRESS,
                  m_diskHeader.data(), _Offset, _Length, _pBuffer);
  }
  // header info
  if (_Offset >= DISKHEADERINFO_ADDRESS && _Offset < APPLOADER_ADDRESS) {
    WriteToBuffer(DISKHEADERINFO_ADDRESS, sizeof(m_diskHeaderInfo),
                  (u8 *)m_diskHeaderInfo.get(), _Offset, _Length, _pBuffer);
  }
  // apploader
  if (_Offset >= APPLOADER_ADDRESS &&
      _Offset < APPLOADER_ADDRESS + m_apploader.size()) {
    WriteToBuffer(APPLOADER_ADDRESS, m_apploader.size(), m_apploader.data(),
                  _Offset, _Length, _pBuffer);
  }
  // dol
  if (_Offset >= m_dol_address && _Offset < m_dol_address + m_DOL.size()) {
    WriteToBuffer(m_dol_address, m_DOL.size(), m_DOL.data(), _Offset, _Length,
                  _pBuffer);
  }
  // fst
  if (_Offset >= m_fst_address && _Offset < m_dataStartAddress) {
    WriteToBuffer(m_fst_address, m_FSTData.size(), m_FSTData.data(), _Offset,
                  _Length, _pBuffer);
  }
  ...
  std::map<u64, std::string>::const_iterator fileIter =
      m_virtualDisk.lower_bound(_Offset);
...
std::string CVolumeDirectory::GetUniqueID() const {
  return std::string(m_diskHeader.begin(),
                     m_diskHeader.begin() + MAX_ID_LENGTH);
}
...
CVolumeDirectory::GetNames(bool prefer_long) const {
...
std::string
CVolumeDirectory::ExtractDirectoryName(const std::string &_rDirectory) {
...
bool CVolumeDirectory::SetApploader(const std::string &_rApploader) {
  if (!_rApploader.empty()) {
    ...
    m_dol_address =
        ROUND_UP(APPLOADER_ADDRESS + m_apploader.size() + 0x20, 0x20ull);
    return true;
  } else {
...
void CVolumeDirectory::WriteToBuffer(u64 _SrcStartAddress, u64 _SrcLength,
                                     const u8 *_Src, u64 &_Address,
...
void CVolumeDirectory::PadToAddress(u64 _StartAddress, u64 &_Address,
                                    u64 &_Length, u8 *&_pBuffer) const {
...
void CVolumeDirectory::Write32(u32 data, u32 offset,
...
void CVolumeDirectory::WriteEntryData(u32 &entryOffset, u8 type, u32 nameOffset,
                                      u64 dataOffset, u64 length) {
...
void CVolumeDirectory::WriteEntryName(u32 &nameOffset,
                                      const std::string &name) {
  strncpy((char *)&m_FSTData[nameOffset + m_fstNameOffset], name.c_str(),
          name.length() + 1);

  nameOffset += (u32)(name.length() + 1);
}

void CVolumeDirectory::WriteEntry(const File::FSTEntry &entry, u32 &fstOffset,
                                  u32 &nameOffset, u64 &dataOffset,
                                  u32 parentEntryNum) {
  if (entry.isDirectory) {
    ...
    _dbg_assert_(DVDINTERFACE,
                 m_virtualDisk.find(dataOffset) == m_virtualDisk.end());
    m_virtualDisk.emplace(dataOffset, entry.physicalName);

    // 4 byte aligned
    dataOffset =
        ROUND_UP(dataOffset + std::max<u64>(entry.size, 1ull), 0x8000ull);
  }
}
