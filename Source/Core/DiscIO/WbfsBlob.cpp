...
WbfsFileReader::WbfsFileReader(const std::string &filename)
  ...
  m_files[0]->file.Seek(m_hd_sector_size +
                            WII_DISC_HEADER_SIZE /*+ i * m_disc_info_size*/,
                        SEEK_SET);
  m_files[0]->file.ReadBytes(m_wlba_table.data(),
                             m_blocks_per_disc * sizeof(u16));
...
bool WbfsFileReader::ReadHeader() {
  ...
  m_blocks_per_disc =
      (WII_SECTOR_COUNT * WII_SECTOR_SIZE + m_wbfs_sector_size - 1) /
      m_wbfs_sector_size;
  m_disc_info_size = align(
      WII_DISC_HEADER_SIZE + m_blocks_per_disc * sizeof(u16), m_hd_sector_size);
  ...
  if (base_cluster < m_blocks_per_disc) {
    ...
    for (u32 i = 0; i != m_total_files; i++) {
      if (final_address < (m_files[i]->base_address + m_files[i]->size)) {
        m_files[i]->file.Seek(final_address - m_files[i]->base_address,
                              SEEK_SET);
        if (available) {
          u64 till_end_of_file =
              m_files[i]->size - (final_address - m_files[i]->base_address);
...
std::unique_ptr<WbfsFileReader>
WbfsFileReader::Create(const std::string &filename) {
...
bool IsWbfsBlob(const std::string &filename) {
  ...
  return (magic[0] == 'W') && (magic[1] == 'B') && (magic[2] == 'F') &&
         (magic[3] == 'S');
}
