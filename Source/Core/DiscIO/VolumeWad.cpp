...
CVolumeWAD::CVolumeWAD(IBlobReader* _pReader)
	: m_pReader(_pReader), m_offset(0), m_TMD_offset(0), m_opening_banner_offset(0),
	m_header_size(0), m_cert_chain_size(0), m_ticket_size(0), m_TMD_size(0), m_data_app_size(0)
{
	...
	m_opening_banner_offset = m_TMD_offset + ALIGN_40(m_TMD_size) + ALIGN_40(m_data_app_size);
}
...
std::vector<std::string> CVolumeWAD::GetNames() const
{
	...
	for (int i = 0; i != 10; ++i)
	{
		...
		if (footer_size < 0xF1 || !Read(0x9C + (i * bytes_length) + m_opening_banner_offset, bytes_length, (u8*)&temp))
		{
			names.push_back("");
		}
		else
