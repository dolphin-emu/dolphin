...
CVolumeWAD::CVolumeWAD(std::unique_ptr<IBlobReader> reader)
    : m_pReader(std::move(reader)), m_offset(0), m_tmd_offset(0),
      m_opening_bnr_offset(0), m_hdr_size(0), m_cert_size(0), m_tick_size(0),
      m_tmd_size(0), m_data_size(0) {
  ...
  m_tmd_offset =
      ALIGN_40(m_hdr_size) + ALIGN_40(m_cert_size) + ALIGN_40(m_tick_size);
  m_opening_bnr_offset =
      m_tmd_offset + ALIGN_40(m_tmd_size) + ALIGN_40(m_data_size);
}

CVolumeWAD::~CVolumeWAD() {}

bool CVolumeWAD::Read(u64 _Offset, u64 _Length, u8 *_pBuffer,
                      bool decrypt) const {
  if (decrypt)
...
std::string CVolumeWAD::GetMakerID() const {
  ...
  // there
  if (!Read(0x198 + m_tmd_offset, 2, (u8 *)temp) || temp[0] == 0 ||
      temp[1] == 0)
    return "00";
...
IVolume::EPlatform CVolumeWAD::GetVolumeType() const { return WII_WAD; }

std::map<IVolume::ELanguage, std::string>
CVolumeWAD::GetNames(bool prefer_long) const {
