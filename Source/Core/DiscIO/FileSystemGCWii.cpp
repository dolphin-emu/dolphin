...
CFileSystemGCWii::CFileSystemGCWii(const IVolume *_rVolume)
    : IFileSystem(_rVolume), m_Initialized(false), m_Valid(false),
      m_Wii(false) {
  m_Valid = DetectFileSystem();
}

CFileSystemGCWii::~CFileSystemGCWii() { m_FileInfoVector.clear(); }
...
u64 CFileSystemGCWii::ReadFile(const std::string &_rFullPath, u8 *_pBuffer,
                               u64 _MaxBufferSize, u64 _OffsetInFile) {
  if (!m_Initialized)
    InitFileSystem();
  ...
  u64 read_length =
      std::min(_MaxBufferSize, pFileInfo->m_FileSize - _OffsetInFile);

  DEBUG_LOG(DISCIO, "Reading %" PRIx64 " bytes at %" PRIx64
                    " from file %s. Offset: %" PRIx64 " Size: %" PRIx64,
            read_length, _OffsetInFile, _rFullPath.c_str(), pFileInfo->m_Offset,
            pFileInfo->m_FileSize);

  m_rVolume->Read(pFileInfo->m_Offset + _OffsetInFile, read_length, _pBuffer,
                  m_Wii);
  return read_length;
}
...
bool CFileSystemGCWii::ExportApploader(
    const std::string &_rExportFolder) const {
  u32 apploader_size;
  u32 trailer_size;
...
bool CFileSystemGCWii::DetectFileSystem() {
  u32 magic_bytes;
  if (m_rVolume->ReadSwapped(0x18, &magic_bytes, false) &&
      magic_bytes == 0x5D1C9EA3) {
    m_Wii = true;
    return true;
  } else if (m_rVolume->ReadSwapped(0x1c, &magic_bytes, false) &&
             magic_bytes == 0xC2339F3D) {
    m_Wii = false;
    return true;
  }
...
void CFileSystemGCWii::InitFileSystem() {
  ...
  // more than total RAM in a Wii. No file system should use anywhere near that
  // much.
  ...
  for (u32 i = 0; i < root.m_FileSize; i++) {
    ...
    m_FileInfoVector.emplace_back(name_offset,
                                  static_cast<u64>(offset) << shift, size);
    NameTableOffset += 0xC;
  }
...
size_t CFileSystemGCWii::BuildFilenames(const size_t _FirstIndex,
                                        const size_t _LastIndex,
                                        const std::string &_szDirectory,
                                        u64 _NameTableOffset) {
  ...
  while (CurrentIndex < _LastIndex) {
    ...
    // check next index
    CurrentIndex =
        BuildFilenames(CurrentIndex + 1, (size_t)rFileInfo.m_FileSize,
...
u32 CFileSystemGCWii::GetOffsetShift() const { return m_Wii ? 2 : 0; }
