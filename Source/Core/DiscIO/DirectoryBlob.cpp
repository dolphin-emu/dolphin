...
constexpr u64 NONENCRYPTED_DISKHEADER_SIZE = 0x100;
      ...
      m_disk_header(DISKHEADER_SIZE), m_fst_address(0), m_dol_address(0)
{
  ...
  m_virtual_disc.emplace(DISKHEADER_ADDRESS, BI2_ADDRESS, m_disk_header.data());
  ...
  if (m_is_wii)
  {
    m_nonpartition_contents.emplace(DISKHEADER_ADDRESS, NONENCRYPTED_DISKHEADER_SIZE,
                                    m_disk_header_nonencrypted.data());
...
void DirectoryBlobReader::SetDiscHeaderAndDiscType()
{
  ...
  if (m_is_wii)
  {
    m_disk_header_nonencrypted.resize(NONENCRYPTED_DISKHEADER_SIZE);
    ...
    const std::string header_bin_path = m_root_directory + "disc/header.bin";
    {
      ...
      const u64 bytes_to_read = std::min(header_bin.GetSize(), NONENCRYPTED_DISKHEADER_SIZE);
      header_bin.ReadArray<u8>(m_disk_header_nonencrypted.data(), bytes_to_read,
                               &header_bin_bytes_read);
    }

    // If the read failed or was smaller than expected, use the content of sys/boot.bin instead
    const u64 bytes_from_boot_bin = NONENCRYPTED_DISKHEADER_SIZE - header_bin_bytes_read;
    std::copy(m_disk_header.data(), m_disk_header.data() + bytes_from_boot_bin,
              m_disk_header_nonencrypted.data());

    // 0x60 and 0x61 are the only differences between the encrypted and nonencrypted headers
    if (header_bin_bytes_read < 0x60)
      m_disk_header_nonencrypted[0x60] = 0;
    if (header_bin_bytes_read < 0x61)
      m_disk_header_nonencrypted[0x61] = 0;
  }
}
