...
static std::optional<u32> ParsePartitionDirectoryName(const std::string& name)
{
  if (name.size() < 2)
    return {};
  ...
  if (uppercase_name[0] == 'P')
  {
    if (name[1] == '-', name.size() == 6)
      return Common::swap32(reinterpret_cast<const u8*>(name.data() + 2));
                                         ...
                                         const std::string& true_root)
{
  ...
  else
  {
    ...
    {
      PartitionWithType(DirectoryBlobPartition&& partition_, u32 type_)
    ...
    // Contains partitions paired with partition types
    std::vector<PartitionWithType> partitions;
    partitions.emplace_back(std::move(game_partition), DATA_PARTITION);
    ...
    if (ParsePartitionDirectoryName(game_partition_directory_name) == u32(DATA_PARTITION))
    {
      ...
      for (const File::FSTEntry& entry : true_root_entry.children)
      {
        ...
        {
          std::optional<u32> partition_type = ParsePartitionDirectoryName(entry.virtualName);
          if (partition_type && *partition_type != DATA_PARTITION)
          {
    ...
    std::sort(partitions.begin(), partitions.end(),
              [](const PartitionWithType& lhs, const PartitionWithType& rhs) {
                ...
                // Ascending sort by partition type, except UPDATE (1) comes before before DATA (0)
                return (lhs.type > UPDATE_PARTITION || rhs.type > UPDATE_PARTITION) ?
                           lhs.type < rhs.type :
                           lhs.type > rhs.type;
              });
    ...
    for (PartitionWithType& partition : partitions)
    {
      ...
      offset_in_table += 4;
      Write32(partition.type, offset_in_table, &m_partition_table);
      offset_in_table += 4;
