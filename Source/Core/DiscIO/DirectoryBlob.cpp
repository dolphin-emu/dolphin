                                                    ...
                                                    const std::vector<u8>& header_bin)
{
  ...
  std::memcpy(m_disc_header_nonpartition.data(), header_bin.data(),
              std::min<size_t>(header_bin.size(), WII_NONPARTITION_DISCHEADER_SIZE));
...
void DirectoryBlobReader::SetPartitions(std::vector<PartitionWithType>&& partitions)
{
  ...
  for (size_t i = 0; i < partitions.size(); ++i)
  {
    ...
    m_partitions.emplace(partition_offset, std::move(partitions[i].partition));
    m_nonpartition_contents.Add(partition_offset, data_size,
                                ContentPartition{this, 0, partition_offset});
...
void DirectoryBlobPartition::SetDiscHeader(const std::vector<u8>& boot_bin)
{
  ...
  std::memcpy(m_disc_header.data(), boot_bin.data(),
              std::min<size_t>(boot_bin.size(), DISCHEADER_SIZE));
...
void DirectoryBlobPartition::SetBI2(const std::vector<u8>& bi2)
{
  ...
  std::memcpy(m_bi2.data(), bi2.data(), std::min<size_t>(bi2.size(), BI2_SIZE));
