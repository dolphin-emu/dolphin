...
#else
  const std::string& normaliszed_dol_path = dol_path;
#endif
...
void DirectoryBlobReader::SetDiscHeaderAndDiscType()
{
  ...
  if (m_is_wii)
  {
    ...
    u64 header_bin_bytes_read;
    const std::string header_bin_path = m_root_directory + "disc/header.bin";
    {
      File::IOFile header_bin(header_bin_path, "rb");
      const u64 bytes_to_read = std::min<u64>(header_bin.GetSize(), NONENCRYPTED_DISKHEADER_SIZE);
    ...
    // 0x60 and 0x61 are the only differences between encrypted and nonencrypted headers
