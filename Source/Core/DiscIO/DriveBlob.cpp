...
DriveReader::DriveReader(const std::string &drive) {
  ...
  if (IsOK()) {
    ...
    if (!ReadFile(m_disc_handle, buffer.data(), GetSectorSize(), &not_used,
                  nullptr)) {
  ...
  if (IsOK()) {
    ...
    DeviceIoControl(m_disc_handle, IOCTL_CDROM_MEDIA_REMOVAL, &m_lock_cdrom,
                    sizeof(m_lock_cdrom), nullptr, 0, &dwNotUsed, nullptr);
#endif
#else
  ...
  if (m_file) {
    ...
    NOTICE_LOG(DISCIO, "Load from DVD backup failed or no disc in drive %s",
               drive.c_str());
  }
}
...
bool DriveReader::ReadMultipleAlignedBlocks(u64 block_num, u64 num_blocks,
                                            u8 *out_ptr) {
#ifdef _WIN32
  ...
  DWORD bytes_read;
  if (!ReadFile(m_disc_handle, out_ptr,
                static_cast<DWORD>(GetSectorSize() * num_blocks), &bytes_read,
                nullptr)) {
