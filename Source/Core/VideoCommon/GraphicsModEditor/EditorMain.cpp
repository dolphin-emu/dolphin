...
{
  m_editor_events = std::make_unique<EditorEvents>();
  if (!RebuildState())
    return false;

  ImGui::GetIO().ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;
  m_last_mesh_dump_request = std::make_unique<SceneRecordingRequest>();
...
bool EditorMain::RebuildState()
{
  ...
  m_change_occurred_event =
      m_editor_events->change_occurred_event.Register([this] { OnChangeOccured(); });
  ...
  m_asset_reload_event = m_editor_events->asset_reload_event.Register(
      [&custom_resource_manager, this](const VideoCommon::CustomAssetLibrary::AssetID& asset_id) {
        if (asset_id != "")
        {
          custom_resource_manager.MarkAssetDirty(asset_id);
          OnChangeOccured();
        }
      });

  const std::string graphics_mod_builtin =
      WithUnifiedPathSeparators(File::GetSysDirectory() + GRAPHICSMOD_DIR + "/Builtin");
  const std::filesystem::path graphics_mod_builtin_fs{graphics_mod_builtin};
  m_state->m_user_data.m_asset_library->AddBuiltinAsset(
      graphics_mod_builtin_fs / "Shaders" / "pbr.rastershader", "__dolphin_pbr");
  m_state->m_user_data.m_asset_library->AddBuiltinAsset(
      graphics_mod_builtin_fs / "Shaders" / "pbr_arm.rastershader", "__dolphin_pbr_arm");
  m_state->m_user_data.m_asset_library->Watch(PathToString(graphics_mod_builtin_fs / "Shaders"));
  ...
  {
    VideoCommon::Assets::AssetMap normal_visual_shader_asset_map;
    normal_visual_shader_asset_map.try_emplace("metadata", pipeline_path_root_fs /
                                                               "normal_visualization" /
                                                               "normal-visualization.rastershader");
    normal_visual_shader_asset_map.try_emplace("pixel_shader", pipeline_path_root_fs /
                                                                   "normal_visualization" /
                                                                   "normal-visualization.ps.glsl");
    normal_visual_shader_asset_map.try_emplace("vertex_shader", pipeline_path_root_fs /
                                                                    "normal_visualization" /
                                                                    "normal-visualization.vs.glsl");
    m_state->m_editor_data.m_asset_library->SetAssetIDMapData(
        "normal_visualization_shader", std::move(normal_visual_shader_asset_map));

    VideoCommon::Assets::AssetMap normal_visual_material_asset_map;
    normal_visual_material_asset_map.try_emplace(
        "", pipeline_path_root_fs / "normal_visualization" / "normal-visualization.rastermaterial");
    m_state->m_editor_data.m_asset_library->SetAssetIDMapData(
        "editor-normal-vis-material", std::move(normal_visual_material_asset_map));
  }
  ...
  if (ImGui::BeginMainMenuBar())
  {
    ...
    if (ImGui::BeginMenu("Scene"))
    {
                          ...
                          m_editor_session_in_progress))
      {
        ...
        {
          picojson::object obj;
          obj.emplace("draw_call_id", fmt::to_string(std::to_underlying(draw_call_id)));
      ...
      if (ImGui::MenuItem("View normals", nullptr, &m_state->m_editor_data.m_view_normals))
      {
      }
      ImGui::EndMenu();
    }
...
EditorEvents& EditorMain::GetEditorEvents()
{
  return *m_editor_events;
}
}  // namespace GraphicsModEditor
