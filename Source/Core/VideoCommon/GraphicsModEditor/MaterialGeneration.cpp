...
void GenerateMaterials(MaterialGenerationContext* generation_context, std::string* error)
{
  ...
  const auto files =
      Common::DoFileSearch({std::string{generation_context->input_path}}, {".png", ".dds"}, true);
  for (const auto& filename : files)
  {
    const std::filesystem::path filepath{filename};
    std::filesystem::copy(filepath, output_dir, std::filesystem::copy_options::overwrite_existing,
                          ec);
    if (ec)
    {
      *error = fmt::format("Failed to copy file '{}', error was {}", filename, ec);
      return;
    }

    generation_context->state->m_user_data.m_asset_library->AddAsset(output_dir /
                                                                     filepath.filename());
    const auto texture_asset =
        generation_context->state->m_user_data.m_asset_library->GetAssetFromPath(
            output_dir / filepath.filename());
    if (!texture_asset)
    {
      *error = fmt::format("Failed to asset from path '{}'",
                           PathToString(output_dir / filepath.filename()));
      return;
    }
  ...
  for (const auto& lookup : texture_lookups)
  {
         ...
         generation_context->material_property_index_to_texture_filter)
    {
      ...
      {
        output_material.properties[index].m_value = asset_id_iter->second;
      }
      else
      {
        skip_count++;
    ...
    if (skip_count == generation_context->material_property_index_to_texture_filter.size())
      continue;
