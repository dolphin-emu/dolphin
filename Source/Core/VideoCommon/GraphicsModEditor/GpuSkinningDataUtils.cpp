                       ...
                       VideoCommon::MeshData* final_mesh_data)
{
       ...
       chunk_index++)
  {
    ...
    while (indice_index < reference_chunk.num_indices)
    {
      ...
      const auto draw_call =
          GetClosestDrawCall(v0_transformed, v1_transformed, v2_transformed, native_mesh_data);
      ...
      {
        intermediate_chunk_data.indexes.emplace_back(i0, *original_vertex_index);
      }

      if (const auto original_vertex_index =
              GetNearestOriginalVertexIndex(v1_transformed, draw_call, native_mesh_data))
      {
        intermediate_chunk_data.indexes.emplace_back(i1, *original_vertex_index);
      }

      if (const auto original_vertex_index =
              GetNearestOriginalVertexIndex(v2_transformed, draw_call, native_mesh_data))
      {
        intermediate_chunk_data.indexes.emplace_back(i2, *original_vertex_index);
      }
  ...
  {
    auto& per_draw_replacement_chunk_list = final_mesh_data->m_gpu_skinning_chunks[draw_call];
    ...
    for (const auto& [chunk_index, intermediate_data] : chunk_to_mesh_data)
    {
      ...
      final_replacement_chunk.components_available = reference_chunk.components_available;
      final_replacement_chunk.material_name = reference_chunk.material_name;
      final_replacement_chunk.primitive_type = reference_chunk.primitive_type;
      final_replacement_chunk.transform = reference_chunk.transform;
      final_replacement_chunk.vertex_declaration = reference_chunk.vertex_declaration;
      final_replacement_chunk.vertex_stride = reference_chunk.vertex_declaration.stride;
      ...
      for (const auto& [replacement_index, native_index] : intermediate_data.indexes)
      {
...
void CalculateMeshDataWithSkinning(const NativeMeshData& native_mesh_data,
                                   ...
                                   u32 components_to_copy, VideoCommon::MeshData* final_mesh_data)
{
  CalculateMeshData(native_mesh_data, reference_mesh_data, components_to_copy, final_mesh_data);
}
}  // namespace GraphicsModEditor
