...
{
  picojson::object obj;
  VideoCommon::RasterSurfaceShaderData::ToJson(obj, context.shader_template);

  return GenerateShader(output_path, context.pixel_name, context.vertex_name, context.metadata_name,
                        context.shader_template.pixel_source, context.shader_template.vertex_source,
                        picojson::value{obj}.serialize(), source);
}
