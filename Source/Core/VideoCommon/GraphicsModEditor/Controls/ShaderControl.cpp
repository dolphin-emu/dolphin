                              ...
                              VideoCommon::RasterShaderData* shader)
{
  ...
  if (ImGui::CollapsingHeader("Properties", ImGuiTreeNodeFlags_DefaultOpen))
  {
    parse_properties(shader->m_uniform_properties);
    ImGui::Separator();
    AddShaderProperty(shader, &shader->m_uniform_properties, "pixel");
  }
  ...
  if (ImGui::CollapsingHeader("Textures", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    for (std::size_t i = 0; i < shader->m_samplers.size(); i++)
    {
      ImGui::PushID(static_cast<int>(i));
      auto& sampler = shader->m_samplers[i];
    ...
    {
      std::erase(shader->m_samplers, *sampler);
    }
    ...
    if (ImGui::Button("Add"))
    {
      ...
      shader->m_samplers.push_back(std::move(sampler_data));
      GraphicsModEditor::EditorEvents::ChangeOccurredEvent::Trigger();
    }
    ...
    std::map<std::string, VideoCommon::ShaderProperty2>* properties, std::string_view type)
{
  ...
  if (ImGui::BeginPopupModal(fmt::format("AddShader{}PropPopup", type).c_str(), nullptr))
  {
    ...
    if (ImGui::Button("Add"))
    {
      ...
      shader->m_uniform_properties.insert_or_assign(m_add_property_name,
                                                    std::move(shader_property));
      GraphicsModEditor::EditorEvents::ChangeOccurredEvent::Trigger();
      ImGui::CloseCurrentPopup();
    }
    ImGui::SameLine();
