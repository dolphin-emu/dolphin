...
AssetBrowserPanel::AssetBrowserPanel(EditorState& state) : m_state(state)
{
  ...
  m_browse_event = GetEditorEvents().jump_to_asset_in_browser_event.Register(
  ...
  if (ImGui::BeginTable("AsetBrowserContentTable", column_count))
  {
    ...
    if (m_is_mesh_import_active)
    {
      if (Controls::ShowMeshImportWindow(m_mesh_import_filename, &m_mesh_import_import_materials,
                                         &m_mesh_import_treat_as_cpu_skinned,
                                         &m_mesh_import_components_to_copy))
      {
        m_is_mesh_import_active = false;
        m_mesh_import_import_materials = false;
        m_mesh_import_treat_as_cpu_skinned = false;
        m_mesh_import_components_to_copy = 0;
      }
    }
                                           ...
                                           ImGuiPopupFlags_NoOpenOverExistingPopup))
    {
      if (ImGui::BeginMenu("Create"))
      {
        ...
        {
          if (ImGui::MenuItem("Standard Material"))
          {
            VideoCommon::MaterialData material;
            material.shader_asset = "__dolphin_pbr";
            const std::string name = FindNewName("New Material", ".rastermaterial", m_current_path);
            const auto asset = GenerateMaterial(name, m_current_path, material,
                                                m_state.m_user_data.m_asset_library.get());
            if (asset)
            {
              auto& editor_events = GetEditorEvents();
              editor_events.items_selected_event.Trigger(std::set<SelectableType>{asset});
              editor_events.change_occurred_event.Trigger();
              BeginRename(asset->m_asset_id, PathToString(asset->m_asset_path.stem()));
            }
          }
          if (ImGui::MenuItem("Standard Material (ARM)"))
          {
            VideoCommon::MaterialData material;
            material.shader_asset = "__dolphin_pbr_arm";
            const std::string name = FindNewName("New Material", ".rastermaterial", m_current_path);
            const auto asset = GenerateMaterial(name, m_current_path, material,
                                                m_state.m_user_data.m_asset_library.get());
            if (asset)
          ...
          if (ImGui::MenuItem("Custom Material"))
          {
            const std::string name = FindNewName("New Material", ".rastermaterial", m_current_path);
        ...
        if (ImGui::MenuItem("Render Target"))
        {
          ...
          data.sampler = {};
          data.texture_type = AbstractTextureType::Texture_2D;
          data.width = 128;
          data.height = 128;
          data.render_type = VideoCommon::RenderTargetData::RenderType::ScreenColor;
          const auto asset =
      ...
      if (ImGui::BeginMenu("Generate Materials From Textures"))
      {
        const auto set_material_generation_details = [this] {
        ...
        if (ImGui::MenuItem("Standard Material..."))
        {
          set_material_generation_details();
          m_material_generation_context.material_template_data.shader_asset = "__dolphin_pbr";
        }
        if (ImGui::MenuItem("Standard Material (ARM)..."))
        {
          set_material_generation_details();
          m_material_generation_context.material_template_data.shader_asset = "__dolphin_pbr_arm";
        }
        if (ImGui::MenuItem("Custom Material..."))
        {
          set_material_generation_details();
                                    ...
                                    AbstractTexture* icon_texture, std::string_view drag_drop_name)
{
  ...
  if (asset)
  {
    ...
    {
      EndRename();
      GetEditorEvents().items_selected_event.Trigger(std::set<SelectableType>{asset});
    }
    if (ImGui::BeginPopupContextItem())
    {
      EndRename();
      if (ImGui::MenuItem("Remove from project"))
      {
        ...
        auto& editor_events = GetEditorEvents();
        editor_events.items_selected_event.Trigger(std::set<SelectableType>{});
        editor_events.change_occurred_event.Trigger();
      }
    ...
    if (m_renamed_asset_id == asset->m_asset_id)
    {
                                ...
                                ImGuiWindowFlags_NoResize | ImGuiWindowFlags_ChildWindow))
      {
        ...
        const ImVec4 black_border(0, 0, 0, 1);
        ImGui::Image(*m_state.m_editor_data.m_name_to_texture["error"].get(), ImVec2(25, 25),
                     ImVec2(0, 0), ImVec2(1, 1), red_tint, black_border);
        ImGui::SameLine();
