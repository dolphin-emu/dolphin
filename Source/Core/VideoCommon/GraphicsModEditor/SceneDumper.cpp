                                     ...
                                     AdditionalDrawData additional_draw_data)
{
  ...
  if (m_record_request.m_save_original_mesh_data)
  {
    const auto transform_type =
        declaration.posmtx.enable ?
            m_record_request.m_apply_gpu_skinning ? TransformType::GPU : TransformType::None :
            TransformType::Object;
    m_gpu_skinning_dumper.AddData(draw_call_id, draw_data, transform_type,
  ...
  else
  {
    ...
    else
    {
      ...
      if (draw_data.uid->blending_state.blend_enable && m_record_request.m_enable_blending)
      {
  ...
  index_accessor.name = fmt::format("Accessor 'INDEX' for {}", std::to_underlying(draw_call_id));
  ...
  if (declaration.position.enable)
  {
    ...
    accessor.name = fmt::format("Accessor 'POSITION' for {}", std::to_underlying(draw_call_id));
    ...
    accessor.count = draw_data.vertex_count;
    ...
    vert_ptr = draw_data.vertex_data;
    for (u32 vert_index = 0; vert_index < draw_data.vertex_count; vert_index++)
    {
      Common::Vec3 vertex_position;
      std::memcpy(&vertex_position.x, vert_ptr + accessor.byteOffset, sizeof(float));
  ...
  for (std::size_t i = 0; i < declaration.colors.size(); i++)
  {
    if (declaration.colors[i].enable)
    {
      ...
      tinygltf::Accessor accessor;
      accessor.name =
          fmt::format("Accessor '{}' for {}", color_names[i], std::to_underlying(draw_call_id));
      accessor.bufferView = static_cast<int>(m_impl->m_model.bufferViews.size());
  ...
  for (std::size_t i = 0; i < declaration.normals.size(); i++)
  {
    if (declaration.normals[i].enable)
    {
      ...
      tinygltf::Accessor accessor;
      accessor.name =
          fmt::format("Accessor '{}' for {}", norm_names[i], std::to_underlying(draw_call_id));
      accessor.bufferView = static_cast<int>(m_impl->m_model.bufferViews.size());
      accessor.componentType = ComponentFormatAndIntegerToComponentType(
  ...
  for (std::size_t i = 0; i < declaration.texcoords.size(); i++)
  {
    ...
    if (declaration.texcoords[i].enable && declaration.texcoords[i].components == 2)
    {
      ...
      tinygltf::Accessor accessor;
      accessor.name =
          fmt::format("Accessor '{}' for {}", texcoord_names[i], std::to_underlying(draw_call_id));
      accessor.bufferView = static_cast<int>(m_impl->m_model.bufferViews.size());
      accessor.componentType = ComponentFormatAndIntegerToComponentType(
  ...
  vertex_buffer_view.byteLength = draw_data.vertex_count * stride;
  ...
  const auto vert_data = static_cast<const unsigned char*>(draw_data.vertex_data);
  ...
  // Node data
  tinygltf::Node node;
  node.name = fmt::format("Node {} for xfb {}", std::to_underlying(draw_call_id),
  ...
  // Mesh data
  tinygltf::Mesh mesh;
  mesh.name = fmt::format("Mesh {}", std::to_underlying(draw_call_id));
...
void SceneDumper::Record(const SceneRecordingRequest& request)
{
  ...
  m_gpu_skinning_dumper.SetOutputPath(fmt::format("{}.nativemesh", base_name));
