...
private:
  ...
  // Hacky... (can't include directly because it includes imgui)
  // need to clean up EditorTypes to not include so much
  std::unique_ptr<EditorEvents> m_editor_events;
  ...
  std::unique_ptr<SceneRecordingRequest> m_last_mesh_dump_request;
