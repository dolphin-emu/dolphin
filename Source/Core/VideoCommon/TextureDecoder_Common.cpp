...
int TexDecoder_GetTexelSizeInNibbles(int format) {
  ...
  case GX_CTF_Z16L:
    return 4;
  default:
    PanicAlert("Unsupported Texture Format (%08x)! (GetTexelSizeInNibbles)",
               format);
    return 1;
  }
}
...
int TexDecoder_GetBlockWidthInTexels(u32 format) {
  ...
  case GX_CTF_Z16L:
    return 4;
  default:
    PanicAlert("Unsupported Texture Format (%08x)! (GetBlockWidthInTexels)",
               format);
    return 8;
  }
}
...
int TexDecoder_GetBlockHeightInTexels(u32 format) {
  ...
  case GX_CTF_Z16L:
    return 4;
  default:
    PanicAlert("Unsupported Texture Format (%08x)! (GetBlockHeightInTexels)",
               format);
    return 4;
  }
}
...
// With the exception of c4/c8/c14 paletted texture formats (which are handled
// elsewhere)
// this is the format the game should be using when it is drawing an EFB copy
// back.
int TexDecoder_GetEfbCopyBaseFormat(int format) {
  ...
  case GX_TF_CMPR:
  default:
    PanicAlert("Unsupported Texture Format (%08x)! (GetEfbCopyBaseFormat)",
               format);
    return format & 0xf;
  }
}
...
static const char *texfmt[] = {
    // pixel
    "I4", "I8", "IA4", "IA8", "RGB565", "RGB5A3", "RGBA8", "0x07", "C4", "C8",
    "C14X2", "0x0B", "0x0C", "0x0D", "CMPR", "0x0F",
    // Z-buffer
    "0x10", "Z8", "0x12", "Z16", "0x14", "0x15", "Z24X8", "0x17", "0x18",
    "0x19", "0x1A", "0x1B", "0x1C", "0x1D", "0x1E", "0x1F",
    // pixel + copy
    "CR4", "0x21", "CRA4", "CRA8", "0x24", "0x25", "CYUVA8", "CA8", "CR8",
    "CG8", "CB8", "CRG8", "CGB8", "0x2D", "0x2E", "0x2F",
    // Z + copy
    "CZ4", "0x31", "0x32", "0x33", "0x34", "0x35", "0x36", "0x37", "0x38",
    "CZ8M", "CZ8L", "0x3B", "CZ16L", "0x3D", "0x3E", "0x3F",
};

static void TexDecoder_DrawOverlay(u8 *dst, int width, int height,
                                   int texformat) {
...
void TexDecoder_Decode(u8 *dst, const u8 *src, int width, int height,
                       int texformat, const u8 *tlut, TlutFormat tlutfmt) {
  _TexDecoder_DecodeImpl((u32 *)dst, src, width, height, texformat, tlut,
                         tlutfmt);
...
void TexDecoder_DecodeTexel(u8 *dst, const u8 *src, int s, int t,
                            int imageWidth, int texformat, const u8 *tlut_,
                            TlutFormat tlutfmt) {
  ...
  */
  ...
  case GX_TF_CMPR: {
    ...
    case 3:
      ...
      break;
    case 6:
      color = MakeRGBA((int)ceil((float)(red1 + red2) / 2),
...
void TexDecoder_DecodeTexelRGBA8FromTmem(u8 *dst, const u8 *src_ar,
                                         const u8 *src_gb, int s, int t,
                                         int imageWidth) {
  u16 sBlk = s >> 2;
  u16 tBlk = t >> 2;
  u16 widthBlks =
      (imageWidth >> 2) +
      1; // TODO: Looks wrong. Shouldn't this be ((imageWidth-1)>>2)+1 ?
...
void TexDecoder_DecodeRGBA8FromTmem(u8 *dst, const u8 *src_ar, const u8 *src_gb,
