...
template <class T>
static void EmitVertex(T &out, geometry_shader_uid_data *uid_data,
                       const char *vertex, API_TYPE ApiType,
                       bool first_vertex = false);
template <class T>
static void EndPrimitive(T &out, geometry_shader_uid_data *uid_data,
                         API_TYPE ApiType);
...
static T GenerateGeometryShader(u32 primitive_type, API_TYPE ApiType) {
  ...
  geometry_shader_uid_data *uid_data =
      out.template GetUidData<geometry_shader_uid_data>();
  if (uid_data != nullptr)
  ...
  if (primitive_type == PRIMITIVE_LINES) {
    if (ApiType == API_OPENGL) {
      ...
      AssignVSOutputMembers(out, "start", "vs[0]", uid_data->numTexGens,
                            uid_data->pixel_lighting);
      AssignVSOutputMembers(out, "end", "vs[1]", uid_data->numTexGens,
                            uid_data->pixel_lighting);
    } else {
    ...
    out.Write(
        "\tfloat2 offset;\n"
        "\tfloat2 to = abs(end.pos.xy / end.pos.w - start.pos.xy / "
        "start.pos.w);\n"
        // FIXME: What does real hardware do when line is at a 45-degree angle?
        // FIXME: Lines aren't drawn at the correct width. See Twilight Princess
        // map.
    ...
    out.Write("\tfloat2 offset = float2(" I_LINEPTPARAMS ".w / " I_LINEPTPARAMS
              ".x, -" I_LINEPTPARAMS ".w / " I_LINEPTPARAMS
              ".y) * center.pos.w;\n");
  }

  if (g_ActiveConfig.iStereoMode > 0) {
    // If the GPU supports invocation we don't need a for loop and can simply
    // use the
  ...
  if (ApiType == API_OPENGL) {
    ...
    AssignVSOutputMembers(out, "f", "vs[i]", uid_data->numTexGens,
                          uid_data->pixel_lighting);
  } else {
  ...
  if (g_ActiveConfig.iStereoMode > 0) {
    ...
    // For stereoscopy add a small horizontal offset in Normalized Device
    // Coordinates proportional
    // to the depth of the vertex. We retrieve the depth value from the
    // w-component of the projected
    // vertex which contains the negated z-component of the original vertex.
    // For negative parallax (out-of-screen effects) we subtract a convergence
    // value from
    // the depth value. This results in objects at a distance smaller than the
    // convergence
    // distance to seemingly appear in front of the screen.
    // This formula is based on page 13 of the "Nvidia 3D Vision Automatic, Best
    // Practices Guide"
    out.Write("\tf.pos.x += " I_STEREOPARAMS
  ...
  if (g_ActiveConfig.iStereoMode > 0 &&
      !g_ActiveConfig.backend_info.bSupportsGSInstancing)
    out.Write("\t}\n");
