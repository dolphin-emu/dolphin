...
static T GenerateGeometryShader(u32 primitive_type, API_TYPE ApiType)
{
	...
	if (ApiType == API_OPENGL)
	{
		...
		GenerateVSOutputMembers<T>(out, ApiType, DriverDetails::HasBug(DriverDetails::BUG_BROKENINTERFACESTORAGEQUALIFIERS) ? "" : "out", GetInterpolationQualifier());

		if (g_ActiveConfig.iStereoMode > 0)
			out.Write("\tflat %s int layer;\n", DriverDetails::HasBug(DriverDetails::BUG_BROKENINTERFACESTORAGEQUALIFIERS) ? "" : "out");
