...
void PerformanceMetrics::DrawImGuiStats(const float backbuffer_scale)
{
  ...
  if (g_ActiveConfig.bShowGraphs)
  {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(window_x, window_y), set_next_position_condition, ImVec2(1.0f, 0.0f));
    ...
    if (ImGui::Begin("PerformanceGraphs", nullptr, graph_flags))
    {
      ...
      static constexpr std::array<double, num_ticks> tick_marks = {0.0, 1000.0 / 360.0,
          1000.0 / 240.0, 1000.0 / 180.0, 1000.0 / 120.0, 1000.0 / 90.00, 1000.0 / 59.94,
          1000.0 / 40.00, 1000.0 / 29.97, 1000.0 / 24.00, 1000.0 / 20.00, 1000.0 / 15.00,
          1000.0 / 10.00, 1000.0 / 5.000, 1000.0 / 2.000, 1000.0, 2000.0};
                  ...
                  ImPlotFlags_NoInputs))
      {
        ...
        ImPlot::SetupAxes(nullptr, nullptr,
            ImPlotAxisFlags_Lock | ImPlotAxisFlags_Invert | ImPlotAxisFlags_NoDecorations |
  ...
  if (g_ActiveConfig.bShowFPS || g_ActiveConfig.bShowFTimes)
  {
    ...
    if (ImGui::Begin("FPSStats", nullptr, imgui_flags))
    {
      ...
      {
        ImGui::TextColored(
            ImVec4(r, g, b, 1.0f), "dt:%6.2lfms", DT_ms(m_fps_counter.GetDtAvg()).count());
        ImGui::TextColored(
            ImVec4(r, g, b, 1.0f), " ±:%6.2lfms", DT_ms(m_fps_counter.GetDtStd()).count());
      }
    }
    ImGui::End();
  }
  ...
  if (g_ActiveConfig.bShowVPS || g_ActiveConfig.bShowVTimes)
  {
    ...
    if (ImGui::Begin("VPSStats", nullptr, imgui_flags))
    {
      ...
      {
        ImGui::TextColored(
            ImVec4(r, g, b, 1.0f), "dt:%6.2lfms", DT_ms(m_vps_counter.GetDtAvg()).count());
        ImGui::TextColored(
            ImVec4(r, g, b, 1.0f), " ±:%6.2lfms", DT_ms(m_vps_counter.GetDtStd()).count());
      }
    }
    ImGui::End();
  }
