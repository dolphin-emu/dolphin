...
template <class T> static T GenerateVertexShader(API_TYPE api_type) {
  T out;
  // Non-uid template parameters will write to the dummy data (=> gets optimized
  // out)
  vertex_shader_uid_data dummy_data;
  vertex_shader_uid_data *uid_data =
      out.template GetUidData<vertex_shader_uid_data>();
  if (uid_data == nullptr)
    uid_data = &dummy_data;
  ...
  if (api_type == API_OPENGL) {
    ...
    if (g_ActiveConfig.backend_info.bSupportsGeometryShaders) {
      ...
      GenerateVSOutputMembers<T>(
          ...
          GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa, false,
                                    true));
      out.Write("} vs;\n");
    } else {
  ...
  if (uid_data->components & VB_HAS_POSMTXIDX) {
    out.Write("float4 pos = float4(dot(" I_TRANSFORMMATRICES
              ...
              "[posmtx+1], rawpos), dot(" I_TRANSFORMMATRICES
              "[posmtx+2], rawpos), 1);\n");

    if (uid_data->components & VB_HAS_NRMALL) {
      out.Write("int normidx = posmtx & 31;\n");
      out.Write("float3 N0 = " I_NORMALMATRICES
                "[normidx].xyz, N1 = " I_NORMALMATRICES
                "[normidx+1].xyz, N2 = " I_NORMALMATRICES "[normidx+2].xyz;\n");
    }

    if (uid_data->components & VB_HAS_NRM0)
      out.Write("float3 _norm0 = normalize(float3(dot(N0, rawnorm0), dot(N1, "
                "rawnorm0), dot(N2, rawnorm0)));\n");
    if (uid_data->components & VB_HAS_NRM1)
      out.Write("float3 _norm1 = float3(dot(N0, rawnorm1), dot(N1, rawnorm1), "
                "dot(N2, rawnorm1));\n");
    if (uid_data->components & VB_HAS_NRM2)
      out.Write("float3 _norm2 = float3(dot(N0, rawnorm2), dot(N1, rawnorm2), "
                "dot(N2, rawnorm2));\n");
  } else {
    out.Write("float4 pos = float4(dot(" I_POSNORMALMATRIX
              "[0], rawpos), dot(" I_POSNORMALMATRIX
  ...
  out.Write("o.pos = float4(dot(" I_PROJECTION "[0], pos), dot(" I_PROJECTION
            "[1], pos), dot(" I_PROJECTION "[2], pos), dot(" I_PROJECTION
            "[3], pos));\n");
  ...
  GenerateLightingShader<T>(out, uid_data->lighting, uid_data->components,
                            "color", "o.colors_");
  ...
  for (unsigned int i = 0; i < uid_data->numTexGens; ++i) {
    ...
    default:
      ...
      if (uid_data->components &
          (VB_HAS_UV0 << (texinfo.sourcerow - XF_SRCTEX0_INROW)))
    ...
    case XF_TEXGEN_EMBOSS_MAP: // calculate tex coords into bump map
      ...
      if (uid_data->components & (VB_HAS_NRM1 | VB_HAS_NRM2)) {
        ...
        out.Write("o.tex%d.xyz = o.tex%d.xyz + float3(dot(ldir, _norm1), "
                  "dot(ldir, _norm2), 0.0);\n",
                  i, texinfo.embosssourceshift);
      } else {
        // The following assert was triggered in House of the Dead Overkill and
        // Star Wars Rogue
        // Squadron 2
      ...
      if (uid_data->components & (VB_HAS_TEXMTXIDX0 << i)) {
        ...
        if (((uid_data->texMtxInfo_n_projection >> i) & 1) == XF_TEXPROJ_STQ)
          out.Write("o.tex%d.xyz = float3(dot(coord, " I_TEXMATRICES
                    "[%d]), dot(coord, " I_TEXMATRICES
                    "[%d]), dot(coord, " I_TEXMATRICES "[%d]));\n",
                    i, 3 * i, 3 * i + 1, 3 * i + 2);
        else
          out.Write("o.tex%d.xyz = float3(dot(coord, " I_TEXMATRICES
    ...
    if (uid_data->dualTexTrans_enabled &&
        texinfo.texgentype == XF_TEXGEN_REGULAR) {
      ...
      out.Write("o.tex%d.xyz = float3(dot(P0.xyz, o.tex%d.xyz) + P0.w, "
                "dot(P1.xyz, o.tex%d.xyz) + "
  ...
  // write the true depth value, if the game uses depth textures pixel shaders
  // will override with
  // the correct values
  ...
  out.Write("o.pos.xy = o.pos.xy - o.pos.w * " I_PIXELCENTERCORRECTION
            ".xy;\n");

  if (api_type == API_OPENGL) {
    if (g_ActiveConfig.backend_info.bSupportsGeometryShaders) {
      AssignVSOutputMembers(out, "vs", "o", uid_data->numTexGens,
                            uid_data->pixel_lighting);
    } else {
      // TODO: Pass interface blocks between shader stages even if geometry
      // shaders
      // are not supported, however that will require at least OpenGL 3.2
      // support.
      for (unsigned int i = 0; i < uid_data->numTexGens; ++i)
