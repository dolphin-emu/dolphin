...
static void TexMtx_ReadDirect_UByte(VertexLoader *loader) {
  ...
  PRIM_LOG("texmtx%d: %d, ", loader->m_texmtxread,
           loader->m_curtexmtx[loader->m_texmtxread]);
  loader->m_texmtxread++;
}
...
void VertexLoader::CompileVertexTranslator() {
  ...
  // TextureCoord
  const u64 tc[8] = {m_VtxDesc.Tex0Coord, m_VtxDesc.Tex1Coord,
  ...
  WriteCall(VertexLoader_Position::GetFunction(
      m_VtxDesc.Position, m_VtxAttr.PosFormat, m_VtxAttr.PosElements));

  m_VertexSize += VertexLoader_Position::GetSize(
      m_VtxDesc.Position, m_VtxAttr.PosFormat, m_VtxAttr.PosElements);
  ...
  if (m_VtxDesc.Normal != NOT_PRESENT) {
    m_VertexSize += VertexLoader_Normal::GetSize(
  ...
  // Texture matrix indices (remove if corresponding texture coordinate isn't
  // enabled)
  for (int i = 0; i < 8; i++) {
    ...
    if (components & (VB_HAS_TEXMTXIDX0 << i)) {
      ...
      if (tc[i] != NOT_PRESENT) {
        // if texmtx is included, texcoord will always be 3 floats, z will be
        // the texmtx index
        m_native_vtx_decl.texcoords[i].components = 3;
        nat_offset += 12;
        WriteCall(m_VtxAttr.texCoord[i].Elements ? TexMtx_Write_Float
                                                 : TexMtx_Write_Float2);
      } else {
        m_native_vtx_decl.texcoords[i].components = 3;
        nat_offset += 12;
      ...
      if (tc[i] != NOT_PRESENT) {
        ...
        m_native_vtx_decl.texcoords[i].components =
            vtx_attr.texCoord[i].Elements ? 2 : 1;
    ...
    if (tc[i] == NOT_PRESENT) {
      ...
      for (; j < 8; ++j) {
        if (tc[j] != NOT_PRESENT) {
          WriteCall(VertexLoader_TextCoord::GetDummyFunction()); // important to
                                                                 // get indices
                                                                 // right!
          break;
        }
      ...
      // tricky!
      if (j == 8 &&
          !((components & VB_HAS_TEXMTXIDXALL) &
            (VB_HAS_TEXMTXIDXALL << (i + 1)))) {
