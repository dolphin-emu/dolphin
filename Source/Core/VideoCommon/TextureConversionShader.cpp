...
uvec4 GetPaletteColor(uint index)
{
  ...
  uint val = texelFetch(s_palette_buffer, int(u_palette_offset + index)).x;
  val = ((val >> 8) | (val << 8)) & 0xFFFFu;

  uvec4 color;
#if defined(PALETTE_FORMAT_IA8)
  uint a = bitfieldExtract(val, 0, 8);
  uint i = bitfieldExtract(val, 8, 8);
  color = uvec4(i, i, i, a);
...
static const std::array<DecodingShaderInfo, 10> s_decoding_shader_info = {
    {{GX_TF_I4, BUFFER_FORMAT_R8_UINT, 0, 8, 8, false,
      R"(
     ...
     {GX_TF_IA4, BUFFER_FORMAT_R8_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_I8, BUFFER_FORMAT_R8_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_IA8, BUFFER_FORMAT_R16_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_RGB565, BUFFER_FORMAT_R16_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_RGB5A3, BUFFER_FORMAT_R16_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_RGBA8, BUFFER_FORMAT_R16_UINT, 0, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_CMPR, BUFFER_FORMAT_R32G32_UINT, 0, 64, 1, true,
      R"(
      ...
      void main()
      {
        ...
        if (c1 > c2)
        {
          ...
          color2 = uvec4(red1 + red3, green1 + green3, blue1 + blue3, 255u);
          color3 = uvec4(red2 - red3, green2 - green3, blue2 - blue3, 255u);
        }
        else
        {
          color2 = uvec4((red1 + red2 + 1u) / 2u,  // Average
                              (green1 + green2 + 1u) / 2u, (blue1 + blue2 + 1u) / 2u, 255u);
          color3 = uvec4(red2, green2, blue2, 0u);  // Color2 but transparent
        }
     ...
     {GX_TF_C4, BUFFER_FORMAT_R8_UINT, 32, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
     ...
     {GX_TF_C8, BUFFER_FORMAT_R8_UINT, 512, 8, 8, false,
      R"(
      layout(local_size_x = 8, local_size_y = 8) in;
...
{
  auto iter = std::find_if(s_decoding_shader_info.begin(), s_decoding_shader_info.end(),
                           [format](const auto& it) { return it.input_format == format; });

  if (iter == s_decoding_shader_info.end())
    return nullptr;
