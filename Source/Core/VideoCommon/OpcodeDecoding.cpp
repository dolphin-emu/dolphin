...
public:
  DOLPHIN_FORCE_INLINE void OnXF(u16 address, u8 count, const u8* data) override
  {
    ...
    {
      // HACK
      LoadXFReg(count, address, DataReader{const_cast<u8*>(data), const_cast<u8*>(data) + count * sizeof(u32)});
  ...
  u32 m_cycles;
  bool m_in_display_list;
};

// This code is very hot (as everything in the FIFO passes through it and it needs to work
// realtime). Thus, some hackery is used above to inline a lot of code into it.
template <bool is_preprocess>
...
template u8* RunFifo<true>(DataReader src, u32* cycles, bool in_display_list);
template u8* RunFifo<false>(DataReader src, u32* cycles, bool in_display_list);
