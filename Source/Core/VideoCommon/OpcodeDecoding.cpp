...
//  Super Mario Galaxy has nearly all geometry and more than half of the state
//  in DLs (great!)

// Note that it IS NOT GENERALLY POSSIBLE to precompile display lists! You can
// compile them as they
// are
// while interpreting them, and hope that the vertex format doesn't change,
// though, if you do it
// right
// when they are called. The reason is that the vertex format affects the sizes
// of the vertices.
...
static void UnknownOpcode(u8 cmd_byte, void *buffer, bool preprocess) {
...

  {
    ...
    PanicAlert("Illegal command %02x\n"
               ...
               cmd_byte, fifo.CPBase, fifo.CPEnd, fifo.CPHiWatermark,
               fifo.CPLoWatermark, fifo.CPReadWriteDistance,
...
void Init() { s_bFifoErrorSeen = false; }
...
u8 *Run(DataReader src, u32 *cycles, bool in_display_list) {
  ...
  while (true) {
    ...
    case GX_NOP:
      totalCycles +=
          6; // Hm, this means that we scan over nop streams pretty slowly...
      break;
    ...
    case GX_CMD_UNKNOWN_METRICS: // zelda 4 swords calls it and checks the
                                 // metrics registers after
                                 // that
      totalCycles += 6;
    ...
    case GX_LOAD_BP_REG:
      // In skipped_frame case: We have to let BP writes through because they
      // set
    ...
    default:
      if ((cmd_byte & 0xC0) == 0x80) {
        ...
        int bytes = VertexLoaderManager::RunVertices(
            ...
            (cmd_byte & GX_PRIMITIVE_MASK) >> GX_PRIMITIVE_SHIFT, num_vertices,
            src, Fifo::WillSkipCurrentFrame(), is_preprocess);
        ...
        ERROR_LOG(VIDEO,
                  "FIFO: Unknown Opcode(0x%02x @ %p, preprocessing = %s)",
                  cmd_byte, opcodeStart, is_preprocess ? "yes" : "no");
    ...
    if (!is_preprocess && g_bRecordFifoData && cmd_byte != GX_CMD_CALL_DL) {
      ...
      FifoRecorder::GetInstance().WriteGPCommand(opcodeStart,
                                                 u32(opcodeEnd - opcodeStart));
    }
  }
