    ...
    VideoCommon::CameraManager& camera_manager)
{
  ...
  const auto render_targets = material_resource ?
                                  material_resource->render_targets :
                                  std::span<GraphicsModSystem::OutputRenderTargetResource>{};
  const auto camera_view = camera_manager.GetCurrentCameraView(render_targets);
  const auto additional_camera_views = camera_manager.GetAdditionalViews(render_targets);
  ...
  // frame buffers...
  const auto camera_views = camera_manager.GetAdditionalViews(render_targets);
  for (const auto additional_camera_view : camera_views)
  {
    if (xfmem.projection.type == ProjectionType::Orthographic &&
        additional_camera_view.skip_orthographic)
                                         ...
                                         VideoCommon::CameraManager& camera_manager)
{
  ...
  for (std::size_t i = 0; i < material_resource->textures.size(); i++)
  {
    ...
    if (sampler == nullptr || texture == nullptr) [[unlikely]]
      continue;
