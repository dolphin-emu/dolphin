...
DataReader VertexManagerBase::PrepareForAdditionalData(int primitive, u32 count,
                                                       u32 stride,
                                                       bool cullall) {
  ...
  if (!s_is_flushed && (count > IndexGenerator::GetRemainingIndices() ||
    ...
    if (count > IndexGenerator::GetRemainingIndices())
      ERROR_LOG(VIDEO, "Too little remaining index values. Use 32-bit or reset "
                       "them on flush.");
...
void VertexManagerBase::Flush() {
  ...
  PRIM_LOG("frame%d:\n texgen=%d, numchan=%d, dualtex=%d, ztex=%d, cole=%d, "
           "alpe=%d, ze=%d",
           g_ActiveConfig.iSaveTargetId, xfmem.numTexGen.numTexGens,
           xfmem.numChan.numColorChans, xfmem.dualTexTrans.enabled,
           bpmem.ztex2.op, (int)bpmem.blendmode.colorupdate,
  ...
  for (unsigned int i = 0; i < xfmem.numChan.numColorChans; ++i) {
    ...
    PRIM_LOG(
        "colchan%d: matsrc=%d, light=0x%x, ambsrc=%d, diffunc=%d, attfunc=%d",
        i, ch->matsource, ch->GetFullLightMask(), ch->ambsource,
        ch->diffusefunc, ch->attnfunc);
    ch = &xfmem.alpha[i];
    PRIM_LOG(
        "alpchan%d: matsrc=%d, light=0x%x, ambsrc=%d, diffunc=%d, attfunc=%d",
        i, ch->matsource, ch->GetFullLightMask(), ch->ambsource,
        ch->diffusefunc, ch->attnfunc);
  }
  ...
  for (unsigned int i = 0; i < xfmem.numTexGen.numTexGens; ++i) {
    ...
    PRIM_LOG("txgen%d: proj=%d, input=%d, gentype=%d, srcrow=%d, embsrc=%d, "
             "emblght=%d, "
             "postmtx=%d, postnorm=%d",
             i, tinfo.projection, tinfo.inputform, tinfo.texgentype,
             tinfo.sourcerow, tinfo.embosssourceshift, tinfo.embosslightshift,
  ...
  // If the primitave is marked CullAll. All we need to do is update the vertex
  // constants and
    ...
    if (bpmem.genMode.numindstages > 0)
      for (unsigned int i = 0; i < bpmem.genMode.numtevstages + 1u; ++i)
        if (bpmem.tevind[i].IsActive() &&
            bpmem.tevind[i].bt < bpmem.genMode.numindstages)
          usedtextures[bpmem.tevindref.getTexMap(bpmem.tevind[i].bt)] = true;

    TextureCacheBase::UnbindTextures();
    for (unsigned int i : usedtextures) {
      const TextureCacheBase::TCacheEntryBase *tentry =
          TextureCacheBase::Load(i);

      if (tentry) {
        g_renderer->SetSamplerState(i & 3, i >> 2, tentry->is_custom_tex);
        PixelShaderManager::SetTexDims(i, tentry->native_width,
  ...
  if (!s_cull_all) {
    ...
    if (PerfQueryBase::ShouldEmulate())
      g_perf_query->EnableQuery(bpmem.zcontrol.early_ztest ? PQG_ZCOMP_ZCOMPLOC
                                                           : PQG_ZCOMP);
    g_vertex_manager->vFlush(useDstAlpha);
    if (PerfQueryBase::ShouldEmulate())
      g_perf_query->DisableQuery(bpmem.zcontrol.early_ztest ? PQG_ZCOMP_ZCOMPLOC
                                                            : PQG_ZCOMP);
  }
  ...
  if (xfmem.numTexGen.numTexGens != bpmem.genMode.numtexgens)
    ERROR_LOG(VIDEO, "xf.numtexgens (%d) does not match bp.numtexgens (%d). "
...
void VertexManagerBase::CalculateZSlope(NativeVertexFormat *format) {
  ...
  for (unsigned int i = 0; i < 3; ++i) {
    ...
    VertexShaderManager::TransformToClipSpace(
        &VertexLoaderManager::position_cache[2 - i][0], &out[i * 4], mtxIdx);
    ...
    out[2 + i * 4] =
        out[2 + i * 4] * inv_w * xfmem.viewport.zRange + xfmem.viewport.farZ;
  }
