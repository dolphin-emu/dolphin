                                                       ...
                                                       bool cullall)
{
  ...
  if (m_current_primitive_type != new_primitive_type)
  {
    ...
    raster_state.Generate(bpmem, new_primitive_type, !g_ActiveConfig.bDisableNearClipping);
...
{
  const PortableVertexDeclaration vert_decl =
      VertexLoaderManager::GetCurrentVertexFormat()->GetVertexDeclaration();
  return (int)((m_cur_buffer_pointer - m_base_buffer_pointer) / vert_decl.stride);
}
