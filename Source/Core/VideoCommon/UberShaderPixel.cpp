...
void WriteCustomShaderStructImpl(ShaderCode* out, u32 num_texgen, bool per_pixel_lighting)
{
  ...
  for (const auto& color_type : std::array<std::string_view, 2>{"colorreg", "alphareg"})
  {
    ...
    //  to set values unless the variable is an index in a for loop.
    // So instead we have to do this if check nonsense
    for (u32 light_count_index = 0; light_count_index < 32; light_count_index++)
    {
      out->Write("\t\t\t\t\tif (light_count == {})\n", light_count_index);
      out->Write("\t\t\t\t\t{{\n");
      out->Write("\t\t\t\t\t\tcustom_data.lights[{}].direction = " I_LIGHTS
                 "[light_index].dir.xyz;\n",
                 light_count_index);
      out->Write("\t\t\t\t\t\tcustom_data.lights[{}].position = " I_LIGHTS
                 "[light_index].pos.xyz;\n",
                 light_count_index);
      out->Write("\t\t\t\t\t\tcustom_data.lights[{}].cosatt = " I_LIGHTS "[light_index].cosatt;\n",
                 light_count_index);
      out->Write("\t\t\t\t\t\tcustom_data.lights[{}].distatt = " I_LIGHTS
                 "[light_index].distatt;\n",
                 light_count_index);
      out->Write("\t\t\t\t\t\tcustom_data.lights[{}].attenuation_type = attnfunc;\n",
                 light_count_index);

      if (color_type == "colorreg")
      {
        out->Write("\t\t\t\t\t\tcustom_data.lights[{}].color = " I_LIGHTS
      ...
      {
        out->Write("\t\t\t\t\t\tcustom_data.lights[{}].color = float3(" I_LIGHTS
                   "[light_index].color.a) / float3(255.0, 255.0, 255.0);\n",
                   light_count_index);
        out->Write("\t\t\t\t\t\tcustom_data.lights[{}].light_type = "
                   "CUSTOM_SHADER_LIGHTING_LIGHT_TYPE_ALPHA;\n",
                   light_count_index);
      }
      out->Write("\t\t\t\t\t}}\n");
    }
    out->Write("\t\t\t\t\tlight_count += 1;\n");
    out->Write("\t\t\t\t}}\n");
    out->Write("\t\t\t}}\n");
    out->Write("\t\t}}\n");
  }
  ...
  out->Write("\tcustom_data.light_count = light_count;\n");
