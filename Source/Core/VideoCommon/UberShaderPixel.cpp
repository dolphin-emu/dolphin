                          ...
                          const pixel_ubershader_uid_data* uid_data)
{
  ...
  WriteSwitch(out, ApiType, "compare", tev_alpha_funcs_table, 2, false);
  out.Write("}}\n"
            "\n"
            "int3 selectColorInput(State s, StageState ss, float4 colors_0, float4 colors_1, "
            "uint index) {{\n");
  WriteSwitch(out, ApiType, "index", tev_c_input_table, 2, false);
  out.Write("}}\n"
            "\n"
            "int selectAlphaInput(State s, StageState ss, float4 colors_0, float4 colors_1, "
            "uint index) {{\n");
  WriteSwitch(out, ApiType, "index", tev_a_input_table, 2, false);
  out.Write("}}\n"
            "\n"
            "int4 getTevReg(in State s, uint index) {{\n");
  WriteSwitch(out, ApiType, "index", tev_regs_lookup_table, 2, false);
  out.Write("}}\n"
            "\n"
            "void setRegColor(inout State s, uint index, int3 color) {{\n");
  WriteSwitch(out, ApiType, "index", tev_c_set_table, 2, true);
  out.Write("}}\n"
            "\n"
            "void setRegAlpha(inout State s, uint index, int alpha) {{\n");
  WriteSwitch(out, ApiType, "index", tev_a_set_table, 2, true);
  out.Write("}}\n"
            "\n");
  ...
  if (use_shader_blend)
  {
    ...
    out.Write("  if (blend_enable) {{\n"
              "    float4 blend_src;\n");
    WriteSwitch(out, ApiType, "blend_src_factor", blendSrcFactor, 4, true);
    WriteSwitch(out, ApiType, "blend_src_factor_alpha", blendSrcFactorAlpha, 4, true);

    out.Write("    float4 blend_dst;\n");
    WriteSwitch(out, ApiType, "blend_dst_factor", blendDstFactor, 4, true);
    WriteSwitch(out, ApiType, "blend_dst_factor_alpha", blendDstFactorAlpha, 4, true);
