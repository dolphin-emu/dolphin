...
public:
  struct TCacheEntryConfig {
    ...
    struct Hasher : std::hash<u64> {
      size_t operator()(const TCacheEntryConfig &c) const {
        u64 id = (u64)c.rendertarget << 63 | (u64)c.layers << 48 |
  ...
  struct TCacheEntryBase {
    ...
    // used to delete textures which haven't been used for
    // TEXTURE_KILL_THRESHOLD frames
    int frameCount;

    // Keep an iterator to the entry in textures_by_hash, so it does not need to
    // be searched when
  ...
  // Removes textures which aren't used for more than TEXTURE_KILL_THRESHOLD
  // frames,
  ...
  virtual void CopyEFB(u8 *dst, u32 format, u32 native_width, u32 bytes_per_row,
  ...
  static void CopyRenderTargetToTexture(u32 dstAddr, unsigned int dstFormat,
                                        u32 dstStride,
                                        PEControl::PixelFormat srcFormat,
                                        const EFBRectangle &srcRect,
                                        bool isIntensity, bool scaleByHalf);

  virtual void ConvertTexture(TCacheEntryBase *entry,
...
private:
  ...
  typedef std::unordered_multimap<TCacheEntryConfig, TCacheEntryBase *,
                                  TCacheEntryConfig::Hasher>
      TexPool;
  static void ScaleTextureCacheEntryTo(TCacheEntryBase **entry, u32 new_width,
                                       u32 new_height);
  static TCacheEntryBase *DoPartialTextureUpdates(TexCache::iterator iter,
                                                  u8 *palette, u32 tlutfmt);
  static void DumpTexture(TCacheEntryBase *entry, std::string basename,
                          unsigned int level);
  ...
  static TCacheEntryBase *ReturnEntry(unsigned int stage,
