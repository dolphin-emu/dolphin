...
public:
  ...
  const AbstractPipeline* GetEFBCopyToVRAMPipeline(
  ...
  const AbstractPipeline* GetTextureReinterpretPipeline(
...
private:
  ...
  std::unique_ptr<AbstractShader> CompileVertexUberShader(
  ...
  const AbstractPipeline* InsertGXPipeline(
      const GXPipelineUid& config, std::unique_ptr<AbstractPipeline> pipeline);
  const AbstractPipeline* InsertGXUberPipeline(
