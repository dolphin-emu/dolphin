...
static inline void DecodeBytes_C4(u32 *dst, const u8 *src, const u8 *tlut_,
                                  TlutFormat tlutfmt) {
...
static inline void DecodeBytes_C8(u32 *dst, const u8 *src, const u8 *tlut_,
                                  TlutFormat tlutfmt) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 8; x++) {
    u8 val = src[x];
...
static inline void DecodeBytes_C14X2(u32 *dst, const u16 *src, const u8 *tlut_,
                                     TlutFormat tlutfmt) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
...
static inline void DecodeBytes_RGBA8(u32 *dst, const u16 *src,
                                     const u16 *src2) {
#if 0
	for (int x = 0; x < 4; x++)
	{
    ...
    for (int y = 0; y < height; y += 8)
      for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
        for (int iy = 0, xStep = 8 * yStep; iy < 8; iy++, xStep++)
          DecodeBytes_C4(dst + (y + iy) * width + x, src + 4 * xStep, tlut,
                         tlutfmt);
    break;
  case GX_TF_I4: {
    ...
    for (int y = 0; y < height; y += 4)
      for (int x = 0; x < width; x += 8)
        for (int iy = 0; iy < 4; ++iy, src += 8) {
          ...
          srcval = (newsrc++)[0];
          (newdst++)[0] =
              srcval | (srcval << 8) | (srcval << 16) | (srcval << 24);
          srcval = (newsrc++)[0];
          (newdst++)[0] =
              srcval | (srcval << 8) | (srcval << 16) | (srcval << 24);
          srcval = (newsrc++)[0];
    ...
    for (int y = 0; y < height; y += 4)
      for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++)
        for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++)
          DecodeBytes_C8((u32 *)dst + (y + iy) * width + x, src + 8 * xStep,
                         tlut, tlutfmt);
    break;
  case GX_TF_IA4: {
    for (int y = 0; y < height; y += 4)
  ...
  case GX_TF_C14X2:
    for (int y = 0; y < height; y += 4)
      for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
        for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++)
          DecodeBytes_C14X2(dst + (y + iy) * width + x,
                            (u16 *)(src + 8 * xStep), tlut, tlutfmt);
    break;
  case GX_TF_RGB565: {
    // Reference C implementation.
    for (int y = 0; y < height; y += 4)
    ...
    // The metroid games use this format almost exclusively.
    {
      for (int y = 0; y < height; y += 8) {
        for (int x = 0; x < width; x += 8) {
          ...
          src += sizeof(DXTBlock);
          DecodeDXTBlock((u32 *)dst + y * width + x + 4, (DXTBlock *)src,
                         width);
          src += sizeof(DXTBlock);
          DecodeDXTBlock((u32 *)dst + (y + 4) * width + x, (DXTBlock *)src,
                         width);
          src += sizeof(DXTBlock);
          DecodeDXTBlock((u32 *)dst + (y + 4) * width + x + 4, (DXTBlock *)src,
                         width);
          src += sizeof(DXTBlock);
        }
      }
