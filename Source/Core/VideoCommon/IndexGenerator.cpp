...
void IndexGenerator::Init() {
  if (g_Config.backend_info.bSupportsPrimitiveRestart) {
    ...
    primitive_table[GX_DRAW_QUADS_2] =
        IndexGenerator::AddQuads_nonstandard<true>;
    ...
    primitive_table[GX_DRAW_QUADS_2] =
        IndexGenerator::AddQuads_nonstandard<false>;
...
void IndexGenerator::AddIndices(int primitive, u32 numVerts) {
  index_buffer_current =
      primitive_table[primitive](index_buffer_current, numVerts, base_index);
  base_index += numVerts;
}

// Triangles
template <bool pr>
__forceinline u16 *IndexGenerator::WriteTriangle(u16 *Iptr, u32 index1,
                                                 u32 index2, u32 index3) {
  *Iptr++ = index1;
  *Iptr++ = index2;
  *Iptr++ = index3;
  if (pr)
  ...
  if (pr) {
    ...
    *Iptr++ = s_primitive_restart;
    ...
    for (u32 i = 2; i < numVerts; ++i) {
      Iptr = WriteTriangle<pr>(Iptr, index + i - 2, index + i - !wind,
                               index + i - wind);
  ...
  for (; i < numVerts; i += 4) {
    if (pr) {
      ...
      *Iptr++ = s_primitive_restart;
      ...
      Iptr =
          WriteTriangle<pr>(Iptr, index + i - 3, index + i - 2, index + i - 1);
      Iptr =
          WriteTriangle<pr>(Iptr, index + i - 3, index + i - 1, index + i - 0);
    }
  }

  // three vertices remaining, so render a triangle
  if (i == numVerts) {
    Iptr = WriteTriangle<pr>(Iptr, index + numVerts - 3, index + numVerts - 2,
                             index + numVerts - 1);
  }
  return Iptr;
}
