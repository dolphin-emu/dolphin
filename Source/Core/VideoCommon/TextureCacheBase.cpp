...
static const int TEXTURE_KILL_THRESHOLD = 64; // Sonic the Fighters (inside
                                              // Sonic Gems Collection) loops a
                                              // 64 frames animation
  ...
  while (iter != tcend) {
    if (iter->second->frameCount == FRAMECOUNT_INVALID) {
      ...
      ++iter;
               ...
               TEXTURE_KILL_THRESHOLD + iter->second->frameCount) {
      if (iter->second->IsEfbCopy()) {
        // Only remove EFB copies when they wouldn't be used anymore(changed
        // hash), because EFB
        // copies living on the
        // host GPU are unrecoverable. Perform this check only every
        // TEXTURE_KILL_THRESHOLD for
        // performance reasons
        if ((_frameCount - iter->second->frameCount) % TEXTURE_KILL_THRESHOLD ==
                1 &&
...
bool TextureCacheBase::TCacheEntryBase::OverlapsMemoryRange(
    u32 range_address, u32 range_size) const {
...
TextureCacheBase::TCacheEntryBase::ApplyPalette(u8 *palette, u32 tlutfmt) {
  ...
  if (decoded_entry) {
    ...
    g_texture_cache->ConvertTexture(decoded_entry, this, palette,
...
void TextureCacheBase::ScaleTextureCacheEntryTo(
    TextureCacheBase::TCacheEntryBase **entry, u32 new_width, u32 new_height) {
  if ((*entry)->config.width == new_width &&
  ...
  if (max < new_width || max < new_height) {
    ERROR_LOG(VIDEO, "Texture too big, width = %d, height = %d", new_width,
              new_height);
    return;
  }
  ...
  if (newentry) {
    newentry->SetGeneralParameters((*entry)->addr, (*entry)->size_in_bytes,
                                   (*entry)->format);
    ...
    if ((*entry)->textures_by_hash_iter != textures_by_hash.end()) {
      newentry->textures_by_hash_iter =
          textures_by_hash.emplace((*entry)->hash, newentry);
    }
...
TextureCacheBase::TCacheEntryBase *
TextureCacheBase::DoPartialTextureUpdates(TexCache::iterator iter_t,
  ...
  // EFB copies are excluded from these updates, until there's an example where
  // a game would
  ...
  u32 block_width =
      TexDecoder_GetBlockWidthInTexels(entry_to_update->format & 0xf);
  u32 block_height =
      TexDecoder_GetBlockHeightInTexels(entry_to_update->format & 0xf);
  u32 block_size =
      block_width * block_height *
      TexDecoder_GetTexelSizeInNibbles(entry_to_update->format & 0xf) / 2;

  u32 numBlocksX =
      (entry_to_update->native_width + block_width - 1) / block_width;

  TexCache::iterator iter = textures_by_address.lower_bound(
      entry_to_update->addr > MAX_TEXTURE_BINARY_SIZE
          ? entry_to_update->addr - MAX_TEXTURE_BINARY_SIZE
          : 0);
  TexCache::iterator iterend = textures_by_address.upper_bound(
      entry_to_update->addr + entry_to_update->size_in_bytes);
  while (iter != iterend) {
    TCacheEntryBase *entry = iter->second;
    if (entry != entry_to_update && entry->IsEfbCopy() &&
        entry->references.count(entry_to_update) == 0 &&
        entry->OverlapsMemoryRange(entry_to_update->addr,
                                   entry_to_update->size_in_bytes) &&
        entry->memory_stride == numBlocksX * block_size) {
      if (entry->hash == entry->CalculateHash()) {
        if (isPaletteTexture) {
          TCacheEntryBase *decoded_entry =
              entry->ApplyPalette(palette, tlutfmt);
          if (decoded_entry) {
            // Link the efb copy with the partially updated texture, so we won't
            // apply this partial
            // update again
        ...
        // Normal textures can't be strided, so the 2 missing cases with src_x >
        // 0 don't exist
        ...
        u32 copy_width = std::min(entry->native_width - src_x,
                                  entry_to_update->native_width - dst_x);
        u32 copy_height = std::min(entry->native_height - src_y,
                                   entry_to_update->native_height - dst_y);

        // If one of the textures is scaled, scale both with the current efb
        // scaling factor
        if (entry_to_update->native_width != entry_to_update->config.width ||
          ...
          ScaleTextureCacheEntryTo(
              &entry, Renderer::EFBToScaledX(entry->native_width),
        ...
        if (isPaletteTexture) {
          // Remove the temporary converted texture, it won't be used anywhere
          // else
          // TODO: It would be nice to convert and copy in one step, but this
          // code path isn't common
          InvalidateTexture(GetTexCacheIter(entry));
        } else {
          // Link the two textures together, so we won't apply this partial
          // update again
          entry->CreateReference(entry_to_update);
          // Mark the texture update as used, as if it was loaded directly
          entry->frameCount = FRAMECOUNT_INVALID;
        ...
        // If the hash does not match, this EFB copy will not be used for
...
void TextureCacheBase::DumpTexture(TCacheEntryBase *entry, std::string basename,
                                   unsigned int level) {
...
TextureCacheBase::TCacheEntryBase *
TextureCacheBase::ReturnEntry(unsigned int stage, TCacheEntryBase *entry) {
  entry->frameCount = FRAMECOUNT_INVALID;
...
TextureCacheBase::TCacheEntryBase *TextureCacheBase::Load(const u32 stage) {
  ...
  const bool use_mipmaps =
      SamplerCommon::AreBpTexMode0MipmapsEnabled(tex.texMode0[id]);
  u32 tex_levels =
      use_mipmaps ? ((tex.texMode1[id].max_lod + 0xf) / 0x10 + 1) : 1;
  ...
  // Hash assigned to texcache entry (also used to generate filenames used for
  // texture dumping and
  ...
  const bool isPaletteTexture =
      (texformat == GX_TF_C4 || texformat == GX_TF_C8 ||
       texformat == GX_TF_C14X2);
  ...
  const u32 texture_size = TexDecoder_GetTextureSizeInBytes(
      expandedWidth, expandedHeight, texformat);
  u32 additional_mips_size = 0; // not including level 0, which is texture_size

  // GPUs don't like when the specified mipmap count would require more than one
  // 1x1-sized LOD in
  // the mipmap chain
  // e.g. 64x64 with 7 LODs would have the mipmap chain
  // 64x64,32x32,16x16,8x8,4x4,2x2,1x1,0x0, so we
  ...
  for (u32 level = 1; level != tex_levels; ++level) {
    ...
    const u32 expanded_mip_width =
        ROUND_UP(CalculateLevelSize(width, level), bsw);
    const u32 expanded_mip_height =
        ROUND_UP(CalculateLevelSize(height, level), bsh);

    additional_mips_size += TexDecoder_GetTextureSizeInBytes(
        expanded_mip_width, expanded_mip_height, texformat);
  }

  const u8 *src_data;
  if (from_tmem)
    src_data = &texMem[bpmem.tex[stage / 4].texImage1[stage % 4].tmem_even *
                       TMEM_LINE_SIZE];
  else
  ...
  // FifiRecorder does it's own memory modification tracking independant of the
  // texture hashing
  // below.
  if (g_bRecordFifoData && !from_tmem)
    FifoRecorder::GetInstance().UseMemory(address,
                                          texture_size + additional_mips_size,
                                          MemoryUpdate::TEXTURE_MAP);

  // TODO: This doesn't hash GB tiles for preloaded RGBA8 textures (instead,
  // it's hashing more data
  // from the low tmem bank than it should)
  base_hash = GetHash64(src_data, texture_size,
                        g_ActiveConfig.iSafeTextureCache_ColorSamples);
  u32 palette_size = 0;
  if (isPaletteTexture) {
    palette_size = TexDecoder_GetPaletteSize(texformat);
    full_hash =
        base_hash ^ GetHash64(&texMem[tlutaddr], palette_size,
                              g_ActiveConfig.iSafeTextureCache_ColorSamples);
  } else {
    full_hash = base_hash;
  }
  ...
  //
  // Find all texture cache entries for the current texture address, and decide
  // whether to use one
  // of
  // them, or to create a new one
  //
  // In most cases, the fastest way is to use only one texture cache entry for
  // the same address.
  // Usually,
  // when a texture changes, the old version of the texture is unlikely to be
  // used again. If there
  // were
  // new cache entries created for normal texture updates, there would be a
  // slowdown due to a huge
  // amount
  // of unused cache entries. Also thanks to texture pooling, overwriting an
  // existing cache entry is
  // faster than creating a new one from scratch.
  //
  // Some games use the same address for different textures though. If the same
  // cache entry was used
  // in
  // this case, it would be constantly overwritten, and effectively there
  // wouldn't be any caching
  // for
  // those textures. Examples for this are Metroid Prime and Castlevania 3.
  // Metroid Prime has
  // multiple
  // sets of fonts on each other stored in a single texture and uses the palette
  // to make different
  // characters visible or invisible. In Castlevania 3 some textures are used
  // for 2 different things
  // or
  // at least in 2 different ways(size 1024x1024 vs 1024x256).
  //
  // To determine whether to use multiple cache entries or a single entry, use
  // the following
  // heuristic:
  // If the same texture address is used several times during the same frame,
  // assume the address is
  // used
  // for different purposes and allow creating an additional cache entry. If
  // there's at least one
  // entry
  // that hasn't been used for the same frame, then overwrite it, in order to
  // keep the cache as
  // small as
  // possible. If the current texture is found in the cache, use that entry.
  //
  // For efb copies, the entry created in CopyRenderTargetToTexture always has
  // to be used, or else
  // it was
  // done in vain.
  ...
  while (iter != iter_range.second) {
    ...
    if (entry->IsEfbCopy() && entry->native_width == nativeW &&
        entry->native_height == nativeH &&
        entry->memory_stride == entry->BytesPerRow()) {
      // EFB copies have slightly different rules as EFB copy formats have
      // different
      ...
      if ((base_hash == entry->hash &&
        ...
        // TODO: We should check format/width/height/levels for EFB copies.
        // Checking
        // format is complicated because EFB copy formats don't exactly match
        // texture formats. I'm not sure what effect checking
        // width/height/levels
        // would have.
        if (!isPaletteTexture ||
            !g_Config.backend_info.bSupportsPaletteConversion)
        ...
        // perform the conversion later.  Currently, we only convert EFB copies
        // to
        // palette textures; we could do other conversions if it proved to be
        // beneficial.
        unconverted_copy = iter;
      } else {
        // Aggressively prune EFB copies: if it isn't useful here, it will
        // probably
        // never be useful again.  It's theoretically possible for a game to do
        // something weird where the copy could become useful in the future, but
        // in
      ...
      if (entry->hash == full_hash && entry->format == full_format &&
          entry->native_levels >= tex_levels &&
    ...
    // Find the texture which hasn't been used for the longest time. Count
    // paletted
    // textures as the same texture here, when the texture itself is the same.
    // This
    ...
    if (entry->frameCount != FRAMECOUNT_INVALID &&
        entry->frameCount < temp_frameCount && !entry->IsEfbCopy() &&
  ...
  //
  // If the texture was fully hashed, the address does not need to match.
  // Identical duplicate
  // textures cause unnecessary slowdowns
  // Example: Tales of Symphonia (GC) uses over 500 small textures in menus, but
  // only around 70
  // different ones
  ...
  // If at least one entry was not used for the same frame, overwrite the oldest
  // one
  ...
  if (g_ActiveConfig.bHiresTextures) {
    hires_tex = HiresTexture::Search(src_data, texture_size, &texMem[tlutaddr],
                                     palette_size, width, height, texformat,
                                     use_mipmaps);
  ...
  const u32 texLevels =
      hires_tex ? (u32)hires_tex->m_levels.size() : tex_levels;
  ...
  if (!hires_tex) {
    if (!(texformat == GX_TF_RGBA8 && from_tmem)) {
      ...
      TexDecoder_Decode(temp, src_data, expandedWidth, expandedHeight,
                        texformat, tlut, (TlutFormat)tlutfmt);
    } else {
      u8 *src_data_gb =
          &texMem[bpmem.tex[stage / 4].texImage2[stage % 4].tmem_odd *
                  TMEM_LINE_SIZE];
      TexDecoder_DecodeRGBA8FromTmem(temp, src_data, src_data_gb, expandedWidth,
                                     expandedHeight);
    }
  }
  ...
  if (g_ActiveConfig.bDumpTextures && !hires_tex) {
    basename = HiresTexture::GenBaseName(src_data, texture_size,
                                         &texMem[tlutaddr], palette_size, width,
                                         height, texformat, use_mipmaps, true);
  ...
  if (hires_tex) {
    ...
    if (from_tmem) {
      ptr_even = &texMem[bpmem.tex[stage / 4].texImage1[stage % 4].tmem_even *
                             TMEM_LINE_SIZE +
                         texture_size];
      ptr_odd = &texMem[bpmem.tex[stage / 4].texImage2[stage % 4].tmem_odd *
                        TMEM_LINE_SIZE];
    }
    ...
    for (u32 level = 1; level != texLevels; ++level) {
      ...
      const u8 *&mip_src_data =
          from_tmem ? ((level % 2) ? ptr_odd : ptr_even) : src_data;
      const u8 *tlut = &texMem[tlutaddr];
      TexDecoder_Decode(temp, mip_src_data, expanded_mip_width,
                        expanded_mip_height, texformat, tlut,
                        (TlutFormat)tlutfmt);
      mip_src_data += TexDecoder_GetTextureSizeInBytes(
          expanded_mip_width, expanded_mip_height, texformat);
...
void TextureCacheBase::CopyRenderTargetToTexture(
    u32 dstAddr, unsigned int dstFormat, u32 dstStride,
    PEControl::PixelFormat srcFormat, const EFBRectangle &srcRect,
    bool isIntensity, bool scaleByHalf) {
  // Emulation methods:
  //
  // - EFB to RAM:
  //      Encodes the requested EFB data at its native resolution to the
  //      emulated RAM using shaders.
  //      Load() decodes the data from there again (using TextureDecoder) if the
  //      EFB copy is being
  //      used as a texture again.
  //      Advantage: CPU can read data from the EFB copy and we don't lose any
  //      important updates to
  //      the texture
  //      Disadvantage: Encoding+decoding steps often are redundant because only
  //      some games read or
  //      modify EFB copies before using them as textures.
  //
  // - EFB to texture:
  //      Copies the requested EFB data to a texture object in VRAM, performing
  //      any color conversion
  //      using shaders.
  //      Advantage: Works for many games, since in most cases EFB copies aren't
  //      read or modified at
  //      all before being used as a texture again.
  //                 Since we don't do any further encoding or decoding here,
  //                 this method is much
  //                 faster.
  //                 It also allows enhancing the visual quality by doing scaled
  //                 EFB copies.
  //
  // - Hybrid EFB copies:
  //      1a) Whenever this function gets called, encode the requested EFB data
  //      to RAM (like EFB to
  //      RAM)
  //      1b) Set type to TCET_EC_DYNAMIC for all texture cache entries in the
  //      destination address
  //      range.
  //          If EFB copy caching is enabled, further checks will (try to)
  //          prevent redundant EFB
  //          copies.
  //      2) Check if a texture cache entry for the specified dstAddr already
  //      exists (i.e. if an EFB
  //      copy was triggered to that address before):
  //      2a) Entry doesn't exist:
  //          - Also copy the requested EFB data to a texture object in VRAM
  //          (like EFB to texture)
  //          - Create a texture cache entry for the target (type =
  //          TCET_EC_VRAM)
  //          - Store a hash of the encoded RAM data in the texcache entry.
  //      2b) Entry exists AND type is TCET_EC_VRAM:
  //          - Like case 2a, but reuse the old texcache entry instead of
  //          creating a new one.
  //      2c) Entry exists AND type is TCET_EC_DYNAMIC:
  //          - Only encode the texture to RAM (like EFB to RAM) and store a
  //          hash of the encoded
  //          data in the existing texcache entry.
  //          - Do NOT copy the requested EFB data to a VRAM object. Reason: the
  //          texture is dynamic,
  //          i.e. the CPU is modifying it. Storing a VRAM copy is useless,
  //          because we'd always end
  //          up deleting it and reloading the data from RAM anyway.
  //      3) If the EFB copy gets used as a texture, compare the source RAM hash
  //      with the hash you
  //      stored when encoding the EFB data to RAM.
  //      3a) If the two hashes match AND type is TCET_EC_VRAM, reuse the VRAM
  //      copy you created
  //      3b) If the two hashes differ AND type is TCET_EC_VRAM, screw your
  //      existing VRAM copy. Set
  //      type to TCET_EC_DYNAMIC.
  //          Redecode the source RAM data to a VRAM object. The entry basically
  //          behaves like a
  //          normal texture now.
  //      3c) If type is TCET_EC_DYNAMIC, treat the EFB copy like a normal
  //      texture.
  //      Advantage: Non-dynamic EFB copies can be visually enhanced like with
  //      EFB to texture.
  //                 Compatibility is as good as EFB to RAM.
  //      Disadvantage: Slower than EFB to texture and often even slower than
  //      EFB to RAM.
  //                    EFB copy cache depends on accurate texture hashing being
  //                    enabled. However,
  //                    with accurate hashing you end up being as slow as
  //                    without a copy cache
  //                    anyway.
  //
  // Disadvantage of all methods: Calling this function requires the GPU to
  // perform a pipeline flush
  // which stalls any further CPU processing.
  //
  // For historical reasons, Dolphin doesn't actually implement "pure" EFB to
  // RAM emulation, but
  ...
  if (srcFormat == PEControl::Z24) {
    ...
    case 12: // Z16L - copy lower 16 depth bits
      // expected to be used as an IA8 texture (upper 8 bits stored as
      // intensity, lower 8 bits
      // stored as alpha)
    ...
    case 8: // I8
      ...
      } else // alpha
      {
        ...
        if (dstFormat == 2) {
          ...
          ColorMask[4] = ColorMask[5] = ColorMask[6] = ColorMask[7] =
              1.0f / 15.0f;
          cbufid = 11;
        } else {
          cbufid = 12;
  ...
  if (dst == nullptr) {
    ERROR_LOG(VIDEO, "Trying to copy from EFB to invalid address 0x%8x",
              dstAddr);
    return;
  }

  const unsigned int tex_w =
      scaleByHalf ? srcRect.GetWidth() / 2 : srcRect.GetWidth();
  const unsigned int tex_h =
      scaleByHalf ? srcRect.GetHeight() / 2 : srcRect.GetHeight();

  unsigned int scaled_tex_w =
      g_ActiveConfig.bCopyEFBScaled ? Renderer::EFBToScaledX(tex_w) : tex_w;
  unsigned int scaled_tex_h =
      g_ActiveConfig.bCopyEFBScaled ? Renderer::EFBToScaledY(tex_h) : tex_h;

  // Remove all texture cache entries at dstAddr
  //   It's not possible to have two EFB copies at the same address, this makes
  //   sure any old efb
  //   copies
  //   (or normal textures) are removed from texture cache. They are also
  //   un-linked from any
  //   partially
  //   updated textures, which forces that partially updated texture to be
  //   updated.
  ...
  if (copy_to_ram) {
    g_texture_cache->CopyEFB(dst, dstFormat, tex_w, bytes_per_row, num_blocks_y,
                             dstStride, srcFormat, srcRect, isIntensity,
                             scaleByHalf);
  } else {
  ...
  if (g_bRecordFifoData) {
    // Mark the memory behind this efb copy as dynamicly generated for the Fifo
    // log
    u32 address = dstAddr;
    for (u32 i = 0; i < num_blocks_y; i++) {
      FifoRecorder::GetInstance().UseMemory(address, bytes_per_row,
                                            MemoryUpdate::TEXTURE_MAP, true);
      address += dstStride;
    }
  }
  ...
  if (dstStride < bytes_per_row) {
    ...
    // I'm pretty sure no game actually wants to do this, it might be caused by
    // a
    // programming bug in the game, or a CPU/Bounding box emulation issue with
    // dolphin.
    // The copy_to_ram code path above handles this "correctly" and scrambles
    // the image
    // but the copy_to_vram code path just saves and uses unscrambled texture
    // instead.

    // To avoid a "incorrect" result, we simply skip doing the copy_to_vram code
    // path
    // so if the game does try to use the scrambled texture, dolphin will grab
    // the scrambled
    // texture (or black if copy_to_ram is also disabled) out of ram.
    ERROR_LOG(VIDEO, "Memory stride too small (%i < %i)", dstStride,
              bytes_per_row);
    copy_to_vram = false;
  }

  // Invalidate all textures that overlap the range of our efb copy.
  // Unless our efb copy has a weird stride, then we want avoid invalidating
  // textures which
  // we might be able to do a partial texture update on.
  // TODO: This also invalidates partial overlaps, which we currently don't have
  // a better way
  //       of dealing with.
  ...
  if (copy_to_vram) {
    ...
    if (entry) {
      ...
      entry->FromRenderTarget(dst, srcFormat, srcRect, scaleByHalf, cbufid,
                              colmat);
...
TextureCacheBase::TexCache::iterator
TextureCacheBase::InvalidateTexture(TexCache::iterator iter) {
...
void TextureCacheBase::TCacheEntryBase::SetEfbCopy(u32 stride) {
  ...
  _assert_msg_(VIDEO, memory_stride >= BytesPerRow(),
...
u64 TextureCacheBase::TCacheEntryBase::CalculateHash() const {
  ...
  if (memory_stride == BytesPerRow()) {
    return GetHash64(ptr, size_in_bytes,
                     g_ActiveConfig.iSafeTextureCache_ColorSamples);
  } else {
    ...
    if (g_ActiveConfig.iSafeTextureCache_ColorSamples != 0) {
      // Hash at least 4 samples per row to avoid hashing in a bad pattern, like
      // just on the left
      // side of the efb copy
      samples_per_row =
          std::max(g_ActiveConfig.iSafeTextureCache_ColorSamples / blocks, 4u);
    }

    for (u32 i = 0; i < blocks; i++) {
      // Multiply by a prime number to mix the hash up a bit. This prevents
      // identical blocks from
      // canceling each other out
      temp_hash =
          (temp_hash * 397) ^ GetHash64(ptr, BytesPerRow(), samples_per_row);
      ptr += memory_stride;
    }
    return temp_hash;
  }
