...
TextureCacheBase::TCacheEntry* TextureCacheBase::Load(const u32 stage)
{
  ...
  if (src_data == nullptr)
  {
    ERROR_LOG(
        VIDEO,
        "TextureCache::Load has an address in Wii memory (%8x) but not in real memory (NULL)!",
        address);
    return nullptr;
  }
  else
...
// gameSrcRect is the EFB rectangle the game is trying to read from, which matches the amount of RAM
// the game has allocated to hold it
...
void TextureCacheBase::CopyRenderTargetToTexture(u32 dstAddr, EFBCopyFormat dstFormat, u32 dstStride,
                                                 ...
                                                 bool isIntensity, bool scaleByHalf)
{
  ...
  const unsigned int our_tex_w =
      scaleByHalf ? ourSrcRect.GetWidth() / 2 :
                    ourSrcRect.GetWidth();  // Currently unused. But Virtual Reality needs them.
  const unsigned int our_tex_h =
  ...
  if (copy_to_ram)
  {
	  ...
	  CopyEFB(dst, format, tex_w, bytes_per_row, num_blocks_y, dstStride, gameSrcRect, scaleByHalf);
  }
  else
  {
  ...
  if (copy_to_vram)
  {
    ...
    if (entry)
    {
      ...
      CopyEFBToCacheEntry(entry, is_depth_copy, gameSrcRect, scaleByHalf, cbufid, colmat);
