...
bool GraphicsModManager::Initialize()
{
  if (g_ActiveConfig.bGraphicMods)
  {
    ...
    m_frame_present_event = GetVideoEvents().after_present_event.Register(
        [this](const PresentInfo& present_info) { OnFramePresented(present_info); });
  }

  return true;
}

void GraphicsModManager::Shutdown()
{
  m_backend.reset();
}
...
void GraphicsModManager::Load(const Config::GraphicsModGroup& config)
{
  ...
  m_backend->SetHostConfig(ShaderHostConfig::GetCurrent());
}
...
void GraphicsModManager::SetEditorBackend(std::unique_ptr<GraphicsModBackend> editor_backend)
{
  ...
  if (m_editor_backend)
    m_editor_backend->SetHostConfig(ShaderHostConfig::GetCurrent());
}
