...
private:
  struct GraphicsMod
  {
    ...
    // No need to hash draw call ids, they are already hashed
    struct DrawCallIDPassthrough
    {
      std::size_t operator()(DrawCallID value) const { return static_cast<std::size_t>(value); }
    };

    Config::HashPolicy m_hash_policy;
    std::unordered_map<DrawCallID, std::vector<GraphicsModAction*>, DrawCallIDPassthrough>
        m_draw_id_to_actions;
