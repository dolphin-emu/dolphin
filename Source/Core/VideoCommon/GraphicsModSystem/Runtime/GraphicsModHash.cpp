                           ...
                           const Common::SmallVector<TextureTags, 8>& tags_per_texture_hash)
{
  ...
  XXH3_state_t mesh_hash_state;
  XXH3_INITSTATE(&mesh_hash_state);
  XXH3_64bits_reset_withSeed(&mesh_hash_state, static_cast<XXH64_hash_t>(1));
  ...
  {
    XXH3_64bits_update(&draw_hash_state, &draw_data.uid->blending_state.hex, sizeof(u32));
    XXH3_64bits_update(&material_hash_state, &draw_data.uid->blending_state.hex, sizeof(u32));
  }
  ...
  if ((hash_policy.attributes & Config::HashAttributes::Indices) == Config::HashAttributes::Indices)
  {
    ...
    XXH3_64bits_update(&mesh_hash_state, draw_data.index_data.data(), draw_data.index_data.size());
  }

  if ((hash_policy.attributes & Config::HashAttributes::VertexLayout) ==
      Config::HashAttributes::VertexLayout)
  {
    const u32 vertex_count = draw_data.vertex_count;
    XXH3_64bits_update(&draw_hash_state, &vertex_count, sizeof(u32));
    XXH3_64bits_update(&mesh_hash_state, &vertex_count, sizeof(u32));
  }
  ...
  else
  {
    ...
    for (std::size_t i = 0; i < draw_data.textures.size(); i++)
    {
      ...
      const auto& texture_tags = tags_per_texture_hash[i];
      if (texture_tags[GraphicsModSystem::TextureTag::Ignore])
        continue;
  ...
  output.mesh_id = static_cast<MeshID>(XXH3_64bits_digest(&mesh_hash_state));
  return output;
}
}  // namespace GraphicsModSystem::Runtime
