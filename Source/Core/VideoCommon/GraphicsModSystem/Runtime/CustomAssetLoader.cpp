...
{
  return LoadOrCreateAsset<CustomTextureAsset>(asset_id, m_textures, std::move(library));
}
...
CustomAssetLoader::LoadGameTexture(const CustomAssetLibrary::AssetID& asset_id,
                                   std::shared_ptr<CustomAssetLibrary> library)
{
  return LoadOrCreateAsset<CustomGameTextureAsset>(asset_id, m_game_textures, std::move(library));
}
}  // namespace VideoCommon
