  ...
  */
  ...
  if (((s32 *)&bpmem)[bp.address] == bp.newvalue) {
    if (!(bp.address == BPMEM_TRIGGER_EFB_COPY ||
          bp.address == BPMEM_CLEARBBOX1 || bp.address == BPMEM_CLEARBBOX2 ||
          bp.address == BPMEM_SETDRAWDONE || bp.address == BPMEM_PE_TOKEN_ID ||
  ...
  case BPMEM_GENMODE: // Set the Generation Mode
    PRIM_LOG("genmode: texgen=%d, col=%d, multisampling=%d, tev=%d, "
             ...
             (u32)bpmem.genMode.multisampling,
             (u32)bpmem.genMode.numtevstages + 1, (u32)bpmem.genMode.cullmode,
  ...
  case BPMEM_IND_MTXC + 6:
    if (bp.changes)
      PixelShaderManager::SetIndMatrixChanged((bp.address - BPMEM_IND_MTXA) /
                                              3);
    return;
  ...
  case BPMEM_BLENDMODE: // Blending Control
    if (bp.changes & 0xFFFF) {
      PRIM_LOG(
          "blendmode: en=%d, open=%d, colupd=%d, alphaupd=%d, dst=%d, src=%d, "
          "sub=%d, mode=%d",
      ...
      // Set Blending Mode
      if (bp.changes &
          0xFF1) // blendenable | alphaupdate | dstfactor | srcfactor | subtract
        SetBlendMode();
  ...
  case BPMEM_CONSTANTALPHA: // Set Destination Alpha
    PixelShaderManager::UpdateBP(bp.address, bp.newvalue);
    PRIM_LOG("constalpha: alp=%d, en=%d", bpmem.dstalpha.alpha,
  ...
  case BPMEM_SETDRAWDONE:
    ...
    case 0x02:
      ...
      DEBUG_LOG(VIDEO, "GXSetDrawDone SetPEFinish (value: 0x%02X)",
                (bp.newvalue & 0xFFFF));
      return;

    default:
      WARN_LOG(VIDEO, "GXSetDrawDone ??? (value 0x%02X)",
               (bp.newvalue & 0xFFFF));
      return;
    }
    return;
  ...
  case BPMEM_LOADTLUT1: // Load a Texture Look Up Table
  {
    ...
    if (g_bRecordFifoData)
      FifoRecorder::GetInstance().UseMemory(addr, tlutXferCount,
                                            MemoryUpdate::TMEM);
  ...
  case BPMEM_ZTEX2: // Z Texture type
  {
    ...
    PRIM_LOG("ztex op=%s, type=%s", pzop[bpmem.ztex2.op],
             pztype[bpmem.ztex2.type]);
#endif
  }
    return;
  // ----------------------------------
  ...
  case BPMEM_DISPLAYCOPYFILTER +
      1: // if (aa) { use sample_pattern } else { use 666666 }
  case BPMEM_DISPLAYCOPYFILTER +
      2: // if (aa) { use sample_pattern } else { use 666666 }
  case BPMEM_DISPLAYCOPYFILTER +
      3:                  // if (aa) { use sample_pattern } else { use 666666 }
  ...
  case BPMEM_PRELOAD_MODE: // Set to 0 when GX_TexModeSync() is called.
    ...
    if (bp.newvalue != 0) {
      ...
      u32 src_addr = tmem_cfg.preload_addr
                     << 5; // TODO: Should we add mask here on GC?
      u32 bytes_read = 0;
      ...
      } else // RGBA8 tiles (and CI14, but that might just be stupid libogc!)
      {
        ...
        for (u32 i = 0; i < tmem_cfg.preload_tile_info.count; ++i) {
          ...
          memcpy(texMem + tmem_addr_odd, src_ptr + bytes_read + TMEM_LINE_SIZE,
                 TMEM_LINE_SIZE);
  ...
  case BPMEM_TEV_COLOR_RA + 6: {
    ...
    if (bpmem.tevregs[num].type_ra) {
      ...
      PixelShaderManager::SetTevKonstColor(num, 3,
                                           (s32)bpmem.tevregs[num].alpha);
    } else {
  ...
  case BPMEM_TEV_COLOR_BG + 6: {
    ...
    if (bpmem.tevregs[num].type_bg) {
      PixelShaderManager::SetTevKonstColor(num, 1,
                                           (s32)bpmem.tevregs[num].green);
      PixelShaderManager::SetTevKonstColor(num, 2,
                                           (s32)bpmem.tevregs[num].blue);
    } else {
  ...
  case BPMEM_SU_TSIZE + 14:
    if (bp.changes) {
      PixelShaderManager::SetTexCoordChanged((bp.address - BPMEM_SU_SSIZE) >>
                                             1);
      GeometryShaderManager::SetTexCoordChanged((bp.address - BPMEM_SU_SSIZE) >>
                                                1);
    }
    return;
  // ------------------------
  ...
  WARN_LOG(VIDEO, "Unknown BP opcode: address = 0x%08x value = 0x%08x",
           bp.address, bp.newvalue);
}
...
#define SetRegName(reg)                                                        \
  *name = #reg;                                                                \
  ...
  case BPMEM_BLENDMODE: // 0x41
  {
    ...
    BlendMode mode;
    mode.hex = cmddata;
    const char *dstfactors[] = {"0",           "1",          "src_color",
                                "1-src_color", "src_alpha",  "1-src_alpha",
                                "dst_alpha",   "1-dst_alpha"};
    const char *srcfactors[] = {"0",           "1",          "dst_color",
                                "1-dst_color", "src_alpha",  "1-src_alpha",
                                "dst_alpha",   "1-dst_alpha"};
    const char *logicmodes[] = {"0",        "s & d",    "s & ~d",   "s",
                                "~s & d",   "d",        "s ^ d",    "s | d",
                                "~(s | d)", "~(s ^ d)", "~d",       "s | ~d",
                                "~s",       "~s | d",   "~(s & d)", "1"};
    *desc = StringFromFormat(
        ...
        no_yes[mode.dither], no_yes[mode.colorupdate], no_yes[mode.alphaupdate],
        dstfactors[mode.dstfactor], srcfactors[mode.srcfactor],
        no_yes[mode.subtract], logicmodes[mode.logicmode]);
  } break;
  ...
  case BPMEM_ZCOMPARE: // 0x43
  {
    ...
    PEControl config;
    config.hex = cmddata;
    const char *pixel_formats[] = {"RGB8_Z24", "RGBA6_Z24", "RGB565_Z16",
                                   "Z24",      "Y8",        "U8",
                                   "V8",       "YUV420"};
    const char *zformats[] = {"linear",
                              "compressed (near)",
                              "compressed (mid)",
                              "compressed (far)",
                              "inv linear",
                              ...
                              "compressed (inv far)"};
    *desc =
        StringFromFormat("EFB pixel format: %s\n"
                         "Depth format: %s\n"
                         "Early depth test: %s\n",
                         pixel_formats[config.pixel_format],
                         zformats[config.zformat], no_yes[config.early_ztest]);
  } break;
  ...
  case BPMEM_EFB_BR: // 0x4A
  {
    ...
    *desc = StringFromFormat("Width: %d\nHeight: %d", width_height.x + 1,
                             width_height.y + 1);
  } break;

  case BPMEM_EFB_ADDR: // 0x4B
    SetRegName(BPMEM_EFB_ADDR);
    *desc = StringFromFormat("Target address (32 byte aligned): 0x%06X",
                             cmddata << 5);
    break;
  ...
  case BPMEM_COPYYSCALE: // 0x4E
    ...
    *desc = StringFromFormat(
        "Scaling factor (XFB copy only): 0x%X (%f or inverted %f)", cmddata,
  ...
  case BPMEM_TRIGGER_EFB_COPY: // 0x52
  {
    ...
    *desc = StringFromFormat(
        ...
        (copy.clamp0 && copy.clamp1)
            ? "Top and Bottom"
            : (copy.clamp0) ? "Top only" : (copy.clamp1) ? "Bottom only"
                                                         : "None",
  ...
  case BPMEM_TX_SETIMAGE0_4 + 3: {
    ...
    int texnum = (cmd < BPMEM_TX_SETIMAGE0_4) ? cmd - BPMEM_TX_SETIMAGE0
                                              : cmd - BPMEM_TX_SETIMAGE0_4 + 4;
    TexImage0 teximg;
    teximg.hex = cmddata;
    *desc = StringFromFormat("Texture Unit: %i\n"
                             ...
                             "Height: %i\n"
                             "Format: %x\n",
                             texnum, teximg.width + 1, teximg.height + 1,
                             teximg.format);
  } break;
  ...
  case BPMEM_TX_SETIMAGE1_4 + 3: {
    ...
    int texnum = (cmd < BPMEM_TX_SETIMAGE1_4) ? cmd - BPMEM_TX_SETIMAGE1
                                              : cmd - BPMEM_TX_SETIMAGE1_4 + 4;
    TexImage1 teximg;
    teximg.hex = cmddata;
    *desc = StringFromFormat("Texture Unit: %i\n"
                             "Even TMEM Offset: %x\n"
                             ...
                             texnum, teximg.tmem_even, teximg.cache_width,
                             teximg.cache_height, no_yes[teximg.image_type]);
  } break;
  ...
  case BPMEM_TX_SETIMAGE2_4 + 3: {
    ...
    int texnum = (cmd < BPMEM_TX_SETIMAGE2_4) ? cmd - BPMEM_TX_SETIMAGE2
                                              : cmd - BPMEM_TX_SETIMAGE2_4 + 4;
    TexImage2 teximg;
    teximg.hex = cmddata;
    *desc = StringFromFormat("Texture Unit: %i\n"
                             "Odd TMEM Offset: %x\n"
                             "Odd TMEM Width: %i\n"
                             "Odd TMEM Height: %i\n",
                             texnum, teximg.tmem_odd, teximg.cache_width,
                             teximg.cache_height);
  } break;
  ...
  case BPMEM_TX_SETIMAGE3_4 + 3: {
    ...
    int texnum = (cmd < BPMEM_TX_SETIMAGE3_4) ? cmd - BPMEM_TX_SETIMAGE3
                                              : cmd - BPMEM_TX_SETIMAGE3_4 + 4;
    TexImage3 teximg;
    teximg.hex = cmddata;
    *desc =
        StringFromFormat("Texture %i source address (32 byte aligned): 0x%06X",
                         texnum, teximg.image_base << 5);
  } break;
  ...
  case BPMEM_TEV_COLOR_ENV + 30: {
    ...
    cc.hex = cmddata;
    const char *tevin[] = {
        "prev.rgb", "prev.aaa", "c0.rgb",    "c0.aaa",  "c1.rgb",  "c1.aaa",
        "c2.rgb",   "c2.aaa",   "tex.rgb",   "tex.aaa", "ras.rgb", "ras.aaa",
    ...
    *desc = StringFromFormat("Tev stage: %d\n"
                             ...
                             "Scale factor: %s\n"
                             "Dest: %s\n",
                             (data[0] - BPMEM_TEV_COLOR_ENV) / 2, tevin[cc.a],
                             tevin[cc.b], tevin[cc.c], tevin[cc.d],
                             tevbias[cc.bias], tevop[cc.op], no_yes[cc.clamp],
  ...
  case BPMEM_TEV_ALPHA_ENV + 30: {
    ...
    *desc = StringFromFormat("Tev stage: %d\n"
                             ...
                             "Ras sel: %d\n"
                             "Tex sel: %d\n",
                             (data[0] - BPMEM_TEV_ALPHA_ENV) / 2, tevin[ac.a],
                             tevin[ac.b], tevin[ac.c], tevin[ac.d],
                             tevbias[ac.bias], tevop[ac.op], no_yes[ac.clamp],
                             tevscale[ac.shift], tevout[ac.dest],
  ...
  case BPMEM_ALPHACOMPARE: // 0xF3
  {
    ...
    *desc = StringFromFormat("Test 1: %s (ref: %#02x)\n"
                             "Test 2: %s (ref: %#02x)\n"
                             "Logic: %s\n",
                             functions[test.comp0], (int)test.ref0,
                             functions[test.comp1], (int)test.ref1,
                             logic[test.logic]);
    break;
  }
