...
void Statistics::ResetFrame() { memset(&thisFrame, 0, sizeof(ThisFrame)); }
...
std::string Statistics::ToString() {
  ...
  str +=
      StringFromFormat("XF loads (DL): %i\n", stats.thisFrame.numXFLoadsInDL);
  str += StringFromFormat("CP loads: %i\n", stats.thisFrame.numCPLoads);
  str +=
      StringFromFormat("CP loads (DL): %i\n", stats.thisFrame.numCPLoadsInDL);
  str += StringFromFormat("BP loads: %i\n", stats.thisFrame.numBPLoads);
  str +=
      StringFromFormat("BP loads (DL): %i\n", stats.thisFrame.numBPLoadsInDL);
  ...
  // TODO : at some point text1 just becomes too huge and overflows, we can't
  // even read the added
  // stuff
  // since it gets added at the far bottom of the screen anyway (actually
  // outside the rendering
  // window)
