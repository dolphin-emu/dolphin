...
// For putting a new bug in here, make sure to put a detailed comment above the
// enum
...
enum Bug {
  ...
  // The offset of glBindBufferRange was ignored on all Mesa Gallium3D drivers
  // until 9.1.3
  // Nouveau stored the offset as u16 which isn't enough for all cases with
  // range until 9.1.6
  // I965 has broken data fetches from uniform buffers which results in a
  // dithering until 9.2.0
  BUG_BROKENUBO,
  ...
  // Affected OS: Linux
  // Pinned memory is disabled for index buffer as the AMD driver (the only one
  // with pinned memory
  // support) seems
  // to be broken. We just get flickering/black rendering when using pinned
  // memory here -- degasus -
  // 2013/08/20
  ...
  // Started Version: -1
  // Ended Version: -1
  // Both Adreno and Mali have issues when you call glBufferSubData or
  // glMapBufferRange
  // The driver stalls in each instance no matter what you do
  // Apparently Mali and Adreno share code in this regard since it was wrote by
  // the same person.
  ...
  // The buffer_storage streaming method is required for greater speed gains in
  // our buffer streaming
  ...
  // Started Version: -1
  // Ended Version: -1
  // The Nvidia driver (both Windows + Linux) doesn't like unsync mapping
  // performance wise.
  // Because of their threaded behavior, they seem not to handle unsync mapping
  // complete unsync,
  ...
  // Bug: Intel's Window driver broke buffer_storage with
  ...
  // Instead of inverting the boolean value it does a binary negation on the
  // full 32bit register
  // This causes a compare against zero to fail in their shader since it is no
  // longer a 0 or 1 value
  // but 0xFFFFFFFF or 0xFFFFFFFE depending on what the boolean value was before
  // the negation.
  //
  // This bug has a secondary issue tied to it unlike other bugs.
  // The correction of this bug is to check the boolean value against false
  // which results in us
  // not doing a negation of the source but instead checking against the boolean
  // value we want.
  // The issue with this is that Intel's Window driver is broken when checking
  ...
  // Ended Version: -1
  // If we disable vsync with eglSwapInterval(dpy, 0) then the screen will stop
  // showing new updates
  ...
  // We can't actually detect what the driver version is on Android, so until
  // the driver version
  // lands that displays the version in
  // the GL_VERSION string, we will have to force vsync to be enabled at all
  // times.
  BUG_BROKENVSYNC,
  ...
  // Mesa introduced geometry shader support for radeon and sandy bridge devices
  // and failed to test
  // it with us.
  ...
  // Qualcomm seems to have lots of overhead on exlicit flushing, but the
  // coherent mapping path is
  // fine.
  ...
  // Started Version: -1
  // Ended Version: -1
  // Bounding box reads use glGetBufferSubData to read back the contents of the
  // SSBO, but this is
  // slow on AMD drivers, compared to
  // using glMapBufferRange. glMapBufferRange is slower on Nvidia drivers, we
  // suspect due to the
  // first call moving the buffer from
  // GPU memory to system memory. Use glMapBufferRange for BBox reads on AMD,
...
// Once Vendor and driver version is set, this will return if it has the
// applicable bug passed to
// it.
bool HasBug(Bug bug);
}
