...
bool Renderer::WantNextFrameDump()
{
#if defined(HAVE_LIBAV) || defined(_WIN32)
...
void Renderer::PushFrameDumpData(const u8* data, int w, int h, AVIDump::DumpFormat format,
                                 bool swap_upside_down)
{
  ...
  if (w != m_last_framedump_width || h != m_last_framedump_height ||
      format != m_last_framedump_format)
  {
  ...
  // TODO: Refactor this. Right now it's needed for flipping the image.
  memcpy(frame_data.data(), data, image_size);
