...
static bool IsOnThread() { return SConfig::GetInstance().bCPUThread; }
...
static inline u16 ReadLow(u32 _reg) { return (u16)(_reg & 0xFFFF); }
static inline u16 ReadHigh(u32 _reg) { return (u16)(_reg >> 16); }
...
void Init() {
  ...
  et_UpdateInterrupts =
      CoreTiming::RegisterEvent("CPInterrupt", UpdateInterrupts_Wrapper);
}
...
void RegisterMMIO(MMIO::Mapping *mmio, u32 base) {
  ...
  for (auto &mapped_var : directly_mapped_vars) {
    ...
    mmio->Register(
        base | mapped_var.addr, MMIO::DirectRead<u16>(mapped_var.ptr),
  ...
  mmio->Register(
      ...
      MMIO::ComplexWrite<u16>(
          [](u32, u16 val) { WriteLow(fifo.CPBreakpoint, val & 0xffe0); }));
  mmio->Register(base | FIFO_BP_HI, MMIO::DirectRead<u16>(MMIO::Utils::HighPart(
                                        &fifo.CPBreakpoint)),
                 MMIO::ComplexWrite<u16>(
                     [](u32, u16 val) { WriteHigh(fifo.CPBreakpoint, val); }));
  ...
  mmio->Register(base | PERF_SELECT, MMIO::InvalidRead<u16>(),
                 MMIO::Nop<u16>());
  ...
  mmio->Register(
      ...
      IsOnThread() ? MMIO::ComplexRead<u16>([](u32) {
        if (fifo.CPWritePointer >= fifo.SafeCPReadPointer)
          ...
          return ReadLow(fifo.CPEnd - fifo.SafeCPReadPointer +
                         fifo.CPWritePointer - fifo.CPBase + 32);
      })
                   : MMIO::DirectRead<u16>(
                         MMIO::Utils::LowPart(&fifo.CPReadWriteDistance)),
      MMIO::DirectWrite<u16>(MMIO::Utils::LowPart(&fifo.CPReadWriteDistance),
                             0xFFE0));
  mmio->Register(
      base | FIFO_RW_DISTANCE_HI,
      IsOnThread() ? MMIO::ComplexRead<u16>([](u32) {
        if (fifo.CPWritePointer >= fifo.SafeCPReadPointer)
          return ReadHigh(fifo.CPWritePointer - fifo.SafeCPReadPointer);
        else
          return ReadHigh(fifo.CPEnd - fifo.SafeCPReadPointer +
                          fifo.CPWritePointer - fifo.CPBase + 32);
      })
                   : MMIO::DirectRead<u16>(
                         MMIO::Utils::HighPart(&fifo.CPReadWriteDistance)),
      MMIO::ComplexWrite<u16>([](u32, u16 val) {
  ...
  mmio->Register(
      ...
      IsOnThread()
          ? MMIO::DirectRead<u16>(
                MMIO::Utils::HighPart(&fifo.SafeCPReadPointer))
...
void GatherPipeBursted() {
  ...
  if (!m_CPCtrlReg.GPLinkEnable) {
    if (IsOnThread() && !Fifo::UseDeterministicGPUThread()) {
      // In multibuffer mode is not allowed write in the same FIFO attached to
      // the GPU.
  ...
  // If the game is running close to overflowing, make the exception checking
  // more frequent.
...
bool IsInterruptWaiting() { return s_interrupt_waiting.load(); }
...
void SetCPStatusFromGPU() {
  ...
  if (fifo.bFF_BPEnable) {
    if (fifo.CPBreakpoint == fifo.CPReadPointer) {
      ...
      if (fifo.bFF_Breakpoint)
        INFO_LOG(COMMANDPROCESSOR, "Cleared breakpoint at %i",
                 fifo.CPReadPointer);
...
void ProcessFifoEvents() {
  if (IsOnThread() &&
      (s_interrupt_waiting.load() || s_interrupt_finish_waiting.load() ||
...
void SetCpStatusRegister() {
  ...
  m_CPStatusReg.ReadIdle =
      !fifo.CPReadWriteDistance || (fifo.CPReadPointer == fifo.CPWritePointer);
  m_CPStatusReg.CommandIdle = !fifo.CPReadWriteDistance ||
                              Fifo::AtBreakpoint() || !fifo.bFF_GPReadEnable;
  m_CPStatusReg.UnderflowLoWatermark = fifo.bFF_LoWatermark;
  m_CPStatusReg.OverflowHiWatermark = fifo.bFF_HiWatermark;

  INFO_LOG(COMMANDPROCESSOR, "\t Read from STATUS_REGISTER : %04x",
           m_CPStatusReg.Hex);
  DEBUG_LOG(
      COMMANDPROCESSOR,
...
void SetCpControlRegister() {
  ...
  DEBUG_LOG(COMMANDPROCESSOR,
            "\t GPREAD %s | BP %s | Int %s | OvF %s | UndF %s | LINK %s",
            fifo.bFF_GPReadEnable ? "ON" : "OFF",
            fifo.bFF_BPEnable ? "ON" : "OFF", fifo.bFF_BPInt ? "ON" : "OFF",
...
// We don't emulate proper GP timing anyway at the moment, so it would just slow
// down emulation.
