...
static T GeneratePixelShader(DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType)
{
	...
	else if (bpmem.UseEarlyDepthTest() && (uid_data->fast_depth_calc || bpmem.alpha_test.TestResult() == AlphaTest::UNDETERMINED))
	{
	...
	if (ApiType == API_OPENGL)
	{
		...
		if (g_ActiveConfig.backend_info.bSupportsGeometryShaders)
		{
			...
			GenerateVSOutputMembers<T>(out, ApiType, uid_data->genMode_numtexgens, uid_data->per_pixel_lighting, GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa, true, true));
	...
	else // D3D
	{
		...
		// compute window position if needed because binding semantic WPOS is not widely supported
		for (unsigned int i = 0; i < uid_data->genMode_numtexgens; ++i)
			out.Write(",\n  in %s float3 uv%d : TEXCOORD%d", GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa), i, i);
		out.Write(",\n  in %s float4 clipPos : TEXCOORD%d", GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa), uid_data->genMode_numtexgens);
		if (uid_data->per_pixel_lighting)
		{
			out.Write(",\n  in %s float3 Normal : TEXCOORD%d", GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa), uid_data->genMode_numtexgens + 1);
			out.Write(",\n  in %s float3 WorldPos : TEXCOORD%d", GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa), uid_data->genMode_numtexgens + 2);
		}
		if (uid_data->stereo)
