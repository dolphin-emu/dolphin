...
static const char *tevRasTable[] = {
    ...
    "(int4(1, 1, 1, 1) * (alphabump | (alphabump >> 5)))", // normalized bump
                                                           // alpha (0..255)
    "int4(0, 0, 0, 0)",                                    // zero
};

static const char *tevCOutputTable[] = {"prev.rgb", "c0.rgb", "c1.rgb",
                                        "c2.rgb"};
static const char *tevAOutputTable[] = {"prev.a", "c0.a", "c1.a", "c2.a"};

template <class T>
static void WriteStage(T &out, pixel_shader_uid_data *uid_data, int n,
                       API_TYPE ApiType);
template <class T>
static void WriteTevRegular(T &out, const char *components, int bias, int op,
                            int clamp, int shift);
template <class T>
static void SampleTexture(T &out, const char *texcoords, const char *texswap,
                          int texmap, bool stereo, API_TYPE ApiType);
template <class T>
static void WriteAlphaTest(T &out, pixel_shader_uid_data *uid_data,
...
static T GeneratePixelShader(DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType) {
  ...
  pixel_shader_uid_data *uid_data =
      out.template GetUidData<pixel_shader_uid_data>();
  if (uid_data == nullptr)
    uid_data = &dummy_data;
...

  {
    const bool forced_early_z =
        ...
        // We can't allow early_ztest for zfreeze because depth is overridden
        // per-pixel.
        // This means it's impossible for zcomploc to be emulated on a zfrozen
        // polygon.
        && !(bpmem.zmode.testenable && bpmem.genMode.zfreeze);
    const bool per_pixel_depth =
        (bpmem.ztex2.op != ZTEXTURE_DISABLE && bpmem.UseLateDepthTest()) ||
        (!g_ActiveConfig.bFastDepthCalc && bpmem.zmode.testenable &&
         !forced_early_z) ||
        (bpmem.zmode.testenable && bpmem.genMode.zfreeze);
  ...
  if (uid_data->forced_early_z) {
    // Zcomploc (aka early_ztest) is a way to control whether depth test is done
    // before
    // or after texturing and alpha test. PC graphics APIs used to provide no
    // way to emulate
    // this feature properly until 2012: Depth tests were always done after
    // alpha testing.
    // Most importantly, it was not possible to write to the depth buffer
    // without also writing
    // a color value (unless color writing was disabled altogether).

    // OpenGL 4.2 actually provides two extensions which can force an early z
    // test:
    //  * ARB_image_load_store has 'layout(early_fragment_tests)' which forces
    //  the driver to do z
    //  and stencil tests early.
    //  * ARB_conservative_depth has 'layout(depth_unchanged) which signals to
    //  the driver that it
    //  can make optimisations
    //    which assume the pixel shader won't update the depth buffer.

    // early_fragment_tests is the best option, as it requires the driver to do
    // early-z and defines
    // early-z exactly as
    // we expect, with discard causing the shader to exit with only the depth
    // buffer updated.

    // Conservative depth's 'depth_unchanged' only hints to the driver that an
    // early-z optimisation
    // can be made and
    // doesn't define what will happen if we discard the fragment. But the way
    // modern graphics hardware is implemented
    // means it is not unreasonable to expect the same behaviour as
    // early_fragment_tests.
    // We can also assume that if a driver has gone out of its way to support
    // conservative depth and
    // not image_load_store
    // as required by OpenGL 4.2 that it will be doing the optimisation.
    // If the driver doesn't actually do an early z optimisation, ZCompLoc will
    // be broken and depth
    // will only be written
    // if the alpha test passes.

    // We support Conservative as a fallback, because many drivers based on Mesa
    // haven't implemented
    // all of the
    // ARB_image_load_store extension yet.

    // D3D11 also has a way to force the driver to enable early-z, so we're fine
    // here.
    if (ApiType == API_OPENGL) {
      // This is a #define which signals whatever early-z method the driver
      // supports.
      ...
      out.Write("[earlydepthstencil]\n");
             ...
             (uid_data->fast_depth_calc ||
              bpmem.alpha_test.TestResult() == AlphaTest::UNDETERMINED)) {
    static bool warn_once = true;
    if (warn_once)
      WARN_LOG(VIDEO,
               "Early z test enabled but not possible to emulate with current "
               "configuration. Make sure to enable fast depth calculations. If "
               "this message "
               "still shows up your hardware isn't able to emulate the feature "
               "properly (a "
  ...
  } else // D3D
  {
    ...
    for (unsigned int i = 0; i < uid_data->genMode_numtexgens; ++i)
      out.Write(",\n  in %s float3 uv%d : TEXCOORD%d",
                GetInterpolationQualifier(), i, i);
    out.Write(",\n  in %s float4 clipPos : TEXCOORD%d",
              GetInterpolationQualifier(), uid_data->genMode_numtexgens);
    if (uid_data->per_pixel_lighting) {
      out.Write(",\n  in %s float3 Normal : TEXCOORD%d",
                GetInterpolationQualifier(), uid_data->genMode_numtexgens + 1);
      out.Write(",\n  in %s float3 WorldPos : TEXCOORD%d",
                GetInterpolationQualifier(), uid_data->genMode_numtexgens + 2);
    }
    if (uid_data->stereo)
  ...
  out.Write("\tint4 c0 = " I_COLORS "[1], c1 = " I_COLORS "[2], c2 = " I_COLORS
            "[3], prev = " I_COLORS "[0];\n"
            "\tint4 rastemp = int4(0, 0, 0, 0), textemp = int4(0, 0, 0, 0), "
            "konsttemp = int4(0, 0, "
            "0, 0);\n"
            ...
            "\tint4 "
            "tevin_a=int4(0,0,0,0),tevin_b=int4(0,0,0,0),tevin_c=int4(0,0,0,0),"
            "tevin_d=int4(0,0,0,"
            "0);\n\n"); // tev combiner inputs
  ...
  if (uid_data->per_pixel_lighting) {
    ...
    // The lighting shader only needs the two color bits of the 23bit component
    // bit array.
    ...
    // TODO: Our current constant usage code isn't able to handle more than one
    // buffer.
    //       So we can't mark the VS constant as used here. But keep them here
    //       as reference.
    // out.SetConstantsUsed(C_PLIGHT_COLORS, C_PLIGHT_COLORS+7); // TODO: Can be
    // optimized further
    // out.SetConstantsUsed(C_PLIGHTS, C_PLIGHTS+31); // TODO: Can be optimized
    // further
    // out.SetConstantsUsed(C_PMATERIALS, C_PMATERIALS+3);
    GenerateLightingShader<T>(out, uid_data->lighting,
                              uid_data->components << VB_COL_SHIFT, "colors_",
                              "col");
  }
  ...
  if (uid_data->genMode_numtexgens == 0) {
    ...
    out.SetConstantsUsed(C_TEXDIMS,
                         C_TEXDIMS + uid_data->genMode_numtexgens - 1);
    for (unsigned int i = 0; i < uid_data->genMode_numtexgens; ++i) {
  ...
  if (uid_data->genMode_numindstages > 0) {
    for (unsigned int i = 0; i < numStages; ++i) {
      if (bpmem.tevind[i].IsActive() &&
          bpmem.tevind[i].bt < uid_data->genMode_numindstages)
  ...
  for (u32 i = 0; i < uid_data->genMode_numindstages; ++i) {
    if (uid_data->nIndirectStagesUsed & (1 << i)) {
      uid_data->SetTevindrefValues(i, bpmem.tevindref.getTexCoord(i),
                                   bpmem.tevindref.getTexMap(i));
      ...
      SampleTexture<T>(out, "float2(tempcoord)", "abg", texmap,
                       uid_data->stereo, ApiType);
    }
  }

  for (unsigned int i = 0; i < numStages; i++)
    WriteStage<T>(out, uid_data, i,
                  ApiType); // build the equation for this stage
...
#define MY_STRUCT_OFFSET(str, elem) ((u32)((u64) & (str).elem - (u64) & (str)))
  ...
  // NOTE: Fragment may not be discarded if alpha test always fails and early
  // depth test is enabled
  // (in this case we need to write a depth value if depth test passes
  // regardless of the alpha testing result)
  if (uid_data->Pretest == AlphaTest::UNDETERMINED ||
      (uid_data->Pretest == AlphaTest::FAIL && uid_data->late_ztest))
    WriteAlphaTest<T>(out, uid_data, ApiType, dstAlphaMode,
                      uid_data->per_pixel_depth);
  ...
  if (uid_data->zfreeze) {
    ...
    out.Write("\tint zCoord = int(" I_ZSLOPE ".z + " I_ZSLOPE
    ...
    // It should be correct, but it seems not to be as accurate as required.
    // TODO: Find out why!
    // For disabled FastDepth we just calculate the depth value again.
    // The performance impact of this additional calculation doesn't matter, but
    // it prevents
    // the host GPU driver from performing any early depth test optimizations.
    out.SetConstantsUsed(C_ZBIAS + 1, C_ZBIAS + 1);
    // the screen space depth value = far z + (clip z / clip w) * z range
    out.Write("\tint zCoord = " I_ZBIAS
              "[1].x + int((clipPos.z / clipPos.w) * float(" I_ZBIAS
              "[1].y));\n");
  } else {
    if (ApiType == API_D3D)
  ...
  // depth texture can safely be ignored if the result won't be written to the
  // depth buffer
  // (early_ztest) and isn't used for fog either
  const bool skip_ztexture = !uid_data->per_pixel_depth && !uid_data->fog_fsel;

  // Note: z-textures are not written to depth buffer if early depth test is
  // used
  ...
  // Note: depth texture output is only written to depth buffer if late depth
  // test is used
  // theoretical final depth value is used for fog calculation, though, so we
  // have to emulate ztextures anyway
  if (uid_data->ztex_op != ZTEXTURE_DISABLE && !skip_ztexture) {
    // use the texture input of the last texture stage (textemp), hopefully this
    // has been read and
    // is in correct format...
    out.SetConstantsUsed(C_ZBIAS, C_ZBIAS + 1);
    out.Write("\tzCoord = idot(" I_ZBIAS "[0].xyzw, textemp.xyzw) + " I_ZBIAS
              "[1].w %s;\n",
  ...
  if (dstAlphaMode == DSTALPHA_ALPHA_PASS) {
    ...
    out.Write("\tocol0 = float4(float3(prev.rgb), float(" I_ALPHA
              ".a)) / 255.0;\n");
  } else {
  ...
  if (uid_data->bounding_box) {
    ...
    out.Write("\tif(bbox_data[0] > int(rawpos.x)) %sMin(bbox_data[0], "
              "int(rawpos.x));\n"
              "\tif(bbox_data[1] < int(rawpos.x)) %sMax(bbox_data[1], "
              "int(rawpos.x));\n"
              "\tif(bbox_data[2] > int(rawpos.y)) %sMin(bbox_data[2], "
              "int(rawpos.y));\n"
              "\tif(bbox_data[3] < int(rawpos.y)) %sMax(bbox_data[3], "
              "int(rawpos.y));\n",
...
static void WriteStage(T &out, pixel_shader_uid_data *uid_data, int n,
  ...
  if (stage.hasindstage) {
    ...
    // perform the indirect op on the incoming regular coordinates using
    // iindtex%d as the offset coords
    if (tevind.bs != ITBA_OFF) {
      const char *tevIndAlphaSel[] = {"", "x", "y", "z"};
      const char *tevIndAlphaMask[] = {
          "248", "224", "240",
          "248"}; // 0b11111000, 0b11100000, 0b11110000, 0b11111000
      out.Write("alphabump = iindtex%d.%s & %s;\n", tevind.bt,
                tevIndAlphaSel[tevind.bs], tevIndAlphaMask[tevind.fmt]);
    } else {
    ...
    if (tevind.mid != 0) {
      ...
      const char *tevIndBiasField[] = {"",   "x",  "y",  "xy", "z",
                                       "xz", "yz", "xyz"};   // indexed by bias
      ...
      if (tevind.bias == ITB_S || tevind.bias == ITB_T || tevind.bias == ITB_U)
        out.Write("\tiindtevcrd%d.%s += int(%s);\n", n,
                  tevIndBiasField[tevind.bias], tevIndBiasAdd[tevind.fmt]);
      else if (tevind.bias == ITB_ST || tevind.bias == ITB_SU ||
               tevind.bias == ITB_TU)
      ...
      // multiply by offset matrix and scale - calculations are likely to
      // overflow badly,
      // yet it works out since we only care about the lower 23 bits (+1 sign
      // bit) of the result
      if (tevind.mid <= 3) {
        ...
        out.Write("\tint2 indtevtrans%d = int2(idot(" I_INDTEXMTX
                  "[%d].xyz, iindtevcrd%d), idot(" I_INDTEXMTX
                  "[%d].xyz, iindtevcrd%d)) >> 3;\n",
                  n, mtxidx, n, mtxidx + 1, n);

        // TODO: should use a shader uid branch for this for better performance
        out.Write("\tif (" I_INDTEXMTX
                  "[%d].w >= 0) indtevtrans%d >>= " I_INDTEXMTX "[%d].w;\n",
                  mtxidx, n, mtxidx);
        out.Write("\telse indtevtrans%d <<= (-" I_INDTEXMTX "[%d].w);\n", n,
                  mtxidx);
      } else if (tevind.mid <= 7 && bHasTexCoord) { // s matrix
        _assert_(tevind.mid >= 5);
        int mtxidx = 2 * (tevind.mid - 5);
        out.SetConstantsUsed(C_INDTEXMTX + mtxidx, C_INDTEXMTX + mtxidx);

        out.Write(
            "\tint2 indtevtrans%d = int2(fixpoint_uv%d * iindtevcrd%d.xx);\n",
            n, stage.tevorders_texcoord, n);
        ...
        out.Write("\telse indtevtrans%d <<= (-" I_INDTEXMTX "[%d].w);\n", n,
                  mtxidx);
      } else if (tevind.mid <= 11 && bHasTexCoord) { // t matrix
        _assert_(tevind.mid >= 9);
        int mtxidx = 2 * (tevind.mid - 9);
        out.SetConstantsUsed(C_INDTEXMTX + mtxidx, C_INDTEXMTX + mtxidx);

        out.Write(
            "\tint2 indtevtrans%d = int2(fixpoint_uv%d * iindtevcrd%d.yy);\n",
            n, stage.tevorders_texcoord, n);
    ...
    // Wrapping
    // ---------
    const char *tevIndWrapStart[] = {"0",       "(256<<7)", "(128<<7)",
                                     "(64<<7)", "(32<<7)",  "(16<<7)",
                                     "1"}; // TODO: Should the last one be 1 or
                                           // (1<<7)?
    ...
    if (tevind.sw == ITW_OFF)
      ...
      out.Write("\twrappedcoord.x = fixpoint_uv%d.x & (%s - 1);\n",
                stage.tevorders_texcoord, tevIndWrapStart[tevind.sw]);
    ...
    if (tevind.tw == ITW_OFF)
      ...
      out.Write("\twrappedcoord.y = fixpoint_uv%d.y & (%s - 1);\n",
                stage.tevorders_texcoord, tevIndWrapStart[tevind.tw]);
  ...
  if (cc.a == TEVCOLORARG_RASA || cc.a == TEVCOLORARG_RASC ||
    ...
    uid_data->stagehash[n].tevorders_colorchan =
        bpmem.tevorders[n / 2].getColorChan(n & 1);

    char rasswap[5] = {
        "rgba"[stage.tevksel_swap1a], "rgba"[stage.tevksel_swap2a],
        "rgba"[stage.tevksel_swap1b], "rgba"[stage.tevksel_swap2b], '\0'};

    out.Write("\trastemp = %s.%s;\n", tevRasTable[stage.tevorders_colorchan],
              rasswap);
  }

  uid_data->stagehash[n].tevorders_enable =
      bpmem.tevorders[n / 2].getEnable(n & 1);
  if (stage.tevorders_enable) {
    ...
    uid_data->stagehash[n].tevorders_texmap =
    ...
    char texswap[5] = {
        "rgba"[stage.tevksel_swap1c], "rgba"[stage.tevksel_swap2c],
        "rgba"[stage.tevksel_swap1d], "rgba"[stage.tevksel_swap2d], '\0'};

    out.Write("\ttextemp = ");
    SampleTexture<T>(out, "float2(tevcoord.xy)", texswap, texmap,
                     uid_data->stereo, ApiType);
  } else {
  ...
  if (cc.d == TEVCOLORARG_C0 || cc.d == TEVCOLORARG_A0 ||
      ac.d == TEVALPHAARG_A0)
    out.SetConstantsUsed(C_COLORS + 1, C_COLORS + 1);

  if (cc.d == TEVCOLORARG_C1 || cc.d == TEVCOLORARG_A1 ||
      ac.d == TEVALPHAARG_A1)
    out.SetConstantsUsed(C_COLORS + 2, C_COLORS + 2);

  if (cc.d == TEVCOLORARG_C2 || cc.d == TEVCOLORARG_A2 ||
      ac.d == TEVALPHAARG_A2)
  ...
  if (cc.bias != TEVBIAS_COMPARE) {
    ...
    const char *function_table[] = {
        ...
        "((tevin_a.r == tevin_b.r) ? tevin_c.rgb : "
        "int3(0,0,0))", // TEVCMP_R8_EQ
        "((idot(tevin_a.rgb, comp16) >  idot(tevin_b.rgb, "
        "comp16)) ? tevin_c.rgb : "
        "int3(0,0,0))", // TEVCMP_GR16_GT
        "((idot(tevin_a.rgb, comp16) == idot(tevin_b.rgb, "
        "comp16)) ? tevin_c.rgb : "
        "int3(0,0,0))", // TEVCMP_GR16_EQ
        "((idot(tevin_a.rgb, comp24) >  idot(tevin_b.rgb, "
        "comp24)) ? tevin_c.rgb : "
        "int3(0,0,0))", // TEVCMP_BGR24_GT
        "((idot(tevin_a.rgb, comp24) == idot(tevin_b.rgb, "
        "comp24)) ? tevin_c.rgb : "
        "int3(0,0,0))", // TEVCMP_BGR24_EQ
        "(max(sign(tevin_a.rgb - tevin_b.rgb), int3(0,0,0)) * "
        "tevin_c.rgb)", // TEVCMP_RGB8_GT
        "((int3(1,1,1) - sign(abs(tevin_a.rgb - tevin_b.rgb))) "
        "* tevin_c.rgb)" // TEVCMP_RGB8_EQ
    };
  ...
  if (ac.bias != TEVBIAS_COMPARE) {
    ...
    const char *function_table[] = {
        ...
        "((idot(tevin_a.rgb, comp16) >  idot(tevin_b.rgb, comp16)) ? tevin_c.a "
        ": 0)", // TEVCMP_GR16_GT
        "((idot(tevin_a.rgb, comp16) == idot(tevin_b.rgb, comp16)) ? tevin_c.a "
        ": 0)", // TEVCMP_GR16_EQ
        "((idot(tevin_a.rgb, comp24) >  idot(tevin_b.rgb, comp24)) ? tevin_c.a "
        ": 0)", // TEVCMP_BGR24_GT
        "((idot(tevin_a.rgb, comp24) == idot(tevin_b.rgb, comp24)) ? tevin_c.a "
        ": 0)",                                       // TEVCMP_BGR24_EQ
...
static void WriteTevRegular(T &out, const char *components, int bias, int op,
  ...
  // - c is scaled from 0..255 to 0..256, which allows dividing the result by
  // 256 instead of 255
  // - if scale is bigger than one, it is moved inside the lerp calculation for
  // increased accuracy
  // - a rounding bias is added before dividing by 256
  out.Write("(((tevin_d.%s%s)%s)", components, tevBiasTable[bias],
            tevScaleTableLeft[shift]);
  out.Write(" %s ", tevOpTable[op]);
  out.Write("(((((tevin_a.%s<<8) + "
            "(tevin_b.%s-tevin_a.%s)*(tevin_c.%s+(tevin_c.%s>>7)))%s)%s)>>8)",
...
static void SampleTexture(T &out, const char *texcoords, const char *texswap,
  ...
  if (ApiType == API_D3D)
    out.Write(
        "iround(255.0 * Tex[%d].Sample(samp[%d], float3(%s.xy * " I_TEXDIMS
        "[%d].xy, %s))).%s;\n",
        texmap, texmap, texcoords, texmap, stereo ? "layer" : "0.0", texswap);
  else
    out.Write("iround(255.0 * texture(samp[%d], float3(%s.xy * " I_TEXDIMS
              "[%d].xy, %s))).%s;\n",
...
static void WriteAlphaTest(T &out, pixel_shader_uid_data *uid_data,
  ...
  out.Write("%s", tevAlphaFunclogicTable[uid_data->alpha_test_logic]); // lookup
                                                                       // the
                                                                       // logic
                                                                       // op
  ...
  // ZCOMPLOC HACK:
  // The only way to emulate alpha test + early-z is to force early-z in the
  // shader.
  // As this isn't available on all drivers and as we can't emulate this feature
  // otherwise,
  // we are only able to choose which one we want to respect more.
  // Tests seem to have proven that writing depth even when the alpha test fails
  // is more
  // important that a reliable alpha test, so we just force the alpha test to
  // always succeed.
...
static void WriteFog(T &out, pixel_shader_uid_data *uid_data) {
  ...
  if (uid_data->fog_proj == 0) {
    // perspective
    // ze = A/(B - (Zs >> B_SHF)
    // TODO: Verify that we want to drop lower bits here! (currently taken over
    // from software
    // renderer)
    //       Maybe we want to use "ze = (A << B_SHF)/((B << B_SHF) - Zs)"
    //       instead?
  ...
  if (uid_data->fog_RangeBaseEnabled) {
    ...
    out.Write("\tfloat x_adjust = (2.0 * (rawpos.x / " I_FOGF
              "[0].y)) - 1.0 - " I_FOGF "[0].x;\n");
    out.Write("\tx_adjust = sqrt(x_adjust * x_adjust + " I_FOGF
  ...
  out.Write("\tprev.rgb = (prev.rgb * (256 - ifog) + " I_FOGCOLOR
            ".rgb * ifog) >> 8;\n");
}
...
ShaderCode GeneratePixelShaderCode(DSTALPHA_MODE dstAlphaMode,
                                   API_TYPE ApiType) {
