...
// FIXME: Some of the video card's capabilities (BBox support, EarlyZ support,
// dstAlpha support)
// leak
//        into this UID; This is really unhelpful if these UIDs ever move from
//        one machine to
//        another.
PixelShaderUid GetPixelShaderUid(DSTALPHA_MODE dstAlphaMode) {
  ...
  for (unsigned int n = 0; n < numStages; n++) {
    ...
    uid_data->stagehash[n].hasindstage =
        bpmem.tevind[n].bt < bpmem.genMode.numindstages;
    ...
    uid_data->stagehash[n].ac =
        ac.hex & 0xFFFFF0; // Storing rswap and tswap later
...
#define MY_STRUCT_OFFSET(str, elem) ((u32)((u64) & (str).elem - (u64) & (str)))
  ...
  if (uid_data->Pretest == AlphaTest::UNDETERMINED ||
    ...
    // shader.
    // As this isn't available on all drivers and as we can't emulate this
    // feature otherwise,
    // we are only able to choose which one we want to respect more.
    // Tests seem to have proven that writing depth even when the alpha test
    // fails is more
...
ShaderCode GeneratePixelShaderCode(DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType,
  ...
  GenerateVSOutputMembers(out, ApiType, uid_data->genMode_numtexgens,
                          uid_data->per_pixel_lighting, "");
  out.Write("};\n");
    ...
    if (g_ActiveConfig.backend_info.bSupportsGeometryShaders) {
      ...
      GenerateVSOutputMembers(out, ApiType, uid_data->genMode_numtexgens,
                              ...
                              GetInterpolationQualifier(
                                  uid_data->msaa, uid_data->ssaa, true, true));
  ...
  for (u32 i = 0; i < uid_data->genMode_numindstages; ++i) {
    if (uid_data->nIndirectStagesUsed & (1 << i)) {
      ...
      SampleTexture(out, "float2(tempcoord)", "abg", texmap, uid_data->stereo,
                    ApiType);
    }
  }
  ...
  if (uid_data->Pretest == AlphaTest::UNDETERMINED ||
      (uid_data->Pretest == AlphaTest::FAIL && uid_data->late_ztest))
    WriteAlphaTest(out, uid_data, ApiType, dstAlphaMode,
                   uid_data->per_pixel_depth);
...
static void WriteStage(ShaderCode &out, const pixel_shader_uid_data *uid_data,
  ...
  if (stage.tevorders_enable) {
    ...
    out.Write("\ttextemp = ");
    SampleTexture(out, "float2(tevcoord.xy)", texswap, stage.tevorders_texmap,
  ...
  if (cc.bias != TEVBIAS_COMPARE) {
    ...
    const char *function_table[] = {
        ...
        "((idot(tevin_a.rgb, comp16) >  idot(tevin_b.rgb, comp16)) ? "
        "tevin_c.rgb : int3(0,0,0))", // TEVCMP_GR16_GT
        "((idot(tevin_a.rgb, comp16) == idot(tevin_b.rgb, comp16)) ? "
        "tevin_c.rgb : int3(0,0,0))", // TEVCMP_GR16_EQ
        "((idot(tevin_a.rgb, comp24) >  idot(tevin_b.rgb, comp24)) ? "
        "tevin_c.rgb : int3(0,0,0))", // TEVCMP_BGR24_GT
        "((idot(tevin_a.rgb, comp24) == idot(tevin_b.rgb, comp24)) ? "
        "tevin_c.rgb : int3(0,0,0))", // TEVCMP_BGR24_EQ
