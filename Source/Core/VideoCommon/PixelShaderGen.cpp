...
static inline void GeneratePixelShader(T& out, DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType, u32 components)
{
	...
	if (DriverDetails::HasBug(DriverDetails::BUG_BROKENIVECSHIFTS))
	{
		...
		//these functions all have the same name to enable them to be used no mater what code is generated
		//example: tev color op code uses .rgb as a swizzle, but alpha code only uses .a
