...
PixelShaderUid GetPixelShaderUid(DSTALPHA_MODE dstAlphaMode)
{
	...
	if (!uid_data->forced_early_z && bpmem.UseEarlyDepthTest() && (!uid_data->fast_depth_calc || bpmem.alpha_test.TestResult() == AlphaTest::UNDETERMINED))
	{
...
ShaderCode GeneratePixelShaderCode(DSTALPHA_MODE dstAlphaMode, API_TYPE ApiType, const pixel_shader_uid_data *uid_data)
{
	...
	if (ApiType == API_OPENGL)
	{
		...
		if (g_ActiveConfig.backend_info.bSupportsGeometryShaders)
		{
			...
			GenerateVSOutputMembers(out, ApiType, uid_data->genMode_numtexgens, uid_data->per_pixel_lighting, GetInterpolationQualifier(uid_data->msaa, uid_data->ssaa, true, true));
...
static void WriteStage(ShaderCode& out, const pixel_shader_uid_data* uid_data, int n, API_TYPE ApiType)
{
	...
	if (stage.tevorders_enable)
	{
		...
		out.Write("\ttextemp = ");
		SampleTexture(out, "float2(tevcoord.xy)", texswap, stage.tevorders_texmap, uid_data->stereo, ApiType);
	}
	else
	{
