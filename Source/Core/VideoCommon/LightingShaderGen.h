...
static void GenerateLightShader(ShaderCode &object,
                                const LightingUidData &uid_data, int index,
  ...
  case LIGHTATTN_SPEC:
    object.Write("ldir = normalize(" LIGHT_POS ".xyz - pos.xyz);\n",
                 LIGHT_POS_PARAMS(index));
    object.Write(
        "attn = (dot(_norm0, ldir) >= 0.0) ? max(0.0, dot(_norm0, " LIGHT_DIR
        ".xyz)) : 0.0;\n",
    ...
    object.Write("attn = max(0.0f, dot(cosAttn, float3(1.0, attn, attn*attn))) "
                 "/ dot(distAttn, "
  ...
  case LIGHTATTN_SPOT:
    ...
    object.Write("attn = max(0.0, " LIGHT_COSATT ".x + " LIGHT_COSATT
                 ".y*attn + " LIGHT_COSATT ".z*attn*attn) / dot(" LIGHT_DISTATT
                 ".xyz, float3(1.0,dist,dist2));\n",
  ...
  case LIGHTDIF_NONE:
    object.Write("lacc.%s += int%s(round(attn * float%s(" LIGHT_COL ")));\n",
                 swizzle, swizzle_components, swizzle_components,
                 LIGHT_COL_PARAMS(index, swizzle));
    break;
  case LIGHTDIF_SIGN:
  case LIGHTDIF_CLAMP:
    object.Write("lacc.%s += int%s(round(attn * %sdot(ldir, _norm0)) * "
                 "float%s(" LIGHT_COL ")));\n",
...
static void GenerateLightingShaderCode(ShaderCode &object,
                                       const LightingUidData &uid_data,
                                       int components, const char *inColorName,
                                       const char *dest) {
    ...
    if (uid_data.enablelighting & (1 << j)) {
      if (uid_data.ambsource & (1 << j)) // from vertex
      {
        if (components & (VB_HAS_COL0 << j))
          ...
          // TODO: this isn't verified. Here we want to read the ambient from
          // the vertex,
          // but the vertex itself has no color. So we don't know which value to
          // read.
          // Returning 1.0 is the same as disabled lightning, so this could be
          // fine
    ...
    if (uid_data.enablelighting & (1 << (j + 2))) {
      if (uid_data.ambsource & (1 << (j + 2))) // from vertex
      {
        if (components & (VB_HAS_COL0 << j))
          ...
          // TODO: The same for alpha: We want to read from vertex, but the
          // vertex has no color
