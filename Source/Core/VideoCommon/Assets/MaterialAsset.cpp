    ...
    const picojson::array& values_data, std::vector<MaterialProperty>* material_property)
{
  for (const auto& value_data : values_data)
  {
    ...
    {
      ERROR_LOG_FMT(
          VIDEO, "Asset '{}' failed to parse the json, value is not the right json type", asset_id);
      return false;
    }
    ...
    {
      ERROR_LOG_FMT(
          VIDEO, "Asset '{}' failed to parse the json, value entry 'type' not found", asset_id);
      return false;
    }
...
bool MaterialData::FromJson(
    const CustomAssetLibrary::AssetID& asset_id, const picojson::object& json, MaterialData* data)
{
  ...
  {
    ERROR_LOG_FMT(
        VIDEO, "Asset '{}' failed to parse json, 'shader_asset' not found or wrong type", asset_id);
    return false;
  }
  ...
  {
    ERROR_LOG_FMT(
        VIDEO, "Asset '{}' failed to parse json, 'textures' is not the right json type", asset_id);
    return false;
  }
  ...
  {
    ERROR_LOG_FMT(
        VIDEO, "Asset '{}' failed to parse json, 'textures' must contain objects", asset_id);
    return false;
  }
...
void MaterialData::ToJson(picojson::object* obj, const MaterialData& data)
{
  ...
  if (data.depth_state)
  {
    ...
    depth_state_json.emplace(
        "test_enable", static_cast<double>(data.depth_state->test_enable.Value()));
    depth_state_json.emplace(
        "update_enable", static_cast<double>(data.depth_state->update_enable.Value()));
  ...
  if (data.blending_state)
  {
    ...
    blending_state_json.emplace(
        "blend_enable", static_cast<double>(data.blending_state->blend_enable.Value()));
    blending_state_json.emplace(
        "logic_op_enable", static_cast<double>(data.blending_state->logic_op_enable.Value()));
    blending_state_json.emplace(
        "color_update", static_cast<double>(data.blending_state->color_update.Value()));
    blending_state_json.emplace(
        "alpha_update", static_cast<double>(data.blending_state->alpha_update.Value()));
    blending_state_json.emplace(
        "subtract", static_cast<double>(data.blending_state->subtract.Value()));
    blending_state_json.emplace(
        "subtract_alpha", static_cast<double>(data.blending_state->subtract_alpha.Value()));
    blending_state_json.emplace(
        "use_dual_src", static_cast<double>(data.blending_state->use_dual_src.Value()));
    blending_state_json.emplace(
        "dst_factor", static_cast<double>(data.blending_state->dst_factor.Value()));
    blending_state_json.emplace(
        "src_factor", static_cast<double>(data.blending_state->src_factor.Value()));
    blending_state_json.emplace(
        "dst_factor_alpha", static_cast<double>(data.blending_state->dst_factor_alpha.Value()));
    blending_state_json.emplace(
        "src_factor_alpha", static_cast<double>(data.blending_state->src_factor_alpha.Value()));
    blending_state_json.emplace(
        "logic_mode", static_cast<double>(data.blending_state->logic_mode.Value()));
