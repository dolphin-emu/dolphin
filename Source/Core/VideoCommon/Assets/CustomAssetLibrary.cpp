                                                                 ...
                                                                 TextureData* data)
{
  ...
  {
    auto& [m_levels] = data->m_texture.m_slices[slice_index];
    const auto& first_mip = m_levels[0];

    // Verify that each mip level is the correct size (divide by 2 each time).
    u32 current_mip_width = first_mip.width;
    u32 current_mip_height = first_mip.height;
    for (u32 mip_level = 1; mip_level < static_cast<u32>(m_levels.size()); mip_level++)
    {
      if (current_mip_width != 1 || current_mip_height != 1)
      {
        ...
        const auto& [_data, _format, width, height, _row_length] = m_levels[mip_level];
        if (current_mip_width == width && current_mip_height == height)
          continue;
        ...
        ERROR_LOG_FMT(VIDEO,
                      ...
                      "mipmap level {} "
                      "must be {}x{}.",
                      width, height, asset_id, slice_index, mip_level,
