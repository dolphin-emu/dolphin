                                ...
                                const picojson::object& json, RasterShaderData* data)
{
  ...
  if (!parse_properties("properties", data->m_pixel_source, &data->m_uniform_properties))
    return false;

  if (!parse_samplers("samplers", &data->m_samplers))
    return false;
...
void RasterShaderData::ToJson(picojson::object& obj, const RasterShaderData& data)
{
  ...
  picojson::array json_properties;
  for (const auto& [name, property] : data.m_uniform_properties)
  {
