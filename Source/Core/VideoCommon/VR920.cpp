...
bool VR920_StartStereo3D()
{
  ...
  if (!iwrstdrv_lib)
  {
    ...
    if (iwrstdrv_lib)
    {
      ...
      Vuzix_WaitForStereoAck =
          (PVUZIX_BYTEHANDLE)GetProcAddress(iwrstdrv_lib, "IWRSTEREO_WaitForAck");
      Vuzix_CloseStereo = (PVUZIX_CLOSEHANDLE)GetProcAddress(iwrstdrv_lib, "IWRSTEREO_Close");
      Vuzix_GetStereoVersion =
          (PVUZIX_GETVERSION)GetProcAddress(iwrstdrv_lib, "IWRSTEREO_GetVersion");
    }
    else
    {
      ...
      WARN_LOG(
          VR,
          "Vuzix VR920 stereoscopic 3D driver not installed. VR920 support will not be available.");
      return false;
    }
  }
  if (Vuzix_GetStereoVersion)
  {
    ...
    PanicAlertT("Vuzix 3D DLL %d.%d.%d.%d, tracker: %d.%d,\n usb: %d.%d, video: %d",
                g_vuzix_3dversion.dll_v1, g_vuzix_3dversion.dll_v2, g_vuzix_3dversion.dll_v3,
                g_vuzix_3dversion.dll_v4, g_vuzix_3dversion.tracker_firmware_major,
                g_vuzix_3dversion.tracker_firmware_minor, g_vuzix_3dversion.usb_firmware_major,
                g_vuzix_3dversion.usb_firmware_minor, g_vuzix_3dversion.video_firmware);
  }
...
void LoadVR920()
{
  ...
  if (!iwrdrv_lib)
  {
    ...
    else
    {
      ...
      WARN_LOG(
          VR,
          "Vuzix VR920 tracker driver is not installed, so Dolphin can't support the Vuzix VR920.");
    }
  }
  if ((!g_has_vr920) && Vuzix_OpenTracker && Vuzix_OpenTracker() == ERROR_SUCCESS)
  {
    ...
    if (Vuzix_GetVersion)
    {
      ...
      if (g_vuzix_productid <= 227 &&
          g_vuzix_version.usb_firmware_major * 100 + g_vuzix_version.usb_firmware_minor < 109)
      {
        PanicAlertT("Your Icuiti/Vuzix VR920 firmware (1.%d%02d.%d%02d:%d) is too old to support "
                    "freezeframe 3D. Go "
                    "to:\nhttp://www.vuzix.com/support/Downloads_drivers_archive/\nand download "
                    "and install the latest (1.110.104.13) firmware.",
      ...
      if (g_vuzix_productid <= 227 &&
          g_vuzix_version.usb_firmware_major * 100 + g_vuzix_version.usb_firmware_minor < 110)
      {
        WARN_LOG(VR, "Your Icuiti/Vuzix VR920 firmware (1.%d%02d.%d%02d:%d) is out of date. Go "
                     "to:\nhttp://www.vuzix.com/support/Downloads_drivers_archive/\nand download "
                     "and install the latest (1.110.104.13) firmware.",
                 g_vuzix_version.usb_firmware_major, g_vuzix_version.usb_firmware_minor,
                 g_vuzix_version.tracker_firmware_major, g_vuzix_version.tracker_firmware_minor,
