...
struct Message {
  ...
  Message(const std::string &s, u32 ts, u32 rgba)
      : m_str(s), m_timestamp(ts), m_rgba(rgba) {}
...
void AddMessage(const std::string &message, u32 ms = Duration::SHORT,
                u32 rgba = Color::YELLOW);
void AddTypedMessage(MessageType type, const std::string &message,
                     u32 ms = Duration::SHORT, u32 rgba = Color::YELLOW);
void DrawMessage(const Message &msg, int top, int left,
                 int time_left); // draw one message
void DrawMessages(); // draw the current messages on the screen. Only call once
                     // per frame.
void ClearMessages();

// On-screen callbacks
enum class CallbackType { Initialization, OnFrame, Shutdown };
