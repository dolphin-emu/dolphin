...
alignas(16) static const float scale_factors[] = {
    1.0 / (1ULL << 0),  1.0 / (1ULL << 1),  1.0 / (1ULL << 2),
    1.0 / (1ULL << 3),  1.0 / (1ULL << 4),  1.0 / (1ULL << 5),
    1.0 / (1ULL << 6),  1.0 / (1ULL << 7),  1.0 / (1ULL << 8),
    1.0 / (1ULL << 9),  1.0 / (1ULL << 10), 1.0 / (1ULL << 11),
    1.0 / (1ULL << 12), 1.0 / (1ULL << 13), 1.0 / (1ULL << 14),
    1.0 / (1ULL << 15), 1.0 / (1ULL << 16), 1.0 / (1ULL << 17),
    1.0 / (1ULL << 18), 1.0 / (1ULL << 19), 1.0 / (1ULL << 20),
    1.0 / (1ULL << 21), 1.0 / (1ULL << 22), 1.0 / (1ULL << 23),
    1.0 / (1ULL << 24), 1.0 / (1ULL << 25), 1.0 / (1ULL << 26),
    1.0 / (1ULL << 27), 1.0 / (1ULL << 28), 1.0 / (1ULL << 29),
    1.0 / (1ULL << 30), 1.0 / (1ULL << 31),
};

VertexLoaderARM64::VertexLoaderARM64(const TVtxDesc &vtx_desc,
                                     const VAT &vtx_att)
  ...
  if (attribute & MASK_INDEXED) {
    ...
    if (array == ARRAY_POSITION) {
      EOR(scratch2_reg, scratch1_reg, 0,
          attribute == INDEX8 ? 7 : 15); // 0xFF : 0xFFFF
    ...
    ADD(EncodeRegTo64(reg), EncodeRegTo64(scratch1_reg),
        EncodeRegTo64(scratch2_reg));
  } else
    ADD(reg, src_reg, m_src_ofs);
}

s32 VertexLoaderARM64::GetAddressImm(int array, u64 attribute,
...
int VertexLoaderARM64::ReadVertex(u64 attribute, int format, int count_in,
                                  int count_out, bool dequantize,
                                  u8 scaling_exponent,
  ...
  int load_size = load_bytes == 1 ? 1 : load_bytes <= 2
                                            ? 2
                                            : load_bytes <= 4
                                                  ? 4
                                                  : load_bytes <= 8 ? 8 : 16;
  ...
  if (offset == -1) {
    if (count_in == 1)
      m_float_emit.LDR(elem_size, INDEX_UNSIGNED, coords,
                       EncodeRegTo64(scratch1_reg), 0);
    else
  ...
  if (format != FORMAT_FLOAT) {
    ...
    switch (format) {
    case FORMAT_UBYTE:
      m_float_emit.UXTL(8, EncodeRegToDouble(coords),
                        EncodeRegToDouble(coords));
      m_float_emit.UXTL(16, EncodeRegToDouble(coords),
                        EncodeRegToDouble(coords));
      break;
    case FORMAT_BYTE:
      m_float_emit.SXTL(8, EncodeRegToDouble(coords),
                        EncodeRegToDouble(coords));
      m_float_emit.SXTL(16, EncodeRegToDouble(coords),
                        EncodeRegToDouble(coords));
      break;
    case FORMAT_USHORT:
      m_float_emit.REV16(8, EncodeRegToDouble(coords),
                         EncodeRegToDouble(coords));
      m_float_emit.UXTL(16, EncodeRegToDouble(coords),
                        EncodeRegToDouble(coords));
      break;
    ...
    if (dequantize && scaling_exponent) {
      m_float_emit.LDR(32, INDEX_UNSIGNED, scale, scale_reg,
                       scaling_exponent * 4);
  ...
  if (native_format == &m_native_vtx_decl.position) {
    ...
    MOVI2R(EncodeRegTo64(scratch2_reg),
           (u64)VertexLoaderManager::position_cache);
    ADD(EncodeRegTo64(scratch1_reg), EncodeRegTo64(scratch2_reg),
        EncodeRegTo64(count_reg),
...
void VertexLoaderARM64::ReadColor(u64 attribute, int format, s32 offset) {
  ...
  case FORMAT_16B_565:
    ...
    ORR(scratch2_reg, scratch2_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSR, 5));
    ...
    ORR(scratch2_reg, scratch2_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSR, 6));
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSL, 8));

    // R
    UBFM(scratch2_reg, scratch3_reg, 11, 15);
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSL, 3));
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSR, 2));
  ...
  case FORMAT_16B_4444:
    ...
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSL, 16));

    // A
    UBFM(scratch2_reg, scratch3_reg, 8, 11);
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSL, 24));

    // Final duplication
    ORR(scratch1_reg, scratch1_reg, scratch1_reg,
        ArithOption(scratch1_reg, ST_LSL, 4));
  ...
  case FORMAT_24B_6666:
    ...
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSL, 2));
    ORR(scratch1_reg, scratch1_reg, scratch2_reg,
        ArithOption(scratch2_reg, ST_LSR, 4));
...
void VertexLoaderARM64::GenerateVertexLoader() {
  ...
  // If we need to use those, we need to retain the lower 64bits(!) of the
  // register

  const u64 tc[8] = {
      m_VtxDesc.Tex0Coord, m_VtxDesc.Tex1Coord, m_VtxDesc.Tex2Coord,
  ...
  bool need_scale = (m_VtxAttr.ByteDequant && m_VtxAttr.PosFrac) ||
                    (has_tc && has_tc_scale) || m_VtxDesc.Normal;
  ...
  u32 texmatidx_ofs[8];
  const u64 tm[8] = {
      m_VtxDesc.Tex0MatIdx, m_VtxDesc.Tex1MatIdx, m_VtxDesc.Tex2MatIdx,
      m_VtxDesc.Tex3MatIdx, m_VtxDesc.Tex4MatIdx, m_VtxDesc.Tex5MatIdx,
      m_VtxDesc.Tex6MatIdx, m_VtxDesc.Tex7MatIdx,
  };
  for (int i = 0; i < 8; i++) {
    if (tm[i])
  ...
  // Position
  {
    ...
    s32 offset = GetAddressImm(ARRAY_POSITION, m_VtxDesc.Position,
                               EncodeRegTo64(scratch1_reg), load_size);
    int pos_elements = m_VtxAttr.PosElements + 2;
    ReadVertex(m_VtxDesc.Position, m_VtxAttr.PosFormat, pos_elements,
               pos_elements, m_VtxAttr.ByteDequant, m_VtxAttr.PosFrac,
               &m_native_vtx_decl.position, offset);
  }
  ...
  if (m_VtxDesc.Normal) {
    ...
    for (int i = 0; i < (m_VtxAttr.NormalElements ? 3 : 1); i++) {
      if (!i || m_VtxAttr.NormalIndex3) {
        ...
        int load_size =
            load_bytes == 1 ? 1 : load_bytes <= 2 ? 2 : load_bytes <= 4
                                                            ? 4
                                                            : load_bytes <= 8
                                                                  ? 8
                                                                  : 16;

        offset = GetAddressImm(ARRAY_NORMAL, m_VtxDesc.Normal,
      ...
      int bytes_read =
          ReadVertex(m_VtxDesc.Normal, m_VtxAttr.NormalFormat, 3, 3, true,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (col[i]) {
      ...
      s32 offset = GetAddressImm(ARRAY_COLOR + i, col[i],
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (tc[i]) {
      ...
      s32 offset = GetAddressImm(ARRAY_TEXCOORD0 + i, tc[i],
                                 EncodeRegTo64(scratch1_reg), load_size);
      u8 scaling_exponent = m_VtxAttr.texCoord[i].Frac;
      ReadVertex(tc[i], m_VtxAttr.texCoord[i].Format, elements,
                 tm[i] ? 2 : elements, m_VtxAttr.ByteDequant, scaling_exponent,
    ...
    if (tm[i]) {
      ...
      if (tc[i]) {
        ...
        if (m_dst_ofs < 256) {
          ...
          // If m_dst_ofs isn't 8byte aligned we can't store an 8byte zero
          // register
...
int VertexLoaderARM64::RunVertices(DataReader src, DataReader dst, int count) {
  ...
  return ((int (*)(u8 * src, u8 * dst, int count))region)(
      src.GetPointer(), dst.GetPointer(), count);
}
