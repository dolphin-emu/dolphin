...
static u32 Read24(const u8 *addr) { return Read32(addr) | AMASK; }
...
template <typename I> void Color_ReadIndex_16b_565(VertexLoader *loader) {
...
template <typename I> void Color_ReadIndex_24b_888(VertexLoader *loader) {
  auto const Index = DataRead<I>();
  const u8 *iAddress =
      VertexLoaderManager::cached_arraybases[ARRAY_COLOR + loader->m_colIndex] +
      (Index * g_main_cp_state.array_strides[ARRAY_COLOR + loader->m_colIndex]);
...
template <typename I> void Color_ReadIndex_32b_888x(VertexLoader *loader) {
  auto const Index = DataRead<I>();
  const u8 *iAddress =
      VertexLoaderManager::cached_arraybases[ARRAY_COLOR + loader->m_colIndex] +
      (Index * g_main_cp_state.array_strides[ARRAY_COLOR + loader->m_colIndex]);
...
template <typename I> void Color_ReadIndex_16b_4444(VertexLoader *loader) {
  auto const Index = DataRead<I>();
  const u8 *const address =
      VertexLoaderManager::cached_arraybases[ARRAY_COLOR + loader->m_colIndex] +
      (Index * g_main_cp_state.array_strides[ARRAY_COLOR + loader->m_colIndex]);
...
template <typename I> void Color_ReadIndex_24b_6666(VertexLoader *loader) {
  auto const Index = DataRead<I>();
  const u8 *pData =
      VertexLoaderManager::cached_arraybases[ARRAY_COLOR + loader->m_colIndex] +
      (Index *
       g_main_cp_state.array_strides[ARRAY_COLOR + loader->m_colIndex]) -
      1;
  u32 val = Common::swap32(pData);
  SetCol6666(loader, val);
}

template <typename I> void Color_ReadIndex_32b_8888(VertexLoader *loader) {
  auto const Index = DataRead<I>();
  const u8 *iAddress =
      VertexLoaderManager::cached_arraybases[ARRAY_COLOR + loader->m_colIndex] +
      (Index * g_main_cp_state.array_strides[ARRAY_COLOR + loader->m_colIndex]);
