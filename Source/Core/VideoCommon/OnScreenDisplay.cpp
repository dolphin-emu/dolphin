...
void AddTypedMessage(MessageType type, const std::string &message, u32 ms, u32 rgba)
{
	...
	s_msg_list.insert(std::pair<MessageType, Message>(type, Message(s, Common::Timer::GetTimeMs() + ms, rgba)));
}

void AddMessage(const std::string& message, u32 ms, u32 rgba)
{
	std::string s = message;
	s_msg_list.insert(std::pair<MessageType, Message>(MessageType::Typeless, Message(s, Common::Timer::GetTimeMs() + ms, rgba)));
}
