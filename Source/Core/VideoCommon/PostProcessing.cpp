...
void PostProcessingConfiguration::LoadOptionsConfiguration()
{
  ...
  for (auto& option : m_options | std::views::values)
  {
    switch (option.m_type)
    {
    case ConfigurationOption::OptionType::Bool:
      ini.GetOrCreateSection(section)->Get(option.m_option_name, &option.m_bool_value,
                                           option.m_bool_value);
      break;
    case ConfigurationOption::OptionType::Integer:
    {
      std::string value;
      ini.GetOrCreateSection(section)->Get(option.m_option_name, &value);
      if (!value.empty())
      {
        auto integer_values = option.m_integer_values;
        if (TryParseVector(value, &integer_values))
        {
          option.m_integer_values = integer_values;
        }
      }
    }
    break;
    case ConfigurationOption::OptionType::Float:
    {
      ...
      if (!value.empty())
      {
        auto float_values = option.m_float_values;
        if (TryParseVector(value, &float_values))
        {
          option.m_float_values = float_values;
        }
      }
    }
    break;
...
void PostProcessingConfiguration::SaveOptionsConfiguration()
{
  ...
  for (auto& option : m_options | std::views::values)
  {
    ...
    case ConfigurationOption::OptionType::Bool:
    {
      ini.GetOrCreateSection(section)->Set(option.m_option_name, option.m_bool_value);
    }
    break;
    case ConfigurationOption::OptionType::Integer:
    {
      std::string value;
      for (size_t i = 0; i < option.m_integer_values.size(); ++i)
      {
        value += fmt::format("{}{}", option.m_integer_values[i],
                             i == (option.m_integer_values.size() - 1) ? "" : ", ");
      }
      ini.GetOrCreateSection(section)->Set(option.m_option_name, value);
    }
    break;
    case ConfigurationOption::OptionType::Float:
    {
      ...
      for (size_t i = 0; i < option.m_float_values.size(); ++i)
      {
        value << option.m_float_values[i];
        if (i != (option.m_float_values.size() - 1))
          value << ", ";
      }
      ini.GetOrCreateSection(section)->Set(option.m_option_name, value.str());
    }
    break;
    }
  }
                                       ...
                                       bool user_post_process, bool intermediary_buffer)
{
  ...
  for (auto& option : m_config.GetOptions() | std::views::values)
  {
    union
    {
      u32 as_bool[4];
    ...
    case PostProcessingConfiguration::ConfigurationOption::OptionType::Bool:
      value.as_bool[0] = option.m_bool_value ? 1 : 0;
      break;

    case PostProcessingConfiguration::ConfigurationOption::OptionType::Integer:
      ASSERT(option.m_integer_values.size() <= 4);
      std::copy_n(option.m_integer_values.begin(), option.m_integer_values.size(), value.as_int);
      break;

    case PostProcessingConfiguration::ConfigurationOption::OptionType::Float:
      ASSERT(option.m_float_values.size() <= 4);
      std::copy_n(option.m_float_values.begin(), option.m_float_values.size(), value.as_float);
      break;
    }
