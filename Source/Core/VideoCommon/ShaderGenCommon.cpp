...
ShaderHostConfig ShaderHostConfig::GetCurrent(const BackendInfo& backend_info)
{
  ShaderHostConfig bits = {};
  bits.msaa = g_ActiveConfig.iMultisamples > 1;
  bits.ssaa =
      g_ActiveConfig.iMultisamples > 1 && g_ActiveConfig.bSSAA && backend_info.bSupportsSSAA;
  ...
  bits.backend_bitfield = backend_info.bSupportsBitfield;
  bits.backend_dynamic_sampler_indexing = backend_info.bSupportsDynamicSamplerIndexing;
  bits.backend_shader_framebuffer_fetch = backend_info.bSupportsFramebufferFetch;
  bits.backend_logic_op = backend_info.bSupportsLogicOp;
  bits.backend_palette_conversion = backend_info.bSupportsPaletteConversion;
  ...
  bits.backend_sampler_lod_bias = backend_info.bSupportsLodBiasInSampler;
  bits.backend_dynamic_vertex_loader = backend_info.bSupportsDynamicVertexLoader;
  bits.backend_vs_point_line_expand = g_ActiveConfig.UseVSForLinePointExpand(backend_info);
  return bits;
}
                                       ...
                                       bool include_api)
{
  ...
  if (include_host_config)
  {
    ...
    ShaderHostConfig host_config = ShaderHostConfig::GetCurrent(backend_info);
