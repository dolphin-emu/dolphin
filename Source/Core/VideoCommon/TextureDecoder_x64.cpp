...
static inline void DecodeBytes_C4_IA8(u32 *dst, const u8 *src,
                                      const u8 *tlut_) {
...
static inline void DecodeBytes_C4_RGB565(u32 *dst, const u8 *src,
                                         const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
    u8 val = src[x];
...
static inline void DecodeBytes_C4_RGB5A3(u32 *dst, const u8 *src,
                                         const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
    u8 val = src[x];
...
static inline void DecodeBytes_C8_IA8(u32 *dst, const u8 *src,
                                      const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 8; x++) {
...
static inline void DecodeBytes_C8_RGB565(u32 *dst, const u8 *src,
                                         const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 8; x++) {
    u8 val = src[x];
...
static inline void DecodeBytes_C8_RGB5A3(u32 *dst, const u8 *src,
                                         const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 8; x++) {
    u8 val = src[x];
...
static inline void DecodeBytes_C14X2_IA8(u32 *dst, const u16 *src,
                                         const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
...
static inline void DecodeBytes_C14X2_RGB565(u32 *dst, const u16 *src,
                                            const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
    u16 val = Common::swap16(src[x]);
...
static inline void DecodeBytes_C14X2_RGB5A3(u32 *dst, const u16 *src,
                                            const u8 *tlut_) {
  const u16 *tlut = (u16 *)tlut_;
  for (int x = 0; x < 4; x++) {
    u16 val = Common::swap16(src[x]);
...
static void DecodeDXTBlock(u32 *dst, const DXTBlock *src, int pitch) {
  // S3TC Decoder (Note: GCN decodes differently from PC so we can't use native
  // support)
  // Needs more speed.
  ...
  if (c1 > c2) {
    ...
    colors[2] =
        MakeRGBA((red1 + red2 + 1) / 2, // Average
...
// JSD 01/06/11:
// TODO: we really should ensure BOTH the source and destination addresses are
// aligned to 16-byte
// boundaries to
// squeeze out a little more performance. _mm_loadu_si128/_mm_storeu_si128 is
// slower than
// _mm_load_si128/_mm_store_si128
// because they work on unaligned addresses. The processor is free to make the
// assumption that
// addresses are multiples
// of 16 in the aligned case.
// TODO: complete SSE2 optimization of less often used texture formats.
// TODO: refactor algorithms using _mm_loadl_epi64 unaligned loads to prefer
// 128-bit aligned loads.

void _TexDecoder_DecodeImpl(u32 *dst, const u8 *src, int width, int height,
      ...
      for (int y = 0; y < height; y += 8)
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 8 * yStep; iy < 8; iy++, xStep++)
            DecodeBytes_C4_RGB5A3(dst + (y + iy) * width + x, src + 4 * xStep,
                                  tlut);
    } else if (tlutfmt == GX_TL_IA8) {
      for (int y = 0; y < height; y += 8)
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 8 * yStep; iy < 8; iy++, xStep++)
            DecodeBytes_C4_IA8(dst + (y + iy) * width + x, src + 4 * xStep,
                               tlut);
    } else if (tlutfmt == GX_TL_RGB565) {
      for (int y = 0; y < height; y += 8)
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 8 * yStep; iy < 8; iy++, xStep++)
            DecodeBytes_C4_RGB565(dst + (y + iy) * width + x, src + 4 * xStep,
                                  tlut);
    }
    break;
  case GX_TF_I4: {
    ...
    if (cpu_info.bSSSE3) {
      const __m128i mask9180 =
          _mm_set_epi8(9, 9, 9, 9, 1, 1, 1, 1, 8, 8, 8, 8, 0, 0, 0, 0);
      const __m128i maskB3A2 =
          _mm_set_epi8(11, 11, 11, 11, 3, 3, 3, 3, 10, 10, 10, 10, 2, 2, 2, 2);
      const __m128i maskD5C4 =
          _mm_set_epi8(13, 13, 13, 13, 5, 5, 5, 5, 12, 12, 12, 12, 4, 4, 4, 4);
      const __m128i maskF7E6 =
          _mm_set_epi8(15, 15, 15, 15, 7, 7, 7, 7, 14, 14, 14, 14, 6, 6, 6, 6);
      for (int y = 0; y < height; y += 8)
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 8; iy += 2, xStep++) {
            const __m128i r0 =
                _mm_loadl_epi64((const __m128i *)(src + 8 * xStep));
            // We want the hi 4 bits of each 8-bit word replicated to 32-bit
            // words:
            // (00000000 00000000 HhGgFfEe DdCcBbAa) -> (00000000 00000000
            // HHGGFFEE DDCCBBAA)
            ...
            _mm_storeu_si128((__m128i *)(dst + (y + iy + 1) * width + x + 4),
                             o4);
          }
    } else
#endif
      ...
      for (int y = 0; y < height; y += 8)
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 8; iy += 2, xStep++) {
            const __m128i r0 =
                _mm_loadl_epi64((const __m128i *)(src + 8 * xStep));
            // Shuffle low 64-bits with itself to expand from (0000 0000 hgfe
            // dcba) to (hhgg ffee
            // ddcc bbaa)
            ...
            // We want the hi 4 bits of each 8-bit word replicated to 32-bit
            // words:
            // (HhHhGgGg FfFfEeEe DdDdCcCc BbBbAaAa) & kMask_xf0 -> (H0H0G0G0
            // F0F0E0E0 D0D0C0C0
            // B0B0A0A0)
            ...
            // Shuffle low 64-bits with itself to expand from (HHHHGGGG FFFFEEEE
            // DDDDCCCC BBBBAAAA)
            // to (DDDDDDDD CCCCCCCC BBBBBBBB AAAAAAAA)
            const __m128i i15 = _mm_unpacklo_epi8(i11, i11);
            // (DDDDDDDD CCCCCCCC BBBBBBBB AAAAAAAA) -> (BBBBBBBB BBBBBBBB
            // AAAAAAAA AAAAAAAA)
            const __m128i i151 = _mm_unpacklo_epi8(i15, i15);
            // (DDDDDDDD CCCCCCCC BBBBBBBB AAAAAAAA) -> (DDDDDDDD DDDDDDDD
            // CCCCCCCC CCCCCCCC)
            const __m128i i152 = _mm_unpackhi_epi8(i15, i15);

            // Shuffle hi  64-bits with itself to expand from (HHHHGGGG FFFFEEEE
            // DDDDCCCC BBBBAAAA)
            // to (HHHHHHHH GGGGGGGG FFFFFFFF EEEEEEEE)
            const __m128i i16 = _mm_unpackhi_epi8(i11, i11);
            // (HHHHHHHH GGGGGGGG FFFFFFFF EEEEEEEE) -> (FFFFFFFF FFFFFFFF
            // EEEEEEEE EEEEEEEE)
            const __m128i i161 = _mm_unpacklo_epi8(i16, i16);
            // (HHHHHHHH GGGGGGGG FFFFFFFF EEEEEEEE) -> (HHHHHHHH HHHHHHHH
            // GGGGGGGG GGGGGGGG)
            ...
            // _mm_and_si128(i151, kMask_x00000000ffffffff) takes i151 and masks
            // off 1st and 3rd
            // 32-bit words
            // (BBBBBBBB BBBBBBBB AAAAAAAA AAAAAAAA) -> (00000000 BBBBBBBB
            // 00000000 AAAAAAAA)
            // _mm_and_si128(i251, kMask_xffffffff00000000) takes i251 and masks
            // off 2nd and 4th
            // 32-bit words
            // (bbbbbbbb bbbbbbbb aaaaaaaa aaaaaaaa) -> (bbbbbbbb 00000000
            // aaaaaaaa 00000000)
            // And last but not least, _mm_or_si128 ORs those two together,
            // giving us the
            // interleaving we desire:
            // (00000000 BBBBBBBB 00000000 AAAAAAAA) | (bbbbbbbb 00000000
            // aaaaaaaa 00000000) ->
            // (bbbbbbbb BBBBBBBB aaaaaaaa AAAAAAAA)
            const __m128i kMask_x00000000ffffffff = _mm_set_epi32(
                0x00000000L, 0xffffffffL, 0x00000000L, 0xffffffffL);
            const __m128i kMask_xffffffff00000000 = _mm_set_epi32(
                0xffffffffL, 0x00000000L, 0xffffffffL, 0x00000000L);
            const __m128i o1 =
                _mm_or_si128(_mm_and_si128(i151, kMask_x00000000ffffffff),
                             _mm_and_si128(i251, kMask_xffffffff00000000));
            const __m128i o2 =
                _mm_or_si128(_mm_and_si128(i152, kMask_x00000000ffffffff),
                             _mm_and_si128(i252, kMask_xffffffff00000000));

            // These two are for the next row; same pattern as above. We batched
            // up two rows because
            // our input was 64 bits.
            const __m128i o3 =
                _mm_or_si128(_mm_and_si128(i161, kMask_x00000000ffffffff),
                             _mm_and_si128(i261, kMask_xffffffff00000000));
            const __m128i o4 =
                _mm_or_si128(_mm_and_si128(i162, kMask_x00000000ffffffff),
      ...
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; ++iy, xStep++) {
            const __m128i mask3210 =
                _mm_set_epi8(3, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 0, 0, 0, 0);

            const __m128i mask7654 =
                _mm_set_epi8(7, 7, 7, 7, 6, 6, 6, 6, 5, 5, 5, 5, 4, 4, 4, 4);
            __m128i *quaddst, r, rgba0, rgba1;
            // Load 64 bits from `src` into an __m128i with upper 64 bits
            // zeroed: (0000 0000 hgfe
            // dcba)
    ...
    // Produces an ~86% speed improvement over reference C implementation.
    {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++) {
          ...
          // TODO: is it more efficient to group the loads together sequentially
          // and also the stores
          // at the end?
          // _mm_stream instead of _mm_store on my AMD Phenom II x410 made
          // performance significantly
          // WORSE, so I
          // went with _mm_stores. Perhaps there is some edge case here creating
          // the terrible
          // performance or we're
          // not aligned to 16-byte boundaries. I don't know.
          __m128i *quaddst;

          // Load 64 bits from `src` into an __m128i with upper 64 bits zeroed:
          // (0000 0000 hgfe
          // dcba)
          const __m128i r0 = _mm_loadl_epi64((const __m128i *)src2);
          // Shuffle low 64-bits with itself to expand from (0000 0000 hgfe
          // dcba) to (hhgg ffee ddcc
          // bbaa)
          const __m128i r1 = _mm_unpacklo_epi8(r0, r0);

          // Shuffle low 64-bits with itself to expand from (hhgg ffee ddcc
          // bbaa) to (dddd cccc bbbb
          // aaaa)
          const __m128i rgba0 = _mm_unpacklo_epi8(r1, r1);
          // Shuffle hi 64-bits with itself to expand from (hhgg ffee ddcc bbaa)
          // to (hhhh gggg ffff
          // eeee)
  ...
  case GX_TF_C8:
    if (tlutfmt == GX_TL_RGB5A3) {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++)
            DecodeBytes_C8_RGB5A3((u32 *)dst + (y + iy) * width + x,
                                  src + 8 * xStep, tlut);
    } else if (tlutfmt == GX_TL_IA8) {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++)
            DecodeBytes_C8_IA8(dst + (y + iy) * width + x, src + 8 * xStep,
                               tlut);
    } else if (tlutfmt == GX_TL_RGB565) {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps8; x < width; x += 8, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++)
            DecodeBytes_C8_RGB565(dst + (y + iy) * width + x, src + 8 * xStep,
                                  tlut);
    }
    break;
  case GX_TF_IA4: {
  ...
  case GX_TF_IA8: {
    ...
    if (cpu_info.bSSSE3) {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++) {
            const __m128i mask =
                _mm_set_epi8(6, 7, 7, 7, 4, 5, 5, 5, 2, 3, 3, 3, 0, 1, 1, 1);
            // Load 4x 16-bit IA8 samples from `src` into an __m128i with upper
            // 64 bits zeroed:
            // (0000 0000 hgfe dcba)
            const __m128i r0 =
                _mm_loadl_epi64((const __m128i *)(src + 8 * xStep));
    ...
    {
      const __m128i kMask_xf0 =
          _mm_set_epi32(0x00000000L, 0x00000000L, 0xff00ff00L, 0xff00ff00L);
      const __m128i kMask_x0f =
          _mm_set_epi32(0x00000000L, 0x00000000L, 0x00ff00ffL, 0x00ff00ffL);
      const __m128i kMask_xf000 =
          _mm_set_epi32(0xff000000L, 0xff000000L, 0xff000000L, 0xff000000L);
      const __m128i kMask_x0fff =
          _mm_set_epi32(0x00ffffffL, 0x00ffffffL, 0x00ffffffL, 0x00ffffffL);
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++) {
            // Expands a 16-bit "IA" to a 32-bit "AIII". Each char is an 8-bit
            // value.
            ...
            // Logical shift all 16-bit words right by 8 bits (0000 0000 hgfe
            // dcba) to (0000 0000
            // 0h0f 0d0b)
            // This gets us only the I components.
            const __m128i i0 = _mm_srli_epi16(r0, 8);

            // Now join up the I components from their original positions but
            // mask out the A
            // components.
            // (0000 0000 hgfe dcba) &      kMask_xFF00      -> (0000 0000 h0f0
            // d0b0)
            // (0000 0000 h0f0 d0b0) | (0000 0000 0h0f 0d0b) -> (0000 0000 hhff
            // ddbb)
            const __m128i i1 = _mm_or_si128(_mm_and_si128(r0, kMask_xf0), i0);

            // Shuffle low 64-bits with itself to expand from (0000 0000 hhff
            // ddbb) to (hhhh ffff
            // dddd bbbb)
            ...
            // Now that we have the I components in 32-bit word form, time work
            // out the A components
            // into
            // their final positions.

            // (0000 0000 hgfe dcba) &      kMask_x00FF      -> (0000 0000 0g0e
            // 0c0a)
            ...
            // (0hhh 0fff 0ddd 0bbb) | (g000 e000 c000 a000) -> (ghhh efff cddd
            // abbb)
    ...
    for (int y = 0; y < height; y += 4)
      for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
        for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++) {
          ...
          // The big-endian 16-bit colors `ba` and `dc` look like
          // 0b_gggBBBbb_RRRrrGGg in a little
          // endian xmm register
          // Unpack `hgfe dcba` to `hhgg ffee ddcc bbaa`, where each 32-bit word
          // is now
          ...
          const __m128i abgr888x4 = _mm_or_si128(
              _mm_or_si128(_mm_or_si128(r0, r1), _mm_or_si128(g0, g1)),
  ...
  case GX_TF_RGB5A3: {
    ...
    // This is the hard-coded 0xFF alpha constant that is ORed in place after
    // the RGB are calculated
      ...
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++) {
            ...
            const __m128i mask =
                _mm_set_epi8(-128, -128, 6, 7, -128, -128, 4, 5, -128, -128, 2,
                             3, -128, -128, 0, 1);
            const __m128i valV = _mm_shuffle_epi8(
                _mm_loadl_epi64((const __m128i *)(src + 8 * xStep)), mask);
            int cmp = _mm_movemask_epi8(
                valV); // MSB: 0x2 = val0; 0x20=val1; 0x200 = val2; 0x2000=val3
            if ((cmp & 0x2222) == 0x2222) // SSSE3 case #1: all 4 pixels are in
                                          // RGB555 and alpha = 0xFF.
            {
              // Swizzle bits: 00012345 -> 12345123

              // r0 = (((val0>>10) & 0x1f) << 3) | (((val0>>10) & 0x1f) >> 2);
              const __m128i tmprV =
                  _mm_and_si128(_mm_srli_epi16(valV, 10), kMask_x1f);
              const __m128i rV = _mm_or_si128(_mm_slli_epi16(tmprV, 3),
                                              _mm_srli_epi16(tmprV, 2));

              // g0 = (((val0>>5 ) & 0x1f) << 3) | (((val0>>5 ) & 0x1f) >> 2);
              const __m128i tmpgV =
                  _mm_and_si128(_mm_srli_epi16(valV, 5), kMask_x1f);
              const __m128i gV = _mm_or_si128(_mm_slli_epi16(tmpgV, 3),
                                              _mm_srli_epi16(tmpgV, 2));

              // b0 = (((val0    ) & 0x1f) << 3) | (((val0    ) & 0x1f) >> 2);
              const __m128i tmpbV = _mm_and_si128(valV, kMask_x1f);
              const __m128i bV = _mm_or_si128(_mm_slli_epi16(tmpbV, 3),
                                              _mm_srli_epi16(tmpbV, 2));
            ...
            } else if (!(cmp & 0x2222)) // SSSE3 case #2: all 4 pixels are in
                                        // RGBA4443.
            {
              ...
              const __m128i tmprV =
                  _mm_and_si128(_mm_srli_epi16(valV, 8), kMask_x0f);
              const __m128i rV = _mm_or_si128(_mm_slli_epi16(tmprV, 4), tmprV);

              // g0 = (((val0>>4 ) & 0xf) << 4) | ((val0>>4 ) & 0xf);
              const __m128i tmpgV =
                  _mm_and_si128(_mm_srli_epi16(valV, 4), kMask_x0f);
              ...
              // a0 = (((val0>>12) & 0x7) << 5) | (((val0>>12) & 0x7) << 2) |
              // (((val0>>12) & 0x7) >>
              // 1);
              const __m128i tmpaV =
                  _mm_and_si128(_mm_srli_epi16(valV, 12), kMask_x07);
              const __m128i aV =
                  _mm_or_si128(_mm_slli_epi16(tmpaV, 5),
                               _mm_or_si128(_mm_slli_epi16(tmpaV, 2),
                                            _mm_srli_epi16(tmpaV, 1)));

              // newdst[0] = r0 | (g0 << 8) | (b0 << 16) | (a0 << 24);
              const __m128i final = _mm_or_si128(
                  _mm_or_si128(rV, _mm_slli_epi32(gV, 8)),
                  _mm_or_si128(_mm_slli_epi32(bV, 16), _mm_slli_epi32(aV, 24)));
              _mm_storeu_si128((__m128i *)newdst, final);
            } else {
              // TODO: Vectorise (Either 4-way branch or do both and select is
              // better than this)
              u32 *vals = (u32 *)&valV;
              int r, g, b, a;
              for (int i = 0; i < 4; ++i) {
      ...
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++)
          for (int iy = 0, xStep = 4 * yStep; iy < 4; iy++, xStep++) {
            ...
            const __m128i valV =
                _mm_set_epi16(0, val3, 0, val2, 0, val1, 0, val0);

            // Need to check all 4 pixels' MSBs to ensure we can do
            // data-parallelism:
            if (((val0 & 0x8000) & (val1 & 0x8000) & (val2 & 0x8000) &
                 (val3 & 0x8000)) == 0x8000) {
              ...
              _mm_storeu_si128((__m128i *)newdst, final);
            } else if (((val0 & 0x8000) | (val1 & 0x8000) | (val2 & 0x8000) |
                        (val3 & 0x8000)) == 0x0000) {
  ...
  case GX_TF_RGBA8: // speed critical
  {
    ...
    if (cpu_info.bSSSE3) {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++) {
          ...
          const __m128i mask0312 = _mm_set_epi8(12, 15, 13, 14, 8, 11, 9, 10, 4,
                                                7, 5, 6, 0, 3, 1, 2);
          ...
          const __m128i rgba00 =
              _mm_shuffle_epi8(_mm_unpacklo_epi8(ar0, gb0), mask0312);
          const __m128i rgba01 =
              _mm_shuffle_epi8(_mm_unpackhi_epi8(ar0, gb0), mask0312);
          const __m128i rgba10 =
              _mm_shuffle_epi8(_mm_unpacklo_epi8(ar1, gb1), mask0312);
          const __m128i rgba11 =
              _mm_shuffle_epi8(_mm_unpackhi_epi8(ar1, gb1), mask0312);
    ...
    // Produces a ~68% speed improvement over reference C implementation.
    {
      for (int y = 0; y < height; y += 4)
        for (int x = 0, yStep = (y / 4) * Wsteps4; x < width; x += 4, yStep++) {
          // Input is divided up into 16-bit words. The texels are split up into
          // AR and GB
          ...
          //
          // Input  is ([A 7][R 7][A 6][R 6] [A 5][R 5][A 4][R 4] [A 3][R 3][A
          // 2][R 2] [A 1][R 1][A
          // 0][R 0])
          //           ([A f][R f][A e][R e] [A d][R d][A c][R c] [A b][R b][A
          //           a][R a] [A 9][R 9][A
          //           8][R 8])
          //           ([G 7][B 7][G 6][B 6] [G 5][B 5][G 4][B 4] [G 3][B 3][G
          //           2][B 2] [G 1][B 1][G
          //           0][B 0])
          //           ([G f][B f][G e][B e] [G d][B d][G c][B c] [G b][B b][G
          //           a][B a] [G 9][B 9][G
          //           8][B 8])
          //
          ...
          const u8 *src2 = src + 64 * yStep;
          // Loads the 1st half of AR components ([A 7][R 7][A 6][R 6] [A 5][R
          // 5][A 4][R 4] [A 3][R
          // 3][A 2][R 2] [A 1][R 1][A 0][R 0])
          const __m128i ar0 = _mm_loadu_si128((__m128i *)src2);
          // Loads the 2nd half of AR components ([A f][R f][A e][R e] [A d][R
          // d][A c][R c] [A b][R
          // b][A a][R a] [A 9][R 9][A 8][R 8])
          const __m128i ar1 = _mm_loadu_si128((__m128i *)src2 + 1);
          // Loads the 1st half of GB components ([G 7][B 7][G 6][B 6] [G 5][B
          // 5][G 4][B 4] [G 3][B
          // 3][G 2][B 2] [G 1][B 1][G 0][B 0])
          const __m128i gb0 = _mm_loadu_si128((__m128i *)src2 + 2);
          // Loads the 2nd half of GB components ([G f][B f][G e][B e] [G d][B
          // d][G c][B c] [G b][B
          ...
          // ([A 7][R 7][A 6][R 6] [A 5][R 5][A 4][R 4] [A 3][R 3][A 2][R 2] [A
          // 1][R 1][A 0][R 0])
          // -> ([A 3][A 3][R 3][R 3] [A 2][A 2][R 2][R 2] [A 1][A 1][R 1][R 1]
          // [A 0][A 0][R 0][R
          // 0])
          const __m128i aarr00 = _mm_unpacklo_epi8(ar0, ar0);
          // ([A 7][R 7][A 6][R 6] [A 5][R 5][A 4][R 4] [A 3][R 3][A 2][R 2] [A
          // 1][R 1][A 0][R 0])
          // -> ([A 7][A 7][R 7][R 7] [A 6][A 6][R 6][R 6] [A 5][A 5][R 5][R 5]
          // [A 4][A 4][R 4][R
          // 4])
          const __m128i aarr01 = _mm_unpackhi_epi8(ar0, ar0);
          // ([A f][R f][A e][R e] [A d][R d][A c][R c] [A b][R b][A a][R a] [A
          // 9][R 9][A 8][R 8])
          // -> ([A b][A b][R b][R b] [A a][A a][R a][R a] [A 9][A 9][R 9][R 9]
          // [A 8][A 8][R 8][R
          // 8])
          const __m128i aarr10 = _mm_unpacklo_epi8(ar1, ar1);
          // ([A f][R f][A e][R e] [A d][R d][A c][R c] [A b][R b][A a][R a] [A
          // 9][R 9][A 8][R 8])
          // -> ([A f][A f][R f][R f] [A e][A e][R e][R e] [A d][A d][R d][R d]
          // [A c][A c][R c][R
          // c])
          const __m128i aarr11 = _mm_unpackhi_epi8(ar1, ar1);

          // Move A right 16 bits and mask off everything but the lowest  8 bits
          // to get A in its
          // final place:
          const __m128i ___a00 =
              _mm_and_si128(_mm_srli_epi32(aarr00, 16), kMask_x000f);
          // Move R left  16 bits and mask off everything but the highest 8 bits
          // to get R in its
          // final place:
          const __m128i r___00 =
              _mm_and_si128(_mm_slli_epi32(aarr00, 16), kMask_xf000);
          // OR the two together to get R and A in their final places:
          const __m128i r__a00 = _mm_or_si128(r___00, ___a00);

          const __m128i ___a01 =
              _mm_and_si128(_mm_srli_epi32(aarr01, 16), kMask_x000f);
          const __m128i r___01 =
              _mm_and_si128(_mm_slli_epi32(aarr01, 16), kMask_xf000);
          const __m128i r__a01 = _mm_or_si128(r___01, ___a01);

          const __m128i ___a10 =
              _mm_and_si128(_mm_srli_epi32(aarr10, 16), kMask_x000f);
          const __m128i r___10 =
              _mm_and_si128(_mm_slli_epi32(aarr10, 16), kMask_xf000);
          const __m128i r__a10 = _mm_or_si128(r___10, ___a10);

          const __m128i ___a11 =
              _mm_and_si128(_mm_srli_epi32(aarr11, 16), kMask_x000f);
          const __m128i r___11 =
              _mm_and_si128(_mm_slli_epi32(aarr11, 16), kMask_xf000);
          const __m128i r__a11 = _mm_or_si128(r___11, ___a11);

          // Expand the GB components to fill out 32-bit words:
          // ([G 7][B 7][G 6][B 6] [G 5][B 5][G 4][B 4] [G 3][B 3][G 2][B 2] [G
          // 1][B 1][G 0][B 0])
          // -> ([G 3][G 3][B 3][B 3] [G 2][G 2][B 2][B 2] [G 1][G 1][B 1][B 1]
          // [G 0][G 0][B 0][B
          // 0])
          const __m128i ggbb00 = _mm_unpacklo_epi8(gb0, gb0);
          // ([G 7][B 7][G 6][B 6] [G 5][B 5][G 4][B 4] [G 3][B 3][G 2][B 2] [G
          // 1][B 1][G 0][B 0])
          // -> ([G 7][G 7][B 7][B 7] [G 6][G 6][B 6][B 6] [G 5][G 5][B 5][B 5]
          // [G 4][G 4][B 4][B
          // 4])
          const __m128i ggbb01 = _mm_unpackhi_epi8(gb0, gb0);
          // ([G f][B f][G e][B e] [G d][B d][G c][B c] [G b][B b][G a][B a] [G
          // 9][B 9][G 8][B 8])
          // -> ([G b][G b][B b][B b] [G a][G a][B a][B a] [G 9][G 9][B 9][B 9]
          // [G 8][G 8][B 8][B
          // 8])
          const __m128i ggbb10 = _mm_unpacklo_epi8(gb1, gb1);
          // ([G f][B f][G e][B e] [G d][B d][G c][B c] [G b][B b][G a][B a] [G
          // 9][B 9][G 8][B 8])
          // -> ([G f][G f][B f][B f] [G e][G e][B e][B e] [G d][G d][B d][B d]
          // [G c][G c][B c][B
          // c])
          const __m128i ggbb11 = _mm_unpackhi_epi8(gb1, gb1);

          // G and B are already in perfect spots in the center, just remove the
          // extra copies in the
  ...
  case GX_TF_CMPR: // speed critical
    ...
    {
      // JSD optimized with SSE2 intrinsics.
      // Produces a ~50% improvement for x86 and a ~40% improvement for x64 in
      // speed over reference
      // C implementation.
      // The x64 compiled reference C code is faster than the x86 compiled
      // reference C code, but the
      // SSE2 is
      // faster than both.
      for (int y = 0; y < height; y += 8) {
        for (int x = 0, yStep = (y / 8) * Wsteps8; x < width; x += 8, yStep++) {
          ...
          // SSE2's 128-bit
          // registers.
          // This is ideal because a single DXT block contains 2 RGBA colors
          // when decoded from their
          // 16-bit.
          // Two DXT blocks therefore contain 4 RGBA colors to be processed. The
          // processing is
          // parallelizable
          // at this level, so we do.
          for (int z = 0, xStep = 2 * yStep; z < 2; ++z, xStep++) {
            // JSD NOTE: You may see many strange patterns of behavior in the
            // below code, but they
            // are for performance reasons. Sometimes, calculating what should
            // be obvious hard-coded
            // constants is faster than loading their values from memory.
            // Unfortunately, there is no
            // way to inline 128-bit constants from opcodes so they must be
            // loaded from memory. This
            // seems a little ridiculous to me in that you can't even generate a
            // constant value of 1
            // without
            // having to load it from memory. So, I stored the minimal constant
            // I could, 128-bits
            // worth
            // of 1s :). Then I use sequences of shifts to squash it to the
            // appropriate size and bit
            // positions that I need.

            const __m128i allFFs128 =
            ...
            const __m128i dxt = _mm_loadu_si128(
                (__m128i *)(src + sizeof(struct DXTBlock) * 2 * xStep));
            ...
            const __m128i c0 = _mm_or_si128(
                c1, _mm_srli_si128(
                        _mm_slli_si128(_mm_unpacklo_epi16(dxt, dxt), 8), 8));

            // Compare rgb0 to rgb1:
            // Each 32-bit word will contain either 0xFFFFFFFF or 0x00000000 for
            // true/false.
            const __m128i c0cmp =
                _mm_srli_epi32(_mm_slli_epi32(_mm_srli_epi64(c0, 8), 16), 16);
            ...
            // green:
            // NOTE: We start with the larger number of bits (6) firts for G and
            // shift the mask down
            // 1 bit to get a 5-bit mask
            // later for R and B components.
            // low6mask == _mm_set_epi32(0x0000FC00, 0x0000FC00, 0x0000FC00,
            // 0x0000FC00)
            const __m128i low6mask =
                _mm_slli_epi32(_mm_srli_epi32(allFFs128, 24 + 2), 8 + 2);
            const __m128i gtmp = _mm_srli_epi32(c0, 3);
            const __m128i g0 = _mm_and_si128(gtmp, low6mask);
            // low3mask == _mm_set_epi32(0x00000300, 0x00000300, 0x00000300,
            // 0x00000300)
            const __m128i g1 = _mm_and_si128(
                _mm_srli_epi32(gtmp, 6),
                _mm_set_epi32(0x00000300, 0x00000300, 0x00000300, 0x00000300));
            argb888x4 = _mm_or_si128(g0, g1);
            // red:
            // low5mask == _mm_set_epi32(0x000000F8, 0x000000F8, 0x000000F8,
            // 0x000000F8)
            const __m128i low5mask =
                _mm_slli_epi32(_mm_srli_epi32(low6mask, 8 + 3), 3);
            const __m128i r0 = _mm_and_si128(c0, low5mask);
            const __m128i r1 = _mm_srli_epi32(r0, 5);
            argb888x4 = _mm_or_si128(argb888x4, _mm_or_si128(r0, r1));
            // blue:
            // _mm_slli_epi32(low5mask, 16) == _mm_set_epi32(0x00F80000,
            // 0x00F80000, 0x00F80000,
            // 0x00F80000)
            const __m128i b0 = _mm_and_si128(_mm_srli_epi32(c0, 5),
                                             _mm_slli_epi32(low5mask, 16));
            const __m128i b1 = _mm_srli_epi16(b0, 5);
            // OR in the fixed alpha component
            // _mm_slli_epi32( allFFs128, 24 ) == _mm_set_epi32(0xFF000000,
            // 0xFF000000, 0xFF000000,
            // 0xFF000000)
            argb888x4 = _mm_or_si128(
                _mm_or_si128(argb888x4, _mm_slli_epi32(allFFs128, 24)),
                _mm_or_si128(b0, b1));
            // calculate RGB2 and RGB3:
            const __m128i rgb0 =
                _mm_shuffle_epi32(argb888x4, _MM_SHUFFLE(2, 2, 0, 0));
            const __m128i rgb1 =
                _mm_shuffle_epi32(argb888x4, _MM_SHUFFLE(3, 3, 1, 1));
            const __m128i rrggbb0 = _mm_and_si128(_mm_unpacklo_epi8(rgb0, rgb0),
                                                  _mm_srli_epi16(allFFs128, 8));
            const __m128i rrggbb1 = _mm_and_si128(_mm_unpacklo_epi8(rgb1, rgb1),
                                                  _mm_srli_epi16(allFFs128, 8));
            const __m128i rrggbb01 = _mm_and_si128(
                _mm_unpackhi_epi8(rgb0, rgb0), _mm_srli_epi16(allFFs128, 8));
            const __m128i rrggbb11 = _mm_and_si128(
                _mm_unpackhi_epi8(rgb1, rgb1), _mm_srli_epi16(allFFs128, 8));
            ...
            if (cmp0 != 0) {
              ...
              const __m128i shr1subshr3 =
                  _mm_sub_epi16(rrggbbsubshr1, rrggbbsubshr3);
              // low8mask16 == _mm_set_epi16(0x00ff, 0x00ff, 0x00ff, 0x00ff,
              // 0x00ff, 0x00ff, 0x00ff,
              // 0x00ff)
              const __m128i low8mask16 = _mm_srli_epi16(allFFs128, 8);
              const __m128i rrggbbdelta =
                  _mm_and_si128(shr1subshr3, low8mask16);
              const __m128i rgbdeltadup =
                  _mm_packus_epi16(rrggbbdelta, rrggbbdelta);
              const __m128i rgbdelta =
                  _mm_srli_si128(_mm_slli_si128(rgbdeltadup, 8), 8);

              rgb2 = _mm_and_si128(_mm_add_epi8(rgb0, rgbdelta),
                                   _mm_srli_si128(allFFs128, 8));
              rgb3 = _mm_and_si128(_mm_sub_epi8(rgb1, rgbdelta),
                                   _mm_srli_si128(allFFs128, 8));
            } else {
              // RGB2b = avg(RGB0, RGB1)
              const __m128i rrggbb21 = _mm_avg_epu16(rrggbb0, rrggbb1);
              const __m128i rgb210 =
                  _mm_srli_si128(_mm_packus_epi16(rrggbb21, rrggbb21), 8);
              rgb2 = rgb210;
              rgb3 = _mm_and_si128(
                  _mm_srli_si128(
                      _mm_shuffle_epi32(argb888x4, _MM_SHUFFLE(1, 1, 1, 1)), 8),
            ...
            if (cmp1 != 0) {
              ...
              const __m128i shr1subshr31 =
                  _mm_sub_epi16(rrggbbsubshr11, rrggbbsubshr31);
              // low8mask16 == _mm_set_epi16(0x00ff, 0x00ff, 0x00ff, 0x00ff,
              // 0x00ff, 0x00ff, 0x00ff,
              // 0x00ff)
              const __m128i low8mask16 = _mm_srli_epi16(allFFs128, 8);
              const __m128i rrggbbdelta1 =
                  _mm_and_si128(shr1subshr31, low8mask16);
              __m128i rgbdelta1 = _mm_packus_epi16(rrggbbdelta1, rrggbbdelta1);
              rgbdelta1 = _mm_slli_si128(rgbdelta1, 8);

              rgb2 = _mm_or_si128(rgb2,
                                  _mm_and_si128(_mm_add_epi8(rgb0, rgbdelta1),
                                                _mm_slli_si128(allFFs128, 8)));
              rgb3 = _mm_or_si128(rgb3,
                                  _mm_and_si128(_mm_sub_epi8(rgb1, rgbdelta1),
                                                _mm_slli_si128(allFFs128, 8)));
            } else {
              // RGB2b = avg(RGB0, RGB1)
              const __m128i rrggbb211 = _mm_avg_epu16(rrggbb01, rrggbb11);
              const __m128i rgb211 =
                  _mm_slli_si128(_mm_packus_epi16(rrggbb211, rrggbb211), 8);
              rgb2 = _mm_or_si128(rgb2, rgb211);

              // _mm_srli_epi32( allFFs128, 8 ) == _mm_set_epi32(0x00FFFFFF,
              // 0x00FFFFFF, 0x00FFFFFF,
              // 0x00FFFFFF)
              // Make this color fully transparent:
              rgb3 = _mm_or_si128(
                  rgb3, _mm_and_si128(
                            _mm_and_si128(rgb1, _mm_srli_epi32(allFFs128, 8)),
                            _mm_slli_si128(allFFs128, 8)));
            }

            // Create an array for color lookups for DXT0 so we can use the
            // 2-bit indices:
            const __m128i mmcolors0 = _mm_or_si128(
                _mm_or_si128(
                    _mm_srli_si128(_mm_slli_si128(argb888x4, 8), 8),
                    _mm_slli_si128(
                        _mm_srli_si128(_mm_slli_si128(rgb2, 8), 8 + 4), 8)),
                _mm_slli_si128(_mm_srli_si128(rgb3, 4), 8 + 4));

            // Create an array for color lookups for DXT1 so we can use the
            // 2-bit indices:
            const __m128i mmcolors1 = _mm_or_si128(
                _mm_or_si128(_mm_srli_si128(argb888x4, 8),
                             _mm_slli_si128(_mm_srli_si128(rgb2, 8 + 4), 8)),
                _mm_slli_si128(_mm_srli_si128(rgb3, 8 + 4), 8 + 4));

// The #ifdef CHECKs here and below are to compare correctness of output against
// the reference code.
