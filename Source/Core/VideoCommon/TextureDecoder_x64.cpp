    ...
    case GX_CTF_XFB:
    {
      for (int y = 0; y < height; y += 1)
      {
        for (int x = 0; x < width; x += 2)
        {
          ...
          u8 R1 = MathUtil::Clamp(int(1.164f * Y1 + 1.596f * V), 0, 255);
          u8 G1 = MathUtil::Clamp(int(1.164f * Y1 - 0.392f * U - 0.813f * V), 0, 255);
          u8 B1 = MathUtil::Clamp(int(1.164f * Y1 + 2.017f * U), 0, 255);

          u8 R2 = MathUtil::Clamp(int(1.164f * Y2 + 1.596f * V), 0, 255);
          u8 G2 = MathUtil::Clamp(int(1.164f * Y2 - 0.392f * U - 0.813f * V), 0, 255);
          u8 B2 = MathUtil::Clamp(int(1.164f * Y2 + 2.017f * U), 0, 255);
