...
// Projection
enum : u32 { XF_TEXPROJ_ST = 0, XF_TEXPROJ_STQ = 1 };

// Input form
enum : u32 { XF_TEXINPUT_AB11 = 0, XF_TEXINPUT_ABC1 = 1 };
...
// Control source
enum : u32 { GX_SRC_REG = 0, GX_SRC_VTX = 1 };

// Light diffuse attenuation function
enum : u32 { LIGHTDIF_NONE = 0, LIGHTDIF_SIGN = 1, LIGHTDIF_CLAMP = 2 };
...
// Projection type
enum : u32 { GX_PERSPECTIVE = 0, GX_ORTHOGRAPHIC = 1 };
...
union INVTXSPEC {
  struct {
    u32 numcolors : 2;
    u32 numnormals : 2; // 0 - nothing, 1 - just normal, 2 - normals and
                        // binormals
    u32 numtextures : 4;
    u32 unused : 24;
  };
...
struct XFMemory {
  ...
  u32 perf1;                // 0x1007
  INVTXSPEC
  hostinfo; // 0x1008 number of textures,colors,normals from vertex input
