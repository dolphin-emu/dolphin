...
typedef std::unordered_map<VertexLoaderUID, std::unique_ptr<VertexLoaderBase>>
    VertexLoaderMap;
...
void UpdateVertexArrayPointers() {
  ...
  for (int i = 0; i < 12; i++) {
    // Only update the array base if the vertex description states we are going
    // to use it.
...
static VertexLoaderBase *RefreshLoader(int vtx_attr_group,
  ...
  if (state->attr_dirty[vtx_attr_group]) {
    // We are not allowed to create a native vertex format on preprocessing as
    // this is on the wrong
    // thread
    ...
    if (iter != s_vertex_loader_map.end()) {
      ...
      s_vertex_loader_map[uid] = VertexLoaderBase::CreateVertexLoader(
          state->vtx_desc, state->vtx_attr[vtx_attr_group]);
...
int RunVertices(int vtx_attr_group, int primitive, int count, DataReader src,
                bool skip_drawing, bool is_preprocess) {
  if (!count)
    return 0;
  ...
  // They still need to go through vertex loading, because we need to calculate
  // a zfreeze refrence
  // slope.
...
NativeVertexFormat *GetCurrentVertexFormat() { return s_current_vtx_fmt; }
