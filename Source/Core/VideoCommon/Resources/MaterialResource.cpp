...
{
  m_material_asset = m_resource_context.asset_cache->CreateAsset<RasterMaterialAsset>(
...
void MaterialResource::ResetData()
{
  if (m_current_data)
  {
    ...
    {
      std::visit(overloaded{[this](TextureAndSamplerResource* resource) {
                              if (resource)
                                resource->RemoveReference(this);
                            },
                            [this](RenderTargetResource* resource) {
                              if (resource)
                                resource->RemoveReference(this);
                            }},
                 texture_resource);
...
Resource::TaskComplete MaterialResource::ProcessData()
{
  ...
  public:
    ...
    bool Compile() override
    {
      ...
      if (m_material_resource_data->m_material_data->cull_mode)
        config.rasterization_state.cullmode = *m_material_resource_data->m_material_data->cull_mode;
  ...
  if (!m_processing_load_data)
  {
    auto wi = m_resource_context.shader_compiler->CreateWorkItem<WorkItem>(
        m_load_data, m_load_data->m_shader_resource->GetData(), &m_uid,
...
void MaterialResource::MarkAsActive()
{
  ...
  for (const auto& texture_resource : m_current_data->m_texture_like_resources)
  {
    std::visit(overloaded{[](TextureAndSamplerResource* resource) {
                            if (resource)
...
void MaterialResource::WriteUniforms(Data* data)
{
  ...
  {
    max_uniformdata_size += VideoCommon::MaterialProperty2::GetMemorySize(property);
  }
  data->m_uniform_data.resize(max_uniformdata_size);

  // Now write the memory
  u8* uniform_data = data->m_uniform_data.data();
  for (const auto& property : data->m_material_data->properties)
  {
    VideoCommon::MaterialProperty2::WriteToMemory(uniform_data, property);
  }
}
}  // namespace VideoCommon
