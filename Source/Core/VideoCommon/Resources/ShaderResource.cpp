                                                   ...
                                                   const RasterShaderData& shader_data)
{
  ...
  for (std::size_t i = 0; i < shader_data.m_samplers.size(); i++)
  {
    const auto& sampler = shader_data.m_samplers[i];
  ...
  for (const auto& [name, property] : shader_data.m_uniform_properties)
  {
    VideoCommon::ShaderProperty2::WriteAsShaderCode(uniform_code, name, property);
  }
  if (!shader_data.m_uniform_properties.empty())
    uniform_code.Write("\n\n");
...
{
  m_shader_asset = m_resource_context.asset_cache->CreateAsset<RasterShaderAsset>(
...
AbstractTextureType ShaderResource::Data::GetTextureType(std::size_t index)
{
  ...
  if (!m_shader_data || index >= m_shader_data->m_samplers.size()) [[unlikely]]
    return AbstractTextureType::Texture_2D;

  return m_shader_data->m_samplers[index].type;
}
...
Resource::TaskComplete ShaderResource::ProcessData()
{
  ...
  public:
    ...
    bool Compile() override
    {
      ...
      shader_host_config.bits = m_shader_bits;
