...
namespace CommandLineParse
{
	...
	{
	public:
		CommandLineConfigLayerLoader(const std::list<std::string>& args, const std::string& video_backend, const std::string& audio_backend)
			: ConfigLayerLoader(OnionConfig::OnionLayerType::LAYER_COMMANDLINE)
		{
			if (video_backend.size())
				m_values.emplace_back(std::make_tuple("Main", "Core", "GFXBackend", video_backend));

			if (audio_backend.size())
				m_values.emplace_back(std::make_tuple("Main", "Core", "DSPHLE", audio_backend == "HLE" ? "True" : "False"));
	...
	optparse::Values& ParseArguments(optparse::OptionParser* parser, int argc, char** argv)
	{
		...
		if (config_args.size())
			OnionConfig::AddLayer(new CommandLineConfigLayerLoader(config_args,
			                                                       (const char*)options.get("video_backend"),
			                                                       (const char*)options.get("audio_emulation")));
