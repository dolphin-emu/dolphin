...
HostDisassemblerLLVM::HostDisassemblerLLVM(const std::string &host_disasm,
  ...
  m_llvm_context = LLVMCreateDisasmCPU(host_disasm.c_str(), cpu.c_str(),
                                       nullptr, 0, 0, nullptr);
...
std::string HostDisassemblerLLVM::DisassembleHostBlock(
    const u8 *code_start, const u32 code_size, u32 *host_instructions_count,
    u64 starting_pc) {
  if (!m_can_disasm)
  ...
  while ((u8 *)disasmPtr < end) {
    ...
    size_t inst_size =
        LLVMDisasmInstruction(m_llvm_context, disasmPtr, (u64)(end - disasmPtr),
...
HostDisassemblerX86::HostDisassemblerX86() { m_disasm.set_syntax_intel(); }

std::string HostDisassemblerX86::DisassembleHostBlock(
    const u8 *code_start, const u32 code_size, u32 *host_instructions_count,
    u64 starting_pc) {
  ...
  while ((u8 *)disasmPtr < end) {
    char inst_disasm[256];
    disasmPtr +=
        m_disasm.disasm64(disasmPtr, disasmPtr, (u8 *)disasmPtr, inst_disasm);
    x86_disasm << inst_disasm << std::endl;
    (*host_instructions_count)++;
  }
