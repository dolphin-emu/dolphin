...
/// @return A bit mask of the given width starting from the least-significant bit.
///
template <Common::Integral T, size_t width>
constexpr T CalcLowMaskFast() noexcept
{
...
///
template <Common::Integral T, size_t width>
constexpr T CalcLowMaskSafe() noexcept
{
  static_assert(width <= BitSize<T>(), "Bit width larger than BitSize<T>");
  return CalcLowMaskSafe<T>(width);
}
...
/// @return The extracted bit.
///
template <size_t start, Common::Integral T>
...
/// @return The extracted bits.
///
template <size_t start, size_t width, Common::Integral T>
