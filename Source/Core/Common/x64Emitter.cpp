...
void OpArg::WriteVEX(
    XEmitter* emit, X64Reg regOp1, X64Reg regOp2, int L, int pp, int mmmmm, int W) const
{
  int R = !(regOp1 & 8);
  int X = !(indexReg & 8);
...
void OpArg::WriteRest(
    XEmitter* emit, int extraBytes, X64Reg _operandReg, bool warn_64bit_offset) const
{
...
void OpArg::WriteNormalOp(
    XEmitter* emit, bool toRM, NormalOp op, const OpArg& operand, int bits) const
{
  X64Reg _operandReg;
  if (IsImm())
  {
  ...
  if (operand.IsImm())
  {
    ...
    else if (operand.scale == SCALE_IMM64 && bits == 64)
    {
      ...
      {
        ASSERT_MSG(
            DYNA_REC, 0, "WriteNormalOp - MOV with 64-bit imm requires register destination");
      }
      // mov reg64, imm64
...
void XEmitter::WriteVEXOp(
    u8 opPrefix, u16 op, X64Reg regOp1, X64Reg regOp2, const OpArg& arg, int W, int extrabytes)
{
...
void XEmitter::WriteVEXOp4(
    u8 opPrefix, u16 op, X64Reg regOp1, X64Reg regOp2, const OpArg& arg, X64Reg regOp3, int W)
{
...
void XEmitter::WriteFMA4Op(
    u8 op, X64Reg dest, X64Reg regOp1, X64Reg regOp2, const OpArg& arg, int W)
{
  if (!cpu_info.bFMA4)
  {
    PanicAlertFmt(
...
void XEmitter::WriteBMIOp(
    int size, u8 opPrefix, u16 op, X64Reg regOp1, X64Reg regOp2, const OpArg& arg, int extrabytes)
{
  if (arg.IsImm())
