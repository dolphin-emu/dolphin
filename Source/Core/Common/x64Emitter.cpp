                      ...
                      const bool warn_64bit_offset) const
{
  ...
  if (mod == 1)  // 8-bit disp
  {
    emit->Write8(static_cast<u8>((s8)(s32)offset));
  }
...
void XEmitter::JMP(const u8* addr, const Jump jump)
{
  ...
  if (jump == Jump::Short)
  {
    ...
    // 8 bits will do
    Write8(0xEB);
    Write8(static_cast<u8>((s8)distance));
  }
  else
  {
    ...
    Write8(0xE9);
    Write32(static_cast<u32>((s32)distance));
  }
}
...
void XEmitter::PUSH(const int bits, const OpArg& reg)
{
  ...
  else if (reg.IsImm())
  {
    ...
    case 8:
      Write8(0x6A);
      Write8(static_cast<u8>((s8)reg.offset));
      break;
    case 16:
      Write8(0x66);
      Write8(0x68);
      Write16(static_cast<u16>((s16)(s32)reg.offset));
      break;
    case 32:
      Write8(0x68);
...
void XEmitter::IMUL(const int bits, const X64Reg regOp, const OpArg& a1, const OpArg& a2)
{
  ...
  if (a2.GetImmBits() == 8 || (a2.GetImmBits() == 16 && static_cast<s8>(a2.offset) == static_cast<s16>(a2.offset)) ||
