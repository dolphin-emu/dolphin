...
LogManager::LogManager()
{
  ...
  {
    auto& container = m_log[static_cast<LogType>(i)];
...
void LogManager::SaveSettings()
{
  ...
  for (int i = 0; i < static_cast<int>(Common::Log::LogType::NUMBER_OF_LOGS); i++)
  {
    const auto& container = m_log[static_cast<LogType>(i)];
...
std::map<std::string, std::string> LogManager::GetLogTypes()
{
  ...
  {
    const auto& container = m_log[static_cast<LogType>(i)];
    LogTypes.emplace(container.m_short_name, container.m_full_name);
  }
  return LogTypes;
}
