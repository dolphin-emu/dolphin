...
bool ReadValue(std::string* value, const std::string& subkey, const std::string& name)
{
  ...
  auto status = RegGetValueA(
      HKEY_LOCAL_MACHINE, subkey.c_str(), name.c_str(), flags, nullptr, nullptr, &value_len);
  if (status != ERROR_SUCCESS && status != ERROR_MORE_DATA)
    return false;

  value->resize(value_len);
  status = RegGetValueA(
      HKEY_LOCAL_MACHINE, subkey.c_str(), name.c_str(), flags, nullptr, value->data(), &value_len);
