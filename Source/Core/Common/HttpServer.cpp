...
static bool SendAll(s32 sock, const char* data, std::size_t size)
{
  for (std::size_t sent = 0; sent < size;)
  {
    const int bytes = send(sock, data + sent, static_cast<int>(size - sent), Common::SEND_FLAGS);
    if (bytes < 0)
    {
...
void HttpServer::Serve()
{
  ...
  while (!m_stop_flag.load())
  {
    ...
    else if (!Common::SetPlatformSocketOptions(client_sock))
    {
...
void HttpServer::HandleClient(s32 client_sock)
{
  ...
  if (version != "HTTP/1.0" && version != "HTTP/1.1")
  {
    ...
    SendAll(client_sock, response.c_str(), response.size());
    return;
  }
  ...
  SendAll(client_sock, reinterpret_cast<const char*>(send_buffer.data()), send_buffer.size());
}
