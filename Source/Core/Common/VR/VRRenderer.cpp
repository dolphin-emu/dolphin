...
void VR_Recenter(engine_t* engine)
{
  ...
  if (engine->appState.FakeSpace != XR_NULL_HANDLE)
  {
...
void VR_InitRenderer(engine_t* engine, bool multiview)
{
  ...
  OXR(xrEnumerateReferenceSpaces(engine->appState.Session, numSpaces, &numSpaces, referenceSpaces));
...
void VR_FinishFrame(engine_t* engine)
{
  ...
  else if ((vrMode == VR_MODE_MONO_SCREEN) || (vrMode == VR_MODE_STEREO_SCREEN))
  {
    ...
    cylinder_layer.subImage.imageRect.extent.width = frameBuffer->ColorSwapChain.Width;
