...
std::optional<ParseDigitsResult> parse_digits(std::string_view str)
{
  ...
  {
    const auto result = std::from_chars(str.data(), str.data() + idx, val);
...
{
  std::size_t start_idx = str.find('<');
  std::string tmpl_args;

  if (start_idx != std::string::npos)
  {
    std::size_t end_idx = str.rfind('>');
    if (end_idx == std::string::npos || end_idx < start_idx)
    {
      return std::nullopt;
    }

    std::string_view args = str.substr(start_idx + 1, end_idx - (start_idx + 1));
...
std::optional<DemangleNameResult> demangle_name(std::string_view str, DemangleOptions options)
{
  ...
  const std::string result_name = {name.begin(), name.end()};
                                                          ...
                                                          DemangleOptions options)
{
  if (str.starts_with('Q'))
  {
    ...
    const std::string_view digit = str.substr(1, 1);
    const auto parse_result = std::from_chars(digit.data(), digit.data() + 1, count);
...
std::optional<DemangleArgResult> demangle_arg(std::string_view str, DemangleOptions options)
{
  ...
  if (str.starts_with('-'))
  {
    ...
    const auto parsed_val = parse_result.value();
    std::size_t size = parsed_val.digits;
    const std::string out_val = fmt::format("-{}", size);
    return {{out_val, "", parsed_val.rest}};
  }
  ...
  if (str.length() > 0 && std::isdigit(str[0]))
  {
    ...
    const auto demangled_name = demangle_name_result.value();
    result += demangled_name.full;
    result += post;
    return {{result, "", demangled_name.rest}};
  }
  ...
  if (str.starts_with('Q'))
  {
    ...
    const auto demangled_qual = demangle_qual_result.value();
    result += demangled_qual.full;
    result += post;
    return {{result, "", demangled_qual.rest}};
  }
  ...
  if (str.starts_with('M'))
  {
    ...
    const auto demangled_qual = demangle_qual_result.value();
    rest = demangled_qual.rest;
    pre = fmt::format("{}::*{}", demangled_qual.full, pre);
  ...
  if (is_member || str.starts_with('F'))
  {
    ...
    const auto demangled_arg = demangle_arg_result.value();

    const std::string const_str = const_member ? " const" : "";
    const std::string res_pre = fmt::format("{} ({}{}", demangled_arg.arg_pre, pre, post);
    const std::string res_post =
        fmt::format(")({}){}{}", demangled_func_args.args, const_str, demangled_arg.arg_post);
    return {{res_pre, res_post, demangled_arg.rest}};
  }
  ...
  if (str.starts_with('A'))
  {
    ...
    result = fmt::format("{}{}{}", pre, demangled_arg.arg_pre, post);
    const std::string ret_post = fmt::format("[{}]{}", count, demangled_arg.arg_post);
    return {{result, ret_post, demangled_arg.rest}};
  }
                                                                 ...
                                                                 DemangleOptions options)
{
  ...
  while (!str.empty())
  {
    ...
    result += demangled_arg.arg_pre;
    result += demangled_arg.arg_post;
    str = demangled_arg.rest;
                                                     ...
                                                     DemangleOptions options)
{
  if (str.starts_with("op"))
  {
    ...
    const auto demangled_arg = demangle_arg_result.value();
    return fmt::format("operator {}{}", demangled_arg.arg_pre, demangled_arg.arg_post);
  }
...
std::optional<std::string> demangle(std::string_view str, DemangleOptions options)
{
  ...
  if (!str.starts_with('F'))
  {
    ...
    const auto demangled_qual = result.value();
    class_name = demangled_qual.class_name;
    qualified = demangled_qual.full;
    str = demangled_qual.rest;
  }
  if (special)
  {
  ...
  if (str.starts_with('F'))
  {
    ...
    const auto func_args = result.value();
    if (options.omit_empty_parameters && func_args.args == "void")
    {
      fn_name = fmt::format("{}()", fn_name);
    }
    else
    {
      fn_name = fmt::format("{}({})", fn_name, func_args.args);
    }
    str = func_args.rest;
  }
  if (str.starts_with('_'))
  {
    ...
    const auto demangled_arg = result.value();
    return_type_pre = demangled_arg.arg_pre;
    return_type_post = demangled_arg.arg_post;
    str = demangled_arg.rest;
  }
  if (!str.empty())
  {
...
std::optional<std::size_t> find_split(std::string_view s, bool special, DemangleOptions options)
{
  ...
  if (special && s.starts_with("op"))
  {
    ...
    const std::string_view rest = result.value().rest;
