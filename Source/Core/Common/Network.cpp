...
{
  const auto different_bits = first.GetIPAddressValue() ^ last.GetIPAddressValue();
  const auto common_high_bits = std::countl_zero(different_bits);
  ...
  else if (std::countr_one(different_bits) == (32 - common_high_bits))
  {
