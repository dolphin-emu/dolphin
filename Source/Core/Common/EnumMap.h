...
public:
  ...
  {
    static_assert(1 << bits == _Size, "Unsafe indexing into EnumMap (may go out of bounds)");
  ...
  constexpr bool InBounds(T key) const { return static_cast<std::size_t>(key) < _Size; }
