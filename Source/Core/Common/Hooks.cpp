...
#define ARGUEMNT_NAME_STRINGS(NAME, ...) \
  static std::array<std::string, std::tuple_size<decltype(std::make_tuple(__VA_ARGS__))>::value> NAME##_argnames = {__VA_ARGS__};
HOOKABLE_EVENT_TABLE(ARGUEMNT_NAME_STRINGS, NAMES_STRING)

namespace Hook
{
// Generate the Definition of all the hooks
#define TYPES_ONLY(TYPE, NAME) TYPE
#define DEFINITOIN(NAME, ...) \
  HookableEvent<__VA_ARGS__> NAME(NAME##_argnames);
HOOKABLE_EVENT_TABLE(DEFINITOIN, TYPES_ONLY)
}
