...
namespace OnionConfig
{
	...
	{
		// The RecursiveOnionPetal can't set since it is used to recursively get values from the layer map.
	...
	static std::map<OnionSystem, std::string> system_to_name =
	{
		...
		{ OnionSystem::SYSTEM_GCKEYBOARD, "GCKeyboard" },
	...
	OnionSystem GetSystemFromName(const std::string& system)
	{
		...
		_assert_msg_(COMMON, false, "Programming error! Couldn't convert '%s' to system!", system.c_str());
