...
kiss_fftr_cfg kiss_fftr_alloc(int nfft, const int inverse_fft, void* mem, size_t* lenmem)
{
  ...
  {
    double phase = -std::numbers::pi * (static_cast<double>(i + 1) / nfft + .5);
    if (inverse_fft)
      phase *= -1;
...
void kiss_fftr(const kiss_fftr_cfg cfg, const kiss_fft_scalar* timedata, kiss_fft_cpx* freqdata)
{
  ...
  {
    fmt::fprintf(stderr, "kiss fft usage error: improper alloc\n");
    exit(1);
  }

  const int ncfft = cfg->substate->nfft;
...
void kiss_fftri(const kiss_fftr_cfg cfg, const kiss_fft_cpx* freqdata, kiss_fft_scalar* timedata)
{
  /* input buffer timedata is stored row-wise */
