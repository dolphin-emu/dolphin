...
bool CubebStream::Init()
{
  ...
  m_work_queue.PushBlocking([this, &return_value] {
    ...
    if (m_ctx)
    {
      ...
      // Create per-GBA streams for audio routing when enabled
      if (return_value && Config::Get(Config::MAIN_GBA_AUDIO_ROUTING_ENABLED))
      {
        ...
        for (std::size_t i = 0; i < m_gba_streams.size(); ++i)
        {
          ...
          const std::string device_id_str = Config::Get(Config::MAIN_GBA_AUDIO_OUTPUT_DEVICE[i]);
          const cubeb_devid output_devid =
              device_id_str.empty() ?
                  nullptr :
                  static_cast<cubeb_devid>(CubebUtils::GetOutputDeviceById(device_id_str));
          ...
          const int result = cubeb_stream_init(
              m_ctx.get(), &m_gba_streams[i], GBA_STREAM_NAMES[i], nullptr, nullptr, output_devid,
              &gba_params, std::max(BUFFER_SAMPLES, gba_min_latency), GBADataCallback,
              GBAStateCallback, &m_gba_stream_data[i]);

          if (result != CUBEB_OK)
          {
            ERROR_LOG_FMT(AUDIO, "Failed to create Cubeb stream for GBA {} audio split", i + 1);
