...
int VerifyCommand(const std::vector<std::string>& args)
{
  ...
  if (!hashes_to_calculate.crc32 && !hashes_to_calculate.md5 && !hashes_to_calculate.sha1)
  {
    ...
    fmt::println(std::cerr, "Error: No algorithms selected for the operation");
    return EXIT_FAILURE;
  }
  ...
  else
  {
    if (hashes_to_calculate.crc32 && !result.hashes.crc32.empty())
      fmt::println(std::cout, "{}", HashToHexString(result.hashes.crc32));
    else if (hashes_to_calculate.md5 && !result.hashes.md5.empty())
      fmt::println(std::cout, "{}", HashToHexString(result.hashes.md5));
    else if (hashes_to_calculate.sha1 && !result.hashes.sha1.empty())
      fmt::println(std::cout, "{}", HashToHexString(result.hashes.sha1));
    else
    {
      fmt::println(std::cerr, "Error: No hash computed");
      return EXIT_FAILURE;
    }
  }
...
static void PrintFullReport(const DiscIO::VolumeVerifier::Result& result)
{
  if (!result.hashes.crc32.empty())
    fmt::println(std::cout, "CRC32: {}", HashToHexString(result.hashes.crc32));
  else
    fmt::println(std::cout, "CRC32 not computed");

  if (!result.hashes.md5.empty())
    fmt::println(std::cout, "MD5: {}", HashToHexString(result.hashes.md5));
  else
    fmt::println(std::cout, "MD5 not computed");

  if (!result.hashes.sha1.empty())
    fmt::println(std::cout, "SHA1: {}", HashToHexString(result.hashes.sha1));
  else
    fmt::println(std::cout, "SHA1 not computed");

  fmt::println(std::cout, "Problems Found: {}", result.problems.empty() ? "No" : "Yes");
