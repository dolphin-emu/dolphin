...
static void ExtractFile(const DiscIO::Volume &disc_volume,
...
static void ExtractDirectory(const DiscIO::Volume &disc_volume,
                             const DiscIO::Partition &partition,
                             const std::string &path, const std::string &out,
                             bool mute) {
  const DiscIO::FileSystem *filesystem = disc_volume.GetFileSystem(partition);
  ...
  ExportDirectory(disc_volume, partition, *info, true, path, out,
                  [&files, &size, &mute](const std::string &current) {
                    files++;
                    const float progress = static_cast<float>(files) /
                                           static_cast<float>(size) * 100;
                    if (!mute)
...
static bool ExtractSystemData(const DiscIO::Volume &disc_volume,
                              const DiscIO::Partition &partition,
        ...
        if (!options.is_set("singular")) {
          ...
          if (auto file_info = GetFileInfo(*disc_volume, p, singular_file_path);
            ...
            if (file_info->IsDirectory())
              ExtractDirectory(*disc_volume, p, singular_file_path,
                               output_folder_path, mute);
            else
