...
TEST(MathUtil, FlushToZero) {
  ...
  // Casting away the volatile attribute is required in order for msvc to
  // resolve this to the
  ...
  EXPECT_EQ(+0.0,
            MathUtil::FlushToZero(+std::numeric_limits<double>::denorm_min()));
  EXPECT_EQ(-0.0,
            MathUtil::FlushToZero(-std::numeric_limits<double>::denorm_min()));
  EXPECT_EQ(+0.0,
            MathUtil::FlushToZero(+std::numeric_limits<double>::min() / 2));
  EXPECT_EQ(-0.0,
            MathUtil::FlushToZero(-std::numeric_limits<double>::min() / 2));
  ...
  // Test all subnormals as well as an equally large set of random normal
  // floats.
