...
public:
  ...
  static void ExpectMapped(u32 logical_address, u32 physical_address)
  {
    ...
    *physical_ptr = ++s_counter;
    EXPECT_EQ(*logical_ptr, s_counter) << "Page wasn't mapped, against expectations";

    *logical_ptr = ++s_counter;
    EXPECT_EQ(*physical_ptr, s_counter) << "Page wasn't mapped, against expectations";
  }
...
#define ASAN_DISABLE
  ...
  static void SetPTE(UPTE_Lo pte1, UPTE_Hi pte2, u32 logical_address, u32 index)
  {
    ...
    u32 pteg_addr = ((hash & ppc_state.pagetable_hashmask) << 6) | ppc_state.pagetable_base;
    u32 pte_addr = (index & 7) * 8 + pteg_addr;

    u8* physical_base = system.GetMemory().GetPhysicalBase();
    Common::swap32(physical_base + pte_addr, pte1.Hex);
    Common::swap32(physical_base + pte_addr + 4, pte2.Hex);
...
{
  // Secondary PTEs for 0x10140000

  AddHostSizedMapping(0x10140000, 0x00140000, 10);
  ExpectMapped(0x10140000, 0x00140000);

  AddHostSizedMapping(0x10140000, 0x00150000, 12);
  ExpectMapped(0x10140000, 0x00140000);

  AddHostSizedMapping(0x10140000, 0x00160000, 8);
  ExpectMapped(0x10140000, 0x00160000);

  RemoveHostSizedMapping(0x10140000, 0x00140000, 10);
  ExpectMapped(0x10140000, 0x00160000);

  RemoveHostSizedMapping(0x10140000, 0x00160000, 8);
  ExpectMapped(0x10140000, 0x00150000);

  // Primary PTEs for 0x10140000

  AddHostSizedMapping(0x10140000, 0x00140000, 2);
  ExpectMapped(0x10140000, 0x00140000);

  AddHostSizedMapping(0x10140000, 0x00160000, 4);
  ExpectMapped(0x10140000, 0x00140000);

  AddHostSizedMapping(0x10140000, 0x00170000, 0);
  ExpectMapped(0x10140000, 0x00170000);

  RemoveHostSizedMapping(0x10140000, 0x00140000, 2);
  ExpectMapped(0x10140000, 0x00170000);

  RemoveHostSizedMapping(0x10140000, 0x00170000, 0);
  ExpectMapped(0x10140000, 0x00160000);

  // Return to secondary PTE for 0x10140000

  RemoveHostSizedMapping(0x10140000, 0x00160000, 4);
  ExpectMapped(0x10140000, 0x00150000);

  // Secondary PTEs for 0x11140000

  AddHostSizedMapping(0x11140000, 0x01140000, 11);
  ExpectMapped(0x11140000, 0x01140000);

  AddHostSizedMapping(0x11140000, 0x01150000, 13);
  ExpectMapped(0x11140000, 0x01140000);

  AddHostSizedMapping(0x11140000, 0x01160000, 9);
  ExpectMapped(0x11140000, 0x01160000);

  RemoveHostSizedMapping(0x11140000, 0x01140000, 11);
  ExpectMapped(0x11140000, 0x01160000);

  RemoveHostSizedMapping(0x11140000, 0x01160000, 9);
  ExpectMapped(0x11140000, 0x01150000);

  // Primary PTEs for 0x11140000

  AddHostSizedMapping(0x11140000, 0x01140000, 3);
  ExpectMapped(0x11140000, 0x01140000);

  AddHostSizedMapping(0x11140000, 0x01160000, 5);
  ExpectMapped(0x11140000, 0x01140000);

  AddHostSizedMapping(0x11140000, 0x01170000, 1);
  ExpectMapped(0x11140000, 0x01170000);

  RemoveHostSizedMapping(0x11140000, 0x01140000, 3);
  ExpectMapped(0x11140000, 0x01170000);

  RemoveHostSizedMapping(0x11140000, 0x01170000, 1);
  ExpectMapped(0x11140000, 0x01160000);

  // Return to secondary PTE for 0x11140000

  RemoveHostSizedMapping(0x11140000, 0x01160000, 5);
  ExpectMapped(0x11140000, 0x01150000);

  // Check that 0x10140000 is still working properly

  ExpectMapped(0x10140000, 0x00150000);

  AddHostSizedMapping(0x10140000, 0x00140000, 0);
  ExpectMapped(0x10140000, 0x00140000);

  // Check that 0x11140000 is still working properly

  ExpectMapped(0x11140000, 0x01150000);

  AddHostSizedMapping(0x11140000, 0x01140000, 1);
  ExpectMapped(0x11140000, 0x01140000);
}
...
TEST_F(PageTableHostMappingTest, ChangeAddress)
{
  ...
  AddHostSizedMapping(0x10180000, 0x00190000, 0);
  ExpectMapped(0x10180000, 0x00190000);

  // Change logical address
  AddHostSizedMapping(0x11180000, 0x00190000, 0);
  ExpectMapped(0x11180000, 0x00190000);
  ExpectNotMapped(0x10180000);

  // Change both logical address and physical address
  AddHostSizedMapping(0x10180000, 0x01190000, 0);
  ExpectMapped(0x10180000, 0x01190000);
  ExpectNotMapped(0x11180000);
}

TEST_F(PageTableHostMappingTest, InvalidPhysicalAddress)
{
  AddHostSizedMapping(0x101a0000, 0x0ff00000, 0);
  ExpectNotMapped(0x101a0000);
}
...
{
  {
    ...
    for (u32 i = 0; i < s_minimum_mapping_size; i += PowerPC::HW_PAGE_SIZE)
    {
      auto [pte1, pte2] = CreateMapping(0x201b0000 + i, 0x001b0000 + i);
      pte1.VSID = 0xabc;
      SetPTE(pte1, pte2, 0x201b0000 + i, 0);
    }
  }
  ExpectNotMapped(0x201b0000);

  SetSR(2, 0xabc);
  ExpectMapped(0x201b0000, 0x001b0000);
  ExpectNotMapped(0x301b0000);

  SetSR(3, 0xabc);
  ExpectMapped(0x201b0000, 0x001b0000);
  ExpectMapped(0x301b0000, 0x001b0000);
  ExpectNotMapped(0x101b0000);
}

TEST_F(PageTableHostMappingTest, ResizePageTable)
{
  AddHostSizedMapping(0x101c0000, 0x001c0000, 0);
  AddHostSizedMapping(0x105c0000, 0x001d0000, 1);
  ExpectMapped(0x101c0000, 0x001c0000);
  ExpectMapped(0x105c0000, 0x001d0000);

  SetSDR(ALIGNED_PAGE_TABLE_BASE, ALIGNED_PAGE_TABLE_MASK_LARGE);
  ExpectMapped(0x101c0000, 0x001c0000);
  ExpectNotMapped(0x105c0000);

  AddHostSizedMapping(0x105c0000, 0x001e0000, 1);
  ExpectMapped(0x101c0000, 0x001c0000);
  ExpectMapped(0x105c0000, 0x001e0000);

  AddHostSizedMapping(0x105d0000, 0x001c0000, 1);
  ExpectMapped(0x105d0000, 0x001c0000);

  SetSDR(ALIGNED_PAGE_TABLE_BASE, ALIGNED_PAGE_TABLE_MASK_SMALL);
  ExpectMapped(0x101c0000, 0x001c0000);
  ExpectMapped(0x105c0000, 0x001d0000);
  ExpectNotMapped(0x105d0000);
}
...
TEST_F(PageTableHostMappingTest, MisalignedPageTable)
{
  ...
  AddHostSizedMapping(0x109f0000, 0x001f0000, 4);
  ExpectMapped(0x109f0000, 0x001f0000);

  SetSDR(MISALIGNED_PAGE_TABLE_BASE, MISALIGNED_PAGE_TABLE_MASK);

  ExpectNotMapped(0x109f0000);

  AddHostSizedMapping(0x101f0000, 0x00200000, 0);
  AddHostSizedMapping(0x105f0000, 0x00210000, 1);
  AddHostSizedMapping(0x109f0000, 0x00220000, 2);
  AddHostSizedMapping(0x10df0000, 0x00230000, 3);

  ExpectMapped(0x101f0000, 0x00200000);
  ExpectMapped(0x105f0000, 0x00210000);
  ExpectMapped(0x109f0000, 0x00220000);
  ExpectMapped(0x10df0000, 0x00230000);
...
TEST_F(PageTableHostMappingTest, HoleInMask)
{
  ...
  AddHostSizedMapping(0x10640000, 0x00280000, 4);
  ExpectMapped(0x10640000, 0x00280000);

  SetSDR(HOLE_MASK_PAGE_TABLE_BASE, HOLE_MASK_PAGE_TABLE_MASK);

  ExpectNotMapped(0x10640000);

  AddHostSizedMapping(0x10240000, 0x00240000, 0);
  AddHostSizedMapping(0x10640000, 0x00250000, 1);
  AddHostSizedMapping(0x10a40000, 0x00260000, 2);
  AddHostSizedMapping(0x10e40000, 0x00270000, 3);

  ExpectMapped(0x10240000, 0x00240000);
  ExpectMapped(0x10640000, 0x00250000);
  ExpectMapped(0x10a40000, 0x00260000);
  ExpectMapped(0x10e40000, 0x00270000);
