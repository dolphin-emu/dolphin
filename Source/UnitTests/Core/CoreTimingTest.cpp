...
void AdvanceAndCheck(u32 idx, u64 downcount, int cpu_downcount = 0)
{
...
TEST(CoreTiming, PredictableLateness)
{
  ...
  s_lateness = 10;
  AdvanceAndCheck(0, 90, -10);  // (100 - 10)
  s_lateness = 50;
  AdvanceAndCheck(1, MAX_SLICE_LENGTH, -50);
}
...
TEST(CoreTiming, ScheduleIntoPast)
{
  ...
  static auto ChainCallback = [](u64 userdata, s64 lateness) {
  ...
  s_lateness = 1000;
  AdvanceAndCheck(0, MAX_SLICE_LENGTH);  // Run cb_chain into late cb_a
  ...
  AdvanceAndCheck(1, MAX_SLICE_LENGTH);
  ...
  s_lateness = 1000;
  AdvanceAndCheck(0, MAX_SLICE_LENGTH);
}
...
TEST(CoreTiming, Overclocking)
{
  ...
  // Clear s_is_global_timer_sane. Update s_last_OC_factor.
  CoreTiming::Advance();
  ...
  AdvanceAndCheck(4, 2 * MAX_SLICE_LENGTH);
