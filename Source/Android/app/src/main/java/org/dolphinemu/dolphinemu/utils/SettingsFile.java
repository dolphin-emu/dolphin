...
public final class SettingsFile
{
	...
	private static Setting settingFromLine(SettingSection current, String line, String fileName)
	{
		...
		try
		{
			...
			return new IntSetting(key, current.getName(), fileName, valueAsInt);
		}
		...
		try
		{
			...
			return new FloatSetting(key, current.getName(), fileName, valueAsFloat);
		}
		catch (NumberFormatException ex)
		{
		}
		...
		{
			case "True":
				return new BooleanSetting(key, current.getName(), fileName, true);
			case "False":
				return new BooleanSetting(key, current.getName(), fileName, false);
			default:
				return new StringSetting(key, current.getName(), fileName, value);
		}
	}
