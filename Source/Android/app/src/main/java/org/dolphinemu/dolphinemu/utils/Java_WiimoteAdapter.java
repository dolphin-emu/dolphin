...
{
	static int MAX_PAYLOAD = 23;
	static int MAX_WIIMOTES = 4;
	...
	private static void RequestPermission()
	{
		...
		for (Map.Entry<String, UsbDevice> pair : devices.entrySet())
		{
			...
			if (dev.getProductId() == 0x0306 && dev.getVendorId() == 0x057e)
			{
	...
	public static int Output(int index, byte[] buf, int size)
	{
		...
		int LIBUSB_REQUEST_TYPE_CLASS  = (1 << 5);
		int LIBUSB_RECIPIENT_INTERFACE = 0x1;
		int LIBUSB_ENDPOINT_OUT        = 0;

		int HID_SET_REPORT = 0x9;
		int HID_OUTPUT = (2 << 8);
		...
		for (Map.Entry<String, UsbDevice> pair : devices.entrySet())
		{
			...
			if (dev.getProductId() == 0x0306 && dev.getVendorId() == 0x057e)
			{
				if (manager.hasPermission(dev))
				{
					...
					Log.warning("Number of configurations: " + dev.getConfigurationCount());
					Log.warning("Number of Interfaces: " + dev.getInterfaceCount());
					Log.warning("Number of Interfaces from conf: " + conf.getInterfaceCount());
					...
					if (dev.getInterfaceCount() > 0)
					{
						for (int i = 0; i < MAX_WIIMOTES; ++i)
						{
							...
							Log.warning("Interface " + i + " endpoint count:" + usb_intf[i].getEndpointCount());
						}
						return true;
					}
					else
