...
object ContentHandler {
    ...
    fun doFileSearch(
        ...
        try {
            ...
            fun addChildren(path: String, parentDocumentId: String) {
                forEachChild(uri, parentDocumentId) { displayName, childDocumentId, isDirectory ->
                    val childPath = "$path/$displayName"
                    if (acceptAll || (!isDirectory && extensionCheck(displayName))) {
                        result.add(childPath)
                    }
                    if (recursive && isDirectory) {
                        addChildren(childPath, childDocumentId)
                    }
                }
            }

            addChildren(directory, documentId)
        } catch (_: Exception) {
        }
    ...
    private fun forEachChild(
        uri: Uri,
        documentId: String,
        callback: (displayName: String, documentId: String, isDirectory: Boolean) -> Unit
    ) {
        try {
