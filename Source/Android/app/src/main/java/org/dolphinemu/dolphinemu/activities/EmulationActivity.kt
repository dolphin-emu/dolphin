...
class EmulationActivity : AppCompatActivity(), ThemeProvider {
    ...
    fun showGbaControlsMenu(anchor: View) {
        ...
        popup.menu.findItem(R.id.menu_emulation_gba_touch_interaction)?.apply {
            isVisible = !gba.isGbaSnapped
            isChecked = gba.isGbaTouchEnabled
        }
        popup.menu.findItem(R.id.menu_emulation_gba_reset)?.isVisible = !gba.isGbaSnapped
        popup.menu.findItem(R.id.menu_emulation_gba_linear_filtering)?.isChecked = gba.isLinearFiltering
        popup.setOnMenuItemClickListener { item: MenuItem -> onOptionsItemSelected(item) }
        popup.show()
    }
    ...
    private fun handleCheckableMenuAction(menuAction: Int, item: MenuItem) {
        when (menuAction) {
            ...
            MENU_ACTION_GBA_TOUCH_INTERACTION -> {
                item.isChecked = !item.isChecked
                gba.toggleGbaTouch()
            }

            MENU_ACTION_GBA_LINEAR_FILTERING -> {
                item.isChecked = !item.isChecked
                gba.toggleLinearFiltering()
            }
        }
    }
    ...
    fun handleMenuAction(menuAction: Int) {
        when (menuAction) {
            ...
            MENU_ACTION_GBA_TOUCH_INTERACTION -> gba.toggleGbaTouch()
            MENU_ACTION_GBA_LINEAR_FILTERING -> gba.toggleLinearFiltering()
        }
    }
    ...
    companion object {
        ...
        const val MENU_ACTION_GBA_TOUCH_INTERACTION = 45
        const val MENU_ACTION_GBA_LINEAR_FILTERING = 46
        ...
        init {
            buttonsActionsMap.apply {
                ...
                append(R.id.menu_emulation_gba_touch_interaction, MENU_ACTION_GBA_TOUCH_INTERACTION)
                append(R.id.menu_emulation_gba_linear_filtering, MENU_ACTION_GBA_LINEAR_FILTERING)
            }
        }
