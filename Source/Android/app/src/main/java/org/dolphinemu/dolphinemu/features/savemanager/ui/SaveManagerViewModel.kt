...
class SaveManagerViewModel : ViewModel() {
    ...
    fun refreshAvailableFiles() {
        ...
        // Add priority folders (ensure they exist so they can be targeted for copies)
    ...
    suspend fun importSave(srcPath: String): Boolean = withContext(Dispatchers.IO) {
        val result = SaveManagerNative.importWiiSaveFile(srcPath)
        if (result == 0) {
            loadSaves()
            true
        } else {
