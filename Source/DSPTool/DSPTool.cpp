...
// This test goes from text ASM to binary to text ASM and once again back to
// binary.
...
#define CHK(a)                                                                 \
  ...
  // be roundtripped. We don't expect it to always succeed but it'll be sure to
  // generate
  ...
  */
  ...
  if
  (File::ReadFileToString("C:/devkitPro/examples/wii/asndlib/dsptest/dsp_test.ds",
  &dsp_test))
    SuperTrip(dsp_test.c_str());

  //.File::ReadFileToString("C:/devkitPro/trunk/libogc/libasnd/dsp_mixer/dsp_mixer.s",
  &dsp_test);
  // This is CLOSE to working. Sorry about the local path btw. This is
  preliminary code.
*/
...
int main(int argc, const char *argv[]) {
  if (argc == 1 ||
    ...
    printf("USAGE: DSPTool [-?] [--help] [-f] [-d] [-m] [-p <FILE>] [-o "
           "<FILE>] [-h <FILE>] <DSP "
           "ASSEMBLER FILE>\n");
    ...
    printf("-ps <DUMP FILE>: Print results of DSPSpy register dump (disable SR "
           "output)\n");
    printf("-pm <DUMP FILE>: Print results of DSPSpy register dump (convert "
           "PROD values)\n");
    printf("-psm <DUMP FILE>: Print results of DSPSpy register dump (convert "
           "PROD values/disable "
           "SR output)\n");
  ...
  bool disassemble = false, compare = false, multiple = false,
       outputSize = false, force = false, print_results = false,
       print_results_prodhack = false, print_results_srhack = false;
  ...
  if (multiple &&
      (compare || disassemble || !output_name.empty() || input_name.empty())) {
        ...
        if ((print_results_prodhack) && (reg >= 0x15) && (reg <= 0x17)) {
          ...
          case 0x15: // DSP_REG_PRODM
            last_reg = reg_vector.at((step * 32 - 32) + reg) +
                       reg_vector.at((step * 32 - 32) + reg + 2);
            current_reg = reg_vector.at(step * 32 + reg) +
                          reg_vector.at(step * 32 + reg + 2);
            break;
          case 0x16: // DSP_REG_PRODH
            htemp = ((reg_vector.at(step * 32 + reg - 1) +
                      reg_vector.at(step * 32 + reg + 1)) &
                     ~0xffff) >>
                    16;
